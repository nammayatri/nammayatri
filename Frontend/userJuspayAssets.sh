...
find android-native/app/src/user/res/drawable | grep ".png" | cut -d "/" -f 7 | sed 's/.png//' | awk '{print "\"" $1 "\" : true," }' >> assests.json
find android-native/app/src/user/res/drawable | grep ".xml" | cut -d "/" -f 7 | sed 's/.xml//' | awk '{print "\"" $1 "\" : true," }' >> assests.json
...
cat juspay_assets_fonts.json | json_pp | tee ./android-native/app/src/user/assets/juspay/juspay_assets.json
