...
module Screens.RegistrationScreenV2.Controller where
...
import Helpers.Utils (contactSupportNumber, getLatestAndroidVersion, getHvErrorMsg)
...
eval (CallHV workFLowId inputJson) state = 
  continueWithCmd state {props {dontAllowHvRelaunch = true}}
    [ do
        ...
        push <- getPushFn Mb.Nothing "RegistrationScreenV2"
        if EHC.os == "ANDROID" 
          then do
    ...
    [ do
        void $ launchAff $ EHC.flowRunner defaultGlobalState
          $ do
              ...
              push <- liftFlow $ getPushFn Mb.Nothing "RegistrationScreenV2"
...
eval (InAppKeyboardModalAction (InAppKeyboardModal.OnClickBack text)) state = do
  let
    ...
    focusIndex = DS.length referralCode
  continue state { props = state.props { enterOtpFocusIndex = focusIndex, isValidReferralCode = true }, data {referralCode = referralCode, refereeName = Mb.Nothing} }
...
eval (InAppKeyboardModalAction (InAppKeyboardModal.PrimaryButtonAction PrimaryButtonController.OnClick)) state = continueWithCmd state [pure $ InAppKeyboardModalAction (InAppKeyboardModal.OnClickDone state.data.referralCode)]
