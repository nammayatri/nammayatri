...
data ScreenOutput
  = GoToEnterMobileScreen ChooseLanguageScreenState
...
eval (MenuButtonAction (MenuButton.OnSelection btnState)) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_driver_choose_language"
...
eval (PrimaryButtonActionController (PrimaryButton.OnClick)) state = do
  let
    _ = unsafePerformEffect $ logEventWithParams state.data.logField "ny_driver_choose_language_confirmation" "Language" state.props.selectedLanguage
