...
import Data.Maybe (isJust, fromMaybe, Maybe(..), isNothing)
...
screen initialState =
  ...
  , globalEvents:
      [ ( \push -> do
            ...
            when (isNothing initialState.data.carouselItem) $ void $ launchAff $ flowRunner defaultGlobalState $ computeListItem push initialState
            pure $ pure unit
        )
      ]
  , eval:
...
driverReferralCode push state item =
  ...
  in
    linearLayout
      ...
      [ CarouselHolder.carouselView push $ getCarouselConfig item state
      , linearLayout
          [ height WRAP_CONTENT
          , width MATCH_PARENT
          , gravity CENTER
...
getCarouselConfig ∷ forall a. ListItem → BenefitsScreenState → CarouselHolder.CarouselHolderConfig CarouselProps Action
    ...
    [ linearLayout
        ...
        , textView
            $ [ width MATCH_PARENT
              ...
              , onClick push $ const $ ShowReferedInfo NO_REFERRAL_POPUP 1
              , margin $ MarginVertical 10 7
              , color Color.blue800
              ]
...
statusPillView push state status pillMargin =
  ...
  where
    ...
    getPropertyAccordingToStatus = case status of
      "COMPLETED" -> {text : getString COMPLETED_STR, textColor : Color.white900, fontStyle : FontStyle.body19 LanguageStyle, cornerRadius : 8.0, shouldImageBeVisible : true, pillBackgroundColor : Color.green900, pillImage : "ny_ic_white_tick"} 
      "PENDING" -> {text : getString PENDING_STR_C, textColor : Color.white900, fontStyle : FontStyle.body19 LanguageStyle,  cornerRadius : 8.0, shouldImageBeVisible : false, pillBackgroundColor : Color.orange900, pillImage : ""}
      "NEW" -> {text : getString NEW_C, textColor : Color.white900, fontStyle : FontStyle.body19 LanguageStyle, cornerRadius : 8.0, shouldImageBeVisible : false, pillBackgroundColor : state.data.config.benefitsScreen.learnAndEarnItem.statusBackground, pillImage : ""}
      _ -> {text : "", textColor : Color.white900, fontStyle : FontStyle.body19 LanguageStyle, shouldImageBeVisible : false,  cornerRadius : 8.0, pillBackgroundColor : Color.white900, pillImage : ""}


computeListItem :: (Action -> Effect Unit) -> BenefitsScreenState -> Flow GlobalState Unit
computeListItem push state = do
