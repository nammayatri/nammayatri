...
data ScreenOutput
  = GoBack
  | GoToRegisterationScreen OnBoardingSubscriptionScreenState
  | StartFreeTrialExit OnBoardingSubscriptionScreenState
...
eval BackPressed state =
  if state.props.supportPopup then
    exit GoBack
  else
    continue state { props { supportPopup = not state.props.supportPopup } }
...
eval (LoadPlans plans) state = do
  let
    (UiPlansResp planResp) = plans

    config = state.data.subscriptionConfig
  _ <- pure $ setValueToLocalStore DRIVER_SUBSCRIBED "false"
  let
    planList = planListTransformer plans false config.gradientConfig
