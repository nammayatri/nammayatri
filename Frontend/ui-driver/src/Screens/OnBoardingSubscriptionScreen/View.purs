...
screen initialState =
  ...
  , eval:
      ( \state action -> do
          let
            _ = spy "OnBoarding SubscriptionScreen ----- state" state
          let
            _ = spy "OnBoarding SubscriptionScreen --------action" action
          eval state action
      )
  }
...
view push state =
  relativeLayout
        ...
        [ linearLayout
            ...
            , linearLayout
                ...
                [ linearLayout
                    ...
                    , let
                        date = getDateAfterNDays (fromMaybe 0 (fromString (getValueToLocalNativeStore FREE_TRIAL_DAYS)) - 1)
                      in
                        commonTV push
            ...
            , let
                selectedPlan = fromMaybe dummyPlanConfig state.data.selectedPlanItem
              in
                linearLayout
                  [ height WRAP_CONTENT
                  , width MATCH_PARENT
    ...
    , if state.props.supportPopup then
        PrestoAnim.animationSet [ Anim.fadeIn state.props.supportPopup ] $ PopUpModal.view (push <<< PopUpModalAC) (popupModalConfig state)
      else
  ...
  linearLayout
    ...
    , linearLayout
        ...
        , let
            ...
            font2 = if check then FontStyle.body16 else FontStyle.body3
          in
            linearLayout
              [ weight 1.0
              , height WRAP_CONTENT
