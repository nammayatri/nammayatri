...
infoView push state =
  let freeTrialDuration = fromMaybe 7 state.data.freeTrialDuration
      title = getVarString SEVEN_DAY_FREE_TRIAL_ACTIVATED [show freeTrialDuration]
      desc = getVarString TAKE_UNLIMITED_RIDES_FOR_THE_NEXT_SEVEN_DAYS [show freeTrialDuration]
  in
  linearLayout
  [ height WRAP_CONTENT
  , width MATCH_PARENT
...
workFlowView push state = 
  let freeTrialDuration = fromMaybe 7 state.data.freeTrialDuration
      planStarts = getVarString PLAN_STARTS [show freeTrialDuration]
      freeTrialReminder = getVarString FREE_TRIAL_REMINDER [show (freeTrialDuration - 2)]
  in
  linearLayout
  [ height WRAP_CONTENT
  , width MATCH_PARENT
