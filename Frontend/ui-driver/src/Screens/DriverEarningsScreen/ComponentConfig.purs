...
genericHeaderConfig state = let
  ...
  genericHeaderConfig' = config 
    ...
    , textConfig {
        text = if not state.data.config.coinsConfig.enableYatriCoins then "Earnings" else "Use Coins"
...
earningsPopupConfig state = let
    ...
    ,primaryText {
        text = case state.props.popupType of
                  ...
                  ST.COIN_TO_CASH_FAIL_POPUP -> "Failed to use coins. Please try again later"
    ...
    option1 {
      text = case state.props.popupType of
                ...
                ST.COIN_TO_CASH_FAIL_POPUP -> "Try Again"
    ...
    option2 { 
      visibility = case state.props.popupType of
                      ...
                      _ -> true
      , text = case state.props.popupType of
                  ST.COIN_TO_CASH_POPUP -> "Go Back"
                  ST.COIN_TO_CASH_FAIL_POPUP -> "Go Back"
