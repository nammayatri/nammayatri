...
eval (CoinTransactionResponseAction (CoinTransactionRes resp)) state = do
  let events = map (\(CoinTransactionHistoryItem item) -> {event : getEventName item.eventFunction, timestamp : item.createdAt, coins : item.coins, cash : 0.0}) resp.coinTransactionHistory
