...
eval (ChangeTab subView') state = updateAndExit state{props{callRideHistoryAPI = true}} $ (ChangeDriverEarningsTab subView')
...
eval (PopUpModalAC PopUpModal.OnButton1Click) state = do
  case state.props.popupType of
    ST.COIN_TO_CASH_POPUP -> do
      ...
      let newState = state{props{subView = ST.YATRI_COINS_VIEW, popupType = ST.NO_POPUP, showCoinsRedeemedAnim = ""}}
      updateAndExit newState $ RefreshScreen newState
    ST.COIN_TO_CASH_FAIL_POPUP -> do
      _ <- pure $ clearTimer state.data.timerID
      let newState = state{props{subView = ST.USE_COINS_VIEW, popupType = ST.NO_POPUP, showCoinsRedeemedAnim = ""}}
      updateAndExit newState $ RefreshScreen newState
...
eval (PopUpModalAC PopUpModal.DismissPopup) state = do
   _ <- pure $ clearTimer state.data.timerID
   continue state {props{ popupType = ST.NO_POPUP, showCoinsRedeemedAnim = ""}}
...
eval (BarViewSelected index) state = do
  ...
  let selectedBarData = { fromDate : case mbSelectedBarData of
    ...
    totalEarnings : case mbSelectedBarData of
                      Just mbSelectedBarData -> fromMaybe 0 mbSelectedBarData.earnings
                      Nothing -> 0,
...
eval (RideHistoryAPIResponseAction rideList) state = do
  ...
  continue $ state {data{earningHistoryItems = earningHistoryItemList}, props{showShimmer = false, callRideHistoryAPI = false}}
...
getEarningForDate earningLst date = 
  case DA.find (\e -> e.rideDate == date) earningLst of
    Nothing -> Just {earnings : Just 0, rideDistance : 0, rideDate : date, noOfRides : 0, percentLength : 0.0}
    _ -> Nothing
...
getAllWeeksData cachedEarnings earningList dates = do
  let objList = map (\x -> {earnings : Just 0, rideDistance : 0, rideDate : x, noOfRides : 0, percentLength : 0.0}) dates
...
updateToState state = do
  let currentWeekData = getWeeklyEarningsPercentage (getEarningDataOfWeek 3 state.data.weeklyEarningData)
      ...
      noOfDaysToNearestSunday = 6 - case findIndex (DS.take 3 dayName) state.props.weekDay of
                                      Just index -> index
                                      Nothing -> 0
  state{props{weekIndex = 3, selectedBarIndex = -1, currWeekData = currentWeekData, totalEarningsData = getTotalCurrentWeekData currentWeekData, currentWeekMaxEarning = currWeekMaxEarning, callRideHistoryAPI = true}}