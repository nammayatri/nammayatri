...
instance loggableAction :: Loggable Action where
  performLog action appId = case action of
    ...
    BackPressed flag -> do
      ...
      if flag then
        trackAppScreenEvent appId (getScreen ADD_VEHICLE_DETAILS_SCREEN) "in_screen" "backpress_in_open_rc_manual"
      else
...
data ScreenOutput
  = ValidateDetails AddVehicleDetailsScreenState
...
data Action
  = WhatsAppSupport
  | BackPressed Boolean
  | PrimarySelectItemAction PrimarySelectItem.Action
  | NoAction
...
eval (BackPressed flag) state = do
  ...
  if (state.props.validateProfilePicturePopUp) then do
    if (state.props.fileCameraOption) then do
      ...
      continueWithCmd state { props { validateProfilePicturePopUp = false, fileCameraPopupModal = false, fileCameraOption = false, imageCaptureLayoutView = false } }
        [ do
  ...
  else if (state.props.openRCManual) then
    continue state { props { openRCManual = false } }
  else
    exit $ GoBack state
...
eval ScreenClick state = do
  let
    newState = state { props = state.props { openSelectVehicleTypeModal = false } }
  continue newState

eval RemoveUploadedFile state = do
  let
    newState = state { props = state.props { rcAvailable = false, rc_name = "", isValidState = false }, data = state.data { rc_base64 = "" } }
  continue newState

eval (VehicleRegistrationNumber val) state = do
  let
    newState = state { data = state.data { vehicle_registration_number = toUpper val }, props = state.props { isValidState = (checkRegNum (toUpper val) && state.props.rcAvailable) } }
  continue newState

eval (ReEnterVehicleRegistrationNumber val) state = do
  let
    newState = state { data = state.data { reEnterVehicleRegistrationNumber = toUpper val }, props = state.props { isValidState = (checkRegNum (toUpper val) && state.props.rcAvailable) } }
  continue newState

eval (VehicleModelName val) state = do
  _ <- pure $ disableActionEditText (getNewIDWithTag "VehicleModelName")
  let
    newState = state { data = state.data { vehicle_model_name = val } }
  continue newState

eval (VehicleColour val) state = do
  _ <- pure $ disableActionEditText (getNewIDWithTag "VehicleColour")
  let
    newState = state { data = state.data { vehicle_color = val } }
  continue newState
...
eval (CallBackImageUpload base_64 imageName imagePath) state = do
  ...
  if base_64 /= "" then do
    let
      newState = state { props = state.props { validateProfilePicturePopUp = true, imageCaptureLayoutView = false, rcAvailable = true, rc_name = imageName, isValidState = (checkRegNum (state.data.vehicle_registration_number)) }, data = state.data { rc_base64 = base_64 } }
    continue newState
  else
    continue state { props { isValidState = false } }

eval (UploadFile) state =
...
eval (VehicleRCNumber val) state = do
  ...
  let
    newState = state { data = state.data { vehicle_rc_number = val } }
  continue newState
...
eval (SelectVehicleTypeModalAction (SelectVehicleTypeModal.OnSelect item)) state =
  let
    newState =
      state
        { props =
...
eval (ReferralMobileNumberAction (ReferralMobileNumberController.PrimaryEditTextActionController (PrimaryEditTextController.TextChanged valId newVal))) state = do
  let
    var = if ((charAt 0 newVal) == Just '0' || (charAt 0 newVal) == Just '1') then true else false
  ...
  continue
    state
      { props =
        state.props
          { btnActive = if (length newVal == 10 && not var) then true else false
          , isValid = var
...
eval (PrimaryButtonAction (PrimaryButtonController.OnClick)) state = do
  _ <- pure $ hideKeyboardOnNavigation true
  if isJust state.data.dateOfRegistration then
    exit $ ValidateDataAPICall state
  else if (state.props.openHowToUploadManual == false) then
    continue state { props { openHowToUploadManual = true } }
  else
    continueWithCmd state { props { fileCameraPopupModal = false, fileCameraOption = false } }
      [ do
          _ <- liftEffect $ uploadFile false
          pure NoAction
      ]
...
eval (DatePicker resp year month date) state = do
  case resp of
    "SELECTED" ->
      continue
        state
          { data = state.data { dateOfRegistration = Just $ (dateFormat year) <> "-" <> (dateFormat (month + 1)) <> "-" <> (dateFormat date) <> " 00:00:00.233691+00", dateOfRegistrationView = (show date) <> "/" <> (show (month + 1)) <> "/" <> (show year), rcImageID = "null" }
          , props { isDateClickable = true }
          } -- rcImageID made null to handle fallback
...
eval (ValidateDocumentModalAction (ValidateDocumentModal.PrimaryButtonActionController (PrimaryButtonController.OnClick))) state = do
  if (not state.props.errorVisibility) then do
    ...
    continueWithCmd state { props { validateProfilePicturePopUp = false, errorVisibility = false, fileCameraPopupModal = false, fileCameraOption = false }, data { errorMessage = "" } }
      [ do
          _ <- liftEffect $ uploadFile false
          pure NoAction
      ]
...
validateRegistrationNumber regNum =
  let
    vehicleConfig = (getAppConfig appConfig).vehicle
