...
import Screens.Types (AddVehicleDetailsScreenState, VehicalTypes(..), StageStatus(..),AmbulanceVariant(..))
...
instance loggableAction :: Loggable Action where
  performLog action appId = case action of
    ...
    FacilityExpandAnimation -> trackAppScreenEvent appId (getScreen TRIP_DETAILS_SCREEN) "in_screen" "list_expand_animation_end"
    _ -> pure unit
...
data Action =   WhatsAppSupport | BackPressed Boolean | PrimarySelectItemAction PrimarySelectItem.Action | NoAction
  ...
  | FacilityExpandAnimation
  | SelectAmbulanceVariant AmbulanceType
...
eval AgreeButtonAction state = do
...
eval (AgreePopUp (PopUpModal.OnButton1Click)) state = continueWithCmd state{props{ agreeTermsModal = false}} [pure $ AgreeButtonAction]
...
eval (SelectAmbulanceVariant variant) state = do
  let {airConditioned , ventilator , oxygen} = case variant of
  ...
  continue state{data{airConditioned =  airConditioned  , ventilator = ventilator , oxygen = oxygen} , props{isvariant = show variant , facilities = not state.props.facilities , showIssueOptions = true}}
...
eval FacilityExpandAnimation state = continue state {props {showIssueOptions = false }}
