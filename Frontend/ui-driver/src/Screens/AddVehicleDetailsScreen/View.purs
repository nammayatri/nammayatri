...
screen initialState =
  ...
  , eval:
      ( \state action -> do
          let
            _ = spy "AddVehicleDetailsScreen ----- state" state
          let
            _ = spy "AddVehicleDetailsScreen --------action" action
          eval state action
      )
  }
        ...
        ( [ linearLayout
          ...
          , if state.props.openRCManual then
              linearLayout
                ...
                [ TutorialModal.view (push <<< TutorialModalAction) { imageUrl: fetchImage FF_ASSET "ny_ic_vehicle_registration_card" } ]
            else
              linearLayout [] []
          , if state.props.openRegistrationDateManual then
              linearLayout
                ...
                [ TutorialModal.view (push <<< TutorialModalAction) { imageUrl: fetchImage FF_ASSET "ny_ic_date_of_registration" } ]
            else
              linearLayout [] []
          , if state.props.limitExceedModal then
              linearLayout
                ...
                [ GenericMessageModal.view (push <<< GenericMessageModalAction) { text: (getString ISSUE_WITH_RC_IMAGE), openGenericMessageModal: state.props.limitExceedModal, buttonText: (getString NEXT) } ]
            else
              linearLayout [] []
          , if state.props.openReferralMobileNumber then
              linearLayout
                ...
                [ ReferralMobileNumber.view (push <<< ReferralMobileNumberAction) (referalNumberConfig state) ]
            else
              linearLayout [] []
          ]
            <> if state.props.logoutModalView then
                ...
                []
                  <> if state.props.imageCaptureLayoutView then
                      [ imageCaptureLayout push state ]
                    else
                      []
                        <> if state.props.validateProfilePicturePopUp then
                            [ validateProfilePicturePopUp push state ]
                          else
                            []
                              <> if state.props.fileCameraPopupModal then
...
vehicleRegistrationNumber state push =
  linearLayout
    ...
    [ linearLayout
        ...
        , textView
            $
             -- (Error Indication) [ width WRAP_CONTENT
              , height WRAP_CONTENT
...
validateProfilePicturePopUp push state = ValidateDocumentModal.view (push <<< ValidateDocumentModalAction) (validateProfilePictureModalState state)
  ...
  let
    ...
    inAppModalConfig' =
      config'
        ...
        , verificationStatus =
          if state.props.validating then
            InProgress
          else if state.data.errorMessage /= "" then
            Failure
          else if state.data.rcImageID /= "" then
            Success
          else
            None
        , verificationType = "RC"
...
fileCameraLayout push state = PopUpModal.view (push <<< PopUpModalActions) (fileCameraLayoutConfig state)
...
addRCFromProfileStatusView state push =
  relativeLayout
    ...
    [ linearLayout
        ...
        [ imageView
            [ imageWithFallback
                $ fetchImage FF_ASSET case state.props.multipleRCstatus of
        ...
        , textView
            $ [ text
                  $ getString case state.props.multipleRCstatus of
    ...
    , linearLayout
        ...
        , textView
            $ [ text
                  $ case state.props.multipleRCstatus of
