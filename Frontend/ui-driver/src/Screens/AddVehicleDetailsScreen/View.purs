...
import Services.API (AmbulanceType(..))
          ...
          , visibility $ boolToVisibility $ state.data.vehicleCategory == Just (ST.AmbulanceCategory)
              ...
              , linearLayout
                ...
                , onClick push $ const $ SelectAmbulanceFacility
                  ...
                  , ambulanceFacilityView state push
          ]
...
ambulanceFacilityView :: forall w . ST.AddVehicleDetailsScreenState -> (Action -> Effect Unit) -> PrestoDOM (Effect Unit) w
ambulanceFacilityView state push  = 
  PrestoAnim.animationSet ([] <>
    if EHC.os == "IOS" then
  ...
  linearLayout
    ...
    , stroke ("1," <> Color.borderColorLight)
    , cornerRadius 4.0
    , onAnimationEnd push $ const FacilityExpandAnimation
    ](DA.mapWithIndex (\index variant ->
        linearLayout
        [ height WRAP_CONTENT
        , width MATCH_PARENT
        , padding $ Padding 16 13 16 13
        , onClick push $ const $ SelectAmbulanceVariant variant
        , orientation VERTICAL
        ][  linearLayout
            [ height WRAP_CONTENT
            , width MATCH_PARENT
            , orientation HORIZONTAL
                ...
                , textFromHtml $ HU.getVehicleType $ show variant
