...
primaryButtonConfig state =
  let
    ...
    activate =
      ( (toLower (state.data.vehicle_registration_number) == toLower (state.data.reEnterVehicleRegistrationNumber))
        ...
        { textConfig
          { text =
            if isJust state.data.dateOfRegistration then
              ...
              getString UPLOAD_REGISTRATION_CERTIFICATE
          }
        , width = MATCH_PARENT
        , height = (V 50)
      ...
      AppOnboardingNavBar.config.headerTextConfig
        { text =
          if state.props.openHowToUploadManual then
            getString UPLOAD_REGISTRATION_CERTIFICATE_STR
          else
            getString VEHICLE_REGISTRATION_DETAILS
        }
    }
