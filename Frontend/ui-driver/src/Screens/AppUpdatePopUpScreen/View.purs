...
screen initialState =
  ...
  , eval:
      ( \state action -> do
          let
            _ = spy "AppUpdatePopUpScreen state -----" state
          let
            _ = spy "AppUpdatePopUpScreen--------action" action
          eval state action
      )
  , globalEvents:
      [ ( \push -> do
...
view push state =
  linearLayout
    ...
    [ if (state.updatePopup == AppVersion) then
        updateRequiredView push state
      else if (state.updatePopup == AppUpdated) then
