...
screen initialState =
            ...
            void $ launchAff $ flowRunner defaultGlobalState $ runExceptT $ runBackT
              $ do
                  if initialState.props.subView == PaymentHistory then do
                    ...
                    let
                      tab = if initialState.props.autoPayHistory then "AUTOPAY_INVOICE" else "MANUAL_INVOICE"
  ...
  , eval:
      ( \state action -> do
          let
            _ = spy "PaymentHistoryScreen ----- state" state
          let
            _ = spy "PaymentHistoryScreen --------action" action
          eval state action
      )
  }
...
paymentList push state =
  let
    transactionItems = if state.props.autoPayHistory then state.data.autoPayList else state.data.manualPayList
  in
    PrestoAnim.animationSet [ Anim.fadeIn true ]
      $ if DA.null transactionItems then
            ...
            [ linearLayout
                ...
                [ linearLayout
                    ...
                    ( DA.mapWithIndex
                        ( \index item ->
                            let
                              itemConfig = getStatusConfig item.paymentStatus
                            in
                              linearLayout
                                [ height WRAP_CONTENT
                                , width MATCH_PARENT
  ...
  linearLayout
    ...
    [ imageView
        ...
        , imageWithFallback $ fetchImage FF_ASSET
            $ case state.props.autoPayHistory, state.props.autoPaySetup of
...
transactionDetails push state visibility' =
  let
    config = getTransactionConfig state.data.transactionDetails
          ...
          [ linearLayout
              ...
              , linearLayout
                  ...
                  [ linearLayout
                      ...
                      ( DA.mapWithIndex
                          ( \index item ->
                              transactionHistoryRow push item.title index (DA.length state.data.transactionDetails.details) (item.val /= "")
                                $ case item.key of
...
manualPaymentRidesList push state =
  let
    screenwidth = (screenWidth unit) - 64
  in
    linearLayout
      [ width MATCH_PARENT
      , height WRAP_CONTENT
