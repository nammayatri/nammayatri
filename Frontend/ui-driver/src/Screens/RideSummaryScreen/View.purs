...
view push state  =
  ...
  in
   ...
   ( [
    ...
    ]<>if state.props.showPopUp then [endRidePopView state push]  else [])
...
cancelButtonView  state push = 
  linearLayout [
    ...
    textView $ [
      ...
      ,width WRAP_CONTENT
      , gravity CENTER_HORIZONTAL
      ,textFromHtml $ "<u>" <> "cancelBooking" <> "</u>"
    ]
  ]
...
goToPickUp state push = 
   linearLayout [
        ...
        , cornerRadius  8.0
        , gravity CENTER
        ,  onClick push $ if func state then (const $  GoToMap state.data.activeRideData.dest_lat state.data.activeRideData.dest_lon ) else (const  OnClick)
        , rippleColor Color.rippleShade
        , visibility $ boolToVisibility state.props.throughBanner
    ][
        textView $ [
            width WRAP_CONTENT
            , height $ V 40
            , text $ if func state then " Go to Pickup" else "Done"
...
bannerView  state push  = 
     ...
     , visibility $ boolToVisibility state.props.throughBanner
      ...
      imageView [
       ...
       , width  $ V 400
       , gravity CENTER_HORIZONTAL
       , imageWithFallback $ fetchImage FF_ASSET "ny_ic_ride_scheduled_banner"
      ]
     ]


func :: RideSummaryScreenState -> Boolean
func state =  
   let currentTime = getCurrentUTC "" 
       tripStartTime = fromMaybe "" state.data.activeRideData.tripScheduledAt
  in runFn2 JB.differenceBetweenTwoUTC tripStartTime currentTime <= 7800 
