...
screen initialState =
  ...
  , eval:
      ( \state action -> do
          let
            _ = spy "ChooseCityScreen ----- state" state
          let
            _ = spy "ChooseCityScreen --------action" action
          eval state action
      )
  }
...
view push state =
  Anim.screenAnimation
    $ linearLayout
        ...
        [ if state.props.isMockLocation && state.data.config.enableMockLocation then
...
currentLocationView state push =
  linearLayout
    ...
    , textView
        $ [ text
              $ getString case state.data.locationDetectionFailed, Mb.isNothing state.data.locationSelected, state.props.locationUnserviceable of
...
radioButtonView state push visibility' =
  let
    items = if state.props.currentStage == SELECT_LANG then state.data.config.languageList else transformCityConfig state.data.merchantOperatingCityConfig
              ...
              ( DA.mapWithIndex
                  ( \index language ->
                      PrestoAnim.animationSet
                        ...
                        $ MenuButton.view (push <<< MenuButtonAction) (menuButtonConfig index language selectedVal)
                  )
                  items
              )
          ]
