...
eval AfterRender state =
  if state.props.currentStage == ENABLE_PERMISSION then
...
eval BackPressed state = do
  case state.props.currentStage of
    _
      | state.props.currentStage == DETECT_LOCATION || state.props.currentStage == ENABLE_PERMISSION -> do
        _ <- pure $ minimizeApp ""
        continue state --minimizeApp
    _
      | state.props.currentStage == SELECT_CITY || state.props.currentStage == SELECT_LANG -> continue state { props { currentStage = DETECT_LOCATION } }
    _ -> continue state
  ...
  if state.props.currentStage == ENABLE_PERMISSION then do
    ...
    case state.data.locationSelected of
      Just location -> do
        ...
        let
          mbCity = DA.find (\city' -> city'.cityName == location) state.data.config.cityConfig
        case mbCity of
          Just city -> do
...
eval (LocationPermissionCallBack isLocationPermissionEnabled) state = do
  if isLocationPermissionEnabled then do
    let
      newState = state { props { isLocationPermissionGiven = true, currentStage = DETECT_LOCATION } }
...
eval (UpdatePermission updatedState) state = do
  let
    newState = updatedState { props { currentStage = if updatedState.props.isLocationPermissionGiven then DETECT_LOCATION else updatedState.props.currentStage } }
  continue newState

eval (CurrentLocationCallBack lat long) state = do
  let
    driverLat = fromMaybe 0.0 $ Number.fromString lat
  ...
  if driverLat == 0.0 && driverLon == 0.0 then do
    ...
    let
      distanceFromBangalore = getDistanceBwCordinates (fromMaybe 0.0 $ Number.fromString lat) (fromMaybe 0.0 $ Number.fromString long) 12.9716 77.5946
...
closestCity (Tuple cityName distance) city driverLat driverLon = do
  let
    distanceFromCity = getDistanceBwCordinates driverLat driverLon city.cityLat city.cityLong
