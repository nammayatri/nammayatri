...
data ScreenOutput
  = GoBack
  | SelectCab BookingOptionsScreenState Boolean
...
eval (ChooseVehicleAC (ChooseVehicle.OnSelect config)) state = do
  let
    updatedConfig = map (\item -> item { isSelected = if item.vehicleVariant == config.vehicleVariant then (not item.isSelected) else item.isSelected }) state.data.downgradeOptions
...
eval DowngradeVehicle state = do
  let
    downgraded = not state.props.downgraded
