...
screen initialState =
  ...
  , globalEvents: 
        [( \push -> if not $ initialState.data.config.rateCardScreen.showRateCard then pure $ pure unit else if initialState.data.vehicleType `DA.elem` ["AMBULANCE_TAXI", "AMBULANCE_TAXI_OXY", "AMBULANCE_AC", "AMBULANCE_AC_OXY", "AMBULANCE_VENTILATOR"] then pure $ pure unit else do
            _ <-
...
acCheckForDriversView push state =
  let
    acCheckVisibility = MP.boolToVisibility $ MB.isJust state.data.airConditioned && not (state.data.vehicleType `DA.elem` ["AMBULANCE_TAXI", "AMBULANCE_TAXI_OXY", "AMBULANCE_AC", "AMBULANCE_AC_OXY", "AMBULANCE_VENTILATOR"])
...
rateCardView push state =
  PrestoAnim.animationSet [ Anim.fadeIn true ]
    $ linearLayout
        ...
        [ RateCard.view (push <<< RateCardAction) (rateCardConfig state.data.rateCard state.data.config.rateCardScreen.showTollCharges) ]
