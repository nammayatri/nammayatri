    ...
    linearLayout
      ...
      , linearLayout
        ...
        $ if state.data.vehicleType /= "BIKE" then 
          ...
          , petRidesPreferenceView push state totalPreferencesCount
          ] else []
      , linearLayout
          [ width MATCH_PARENT
          , height WRAP_CONTENT
...
petRidesPreferenceView :: forall w. (Action -> Effect Unit) -> ST.BookingOptionsScreenState -> Int -> PrestoDOM (Effect Unit) w
petRidesPreferenceView push state totalPreferencesCount =
  ...
  linearLayout
    ...
    ][serviceTierItemHorizontal state push item true false (-1) totalPreferencesCount true]
  where
    item :: ST.RidePreference
    item = defaultRidePreferenceOption {name = "Pet Rides", isSelected = fromMaybe false state.props.isPetModeEnabled}
