...
acCheckForDriversView push state =
  let
    ...
    backgroundColor = if airConditionedData.isWorking then state.data.config.primaryToggleBackground else Color.black600
...
toggleView push state enabled default service =
  let
    backgroundColor = if enabled && not service.isUsageRestricted then state.data.config.primaryToggleBackground else Color.black600
  ...
  in
    linearLayout
      ...
      , linearLayout
          ...
          , height WRAP_CONTENT
          , orientation VERTICAL
          , cornerRadius if isBridgeApp then 15.0 else 6.0
          , background if isBridgeApp then Colors.purple500 else Color.golden
...
rateCardBannerView push state = 
  ...
  where peakTime = state.props.peakTime
        bgColor = if peakTime then Color.green900 else state.data.config.primaryToggleBackground
