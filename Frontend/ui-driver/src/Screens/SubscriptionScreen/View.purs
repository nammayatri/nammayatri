  ...
  , padding $ Padding 20 20 20 if length state.data.joinPlanData.allPlans == 1 then 16 else 0
      ...
      , orientation VERTICAL
        ...
        , linearLayout
          ...
          , gravity CENTER_VERTICAL
          , onClick (\action -> do
                        let url = if state.data.config.subscriptionConfig.myPlanYoutubeLink == "" then state.data.config.subscriptionConfig.faqLink else state.data.config.subscriptionConfig.myPlanYoutubeLink
                        _ <- push action
      ...
      , margin $ Margin 16 0 16 16 
        ...
        , imageView
          ...
          , onClick (\action -> do
                      ...
                      _ <- JB.openUrlInApp state.data.config.subscriptionConfig.myPlanYoutubeLink
                      pure unit
                      ) (const NoAction)
          ]
          ]
