...
paymentPendingView push state = let isAutoPayPending = state.props.lastPaymentType == Just "AUTOPAY_REGISTRATION"
  ...
  , visibility if state.data.orderId /= Nothing then VISIBLE else GONE
    ...
    , textView
      ...
      , fontStyle $ FontStyle.medium LanguageStyle
      , color Color.red
      , visibility if isAutoPayPending then VISIBLE else GONE
      ]
    , linearLayout
      [ width MATCH_PARENT
      , height WRAP_CONTENT
...
duesView push state = 
  ...
  , visibility VISIBLE
   ...
   , if (isNothing state.data.orderId && (state.data.myPlanData.autoPayStatus /= ACTIVE_AUTOPAY && state.data.myPlanData.manualDueAmount > 0.0)) then arrowButtonView push (getString ONE_TIME_SETTLEMENT) false OneTimeSettlement state.props.isSelectedLangTamil else dummyView -- 
  ]
