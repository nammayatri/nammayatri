...
screen initialState globalState =
  ...
  , globalEvents:
      [ ( \push -> do
            let
              showSubscriptions = getValueToLocalStore SHOW_SUBSCRIPTIONS == "true"
  ...
  , eval:
      ( \state action -> do
          let
            _ = spy "SubscriptionScreen ----- state" state
          let
            _ = spy "SubscriptionScreen --------action" action
          eval state action
      )
  }
...
loadData push loadPlans loadAlternatePlans loadMyPlans loadHelpCentre errorAction state (GlobalState globalState) = do
  if any (_ == state.props.subView) [ JoinPlan, MyPlan, NoSubView ] then do
    let
      globalProp = globalState.globalProps
    ...
    case locations of
      ...
      Left err ->
        if err.code /= 404 then
          doAff do liftEffect $ push $ errorAction err
        else
          doAff do liftEffect $ push $ loadHelpCentre state.props.currentLat state.props.currentLon []
  else
    pure unit
        ...
        , linearLayout
            ...
            ( map
                ...
                ( [ (getString ZERO_COMMISION), (getString EARN_TODAY_PAY_TOMORROW) ]
                    <> if state.props.joinPlanProps.isIntroductory then
                        [ getString GET_FULL_PAYMENT, getString (GUARANTEED_FIXED_PRICE "GUARANTEED_FIXED_PRICE") ]
                      else if not state.data.config.subscriptionConfig.enableSubscriptionPopups then
                        [ (getString SIGNUP_EARLY_FOR_SPECIAL_OFFERS), getString (GUARANTEED_FIXED_PRICE "GUARANTEED_FIXED_PRICE") ]
                      else
                        [ (getString PAY_ONLY_IF_YOU_TAKE_RIDES), getString (GET_SPECIAL_OFFERS "GET_SPECIAL_OFFERS") ]
                )
            )
        ]
    ]
...
paymentPendingView push state =
  let
    isAutoPayPending = state.props.lastPaymentType == Just AUTOPAY_REGISTRATION_TYPE
  in
    linearLayout
      [ height WRAP_CONTENT
      , width MATCH_PARENT
    ...
    [ linearLayout
        ...
        , linearLayout
            ...
            , onClick
                ( \action -> do
                    let
                      url = if state.data.config.subscriptionConfig.myPlanYoutubeLink == "" then state.data.config.subscriptionConfig.faqLink else HU.splitBasedOnLanguage state.data.config.subscriptionConfig.myPlanYoutubeLink
                    _ <- push action
        ...
        , scrollView
            ...
            [ linearLayout
                ...
                ( map
                    ( \item ->
                        let
                          selectedPlan = state.props.joinPlanProps.selectedPlanItem
                        in
...
headerView push state =
  let
    config = getHeaderConfig state.props.subView (state.props.myPlanProps.dueType /= AUTOPAY_PAYMENT) state.props.myPlanProps.multiTypeDues
  in
    linearLayout
      [ height $ V 55
      , width MATCH_PARENT
      ...
      [ imageView
          ...
          , imageWithFallback
              $ if config.backbutton then
                  HU.fetchImage HU.FF_COMMON_ASSET "ny_ic_chevron_left"
                else if state.props.isEndRideModal then
                  HU.fetchImage HU.FF_ASSET "ny_ic_close_bold"
                else
                  ""
          ]
      , textView
...
myPlanBodyview push state =
  let
    isFreezed = ((state.data.config.subscriptionConfig.enableSubscriptionPopups && state.data.orderId /= Nothing) || state.props.lastPaymentType == Just AUTOPAY_REGISTRATION_TYPE)
            ...
            , linearLayout
                ...
                , linearLayout
                    ...
                    [ textView
                        $ [ text $ "₹" <> HU.getFixedTwoDecimals state.data.myPlanData.totalDueAmount
                          , color
                              if state.props.myPlanProps.overDue then
                                Color.red
                              else if state.props.myPlanProps.multiTypeDues then
                                Color.black900
                              else
                                Color.blue800
                          , weight 1.0
            ...
            , linearLayout
                ...
                [ linearLayout
                    ...
                    [ textView
                        ...
                        <> FontStyle.tags TypoGraphy
                    , commonImageView
                        ( if state.props.myPlanProps.multiTypeDues then
                            "ny_ic_chevron_right"
                          else if state.props.myPlanProps.isDuesExpanded then
                            "ny_ic_chevron_up,https://assets.juspay.in/beckn/nammayatri/nammayatricommon/images/ny_ic_chevron_up.png"
                          else
                            "ny_ic_chevron_down,https://assets.juspay.in/beckn/nammayatri/nammayatricommon/images/ny_ic_chevron_down.png"
                        )
                        12
                        12
                        (MarginRight 4)
...
paymentMethodView push state =
  let
    statusPillData = getAutoPayStatusPillData state.autoPayStatus
  in
    linearLayout
      [ height WRAP_CONTENT
      , width WRAP_CONTENT
...
planCardView push state isSelected clickable' action isSelectedLangTamil showBanner isMyPlan isActivePlan offerBannerProps isIntroductory offerBannerPlans =
  ...
  let
    dummyOfferConfig = { showOfferBanner: false, offerBannerValidTill: "", offerBannerDeadline: "" }
              ...
              [ linearLayout
                  ...
                  , stroke $ "1,"
                      <> ( if isIntroductory then
                            Color.grey900
                          else if isSelected && isActivePlan then
                            Color.transparent
                          else if isSelected && not isMyPlan then
                            Color.blue800
                          else
                            Color.grey900
                        )
...
dueOverViewCard push state isManual =
  let
    items = filter (\item -> if isManual then item.mode /= AUTOPAY_PAYMENT else item.mode == AUTOPAY_PAYMENT) state.data.myPlanData.dueItems
  in
    linearLayout
      [ height WRAP_CONTENT
      , width MATCH_PARENT
...
tripList push trips isManual isExpanded viewDatailsText useFixedHeight =
  let
    adjustedHeight = if length trips == 1 then 30 else 70
  in
    linearLayout
      [ height WRAP_CONTENT
      , width MATCH_PARENT
...
languageSpecificTranslation str variable = case getValueToLocalStore LANGUAGE_KEY of
  "EN_US" -> str <> " " <> variable
  _ -> variable <> " " <> str

getAutoPayStatusPillData :: AutoPayStatus -> { color :: String, status :: String }
getAutoPayStatusPillData autoPayStatus = case autoPayStatus of
  ...
  _
    | autoPayStatus `elem` [ CANCELLED_PSP, RESUME_PENDING ] -> { color: Color.red, status: getString CANCELLED_ }
  _
    | autoPayStatus `elem` [ PENDING, MANDATE_FAILED ] -> { color: Color.orange900, status: getString PENDING_STR }
...
showOfferApplicable state =
  let
    currentPlanOffers = length $ filter (not _.addedFromUI) state.data.managePlanData.currentPlan.offers

    selectedPlanOffers = length $ filter (not _.addedFromUI) state.props.managePlanProps.selectedPlanItem.offers
  in
    state.props.managePlanProps.selectedPlanItem.id /= state.data.managePlanData.currentPlan.id && selectedPlanOffers < currentPlanOffers
...
offerCardBannerView push useMargin visibility' isPlanCard offerBannerProps isFreezed =
  let
    horizontalMargin = if useMargin then 16 else 0
  in
    linearLayout
      [ height MATCH_PARENT
      , width MATCH_PARENT
...
lottieJsonAccordingToLang isOnFreeTrial isIntroductory =
  (HU.getAssetsBaseUrl FunctionCall)
    <> case getValueToLocalStore LANGUAGE_KEY of
        "HI_IN" ->
          if isIntroductory then
            "lottie/ny_ic_subscription_info_hindi_03.json"
          else if isOnFreeTrial then
            "lottie/ny_ic_subscription_info_hindi_01.json"
          else
            "lottie/ny_ic_subscription_info_hindi_02.json"
        "KN_IN" ->
          if isIntroductory then
            "lottie/ny_ic_subscription_info_kannada_02.json"
          else if isOnFreeTrial then
            "lottie/ny_ic_subscription_info_kannada_01.json"
          else
            "lottie/ny_ic_subscription_info_kannada_02.json"
        "TA_IN" ->
          if isIntroductory then
            "lottie/ny_ic_subscription_info_tamil_02.json"
          else if isOnFreeTrial then
            "lottie/ny_ic_subscription_info_tamil_01.json"
          else
            "lottie/ny_ic_subscription_info_tamil_02.json"
        "BN_IN" ->
          if isIntroductory then
            "lottie/ny_ic_subscription_info_bengali_02.json"
          else if isOnFreeTrial then
            "lottie/ny_ic_subscription_info_bengali_01.json"
          else
            "lottie/ny_ic_subscription_info_bengali_02.json"
        "TE_IN" ->
          if isIntroductory then
            "lottie/ny_ic_subscription_info_telugu_03.json"
          else if isOnFreeTrial then
            "lottie/ny_ic_subscription_info_01.json"
          else
            "lottie/ny_ic_subscription_info_02.json"
        _ ->
          if isIntroductory then
            "lottie/ny_ic_subscription_info_03.json"
          else if isOnFreeTrial then
            "lottie/ny_ic_subscription_info_01.json"
          else
            "lottie/ny_ic_subscription_info_02.json"
