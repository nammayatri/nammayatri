...
import Services.API (ReelsResp, FeeType(..), GetCurrentPlanResp(..), GetDriverInfoResp(..), KioskLocationRes(..), KioskLocationResp(..), OrderStatusRes(..), PaymentBreakUp(..), UiPlansResp(..), DriverInfoReq(..), LastPaymentType(..), CoinEntity(..), SafetyPlusData(..))
...
screen initialState (GlobalState globalState) =
  ...
  , globalEvents: [(\push -> do
      void $ launchAff $ flowRunner defaultGlobalState $ do
        ...
        (APITypes.SubscriptionServiceResp services) <- getServicesEnabledForDriver
      ...
      case currentPlan of
        Right resp -> do
          ...
          case resp'.currentPlanDetails of
            Nothing -> do
              uiPlans <- Remote.getUiPlans "null" (Just state.props.serviceName) --- pass service
              case uiPlans of
                Right plansResp -> do
...
getServicesEnabledForDriver ::  Flow GlobalState APITypes.SubscriptionServiceResp
getServicesEnabledForDriver = do
  (GlobalState state) <- getState
  subscriptionServiceResp <- Remote.listServices ""
  case subscriptionServiceResp of
    Right resp -> pure resp
    Left _ -> pure $ APITypes.SubscriptionServiceResp {services : ["YATRI_SUBSCRIPTION"]}
...
tabView :: forall w. (Action -> Effect Unit) -> ST.SubscriptionScreenState -> PrestoDOM (Effect Unit) w
tabView push state =
    linearLayout
      [ height WRAP_CONTENT
      , cornerRadius 24.0
          ...
          linearLayout[
            ...
            , orientation HORIZONTAL
              ...
              textView $ [
                ...
                , width WRAP_CONTENT
                , color Color.charcoalGrey
                , text $ "₹ " <> maybe "0" (\(SafetyPlusData safetyData) -> show $ (safetyData.safetyPlusEarnings.amount)) state.data.myPlanData.safetyPlusData
                , gravity CENTER
