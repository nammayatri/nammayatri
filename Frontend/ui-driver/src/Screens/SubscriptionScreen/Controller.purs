...
data ScreenOutput
  = HomeScreen SubscriptionScreenState
...
eval BackPressed state =
  if state.props.popUpState == Mb.Just SupportPopup then
    updateAndExit state { props { popUpState = Mb.Nothing } } $ HomeScreen state { props { popUpState = Mb.Nothing } }
  else if (not Mb.isNothing state.props.popUpState && not (state.props.popUpState == Mb.Just SuccessPopup)) then
    continue state { props { popUpState = Mb.Nothing } }
  else if state.props.optionsMenuState /= ALL_COLLAPSED then
    continue state { props { optionsMenuState = ALL_COLLAPSED } }
  else if state.props.confirmCancel then
    continue state { props { confirmCancel = false } }
  else if state.props.subView == ManagePlan then
    continue state { props { subView = MyPlan } }
  else if state.props.subView == DuesView then
    continue state { props { subView = MyPlan } }
  else if state.props.subView == DueDetails then do
    let
      subView' = if state.props.myPlanProps.multiTypeDues then DuesView else MyPlan
    continue state { props { subView = subView' }, data { myPlanData { selectedDue = "" } } }
  else if state.props.subView == PlanDetails then
    continue state { props { subView = if state.data.config.subscriptionConfig.optionsMenuItems.managePlan then ManagePlan else MyPlan } }
  else if state.props.subView == FindHelpCentre then
    continue state { props { subView = state.props.prevSubView, kioskLocation = [], noKioskLocation = false, showError = false } }
  else if state.props.myPlanProps.isDueViewExpanded then
    continue state { props { myPlanProps { isDueViewExpanded = false } } }
  else if state.data.myPlanData.autoPayStatus == MANDATE_FAILED && not state.props.isEndRideModal && not state.data.config.subscriptionConfig.enableIntroductoryView && state.data.config.subscriptionConfig.enableSubscriptionSupportPopup then
    continue state { props { popUpState = Mb.Just SupportPopup } }
  else
...
eval (OptionsMenuAction (OptionsMenu.BackgroundClick)) state = continue state { props { optionsMenuState = ALL_COLLAPSED } }

eval (SwitchPlan PrimaryButton.OnClick) state = do
  let
    planId = state.props.managePlanProps.selectedPlanItem.id
  if planId /= "" then do
    _ <- pure $ cleverTapCustomEvent "ny_driver_switch_plan_clicked"
    _ <- pure $ metaLogEvent "ny_driver_switch_plan_clicked"
    let
      _ = unsafePerformEffect $ firebaseLogEvent "ny_driver_switch_plan_clicked"
...
eval ManagePlanAC state = do
  ...
  let
    _ = unsafePerformEffect $ firebaseLogEvent "ny_driver_manage_plan_clicked"
...
eval (HeaderRightClick menuType) state =
  if state.props.optionsMenuState == menuType then
    continue state { props { optionsMenuState = ALL_COLLAPSED } }
  else
    continue state { props { optionsMenuState = menuType } }
...
eval (ConfirmCancelPopup (PopUpModal.OnButton2Click)) state = do
  ...
  let
    _ = unsafePerformEffect $ firebaseLogEvent "ny_driver_cancel_autopay"
...
eval (ResumeAutoPay PrimaryButton.OnClick) state = case state.data.myPlanData.manualDueAmount > 0.0, state.data.myPlanData.autoPayStatus of
...
eval (RetryPaymentAC PrimaryButton.OnClick) state = case state.props.lastPaymentType of
...
eval (BottomNavBarAction (BottomNavBar.OnNavigate screen)) state = do
  let
    newState = state { props { optionsMenuState = ALL_COLLAPSED, myPlanProps { isDueViewExpanded = false } } }
  if screen == "Join" then
    continue state
...
eval ViewHelpCentre state = do
  let
    prevSubViewState = state.props.subView
...
eval (LoadPlans plans) state = do
  ...
  continue
    state
      { data
        { joinPlanData
          { allPlans = planListTransformer plans state.data.config.subscriptionConfig.enableIntroductoryView state.data.config.subscriptionConfig.gradientConfig
...
eval (LoadHelpCentre lat lon kioskLocationList) state = do
  let
    transformedKioskList = transformKioskLocations kioskLocationList lat lon
...
eval (LoadMyPlans plans) state = do
  let
    (GetCurrentPlanResp currentPlanResp) = plans
  case currentPlanResp.currentPlanDetails of
    Mb.Just (planEntity') -> do
      ...
      let
        ...
        newState =
          if state.data.config.subscriptionConfig.enableSubscriptionPopups then
            state
              ...
              , data
                { orderId = currentPlanResp.orderId
      ...
      _ <-
        pure
          $ setCleverTapUserProp
              [ { key: "Plan", value: unsafeToForeign planEntity.name }
      ...
      case currentPlanResp.mandateDetails of
        ...
        Mb.Just (MandateData mandateDetails) -> do
          ...
          _ <-
            pure
              $ setCleverTapUserProp
                  [ { key: "Mandate status", value: unsafeToForeign mandateDetails.status }
...
eval (TryAgainButtonAC PrimaryButton.OnClick) state =
  let
    updateState = state { props { showShimmer = true } }
  in
    if state.props.subView == FindHelpCentre then
      updateAndExit updateState $ RefreshHelpCentre updateState
    else
      updateAndExit updateState $ Refresh
...
getPlanPrice fares priceType = do
  let
    price = (DA.filter (\(PaymentBreakUp item) -> item.component == priceType) fares)
  case price DA.!! 0 of
...
getAllFareFromArray fares titles = do
  let
    matchingFares = (DA.filter (\(PaymentBreakUp fare) -> DA.elem fare.component titles) fares)
  let
    price = (DA.foldl (\acc (PaymentBreakUp fare) -> fare.amount - acc) 0.0 matchingFares)
  price * -1.0

getAutopayStatus :: Mb.Maybe String -> AutoPayStatus
getAutopayStatus autoPayStatus = case autoPayStatus of
...
getDistanceBetweenPoints lat1 lon1 lat2 lon2 = do
  let
    distance = getDistanceBwCordinates lat1 lon1 lat2 lon2
  distance
