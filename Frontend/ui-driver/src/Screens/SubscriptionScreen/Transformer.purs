...
getMultiLanguagePlanData isLocalized planData = do
  let
    dailyUnlimited = getString DAILY_UNLIMITED
  if isLocalized then
    planData
  else do
    if planData.title == dailyUnlimited then
      { title: getString DAILY_UNLIMITED, description: getString DAILY_UNLIMITED_PLAN_DESC }
    else
      { title: getString DAILY_PER_RIDE, description: getString DAILY_PER_RIDE_PLAN_DESC }
...
getPromoConfig offerEntityArr gradientConfig =
  ( map
      ( \(OfferEntity item) -> do
          let
            gradient' = DA.find (\(item') -> item'.id == item.offerId) gradientConfig
...
myPlanListTransformer planEntity isLocalized' config = do
  let
    isLocalized = fromMaybe false isLocalized'
...
planListTransformer (UiPlansResp planResp) isIntroductory gradientConfig =
  ...
  in
    if isIntroductory then
      [ introductoryPlanConfig Language ]
    else
      map (\planEntity -> getPlanCardConfig planEntity isLocalized isIntroductory gradientConfig) sortedPlanEntityList

decodeOfferDescription :: String -> String
decodeOfferDescription str = do
  let
    strArray = split (Pattern "-*$*-") str
...
alternatePlansTransformer (UiPlansResp planResp) state =
  let
    ...
    isLocalized = fromMaybe false planResp.isLocalized
  in
    map (\planEntity -> getPlanCardConfig planEntity isLocalized false state.data.config.subscriptionConfig.gradientConfig) alternatePlansArray
...
getFrequencyText constructor = case constructor of
...
getSelectedId (UiPlansResp planResp) = do
  let
    planEntityArray = planResp.list
  let
    dailyPerRidePlan = (DA.find (\(PlanEntity item) -> item.name == getString DAILY_UNLIMITED) planEntityArray)
...
getSelectedPlan (UiPlansResp planResp) gradientConfig = do
  ...
  case planEntity' of
    Just entity ->
      let
        (PlanEntity planEntity) = entity

        isLocalized = fromMaybe false planResp.isLocalized
      in
        Just $ getPlanCardConfig entity isLocalized false gradientConfig
    Nothing -> Nothing
...
getPlanCardConfig (PlanEntity planEntity) isLocalized isIntroductory gradientConfig =
  let
    planData = getMultiLanguagePlanData isLocalized { title: planEntity.name, description: planEntity.description }
  in
    { id: planEntity.id
    ...
    , isSelected: false
    , offers:
        ( if planEntity.freeRideCount > 0 then
            [ freeRideOfferConfig Language ]
          else if isIntroductory then [ introductoryOfferConfig Language ] else []
        )
          <> getPromoConfig planEntity.offers gradientConfig
...
constructDues duesArr showFeeBreakup =
  ( mapWithIndex
      ( \ind (DriverDuesEntity item) ->
          let
            offerAndPlanDetails = fromMaybe "" item.offerAndPlanDetails
          in
...
getFeeBreakup maxRidesEligibleForCharge planAmount totalRides = case maxRidesEligibleForCharge of
  Nothing -> "₹" <> getFixedTwoDecimals planAmount
  Just maxRides -> do
    let
      ridesToConsider = min totalRides maxRides
    if ridesToConsider /= 0 then
      ...
      getString $ NO_OPEN_MARKET_RIDES "NO_OPEN_MARKET_RIDES"
