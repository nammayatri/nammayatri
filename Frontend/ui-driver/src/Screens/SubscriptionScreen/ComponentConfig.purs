...
clearDueButtonConfig state = let
    ...
    buttonText = 
      case state.data.myPlanData.manualDueAmount > 0.0, state.data.myPlanData.autoPayStatus, isJust state.data.orderId of
        true, ACTIVE_AUTOPAY, true  -> (getString RETRY_STR) <> "  " <>  if state.props.myPlanProps.overDue then (getString CLEAR_DUES) else (getString CLEAR_MANUAL_DUES) <> "(R$" <> getFixedTwoDecimals state.data.myPlanData.manualDueAmount <> ")"
        true, ACTIVE_AUTOPAY, false  -> if state.props.myPlanProps.overDue then (getString CLEAR_DUES) else (getString CLEAR_MANUAL_DUES) <> "(R$" <> getFixedTwoDecimals state.data.myPlanData.manualDueAmount <> ")"
        true, _, true  -> (getString RETRY_AUTOPAY) <> " & " <>  (getString CLEAR_DUES) <> " (R$" <> getFixedTwoDecimals state.data.myPlanData.manualDueAmount <> ")" 
        true, _, false  -> (getString SETUP_AUTOPAY_STR) <> " & " <>  (getString CLEAR_DUES) <> " (R$" <> getFixedTwoDecimals state.data.myPlanData.manualDueAmount <> ")" 
...
dueDetailsListState state = let 
    calculateCharges count charges = 
      ...
      else Just $ show count <> " " <> getString (if count > 1 then RIDES else RIDE) <> " x R$" <> getFixedTwoDecimals (charges / DI.toNumber count) <> " " <> getString GST_INCLUDE
  in
  {
  dues : map (\ item -> do
