...
clearDueButtonConfig state =
  let
    config = PrimaryButton.config

    buttonText = case state.data.myPlanData.manualDueAmount > 0.0, state.data.myPlanData.autoPayStatus, isJust state.data.orderId of
...
resumeAutopayButtonConfig state =
  let
    config = PrimaryButton.config
  ...
  let
    ...
    popUpConf' =
      config
        ...
        , primaryText
          { text =
            case state.props.popUpState of
        ...
        , secondaryText
          { text =
            if state.props.popUpState == Mb.Just FailedPopup then
              getString YOUR_PAYMENT_WAS_UNSUCCESSFUL
            else if state.props.popUpState == Mb.Just SupportPopup then
              getString NEED_HELP_JOINING_THE_PLAN
            else if state.data.managePlanData.currentPlan.title == getString DAILY_PER_RIDE then
              getString DAILY_UNLIMITED_OFFER_NOT_AVAILABLE
            else
              ""
...
confirmCancelPopupConfig state =
  let
    config = PopUpModalConfig.config
...
getSelectedAlternatePlan state = do
  let
    plan = (DA.filter (\item -> item.id == state.props.managePlanProps.selectedPlanItem.id) state.data.managePlanData.alternatePlans)
  case plan DA.!! 0 of
...
getSelectedJoiningPlan state = do
  case state.props.joinPlanProps.selectedPlanItem of
    Mb.Just planEntity -> do
      let
        plan = (DA.filter (\item -> item.id == planEntity.id) state.data.joinPlanData.allPlans)
      case plan DA.!! 0 of
...
optionsMenuConfig state =
  let
    optionsMenuItems = state.data.config.subscriptionConfig.optionsMenuItems
  in
...
getHeaderConfig subView isManualPayDue isMultiDueType = case subView of
  ...
  ST.DueDetails ->
    { title:
        getString case isMultiDueType, isManualPayDue of
...
type HeaderData
  = { title :: String, actionText :: String, backbutton :: Boolean }
...
dueDetailsListState state =
  let
    calculateCharges count charges =
      if count == 0 && charges == 0.0 then
        Nothing
      else
        Just $ show count <> " " <> getString (if count > 1 then RIDES else RIDE) <> " x ₹" <> HU.getFixedTwoDecimals (charges / DI.toNumber count) <> " " <> getString GST_INCLUDE
  in
    { dues:
        map
          ( \item -> do
              let
                planOfferData = decodeOfferPlan item.plan
