...
screen initialState =
  ...
  , eval:
      \action state -> do
        let
          _ = spy "SelectLanguageScreen action " action
        let
          _ = spy "SelectLanguageScreen state " state
        eval action state
  }
...
menuButtonsView state push =
  scrollView
    ...
    [ linearLayout
        ...
        ( DA.mapWithIndex
            ( \index language ->
                MenuButton.view (push <<< MenuButtonAction) (menuButtonConfig state language index)
            )
