...
data ScreenOutput
  = GoBack DriverDetailsScreenState
...
eval BackPressed state = do
  if state.props.keyboardModalType /= NONE then
    continueWithCmd state
      [ do
          pure $ InAppKeyboardModalMobile (InAppKeyboardModal.BackPressed)
      ]
  else if state.props.genderSelectionModalShow then
    continueWithCmd state
      [ do
    ...
    exit
      ( GoBack
          state
            { data = state.data { driverEditAlternateMobile = Nothing }
            , props =
              state.props
...
eval (InAppKeyboardModalMobile (InAppKeyboardModal.OnSelection key index)) state = do
  let
    newVal = if not state.props.isEditAlternateMobile then ((fromMaybe "" state.data.driverAlternateMobile) <> key) else ((fromMaybe "" state.data.driverEditAlternateMobile) <> key)
...
eval (InAppKeyboardModalMobile (InAppKeyboardModal.OnClickBack text)) state = do
  if (text == (getString ENTER_MOBILE_NUMBER)) then
    continue state
  else do
    let
      newVal = (if length (text) > 0 then (take (length (text) - 1) text) else "")
...
eval (PopUpModalAction (PopUpModal.OnButton1Click)) state = continue state { props { removeNumberPopup = false } }
...
eval (GenderSelectionModalAction (SelectListModal.Button2 PrimaryButtonController.OnClick)) state = do
  let
    genderSelected = case state.data.genderSelectionModal.activeIndex of
...
getGenderState gender = case gender of
  Just value -> case value of
    "UNKNOWN" -> Nothing
    _ -> Just value
  Nothing -> Nothing
