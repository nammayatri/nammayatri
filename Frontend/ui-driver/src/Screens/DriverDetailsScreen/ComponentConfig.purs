...
removeAlternateNumberConfig state =
  let
    ...
    popUpConfigs' =
      config
        ...
        , isSelectButtonActive =
          case state.data.genderSelectionModal.activeIndex of
            Just index -> true
            Nothing -> false
...
enterOtpExceededModalStateConfig state =
  let
    config = PopUpModalConfig.config
  ...
  let
    ...
    inAppModalConfig' =
      config'
        ...
        , imageConfig
          { alpha =
            case state.data.driverAlternateMobile of
              Nothing -> 0.3
              Just _ ->
                if (length (fromMaybe "" state.data.driverAlternateMobile) < 10 || not state.props.checkAlternateNumber || (state.props.isEditAlternateMobile && length (fromMaybe "" state.data.driverEditAlternateMobile) < 10) || state.props.numberExistError) then
                  0.3
                else
                  1.0
          }
