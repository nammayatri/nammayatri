...
screen initialState =
  ...
  , eval:
      ( \state action -> do
          let
            _ = spy "DriverDetailsScreen state -----" state
          let
            _ = spy "DriverDetailsScreen --------action" action
          eval state action
      )
  }
    ...
    Anim.screenAnimation
      $ relativeLayout
          ...
          ( [ linearLayout
            ...
            , if state.props.keyboardModalType == ST.MOBILE__NUMBER then
            ...
            , if state.props.keyboardModalType == ST.OTP then
              ...
              <> if state.props.otpAttemptsExceeded then
...
enterOtpExceededModal push state = PopUpModal.view (push <<< PopUpModalActions) (enterOtpExceededModalStateConfig state)

enterMobileNumberModal :: forall w. (Action -> Effect Unit) -> ST.DriverDetailsScreenState -> PrestoDOM (Effect Unit) w
enterMobileNumberModal push state = InAppKeyboardModal.view (push <<< InAppKeyboardModalMobile) (enterMobileNumberState state)
...
removeAlternateNumber push state = PopUpModal.view (push <<< PopUpModalAction) (removeAlternateNumberConfig state)

selectYourGender :: forall w. (Action -> Effect Unit) -> ST.DriverDetailsScreenState -> PrestoDOM (Effect Unit) w
selectYourGender push state = SelectListModal.view (push <<< GenderSelectionModalAction) (selectYourGenderConfig state)
