...
screen initialState =
  ...
  , eval : \action state -> do
      let _ = spy "OperationHubScreen ----- state" state
      let _ = spy "OperationHubScreen --------action" action
      eval action state
  }
...
view push state = 
  ...
  [ Keyed.relativeLayout
    ...
    , weight 1.0
        ...
        <> if state.props.contactSupportModal /= ST.HIDE then [Tuple "bottomDrawerList" $ BottomDrawerList.view (push <<< BottomDrawerListAC) (bottomDrawerListConfig state)] else [Tuple "" $ linearLayout[][]]
        <> if state.props.menuOptions then [Tuple "menuOptionModal" $ menuOptionModal push state] else []
...
dropDownView push state =
  let operationHubList = fromMaybe [] state.data.operationHubList
  in
  ...
  linearLayout
    ...
    (DA.mapWithIndex(\index item -> do
       let (API.OperationHub hub) = item
       linearLayout
        [ height WRAP_CONTENT
        , width MATCH_PARENT
