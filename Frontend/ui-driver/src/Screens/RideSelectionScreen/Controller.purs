...
instance loggableAction :: Loggable Action where
  performLog action appId = case action of
    ...
    IndividualRideCardAction (IndividualRideCardController.Select index) -> do
      ...
      trackAppEndScreen appId (getScreen RIDE_SELECTION_SCREEN)
    ScrollStateChanged scrollState -> trackAppActionClick appId (getScreen RIDE_SELECTION_SCREEN) "in_screen" "scroll_state_changed"
    _ -> pure unit
...
eval (RideHistoryAPIResponseAction rideList) state = do
  let
    bufferCardDataPrestoList = (rideHistoryListTransformer rideList state.selectedCategory.categoryAction)
  let
    filteredRideList = (rideListResponseTransformer rideList state.selectedCategory.categoryAction)
  _ <- pure $ setRefreshing "2000030" false
  let
    loadBtnDisabled = length rideList == 0
  continue
    state
...
rideHistoryListTransformer list categoryAction =
  ( map
      ( \(RidesInfo ride) ->
          let
            specialLocationConfig = HU.getRideLabelData ride.specialLocationTag
          in
            ...
            , rideDistance:
                toPropValue $ (parseFloat (toNumber (fromMaybe 0 ride.chargeableDistance) / 1000.0) 2) <> " km " <> (getString RIDE)
