  ...
  popUpConfig' = config'{
    ...
    coverImageConfig {
      imageUrl = fetchImage FF_ASSET  if (state.data.activeRide.specialLocationTag == Nothing || (HU.getRequiredTag state.data.activeRide.specialLocationTag) == Nothing) 
                    then if (RC.decodeVehicleType $ getValueToLocalStore VEHICLE_CATEGORY) == Just ST.AmbulanceCategory then "ic_cancel_prevention_ambulance" else "ny_ic_frequent_cancellation_blocking"
