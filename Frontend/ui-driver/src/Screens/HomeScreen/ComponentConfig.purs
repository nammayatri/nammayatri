...
getRideCompletedConfig state = let 
  ...
  where 
    additionalCharges = [
      {
        text : if state.data.toll.tollAmbigous then getString TOLL_CHARGES_MAYBE_APPLICABLE else if state.data.toll.finalCharge > 0.0  then getString $ RIDE_TOLL_FARE_INCLUDES $  (getCurrency appConfig) <> (show $ round $ state.data.toll.finalCharge) else getString TOLL_ROAD_CHANGED
