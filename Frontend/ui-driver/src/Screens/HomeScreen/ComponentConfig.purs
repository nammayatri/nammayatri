...
waitTimeInfoCardConfig state =
  ...
  where
  rideInProgress = state.data.activeRide.status == SA.INPROGRESS
        ...
        , imageUrl =
          fetchImage FF_ASSET
            $ case bannerType of
                ...
                _
                  | bannerType == CLEAR_DUES_BANNER || bannerType == LOW_DUES_BANNER -> "ny_ic_clear_dues"
...
getAccessibilityHeaderText state =
  if state.data.activeRide.status == NEW then case state.data.activeRide.disabilityTag, state.data.activeRide.notifiedCustomer of
