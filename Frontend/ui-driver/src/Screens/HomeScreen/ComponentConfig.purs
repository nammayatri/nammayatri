  ...
  let
    ...
    rideActionModalConfig' =
      config
        ...
        , customerName =
          if DS.length (fromMaybe "" ((DS.split (DS.Pattern " ") (state.data.activeRide.riderName)) DA.!! 0)) < 4 then
            (fromMaybe "" ((DS.split (DS.Pattern " ") (state.data.activeRide.riderName)) DA.!! 0)) <> " " <> (fromMaybe "" ((DS.split (DS.Pattern " ") (state.data.activeRide.riderName)) DA.!! 1))
          else
    ...
    cancelRideModalConfig' =
      config
        ...
        , isSelectButtonActive =
          case state.data.cancelRideModal.activeIndex of
            Just index -> true
            Nothing -> false
        }
  in
...
autpPayBannerCarousel state action =
  ...
  where
  actionIconUrl_ =
    (HU.getAssetLink FunctionCall)
      <> case state.props.autoPayBanner of
...
paymentPendingPopupConfig state =
  let
    popupType = state.props.subscriptionPopupType
...
cancelConfirmationConfig state =
  let
    config' = PopUpModal.config
    ...
    popUpConfig' =
      config'
        ...
        , primaryText
          { text =
            case state.data.activeRide.specialLocationTag of
        ...
        , coverImageConfig
          { imageUrl =
            fetchImage FF_ASSET
              if (state.data.activeRide.specialLocationTag == Nothing || (HU.getRequiredTag state.data.activeRide.specialLocationTag) == Nothing) then
                if (RC.decodeVehicleType $ getValueToLocalStore VEHICLE_CATEGORY) == Just ST.CarCategory then "ic_cancel_prevention_cab" else "ic_cancel_prevention"
              else
                (HU.getRideLabelData state.data.activeRide.specialLocationTag).cancelConfirmImage
          , visibility = VISIBLE
...
chatViewConfig state =
  let
    suggestionList =
      if showSuggestions state then
        if (state.data.activeRide.tripType == ST.Rental && state.props.currentStage == ST.ChatWithCustomer) then
          getSuggestionsfromKey chatSuggestion "dsRentalInitial"
        else if (state.data.activeRide.notifiedCustomer) then
          getSuggestionsfromKey chatSuggestion "driverInitialAP"
        else
          getSuggestionsfromKey chatSuggestion "driverInitialBP"
      else
...
enterOdometerReadingConfig state =
  let
    config' = InAppKeyboardModal.config
...
specialZonePopupConfig state =
  let
    config = RequestInfoCard.config

    specialZonePopupConf' =
      config
        { title
...
waitTimeInfoCardConfig state =
  ...
  in
    RequestInfoCard.config
      ...
      , primaryText
        { text =
          if rideInProgress then
            getVarString THIS_EXTRA_AMOUNT_THE_CUSTOMER_WILL_PAY [ show maxWaitTimeInMinutes ]
          else
            getString HOW_LONG_WAITED_FOR_PICKUP
  ...
  where
  rideInProgress = state.data.activeRide.status == API.INPROGRESS
...
makePaymentState state =
  let
    payableAndGST = EHC.formatCurrencyWithCommas (show state.data.paymentState.payableAndGST)
...
getChargesBreakup paymentBreakUpArr =
  map
    ( \(PaymentBreakUp item) ->
        { val: "₹" <> (show item.amount)
        , key:
            case item.component of
      ...
      config
        ...
        , title =
          case bannerType of
            ...
            _
              | bannerType == CLEAR_DUES_BANNER || bannerType == LOW_DUES_BANNER -> getVarString CLEAR_DUES_BANNER_TITLE [ dues ]
        ...
        , titleColor =
          case bannerType of
            DUE_LIMIT_WARNING_BANNER -> Color.red
            _
              | bannerType == CLEAR_DUES_BANNER || bannerType == LOW_DUES_BANNER -> Color.black900
            _ -> Color.white900
        , actionText
          { text =
            case bannerType of
              _
                | bannerType == DUE_LIMIT_WARNING_BANNER || bannerType == CLEAR_DUES_BANNER || bannerType == LOW_DUES_BANNER -> getString PAY_NOW
        ...
        , imagePadding =
          case bannerType of
            _
              | bannerType == CLEAR_DUES_BANNER || bannerType == LOW_DUES_BANNER -> PaddingTop 0
...
genericAccessibilityPopUpConfig state =
  let
    config' =
      PopUpModal.config
        { gravity = CENTER
...
getDisabilityTypeVideo state pwdtype =
  let
    cityConfig = HU.getCityConfig state.data.config.cityConfig (getValueToLocalNativeStore DRIVER_LOCATION)
  ...
  in
    maybe "" (\obj -> obj.videoUrl) disabilityToVideo
...
getAccessibilityPopupData state pwdtype isDriverArrived =
  ...
  in
    case pwdtype, isDriverArrived of
      Just ST.BLIND_AND_LOW_VISION, true ->
        accessibilityConfig'
      ...
      Just ST.BLIND_AND_LOW_VISION, false ->
        accessibilityConfig'
      ...
      Just ST.LOCOMOTOR_DISABILITY, true ->
        accessibilityConfig'
      ...
      Just ST.LOCOMOTOR_DISABILITY, false ->
        accessibilityConfig'
  ...
  getUpdatedAssets cityConfig =
    if cityConfig.cityCode == "std:040" then case pwdtype, isDriverArrived of
    ...
    else case pwdtype, isDriverArrived of
...
getAccessibilityHeaderText state =
  ...
  in
    ...
    else if state.data.activeRide.status == NEW then case state.data.activeRide.disabilityTag, state.data.activeRide.notifiedCustomer of
    ...
    else case state.data.activeRide.disabilityTag of
...
getRideCompletedConfig state =
  let
    ...
    viewOrderConfig =
      [ { condition: isJust metroRideCoins, elementView: RideCompletedCard.COINS_EARNED_VIEW }
    ...
    config' =
      config
        ...
        , driverBottomCard
          ...
          , savedMoney =
            (if state.data.config.rideCompletedCardConfig.showSavedCommission then [ { amount: (state.data.endRideData.finalAmount * 18) / 100, reason: getString SAVED_DUE_TO_ZERO_COMMISSION } ] else [])
              <> ( case state.data.endRideData.tip of
  ...
  metroRideCoins = case DA.find (\(API.CoinsEarned item) -> item.eventType == "MetroRideCompleted") state.data.coinsEarned of
...
getRatingCardConfig state =
  RatingCard.ratingCardConfig
    { data
...
gotoKnowMoreConfig state =
  PopUpModal.config
    ...
    , secondaryText
      { text = getString THIS_FEATURE_WILL_BE_APPLICABLE
...
gotoRequestPopupConfig state =
  ...
  where
  (PopupReturn strings) = gotoCounterStrings state.data.driverGotoState.goToPopUpType
...
gotoCounterStrings popupType =
  let
    vehicleVarient = (getValueFromCache (show VEHICLE_VARIANT) JB.getKeyInSharedPrefKeys)
  in
    case popupType of
      MORE_GOTO_RIDES ->
        PopupReturn
          { primaryText: getString MORE_GOTO_RIDE_COMING
      ...
      REDUCED 0 ->
        PopupReturn
          { primaryText: getString GOTO_REDUCED_TO_ZERO
      ...
      REDUCED n ->
        PopupReturn
          { primaryText: getString GOTO_REDUCED_TO <> " " <> show n
      ...
      VALIDITY_EXPIRED ->
        PopupReturn
          { primaryText: getString VALIDITY_EXPIRED_STR
      ...
      REACHED_HOME ->
        PopupReturn
          { primaryText: getString GOTO_LOC_REACHED
  ...
  getImage current new =
    let
      isVehicleRickshaw = (getValueFromCache (show VEHICLE_VARIANT) JB.getKeyInSharedPrefKeys) == "AUTO_RICKSHAW"
    in
      if isVehicleRickshaw then fetchImage FF_ASSET current else new

------------------------------------------------------------------------------gotoLocInRange------------------------------------------------------------------------------------
gotoLocInRangeConfig :: ST.HomeScreenState -> PopUpModal.Config
gotoLocInRangeConfig _ =
  PopUpModal.config
    { optionButtonOrientation = "HORIZONTAL"
    , buttonLayoutMargin = Margin 16 0 16 20
    , gravity = CENTER
...
disableGotoConfig _ =
  PopUpModal.config
    ...
    , secondaryText
      { text = getString YOU_STILL_HAVE_TIME_LEFT
...
gotoButtonConfig state =
  ...
  where
  gotoTimer = gotoTimerConfig state.data.driverGotoState.isGotoEnabled
...
vehicleNotSupportedPopup state =
  let
    ...
    vehicletype = case state.data.vehicleType of
      _
        | elem state.data.vehicleType [ "AMBULANCE_TAXI", "AMBULANCE_TAXI_OXY", "AMBULANCE_AC", "AMBULANCE_AC_OXY", "AMBULANCE_VENTILATOR" ] -> getString AMBULANCE
      ...
      , coverImageConfig
        { imageUrl =
          case state.data.linkedVehicleCategory of
            ...
            _
              | elem state.data.linkedVehicleCategory [ "AMBULANCE_TAXI", "AMBULANCE_TAXI_OXY", "AMBULANCE_AC", "AMBULANCE_AC_OXY", "AMBULANCE_VENTILATOR" ] -> fetchImage FF_ASSET "ny_ic_ambulance_not_supported"
...
selectPlansModalState state =
  SelectPlansModal.config
