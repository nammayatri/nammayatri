...
getAccessibilityHeaderText state = 
  let config = accessibilityConfig Config
    in  if state.data.activeRide.bookingFromOtherPlatform then
          config {primaryText = (getString $ SOME_FEATURE_ARE_NOT_AVAILABLE_WITH_THIS_PROVIDER (getString $ MERCHANT_NAME "")) <> ".&nbsp;<span style=\"color:#2194FF;\">Why?</span>&nbsp", secondaryText = "", imageUrl = fetchImage FF_ASSET "ny_ic_alert_hexagon", background = Color.grey900, textColor = Color.black700, clickable = true}
        else
