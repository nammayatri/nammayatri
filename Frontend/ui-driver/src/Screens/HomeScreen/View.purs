...
import Services.API (GetRidesHistoryResp(..), OrderStatusRes(..), Status(..), DriverProfileStatsReq(..), GetDriverInfoReq(..),LocationInfo(..),RidesInfo(..), StopLocation(..))
      ...
      , linearLayout
        ...
        , alignParentBottom "true,-1"
            ...
            , PP.cornerRadii $ PTD.Corners 40.0 true true false false
              ...
              , if state.data.linkedVehicleCategory /= "AUTO_RICKSHAW" && state.props.driverStatusSet == ST.Offline
                  then if state.data.bankChargesEnabled then bookingPreferenceNavView push state else bankDetailsEntryView push state
                  else dummyTextView
            ]
        ]
    , linearLayout
