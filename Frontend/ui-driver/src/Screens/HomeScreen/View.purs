...
import Services.Accessor (_lat, _lon, _routeCode, _stopName, _stopCode, _routeCode, _stopLat, _stopLong, _routeInfo, _source, _destination, _allowStartRideFromQR, _routeInfo, _source, _destination, _routeCode, _stopName )
        ...
        ( \push -> do
          ...
          case localStage of
            ...
            _                -> do
                                ...
                                when (HU.specialVariantsForTracking FunctionCall && (not $ initialState.props.whereIsMyBusConfig.linkTripPopup) && isNothing initialState.data.whereIsMyBusData.trip) do
...
view push state =
  ...
  [ relativeLayout
      ...
      , if (state.props.whereIsMyBusConfig.selectBusRouteDropdown) then selectBusRoutePopup push state else dummyTextView
      , if (state.props.whereIsMyBusConfig.selectBusDriverDropdown) then selectBusDriverPopup push state else dummyTextView
      , if (state.props.whereIsMyBusConfig.selectBusConductorDropdown) then selectBusConductorPopup push state else dummyTextView
  ]
  where 
  ...
  where 
    ...
    mkRouteButtonConfig state = 
      let selectedRoutes = state.props.whereIsMyBusConfig.selectedRoutes
          (API.AvailableRoutesList availableRoutes) = fromMaybe dummyAvailableRoutesList state.data.whereIsMyBusData.availableRoutes
