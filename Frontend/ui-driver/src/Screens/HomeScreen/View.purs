...
screen initialState =
  ...
  , globalEvents:
      [ ( \push -> do
            ...
            let
              localStage = getValueToLocalNativeStore LOCAL_STAGE
            if (localStage /= "RideAccepted" && localStage /= "ChatWithCustomer" && initialState.data.activeRide.waitTimerId /= "") then do
              ...
              pure unit
            void
              if (DA.any (_ == localStage) [ "RideRequested", "HomeScreen", "__failed" ]) && initialState.data.driverGotoState.isGotoEnabled then
              ...
              else if (initialState.data.driverGotoState.timerId /= "") then
                pure $ EHC.clearTimer initialState.data.driverGotoState.timerId
              else
                pure unit
            case localStage of
              "RideRequested" -> do
                if (getValueToLocalStore RIDE_STATUS_POLLING) == "False" then do
                  ...
                  let
                    secondsOver = if EHC.getExpiryTime (getValueToLocalNativeStore RIDE_REQUEST_TIME) true >= (rideRequestPollingData.duration) then (rideRequestPollingData.duration) else EHC.getExpiryTime (getValueToLocalNativeStore RIDE_REQUEST_TIME) true
              ...
              "RideAccepted" -> do
                let
                  waitTime = DS.split (DS.Pattern "<$>") (getValueToLocalStore WAITING_TIME_VAL)
...
view push state =
  frameLayout
    ...
    $ [ relativeLayout
      ...
      , if (state.props.subscriptionPopupType == ST.FREE_TRIAL_POPUP) && state.data.config.subscriptionConfig.enableSubscriptionPopups then
          PopUpModal.view (push <<< FreeTrialEndingAC) (freeTrialEndingPopupConfig state)
        else
      ...
      , if (DA.any (_ == state.props.subscriptionPopupType) [ ST.SOFT_NUDGE_POPUP, ST.LOW_DUES_CLEAR_POPUP, ST.GO_ONLINE_BLOCKER ] && state.data.config.subscriptionConfig.enableSubscriptionPopups) then
          PopUpModal.view (push <<< PaymentPendingPopupAC) (paymentPendingPopupConfig state)
        else
          linearLayout [ visibility GONE ] []
...
driverMapsHeaderView push state =
    ...
    [ linearLayout
        ...
        , frameLayout
            ...
            , linearLayout
                ...
                , if showGenderBanner then
                    genderBannerView state push
                  else if state.data.paymentState.paymentStatusBanner then
                    paymentStatusBanner state push
                  else if (state.props.autoPayBanner /= ST.NO_SUBSCRIPTION_BANNER && state.props.driverStatusSet /= ST.Offline) then
                    autoPayBannerView state push false
                  else
                    dummyTextView
                ]
            ]
  ...
  where
  showGenderBanner = state.props.showGenderBanner && state.props.driverStatusSet /= ST.Offline && getValueToLocalStore IS_BANNER_ACTIVE == "True" && state.props.autoPayBanner == ST.NO_SUBSCRIPTION_BANNER
...
accessibilityBanner state push =
  ...
  where
  bannerVisibility = state.props.driverStatusSet == ST.Offline && state.props.currentStage == HomeScreen && state.data.config.purpleRideConfig.showPurpleVideos
...
gotoRecenterAndSupport state push =
  linearLayout
    ...
    [ linearLayout
        ...
        [ if state.data.driverGotoState.gotoEnabledForMerchant && state.data.config.gotoConfig.enableGoto then
...
alternateNumberOrOTPView state push =
  ...
  where
  showAddAltNumber = (state.data.driverAlternateMobile == Nothing || state.props.showlinkAadhaarPopup) && state.props.statusOnline
...
offlineView push state =
  let
    showGoInYellow = state.data.config.subscriptionConfig.enableSubscriptionPopups && state.data.paymentState.driverBlocked || (state.data.paymentState.totalPendingManualDues > state.data.config.subscriptionConfig.highDueWarningLimit)
  in
    linearLayout
      [ width MATCH_PARENT
      , height MATCH_PARENT
  ...
  in
    linearLayout
      ...
      , linearLayout
          ...
          , gravity CENTER_HORIZONTAL
          , stroke
              if state.props.driverStatusSet == Offline then
                ("2," <> Color.red)
              else if (((getValueToLocalStore IS_DEMOMODE_ENABLED) == "true") && ((state.props.driverStatusSet == Online) || state.props.driverStatusSet == Silent)) then
                ("2," <> Color.yellow900)
              else if state.props.driverStatusSet == Online then
                ("2," <> Color.darkMint)
              else
                ("2," <> Color.blue800)
          , cornerRadius 50.0
...
driverStatusPill pillConfig push state index =
  linearLayout
    ...
    , margin (Margin 10 10 10 10)
    , background
        $ case (getDriverStatusResult index state.props.driverStatusSet pillConfig.status) of
  ...
  PrestoAnim.entryAnimationSetForward [ Anim.translateInXForwardAnim true, Anim.fadeIn true ]
    $ linearLayout
        ...
        , linearLayout
            ...
            []
        , if DA.null state.data.driverGotoState.savedLocationsArray then
...
savedLocationListView push state =
  scrollView
    ...
    [ linearLayout
        ...
        [ linearLayout
            ...
            ( map
                ( \item ->
                    GoToLocationModal.view (push <<< GoToLocationModalAC) (locationListItemConfig item state)
                )
                state.data.driverGotoState.savedLocationsArray
            )
        , linearLayout
            [ width MATCH_PARENT
            , height WRAP_CONTENT
  ...
  where
  showAddGoto = DA.length state.data.driverGotoState.savedLocationsArray < state.data.config.gotoConfig.maxGotoLocations
...
enterOtpModal push state = InAppKeyboardModal.view (push <<< InAppKeyboardModalAction) (enterOtpStateConfig state)
...
offlineNavigationLinks push state =
  ...
  where
  navLinksArray =
    [ { title: getString ADD_GOTO, icon: "ny_ic_loc_goto", action: AddGotoAC }
...
bottomNavBar push state = BottomNavBar.view (push <<< BottomNavBarAction) (navData ScreenNames.HOME_SCREEN state.data.config.bottomNavConfig)
...
popupModals push state =
  PrestoAnim.animationSet [ Anim.fadeIn true ]
    $ linearLayout
        ...
        [ PopUpModal.view (push <<< clickAction popupType) case popupType of
  ...
  where
  popupType =
    if state.data.driverGotoState.gotoLocInRange then
      ST.LocInRange
    ...
    case activeRideResponse of
      Right (GetRidesHistoryResp rideList) -> do
        if not (DA.null rideList.list) then do
          ...
          if isLocalStageOn HomeScreen then
            pure unit
          else
            doAff do liftEffect $ push $ action "CANCELLED_PRODUCT"
      Left err -> pure unit
  else
    pure unit
      ...
      Right (GetRidesHistoryResp rideList) -> do
        if (DA.null rideList.list) && count > 0 then do
          ...
          if isLocalStageOn RideAccepted then
            ...
            doAff do liftEffect $ push $ action "DRIVER_ASSIGNMENT"
      Left err -> pure unit
  else
    pure unit
...
getDriverStatusResult index driverStatus currentStatus = case (getValueToLocalStore IS_DEMOMODE_ENABLED) of
  "true" ->
    if (index == 2) then
      ...
      DEFAULT
  _ ->
    if (driverStatus == currentStatus) then
      ACTIVE
    else
      DEFAULT
