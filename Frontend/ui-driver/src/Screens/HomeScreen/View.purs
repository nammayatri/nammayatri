          ...
          , height MATCH_PARENT
            ...
            , linearLayout
              ...
              , gravity BOTTOM
                ...
                <> if not (state.data.linkedVehicleCategory `elem` ["AUTO_RICKSHAW"] )&& DA.any (_ == state.props.driverStatusSet) [ST.Online, ST.Silent] then [bookingPreferenceNavView push state] else []
            ]
        ]
...
offlineView push state =
  ...
  where
    isBookingPreferenceVisible = 
      not (state.data.linkedVehicleCategory `elem` ["AUTO_RICKSHAW"] && HU.isAmbulance state.data.linkedVehicleCategory)
