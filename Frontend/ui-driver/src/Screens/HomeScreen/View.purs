...
view push state =
  ...
  , width MATCH_PARENT
      ...
      , case HU.getPaymentNudgeFromSharedPrefs PAYMENT_NUDGE of
          Just configObject -> if ((isLocalStageOn HomeScreen) && (MU.getMerchant FunctionCall) == MU.NAMMAYATRI) then PopUpModal.view (push <<< PaymentNudgeAC configObject) (paymentNudgeConfig false configObject) else linearLayout[visibility GONE][]
          Nothing -> linearLayout[visibility GONE][]
      ...
      , if (getValueToLocalNativeStore SHOW_FREE_TRIAL_ENDING == "true" ) && (MU.getMerchant FunctionCall) == MU.NAMMAYATRI && numberOfDaysLeft < 4 && numberOfDaysLeft > 0
           then PopUpModal.view (push <<< FreeTrialEndingAC) (freeTrialEndingPopupConfig numberOfDaysLeft) 
           else linearLayout[visibility GONE][]
      , if state.props.driverBlocked && (MU.getMerchant FunctionCall) == MU.NAMMAYATRI then PopUpModal.view (push <<< PaymentPendingBlockerAC state.props.autoPaySet) (paymentPendingBlockerPopup state.props.autoPaySet) else linearLayout[visibility GONE][]
