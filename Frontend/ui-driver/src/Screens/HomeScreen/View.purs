...
rideStatusPolling :: forall action. String -> Number -> HomeScreenState -> (action -> Effect Unit) -> (String -> Maybe NotificationBody -> action) -> Flow GlobalState Unit
        ...
        if not (DA.null rideList.list) then do
          ...
          rideStatusPolling pollingId duration state push action
            ...
            if isLocalStageOn HomeScreen then pure unit
              else doAff do liftEffect $ push $ action "CANCELLED_PRODUCT" Nothing
      Left err -> pure unit
    else pure unit
...
rideRequestPolling :: forall action. String -> Int -> Number -> HomeScreenState -> (action -> Effect Unit) -> (String -> Maybe NotificationBody -> action) -> Flow GlobalState Unit
        ...
        if (DA.null rideList.list) && count > 0 then do
          ...
          rideRequestPolling pollingId (count - 1) duration state push action
            ...
            if isLocalStageOn RideAccepted then pure unit
              else doAff do liftEffect $ push $ action "DRIVER_ASSIGNMENT" Nothing
      Left err -> pure unit
    else pure unit
...
checkCurrentRide :: forall action.(action -> Effect Unit) -> (String -> Maybe NotificationBody -> action) -> HomeScreenState -> Flow GlobalState Unit
        ...
        if (DA.null rideList.list) || isJust initialState.data.advancedRideData then
          pure unit
            ...
            case (rideList.list DA.!! 0) of
              Just ride -> doAff do liftEffect $ push $ action "DRIVER_ASSIGNMENT" Nothing
              Nothing -> pure unit
      Left err -> pure unit
