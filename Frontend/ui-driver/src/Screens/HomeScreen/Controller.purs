...
eval (GoToButtonClickAC PrimaryButtonController.OnClick) state = do
  ...
  if state.data.driverGotoState.isGotoEnabled then
    continue state { data { driverGotoState { confirmGotoCancel = true } } }
  else if state.data.driverGotoState.gotoCount <= 0 then
    continue state
  else do
...
eval (GotoRequestPopupAction (PopUpModal.OnButton1Click)) state = case state.data.driverGotoState.goToPopUpType of
...
eval (UpdateGoHomeTimer timerID timeInMinutes sec) state =
  if sec <= 0 then do
    ...
    continue state { data { driverGotoState { timerInMinutes = timeInMinutes <> " " <> (getString MIN_LEFT), timerId = timerID } } }
...
eval BackPressed state = do
  ...
  else if state.props.showRideRating then
    continue state { props { showRideRating = false } }
  ...
  else if state.props.endRidePopUp then
    continue state { props { endRidePopUp = false } }
  else if (state.props.showlinkAadhaarPopup && state.props.showAadharPopUp) then
    continue state { props { showAadharPopUp = false } }
  else if state.props.silentPopUpView then
    continue state { props { silentPopUpView = false } }
  else if state.data.paymentState.showBlockingPopup then
    continue state { data { paymentState { showBlockingPopup = false } } }
  else if state.props.subscriptionPopupType /= ST.NO_SUBSCRIPTION_POPUP then
    continue state { props { subscriptionPopupType = ST.NO_SUBSCRIPTION_POPUP } }
  else if state.data.driverGotoState.goToInfo then
    continue state { data { driverGotoState { goToInfo = false } } }
  else if state.data.driverGotoState.gotoLocInRange then
    continue state { data { driverGotoState { gotoLocInRange = false } } }
  else if state.data.driverGotoState.confirmGotoCancel then
    continue state { data { driverGotoState { confirmGotoCancel = false } } }
  else if state.data.driverGotoState.showGoto then
    continue state { data { driverGotoState { showGoto = false } } }
  else if state.data.driverGotoState.goToPopUpType /= ST.NO_POPUP_VIEW then
    continue state { data { driverGotoState { goToPopUpType = ST.NO_POPUP_VIEW } } }
  else if state.props.showContactSupportPopUp then
    continue state { props { showContactSupportPopUp = false } }
  else if state.props.accountBlockedPopup then
    continue state { props { accountBlockedPopup = false } }
  else do
    _ <- pure $ minimizeApp ""
    continue state

eval TriggerMaps state =
...
eval (Notification notificationType) state = do
  ...
  if (checkNotificationType notificationType ST.DRIVER_REACHED && (state.props.currentStage == ST.RideAccepted || state.props.currentStage == ST.ChatWithCustomer) && (not state.data.activeRide.notifiedCustomer)) then do
    let
      newState = state { props { showAccessbilityPopup = isJust state.data.activeRide.disabilityTag } }
...
eval (ShowMap key lat lon) state =
  continueWithCmd state
    [ do
...
eval (OfferPopupAC PopUpModal.OnButton1Click) state = do
  ...
  let
    _ = unsafePerformEffect $ firebaseLogEvent "ny_driver_in_app_popup_join_now"
...
eval (PaymentPendingPopupAC PopUpModal.OnButton1Click) state = do
  if state.props.subscriptionPopupType == ST.GO_ONLINE_BLOCKER then do
    ...
    let
      _ = unsafePerformEffect $ firebaseLogEvent "ny_driver_due_payment_settle_now"
    ...
    if state.props.subscriptionPopupType == ST.SOFT_NUDGE_POPUP then do
      ...
      let
        _ = unsafePerformEffect $ firebaseLogEvent "ny_driver_payment_pending_soft_nudge_plan"
      pure unit
...
eval (PaymentPendingPopupAC PopUpModal.OptionWithHtmlClick) state = do
  if state.props.subscriptionPopupType == ST.GO_ONLINE_BLOCKER then do
    ...
    let
      _ = unsafePerformEffect $ firebaseLogEvent "ny_driver_due_payment_view_details"
    ...
    let
      _ = unsafePerformEffect $ firebaseLogEvent "ny_driver_payment_pending_soft_nudge_plan_go_online"
...
eval (PaymentPendingPopupAC PopUpModal.DismissPopup) state = do
  if (state.props.subscriptionPopupType == ST.SOFT_NUDGE_POPUP) then do
    ...
    let
      _ = unsafePerformEffect $ firebaseLogEvent "ny_driver_payment_pending_soft_nudge_plan_dismiss"
    pure unit
  else if (state.props.subscriptionPopupType == ST.LOW_DUES_CLEAR_POPUP) then do
    _ <- pure $ setValueToLocalStore APP_SESSION_TRACK_COUNT "shown"
    pure unit
...
eval (InAppKeyboardModalAction (InAppKeyboardModal.OnclickTextBox index)) state = do
  let
    focusIndex = if index > (length state.props.rideOtp) then (length state.props.rideOtp) else index
  let
    rideOtp = take index state.props.rideOtp
...
eval (InAppKeyboardModalAction (InAppKeyboardModal.OnClickDone text)) state = do
  let
    exitState = if state.props.zoneRideBooking then StartZoneRide state else StartRide state
  exit exitState
...
eval (RideActionModalAction (RideActionModal.OnNavigate)) state = do
  _ <- pure $ setValueToLocalStore TRIGGER_MAPS "false"
  let
    lat = if (state.props.currentStage == ST.RideAccepted || state.props.currentStage == ST.ChatWithCustomer) then state.data.activeRide.src_lat else state.data.activeRide.dest_lat
...
eval (RideActionModalAction (RideActionModal.CallCustomer)) state =
  continueWithCmd state
    [ do
...
eval (OpenChatScreen) state = do
  if not state.props.chatcallbackInitiated || state.data.activeRide.disabilityTag == Just ST.BLIND_AND_LOW_VISION then
    continue state
  else do
...
eval (RideActionModalAction (RideActionModal.MessageCustomer)) state = do
  if not state.props.chatcallbackInitiated then
    ...
    continueWithCmd state { props { currentStage = ST.ChatWithCustomer, sendMessageActive = false, unReadMessages = false, isChatOpened = true } }
      [ do
...
eval (UpdateMessages message sender timeStamp size) state = do
  if not state.props.chatcallbackInitiated then
    continue state { props { canSendSuggestion = true } }
  else do
    continueWithCmd state { data { messagesSize = size }, props { canSendSuggestion = true } }
      [ do
          pure $ (RideActionModalAction (RideActionModal.LoadMessages))
      ]
  ...
  case (Array.last allMessages) of
    Just value ->
      if value.message == "" then
        continue state { data { messagesSize = show (fromMaybe 0 (fromString state.data.messagesSize) + 1) }, props { canSendSuggestion = true } }
      else if value.sentBy == "Driver" then
        updateMessagesWithCmd state { data { messages = allMessages, suggestionsList = [] }, props { canSendSuggestion = true } }
      else do
        let
          readMessages = fromMaybe 0 (fromString (getValueToLocalNativeStore READ_MESSAGES))
        let
          unReadMessages = (if (readMessages == 0 && state.props.currentStage /= ST.ChatWithCustomer) then true else (if (readMessages < (Array.length allMessages) && state.props.currentStage /= ST.ChatWithCustomer) then true else false))
        let
          suggestions = getDriverSuggestions state $ getSuggestionsfromKey value.message
...
eval (ChatViewActionController (ChatView.SendMessage)) state = do
  if state.data.messageToBeSent /= "" then
...
eval (ChatViewActionController (ChatView.SendSuggestion chatSuggestion)) state = do
  if state.props.canSendSuggestion then do
    let
      message = if isPreviousVersion (getValueToLocalStore VERSION_NAME) (getPreviousVersion (getMerchant Common.FunctionCall)) then (getMessageFromKey chatSuggestion "EN_US") else chatSuggestion
    _ <- pure $ sendMessage message
    let
      _ = unsafePerformEffect $ logEvent state.data.logField $ toLower $ (replaceAll (Pattern "'") (Replacement "") (replaceAll (Pattern ",") (Replacement "") (replaceAll (Pattern " ") (Replacement "_") chatSuggestion)))
...
eval (RideActionModalAction (RideActionModal.LocationTracking)) state = do
  let
    newState = state { props { showDottedRoute = not state.props.showDottedRoute } }
...
eval (WaitTimerCallback timerID _ seconds) state = continue state { data { activeRide { waitTimerId = timerID, waitTimeSeconds = seconds } } }
...
eval (CancelRideModalAction (SelectListModal.Button2 PrimaryButtonController.OnClick)) state = do
  pure $ hideKeyboardOnNavigation true
  let
    cancelReasonSelected = case state.data.cancelRideModal.activeIndex of
  ...
  case cancelReasonSelected of
    Just reason -> do
      ...
      if (reason.reasonCode == "OTHER") then
        exit $ SelectListModal state { props = state.props { cancelRideModalShow = false, cancelConfirmationPopup = false, zoneRideBooking = true } }
      else do
        let
          newState = state { data = state.data { cancelRideModal = state.data.cancelRideModal { selectedReasonCode = reason.reasonCode, selectedReasonDescription = reason.description } }, props = state.props { cancelRideModalShow = false, otpAttemptsExceeded = false, cancelConfirmationPopup = false, zoneRideBooking = true } }
...
eval (PopUpModalCancelConfirmationAction (PopUpModal.CountDown seconds id status timerID)) state = do
  if status == "EXPIRED" && seconds == 0 then do
    ...
    continue state { data { cancelRideConfirmationPopUp { delayInSeconds = seconds, timerID = timerID, continueEnabled = false } } }
...
eval (CurrentLocation lat lng) state = do
  let
    newState = state { data { currentDriverLat = getLastKnownLocValue ST.LATITUDE lat, currentDriverLon = getLastKnownLocValue ST.LONGITUDE lng } }
...
eval (IsMockLocation isMock) state = do
  let
    val = false --isMock == "true" -- TODO:: Handle it properly @ashkiriti
...
eval (TimeUpdate time lat lng) state = do
  let
    driverLat = getLastKnownLocValue ST.LATITUDE lat
    ...
    [ do
        ...
        case state.data.config.waitTimeConfig.enableWaitTime, state.props.currentStage, state.data.activeRide.notifiedCustomer, state.data.snappedOrigin of
          ...
          true, ST.RideAccepted, false, Just snapped -> do
            let
              dist = (getDistanceBwCordinates driverLat driverLong snapped.lat snapped.lon)
        ...
        void $ launchAff $ flowRunner defaultGlobalState
          $ do
              ...
              case rideRouteResp of
                Left _ -> pure unit
                Right (API.RideRouteResp resp) -> case Array.head resp.points of
                  Just (API.LatLong loc) -> do
                    let
                      dist = getDistanceBwCordinates lat lng loc.lat loc.lon
...
eval (SwitchDriverStatus status) state =
  if state.data.paymentState.driverBlocked && not state.data.paymentState.subscribed then
    continue state { props { subscriptionPopupType = ST.GO_ONLINE_BLOCKER } }
  else if state.data.paymentState.driverBlocked then
    continue state { data { paymentState { showBlockingPopup = true } } }
  ...
  else if state.props.driverStatusSet == status then
    continue state
  else case status of
...
eval RemovePaymentBanner state =
  if state.data.paymentState.blockedDueToPayment then
    continue state
  else
    continue state { data { paymentState { paymentStatusBanner = false } } }
...
eval (PopUpModalChatBlockerAction PopUpModal.OnButton2Click) state =
  continueWithCmd state { props { showChatBlockerPopUp = false } }
    [ do
        pure $ RideActionModalAction (RideActionModal.MessageCustomer)
    ]
...
eval (PopUpModalChatBlockerAction PopUpModal.OnButton1Click) state =
  continueWithCmd state { props { showChatBlockerPopUp = false } }
    [ do
...
activeRideDetail state (RidesInfo ride) =
  let
    waitTimeSeconds = DS.split (DS.Pattern "<$>") (getValueToLocalStore TOTAL_WAITED)
  ...
  in
    ...
    , specialLocationTag:
        if isJust ride.disabilityTag then
          Just "Accessibility"
        else if isJust ride.driverGoHomeRequestId then
          Just "GOTO"
        else
          ride.specialLocationTag
    , --  "None_SureMetro_PriorityDrop",--"GOTO", disabilityTag:
        case ride.disabilityTag of
...
getLastKnownLocValue lType val =
  let
    lastKnownValue = fromMaybe 0.0 $ Number.fromString $ getValueToLocalNativeStore $ if lType == ST.LATITUDE then LAST_KNOWN_LAT else LAST_KNOWN_LON

    currentVal = if (fromMaybe 0.0 (Number.fromString val)) == 0.0 then Nothing else (Number.fromString val)
  in
    fromMaybe lastKnownValue currentVal
...
getPeekHeight state =
  let
    headerLayout = runFn1 JB.getLayoutBounds $ getNewIDWithTag "rideActionHeaderLayout"
...
getPreviousVersion merchant = case merchant of
  NAMMAYATRI -> "1.4.8"
  YATRI -> "2.3.0"
  YATRISATHI -> "0.1.8"
  _ -> "100.100.100"
