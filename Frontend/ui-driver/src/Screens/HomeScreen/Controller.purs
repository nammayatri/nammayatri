...
data Action = NoAction
            ...
            | ChooseBusBadge (Array ST.FleetBadgeDrivers) Boolean (Array ST.FleetBadgeDrivers) Boolean
...
eval TriggerMaps state = continueWithCmd state[ do
  ...
  if state.data.activeRide.tripType == ST.Rental then
    ...
    if getDistanceBwCordinates state.data.currentDriverLat state.data.currentDriverLon state.data.activeRide.dest_lat state.data.activeRide.dest_lon  > 0.200 then do
      let driveMode =  if state.props.currentStage == ST.RideAccepted && ((state.data.vehicleType == "AUTO_RICKSHAW" && state.data.cityConfig.cityName == "Chennai") || (state.data.vehicleType `DA.elem` ["BIKE", "DELIVERY_BIKE", "BIKE_PLUS", "E_RICKSHAW"])) then  "TWOWHEELER" else "DRIVE"
...
eval (LinkBusRouteToDriver action) state =  
  case action of
    PopUpModal.SelectRoute (RouteDisplayController.Select index busRouteNumber) ->
      let selectedRoute = state.data.whereIsMyBusData.availableRoutes >>= \(routes) -> routes Array.!! index
...
eval (ChooseBusBadge badges hasMoreDriver conductor hasMoreConductor) state = do 
  continue state {
      props {
        whereIsMyBusConfig {
          ...
          showLoadMoreForDrivers = hasMoreDriver,
          showLoadMoreForConductors = hasMoreConductor
        }
      }}
...
eval (OnSelectBusRoute action) state = 
  case action of
    RouteDisplayController.Click ->  
      let availableRoutesList = fromMaybe [] state.data.whereIsMyBusData.availableRoutes
...
eval (SelectBusRoute action) state = 
  case action of
    PopUpModal.OnButton2Click -> 
      continue state { props { whereIsMyBusConfig { selectBusRouteDropdown = false}}}
    PopUpModal.DismissPopup -> continue state { props { whereIsMyBusConfig { selectBusRouteDropdown = false}}}
...
eval (SelectBusDriver action) state = 
  case action of
    PopUpModal.OnButton2Click -> let _ = hideKeyboardOnNavigation true
      in continue state { props { whereIsMyBusConfig { selectBusDriverDropdown = false}}}
    PopUpModal.DismissPopup -> let _ = hideKeyboardOnNavigation true
      in continue state { props { whereIsMyBusConfig { selectBusDriverDropdown = false}}}
    PopUpModal.SearchableListAction searchListAction -> 
      case searchListAction of
         SearchableList.OnTextChange searchString ->
          continueWithCmd state { props { whereIsMyBusConfig { badgeSarchString = Just searchString }}} [ do
              void $ launchAff $ EHC.flowRunner defaultGlobalState $ do
                ...
                liftFlow $ case badgeResp of
                  Right badges ->
                    let transformedBadges = (getFleetBadgeDriver badges)
                    in push $ ChooseBusBadge transformedBadges (10 <= Array.length transformedBadges) (fromMaybe [] state.data.whereIsMyBusData.fleetConductor) state.props.whereIsMyBusConfig.showLoadMoreForConductors
                  _ -> push NoAction
              pure NoAction
            ]
         SearchableList.OnOptionClick option ->
            let _ = hideKeyboardOnNavigation true
            in continue state { props { whereIsMyBusConfig { selectedFleetDriverBadge = Just option.value, selectBusDriverDropdown = false } }}
         SearchableList.LoadMoreOptions -> continueWithCmd state [ do
              void $ launchAff $ EHC.flowRunner defaultGlobalState $ do
                badgeResp <- Remote.getWmbFleetBadges 10 (Array.length state.props.whereIsMyBusConfig.searchableListConfig.fleetBadgeDrivers) "DRIVER" state.props.whereIsMyBusConfig.badgeSarchString
                push <- liftFlow $ getPushFn Nothing "HomeScreen"
                liftFlow $ case badgeResp of
                  Right badges -> 
                    let transformedBadges = (getFleetBadgeDriver badges)
                    in push $ ChooseBusBadge (state.props.whereIsMyBusConfig.searchableListConfig.fleetBadgeDrivers <> transformedBadges) (10 <= Array.length transformedBadges) (fromMaybe [] state.data.whereIsMyBusData.fleetConductor) state.props.whereIsMyBusConfig.showLoadMoreForConductors
                  _ -> push NoAction
              pure NoAction
            ]
         SearchableList.OnFocus focus -> continue state
...
eval (SelectBusConductor action) state = 
  case action of
    PopUpModal.OnButton2Click -> let _ = hideKeyboardOnNavigation true
      in continue state { props { whereIsMyBusConfig { selectBusConductorDropdown = false}}}
    PopUpModal.DismissPopup -> let _ = hideKeyboardOnNavigation true
      in continue state { props { whereIsMyBusConfig { selectBusConductorDropdown = false}}}
    PopUpModal.SearchableListAction searchListAction -> 
      case searchListAction of
          ...
          continueWithCmd state { props { whereIsMyBusConfig { badgeSarchString = Just searchString }}} [ do
              void $ launchAff $ EHC.flowRunner defaultGlobalState $ do
                ...
                liftFlow $ case badgeResp of
                  Right badges -> 
                    let transformedBadges = (getFleetBadgeDriver badges)
                    in push $ ChooseBusBadge (fromMaybe [] state.data.whereIsMyBusData.fleetBadgeDrivers) state.props.whereIsMyBusConfig.showLoadMoreForDrivers transformedBadges (10 <= Array.length transformedBadges)
                  _ -> push NoAction
              pure NoAction
            ]
         SearchableList.OnOptionClick option ->
            let _ = hideKeyboardOnNavigation true
            in continue state { props { whereIsMyBusConfig { selectedFleetConductor = Just option.value, selectBusConductorDropdown = false } }}
         SearchableList.LoadMoreOptions -> continueWithCmd state [ do
              void $ launchAff $ EHC.flowRunner defaultGlobalState $ do
                badgeResp <- Remote.getWmbFleetBadges 10 0 "CONDUCTOR"  state.props.whereIsMyBusConfig.badgeSarchString
                push <- liftFlow $ getPushFn Nothing "HomeScreen"
                liftFlow $ case badgeResp of
                  Right badges -> 
                    let transformedBadges = getFleetBadgeDriver badges
                    in push $ ChooseBusBadge (fromMaybe [] state.data.whereIsMyBusData.fleetBadgeDrivers) state.props.whereIsMyBusConfig.showLoadMoreForDrivers (state.props.whereIsMyBusConfig.searchableListConfig.fleetConductor <> transformedBadges) (10 <= Array.length transformedBadges)
                  _ -> push NoAction
              pure NoAction
            ]
         SearchableList.OnFocus focus -> continue state
...
eval (ShowLinkPopup (API.AvailableRoutesList availableRoutesList) fleetBadgeDrivers fleetConductorBadge) state =
  if Array.null availableRoutesList || Array.null fleetBadgeDrivers then
    continue state
  else
    continue state 
      { data {
      ...
      , props {
          whereIsMyBusConfig {
            ...
            showLoadMoreForRoutes = 10 <= Array.length availableRoutesList,
            showLoadMoreForDrivers = 10 <= Array.length fleetBadgeDrivers,
            showLoadMoreForConductors = 10 <= Array.length fleetConductorBadge
          }
        }
      }
