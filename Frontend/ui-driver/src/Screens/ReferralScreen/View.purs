...
screen initialState =
            ...
            void $ launchAff $ flowRunner defaultGlobalState $ runExceptT $ runBackT
              $ do
                  case initialState.props.stage of
                    ST.LeaderBoard -> do
                      case initialState.props.leaderBoardType of
                        ST.Daily -> do
                          let
                            selectedDay =
                              if initialState.props.selectedDay.utcDate == "" then case last (getPastDays 1) of
                                Just day -> day
                                Nothing -> initialState.props.selectedDay
                              else
                                initialState.props.selectedDay
                        ...
                        ST.Weekly -> do
                          let
                            selectedWeek =
                              if any (_ == "") [ initialState.props.selectedWeek.utcStartDate, initialState.props.selectedWeek.utcEndDate ] then case last (getPastWeeks 1) of
                                Just week -> week
                                Nothing -> initialState.props.selectedWeek
                              else
                                initialState.props.selectedWeek
                    ...
                    ST.QRScreen -> do
                      let
                        refCode = getValueToLocalStore REFERRAL_CODE
  ...
  , eval:
      ( \action state -> do
          let
            _ = spy "Referral state -----" state
          let
            _ = spy "Referral--------action" action
          eval action state
      )
  }
              ...
              [ linearLayout
                  ...
                  ( [ linearLayout
                    ...
                    , linearLayout
                      ...
                      <> if state.props.stage == ST.SuccessScreen then
                          [ commonView push "ny_ic_green_tick" (getString YOUR_REFERRAL_CODE_IS_LINKED) (getString YOU_CAN_NOW_EARN_REWARDS) state ]
                        else
                          []
                            <> if state.props.stage == ST.ComingSoonScreen then
                                [ commonView push "ny_ic_comming_soon_poster" (getString COMING_SOON) (getString COMING_SOON_DESCRIPTION) state ]
                              else
                                []
                                  <> if state.props.stage == ST.ReferralFlow then
                                      [ referralEnrolmentFlow push state, continueButtonView push state ]
                                    else
                                      []
                                        <> if state.props.stage == ST.QRScreen then
    ...
    linearLayout
      ...
      [ linearLayout
          ...
          [ textView
              ...
              , height WRAP_CONTENT
              , text
                  $ if state.props.leaderBoardType == ST.Daily then
...
congratsBar state =
  let
    rank = state.props.currentDriverData.rank
  in
    linearLayout
      ( [ width MATCH_PARENT
        , height (V 44)
...
rankCard item aboveThreshold state =
  let
    currentDriverData = state.props.currentDriverData
...
topRankers state =
  let
    rankersList = state.props.rankersData
...
imageWrtGenderRank gender rank = case gender, rank of
  "MALE", 1 -> "ny_ic_rank1"
  "MALE", 2 -> "ny_ic_rank2"
  "MALE", 3 -> "ny_ic_rank3"
...
rankers size rank themeColor showCrown fontSize detail imageUrl =
  let
    bottomMargin = ceil ((toNumber size) / 7.0)
...
dateSelector push state =
  let
    leaderBoardType = state.props.leaderBoardType
  in
    linearLayout
      [ width MATCH_PARENT
      , height MATCH_PARENT
    ...
    , afterRender
        ( \action -> do
            if state.props.stage == ST.SuccessScreen then do
              void $ launchAff $ flowRunner defaultGlobalState $ runExceptT $ runBackT $ lift $ lift
                $ doAff do
                    if (os == "IOS") then
                      liftEffect $ startTimerWithTime (show state.props.seconds) state.props.id "1" push SuccessScreenExpireCountDwon
                    else
...
checkDriverWithZeroRides item aboveThreshold state =
  let
    currentDriverData = state.props.currentDriverData
  in
    aboveThreshold && (item == currentDriverData && currentDriverData.rank == 0)

checkDate :: ST.ReferralScreenState -> Boolean
checkDate state =
  if state.props.leaderBoardType == ST.Daily then
    (getcurrentdate "") == (convertUTCtoISC state.props.selectedDay.utcDate "YYYY-MM-DD")
  else
    (getcurrentdate "") <= (convertUTCtoISC state.props.selectedWeek.utcEndDate "YYYY-MM-DD")
