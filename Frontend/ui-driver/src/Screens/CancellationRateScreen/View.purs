...
view push state = 
  let configs = reduceCancellationRate "cancellationRateConfigs"
  in
    linearLayout
    [ height MATCH_PARENT
    , width MATCH_PARENT
...
cancellationRateData state push =
  linearLayout
    ...
    [ linearLayout
      ...
      [ infoTileView state { primaryText: show state.data.cancelledRides, secondaryText: "/" <> show state.data.assignedRides, subText: getString RIDES_CANCELLED, margin: Margin 0 0 0 0 }
      , linearLayout
          [ height MATCH_PARENT
          , width (V 1)
          ...
          []
      , infoTileView state { primaryText: "₹ " <> EHC.formatCurrencyWithCommas (show state.data.missedEarnings), secondaryText: "", subText: getString EARNINGS_MISSED, margin: Margin 0 0 0 0 }
      ]
    ]
