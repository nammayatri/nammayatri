...
import Helpers.Utils (fetchImage, FetchImageFrom(..), getPastYears, getVehicleVariant)
...
inputTextConfig state =
  { data:
    { title : if state.data.inputTextState.component == ST.Pledge then getString PLEDGE else getString $ WHY_NY $ getVehicleVariant state.data.vehicleType
    , doneButtonConfig:
