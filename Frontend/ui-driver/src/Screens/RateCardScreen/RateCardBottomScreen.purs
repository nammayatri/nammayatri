...
type State = { serviceTier :: Maybe String
...
view push state =
    ...
    [ Anim.fadeInWithDuration 100 true
    ]$
    linearLayout
     [ height MATCH_PARENT
     , width MATCH_PARENT
...
getNightCharges state = 
  let fromRateCard = foldr (\acc item -> if DS.contains (DS.Pattern "daytime charges") item then acc <> item else acc) "" state.rateCardData.fareInfoDescription
  in if fromRateCard == "" then "1.5x of daytime charges applicable at night from 10 PM to 5 AM" else fromRateCard


getUpdatedFareList :: State -> Array {key:: String , val ::String}
getUpdatedFareList state =
  let cur = getCurrency appConfig
  in
    (case state.ridePreference.perKmRate of
      Just fare ->  [ { key : "Per Mile Fare"
      , val : cur <> show fare <> "/mi"
      }
      ]
      Nothing -> [] )
      <> (case state.ridePreference.perMinRate of
            Just fare ->  [ { key : "Per Minute Fare"
            , val : cur <> show fare <> "/mi"
            }
            ]
            Nothing -> [])
