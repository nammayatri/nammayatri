...
reEnterLicenceNumber state push = 
  ...
  , visibility if state.data.dateOfIssue /= Nothing then GONE else VISIBLE
   ...
   , textView
      ...
      , visibility $ if (DS.toLower(state.data.driver_license_number) /= DS.toLower(state.data.reEnterDriverLicenseNumber) && (DS.length state.data.reEnterDriverLicenseNumber /= 0)) then VISIBLE else GONE
      , color Color.red
      , margin (MarginBottom 10)
      ]
 ]
