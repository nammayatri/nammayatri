...
frontUploadSection state push =
  ...
  [ width MATCH_PARENT
    ...
    , clickable $ isStrEmpty  state.data.imageFront
    , visibility if state.data.dateOfIssue /= Nothing then GONE else VISIBLE
  ][
    textView
  ...
  , linearLayout
    ...
    , stroke ("1," <> Color.borderGreyColor)
      ...
      textView
      ([ text if (isStrEmpty  state.data.imageFront) then (getString UPLOAD_FRONT_SIDE) else state.data.imageNameFront
...
backUploadSection state push = 
  ...
  , clickable $ isStrEmpty state.data.imageBack
  ][
    textView
  ...
  , linearLayout
    ...
    , stroke ("1," <> Color.borderGreyColor)
      ...
      textView
      ([ text if (isStrEmpty state.data.imageBack) then (getString UPLOAD_BACK_SIDE) else state.data.imageNameBack
  ...
  , linearLayout
    ...
    , stroke ("1," <> Color.borderGreyColor)
      ...
      [ width MATCH_PARENT
        ...
        ([ text if (isStrEmpty state.data.dob) then (getString SELECT_DATE_OF_BIRTH) else state.data.dobView
        , color if (isStrEmpty state.data.dob) then Color.darkGrey else Color.greyTextColor
        , weight 1.0
        , padding (PaddingRight 15)
        ] <> FontStyle.subHeading1 TypoGraphy)
      , imageView
