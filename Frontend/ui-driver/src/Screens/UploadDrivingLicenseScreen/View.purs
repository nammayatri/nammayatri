...
screen initialState =
  ...
  , eval:
      \action state -> do
        let
          _ = printLog "UploadDrivingLicenseScreen state -----" state
        ...
        ( [ linearLayout
          ...
          , if state.props.openLicenseManual then
              linearLayout
                ...
                [ TutorialModal.view (push <<< TutorialModalAction) { imageUrl: fetchImage FF_ASSET "ny_ic_driver_license_card" } ]
            else
              linearLayout [] []
          , if state.props.openDateOfIssueManual then
              linearLayout
                ...
                [ TutorialModal.view (push <<< TutorialModalAction) { imageUrl: fetchImage FF_ASSET "ny_ic_date_of_issue" } ]
            else
              linearLayout [] []
          , if state.props.openGenericMessageModal then
              linearLayout
                ...
                [ GenericMessageModal.view (push <<< GenericMessageModalAction) { text: (getString ISSUE_WITH_DL_IMAGE), openGenericMessageModal: state.props.openGenericMessageModal, buttonText: (getString NEXT) } ]
            else
              linearLayout [] []
          ]
            <> if state.props.logoutPopupModal then
  ...
  in
    linearLayout
      ...
      , linearLayout
          ...
          , if not (DSC.null state.data.imageFront) then
...
backUploadSection state push =
  linearLayout
    ...
    , linearLayout
        ...
        , if not (DSC.null state.data.imageBack) then
...
validateProfilePictureModal push state = ValidateDocumentModal.view (push <<< ValidateDocumentModalAction) (validateProfilePictureModalState state)
  ...
  let
    status =
      if state.props.validating then
        ST.InProgress
      else if not (DSC.null state.data.errorMessage) then
        ST.Failure
      else if (not (DSC.null state.data.imageIDFront)) || (not (DSC.null state.data.imageIDBack)) then
        ST.Success
      else
        ST.None
