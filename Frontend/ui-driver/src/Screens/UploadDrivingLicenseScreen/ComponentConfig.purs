        ...
        { textConfig
          { text =
            if isJust state.data.dateOfIssue then
              ...
              getString UPLOAD_PHOTO
            else
              getString UPLOAD_DRIVING_LICENSE
          }
        , width = MATCH_PARENT
...
primaryEditTextConfigReEnterDl state =
  let
    config = PrimaryEditText.config
  ...
  AppOnboardingNavBar.config
    ...
    , headerTextConfig =
      AppOnboardingNavBar.config.headerTextConfig
        { text =
          if state.props.openHowToUploadManual then
            getString UPLOAD_DRIVING_LICENSE
          else
            getString DRIVING_LICENSE_DETAILS
        }
    }
