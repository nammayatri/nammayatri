...
primaryEditTextConfigReEnterDl state = let 
    ...
    primaryEditTextConfig' = config
      ...
      , stroke = if (DS.toLower(state.data.driver_license_number) /= DS.toLower(state.data.reEnterDriverLicenseNumber) && (DS.length state.data.reEnterDriverLicenseNumber /= 0)) then ("1," <> Color.red) else ("1," <> Color.borderColorLight)
      , topLabel
