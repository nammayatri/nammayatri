...
updateVehicleDetailsFields state push =
  scrollView
    ...
    [ linearLayout
        ...
        ( map
            ( \optionItem ->
                if (optionItem.title == VehicleType) then
                  vehicleTypeView push state
                else if (optionItem.title == VehicleRC) then
                  uploadRCView state push
                else
                  PrimaryEditText.view (push <<< PrimaryEditTextActionController) (primaryEditTextConfig (getTitle optionItem.title) (getValue optionItem.title state))
            )
            viewsItemList
        )
    ]
