...
eval (RideAllocationModalAction (RideAllocationModal.Decline id)) state = do
  let
    rides = Array.filter (\ride -> ride.id /= id) state.data.availableRides
  if (Array.length rides) > 0 then do
    let
      newState = state { data = state.data { availableRides = rides } }
    continue newState
  else do
...
eval (RideAllocationModalAction (RideAllocationModal.CountDown seconds id status timerID)) state = do
  ...
  if status == "EXPIRED" then do
    ...
    let
      newState = state { data = state.data { availableRides = map (\ride -> if ride.id == id then ride { timer = seconds } else ride) state.data.availableRides } }
    continue newState

eval (RideAllocationModalAction (RideAllocationModal.IncreasePrice id)) state = do
  let
    newState = state { data = state.data { availableRides = map (\ride -> if ride.id == id then ride { totalAmount = ride.totalAmount + ride.increasePrice } else ride) state.data.availableRides } }
  continue newState

eval (RideAllocationModalAction (RideAllocationModal.DecreasePrice id)) state = do
  let
    newState = state { data = state.data { availableRides = map (\ride -> if ride.id == id then ride { totalAmount = ride.totalAmount - ride.decreasePrice } else ride) state.data.availableRides } }
  continue newState

eval (PopUpCallBack stringifyPayload) state = do
  _ <- pure $ printLog "stringify payload" stringifyPayload
  let
    entityPayload = (objectToAllocationType stringifyPayload)
