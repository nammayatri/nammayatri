...
data ScreenOutput
  ...
  | GoToHome
  | LogoutAccount
  | GoToRegisteration PermissionsScreenState
    ...
    [ do
        ...
        pure
          $ UpdateAllChecks
              state
                { props
                  { isNotificationPermissionChecked = isNotificationPermissionChecked
...
eval (NotificationPermissionCallBack isNotificationPermissionEnabled) state = do
  if isNotificationPermissionEnabled then do
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "permission_granted_notification"
...
eval (OverlayPermissionSwitchCallBack isOverlayPermissionEnabled) state = do
  if isOverlayPermissionEnabled then do
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "permission_granted_overlay"
...
eval (BatteryUsagePermissionCallBack isBatteryOptimizationEnabled) state = do
  if isBatteryOptimizationEnabled then do
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "permission_granted_battery"
...
eval (LocationPermissionCallBack isLocationPermissionEnabled) state = do
  let
    newState = state { props { isLocationPermissionChecked = isLocationPermissionEnabled } }
  if isLocationPermissionEnabled then do
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "permission_granted_location"
    continue newState
  else
    continue newState

eval (ItemClick itemType) state = case itemType of
  Notifications -> do
    if not (state.props.isNotificationPermissionChecked) then do
      continueWithCmd state
        [ do
            isNotificationPermission <- isNotificationPermissionEnabled unit
            if (isNotificationPermission) then
              pure UpdateNotificationPermissionState
            else do
  ...
  Overlay -> do
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "permission_btn_click_overlay"
    if not (state.props.isOverlayPermissionChecked) then do
      continueWithCmd state
        [ do
            isOverlayPermission <- isOverlayPermissionEnabled unit
            if isOverlayPermission then
              pure UpdateOverlayPermissionState
            else do
  ...
  LocationPermission -> do
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "permission_btn_click_location"
    if not (state.props.isLocationPermissionChecked) then do
      continueWithCmd state
        [ do
            isLocationPermission <- isLocationPermissionEnabled unit
            if isLocationPermission then
  ...
  AutoStart -> do
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "permission_btn_click_autostart"
    if not (state.props.isAutoStartPermissionChecked) then do
      continueWithCmd state { props { isAutoStartPermissionChecked = true } }
        [ do
  ...
  Battery -> do
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "permission_btn_click_battery"
    if not (state.props.isBatteryOptimizationChecked) then do
      continueWithCmd state
        [ do
            isBatteryUsagePermission <- isBatteryPermissionEnabled unit
            if (isBatteryUsagePermission) then
              pure UpdateBatteryPermissionState
            else do
...
getTitle permission = case permission of
...
getDescription permission = case permission of
