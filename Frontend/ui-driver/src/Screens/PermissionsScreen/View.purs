...
screen initialState =
  ...
  , eval:
      ( \state action -> do
          let
            _ = spy "PermissionsScreen ----- state" state
          let
            _ = spy "PermissionsScreen --------action" action
          eval state action
      )
  }
  ...
  linearLayout
    ...
    [ linearLayout
        ...
        ( map
            ( \item ->
                let
                  isPermissionEnabled = case item.permission of
            ...
            ( permissionsList state
                <> if (state.props.androidVersion > 12 && state.data.config.permissions.notification) then
                    [ { permission: Notifications, icon: "" } ]
                  else
                    []
            )
        )
