...
import PrestoDOM (Gravity(..), Length(..), Margin(..), Orientation(..), Padding(..), PrestoDOM, Screen, Visibility(..),background, color, fontStyle, gravity, height,  margin, orientation, padding, text, textSize, textView, width, visibility, cornerRadius)
...
alternateMobileNumberConfig state = let 
  ...
  referalNumberConfig' = config'{
    ...
    , letterSpacing = if  state.props.enterOtp then 5.0 else 0.0
