      ...
      [ ( \push -> do
            if screenType == "StatusScreen" then do
              void $ launchAff $ EHC.flowRunner defaultGlobalState $ runExceptT $ runBackT
                $ do
                    if (initialState.props.enterMobileNumberView || initialState.props.enterOtp) then
                      pure unit
                    else do
...
drivingLicenseCardDetails state =
  ...
  , "image":
      fetchImage FF_ASSET
        $ case state.data.dlVerificationStatus of
...
vehicleCardDetails state =
  ...
  , "image":
      fetchImage FF_ASSET
        $ case state.data.rcVerificationStatus of
            "VALID" -> "ny_ic_check_mark"
            "PENDING" -> "ny_ic_pending"
            "FAILED" -> "ny_ic_api_failure_popup"
            "NO_DOC_AVAILABLE" -> "ny_ic_help_circle"
