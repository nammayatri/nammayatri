...
data ScreenOutput
  ...
  | GoToEnterOtp ApplicationStatusScreenState
  | AddMobileNumber ApplicationStatusScreenState
...
eval (CompleteOnBoardingAction PrimaryButtonController.OnClick) state = do
  let
    timelimit = (getExpiryTime (getValueToLocalStore INVALID_OTP_TIME) true) / 60
  if state.props.onBoardingFailure then
    continue state { props { popupview = true } }
  else do
    if not (timelimit <= 10) || (getValueToLocalStore INVALID_OTP_TIME == "__failed") then
      continue state { props { enterMobileNumberView = true, isValidOtp = false, isAlternateMobileNumberExists = false, isValidAlternateNumber = false }, data { mobileNumber = "" } }
    else
...
eval (DriverRegistrationStatusAction (DriverRegistrationStatusResp resp)) state = do
  if (resp.dlVerificationStatus == "VALID" && resp.rcVerificationStatus == "VALID") then
    ...
    let
      popup_visibility = any (_ == resp.dlVerificationStatus) [ "FAILED", "INVALID" ] || any (_ == resp.rcVerificationStatus) [ "FAILED", "INVALID" ]
    let
      onBoardingStatus = any (_ == resp.dlVerificationStatus) [ "LIMIT_EXCEED", "NO_DOC_AVAILABLE" ] || any (_ == resp.rcVerificationStatus) [ "LIMIT_EXCEED", "NO_DOC_AVAILABLE" ]
    let
      timeDifference = (getExpiryTime (getValueToLocalStore DOCUMENT_UPLOAD_TIME) true) / 3600
    if (timeDifference >= 48 && (resp.dlVerificationStatus == "PENDING" || resp.rcVerificationStatus == "PENDING")) then
      continue state { data { dlVerificationStatus = resp.dlVerificationStatus, rcVerificationStatus = resp.rcVerificationStatus }, props { onBoardingFailure = true, isVerificationFailed = false } }
    else
      continue state { data { dlVerificationStatus = resp.dlVerificationStatus, rcVerificationStatus = resp.rcVerificationStatus }, props { onBoardingFailure = onBoardingStatus, isVerificationFailed = popup_visibility } }
...
eval (AlternateMobileNumberAction (ReferralMobileNumberController.OnBackClick)) state = do
  if state.props.enterOtp then do
    continueWithCmd state { props { enterOtp = false, buttonVisibilty = true } }
      [ do
          _ <- pure $ setText (getNewIDWithTag "Referalnumber") state.data.mobileNumber
          pure Dummy
      ]
  else
    continue state { props { enterMobileNumberView = false } }
...
eval (AlternateMobileNumberAction (ReferralMobileNumberController.PrimaryButtonActionController (PrimaryButtonController.OnClick))) state = do
  if state.props.enterOtp then
    exit $ AddMobileNumber state
  else do
    continueWithCmd state
      [ do
...
eval (AlternateMobileNumberAction (ReferralMobileNumberController.PrimaryEditTextActionController (PrimaryEditTextController.TextChanged valId newVal))) state = do
  if state.props.enterOtp then do
    if length newVal == 4 then do
      ...
      continue state { props { buttonVisibilty = false, isValidOtp = false } }
  else do
    _ <-
      if length newVal >= 10 then do
        ...
        pure unit
    let
      isValidMobileNumber = case (charAt 0 newVal) of
        Just a ->
    ...
    if (length newVal == 10 && isValidMobileNumber) then do
      continue
        state
          { props
            { buttonVisibilty = if (length newVal == 10 && isValidMobileNumber && not state.props.isAlternateMobileNumberExists) then true else false
            , isValidAlternateNumber = not isValidMobileNumber
            }
          , data { mobileNumber = if (length newVal == 10 && isValidMobileNumber) then newVal else "" }
          }
    else
      continue
        state
          { props { isValidAlternateNumber = not isValidMobileNumber, buttonVisibilty = false, isAlternateMobileNumberExists = false }
          }
