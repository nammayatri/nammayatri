...
eval BackPressed state =
  if state.props.showPaymentHistory then
    continue state { props { showPaymentHistory = false } }
  else
    exit GoBack
...
eval (ScrollStateChanged scrollState) state = do
  _ <- case scrollState of
    SCROLL_STATE_FLING -> pure $ setEnabled "2000030" false
    _ -> pure unit
  continue state
...
eval (BottomNavBarAction (BottomNavBar.OnNavigate screen)) state = do
  case screen of
    ...
    "Alert" -> do
      ...
      let
        _ = unsafePerformEffect $ logEvent state.logField "ny_driver_alert_click"
...
eval (IndividualRideCardAction (IndividualRideCardController.Select index)) state = do
  let
    filteredRideList = rideListFilter state.currentTab state.rideList
...
eval (RideHistoryAPIResponseAction rideList) state = do
  let
    bufferCardDataPrestoList = (rideHistoryListTransformer rideList)
  let
    filteredRideList = (rideListResponseTransformer rideList)
  _ <- pure $ setRefreshing "2000030" false
  let
    loadBtnDisabled = if (length rideList == 0) then true else false
...
eval (DatePickerAC (DatePickerModel.OnDateSelect idx item)) state = do
  let
    newState = state { datePickerState { activeIndex = idx, selectedItem = item }, rideList = [], prestoListArrayItems = [] }
...
eval (PaymentHistoryModelAC (PaymentHistoryModel.PaymentHistoryListItemAC (PaymentHistoryModelItem.OnClick id))) state = do
  let
    updatedData = map (\item -> if item.id == id then item { isSelected = not item.isSelected } else item) state.data.paymentHistory.paymentHistoryList
...
rideHistoryListTransformer list =
  ( map
      ( \(RidesInfo ride) ->
          let
            accessibilityTag = (getDisabilityType ride.disabilityTag)
          ...
          in
            ...
            , rideDistance:
                toPropValue $ (parseFloat (toNumber (fromMaybe 0 ride.chargeableDistance) / 1000.0) 2) <> " km Ride"
                  <> case ride.riderName of
                      Just name -> " with " <> name
                      Nothing -> ""
