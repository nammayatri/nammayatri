...
recentRideDetails state push =
  let
    vw = (toNumber (screenWidth unit)) / 100.0
  in
    linearLayout
      [ width MATCH_PARENT
      , height WRAP_CONTENT
...
categoryView categoryGroup marginLeft marginRight state push =
  ...
  in
    linearLayout
      ...
      , onClick push
          ( case categoryGroup.categoryAction of
...
issueListModal push state = IssueListFlow.view (push <<< IssueScreenModal) (issueListState state)
