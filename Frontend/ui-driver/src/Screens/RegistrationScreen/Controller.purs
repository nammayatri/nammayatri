...
eval (CallHV workFLowId inputJson) state = 
  continueWithCmd state 
    [ do
        ...
        hvResult <- runEffectFn6 JB.initHVSdk state.data.accessToken workFLowId transactionId true "en" inputJson 
        if hvResult then pure NoAction else pure UpdateApkAction
    ]
...
eval UpdateApkAction state = exit $ GoToAppUpdatePopUpScreen state
