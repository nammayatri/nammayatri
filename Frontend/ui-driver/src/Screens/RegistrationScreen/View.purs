...
view push state =
  ...
  in
    Anim.screenAnimation
      ...
      $ [ chooseVehicleView push state
        , linearLayout
            ...
            , linearLayout
                ...
                , visibility $ boolToVisibility $ statusForOnboardingCompletion state
                ]
                []
            , linearLayout
                [ height WRAP_CONTENT
      ...
      <> if state.props.logoutModalView || state.props.confirmChangeVehicle then [ popupModal push state ] else []
      ...
      <> if state.props.menuOptions then 
            [ linearLayout 
                ...
                ][ OptionsMenu.view (push <<< OptionsMenuAction) (optionsMenuConfig state) ]] else []
  ...
  linearLayout
    ...
    ,  linearLayout
      ...
      , weight 1.0
          ...
          [ text item.stageName -- $ Constant.transformDocText item.stage
          , color Color.black800
          ] <> FontStyle.body1 TypoGraphy
        , textView $
...
popupModal push state =
    ...
    where 
      action = if state.props.logoutModalView then PopUpModalLogoutAction else ChangeVehicleAC
      popupConfig = if state.props.logoutModalView then (logoutPopUp Language) else changeVehicleConfig state
...
enterReferralCode push state =
  let allStepsCompleted = statusForOnboardingCompletion state
    in linearLayout
            [ width MATCH_PARENT
            , height WRAP_CONTENT
            , orientation HORIZONTAL
    ...
    _ -> do
          let documentStatusArr = state.data.documentStatusList
              ...
              findStatus = if step `elem` vehicleDoc 
                          then find (\docStatus -> docStatus.docType == step && docStatus.vehicleType == state.data.vehicleCategory) documentStatusArr
...
statusForOnboardingCompletion :: ST.RegistrationScreenState -> Boolean
statusForOnboardingCompletion state = state.props.driverEnabled --all (_ == COMPLETED) $ [ state.data.vehicleDetailsStatus, state.data.drivingLicenseStatus, state.data.permissionsStatus ] -- Decide on case of cabs