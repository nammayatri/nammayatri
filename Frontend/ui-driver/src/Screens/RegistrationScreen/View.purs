...
screen initialState =
  ...
  , eval:
      ( \state action -> do
          let
            _ = spy "RegistrationScreen ----- state" state
          let
            _ = spy "RegistrationScreen --------action" action
          eval state action
      )
  }
...
view push state =
  let
    isShowSubscriptions = getValueToLocalNativeStore SHOW_SUBSCRIPTIONS == "true"
...
getStatus step state = case step of
...
messageView push state =
  let
    isHidden = all (\x -> x `elem` [ NOT_STARTED, COMPLETED ]) [ state.data.vehicleDetailsStatus, state.data.drivingLicenseStatus ] && isNothing state.props.limitReachedFor
  in
    linearLayout
      [ width MATCH_PARENT
      , height WRAP_CONTENT
      ...
      [ textView
          $ [ text
                if isJust state.props.limitReachedFor then case state.props.limitReachedFor of
                  Just "RC" -> getString RC_UPLOAD_LIMIT_REACHED
                  Just "DL" -> getString DL_UPLOAD_LIMIT_REACHED
                  _ -> ""
                else if state.data.vehicleDetailsStatus == FAILED && state.data.drivingLicenseStatus == FAILED then
                  getString RC_AND_DL_UPLOAD_FAILED
                else if state.data.vehicleDetailsStatus == FAILED then
                  getString RC_UPLOAD_FAILED
                else if state.data.drivingLicenseStatus == FAILED then
                  getString DL_UPLOAD_FAILED
                else
      ...
      , textView
          $ [ text
                if state.data.vehicleDetailsStatus == FAILED || state.data.drivingLicenseStatus == FAILED then
                  getString PLEASE_RETRY_THE_UPLOAD_AGAIN
                else
                  state.data.lastUpdateTime
            , gravity CENTER
            , margin $ MarginLeft 6
...
checkLimitReached step limitReachedFor = case limitReachedFor of
