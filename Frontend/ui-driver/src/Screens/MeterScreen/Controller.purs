...
eval DestinationClear state = do
  if (state.props.isSearchLocation /= ST.LocateOnMap) then do
    ...
    pure unit
  continue state { data { destination = "", destinationAddress = dummyAddress}, props {  isRideServiceable = true, searchLocationModelProps{crossBtnDestVisibility = false }} }


eval (DebounceCallBack searchString _) state = do
  let isMapSearchLocation = any (_ == state.data.isSearchLocation) [ST.LocateOnMap, ST.RouteMap]
  if (STR.length searchString > 2) && not isMapSearchLocation then
...
eval (VoiceToText text) state = do
  pure $ setText (getNewIDWithTag "DestinationEditTextMeterSreen") text
  continue state
