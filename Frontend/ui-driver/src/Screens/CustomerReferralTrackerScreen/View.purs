...
import Screens.CustomerReferralTrackerScreen.ScreenData (CustomerReferralTrackerScreenState, DailyEarning(..), PayoutStatus(..), Stage(..), dummyDateItem)
                ...
                $ do
                    if initialState.props.callEarningsAPI then do
                      let
                        ...
                        reqDate = if firstDate == "" || lastDate == "" then [] else if firstDate == lastDate then [firstDate] else [firstDate, lastDate]
                      (GetRidesSummaryListResp rideSummaryResp) <- Remote.getRideSummaryListReqBT reqDate
...
view push state =
  relativeLayout
    ...
    ] <> if state.props.showMenu && state.data.currentStage == Tracker then [menuView push state] else []
...
addUPIView push state = 
  ...
  , visibility $ boolToVisibility $ isNothing state.data.upiID
  ][ linearLayout
      [ height WRAP_CONTENT
      , width WRAP_CONTENT
      ][ linearLayout
         ...
         , margin $ Margin 16 16 16 16
          ...
          , linearLayout
            ...
            , background Color.orange900
               ...
               [ text $ (getString $ PAY_TO_ADD_UPI "₹1") <> " →"
...
paymentPendingView push state = 
  ...
  , margin $ MarginBottom 16
    ...
    , width WRAP_CONTENT
      ...
      , imageView
        [ imageWithFallback $ fetchImage COMMON_ASSET "ny_ic_payment_pending"
        , height $ V 96
        , width $ V 84
        , margin $ MarginRight 16
        ]
...
emptyReferralView push state = 
  ...
  , visibility $ boolToVisibility $ state.data.referralCount < 1
   ...
   , textView $ 
     [ text $ getString $ EARN_FOR_EACH_REFERRAL "₹100"
     , width MATCH_PARENT
     , margin $ MarginTop 28
     , gravity CENTER
...
upiDetailsView push state = 
  ...
  , background "#F4F7FF"
  , orientation VERTICAL
  , visibility $ boolToVisibility $ isJust state.data.upiID
  ][ textView $ 
    [ text $ "UPI ID"
...
addUPIButton push state = 
  ...
  ][ PrimaryButton.view (push <<< AddUPIAction) (addUPIDetailsButtonConfig state)]
...
infoCardView push state = 
  ...
  , gravity CENTER
  , clickable false
  ][ PopUpModal.view (push <<< PopUpModalAction) (paymentSuccessConfig state) ]
...
transactionHistoryView push state = 
  ...
  , gravity CENTER_HORIZONTAL
   ...
   , textView $
     [ text $ getString $ RECEIVED "₹320"
     , color Color.black800
     ] <> FontStyle.h2 TypoGraphy
   , textView $ 
     [ text $ getString $ CREDITED_ON "26 Dec 2021,5:50 PM"
     , color Color.black700
     , margin $ MarginTop 8
     ] <> FontStyle.body3 TypoGraphy
   , linearLayout
  ...
  , visibility $ boolToVisibility $ state.data.referralCount > 0
    ...
    , linearLayout
      ...
      , textView $ 
        [ height WRAP_CONTENT
        , width WRAP_CONTENT
        , text $ "₹" <> (formatCurrencyWithCommas (show state.data.totalEarningsData.totalEarnings))
        , color Color.black900
    ...
    , margin $ MarginBottom 12
     ...
     , linearLayout
       ...
       , gravity RIGHT
          ...
          [ text $ "₹" <> formatCurrencyWithCommas (show $ item.earnings)
          , color priceColor
          , gravity RIGHT
          ] <> FontStyle.body6 TypoGraphy
        , textView $ 
...
payoutStatusItem push item = 
  let textColor = if item.status == Verifying then Color.orange900 else if item.status == Processing then Color.blue800 else if item.status == Failed then Color.red900 else Color.black700
      color' = if item.status == Verifying then "#1AFF8533" else if item.status == Processing then Color.blue600 else if item.status == Failed then "#14E55454" else Color.white900
  in
  linearLayout
  [ height WRAP_CONTENT
  , width WRAP_CONTENT
    ...
    , orientation VERTICAL
      ...
      , gravity CENTER
         ...
         , width WRAP_CONTENT
            ...
            , background if selectedIndex < 0 || selectedIndex == index then Color.blue800 else "#1A2194FF"
            , cornerRadius 4.0
            ][]
          ]
      ]
