  ...
  let
    ...
    requestInfoCardConfig' =
      config
        ...
        , coverImageConfig
          { imageUrl = HU.fetchImage HU.COMMON_ASSET "ny_failed"
          , height = V 200
          , width = MATCH_PARENT
          , visibility = VISIBLE
...
referralStepsViewConfig state = ReferralStepsView.config{referralSteps = getStepsConfig state.data.selectedItem, highlightTitle = isNothing state.data.selectedItem}
...
getStepsConfig mbEarning = case mbEarning of 
                            Just earning -> case earning.status of 
                                            Verifying -> (take 2 doneStepsData) <> [fromMaybe dummyStepConfig (inProgressData !! 2)] <> drop 3 pendingStepsData
                                            Processing -> (take 3 doneStepsData) <> [fromMaybe dummyStepConfig (inProgressData !! 3)] <> drop 4 pendingStepsData
                                            _ -> []
                            Nothing -> pendingStepsData


pendingStepsData :: Array ReferralStepsView.StepConfig
pendingStepsData = 
  [ { icon : "ny_ic_corner_up_right_grey"
    , title : getString REFER_CUSTOMER
    , background : Color.grey700
    , status : ReferralStepsView.Pending
    }
  , { icon : "ny_ic_car_outline_grey"
    , title : getString CUSTOMER_COMPLETED_FIRST_RIDE
    , background : Color.grey700
    , status : ReferralStepsView.Pending
    }
  , { icon : "ny_ic_search_grey"
    , title : getString VERIFYING
    , background : Color.grey700
    , status : ReferralStepsView.Pending
    }
  , { icon : "ny_ic_more_horizontal_grey"
    , title : getString PROCESSING
    , background : Color.grey700
    , status : ReferralStepsView.Pending
    }
  , { icon : "ny_ic_payment_credited_grey"
...
doneStepsData :: Array ReferralStepsView.StepConfig
doneStepsData = 
  [ { icon : "ny_ic_corner_up_right_white"
    , title : getString REFER_CUSTOMER
    , background : Color.green900
    , status : ReferralStepsView.Done
    }
  , { icon : "ny_ic_car_outline_white"
    , title : getString CUSTOMER_COMPLETED_FIRST_RIDE
    , background : Color.green900
    , status : ReferralStepsView.Done
    }
  , { icon : "ny_ic_search_white"
    , title : getString VERIFYING
    , background : Color.green900
    , status : ReferralStepsView.Done
    }
  , { icon : "ny_ic_more_horizontal_white"
    , title : getString PROCESSING
    , background : Color.green900
    , status : ReferralStepsView.Done
    }
...
inProgressData :: Array ReferralStepsView.StepConfig
inProgressData =
  [ { icon : "ny_ic_corner_up_right_white"
    , title : getString REFER_CUSTOMER
...
dummyStepConfig :: ReferralStepsView.StepConfig
dummyStepConfig = 
  { icon : "ny_ic_corner_up_right_grey"
  , title : getString REFER_CUSTOMER
  , background : Color.grey700
