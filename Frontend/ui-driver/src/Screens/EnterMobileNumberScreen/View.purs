...
import PrestoDOM (Gravity(..), Length(..), LetterSpacing(..), Margin(..), Orientation(..), Padding(..), PrestoDOM, LoggableScreen, Visibility(..), alpha, background, clickable, color, cornerRadius, frameLayout, gravity, height, imageUrl, imageView, linearLayout, margin, onBackPressed, onClick, orientation, padding, stroke, text, textView, visibility, weight, width, afterRender, imageWithFallback, singleLine, textFromHtml)
...
screen :: ST.EnterMobileNumberScreenState -> LoggableScreen Action ST.EnterMobileNumberScreenState ScreenOutput
screen initialState =
  { initialState
  , view
  ...
  , eval: (\action state -> do
      ...
      eval action state)
  , parent : Nothing
  , logWhitelist : initialState.data.config.logWhitelistConfig.enterMobileNumberScreenLogWhitelist
  }
