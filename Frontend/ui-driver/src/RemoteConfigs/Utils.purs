...
import RemoteConfig.Types (ReelItem, ReelButtonConfig, HVConfigs, ReferralPopUpDelays, CancellationRateConfig, CancellationRateEntity(..), CancellationThresholdConfig, MetroCoinsEvent)
...
reelsData key =
  let
    reelDataString = getReelsData $ fetchRemoteConfigString key
  in
    decodeForeignAny reelDataString defaultReelsData
...
reduceCancellationRate key =
  let
    cancellationDataString = fetchRemoteConfigString key
  in
    decodeForeignAny (parseJSON cancellationDataString) []
...
getReferralPopUpDelays popUpType = do
  let
    config = fetchRemoteConfigString "referral_pop_up_delays"
...
getReferralBonusVideo city =
  let
    config = fetchRemoteConfigString "referral_bonus_videos"
...
getMetroCoinsEvent city = do
  let
    config = fetchRemoteConfigString "metro_coins_event"
