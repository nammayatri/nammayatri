...
onBoardingFlow = do
  ...
  GlobalState allState <- getState
  useHVDlSdkEnabled <- pure $ fetchRemoteConfigString "use_hv_dl_sdk" == "true"
  DriverRegistrationStatusResp driverRegistrationResp <- driverRegistrationStatusBT $ DriverRegistrationStatusReq true useHVDlSdkEnabled
  let cityConfig = getCityConfig config.cityConfig (getValueToLocalStore DRIVER_LOCATION)
          ...
          case registerDriverRCResp of
            Right (API.ApiSuccessResult resp) -> do
              ...
              if (not addRcFromProf) then do
                ...
                useHVDlSdkEnabled <- pure $ fetchRemoteConfigString "use_hv_dl_sdk" == "true"
                (DriverRegistrationStatusResp resp ) <- driverRegistrationStatusBT $ DriverRegistrationStatusReq true useHVDlSdkEnabled
