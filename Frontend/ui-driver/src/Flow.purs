  ...
  case flow of
      ...
      case validateImageResp of
       Right (ValidateImageRes resp) -> do
        ...
        if (state.data.rcImageID == "IMAGE_NOT_VALIDATED") then do
          ...
          registerDriverRCResp <- lift $ lift $ Remote.registerDriverRC (makeDriverRCReq state.data.vehicle_registration_number resp.imageId state.data.dateOfRegistration true state.data.vehicleCategory state.data.airConditioned state.data.oxygen state.data.ventilator)
    ...
    VALIDATE_RC_DATA_API_CALL state -> do
        ...
        if (state.data.rcImageID == "IMAGE_NOT_VALIDATED") then do
          ...
          registerDriverRCResp <- lift $ lift $ Remote.registerDriverRC (makeDriverRCReq state.data.vehicle_registration_number state.data.rcImageID state.data.dateOfRegistration true state.data.vehicleCategory state.data.airConditioned state.data.oxygen state.data.ventilator)
          void $ pure $ setValueToLocalStore ENTERED_RC state.data.vehicle_registration_number
          case registerDriverRCResp of
            Right (DriverRCResp resp) -> do
              _ <- pure $ toast $ getString RC_ADDED_SUCCESSFULLY
...
selectLanguageFlow = do
  ...
  case action of
    ...
    LANGUAGE_CONFIRMED state -> pure unit
      -- do
    ...
    activeRidePatch activeRideResponse allState onBoardingSubscriptionViewCount = 
      case (activeRideResponse.list !! 0 ) of
        Just (RidesInfo ride) -> do
          ...
          modifyScreenState $ HomeScreenStateType (\homeScreen -> homeScreen { data{ activeRide = activeRide{source = sourceMod, destination = destinationMod, tripType = RC.rideTypeConstructor ride.tripCategory, tripScheduledAt =  ride.tripScheduledAt }}, props{ silentPopUpView = false, goOfflineModal = false}})
        Nothing -> do
  ...
  case action of
    ...
    UPDATE_ROUTE state -> do
      let srcLat = if state.props.currentStage == RideAccepted then
          ...
          state.data.currentDriverLat 
        else state.data.activeRide.src_lat

      let srcLon = if state.props.currentStage == RideAccepted then 
          state.data.currentDriverLon 
        else if state.data.activeRide.tripType == ST.Rental then 
          state.data.currentDriverLon 
        else state.data.activeRide.src_lon

      let destLat = if state.props.currentStage == RideAccepted then 
          state.data.activeRide.src_lat 
        else if state.data.activeRide.tripType == ST.Rental then 
          (if isJust state.data.activeRide.nextStopAddress then 
            fromMaybe 0.0 state.data.activeRide.nextStopLat 
          else fromMaybe (state.data.activeRide.src_lat) (state.data.activeRide.lastStopLat))
        else state.data.activeRide.dest_lat

      let destLon = if state.props.currentStage == RideAccepted then 
          state.data.activeRide.src_lon 
        else if state.data.activeRide.tripType == ST.Rental then 
          (if isJust state.data.activeRide.nextStopAddress then 
            fromMaybe 0.0 state.data.activeRide.nextStopLon 
          else fromMaybe (state.data.activeRide.src_lon) (state.data.activeRide.lastStopLon)) 
        else state.data.activeRide.dest_lon

      let source = if state.props.currentStage == RideAccepted then 
          "" 
        else if state.data.activeRide.tripType == ST.Rental then 
          fromMaybe state.data.activeRide.source state.data.activeRide.lastStopAddress 
        else state.data.activeRide.source

      let destination = if state.props.currentStage == RideAccepted then 
          state.data.activeRide.source 
        else if state.data.activeRide.tripType == ST.Rental then 
          fromMaybe (fromMaybe state.data.activeRide.source state.data.activeRide.lastStopAddress) state.data.activeRide.nextStopAddress 
        else fromMaybe "" state.data.activeRide.destination
