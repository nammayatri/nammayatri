...
homeScreenFlow = do
  ...
  case action of
    ...
    UPDATE_ROUTE state -> do
      ...
      if (state.data.activeRide.tripType == ST.Rental) && (state.props.currentStage /= RideAccepted || state.props.currentStage /= ChatWithCustomer ) && isNothing state.data.activeRide.nextStopAddress then do
          void $ pure $ removeAllPolylines ""
