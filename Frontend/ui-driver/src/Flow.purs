...
driverClaimRewardFlow = do
  ...
  let numberOfRides = fromMaybe 0 getDriverInfoResp.numberOfRides 
      ...
      cancellationRate = fromMaybe 0 getDriverInfoResp.cancellationRateInWindow
      driverTag = ""
      showNominationView = getNominationViewConfig.show
  modifyScreenState $ DriverClaimRewardScreenStateType (\driverClaimRewardScreen -> driverClaimRewardScreen { data {numberOfRides = Just numberOfRides, safetyScore = Just safetyScore, rating = Just rating, cancellationRateInWindow = Just cancellationRate , driverTag = driverTag} , props {showNominationView = showNominationView}})
