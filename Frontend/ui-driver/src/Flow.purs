...
driverEarningsFlow = do 
  (GlobalState globalState) <- getState
  appConfig <- getAppConfig Constants.appConfig
  modifyScreenState $ DriverEarningsScreenStateType (\driverEarningsScreen -> driverEarningsScreen{data{isAutopayActive = globalState.homeScreen.data.paymentState.autoPayStatus == ACTIVE_AUTOPAY, config = appConfig}})
