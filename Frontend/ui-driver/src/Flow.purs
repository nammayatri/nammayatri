...
getDriverInfoFlow event activeRideResp driverInfoResp updateShowSubscription isAdvancedBookingEnabled = do
  ...
  case driverInfoResp of
    ...
    Nothing -> do
      getDriverInfoApiResp <- lift $ lift $ Remote.getDriverInfoApi (GetDriverInfoReq{isAdvancedBookingEnabled : isAdvancedBookingEnabled})
      ...
      case activateRCResp of
        ...
        where 
          refreshDriverProfile = do 
            getDriverInfoApiResp <- lift $ lift $ Remote.getDriverInfoApi (GetDriverInfoReq{isAdvancedBookingEnabled : Nothing})
        ...
        _ -> do
          for_ activeRideResponse.list $ \(RidesInfo ride) -> do
            ...
            let stateChange = if (ride.bookingType == Just ADVANCED) 
                                  ...
                                  else (HomeScreenStateType (\homeScreen -> homeScreen { data{ activeRide = activeRide{source = sourceMod,destination = destinationMod, lastStopAddress = lastStopAddressMod, nextStopAddress = nextStopAddressMod}}, props{ silentPopUpView = false, goOfflineModal = false}}))
...
homeScreenFlow = do
  ...
  case action of
      ...
      case (endRideResp) of
        Right (API.EndRideResponse response) -> do
          when state.data.driverGotoState.isGotoEnabled do
            getDriverInfoResp <- Remote.getDriverInfoBT (GetDriverInfoReq { isAdvancedBookingEnabled : Nothing})
      ...
      where
        onSuccessEndRide = do
            ...
            if getValueToLocalStore HAS_TAKEN_FIRST_RIDE == "true" then do
              getDriverInfoResp <- Remote.getDriverInfoBT (GetDriverInfoReq { isAdvancedBookingEnabled : Just state.data.cityConfig.enableAdvancedBooking})
    ...
    GO_TO_CANCEL_RIDE {id, info , reason} state -> do
      ...
      void $ updateStage $ HomeScreenStage HomeScreen
      when state.data.driverGotoState.isGotoEnabled do
        driverInfoResp <- Remote.getDriverInfoBT (GetDriverInfoReq { isAdvancedBookingEnabled : Just state.data.cityConfig.enableAdvancedBooking})
    ...
    UPDATE_ROUTE state -> do
      ...
      let srcDestConfig = getSrcDestConfig state
          ...
          destMarkerConfig = JB.defaultMarkerConfig{ markerId = "ny_ic_dest_marker", pointerIcon = "ny_ic_dest_marker", primaryText = destination }
...
setSubscriptionStatus paymentStatus apiPaymentStatus planCardConfig = do
  ...
  case paymentStatus of
    Success -> do 
      ...
      getDriverInfoApiResp <- lift $ lift $ Remote.getDriverInfoApi (GetDriverInfoReq{isAdvancedBookingEnabled : Just globalState.homeScreen.data.cityConfig.enableAdvancedBooking})
      case getDriverInfoApiResp of
        Right resp -> modifyScreenState $ GlobalPropsType $ \globalProps -> globalProps{driverInformation = Just resp}
        Left _ -> pure unit
      updateDriverDataToStates
...
getDriverInfoDataFromCache (GlobalState globalState) mkCall = do
  if not mkCall && isJust globalState.globalProps.driverInformation then do 
    ...
    driverInfoResp <- Remote.getDriverInfoBT (GetDriverInfoReq {isAdvancedBookingEnabled : Just globalState.homeScreen.data.cityConfig.enableAdvancedBooking})
    modifyScreenState $ GlobalPropsType $ \globalProps -> globalProps{driverInformation = Just $ driverInfoResp}
    updateDriverDataToStates
    pure driverInfoResp
