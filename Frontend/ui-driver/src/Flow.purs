...
onBoardingFlow = do
  ...
  let limitReachedFor = if driverRegistrationResp.rcVerificationStatus == "LIMIT_EXCEED" then Just "RC"
      ...
      variantList = (if checkAvailability _.autos then [ST.AutoCategory] else []) <> 
              (if checkAvailability _.cabs then [ST.CarCategory] else []) <> 
             (if checkAvailability _.ambulances then [ST.AmbulanceCategory] else []) 
