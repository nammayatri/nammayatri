        ...
        Just (ST.PAN_DETAILS (ST.PanDetails detail)) | (isJust detail.panURL && isJust detail.pan) -> do
          ...
          case resp of
            Right (ValidateImageRes response)-> do
              resp <- lift $ lift $ Remote.registerDriverPAN (Remote.makePANCardReq true currentTime detail.dob detail.name (Just response.imageId) Nothing (fromMaybe "" detail.pan) (convertedStatus))
              case resp of
                Left _ -> void $ pure $ toast "Some Error Occurred. Please try again."
                Right response -> pure unit
