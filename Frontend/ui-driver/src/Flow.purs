...
homeScreenFlow = do
  ...
  case action of             
    ...
    GO_TO_METER_SCREEN -> meterScreenFlow
  homeScreenFlow
      ...
      case resp of
        Right (API.GetPlaceNameResp placeNameResp) ->
          case placeNameResp!!0 of
            Just (API.PlaceName placeName) -> do
              let (API.LatLong latLong) = placeName.location
              modifyScreenState $ MeterScreenStateType $ \meterScreen -> state{props {destinationLat = latLong.lat, destinationLng = latLong.lon}, data {destinationAddress = encodeAddress placeName.formattedAddress placeName.addressComponents (Just placeId) latLong.lat latLong.lon}}
      ...
      case resp of
        Right (API.GetPlaceNameResp placeNameResp) ->
          case placeNameResp!!0 of
            Just (API.PlaceName placeName) -> do
              let (API.LatLong latLong) = placeName.location
              modifyScreenState $ MeterScreenStateType $ \meterScreen -> state{props {destinationLat = latLong.lat, destinationLng = latLong.lon}, data {destination = placeName.formattedAddress, destinationAddress = encodeAddress placeName.formattedAddress placeName.addressComponents Nothing latLong.lat latLong.lon}}
            Nothing -> pure unit
        Left _ -> pure unit
      meterScreenFlow
