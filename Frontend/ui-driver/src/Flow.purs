...
import Common.Types.App (Version(..), LazyCheck(..), Event, FCMBundleUpdate, CategoryListType, GlobalPayload, EventPayload(..))
...
import Services.Accessor (_lat, _lon, _id, _orderId, _moduleId, _languagesAvailableForQuiz, _languagesAvailableForVideos, _deepLinkJSON, _payload, _show_splash)
...
baseAppFlow baseFlow event driverInfoResponse = do
  let
    gPayload = EHC.getGlobalPayload "__payload"
  when baseFlow $ initUIWrapper ""
  let
    bundleSplashConfig = RemoteConfig.getBundleSplashConfig "splash"

    hybridInit = fromMaybe true $ gPayload >>= (\payload -> payload ^. _payload ^. _show_splash)
  when (baseFlow && bundleSplashConfig.enable && hybridInit)
    $ do
        toggleSetupSplash true
        let
          _ = DU.setKeyInWindow "forceAppToNoInternetScreen" true
        pure unit
  ...
  cacheAppParameters versionCode baseFlow = do
    ...
    void $ pure
      $ setCleverTapUserProp
          [ { key: "App Version", value: unsafeToForeign versionName }
    ...
    if ((movedToOfflineDate /= "" && isYesterday movedToOfflineDate) || movedToOfflineDate == "__failed") then
      setValueToLocalStore MOVED_TO_OFFLINE_DUE_TO_HIGH_DUE ""
    else
      pure unit
    when ((getValueToLocalStore SESSION_ID == "__failed") || (getValueToLocalStore SESSION_ID == "(null)"))
      $ do
          setValueToLocalStore SESSION_ID (generateSessionId unit)
    if (driverId == "__failed") then
  ...
  initialFlow = do
    ...
    let
      regToken = getValueToLocalStore REGISTERATION_TOKEN
  ...
  updateNightSafetyPopup = do
    let
      curr_time = convertUTCtoISC (getCurrentUTC "") "HH:mm:ss"
  ...
  showParcelIntroductionPopup = do
    let
      showParcelInfo = getValueToLocalStore SHOW_PARCEL_INTRODUCTION_POPUP
...
checkRideAndInitiate event driverInfoResponse = do
  ...
  Tuple mbRideListResponse activeRide <- case driverInfoResponse of
    Just (Right (GetDriverInfoResp driverInfoResp)) -> do
    ...
    _ -> do
      ...
      let
        activeRide = not (null rideListResponse.list)
  ...
  languageExists languages lang =
    let
      firstTwoChars = toLower $ take 2 lang
    in
      firstTwoChars `elem` languages

checkVersion :: Int -> FlowBT String Unit
checkVersion versioncode = do
  when (getValueToLocalNativeStore IS_RIDE_ACTIVE /= "true" && versioncode < (getLatestAndroidVersion (getMerchant FunctionCall)))
    $ do
        hideSplashAndCallFlow $ pure unit
...
getMerchantName merchant = case merchant of
...
loginFlow mbEvent = do
  ...
  when (allState.globalProps.addTimestamp)
    $ do
...
getDriverInfoFlow event activeRideResp driverInfoResp updateShowSubscription isAdvancedBookingEnabled updateCTBasicData = do
  ...
  runDriverInfoFlow driverInfoRes updateFeatureFlags = do
    ...
    case driverInfoRes of
      Right (GetDriverInfoResp getDriverInfoResp) -> do
        let
          cityConfigFromCityCode = HU.getCityConfigFromCityCode config.cityConfig (fromMaybe "" getDriverInfoResp.operatingCity)
        ...
        when (updateFeatureFlags && cityConfig.callDriverInfoPost)
          $ do
        ...
        if getDriverInfoResp.enabled then do
          if (HU.isParentView FunctionCall) then do
            ...
            if getValueToLocalStore IS_DRIVER_ENABLED == "false" then do
              ...
              let
                category = fromMaybe "" linkedVehicle.category
              when (category /= "")
                $ do
          ...
          if updateShowSubscription then
          ...
          if getDriverInfoResp.verified then
      ...
      Left errorPayload -> do
        if ((decodeErrorCode errorPayload.response.errorMessage) == "VEHICLE_NOT_FOUND" || (decodeErrorCode errorPayload.response.errorMessage) == "DRIVER_INFORMATON_NOT_FOUND") then
          onBoardingFlow
        else do
          ...
          if getValueToLocalStore IS_DRIVER_ENABLED == "true" then do
            ...
            if permissionsGiven then
              ...
              permissionsScreenFlow event activeRideResp Nothing
          else do
            onBoardingFlow
  ...
  getUpdateToken token =
    let
      initialData = Remote.mkUpdateDriverInfoReq ""
  ...
  if cityConfig.variantSubscriptionConfig.enableVariantBasedSubscription then do
    case vehicleVariant of
      Just (API.Vehicle vehicle) -> do
        let
          category = RC.getCategoryFromVariant vehicle.variant
        case category of
    ...
    case response of
      Right (API.GetAllRcDataResp getAllRcsDataResp) -> do
        let
          isCabVariant =
            foldl
              ( \acc (API.GetAllRcDataRecords item) -> do
                  let
                    API.VehicleDetails details = item.rcDetails
                  if details.vehicleVariant == Just "AUTO_RICKSHAW" then
                    false
                  else if item.rcActive then
                    acc && true
                  else
                    acc
              )
...
handleDeepLinksFlow event activeRideResp isActiveRide = do
  ...
  case event of -- TODO:: Need to handle in generic way for all screens. Could be part of flow refactoring
    Just e -> case e.data of
      ...
      _
        | startsWith "ginit" e.data -> hideSplashAndCallFlow $ gullakDeeplinkFlow e.data
      _ -> pure unit
    Nothing -> pure unit
  (GlobalState allState) <- getState
...
gullakDeeplinkFlow event = do
  let
    token = getValueToLocalStore GULLAK_TOKEN
...
startGrpcService =
  let
    grpcServiceRunning = runFn1 JB.isServiceRunning "in.juspay.mobility.app.GRPCNotificationService"

    locationUpdateServiceRunning = runFn1 JB.isServiceRunning "in.juspay.mobility.app.LocationUpdateService"
  in
    if locationUpdateServiceRunning && (not grpcServiceRunning) then
      void $ pure $ JB.startService "in.juspay.mobility.app.GRPCNotificationService"
    else
      pure unit
...
convertToRequestStatus status = case status of
...
getSdkTokenFromCache = do
  let
    cachedData = getValueToLocalStore CACHED_SDK_TOKEN_DATA
  if cachedData /= "__failed" then do
    let
      splitData = split (Pattern ":") cachedData
    case splitData of
      [ token, expiry ] -> do
        let
          isExpired = runFn1 JB.isSdkTokenExpired expiry
        if isExpired then
          cacheNewToken
        else
          pure token
  ...
  cacheNewToken = do
    ...
    let
      expiry = runFn1 JB.makeSdkTokenExpiry hvSdkTokenExp
...
onBoardingFlow = do
  ...
  if driverEnabled && config.feature.enableAutoReferral && (cityConfig.showDriverReferral || config.enableDriverReferral) then do
    let
      referralCode = getReferralCode (getValueToLocalStore REFERRER_URL)
  ...
  let
    limitReachedFor =
      if driverRegistrationResp.rcVerificationStatus == "LIMIT_EXCEED" then
        Just "RC"
      else if driverRegistrationResp.dlVerificationStatus == "LIMIT_EXCEED" then
        Just "DL"
      else
        Nothing
  ...
  case flow of
    UPLOAD_DRIVER_LICENSE state -> do
      modifyScreenState $ UploadDrivingLicenseScreenStateType
        $ \_ ->
            UploadDrivingLicenseScreenData.initData
              { data
    ...
    UPLOAD_VEHICLE_DETAILS state rcNumberPrefixList -> do
      modifyScreenState $ AddVehicleDetailsScreenStateType
        $ \_ ->
            AddVehicleDetailsScreenData.initData
              { data
    ...
    AADHAAR_PAN_SELFIE_UPLOAD state (ST.HyperVergeKycResult result) -> do
      ...
      let
        status = fromMaybe "needs_review" result.status
      let
        convertedStatus = convertToRequestStatus status
      void $ lift $ lift $ loaderText (getString VALIDATING) (getString PLEASE_WAIT_WHILE_IN_PROGRESS)
      void $ lift $ lift $ toggleLoader true
      case result.details of
        Just (ST.LIVE_SELFIE (ST.LiveSelfie detail))
          | (isJust detail.selfieURL) -> do
            if result.status == (Just "auto_declined") then
              if isJust detail.errorCode then void $ pure $ JB.toast $ getHvErrorMsg detail.errorCode else pure unit
            else
              pure unit
        ...
        Just (ST.PAN_DETAILS (ST.PanDetails detail))
          | (isJust detail.panURL) -> do
            if result.status == (Just "auto_declined") then
              if isJust detail.errorCode then void $ pure $ JB.toast $ getHvErrorMsg detail.errorCode else pure unit
            else
              pure unit
            let
              panNum = fromMaybe "" detail.pan
            if DS.null panNum then do
                  ...
                  case resp of
                    Right (ValidateImageRes response) -> do
                      ...
                      case resp of
                        ...
                        Right response -> pure unit
                    Left errPayload -> if result.status /= (Just "auto_declined") then void $ pure $ toast $ Remote.getCorrespondingErrorMessage errPayload else pure unit
        Just (ST.AADHAAR_DETAILS (ST.AadhaarCardDetails detail))
          | (isJust detail.aadhaarFrontURL && isJust detail.aadhaarBackURL) -> do
            if result.status == (Just "auto_declined") then
              if isJust detail.errorCode then void $ pure $ JB.toast $ getHvErrorMsg detail.errorCode else pure unit
            else
              pure unit
            let
              aadhaarNum = fromMaybe "" detail.idNumber
            if DS.null aadhaarNum || detail.errorCode == Just "113" -- or condition added to make sure both sides are detected then do
              ...
              case imageFront, imageBack of
                Just imageFront, Just imageBack -> do
                  ...
                  case respFrontImage, respBackImage of
                    Right (ValidateImageRes frontResp), Right (ValidateImageRes backResp)
                      | isJust result.transactionId -> do
    ...
    DOCUMENT_CAPTURE_FLOW state doctype -> do
      let
        defState = DocumentCaptureData.initData
  ...
  mkStatusList (DriverRegistrationStatusResp driverRegistrationStatusResp) =
    let
      driversDocument = driverRegistrationStatusResp.driverDocuments
  ...
  transfromDocumentStatusItem statusItem userSelectedVehicle verifiedVehicleCategory regNo =
    map
      ( \(API.DocumentStatusItem documentStatusItem) ->
...
updateDriverVersion dbClientVersion dbBundleVersion = do
  if (isJust dbClientVersion && isJust dbBundleVersion) then do
    ...
    if any (_ == -1) [ clientVersion.minor, clientVersion.major, clientVersion.maintenance, bundleVersion.minor, bundleVersion.major, bundleVersion.maintenance ] then
      pure unit
    else if (bundleVersion' /= bundleVersion || clientVersion' /= clientVersion) then do
      let
        initialData = mkUpdateDriverInfoReq ""
  ...
  case out of
    ENTER_AADHAAR_OTP state -> do
      ...
      case res of
        ...
        Left errorPayload -> do
          let
            errorCode = decodeErrorCode errorPayload.response.errorMessage
          case errorCode of
    ...
    VERIFY_AADHAAR_OTP state -> do
      ...
      case res of
        ...
        Left errorPayload -> do
          let
            stage = if (decodeErrorCode errorPayload.response.errorMessage) == "INVALID_OTP" then VerifyAadhaar else AadhaarDetails
      ...
      case validateImageResp of
        Right (ValidateImageRes resp) -> do
          ...
          if (state.data.rcImageID == "IMAGE_NOT_VALIDATED") then do
            ...
            let
              airConditioned = if state.data.airConditioned == Just true then Just 0 else Just 1
            ...
            case registerDriverRCResp of
              Right (API.ApiSuccessResult resp) -> do
                ...
                (GlobalState state') <- getState
                let
                  profileState = state'.driverProfileScreen
                if (not addRcFromProf) then do
                  ...
                  let
                    multiRcStatus = getStatusValue resp.rcVerificationStatus
        ...
        Left errorPayload -> do
          ...
          if errorPayload.code == 429 && (decodeErrorCode errorPayload.response.errorMessage) == "IMAGE_VALIDATION_EXCEED_LIMIT" then do
            ...
            if state.props.addRcFromProfile then
              addVehicleDetailsflow state.props.addRcFromProfile
            else
              onBoardingFlow
          else if errorPayload.code == 400 || (errorPayload.code == 500 && (decodeErrorCode errorPayload.response.errorMessage) == "UNPROCESSABLE_ENTITY") then do
    ...
    RC_ACTIVATION state -> do
      ...
      case activateRCResp of
        ...
        Left errorPayload -> do
          void $ lift $ lift $ toggleLoader false
          let
            codeMessage = decodeErrorCode errorPayload.response.errorMessage
...
applicationSubmittedFlow screenType = do
  ...
  case action of
    ...
    GO_TO_UPLOAD_DL_SCREEN -> do
      let
        (GlobalState defaultEpassState') = defaultGlobalState
    ...
    RESEND_OTP_TO_ALTERNATE_NUMBER state -> do
      let
        number = state.data.mobileNumber
...
driverProfileFlow = do
  ...
  case action of
    ...
    GO_TO_BOOKING_OPTIONS_SCREEN state -> do
      let
        downgradeOptions = (downgradeOptionsConfig state.data.vehicleSelected) <$> state.data.downgradeOptions
      modifyScreenState
        $ BookingOptionsScreenType
            ( \bookingOptions ->
                bookingOptions
                  { data
                    { vehicleType = state.data.driverVehicleType
    ...
    DRIVER_ALTERNATE_CALL_API1 updatedState -> do
      let
        number = updatedState.data.driverEditAlternateMobile
    ...
    VERIFY_OTP1 state -> do
      let
        toast_value = if not state.props.isEditAlternateMobile then (getString NUMBER_ADDED_SUCCESSFULLY) else (getString NUMBER_EDITED_SUCCESSFULLY)
      ...
      case getVerifyAlternateMobileOtpResp of
        ...
        Left errorPayload -> do
          if (errorPayload.code == 400 && (decodeErrorCode errorPayload.response.errorMessage) == "INVALID_AUTH_DATA") then do
            if (state.data.otpLimit == 1) then do
              ...
              let
                otpExceeded = ((state.data.otpLimit - 1) <= 0)
    ...
    DRIVER_GENDER1 state -> do
      let
        genderSelected = state.data.driverGender
      let
        initialData = mkUpdateDriverInfoReq ""
...
driverDetailsFlow = do
  ...
  case action of
    DRIVER_ALTERNATE_CALL_API updatedState -> do
      let
        number = if (updatedState.props.isEditAlternateMobile) then updatedState.data.driverEditAlternateMobile else updatedState.data.driverAlternateMobile
      getAlternateMobileResp <- lift $ lift $ Remote.validateAlternateNumber (makeValidateAlternateNumberRequest (fromMaybe "" (number)))
      case getAlternateMobileResp of
        Right (DriverAlternateNumberResp resp) -> do
        ...
        Left errorPayload -> do
          let
            alternateNumber = if updatedState.props.isEditAlternateMobile then updatedState.data.driverAlternateMobile else Nothing
          if (errorPayload.code == 400 && ((decodeErrorCode errorPayload.response.errorMessage) == "INVALID_REQUEST")) then do
...
goToLocationFlow = do
  ...
  case action of
    ...
    AUTO_COMPLETE updatedState searchVal currentLat currentLon -> do
      ...
      case resp of
        Right (API.AutoCompleteResp autoCompleteResp) ->
          modifyScreenState
            $ DriverSavedLocationScreenStateType
                ( \_ ->
                    updatedState
                      { data
                        { predictions =
    ...
    GET_LOCATION_NAME state -> do
      ...
      case resp of
        Right (API.GetPlaceNameResp placeNameResp) -> case placeNameResp !! 0 of
          Just (API.PlaceName placeName) -> do
            let
              (API.LatLong latLong) = placeName.location
...
selectLanguageFlow = do
  let
    selectLang = getLanguageLocale languageKey
...
bookingOptionsFlow = do
  ...
  let
    (FeaturesConfigData featuresRemoteConfig) = CommonRC.featuresConfigData (getValueToLocalStore DRIVER_LOCATION)
  let
    filterByDeliveryBike =
      if (not featuresRemoteConfig.enableDeliveryBike) then
        filter (\(API.DriverVehicleServiceTier tier) -> tier.serviceTierType /= DELIVERY_BIKE) resp.tiers
      else
        resp.tiers
  ...
  modifyScreenState
    $ BookingOptionsScreenType
        ( \bookingOptions ->
            bookingOptions
              { data
                { airConditioned = resp.airConditioned
  ...
  case action of
      ...
      case resp of
        Right _ -> do
          let
            toastMessage =
              if toggleVal then
                getString VARIANTS_ARE_SWITCHED
              else
                getString NON_AC_ARE_SWITCHED
    ...
    SELECT_CAB state toggleDowngrade -> do
      ...
      void $ lift $ lift $ toggleLoader true
      let
        canDowngradeToSedan = isJust $ (filter (\item -> item.vehicleVariant == "SEDAN" && item.isSelected) state.data.downgradeOptions) !! 0
  ...
  transfromRidePreferences =
    map
      ( \(API.DriverVehicleServiceTier item) ->
...
helpAndSupportFlow = do
  ...
  case action of
    ...
    REPORT_ISSUE_CHAT_SCREEN selectedCategory -> do
      ...
      let
        categoryName = getCategoryName $ fromMaybe "" selectedCategory.categoryAction
...
permissionsScreenFlow event activeRideResp isActiveRide = do
  ...
  case action of
    ...
    GO_TO_REGISTERATION_SCREEN state -> do
      let
        allChecked = state.props.isNotificationPermissionChecked && state.props.isOverlayPermissionChecked && state.props.isAutoStartPermissionChecked
        ...
        $ RegisterScreenStateType
            ( \registerationScreen ->
                registerationScreen
                  { data
                    { permissionsStatus =
                      case allChecked, partialChecked of
                        true, _ -> ST.COMPLETED
...
myRidesScreenFlow = do
  let
    (GlobalState defGlobalState) = defaultGlobalState
  flow <- UI.rideHistory
  case flow of
    REFRESH state -> do
    ...
    OPEN_PAYMENT_HISTORY state -> do
      ...
      case resp of
        Right (GetPaymentHistoryResp response) -> do
          let
            paymentHistoryArray = getPaymentHistoryItemList response
    ...
    SELECT_RIDE state -> do
      ...
      let
        tripId' = case state.selectedItem of
          ...
          _ -> Nothing
      let
        categoryName = getCategoryName $ fromMaybe "" state.selectedCategory.categoryAction
...
issueReportChatScreenFlow = do
  ...
  case flow of
    ...
    SUBMIT_ISSUE state -> do
      let
        mediaFiles' = case state.data.uploadedAudioId of
      ...
      void $ pure $ hideKeyboardOnNavigation true
      let
        showDescription = DS.length (trim issueInfoRes.description) > 0
      ...
      if state.props.isReversedFlow then do
        let
          options' = map (\x -> x.option) state.data.options
        let
          message =
            (getString SELECT_OPTION_REVERSED) <> "\n"
              <> joinWith "\n" options'
        let
          messages' = concat [ descMessages, mediaMessages', [ (makeChatComponent' message Nothing Nothing Nothing "Bot" (getCurrentUTC "") "Text" (500 * (length mediaMessages' + 2))) ] ]
        ...
        let
          message = makeChatComponent' (getString ISSUE_SUBMITTED_MESSAGE) Nothing Nothing Nothing "Bot" (getCurrentUTC "") "Text" (500 * (length mediaMessages' + 2))
        let
          messages' = concat [ descMessages, mediaMessages', [ message ] ]
    ...
    CALL_CUSTOMER state -> do
      case state.data.tripId of
        Just tripId -> do
          ...
          void $ lift $ lift $ toggleLoader false
          let
            message = makeChatComponent' (getString ISSUE_SUBMITTED_MESSAGE) Nothing Nothing Nothing "Bot" (getCurrentUTC "") "Text" 500
          let
            messages' = snoc state.data.chatConfig.messages message
...
currentRideFlow activeRideResp isActiveRide = do
  ...
  let
    isRequestExpired =
      if (getValueToLocalNativeStore RIDE_REQUEST_TIME) == "__failed" then
        false
      else
        ceil ((toNumber (rideRequestPollingData.duration - (getExpiryTime (getValueToLocalNativeStore RIDE_REQUEST_TIME) true)) * 1000.0) / rideRequestPollingData.delay) > 0
  ...
  case isActiveRide of
    ...
    _ -> do
      if isJust activeRideResp then do
        let
          (GetRidesHistoryResp activeRideResponse) = fromMaybe (GetRidesHistoryResp { list: [] }) activeRideResp
  ...
  when (allState.homeScreen.data.config.profileVerification.aadharVerificationRequired)
    $ do -- TODO :: Should be moved to global events as an async event
        (DriverRegistrationStatusResp resp) <- driverRegistrationStatusBT $ DriverRegistrationStatusReq true
  ...
  activeRidePatch (GetRidesHistoryResp activeRideResponse) allState onBoardingSubscriptionViewCount = do
    case activeRideResponse.list of
      [] -> do
        ...
        void $ updateStage $ HomeScreenStage HomeScreen
        updateDriverDataToStates
        if onBoardingSubscriptionScreenCheck onBoardingSubscriptionViewCount allState.homeScreen.data.config.subscriptionConfig.onBoardingSubscription then
          onBoardingSubscriptionScreenFlow onBoardingSubscriptionViewCount
          ...
          pure unit
      _ -> do
        for_ activeRideResponse.list
          $ \(RidesInfo ride) -> do
              let
                isMeterRide = maybe false (\(API.TripCategory tripCategory) -> tripCategory.contents == Just "MeterRide") ride.tripCategory
              if isMeterRide then do
                ...
                let
                  decodedSource = decodeAddress ride.fromLocation true
                ...
                nextStopAddressMod <-
                  maybe (pure Nothing)
                    ( \decodedNextStopAddress' -> do
                        nextStopMod <- translateString decodedNextStopAddress' 500
                        pure $ Just nextStopMod
                    )
                    decodeNextStopAddress
                lastStopAddressMod <-
                  maybe (pure Nothing)
                    ( \decodedLastStopAddress' -> do
                        lastStopMod <- translateString decodedLastStopAddress' 500
                        pure $ Just lastStopMod
                    )
                    decodeLastStopAddress
                setValueToLocalNativeStore IS_RIDE_ACTIVE "true"
                -- Night Ride Safety PopUp
                when (activeRide.disabilityTag == Just ST.SAFETY)
                  $ do
                      let
                        curr_time = convertUTCtoISC (getCurrentUTC "") "HH:mm:ss"
                ...
                let
                  stateChange =
                    if (ride.bookingType == Just ADVANCED) then
                      (HomeScreenStateType (\homeScreen -> homeScreen { data { advancedRideData = Just activeRide { source = sourceMod, destination = destinationMod } }, props { silentPopUpView = false, goOfflineModal = false, retryRideList = false } }))
                    else
                      (HomeScreenStateType (\homeScreen -> homeScreen { data { activeRide = activeRide { source = sourceMod, destination = destinationMod, lastStopAddress = lastStopAddressMod, nextStopAddress = nextStopAddressMod } }, props { silentPopUpView = false, goOfflineModal = false, isOdometerReadingsRequired = (activeRide.tripType == ST.Rental) && (maybe true (\val -> val) ride.isOdometerReadingsRequired), retryRideList = false } }))
  ...
  noActiveRidePatch allState onBoardingSubscriptionViewCount = do
    ...
    when (allState.homeScreen.props.currentStage /= HomeScreen)
      $ do
...
checkDriverPaymentStatus (GetDriverInfoResp getDriverInfoResp) =
  when
    ( getDriverInfoResp.paymentPending
        && (getValueToLocalStore SHOW_PAYMENT_MODAL)
        /= "false"
    ...
    case resp of
      Right (GetPaymentHistoryResp resopnse) -> case resopnse !! 0 of
        ...
        Nothing -> do
          ...
          case overDueResp of
            Right (GetPaymentHistoryResp overDueResp') -> case overDueResp' !! 0 of
              Just (PaymentDetailsEntity paymentDetailsEntity) ->
                modifyScreenState
                  $ HomeScreenStateType
                      ( \homeScreen ->
...
onBoardingSubscriptionScreenFlow onBoardingSubscriptionViewCount = do
  ...
  modifyScreenState
    $ OnBoardingSubscriptionScreenStateType \onBoardingSubscriptionScreen ->
...
homeScreenFlow = do
  ...
  logField_ <- lift $ lift $ getLogFields
  setValueToLocalStore LOGS_TRACKING "false"
  Events.measureDurationFlowBT "Flow.homeScreenFlow"
    $ do
        void $ pure $ cleverTapSetLocation unit
        if (getValueToLocalNativeStore IS_RIDE_ACTIVE) == "true" && (not $ any (\item -> isLocalStageOn item) [ RideAccepted, RideStarted, ChatWithCustomer ]) then do
  ...
  case action of
    ...
    DRIVER_AVAILABILITY_STATUS state status -> do
      ...
      let
        (Vehicle linkedVehicle) = fromMaybe dummyVehicleObject getDriverInfoRes.linkedVehicle
      if globalState.globalProps.firstTimeOnboardingStatus then
        logDriverStatus linkedVehicle.category status
      else
        pure unit
    ...
    GO_TO_START_RIDE { id, otp, startOdometerReading, startOdometerImage, lat, lon, ts } updatedState -> do
      ...
      case startRideResp of
        Right startRideResp -> do
          let
            chargesOb = HU.getChargesOb updatedState.data.activeRide.tripType updatedState.data.cityConfig updatedState.data.activeRide.driverVehicle
          void $ pure $ setValueToLocalNativeStore RIDE_ID id
          _ <- pure $ hideKeyboardOnNavigation true
          liftFlowBT $ logEventWithMultipleParams logField_ "ny_driver_ride_start"
            $ [ { key: "Service Tier", value: unsafeToForeign updatedState.data.activeRide.serviceTier }
    ...
    GO_TO_CANCEL_RIDE { id, info, reason } state -> do
      liftFlowBT $ logEventWithMultipleParams logField_ "ny_driver_ride_cancelled"
        $ [ { key: "Reason code", value: unsafeToForeign reason }
      ...
      case resp of
        ...
        Right _ -> do
          ...
          case (rideList.list DA.!! 0) of
            Just (rideInfo) -> do
              let
                currActiveRideDetails = activeRideDetail state rideInfo
    ...
    NOTIFY_CUSTOMER state -> do
      if (getValueToLocalStore WAITING_TIME_STATUS == show ST.NoStatus) && (state.data.activeRide.tripType == ST.Rental || state.data.activeRide.tripType == ST.Intercity) then do
        ...
        driverArrived <-
          lift $ lift
            $ Remote.driverArrived (state.data.activeRide.id)
                ( DriverArrivedReq
        ...
        case driverArrived of
          Right _ -> do
            ...
            liftFlowBT $ logEventWithMultipleParams logField_ "ny_driver_i_have_arrived_clicked"
              $ [ { key: "Service Tier", value: unsafeToForeign state.data.activeRide.serviceTier }
                , { key: "Driver Vehicle", value: unsafeToForeign state.data.activeRide.driverVehicle }
    ...
    UPDATE_ROUTE state callDrawRoute -> do
      ...
      if (state.data.activeRide.tripType == ST.Rental) && (state.props.currentStage == RideStarted) && isNothing state.data.activeRide.nextStopAddress then do
        ...
        modifyScreenState $ HomeScreenStateType (\homeScreen -> homeScreen { props { routeVisible = true } })
        let
          specialPickupZone = HU.findSpecialPickupZone destLat destLon
        case specialPickupZone of
          Just pickupZone -> do
            ...
            let
              _ =
                unsafePerformEffect
                  $ runEffectFn1 JB.locateOnMap
                      JB.locateOnMapConfig
                        { lat = destLat
                        , lon = destLon
        ...
        let
          coors = (walkCoordinate srcLon srcLat destLon destLat)
        modifyScreenState $ HomeScreenStateType (\homeScreen -> homeScreen { props { routeVisible = true } })
        void $ pure $ removeAllPolylines ""
        let
        ...
        case shortRoute of
          Just (Route route) -> do
            ...
            void $ pure $ removeAllPolylines ""
            let
              normalRoute = JB.mkRouteConfig coor srcMarkerConfig destMarkerConfig Nothing drawRouteType "LineString" true JB.DEFAULT (mapRouteConfig "" "" false getPolylineAnimationConfig)
            liftFlowBT $ drawRoute [ normalRoute ] (getNewIDWithTag "DriverTrackingHomeScreenMap")
            pure unit
          Nothing -> pure unit
      else do
        ...
        --   Left err -> pure unit
        let
          leftStops = DA.filter (\(API.Stop item) -> maybe true (\(API.StopInformation stopInfo) -> isNothing stopInfo.stopEndLatLng) item.stopInfo) state.data.activeRide.stops

          points =
            (DA.singleton $ API.LatLong { lat: srcLat, lon: srcLon })
              <> ( if state.props.currentStage == RideAccepted then
                    []
                  else
                    map (\(API.Stop item) -> getLatlon item.location) leftStops
                )
        ...
        case eRouteAPIResponse of
          Right (GetRouteResp routeApiResponse) -> do
            let
              shortRoute = (routeApiResponse !! 0)
            case shortRoute of
              Just (Route route) -> do
                let
                  coor = JB.getExtendedPath $ walkCoordinates route.points
                modifyScreenState $ HomeScreenStateType (\homeScreen -> homeScreen { data { activeRide { actualRideDistance = if state.props.currentStage == RideStarted then (toNumber route.distance) else state.data.activeRide.actualRideDistance, duration = route.duration }, route = routeApiResponse }, props { routeVisible = true } })
                pure $ removeMarker "ny_ic_auto"
                void $ pure $ removeAllPolylines ""
                let
              ...
              Nothing -> pure unit
          Left err -> pure unit
      when (state.props.currentStage == RideStarted) $ for_ state.data.activeRide.stops
        $ \(API.Stop stop) -> do
            let
              (API.LocationInfo stopLocation) = stop.location
            pure $ removeMarker $ "stop" <> show stopLocation.lat <> show stopLocation.lon
            when (maybe true (\(API.StopInformation sInfo) -> isNothing sInfo.stopEndLatLng) stop.stopInfo)
              $ do
                  let
    ...
    POST_RIDE_FEEDBACK state -> do
      ...
      when state.data.driverGotoState.isGotoEnabled do
        modifyScreenState $ GlobalPropsType
          $ \globalProps ->
              globalProps
                { gotoPopupType =
                  case globalstate.globalProps.gotoPopupType of
    ...
    ENABLE_GOTO_API state id currentLocation -> do
      ...
      case activateResp of
        Right resp -> do
          ...
          updateDriverDataToStates
        Left errorPayload ->
          if (decodeErrorCode errorPayload.response.errorMessage) == "DRIVER_CLOSE_TO_HOME_LOCATION" then
            modifyScreenState
              $ HomeScreenStateType
                  ( \_ ->
                      state
                        { data
                          { driverGotoState
                            { gotoLocInRange = true
                            , savedLocationsArray = getDisabledLocById id state.data.driverGotoState.savedLocationsArray
                            , selectedGoTo = ""
                            }
                          }
                        }
            ...
            pure $ toast $ Remote.getCorrespondingErrorMessage errorPayload
      if (HU.isParentView FunctionCall) then do --Need To handle GoTo Function in React HomeScren 
    ...
    REFRESH_GOTO state -> do
      let
        defState = HomeScreenData.initData
    ...
    UPDATE_AIR_CONDITIONED isAcWorking -> do
      ...
      case resp of
        Right _ -> do
          ...
          (GetDriverInfoResp getDriverInfoResp) <- getDriverInfoDataFromCache globalState false
          let
            updatedResponse = getDriverInfoResp { checkIfACWorking = Just false }
    ...
    GO_TO_ADD_UPI_SCREEN -> do
      let
        (GlobalState defGlobalState) = defaultGlobalState
      let
        customerReferralTrackerScreenState = defGlobalState.customerReferralTrackerScreen
    ...
    NOTIFY_DRIVER_REACHED_DESTINATION state -> do
      when (state.data.activeRide.tripType == ST.Delivery) do
        driverArrived <-
          lift $ lift
            $ Remote.driverReachedDestination (state.data.activeRide.id)
                ( DriverReachedReq
                    { "lat": state.data.currentDriverLat
                    , "lon": state.data.currentDriverLon
                    }
      ...
      case resp of
        Right _ -> do
          ...
          case (DA.find (\(RidesInfo x) -> x.bookingType == Just CURRENT) activeRideResponse.list) of
            Just ride -> do
              let
                advancedRide = (DA.find (\(RidesInfo x) -> x.bookingType == Just ADVANCED) activeRideResponse.list)
...
handleFcm notificationType state notificationBody = do
  ...
  case notificationType of
    ...
    "DRIVER_ASSIGNMENT" -> do
      let
        (GlobalState defGlobalState) = defaultGlobalState
      when (isJust defGlobalState.homeScreen.data.activeRide.disabilityTag)
        $ do
...
endTheRide id endOtp endOdometerReading endOdometerImage lat lon ts state = do
  ...
  void $ lift $ lift $ toggleLoader true
  let
    numDeviation = Just $ (fromMaybe 0 (fromString (getValueToLocalNativeStore RIDE_WAYPOINT_DEVIATION_COUNT))) >= 3
      ...
      | lt /= 0.0 && ln /= 0.0 -> do
        ...
        case (endRideResp) of
                ...
                $ GlobalPropsType
                    ( \globalProps ->
                        globalProps
                          ...
                          , gotoPopupType =
                            case response.homeLocationReached of
          ...
          Left errorPayload -> do
            ...
            let
              getCurrentRideInfo = head rideHistoryResponse.list
  ...
  onSuccessEndRide = do
    ...
    when (state.props.currentStage == RideStarted) $ for_ state.data.activeRide.stops
      $ \(API.Stop stop) -> do
          let
            (API.LocationInfo stopLocation) = stop.location
          pure $ removeMarker $ "stop" <> show stopLocation.lat <> show stopLocation.lon
    liftFlowBT $ logEventWithMultipleParams logField_ "ny_driver_ride_completed"
      $ [ { key: "Service Tier", value: unsafeToForeign state.data.activeRide.serviceTier }
        , { key: "Driver Vehicle", value: unsafeToForeign state.data.activeRide.driverVehicle }
        ]
    ...
    if getValueToLocalStore HAS_TAKEN_FIRST_RIDE == "true" then do
      getDriverInfoResp <- Remote.getDriverInfoBT ""
      let
        (GetDriverInfoResp getDriverInfoResp) = getDriverInfoResp
      modifyScreenState $ GlobalPropsType $ \globalProps -> globalProps { driverInformation = Just (GetDriverInfoResp getDriverInfoResp) }
      if (isJust getDriverInfoResp.numberOfRides && (fromMaybe 0 getDriverInfoResp.numberOfRides == 1)) then do
        let
...
clearPendingDuesFlow showLoader = do
  ...
  case clearduesResp' of
    Right (ClearDuesResp clearduesResp) -> do
      let
        (CreateOrderRes orderResp) = clearduesResp.orderResp
      ...
      let
        sdkPayload = maybe (encodeJSON orderResp.sdk_payload) (addLanguageToPayload (getPaymentPageLangKey (getLanguageLocale languageKey))) orderResp.sdk_payload_json
      ...
      case orderStatus of
        Right (OrderStatusRes statusResp) -> do
          when (statusResp.status == PS.CHARGED)
            $ do
                _ <-
                  pure $ cleverTapEvent "ny_driver_clear_dues"
                    $ [ { key: "due_amount", value: unsafeToForeign innerpayload.amount }
                      , { key: "clearence_type", value: unsafeToForeign "manual" }
                      ]
                pure unit
          let
            popUpState =
              if statusResp.status == PS.CHARGED then
                Just PaymentSuccessPopup
              else if any (_ == statusResp.status) [ PS.AUTHORIZATION_FAILED, PS.AUTHENTICATION_FAILED, PS.JUSPAY_DECLINED ] then
                Just FailedPopup
              else
                Nothing
          case popUpState of
  ...
  case response of
    Right (SubscribePlanResp listResp) -> do
      if fromScreen /= "MYPLAN" then do
        ...
        pure unit
      let
        (CreateOrderRes orderResp) = listResp.orderResp
      setValueToLocalStore DISABLE_WIDGET "true"
      void $ pure $ cleverTapCustomEvent "ny_driver_payment_page_opened"
      void $ pure $ metaLogEvent "ny_driver_payment_page_opened"
      liftFlowBT $ firebaseLogEvent "ny_driver_payment_page_opened"
      ...
      case orderStatus of
        Right (OrderStatusRes statusResp) -> case statusResp.status of
          PS.CHARGED -> do
  ...
  case action of
    GoToSetupAutoPay state ->
      if state.data.autoPayStatus == ST.SUSPENDED then do
        ...
        nyPaymentFlow state.data.planData "MYPLAN"
    EntityDetailsAPI state id -> do
      paymentEntityDetails <- lift $ lift $ Remote.paymentEntityDetails id
      let
        cityConfig = HU.getCityConfig appConfig.cityConfig (getValueToLocalStore DRIVER_LOCATION)
      case paymentEntityDetails of
        Right (HistoryEntryDetailsEntityV2Resp resp) ->
          modifyScreenState
            $ PaymentHistoryScreenStateType
                ( \paymentHistoryScreen ->
                    paymentHistoryScreen
                      { props { subView = ST.TransactionDetails }
...
ysPaymentFlow = do
  ...
  case response of
    Right (CreateOrderRes listResp) -> do
      let
        (PaymentPagePayload sdk_payload) = listResp.sdk_payload
      setValueToLocalStore DISABLE_WIDGET "true"
      lift $ lift $ doAff $ makeAff \cb -> runEffectFn1 checkPPInitiateStatus (cb <<< Right) $> nonCanceler
      let
        sdkPayload = maybe (encodeJSON listResp.sdk_payload) (addLanguageToPayload (getPaymentPageLangKey (getLanguageLocale languageKey))) listResp.sdk_payload_json
      ...
      case orderStatus of
        Right (OrderStatusRes resp) -> case resp.status of
...
setPaymentStatus paymentStatus (PayPayload payload) = do
  case paymentStatus of
    Success -> do
      let
        currency = getCurrency Constants.appConfig
    ...
    Pending -> do
      ...
      setValueToLocalStore SHOW_PAYMENT_MODAL "false"
      let
        time2PmTo10Am = (withinTimeRange "14:00:00" "10:00:00" (convertUTCtoISC (getCurrentUTC "") "HH:mm:ss"))
...
subScriptionFlow = do
  ...
  GetDriverInfoResp getDriverInfoResp <- getDriverInfoDataFromCache globalState false
  let
    reelsRespData = case reelsResp of
      Left err -> API.ReelsResp { reels: [] }
      Right resp -> resp
    ...
    Vehicle linkedVehicle = fromMaybe dummyVehicleObject getDriverInfoResp.linkedVehicle
  modifyScreenState
    $ SubscriptionScreenStateType \subscriptionScreen ->
        subscriptionScreen
          { data
  ...
  case uiAction of
    ...
    CHECK_ORDER_STATUS state orderId -> do
      ...
      case orderStatus of
        Right (OrderStatusRes statusResp) -> do
          let
            status = if statusResp.status == PS.CHARGED then Success else if any (_ == statusResp.status) [ PS.AUTHORIZATION_FAILED, PS.AUTHENTICATION_FAILED, PS.JUSPAY_DECLINED ] then Failed else Pending
  ...
  case action of
    ...
    TURN_ON_GPS ->
      if not internetCondition then
        noInternetScreenFlow "INTERNET_ACTION"
      else do
...
popUpScreenFlow entityPayload = do
  let
    availableRide = (transformAllocationData entityPayload)
...
getDriverInfoDataFromCache (GlobalState globalState) mkCall = do
  if not mkCall && isJust globalState.globalProps.driverInformation then do
    let
      driverInfoResp = fromMaybe dummyDriverInfo globalState.globalProps.driverInformation
    pure driverInfoResp
  else do
    driverInfoResp <- Remote.getDriverInfoBT ""
    modifyScreenState $ GlobalPropsType $ \globalProps -> globalProps { driverInformation = Just $ driverInfoResp }
...
updateDriverStatusGlobal mode active = do
  ...
  (GetDriverInfoResp getDriverInfoResp) <- getDriverInfoDataFromCache globalState false
  let
    updatedResponse = getDriverInfoResp { mode = Just mode, active = active }
  modifyScreenState $ GlobalPropsType $ \globalProps -> globalProps { driverInformation = Just (GetDriverInfoResp updatedResponse) }
...
notificationFlow mbPayload = do
  ...
  case mbPayload of
    Nothing -> pure unit
    Just payload -> case payload ^. _payload ^. _deepLinkJSON of
      Nothing -> pure unit
      Just (CTA.QueryParam queryParam) -> case queryParam.messageId of
        Nothing -> pure unit
        Just id -> do
        ...
        ( \homeScreen ->
            homeScreen
              { data
                ...
                , driverGotoState
                  { gotoCount = driverGoHomeInfo.cnt
    ...
    $ DriverProfileScreenStateType
        ( \driverProfileScreen ->
            driverProfileScreen
              ...
              , props { canSwitchToRental = (getDriverInfoResp.canSwitchToRental), canSwitchToInterCity = getDriverInfoResp.canSwitchToInterCity, canSwitchToIntraCity = getDriverInfoResp.canSwitchToIntraCity }
              }
        )
  modifyScreenState
    $ ReferralScreenStateType
        ( \referralScreen ->
            referralScreen
              { data
                { driverInfo
...
updateInitialCleverTapUserProps (GetDriverInfoResp getDriverInfoResp) = do
  let
    middleName = case getDriverInfoResp.middleName of
      Just "" -> ""
      Just name -> " " <> name
      Nothing -> ""
...
updateAvailableAppsAndGoToSubs = do
  (GlobalState state) <- getState
  let
    isEndRideScreen = state.homeScreen.props.currentStage == ST.RideCompleted
...
getUpiApps = do
  ...
  let
    req = Remote.mkUpdateDriverInfoReq ""
  let
    appsSupportMandate = runFn1 stringifyJSON $ map _.appName $ filter (_.supportsMandate) resp
  let
    appsNotSupportMandate = runFn1 stringifyJSON $ map _.appName $ filter (not _.supportsMandate) resp
  pure
    $ setCleverTapUserProp
        [ { key: "appsSupportMandate", value: unsafeToForeign appsSupportMandate }
...
updateBannerAndPopupFlags = do
  ...
  let
    ...
    shouldShowPopup =
      getValueToLocalStore APP_SESSION_TRACK_COUNT == "true"
        && getValueToLocalNativeStore IS_RIDE_ACTIVE
        == "false"
        ...
        && getValueToLocalStore SHOW_SUBSCRIPTIONS
        == "true"
    ...
    autopayBannerType =
      if subscriptionConfig.enableSubscriptionPopups && getValueToLocalStore SHOW_SUBSCRIPTIONS == "true" then case autoPayNotActive, isOnFreeTrial FunctionCall, (pendingTotalManualDues /= 0.0) of
        true, true, _ -> FREE_TRIAL_BANNER
        _, false, true -> do
          if pendingTotalManualDues < subscriptionRemoteConfig.low_dues_warning_limit then
            LOW_DUES_BANNER
          else if pendingTotalManualDues >= subscriptionRemoteConfig.low_dues_warning_limit && pendingTotalManualDues < subscriptionRemoteConfig.high_due_warning_limit then
            CLEAR_DUES_BANNER
          else if pendingTotalManualDues >= subscriptionRemoteConfig.high_due_warning_limit && pendingTotalManualDues < subscriptionRemoteConfig.max_dues_limit then
    ...
    subscriptionPopupType = case isOnFreeTrial FunctionCall, autoPayNotActive, shouldShowPopup of
      true, true, true
        | showFreeTrialPopupOnDays -> FREE_TRIAL_POPUP
      true, true, true
        | showFreeTrialPopupOnRides -> FREE_TRIAL_RIDES_POPUP
    ...
    moveDriverToOffline =
      (getValueToLocalStore MOVED_TO_OFFLINE_DUE_TO_HIGH_DUE == "")
        ...
        && getValueToLocalNativeStore IS_RIDE_ACTIVE
        == "false"
        && pendingTotalManualDues
        >= subscriptionRemoteConfig.high_due_warning_limit
        && getDriverInfoResp.mode
        /= Just "OFFLINE"
    ...
    showCoinPopup =
      fromMaybe 0 (fromString $ getValueToLocalStore VISITED_DRIVER_COINS_PAGE) == 0
        && appConfig.feature.enableYatriCoins
        && cityConfig.enableYatriCoins
        && toBool (getValueToLocalNativeStore IS_RIDE_ACTIVE)
        == false
        && fromMaybe 0 (fromString $ getValueToLocalStore INTRODUCING_YATRI_POINTS_POPUP_LIMIT)
        < coinsConfig.rideMoreEarnCoinPopupMaxLimit
    ...
    showReferNowPopUp =
      isPayoutEnabled && showReferralPopUp REFER_NOW_LAST_SHOWN ST.ReferNow && isJust payoutVpa
        && case payoutVpaStatus of
    ...
    showVerifyUPIPopUp =
      isPayoutEnabled && showReferralPopUp VERIFY_UPI_LAST_SHOWN ST.VerifyUPI && isJust payoutVpa
        && case payoutVpaStatus of
            Just status -> status == MANUALLY_ADDED
            Nothing -> false
  when shouldDriverbeOffline $ changeDriverStatus Offline
  when moveDriverToOffline
    $ do
        ...
        ( \homeScreen ->
            homeScreen
              ...
              , props
                ...
                , showAcWorkingPopup =
                  if isNothing allState.homeScreen.props.showAcWorkingPopup then
                    getDriverInfoResp.checkIfACWorking
                  else
                    allState.homeScreen.props.showAcWorkingPopup
                }
              }
        )
  where
  ...
  if ((isPopupShownToday coinPopupInfo.rideMoreEarnCoin && (STR.null coinPopupInfo.rideMoreEarnCoin || (runFn2 isDateNDaysAgo coinPopupInfo.rideMoreEarnCoin coinsConfig.rideMoreEarnCoinIntervalLimit))) && checkCoinIsEnabled && isAutoRicksaw && withinTimeRange RC.rideMoreEarnMorePopupStartTime RC.dayEndTime (convertUTCtoISC (getCurrentUTC "") "HH:mm:ss")) then do
    ...
    case resp of
      Right (API.RideStatusPastDaysRes response) ->
        if (response.rideCountPopupValue == false) then
          pure ST.RIDE_MORE_EARN_COIN
        else
          pure ST.NO_COIN_POPUP
...
chooseCityFlow mbEvent = do
  ...
  straightLineDist lat lon state = do
    let
      distanceFromBangalore = getDistanceBwCordinates lat lon 12.9716 77.5946
  ...
  detectCityAPI lat lon state = do
    ...
    case resp of
      Right (API.DetectCityResp resp') -> do
        let
          unserviceableState = ChooseCityScreenStateType \chooseCityScreenState -> chooseCityScreenState { props { locationUnserviceable = true }, data { locationSelected = Nothing } }

          compareStrings = on (==) (toLower <<< trim)
        case resp'.city of
          Just city -> do
            let
              cityInList = any (\cityOb -> compareStrings cityOb.cityName city) state.data.config.cityConfig
...
benefitsScreenFlow = do
  ...
  (GetDriverInfoResp getDriverInfoResp) <- getDriverInfoDataFromCache (GlobalState globalstate) false
  let
    referralCode = getValueToLocalStore REFERRAL_CODE
  ...
  case benefitsScreen of
    ...
    GO_TO_LMS_VIDEO_SCREEN state -> do
      let
        cachedSelectedLanguage = (getValueToLocalStore LMS_SELECTED_LANGUAGE_CACHE)

        cityConfig = (getCityConfig state.data.config.cityConfig (getValueToLocalStore DRIVER_LOCATION))

        selectedLanguage =
          if (cachedSelectedLanguage == "__failed" || cachedSelectedLanguage == "null" || cachedSelectedLanguage == "") then
            if (DS.null cityConfig.languageKey) then (getLanguageLocale languageKey) else cityConfig.languageKey
          else
...
customerReferralTrackerFlow = do
  ...
  logField_ <- lift $ lift $ getLogFields
  let
    customerReferralTrackerScreenState = globalState.customerReferralTrackerScreen
...
addUPIFlow state = do
  ...
  case response of
    Right (API.PayoutRegisterRes payoutRegisterRes) -> do
      let
        (CreateOrderRes createOrderRes) = payoutRegisterRes.orderResp
      ...
      lift $ lift $ doAff $ makeAff \cb -> runEffectFn1 checkPPInitiateStatus (cb <<< Right) $> nonCanceler
      let
        sdkPayload = maybe (encodeJSON createOrderRes.sdk_payload) (addLanguageToPayload (getPaymentPageLangKey (getLanguageLocale languageKey))) createOrderRes.sdk_payload_json
      void $ paymentPageUI sdkPayload
...
checkPaymentStatus orderId = do
  ...
  case orderStatus of
    Right (OrderStatusRes statusResp) -> do
      let
        orderStatus = getOrderStatus statusResp.status
...
lmsVideoScreenFlow = do
  ...
  case lmsVideoScreenAction of
      ...
      case state.props.selectedModule of
        ...
        Just selModule -> do
          ...
          case res of
            Right resp -> do
              let
                (API.LmsGetQuizRes extractedData) = resp
                ...
                $ LmsQuizScreenStateType
                    ( \lmsQuizScreen ->
                        lmsQuizScreen
                          ...
                          , props
                            { selectedLanguage = state.props.selectedLanguage
    ...
    SELECT_LANGUAGE_FOR_VIDEOS state -> do
      let
        genLanguageList = case state.props.selectedModule of
...
selectLanguageForScreenFlow screenName selLanguage genLanguageList = do
  modifyScreenState
    $ SelectLanguageScreenStateType
        ( \selectLangState ->
            selectLangState
              { data { languageList = genLanguageList }
              , props
                { selectedLanguage = selLanguage
...
lmsQuizFlow = do
  ...
  case quizScreenAction of
    ...
    SELECT_LANGUAGE_FOR_QUESTION state -> do
      let
        genLanguageList = case state.props.selectedTranslatedModule of
          Nothing -> state.data.config.languageList
          Just selModule -> HU.generateLanguageList $ selModule ^. _languagesAvailableForVideos
...
goToQuizNextQuestionFlow state = do
  let
    updatedQuestions =
      mapWithIndex
        ( \index question ->
            if (index == state.props.currentQuestionIndex + 1) then
              question { questionStatusDuringQuiz = ST.QUESTION_ATTEMPTING }
            else if (index == state.props.currentQuestionIndex && question.questionStatusDuringQuiz == ST.QUESTION_ATTEMPTING) then
              let
                updatedStatus = case question.previousHistory of
              ...
              in
                question { questionStatusDuringQuiz = updatedStatus }
            else
              question
        )
        state.data.questions
  let
    retryEnabled = getRetryEnabled (state.data.questions !! (state.props.currentQuestionIndex + 1))
  modifyScreenState
    $ LmsQuizScreenStateType
        ( \lmsQuizScreen ->
            lmsQuizScreen
...
retakeQuizFlow state = case state.props.selectedTranslatedModule of
  Nothing -> pure unit
  Just selModule -> do
    res <- lift $ lift $ Remote.getAllLmsQuestions (selModule ^. _moduleId) (HU.getLanguageTwoLetters $ Just state.props.selectedLanguage)
    case res of
      Right resp -> do
        let
          (API.LmsGetQuizRes extractedResp) = resp
        ...
        modifyScreenState
          $ LmsQuizScreenStateType
              ( \lmsQuizScreen ->
                  lmsQuizScreen
                    { data { questions = transformQuizRespToQuestions resp }
                    , props
                      { isRetryEnabled = retryEnabled
                      , showShimmer = false
...
confirmQuestionFlow state =
  let
    moduleId = maybe "" (\selModule -> selModule ^. _moduleId) state.props.selectedTranslatedModule
  in
    case (state.data.questions !! state.props.currentQuestionIndex) of
      ...
      Just questionInfo -> do
        let
          req = case questionInfo.options of
          ...
          Right (API.QuestionConfirmRes resp) -> do
            let
              updatedQuestions =
                map
                  ( \eQuestion ->
                      if eQuestion.questionId == questionInfo.questionId then
                        eQuestion { validationRes = Just (API.QuestionConfirmRes resp), questionStatusDuringQuiz = if resp.validation == API.CORRECT_ANSWER then ST.QUESTION_CORRECT else ST.QUESTION_INCORRECT }
                      else
                        eQuestion
                  )
                  state.data.questions
  ...
  generateQuestionConfirmReq questionId moduleId language selectedOption =
    API.QuestionConfirmReq
...
driverEarningsFlow = do
  ...
  logField_ <- lift $ lift $ getLogFields
  let
    earningScreenState = globalState.driverEarningsScreen
  modifyScreenState $ DriverEarningsScreenStateType (\driverEarningsScreen -> driverEarningsScreen { data { hasActivePlan = globalState.homeScreen.data.paymentState.autoPayStatus /= NO_AUTOPAY, config = appConfig }, props { showShimmer = true } })
  ...
  case uiAction of
    ...
    CONVERT_COIN_TO_CASH state -> do
      ...
      let
        lottieFile = if isRight resp then "ny_ic_coins_redeemed_success.json" else "ny_ic_coins_redeemed_failure.json"
  ...
  updateDriverStats state flow = do
    when (state.props.subView == ST.YATRI_COINS_VIEW)
      $ do
...
rideRequestScreenFlow = do
  ...
  case action of
    TA.GOTO_HOME state -> do
      let
        (ScheduledBookingListResponse listResponse) = state.data.resp
    ...
    TA.GOTO_RIDE_SUMMARY state -> do
      modifyScreenState
        $ RideSummaryScreenStateType
            ( \rideSummaryScreen ->
                RideSummaryScreenData.initData
                  { data
...
handleDriverActivityResp resp = case resp of
  Left err -> do
    let
...
metroWarriorsScreenFlow = do
  let
    config = RC.metroWarriorsConfig (getValueToLocalStore DRIVER_LOCATION) (getValueToLocalStore VEHICLE_VARIANT)
  ...
  case action of
    ...
    TA.UPDATE_WARRIOR_SETTINGS state req@(API.UpdateSpecialLocWarriorInfoReq request) -> do
      ...
      case response of
        Right settings -> do
          let
            warriorSettings = makeStationsData settings state.data.stationList state.data.remoteConfigData
    ...
    SEARCH_LOCATION input updatedState -> do
      ...
      case resp of
        Right (API.AutoCompleteResp autoCompleteResp) -> do
          let
            locList = getLocationList autoCompleteResp.predictions
    ...
    GO_TO_METER_RIDE state -> do
      ...
      case sourcePlaceNameResp of
        Right (API.GetPlaceNameResp placeNameResp) -> do
          let
            _ = spy "GOT PLACE NAME RESPONSE /////////////////////////////" placeNameResp
          case placeNameResp !! 0 of
            Just (API.PlaceName placeName) -> do
              let
                (API.LatLong latLong) = placeName.location
              let
                dstAddr = encodeAddress placeName.formattedAddress placeName.addressComponents Nothing latLong.lat latLong.lon
              let
                _ = spy "GOT ADDRESS /////////////////////////////" placeName
              case allState.meterRideScreen.data.ridesInfo of
                Just (RidesInfo ridesInfoData) -> do
                  let
                    _ = spy "DID API /////////////////////////////" ridesInfoData.id
                  let
                    currentDriverLat = fromMaybe 0.0 $ Number.fromString $ getValueToLocalNativeStore LAST_KNOWN_LAT
                  let
                    currentDriverLon = fromMaybe 0.0 $ Number.fromString $ getValueToLocalNativeStore LAST_KNOWN_LON
                  (LatLon lat lon _) <- getCurrentLocation currentDriverLat currentDriverLon currentDriverLat currentDriverLon 750 false true
                  let
                    srcLat = (fromMaybe 0.0 $ Number.fromString lat)
                  let
                    srcLon = (fromMaybe 0.0 $ Number.fromString lon)
        ...
        Right (API.GetPlaceNameResp placeNameResp) -> case placeNameResp !! 0 of
          Just (API.PlaceName placeName) -> do
            let
              (API.LatLong latLong) = placeName.location
            let
              encodedAddress = encodeAddress placeName.formattedAddress placeName.addressComponents Nothing latLong.lat latLong.lon
            let
              updatedDestination = fromMaybe "" encodedAddress.area
  ...
  case action of
    ...
    MRSO.START_METER_RIDE state -> do
      ...
      let
        srcLon = (fromMaybe 0.0 $ Number.fromString lon)
      let
        sessionToken = JB.getKeyInSharedPrefKeys "SESSION_ID"
      let
        startTime = EHC.getCurrentUTC ""
      let
        referralCode = getValueToLocalStore REFERRAL_CODE
        ...
        Right (API.GetPlaceNameResp placeNameResp) -> case placeNameResp !! 0 of
          Just (API.PlaceName placeName) -> do
            let
              (API.LatLong latLong) = placeName.location
            let
              srcAddr = encodeAddress placeName.formattedAddress placeName.addressComponents Nothing latLong.lat latLong.lon
