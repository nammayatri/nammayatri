...
baseAppFlow baseFlow event driverInfoResponse = do
  ...
  when (baseFlow && bundleSplashConfig.enable)
    $ do
...
getDriverInfoFlow event activeRideResp driverInfoResp updateShowSubscription isAdvancedBookingEnabled updateCTBasicData = do
  ...
  getUpdateToken token =
    let
      UpdateDriverInfoReq initialData = Remote.mkUpdateDriverInfoReq ""
...
updateDriverVersion dbClientVersion dbBundleVersion = do
  if (isJust dbClientVersion && isJust dbBundleVersion) then do
    ...
    if any (_ == -1) [ clientVersion.minor, clientVersion.major, clientVersion.maintenance, bundleVersion.minor, bundleVersion.major, bundleVersion.maintenance ] then
      ...
      let
        (UpdateDriverInfoReq initialData) = mkUpdateDriverInfoReq ""
...
bookingOptionsFlow = do
  ...
  let
    ridePreferences' = transfromRidePreferences resp.tiers
  ...
  case flow of
    ...
    SUBMIT_ISSUE state -> do
      ...
      if state.props.isReversedFlow then do
        ...
        let
          message =
            (getString SELECT_OPTION_REVERSED) <> "\n"
              <> joinWith "\n" options'
        let
          messages' = concat [ descMessages, mediaMessages', [ (makeChatComponent' message Nothing Nothing "Bot" (getCurrentUTC "") "Text" (500 * (length mediaMessages' + 2))) ] ]
        ...
        let
          message = makeChatComponent' (getString ISSUE_SUBMITTED_MESSAGE) Nothing Nothing "Bot" (getCurrentUTC "") "Text" (500 * (length mediaMessages' + 2))
        let
    ...
    CALL_CUSTOMER state -> do
      case state.data.tripId of
        Just tripId -> do
          ...
          void $ lift $ lift $ toggleLoader false
          let
            message = makeChatComponent' (getString ISSUE_SUBMITTED_MESSAGE) Nothing Nothing "Bot" (getCurrentUTC "") "Text" 500
          let
...
currentRideFlow activeRideResp isActiveRide = do
  ...
  homeScreenFlow
  where
  activeRidePatch (GetRidesHistoryResp activeRideResponse) allState onBoardingSubscriptionViewCount = case activeRideResponse.list of
    [] -> do
            ...
            let
              stateChange =
                if (ride.bookingType == Just ADVANCED) then
                  (HomeScreenStateType (\homeScreen -> homeScreen { data { advancedRideData = Just activeRide { source = sourceMod, destination = destinationMod } }, props { silentPopUpView = false, goOfflineModal = false } }))
                else
...
homeScreenFlow = do
  ...
  case action of
    ...
    DRIVER_AVAILABILITY_STATUS state status -> do
      ...
      void $ lift $ lift $ toggleLoader true
      let
        label = if status == Online then "ny_driver_online_mode" else if status == Silent then "ny_driver_silent_mode" else "ny_driver_offline_mode"
    ...
    UPDATE_ROUTE state -> do
      void $ pure $ JB.exitLocateOnMap ""
      let
        srcDestConfig = getSrcDestConfig state
    ...
    $ DriverProfileScreenStateType
        ( \driverProfileScreen ->
            driverProfileScreen
              ...
              , props { canSwitchToRental = (getDriverInfoResp.canSwitchToRental), canSwitchToInterCity = getDriverInfoResp.canSwitchToInterCity }
              }
        )
  modifyScreenState
...
getUpiApps = do
  ...
  let
    (UpdateDriverInfoReq req) = Remote.mkUpdateDriverInfoReq ""
  let
...
getHvErrorMsg errorCode = case errorCode of
