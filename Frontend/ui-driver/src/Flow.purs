...
authenticationFlow _ =
  if EHC.isPreviousVersion (getValueToLocalStore VERSION_NAME) (getPreviousVersion (getMerchant FunctionCall)) then do
...
checkRideAndInitiate event = do
  ...
  let
    activeRide = (not (null activeRideResponse.list))
  activeRide
...
handleDeepLinksFlow event activeRideResp = do
  case event of -- TODO:: Need to handle in generic way for all screens. Could be part of flow refactoring
    Just e -> case e.data of
      "plans"
        | getValueToLocalNativeStore IS_RIDE_ACTIVE /= "true" && getValueToLocalNativeStore DISABLE_WIDGET /= "true" -> do
          liftFlowBT hideSplash
...
onBoardingFlow = do
  ...
  let
    limitReachedFor =
      if resp.rcVerificationStatus == "LIMIT_EXCEED" then
        Just "RC"
      else if resp.dlVerificationStatus == "LIMIT_EXCEED" then
        Just "DL"
      else
        Nothing
        ...
        ( \registerationScreen ->
            registerationScreen
              { data
                ...
                , subscriptionStatus =
                  case getDriverInfoResp.autoPayStatus of
...
selectLanguageFlow = do
  let
    selectLang = getValueToLocalStore LANGUAGE_KEY
...
helpAndSupportFlow = do
  ...
  case action of
    ...
    REPORT_ISSUE_CHAT_SCREEN selectedCategory -> do
                        ...
                        <> if (language == "en") then
                            joinWith " "
                              ( map
                                  ( \optName ->
                                      let
                                        { before, after } = splitAt 1 optName
                                      in
                                        (toUpper before <> after)
                                  )
      ...
      let
        categoryName = getCategoryName selectedCategory.categoryAction
    ...
    SELECT_RIDE state -> do
      ...
      let
        tripId' = case state.selectedItem of
          ...
          _ -> Nothing
      let
        categoryName = getCategoryName state.selectedCategory.categoryAction
...
issueReportChatScreenFlow = do
  ...
  case flow of
    ...
    SUBMIT_ISSUE state -> do
      ...
      _ <- pure $ hideKeyboardOnNavigation true
      let
        showDescription = STR.length (trim issueInfoRes.description) > 0
      let
        descMessages = if showDescription then snoc state.data.chatConfig.messages (makeChatComponent' issueInfoRes.description "Driver" (if (length issueInfoRes.mediaFiles) == 0 then (convertUTCtoISC (getCurrentUTC "") "hh:mm A") else "") "Text" 500) else state.data.chatConfig.messages
      let
        mediaMessages' =
          mapWithIndex
            ( \index media -> do
                if index == length issueInfoRes.mediaFiles - 1 then
      ...
      if state.props.isReversedFlow then do
        ...
        let
          message =
            (getString SELECT_OPTION_REVERSED) <> "\n"
              <> joinWith "\n" options'
        let
          messages' = concat [ descMessages, mediaMessages', [ (makeChatComponent' message "Bot" (convertUTCtoISC (getCurrentUTC "") "hh:mm A") "Text" (500 * (length mediaMessages' + 2))) ] ]
        ...
        let
          message = makeChatComponent' (getString ISSUE_SUBMITTED_MESSAGE) "Bot" (convertUTCtoISC (getCurrentUTC "") "hh:mm A") "Text" (500 * (length mediaMessages' + 2))
        let
    ...
    CALL_CUSTOMER state -> do
      case state.data.tripId of
        Just tripId -> do
          ...
          void $ lift $ lift $ toggleLoader false
          let
            message = makeChatComponent' (getString ISSUE_SUBMITTED_MESSAGE) "Bot" (convertUTCtoISC (getCurrentUTC "") "hh:mm A") "Text" 500
          let
...
currentRideFlow activeRideResp = do
  ...
  homeScreenFlow
  where
  activeRidePatch activeRideResponse allState onBoardingSubscriptionViewCount = case (activeRideResponse.list !! 0) of
...
updateDriverStatus status = do
  if status && getValueToLocalNativeStore DRIVER_STATUS_N == "Silent" then
    Silent
  else if status then
    Online
  else
...
homeScreenFlow = do
  ...
  if (getValueToLocalNativeStore IS_RIDE_ACTIVE) == "true" && (not $ any (\item -> isLocalStageOn item) [ RideAccepted, RideStarted, ChatWithCustomer ]) then
  ...
  case action of
          ...
          $ GlobalPropsType
              ( \globalProps ->
                  globalProps
                    ...
                    , gotoPopupType =
                      case endRideResp.homeLocationReached of
    ...
    UPDATE_ROUTE state -> do
      let
        srcLat = if state.props.currentStage == RideAccepted then state.data.currentDriverLat else state.data.activeRide.src_lat
...
nyPaymentFlow planCardConfig fromScreen = do
  ...
  case getDriverInfoApiResp of
    Right resp -> do
      ...
      let
        (GetDriverInfoResp getDriverInfoResp) = resp
...
getDriverStatesFromCache (GlobalState globalState) = do
  if isJust globalState.globalProps.driverRideStats then do
    let
      driverStats = fromMaybe dummyDriverRideStats globalState.globalProps.driverRideStats
    pure driverStats
  else do
...
updateCleverTapUserProps (GetDriverInfoResp getDriverInfoResp) = do
  ...
  void $ pure
    $ setCleverTapUserProp
        [ { key: "Vehicle Variant", value: unsafeToForeign linkedVehicle.variant }
  ...
  let
    ...
    autopayBannerType =
      if subscriptionConfig.enableSubscriptionPopups && getValueToLocalStore SHOW_SUBSCRIPTIONS == "true" then case autoPayNotActive, isOnFreeTrial FunctionCall, (pendingTotalManualDues /= 0.0) of
        ...
        _, false, true -> do
          if pendingTotalManualDues < subscriptionConfig.lowDuesLimit then
            LOW_DUES_BANNER
          else if pendingTotalManualDues >= subscriptionConfig.lowDuesLimit && pendingTotalManualDues < subscriptionConfig.highDueWarningLimit then
            CLEAR_DUES_BANNER
          else if pendingTotalManualDues >= subscriptionConfig.highDueWarningLimit && pendingTotalManualDues < subscriptionConfig.maxDuesLimit then
    ...
    subscriptionPopupType = case isOnFreeTrial FunctionCall, autoPayNotActive, shouldShowPopup of
      true, true, true -> case freeTrialDays of
        _
          | freeTrialDays == 3 || freeTrialDays == 2 || freeTrialDays == 1 -> FREE_TRIAL_POPUP
    ...
    moveDriverToOffline =
      (getValueToLocalStore MOVED_TO_OFFLINE_DUE_TO_HIGH_DUE == "")
        ...
        >= subscriptionConfig.highDueWarningLimit
...
chooseCityFlow = do
  ...
  let
    cityArray = map (\(API.CityRes item) -> (toUpper item.name)) (resp)
