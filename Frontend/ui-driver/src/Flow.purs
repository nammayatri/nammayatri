...
checkVersion versioncode = do
  when (versioncode < (getLatestAndroidVersion (getMerchant FunctionCall))) $ do
...
homeScreenFlow = do
  ...
  case action of
    ...
    HOMESCREEN_NAV SubscriptionNav -> subScriptionFlow -- paymentHistoryFlow
  ...
  case response of
    Right (SubscribePlanResp listResp) -> do
      case listResp.orderResp of
        Just (CreateOrderRes resp) -> do
          let (PaymentPagePayload sdk_payload) = resp.sdk_payload
          setValueToLocalStore DISABLE_WIDGET "true"
          paymentPageOutput <- paymentPageUI resp.sdk_payload
          setValueToLocalStore DISABLE_WIDGET "false"
          ...
          case orderStatus of
            Right (OrderStatusRes statusResp) ->
              case statusResp.status of
                ...
                _ -> setPaymentStatus Pending sdk_payload.payload
            Left err -> setPaymentStatus Pending sdk_payload.payload --if API fails check with paymentPageOutput
        Nothing -> pure unit -- manual payment, show plan activated 
    Left (error) -> do
...
setPaymentStatus paymentStatus (PayPayload payload) = do
  case payload.mandate of 
    Just mandate -> case paymentStatus of -- ys mandate flow
        Success -> modifyScreenState $ SubscriptionScreenStateType (\subscribeScreenState -> subscribeScreenState { props {popUpState = Just SuccessPopup, paymentStatus = Just paymentStatus }})
        ...
        Pending -> modifyScreenState $ SubscriptionScreenStateType (\subscribeScreenState -> subscribeScreenState { props {paymentStatus = Just paymentStatus }})
...
subScriptionFlow = do
  ...
  case uiAction of
    ...
    NAV RideHistoryNav -> myRidesScreenFlow
    ...
    CANCEL_AUTOPAY state -> do
      void $ lift $ lift $ toggleLoader true
      cancelAutoPayResp <- lift $ lift $ Remote.cancelAutoPay state.data.driverId -- Cancel API
      void $ lift $ lift $ toggleLoader false
      case cancelAutoPayResp of 
        Right resp -> modifyScreenState $ SubscriptionScreenStateType (\subScriptionScreenState -> subScriptionScreenState)
        Left errorPayload -> pure $ toast $ Remote.getCorrespondingErrorMessage errorPayload
      subScriptionFlow
    SWITCH_PLAN state -> do
      void $ lift $ lift $ toggleLoader true
      selectPlanResp <- lift $ lift $ Remote.selectPlan state.data.paymentMode state.data.planId -- SwitchPlan API
      void $ lift $ lift $ toggleLoader false
