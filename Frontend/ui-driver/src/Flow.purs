...
onBoardingFlow = do
  ...
  case flow of
    ...
    AADHAAR_PAN_SELFIE_UPLOAD state (ST.HyperVergeKycResult result) -> do 
        ...
        Just (ST.LIVE_SELFIE (ST.LiveSelfie detail)) | (isJust detail.selfieURL) -> do
          ...
          case image' of 
            ...
            Just image -> do
              ...
              case resp of
                Left _ -> void $ pure $ toast (getString ERROR_OCCURED_PLEASE_TRY_AGAIN_LATER)
                Right response -> pure unit
        Just (ST.PAN_DETAILS (ST.PanDetails detail)) | (isJust detail.panURL && isJust detail.pan) -> do
            ...
            Just image -> do
              ...
              case resp of
                Right (ValidateImageRes response)-> do
                  resp <- lift $ lift $ Remote.registerDriverPAN (Remote.makePANCardReq true currentTime detail.dob detail.name (Just response.imageId) Nothing (fromMaybe "" detail.pan) (convertedStatus) result.transactionId)
                  case resp of
                    Left _ -> void $ pure $ toast (getString ERROR_OCCURED_PLEASE_TRY_AGAIN_LATER)
                    Right response -> pure unit
                _ -> void $ pure $ toast (getString ERROR_OCCURED_PLEASE_TRY_AGAIN_LATER)
            ...
            Just imageFront, Just imageBack -> do
              ...
              case respFrontImage, respBackImage of
                Right (ValidateImageRes frontResp), Right (ValidateImageRes backResp) | isJust result.transactionId -> do
                  resp <- lift $ lift $ Remote.registerDriverAadhaar (Remote.makeAadhaarCardReq (Just backResp.imageId) (Just frontResp.imageId) detail.address true currentTime detail.dob detail.idNumber detail.fullName (convertedStatus) (fromMaybe "" result.transactionId))
                  case resp of
                    Left _ -> void $ pure $ toast (getString ERROR_OCCURED_PLEASE_TRY_AGAIN_LATER)
                    Right response -> pure unit
                _ , _ -> pure unit
      ...
      void $ lift $ lift $ toggleLoader false
      onBoardingFlow
    GO_TO_APP_UPDATE_POPUP_SCREEN _ -> appUpdatedFlow {title : "App Update", description : "", image : ""}
