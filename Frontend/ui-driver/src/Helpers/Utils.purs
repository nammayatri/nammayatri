...
getVehicleType vehicleType = case vehicleType of
...
getRequiredTag maybeLabel = case maybeLabel of
  Just label ->
    if DA.any (_ == label) [ "Accessibility", "GOTO" ] then
      DA.head (DA.filter (\item -> item.label == label) rideLabelConfig)
    else do
      let
        arr = DS.split (DS.Pattern "_") label
      let
        pickup = fromMaybe "" (arr DA.!! 0)
      let
        drop = fromMaybe "" (arr DA.!! 1)
      let
        priority = fromMaybe "" (arr DA.!! 2)
...
getGenderIndex req arr = do
  let
    reqArray = map (\ele -> ele.reasonCode) arr

    reqIndex = DA.elemIndex req reqArray
  reqIndex

getMerchantVehicleSize :: Unit -> Int
getMerchantVehicleSize unit = case getMerchant FunctionCall of
  _ -> 90
...
getCurrentLocation currentLat currentLon defaultLat defaultLon timeOut specialLocation = do
  ...
  if (startRideCurrentLat /= "0.0" && startRideCurrentLong /= "0.0") then
    ...
    if defaultLat /= 0.0 && defaultLon /= 0.0 && currentLat /= 0.0 && currentLon /= 0.0 then do
      let
        distanceDiff = (getDistanceBwCordinates currentLat currentLon defaultLat defaultLon)
        ...
        ( \item ->
            launchAff_
              $ do
                  ...
                  if result then pure unit else liftEffect $ firebaseLogEventWithParams "download_failed" "file_name" item.filePath
        )
        files
...
onBoardingSubscriptionScreenCheck onBoardingSubscriptionViewCount isEnabled =
  isEnabled
    && getValueToLocalNativeStore DRIVER_SUBSCRIBED
    == "false"
    && even onBoardingSubscriptionViewCount
    && onBoardingSubscriptionViewCount
    < 5
    && isOnFreeTrial FunctionCall
    && getValueToLocalNativeStore SHOW_SUBSCRIPTIONS
    == "true"
...
incrementValueOfLocalStoreKey key = do
  let
    value = fromString $ runFn1 getValueToLocalNativeStore key
  case value of
    Just val -> do
      let
        _ = runFn2 setValueToLocalStore (show key) (show (val + 1))
      pure unit
    Nothing -> do
      let
        _ = runFn2 setValueToLocalStore (show key) "1"
      pure unit

contactSupportNumber :: String -> Effect Unit
contactSupportNumber supportType = do
  void $ launchAff $ flowRunner defaultGlobalState $ runExceptT
    $ runBackT do
        config <- getAppConfigFlowBT appConfig
        let
          city = getCityConfig config.cityConfig (getValueToLocalStore DRIVER_LOCATION)
...
splitBasedOnLanguage str =
  let
    strArray = DS.split (DS.Pattern "-*$*-") str
  in
