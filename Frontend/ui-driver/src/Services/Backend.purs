...
withAPIResult url f flow = do
  if (isInvalidUrl url) then
    pure $ Left customError
  else do
    let
      start = getTime unit
    ...
    case resp of
      ...
      Left err -> do
        ...
        let
          userMessage = decodeErrorMessage errResp.errorMessage
...
makeTriggerOTPReq mobileNumber =
  TriggerOTPReq
    let
      operatingCity = getValueToLocalStore DRIVER_LOCATION
    in
  ...
  mkOperatingCity operatingCity =
    if DA.any (_ == operatingCity) [ "__failed", "--" ] then
      Nothing
    else if operatingCity == "Puducherry" then
      Just "Pondicherry"
    else if operatingCity == "Mysore" then
      Just "Bangalore" -- Temp fix TODO: Remove this once Mysore is added in backend
    else
      Just operatingCity
...
driverActiveInactiveBT status status_n = do
  ...
  errorHandler (ErrorPayload errorPayload) = do
    ...
    if accountBlocked then
      modifyScreenState $ HomeScreenStateType (\homeScreen → homeScreen { props { accountBlockedPopup = true } })
    else
      modifyScreenState $ HomeScreenStateType (\homeScreen → homeScreen { props { goOfflineModal = false } })
...
makeEndRideReq lat lon numDeviation tripDistance tripDistanceWithAcc =
  EndRideReq
    { "point":
        Point
          { "lat": lat
...
offerRideBT payload = do
  ...
  errorHandler (ErrorPayload errorPayload) = do
    ...
    if (errorPayload.code == 400) then
      ...
      pure $ toast (getString SOME_ERROR_OCCURED_IN_OFFERRIDE)
    BackT $ pure GoBack

makeOfferRideReq :: String -> Maybe Number -> OfferRideReq
makeOfferRideReq requestId offeredFare =
  OfferRideReq
...
listCancelReasonBT payload = do
  ...
  errorHandler (ErrorPayload errorPayload) = do
    ...
    if (errorPayload.code == 400) then
      ...
      pure $ toast "Some error occured in listCancelReasonBT"
    BackT $ pure GoBack
...
makeGetRouteReq slat slng dlat dlng =
  GetRouteReq
    { "waypoints":
        [ LatLong
            { "lon": slng
...
makeRcActiveOrInactiveReq isActivate rcNo =
...
makeDriverRCReq regNo imageId dateOfRegistration multipleRc =
  DriverRCReq
...
makeDriverDLReq dlNumber dob dateOfIssue imageIdFront imageIdBack =
  DriverDLReq
...
makeValidateImageReq image imageType =
  ValidateImageReq
    { "image": image
    , "imageType": imageType
    }
...
makeReferDriverReq referralNumber =
  ReferDriverReq
...
makeLinkReferralCodeReq referralCode referralLinkPassword =
  LinkReferralCodeReq
...
makeValidateAlternateNumberRequest number =
...
makeResendAlternateNumberOtpRequest number =
...
makeVerifyAlternateNumberOtpRequest otp =
...
leaderBoardBT request = do
  headers <- getHeaders' "" true
  case request of
    (DailyRequest date) -> withAPIResultBT (EP.leaderBoardDaily date) identity errorHandler (lift $ lift $ callAPI headers request)
    (WeeklyRequest fromDate toDate) -> withAPIResultBT (EP.leaderBoardWeekly fromDate toDate) identity errorHandler (lift $ lift $ callAPI headers request)
  where
  errorHandler (ErrorPayload errorPayload) = do
    BackT $ pure GoBack
...
leaderBoard request = do
  headers <- getHeaders "" true
  case request of
    (DailyRequest date) -> withAPIResult (EP.leaderBoardDaily date) unwrapResponse (callAPI headers request)
    (WeeklyRequest fromDate toDate) -> withAPIResult (EP.leaderBoardWeekly fromDate toDate) unwrapResponse (callAPI headers request)
  where
  unwrapResponse (x) = x
...
unVerifiedAadhaarData driverName driverGender driverDob = do
  ...
  makeReq driverName driverGender driverDob =
    UnVerifiedDataReq
...
postRideFeedback rideId rating feedback = do
  ...
  makeReq rideId rating feedback =
    PostRideFeedbackReq
      { rideId: rideId
...
autoComplete searchVal lat lon language = do
  ...
  makeReq searchVal lat lon language =
    AutoCompleteReq
      { components: ""
...
makePlaceNameReq lat lng language =
  GetPlaceNameReq
    { "sessionToken": Just ""
...
makePlaceNameReqByPlaceId placeId language =
  GetPlaceNameReq
    { "sessionToken": Just ""
    , "language": Just language
    , "getBy":
...
addDriverHomeLocation lat lon address tag = do
  ...
  makeAddReq lat lon address tag =
    AddHomeLocationReq
      { position:
          LatLong
            $ { lat: lat
...
updateDriverHomeLocation qParam lat lon address tag = do
  ...
  makeUpdateReq param lat lon address tag =
...
getMerchantOperatingCityListBT _ = do
  let
    id = if (SC.getMerchantId "") == "NA" then getValueToLocalNativeStore MERCHANT_ID else (SC.getMerchantId "")
  headers <- getHeaders' "" false
