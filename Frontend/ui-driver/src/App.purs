...
type FlowBT e a
  = BackT (ExceptT e (Free (FlowWrapper GlobalState))) a
...
data ScreenStage
  = HomeScreenStage HomeScreenStage

data MY_RIDES_SCREEN_OUTPUT
  = HOME_SCREEN
  | PROFILE_SCREEN
...
data REFERRAL_SCREEN_OUTPUT
  = GO_TO_HOME_SCREEN_FROM_REFERRAL_SCREEN
...
data RIDES_SELECTION_SCREEN_OUTPUT
  = REFRESH_RIDES RideSelectionScreenState
  | LOADER_RIDES_OUTPUT RideSelectionScreenState
  | SELECT_RIDE RideSelectionScreenState

data DRIVER_PROFILE_SCREEN_OUTPUT
...
data DRIVER_DETAILS_SCREEN_OUTPUT
  = VERIFY_OTP DriverDetailsScreenState
...
data VEHICLE_DETAILS_SCREEN_OUTPUT
  = UPDATE_VEHICLE_INFO VehicleDetailsScreenState

data ABOUT_US_SCREEN_OUTPUT
  = GO_TO_DRIVER_HOME_SCREEN

data SELECT_LANGUAGE_SCREEN_OUTPUT
  = CHANGE_LANGUAGE

data HELP_AND_SUPPORT_SCREEN_OUTPUT
  = WRITE_TO_US_SCREEN
...
data WRITE_TO_US_SCREEN_OUTPUT
  = GO_TO_HOME_SCREEN_FLOW

data REGISTRATION_SCREEN_OUTPUT
  = UPLOAD_DRIVER_LICENSE RegistrationScreenState
...
data UPLOAD_DRIVER_LICENSE_SCREENOUTPUT
  = VALIDATE_DL_DETAILS UploadDrivingLicenseState
  | VALIDATE_DATA_API UploadDrivingLicenseState
  | GOTO_VEHICLE_DETAILS_SCREEN
  | LOGOUT_ACCOUNT
  | GOTO_ONBOARDING_FLOW

data UPLOAD_ADHAAR_CARD_SCREENOUTPUT
  = GO_TO_ADD_BANK_DETAILS

data BANK_DETAILS_SCREENOUTPUT
  = GO_TO_ADD_VEHICLE_DETAILS

data ADD_VEHICLE_DETAILS_SCREENOUTPUT
  = VALIDATE_DETAILS AddVehicleDetailsScreenState
...
data TRIP_DETAILS_SCREEN_OUTPUT
  = ON_SUBMIT
  | GO_TO_HOME_SCREEN
  | OPEN_HELP_AND_SUPPORT

data PERMISSIONS_SCREEN_OUTPUT
  = DRIVER_HOME_SCREEN
  | LOGOUT_FROM_PERMISSIONS_SCREEN
  | GO_TO_REGISTERATION_SCREEN PermissionsScreenState

data ONBOARDING_SUBSCRIPTION_SCREENOUTPUT
  = MAKE_PAYMENT_FROM_ONBOARDING OnBoardingSubscriptionScreenState
  | REGISTERATION_ONBOARDING OnBoardingSubscriptionScreenState

data HOME_SCREENOUTPUT
...
data REPORT_ISSUE_CHAT_SCREEN_OUTPUT
  ...
  | SUBMIT_ISSUE ReportIssueChatScreenState
  | CALL_CUSTOMER ReportIssueChatScreenState

data RIDE_DETAIL_SCREENOUTPUT
  = GO_TO_HOME_FROM_RIDE_DETAIL
  | SHOW_ROUTE_IN_RIDE_DETAIL

data APPLICATION_STATUS_SCREENOUTPUT
  = GO_TO_HOME_FROM_APPLICATION_STATUS
  | LOGOUT_ACCOUT
...
data EDIT_BANK_DETAILS_SCREEN_OUTPUT
  = EDIT_BANK_DETAILS

data EDIT_AADHAAR_DETAILS_SCREEN_OUTPUT
  = EDIT_AADHAAR_DETAILS

data ENTER_MOBILE_NUMBER_SCREEN_OUTPUT
  = GO_TO_ENTER_OTP EnterMobileNumberScreenState

data ENTER_OTP_SCREEN_OUTPUT
  = RETRY EnterOTPScreenState
  | DRIVER_INFO_API_CALL EnterOTPScreenState

data NO_INTERNET_SCREEN_OUTPUT
  = REFRESH_INTERNET
  | TURN_ON_GPS
  | CHECK_INTERNET

data POPUP_SCREEN_OUTPUT
  = POPUP_REQUEST_RIDE String Number

data DRIVER_RIDE_RATING_SCREEN_OUTPUT
  = CloseScreen
  | SendCustomerFeedBack DriverRideRatingScreenState

data NOTIFICATIONS_SCREEN_OUTPUT
  = REFRESH_SCREEN NotificationsScreenState
...
data ACKNOWLEDGEMENT_SCREEN_OUTPUT
  = EXIT_TO_HOME_SCREEN
  | RETRY_PAYMENT

data AADHAAR_VERIFICATION_SCREEN_OUTPUT
  = ENTER_AADHAAR_OTP AadhaarVerificationScreenState
...
data SUBSCRIPTION_SCREEN_OUTPUT
  = GOTO_HOMESCREEN
...
data PAYMENT_HISTORY_SCREEN_OUTPUT
  = GoToSetupAutoPay PaymentHistoryScreenState
  | EntityDetailsAPI PaymentHistoryScreenState String
  | SWITCH_TAB
  | LOAD_MORE_ITEMS PaymentHistoryScreenState

data APP_UPDATE_POPUP
  = Later
  | UpdateNow

data DRIVE_SAVED_LOCATION_OUTPUT
  = EXIT_FROM_SCREEN
...
data WELCOME_SCREEN_OUTPUT
  = GoToMobileNumberScreen

data CHOOSE_CITY_SCREEN_OUTPUT
  = GoToWelcomeScreen
  | REFRESH_SCREEN_CHOOSE_CITY ChooseCityScreenState

data CHOOSE_LANG_SCREEN_OUTPUT
  = LOGIN_FLOW
