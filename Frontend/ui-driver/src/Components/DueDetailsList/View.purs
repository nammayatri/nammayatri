...
view push state =
  scrollView
    ...
    [ linearLayout
        ...
        ( mapWithIndex
            ( \index item ->
                let
                  rideNumberPrefix = if item.noOfRides < 10 then "0" else ""
                in
                  linearLayout
                    [ width MATCH_PARENT
                    , height WRAP_CONTENT
  ...
  where
  mode item = if item.paymentMode == AUTOPAY_PAYMENT then getString UPI_AUTOPAY_S else if item.isDue then "Manual" else "UPI"
