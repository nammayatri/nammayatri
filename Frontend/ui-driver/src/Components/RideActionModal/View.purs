...
rideActionViewWithLabel push config =
  ...
  , padding $ PaddingTop tagConfig.paddingTop
    , gravity CENTER
      ...
      , margin $ MarginLeft tagConfig.marginLeft
...
showTag config = ((Maybe.isJust config.specialLocationTag) && Maybe.isJust (getRequiredTag config.specialLocationTag)) || config.bookingFromOtherPlatform || config.rideType == ST.Rental || config.rideType == ST.Intercity || config.isInsured
