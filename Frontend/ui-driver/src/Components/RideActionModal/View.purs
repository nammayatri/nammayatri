...
rideTierAndCapacity push config = 
  ...
  where paddingLeft = if Maybe.isJust config.acRide then 4 else 10
        tierName = if config.vehicleType == show SA.AMBULANCE_VENTILATOR then "" else if config.acRide == Maybe.Just true then "AC" else if config.acRide == Maybe.Just false then "Non-AC" else ""
        tierImage = if config.vehicleType == show SA.AMBULANCE_VENTILATOR then "ny_ic_ventilator_blue" else if config.acRide == Maybe.Just true then "ny_ic_ac_white" else if config.acRide == Maybe.Just false then "ny_ic_non_ac_white" else ""
