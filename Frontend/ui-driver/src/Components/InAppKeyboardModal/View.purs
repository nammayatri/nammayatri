...
import PrestoDOM.Properties (background, backgroundDrawable, clickable, color, cornerRadii, cornerRadius, fontStyle, gravity, height, imageUrl, margin, orientation, padding, stroke, text, textSize, weight, width, visibility, letterSpacing, imageWithFallback)
      ...
      [ width MATCH_PARENT
           ...
           , gravity if(state.modalType == KeyboardModalType.OTP) then CENTER else LEFT
             ...
             ([ if state.modalType == KeyboardModalType.OTP then textBoxes push state $ otpTextBoxConfig state 
              ...
              else textView[]] <> [singleTextBox push state] <>
                    [textView $
