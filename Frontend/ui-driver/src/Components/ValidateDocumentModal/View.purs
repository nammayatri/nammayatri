    ...
    , height WRAP_CONTENT
        ...
        , layoutGravity "center"
            ...
            , layoutGravity "center"
                ...
                , layoutGravity "center"
                , cornerRadius 4.0
                , stroke $ (if (state.verificationStatus) then ("4,"<> Color.darkGreen) else ("4,"<> Color.red))
                ]
            ],
        linearLayout
        [ width WRAP_CONTENT
        ...
        , text if (state.verificationStatus) then ("Driving License Validated") else ((getString VERIFICATION_FAILED)<>"  -  "<>(state.failureReason)<>" \n"<>("\nPlease retake photo with Registration \nCertificate inside marked area"))--getString PLEASE_RETAKE_SELFIE))
        ...
        imageView
                ...
                , imageWithFallback "ny_ic_green_box_ticked,https://assets.juspay.in/nammayatri/images/driver/ny_ic_green_box_tick"
                ]
        ]
        ]
    ]
...
primaryButtonConfig state = let 
    ...
    primaryButtonConfig' = config
      { textConfig
      { text = if (state.verificationStatus) then ("CONFIRM AND UPLOAD") else ("RETAKE SELFIE")
