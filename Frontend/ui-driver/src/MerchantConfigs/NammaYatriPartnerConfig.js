...
export function isCurrentMerchantString(key){
    return ((key in kannadaStrings) && (key in hindiStrings) && (key in englishStrings) && (key in tamilStrings));
}

export function getMerchantConfig(key){
    if (key in nammaYatriConfigs){
        return nammaYatriConfigs[key];
    }
...
export function getStringTAValue(key){
