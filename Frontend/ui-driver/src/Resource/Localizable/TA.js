...
export const getTaFromJs  = (key) => {
  const splitedArr = key.trim().split(',');
  const pattern =  taMap.get(splitedArr[0].trim());
  if ( typeof pattern === 'function') {
    let res = pattern.apply(null, splitedArr.slice(1));
    return (typeof value === 'string') ? res : "";
  }else{
    return pattern;
  }
}