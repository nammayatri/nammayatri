...
getFR stringKey =
    case stringKey of
        ...
        CARD_TEXTS -> " est aimé par la plupart de nos clients sur la base des avis et des recommandations."
        TRAININGS_I_COMPLETED -> "Formations que j'ai terminées"
        I_PLEDGE -> "✋🏼  Je m'engage"
        DISCOUNTED -> "Remisé"
        ...
        IS_NOT_SUPPORTED_YET -> " n'est pas encore prise en charge"
        WE_WILL_NOFITY_YOU_WHEN_IT_IS_AVAILABLE vehicle-> "Nous vous informerons lorsque le soutien aux ambulances sera disponible dans votre ville"
        CANNOT_DETECT_PAN_CARD -> "Impossible de détecter le de la carte PAN. Veuillez réessayer."
        CANNOT_DETECT_AADHAAR -> "Impossible de détecter le carte Aadhaar. Veuillez réessayer."
        DOCUMENT_ALREADY_VALIDATED -> "document déjà validé."
        DOCUMENT_UNDER_MANUAL_REVIEW -> "document en cours de révision manuelle"
        ...
        EXPERT_DRIVING -> "Conduite experte"
        CLEAN_VEHICLE -> "Véhicule propre"
        SKILLED_NAVIGATOR -> "Navigateur compétent"
        SAFE_RIDE -> "Trajet sûr"
        POLITE_DRIVER -> "Conducteur poli"
        ON_TIME -> "À l'heure"
        AC_NOT_TURNED_ON -> "Climatisation non activée"
        LATE_PICK_UP_ARRIVAL -> "Retard au ramassage/arrivée"
        ASKED_FOR_MORE_FARE -> "Demande de tarif supplémentaire"
        UNHYGIENIC_VEHICLE -> "Véhicule non hygiénique"
        RASH_DRIVING -> "Conduite imprudente"
        RUDE_DRIVER -> "Conducteur impoli"
        TRAINING -> "Formation"
        FINANCIAL -> "Financier"
        SAFETY -> "Sécurité"
        KIDS_EDUCATION -> "Éducation des enfants"
        BUY_NEW_VEHICLE -> "Acheter un nouveau véhicule"
        NOT_AVAILABLE -> "Non disponible pour le moment"
        PLEASE_WRITE_SOMETHING -> "Veuillez écrire quelque chose"
        BUY_NEW_HOME -> "Acheter une nouvelle maison"
        FAVOURITES -> "Favoris"
        ACCEPT -> "Accepter"
        PASS -> "Passer"
        ...
        TERM_1B time -> "1. L'annulation avant " <> time <>" heure du début prévu du trajet entraînera des pénalités sur la plateforme."
        TERM_2B time-> "2. Soyez en ligne avant " <> time <>" minutes avant le début prévu du trajet."
        TERM_3B time-> "3. Atteignez le lieu de prise en charge du client avant " <> time <>" minutes après l'heure de début prévue du trajet."        
        ...
        INC_2A startTime endTime multiplier -> "Frais de nuit : Conduire entre " <>startTime<> " et " <>endTime<> " aura un multiplicateur de " <>multiplier<>" sur le tarif final."
        INC_2B startTime endTime -> "Suppléments nocturnes: Conduire entre " <>startTime<> " et " <>endTime<> " entraînera l'ajout d'un supplément fixe au tarif final."
        ...
        GO_TO_PICKUP -> "Aller au point de ramassage"
        RIDE_SCHEDULED -> "Trajet planifié!"
        PLEASE_BE_ONLINE time -> "Veuillez être en ligne pendant"<> time <>"minutes"
        BEFORE_THE_RIDE_STARTS -> "Avant que le trajet ne commence"
        TRIP_WILL_BE_ASSIGNED_TO_ANOTHER_DRIVER -> "Sinon, le trajet sera attribué à un autre conducteur"
        RIDE_SUMMARY -> "Résumé du trajet"
        RIDE_ASSIGNED_TO_ANOTHER_DRIVER -> "Trajet attribué à un autre conducteur"
        YOU_CAN_SEE_OTHER_AVAILABLE_RIDE_IN_MORE_RIDES_SECTION -> "Trajet attribué à un autre conducteur"
        ROUND_TRIP -> "Aller-retour"
        UPCOMING -> "À venir"
        FOLLOW_INSTRUCTIONS_TO_AVOID_REASSIGNMENT_OF_RIDE -> "Suivez les instructions <br> pour éviter la réaffectation du trajet"
        BE_WITHIN_10KM_OF_PICKUP -> "Soyez à moins de 10 km du point de prise en charge"
        PLEASE_COLLECT_PARKING_CHARGES -> "Veuillez collecter les frais de stationnement ou d'autres frais."
        ...
        INTERCITY_RIDE -> "Trajet interurbain"
        PLEASE_ENSURE_THAT_YOUR_VEHICLE_IS_READY_FOR_INTERCITY_TRIP -> "Veuillez vous assurer que votre véhicule est prêt pour un trajet interurbain vers "
        PER_KM_CHARGE -> "Tarif par km"
        EXTRA_TIME_CHARGE -> "Frais de temps supplémentaire*"
        ADDED_AT_END_OF_TRIP -> "Ajouté à la fin du trajet"
        DRIVER_ALLOWANCE -> "Indemnité pour les conducteurs"
        ADD_ON_KM_CHARGE -> "Tarif d'ajout par km"
        EXTRA_DISTANCE_CHARGES -> "Frais de distance supplémentaire*"
        BASE_CHARGE charge -> "Tarif de base" <> charge
        THE_CUSTOMER_WILL_PAY_POST_SCHEDULED_RIDE_START_TIME amount -> "Le client paiera "<> amount <>" pour chaque minute supplémentaire d'attente après l'heure prévue de début du trajet."
        CLEAN_AUTO -> ""
        CLEAN_CAB -> ""
