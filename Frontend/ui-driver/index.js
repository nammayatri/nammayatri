...
const blackListFunctions = [
  ...
  "requestPendingLogs",
  "sessioniseLogs",
  "setKeysInSharedPrefs",
  "getLayoutBounds",
];
window.whitelistedNotification = [
  "DRIVER_ASSIGNMENT",
  "CANCELLED_PRODUCT",
  "DRIVER_REACHED",
  "REALLOCATE_PRODUCT",
...
console.log(
  "APP_PERF INDEX_FIREBASE_LOG_PARAMS_START : ",
  new Date().getTime()
);
if (window.JBridge.firebaseLogEventWithParams) {
  Object.getOwnPropertyNames(window.JBridge)
    .filter((fnName) => {
    ...
    .forEach((fnName) => {
      ...
      window.JBridge[fnName] = function () {
        ...
        let shouldLog = true;
        if (window.appConfig) {
          shouldLog = window.appConfig.logFunctionCalls
            ? window.appConfig.logFunctionCalls
            : shouldLog;
        }
        if (shouldLog) {
          window.JBridgeProxy.firebaseLogEventWithParams(
            "ny_fn_" + fnName,
            "params",
...
function setManualEvents(eventName, callbackFunction) {
  ...
  if (!isUndefined(window.__dui_screen)) {
    ...
    if (
      !isUndefined(window.__currScreenName.value0) &&
      window.__dui_screen != window.__currScreenName.value0
    ) {
      console.warn(
        "window.__currScreenName is varying from window.__currScreenName"
      );
    }
  } else {
...
function callInitiateResult() {
  ...
  JBridge.runInJuspayBrowser(
    "onEvent",
    JSON.stringify(jpConsumingBackpress),
    ""
  );
...
window.onMerchantEvent = function (_event, payload) {
  ...
  if (_event == "initiate") {
    ...
    if (clientId == "yatriprovider") {
      ...
      clientId == "jatrisaathiprovider" ||
      clientId == "jatrisaathidriver" ||
      clientId == "yatrisathiprovider"
    ) {
    ...
    try {
      if (
        ...
        window.flowTimeStampObject["onCreateToBundle"] =
          window.prevTimeStamp - onCreateTimeStamp;
        window.flowTimeStampObject["nativeIntiateToBundle"] =
          window.prevTimeStamp - initiateTimeStamp;
        ...
        window.events.onCreateToHomeScreenRenderDuration = new Date(
          clientPaylod.payload.onCreateTimeStamp
        );
        window.events.initAppToHomeScreenRenderDuration = new Date(
          clientPaylod.payload.initiateTimeStamp
        );
      }
      window.flowTimeStampObject["bundleLoadTime"] =
        bundleLoadTime - window.prevTimeStamp;
      window.flowTimeStampObject["bundleToInitiate"] =
        Date.now() - bundleLoadTime;
    ...
    if (
      parsedPayload &&
      parsedPayload.payload &&
      parsedPayload.payload.action == "callDriverAlert" &&
      parsedPayload.payload.id &&
      parsedPayload.payload.popType
    ) {
      ...
      parsedPayload &&
      parsedPayload.payload &&
      parsedPayload.payload.action == "showPopup" &&
      parsedPayload.payload.id &&
      parsedPayload.payload.popType
    ) {
      window.callPopUp(
        parsedPayload.payload.popType,
        parsedPayload.payload.entityPayload
      );
    } else {
      ...
      if (
        parsedPayload.payload.notificationData &&
        parsedPayload.payload.notificationData.notification_type ==
          "NEW_MESSAGE" &&
        parsedPayload.payload.notificationData.entity_ids
      ) {
        purescript.main(
          makeEvent(
            "NEW_MESSAGE",
        ...
        if (
          !checkForReferral(
            parsedPayload.payload.viewParam,
            "REFERRAL_NEW_INTENT"
          )
        ) {
          purescript.onNewIntent(
            makeEvent("DEEP_VIEW", parsedPayload.payload.viewParam)
          )();
        }
      } else if (parsedPayload.payload.view_param) {
        if (
          !checkForReferral(
            parsedPayload.payload.view_param,
            parsedPayload.payload.onNewIntent
              ? "REFERRAL"
              : "REFERRAL_NEW_INTENT"
          )
        ) {
          const deepLinkType = parsedPayload.payload.onNewIntent
            ? "DEEP_VIEW_NEW_INTENT"
            : "DEEP_VIEW";
          const param = parsedPayload.payload.viewParam
            ? parsedPayload.payload.viewParam
            : parsedPayload.payload.view_param;
        ...
        if (
          !checkForReferral(
            parsedPayload.payload.viewParamNewIntent,
            "REFERRAL_NEW_INTENT"
          )
        ) {
          purescript.onNewIntent(
...
window.callUICallback = function () {
  ...
  if (
    ...
    window.__THROTTELED_ACTIONS.indexOf(fName) == -1
  ) {
    window.__PROXY_FN[fName].apply(null, functionArgs);
  } else if (
    window.__LAST_FN_CALLED &&
    fName == window.__LAST_FN_CALLED.fName
  ) {
...
window["onEvent'"] = function (_event, args) {
  ...
  if (_event == "onBackPressed") {
    ...
    (_event == "onKeyboardOpen" || _event == "onKeyboardClose") &&
...
window["onEvent"] = function (jsonPayload, args, callback) {
  // onEvent from hyperPay
