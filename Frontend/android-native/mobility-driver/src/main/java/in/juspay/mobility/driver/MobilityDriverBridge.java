...
public class MobilityDriverBridge extends MobilityCommonBridge {
    ...
    // CallBacks
    private static String storeBatteryUsagePermissionCallBack = null;
    private static String storeOverlayPermissionCallBack = null;
    private static String storeDriverCallBack = null;
    private static String storePopUpCallBack = null;
        ...
        if (PaymentUtils.isClassAvailable("in.juspay.mobility.app.CallBack")) {
            CallBack callBack = new CallBack() {
                ...
                public void imageUploadCallBack(String encImage, String filename) {
                    callImageUploadCallBack(encImage, filename);
                }
                @Override
    ...
    public void storeCallBackForNotification(String callback) {
        ...
        storeLocationPermissionCallBack = null;
        storeOverlayPermissionCallBack = null;
        storeBatteryUsagePermissionCallBack = null;
    }
    ...
    public void setStoreCallBackPopUp(String callback) {
        storePopUpCallBack = callback;
    }

    public void callingStoreCallBackPopUp(JSONObject entity_payload) {
        if (storePopUpCallBack != null) {
            String javascript = String.format(Locale.ENGLISH, "window.callUICallback('%s','%s');",
                    storePopUpCallBack, entity_payload.toString());
            bridgeComponents.getJsCallback().addJsToWebView(javascript);
        }
    }
    ...
    public void callImageUploadCallBack(String stringImage, String imageName) {
        String javascript = String.format(Locale.ENGLISH, "window.callUICallback('%s','%s','%s');",
                storeImageUploadCallBack, stringImage, imageName);
        bridgeComponents.getJsCallback().addJsToWebView(javascript);
    }
    ...
    public void storeCallBackOverlayPermission(String callback) {
        storeOverlayPermissionCallBack = callback;
    }

    public void callOverlayPermissionCallBack(String isPermission) {
        if (storeOverlayPermissionCallBack != null) {
            String javascript = String.format(Locale.ENGLISH, "window.callUICallback('%s','%s');",
                    storeOverlayPermissionCallBack, isPermission);
            bridgeComponents.getJsCallback().addJsToWebView(javascript);
        }
    }
    ...
    public void storeCallBackBatteryUsagePermission(String callback) {
        storeBatteryUsagePermissionCallBack = callback;
    }

    public void callBatteryUsagePermissionCallBack(String isPermission) {
        if (storeBatteryUsagePermissionCallBack != null) {
            String javascript = String.format(Locale.ENGLISH, "window.callUICallback('%s','%s');",
                    storeBatteryUsagePermissionCallBack, isPermission);
            bridgeComponents.getJsCallback().addJsToWebView(javascript);
        }
    }
