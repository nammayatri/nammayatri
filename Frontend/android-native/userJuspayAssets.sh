...
find app/src/user/$1/res/drawable | grep ".png" | cut -d "/" -f 7 | sed 's/.png//' | awk '{print "\"" $1 "\" : true," }' >> assests.json
find app/src/user/$1/res/drawable | grep ".xml" | cut -d "/" -f 7 | sed 's/.xml//' | awk '{print "\"" $1 "\" : true," }' >> assests.json
...
cat juspay_assets.json | json_pp | tee ./app/src/user/$1/assets/juspay/juspay_assets.json
