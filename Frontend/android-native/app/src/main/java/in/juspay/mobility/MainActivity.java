...
public class MainActivity extends AppCompatActivity {
    ...
    private CompletableFuture<JSONObject> initFutureTask;
    long onCreateTimeStamp = 0;
    private String baseUrl = "https://api.sandbox.beckn.juspay.in/dev/dobpp/ui";
    ...
    private String getActiveRide() {
            String rideListUrl = baseUrl + "/driver/ride/list?limit=1&offset=0&onlyActive=true";
            try {
                ...
                MobilityAPIResponse apiResponse = mobilityApiHandler.callAPI(rideListUrl, baseHeaders, null, "GET", false);
                return apiResponse.getResponseBody();
            } catch (Exception error) {
                Log.d(LOG_TAG, "Catch in getActiveRide : " + error);
            }
            return null;
    }

    private String getDriverProfile() {
        String rideListUrl = baseUrl + "/driver/profile";
        try {
            MobilityCallAPI mobilityApiHandler = new MobilityCallAPI();
            Map<String, String> baseHeaders = mobilityApiHandler.getBaseHeaders(context);
            MobilityAPIResponse apiResponse = mobilityApiHandler.callAPI(rideListUrl, baseHeaders, null, "GET", false);
    ...
    private String getDriverProfileStats() {
        final SimpleDateFormat f = new SimpleDateFormat("yyyy-MM-dd",new Locale("en","US"));
        f.setTimeZone(TimeZone.getTimeZone("IST"));
        String currTime = f.format(new Date());
        String rideListUrl = baseUrl + "/driver/profile/stats?day=" + currTime;
        try {
            MobilityCallAPI mobilityApiHandler = new MobilityCallAPI();
            Map<String, String> baseHeaders = mobilityApiHandler.getBaseHeaders(context);
            MobilityAPIResponse apiResponse = mobilityApiHandler.callAPI(rideListUrl, baseHeaders, null, "GET", false);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initFutureTask = CompletableFuture.supplyAsync(() -> {
            CompletableFuture<String> getActiveRideTask = CompletableFuture.supplyAsync(this::getActiveRide);
            CompletableFuture<String> getDriverProfileTask = CompletableFuture.supplyAsync(this::getDriverProfile);
            CompletableFuture<String> getDriverProfileStatsTask = CompletableFuture.supplyAsync(this::getDriverProfileStats);
            ...
            Log.i("APP_PERF", "ON_CREATE_TO_INIT_update remote configs : " + System.currentTimeMillis());

            updateConfigURL();
            Log.i("APP_PERF", "ON_CREATE_TO_INIT : " + System.currentTimeMillis());

            JSONObject results = new JSONObject();
            try {
                String[] allTaskResp = CompletableFuture.allOf(
                                getActiveRideTask, getDriverProfileTask, getDriverProfileStatsTask)
                        .thenApply(voidResult -> new String[]{
                                getActiveRideTask.join(),
                                getDriverProfileTask.join(),
                                getDriverProfileStatsTask.join()
                        }).get();

                String activeRide = allTaskResp[0];
                String driverProfile = allTaskResp[1];
                String driverProfileStats = allTaskResp[2];

                results.put("viewParam", viewParam);
                results.put("deepLinkJson", deepLinkJson);

                JSONObject apiResponse = new JSONObject();

                SharedPreferences sharedPref = context.getSharedPreferences(context.getString(in.juspay.mobility.app.R.string.preference_file_key), Context.MODE_PRIVATE);
                String token = sharedPref.getString("REGISTERATION_TOKEN", "null");
                if (!token.equals("null") && !token.equals("__failed")) {
                    apiResponse.put("activeRide", activeRide);
                    apiResponse.put("driverProfile", driverProfile);
                    apiResponse.put("driverProfileStats", driverProfileStats);
                }

                results.put("apiResponse", apiResponse);
                ...
                switch (event) {
                    case "initiate_result":
                        try {
                            ...
                            initFutureTask.thenAccept(result -> {
                                try {
                                    String viewParam = result.optString("viewParam"), deepLinkJSON = result.optString("deepLinkJson");
                                    JSONObject apiResponse = result.getJSONObject("apiResponse");
                                    ...
                                    if (apiResponse.length() > 0) {
                                        innerPayload.put("apiResponse", apiResponse);
                                    }
