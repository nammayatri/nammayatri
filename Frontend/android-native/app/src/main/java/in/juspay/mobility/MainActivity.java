...
public class MainActivity extends AppCompatActivity {
    ...
    public void updateConfigURL() {
        String merchantId = getResources().getString(R.string.service).equals("nammayatri") ? BuildConfig.MERCHANT_ID_USER : BuildConfig.MERCHANT_ID_DRIVER;
        String baseUrl = getResources().getString(R.string.service).equals("nammayatri") ? BuildConfig.CONFIG_URL_USER : BuildConfig.CONFIG_URL_DRIVER;
        SharedPreferences sharedPref = this.getSharedPreferences(activity.getString(R.string.preference_file_key), Activity.MODE_PRIVATE);
            ...
            try {
                ...
                if (jsonData.has("notification_type") && jsonData.has("entity_ids")) {
                    ...
                    if (type.equals("NEW_MESSAGE")) {
                        hyperServices.process(new JSONObject().put("service", "in.juspay.nammayatri").put("requestId", UUID.randomUUID()).put("payload", new JSONObject().put("action", "callDriverAlert").put("id", id).put("popType", type)));
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
