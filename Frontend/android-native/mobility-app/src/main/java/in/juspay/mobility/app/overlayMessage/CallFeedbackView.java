...
public class CallFeedbackView implements ViewInterface {
    ...
    public View createView(String bundle, Context context, ServiceInterface serviceInterface) {
        ...
        try{
            ...
            okButton.setOnClickListener(view -> Executors.newSingleThreadExecutor().execute(() ->  {
                callFeedbackApi(true);
                serviceInterface.killService();
            }));

            LinearLayout secondaryBtn = view.findViewById(R.id.secondary_button);
            secondaryBtn.setOnClickListener(view -> Executors.newSingleThreadExecutor().execute(() ->  {
    ...
    private void callFeedbackApi(boolean askedExtra){
        ...
        try {
            ...
            mobilityCallAPI.callAPI(url, MobilityCallAPI.getBaseHeaders(context), payload.toString(), new MobilityCallAPI.APICallback() {
                @Override
                public void onResponse(MobilityAPIResponse response) {
                    Log.i(TAG, "response: " + response.getStatusCode() + " " + response.getResponseBody());
                }
