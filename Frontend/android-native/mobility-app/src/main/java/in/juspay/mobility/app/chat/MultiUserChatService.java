...
public class MultiUserChatService extends Service {
    ...
    private final ArrayList<User.Message> messages = new ArrayList<>();
    ...
    private final ChatBinder binder = new ChatBinder();
...
//        MessageOverlayService.registerSendMessageCallBack(sendMessageCallBackOverlay);
    ...
    public void checkAndStartListeners(){
        try {
    ...
    private void startListenerForUser(User user) {
        try {
            if (firestoreInstance != null && user.getChannelId() != null) {
                ListenerRegistration listener = firestoreInstance.collection("Chats").document(user.getChannelId()).collection("messages").orderBy("timestamp", Query.Direction.ASCENDING).addSnapshotListener(
                        (snapshots, e) -> {
                            if (e != null) {
...
//                            int len = snapshots.getDocumentChanges().size();
                            for (DocumentChange dc : snapshots.getDocumentChanges()) {
                                if (dc.getType() == DocumentChange.Type.ADDED) {
                                    user.addMessage(dc.getDocument().getData());
//                                        shouldNotify = true;

//                                        if (count == len && !isChatServiceRunning) {
...
//                                        handleMessage(newMessage, String.valueOf(snapshots.size() - 1));
                            ...
                            for (CallBack cb : callBack) {
                                cb.chatsCallBack(user.getMessages().toString());
                            }
                        }
                );
    ...
    public void addUser (@NonNull User user) {
        if (!users.containsKey(user.getChannelId())) {
    ...
    private void handleMessages() {
        ...
        for (User.Message message : messages) {
            shouldNotify = true;
...
//    }
    ...
    private void handleMessage(User.Message newMessage, String len) {
        try {
    ...
    public void triggerUICallbacks(BridgeComponents bridgeComponents, String callback){
        for (Map.Entry<String, User> entry:   users.entrySet()){
            String script = String.format("window.callUICallback(\"%s\",\"%s\");", callback, entry.getValue().getMessages().toString());
            bridgeComponents.getJsCallback().addJsToWebView(script);
        }
    }
    ...
    public class ChatBinder extends Binder {
        public MultiUserChatService getService() {
            return MultiUserChatService.this;
        }
    }
}
