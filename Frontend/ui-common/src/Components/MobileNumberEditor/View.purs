  ...
  PrestoAnim.animationSet
    ( 
        if EHC.os == "IOS" then
            ...
            [ Anim.listExpandingAnimation $ listExpandingAnimationConfig config.countryCodeField.countryCodeOptionExpended]
          -- else
          --   [ Anim.listExpandingAnimation $ listClosingAnimationConfig config.countryCodeField.countryCodeOptionExpended
          --   , Anim.fadeOut true
          --   ]
    )
    $ scrollView
...
editTextView push config =
  (if (os == "IOS" && (not config.editText.singleLine)) then multiLineEditText else editText)
    ...
    <> ( if config.editText.separator == "" then
          []
        else
