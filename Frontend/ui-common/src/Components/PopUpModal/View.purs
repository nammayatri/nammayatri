                    ...
                    , onAnimationEnd
                        ( \action -> do
                            ...
                            if (supportsInbuildYoutubePlayer unit) then 
                                case mediaType of
                                    "VideoLink" -> pure $ runFn3 setYoutubePlayer (getYoutubeData (getVideoID url) "VIDEO" 0 false) id (show PLAY)
                                    "PortraitVideoLink" -> pure $ runFn3 setYoutubePlayer (getYoutubeData (getVideoID url) "PORTRAIT_VIDEO" 0 false) id (show PLAY)
                                    _ -> pure unit
                                else pure unit
                        )(const NoAction)
                    ][]
