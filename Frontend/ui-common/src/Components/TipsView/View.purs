      ...
      ( mapWithIndex
          ( \index item ->
              linearLayout
                ...
                , accessibilityHint $ "R$" <> show (fromMaybe 100 (state.customerTipArrayWithValues !! index)) <> " Tip" <> (if (state.activeIndex == index) then " Selected" else " : Button")
                    ...
                    ( \itemIndex item ->
                        ...
                        in
                          linearLayout
                            ...
                            , accessibility ENABLE
                            , padding $ if index == 0 then Padding 5 10 5 10 else Padding 8 10 8 10
                            , accessibilityHint $ "R$" <> show (fromMaybe 100 (state.customerTipArrayWithValues !! index)) <> " : Button"
                            , onClick push $ const $ TipBtnClick index (fromMaybe 100 (state.customerTipArrayWithValues !! index))
                            , clickable $ true
                            , background $ if state.activeIndex == index then Color.blue600 else Color.white900
                            , gravity CENTER
