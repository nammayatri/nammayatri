...
view push config =
  linearLayout
    ...
    , case config.primaryContactAndEdit of
        Just item -> primaryContactAndEditView item push
        Nothing -> MP.noView
    ]
      ...
      , relativeLayout
          ( [ width MATCH_PARENT
            ...
            , visibility $ boolToVisibility config.showSosButton
              ...
              <> ( if not config.triggeringSos then
                    [ onClick
                        ( \action -> do
...
primaryContactAndEditView :: forall w. SU.MeasureViewConfig -> (Action -> Effect Unit) -> PrestoDOM (Effect Unit) w
primaryContactAndEditView item push =
  linearLayout
        [ height WRAP_CONTENT
        , width MATCH_PARENT
