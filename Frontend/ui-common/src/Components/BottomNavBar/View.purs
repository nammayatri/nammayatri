    ...
    , gravity CENTER
       ...
       , background Color.white900
         ...
         (\index item ->
            ...
            , onClick push (const (OnNavigate item.screenName))
            ][ textView $
              [ text $ " " <> getString NEW_ <> "✨"
              ...
              , linearLayout
                ...
                , orientation VERTICAL
                , alpha $ if item.activeIcon == "" && index /= state.activeIndex then 0.5 else 1.0
                ][ 
                  -- if ((item.screenName == "alerts_screen") && ((getValueToLocalNativeStore ALERT_RECEIVED) == "true") && state.activeIndex /= 3) then
                  --     lottieLoaderView state push state.activeIndex item.screenName
                  -- else
                      imageView
                      [ width (V 24)
                      , height (V 24)
                      , imageWithFallback if state.activeIndex == index && item.activeIcon /= "" then item.activeIcon else item.defaultIcon
                      ]
                    , textView (
                      [ weight 1.0
                      , height WRAP_CONTENT
...
lottieLoaderView state push activeIndex text =
  ...
  [ width (V 25)
  , height (V 25)
  , visibility VISIBLE -- if((getValueToLocalNativeStore ALERT_RECEIVED) == "false" ) then GONE else VISIBLE
  ][ lottieAnimationView
    [ height (V 25)
    , width (V 25)
