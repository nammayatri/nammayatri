...
view push config = 
  ...
  , onClick push $ const BackPressed
     ...
     , onClick push $ const NoAction
        ...
        , cornerRadii $ Corners 16.0 true true false false
         ...
         , imageView
           ...
           , imageWithFallback case config.currentRateCardType of
              PaymentFareBreakup -> ""
              _ -> if config.nightCharges then "ny_ic_night,https://assets.juspay.in/nammayatri/images/user/ny_ic_night.png" else "ny_ic_day,https://assets.juspay.in/nammayatri/images/user/ny_ic_day.png"
           ]  
         ]
      ,linearLayout
        [ width MATCH_PARENT
        , height if config.currentRateCardType == PaymentFareBreakup then WRAP_CONTENT else V 350 -- check in IOS (Added to handle glitch)
        , orientation HORIZONTAL
      ...
      ][ commonTV push config.buttonText Color.blue800 FontStyle.subHeading1 CENTER 8 (if config.currentRateCardType == DefaultRateCard then Close else GoToDefaultStart)
      ]
    ]      
  ]
