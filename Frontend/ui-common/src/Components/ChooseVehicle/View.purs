  ...
  in
    frameLayout
      ...
      , linearLayout
        ...
        , width $ MATCH_PARENT
        , gravity RIGHT
        , visibility $ boolToVisibility $ (config.vehicleVariant /= "BOOK_ANY") && (config.searchResultType == ESTIMATES || config.searchResultType == QUOTES RENTAL)
        , accessibility DISABLE
        ][linearLayout
          [ height $ V selectedEstimateHeight
          ...
          , onClick push $ const $ case config.showInfo && isActiveIndex of
                                    ...
                                    true  -> if config.showInfo && (config.searchResultType == ESTIMATES || config.searchResultType == QUOTES RENTAL) && not ( config.vehicleVariant `elem` ["AMBULANCE_TAXI", "AMBULANCE_TAXI_OXY", "AMBULANCE_AC", "AMBULANCE_AC_OXY", "AMBULANCE_VENTILATOR"]) then ShowRateCard config else NoAction config                        
          ][]
       ]
    ]
                ...
                ( mapWithIndex
                    ( \itemIndex item ->
                        let
                          ...
                          widthFactor = if item == "Non-AC Mini" then 38 else if (length listItem < 3) && item == "Non-AC Mini" then 20 else if itemCount == 2 then 36 else 28
