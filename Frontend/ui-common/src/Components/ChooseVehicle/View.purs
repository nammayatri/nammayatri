...
vehicleDetailsView push config =
  let selectedVehicle = maybe (getVehicleName config) (\name -> getCustomNameForServiceTier name) config.serviceTierName
  in
  linearLayout
    [ height WRAP_CONTENT
    , width WRAP_CONTENT
  ...
  where
    getCustomNameForServiceTier :: String -> String
    getCustomNameForServiceTier name = 
      case name of
        "Delivery Bike" -> "2 Wheeler"
        _ -> name
