...
module Components.OtpPrimaryEditText.View where

import Prelude (Unit, ($), (<>), (==), (&&), not, const)
...
import Components.OtpPrimaryEditText.Controller (Action(..), Config)
import PrestoDOM (InputType(..),Gravity(..), Length(..), Orientation(..), PrestoDOM, Visibility(..),Margin(..), alpha, background, color, cornerRadius, editText, fontStyle, gravity, height, hint, hintColor, imageUrl, imageView, lineHeight, letterSpacing, linearLayout, margin, onChange, orientation, padding, pattern, singleLine, stroke, text, textSize, textView, visibility, weight, width, id, inputType, multiLineEditText, maxLines, inputTypeI, onFocus, clickable,onClick)
...
textBoxes push state =
  ...
  , onClick push (const (OnClickTextBox state.id))
