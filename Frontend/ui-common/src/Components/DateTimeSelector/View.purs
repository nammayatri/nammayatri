...
returnTextView config = textView $
        [   height WRAP_CONTENT,
            width MATCH_PARENT,
            text $ getString ROUND_TRIP_EXPLAINER,
...
leaveNowButtonConfig state  = let
    ...
    menuButtonConfig' = config {
      titleConfig{
          text = (getString LEAVE_NOW)
        , gravity = CENTER_VERTICAL
        , selectedTextStyle = Body6
        , unselectedTextStyle = Body6
      }
    , accessibilityHint = (getString LEAVE_NOW)
    , radioButtonConfig {
        height = V 16
        , width = V 16
        , cornerRadius = 8.0
...
leaveLaterButtonConfig state  = let
    ...
    menuButtonConfig' = config {
      titleConfig{
          text = (getString RESERVE)
        , gravity = CENTER_VERTICAL
        , selectedTextStyle = Body6
        , unselectedTextStyle = Body6
      }
    , accessibilityHint = (getString RESERVE)
    , radioButtonConfig {
        height = V 16
        , width = V 16
        , cornerRadius = 8.0
