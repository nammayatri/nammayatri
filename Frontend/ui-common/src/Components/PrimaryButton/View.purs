        ...
        [ linearLayout
            ...
            , onClick
                ( \action -> do
                    ...
                    _ <- pure $ startLottieProcess lottieAnimationConfig{ rawJson = getValueFromConfig "apiLoaderLottie", lottieId = (getNewIDWithTag config.id), scaleType = if os == "IOS" then "DEFAULT" else "CENTER_CROP" }
                    push action
                )
                (const OnClick)
