...
getChatConfig state sentBy isLastItem hasTimeStamp =
  if state.userConfig.appType == sentBy then
    {
      margin : Margin ((screenWidth unit)/4) 24 0 if state.spanParent then 0
                                                  else if(os == "IOS" && isLastItem && hasTimeStamp) 
                                                      then 12 else 0,
      gravity : RIGHT,
    ...
    { margin : Margin 0 24 ((screenWidth unit)/4) if state.spanParent then 0
                                                   else if(os == "IOS" && isLastItem && hasTimeStamp) then 12 else 0,
      gravity :  LEFT,
