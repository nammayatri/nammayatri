...
export const killPP = function (services) {
  services.forEach((key) => {
    if (key != JOS.self) {
      ...
      try {
        currJOS.finish(1)(JSON.stringify({ result: "success" }))();
      } catch (err) {
        console.log(err);
      }
...
export const initiatePPSingleInstance = function () {
  if (!JBridge.initiatePP) {
    if (JOS) {
        ...
        const cb = function (code) {
          return function (_response) {
            return function () {
              ...
              console.log(
                "%cHyperpay Terminate Response ",
                "background:darkblue;color:white;font-size:13px;padding:2px",
                code,
                response
              );
            };
      ...
      console.log(
        "%cHyperpay initiate Result - Already Initiated",
        "background:darkblue;color:white;font-size:13px;padding:2px",
        window.__payload
      );
    }
...
export const getAvailableUpiApps = function (resultCb) {
  ...
  if (JOS) {
    try {
        ...
        return function (_response) {
          return function () {
            ...
            try {
              ...
              console.log(
                "%cUPIINTENT initiate Result error",
                "background:darkblue;color:white;font-size:13px;padding:2px",
                err,
                code
              );
      ...
      console.log(
        "%cUPIINTENT initiate Result - Initiated",
        "background:darkblue;color:white;font-size:13px;padding:2px",
        payload
      );
    ...
    console.log(
      "%cUPIINTENT initiate Result - Already Initiated",
      "background:darkblue;color:white;font-size:13px;padding:2px",
      window.__payload
    );
  }
...
export const checkPPInitiateStatus = function (cb, services = microapps) {
  if (
    (ppInitiateStatus() && window.isPPInitiated) ||
    (window.isPPInitiated && checkPPLoadStatus(services))
  ) {
    cb()();
  } else {
        ...
        return function (_response) {
          return function () {
            ...
            if (window.__OS.toUpperCase() == "ANDROID") {
              const ppServices = Object.keys(top.JOSHolder).filter((key) => {
                return key != JOS.self;
              });
              killPP(ppServices);
            }
