...
customError =  { code : 400
  ...
  , response : {
     ...
     , userMessage : getString LT.ERROR_OCCURED_TRY_AGAIN
    }
...
data DriverProfileReq = DriverProfileReq
...
instance standardEncodeDriverProfileReq :: StandardEncode DriverProfileReq where standardEncode (DriverProfileReq) = standardEncode {}
...
instance makeDriverProfileReq :: RestEndpoint DriverProfileReq where
  makeRequest reqBody@(DriverProfileReq) headers = defaultMakeRequest GET (profile "") headers reqBody Nothing
  encodeRequest req = standardEncode req

profile :: String -> String
profile driverId = (getBaseUrl "1") <>"/knowYourFavDriver" <> "/" <> driverId

getDriverProfile driverId= do
        headers <- getHeaders "" true
        withAPIResult (profile driverId)  unwrapResponse $ callAPI headers (DriverProfileReq)
    where
        unwrapResponse (x) = x