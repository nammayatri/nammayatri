...
import Prelude
import Common.Types.App (CalendarModalDateObject, CalendarModalWeekObject, MerchantPayload(..), MobileNumberValidatorResp(..), ModifiedCalendarObject, Payload(..))
...
-- Common Utils
foreign import getFromWindow :: String -> Foreign
...
getFromWindowEff :: EffectFn1 String Foreign
getFromWindowEff = mkEffectFn1 \key -> pure $ getFromWindow key

decodeForeignObject :: forall a. Decode a => String -> Foreign -> a -> a
decodeForeignObject key object defaultObject = case runExcept $ decode object of
  Right decodedObj -> decodedObj
  Left err1 -> do
    let
      _ = printLog "Not able to decode object" $ "Fallbacks to default object for missing Keys" <> (show err1)

      mergedObjects = mergeObjects [ (unsafeToForeign defaultObject), object ]
    case runExcept $ decode mergedObjects of
      Right decodedObjWithDefault -> do
        let
          _ = runFn2 loadInWindow key mergedObjects
        decodedObjWithDefault
      Left err -> do
        let
          _ = printLog "Not able to decode config not able to  find in default config" (show err)
        defaultObject
...
getAppConfigFlowBT :: String -> FlowBT String AppConfig
getAppConfigFlowBT = lift <<< lift <<< getAppConfigFlow

getAppConfigFlow :: String -> Flow GlobalState AppConfig
getAppConfigFlow = liftFlow <<< runEffectFn1 getAppConfigEff

getAppConfigEff :: EffectFn1 String AppConfig
getAppConfigEff =
  mkEffectFn1 \key -> do
    config <- runEffectFn1 getFromWindowEff key
    pure $ decodeForeignObject key config DefaultConfig.config

getAppConfig :: String -> AppConfig
getAppConfig key = do
  let
    config = getFromWindow key
  decodeForeignObject key config DefaultConfig.config
...
-- Bench Marking Utils
foreign import logTimeStampWithKey :: String -> Unit

foreign import logTimeStampWithKeyValue :: Fn2 String Int Unit

foreign import getCurrentTime :: Unit -> Int

logFlowExecTimeWithKey :: forall a. String -> (Flow GlobalState a) -> Flow GlobalState a
logFlowExecTimeWithKey key flow = do
  let
    startTime = getCurrentTime unit
  result <- flow
  let
    endTime = getCurrentTime unit

    timeTaken = endTime - startTime
  liftFlow $ pure $ runFn2 logTimeStampWithKeyValue (key <> "_executionTime") timeTaken
  pure result

logFlowBTExecTimeWithKey :: forall a. String -> FlowBT String a -> FlowBT String a
logFlowBTExecTimeWithKey key flow = do
  let
    startTime = getCurrentTime unit
  result <- flow
  let
    ...
    timeTaken = endTime - startTime
  liftFlowBT $ pure $ runFn2 logTimeStampWithKeyValue (key <> "_executionTime") timeTaken
  pure result

logEffExecTimeWithKey :: forall a b. String -> (a -> Effect b) -> a -> Effect b
logEffExecTimeWithKey key f x = do
  let
    startTime = getCurrentTime unit
  result <- f x
  let
    ...
    timeTaken = endTime - startTime
  pure $ runFn2 logTimeStampWithKeyValue (key <> "_executionTime") timeTaken
  pure result

logExecTimeWithKey :: forall a b. String -> (a -> b) -> a -> b
logExecTimeWithKey key f x = do
  let
    startTime = getCurrentTime unit
    ...
    _ = runFn2 logTimeStampWithKeyValue (key <> "_executionTime") timeTaken
  result

-- Decode Global Payload Utils
getMerchantPayloadFlowBT :: String -> FlowBT String MerchantPayload
getMerchantPayloadFlowBT = lift <<< lift <<< getMerchantPayloadFlow

getMerchantPayloadFlow :: String -> Flow GlobalState MerchantPayload
getMerchantPayloadFlow = liftFlow <<< runEffectFn1 getMerchantPayloadEff

getMerchantPayloadEff :: EffectFn1 String MerchantPayload
getMerchantPayloadEff =
  mkEffectFn1 \key -> do
    config <- runEffectFn1 getFromWindowEff key
    pure $ decodeForeignObject key config dummyGlobalPayload

getMerchantPayload :: String -> MerchantPayload
getMerchantPayload key = do
  let
    config = getFromWindow key
  decodeForeignObject key config dummyGlobalPayload

dummyGlobalPayload :: MerchantPayload
dummyGlobalPayload =
  MerchantPayload
    { betaAssets: Nothing
    , payload:
