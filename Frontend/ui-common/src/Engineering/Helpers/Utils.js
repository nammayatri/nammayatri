...
export const reboot = function () {}; //window.JOS.emitEvent("java")("onEvent")(JSON.stringify({event:"reboot"}))()

export const showSplash = function () {}; //window.JOS.emitEvent("java")("onEvent")(JSON.stringify({event:"show_splash"}))()
...
export const getCurrentDay = function (useMidnightTime) {
  const date = new Date();
  if (useMidnightTime) date.setHours(0, 0, 0, 0);
  return {
    utcDate: date.toISOString(),
    date: date.getDate(),
    shortMonth: date.toLocaleString("default", { month: "short" }),
