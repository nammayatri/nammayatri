...
import JBridge (showMap, debounceFunction, startLottieProcess, toast, lottieAnimationConfig, storeCallBackLocateOnMap, getLayoutBounds, setMapPadding, removeMarker, enableMyLocation)
...
searchLocationScreen initialState globalProps =
  ...
  , eval:
      \action state -> do
        let
          _ = spy "SearchLocationScreen Action" action
  ...
  globalEventsFunc push = do
    ...
    case initialState.props.searchLocStage of
      ...
      ChooseYourRide -> do
        ...
        when (DA.null initialState.data.quotesList)
          $ do
...
view globalProps push state =
  ...
  markerView push state =
    let
      ...
      imageName = case focusedField of
      ...
      labelText =
        if DS.length state.data.defaultGate > state.appConfig.mapConfig.labelTextSize then
...
confirmLocationView push state =
  let
    ...
    _ = spy "INside confirmLocationView state.props" state.props
        ...
        ( DA.mapWithIndex
            ( \index item ->
                linearLayout
                  ...
                  [ MenuButton.view (push <<< MenuButtonAC) (menuButtonConfig state item) ]
            )
...
popUpViews push state globalProps =
  ...
  in
    linearLayout
      ...
      [ if currentStageOn state AllFavouritesStage then
          favouriteLocationModel state globalProps push
        else if (state.props.showSaveFavCard) then
          saveFavCardView push state globalProps
        else if (state.props.locUnserviceable || state.props.isSpecialZone) then
...
footerArray state = case state.props.actionType of
  ...
  _ ->
    if state.props.focussedTextField == MB.Just SearchLocPickup then
      ...
      [ { action: SetLocationOnMap, text: getString SELECT_LOCATION_ON_MAP, buttonType: "SetLocationOnMap", imageName: "ny_ic_locate_on_map,https://assets.juspay.in/nammayatri/images/user/ny_ic_locate_on_map.png" } ]
...
predictionsView push state globalProps =
  let
    ...
    headerText =
      if state.props.isAutoComplete then
        (getString SEARCH_RESULTS)
      else if state.props.actionType == MetroStationSelectionAction then
        "Metro Stations"
      else
  ...
  locationListItemView item index =
    let
      enableErrorFeedback =
        ( state.props.focussedTextField == MB.Just SearchLocPickup
            && state.props.actionType
...
currentStageOn state stage = stage == state.props.searchLocStage
      ...
      case resp of
        ...
        Left err -> do
          ...
          if (err.code == 400 && codeMessage == "ACTIVE_BOOKING_ALREADY_PRESENT") then do
            ...
            if (count == 1) then do
              let
                response = GetQuotesRes { quotes: [], estimates: [], fromLocation: SearchReqLocationAPIEntity { lat: 0.0, lon: 0.0 }, toLocation: MB.Nothing }
              doAff do liftEffect $ push $ action response
            else do
              getEstOrQuotes action flowStatusAction (count - 1) duration push state
    else
