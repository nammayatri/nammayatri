...
import Services.API (QuoteAPIEntity(..), GetQuotesRes(..), OfferRes(..), RentalQuoteAPIDetails(..), QuoteAPIContents(..), Snapped(..), LatLong(..), Route(..), GetBusRouteResp(..))
...
eval (LocationListItemAC _ (LocationListItemController.OnClick item)) state =
  if state.props.actionType == MetroStationSelectionAction || state.props.actionType == BusStationSelectionAction then do
      ...
      let busRouteSelected = item.title
          newState = state {props { routeSelected = busRouteSelected , isAutoComplete = false}}
      void $ pure $ hideKeyboardOnNavigation true
      updateAndExit newState $ PredictionClicked item newState
    else do
      void $ pure $ hideKeyboardOnNavigation true
...
eval (InputViewAC _ (InputViewController.AutoCompleteCallBack value pickUpchanged)) state = do 
  if state.props.actionType == BusRouteSelectionAction then 
    exit $ GoToRouteBusSearch state 
...
eval (InputViewAC _ (InputViewController.InputChanged value)) state = do 
  if (state.props.actionType == MetroStationSelectionAction || state.props.actionType == BusStationSelectionAction )&& not (STR.null value) then do
    ...
    -- else if (state.props.actionType == BusRouteSelectionAction) then do
    --    let newArray = findRouteWithPrefix value state.data.routeSearchedList
    --        canClearText = STR.length value > 2 
    --    continueWithCmd state {data {updatedRouteSearchedList = newArray} , props {canClearText = canClearText , isAutoComplete = canClearText}} [ do
    --      void $ pure $ updateInputString value 
    --      pure NoAction
    -- ]
    else do
...
eval (UpdateLocAndLatLong cachedSearches lat lng) state = do 
  let defaultAddress = if (state.props.actionType == MetroStationSelectionAction || state.props.actionType == BusStationSelectionAction || state.props.actionType == BusRouteSelectionAction ) then "" else "Current Location"
