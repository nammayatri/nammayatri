...
aadhaarNameEditText state = PrimaryEditText.config { 
      ...
      , editText
        { placeholder = "Enter Aadhaar Details"
        , singleLine = true
        , gravity = CENTER_VERTICAL
...
aadhaarOTPEditText state = PrimaryEditText.config { 
      ...
      , topLabel{
        visibility = VISIBLE
      , text = "OTP Sent to your Aadhaar Registered Mobile Number"
      , textStyle = FontStyle.Body3
      , color = Color.black900
      }
      , margin = Margin 0 0 0 0
...
logOutPopUpModelConfig state = PopUpModal.config
            { primaryText { text = "Log Out" }
            , secondaryText { text = "Are you sure you want to log out" }
            , option1 { text = "Go Back", enableRipple = true }
            , option2 { text = "Logout", enableRipple = true }
            }