  ...
  if (activeTag == "") then
    ...
    continue
      state
        ...
        , props
          { isBtnActive =
            if (index == 2 && state.data.addressSavedAs /= "") || (index == 2 && state.props.editLocation == true && state.data.placeName /= "") then
              (not state.props.placeNameExists && true)
            else if (index == 1 || index == 0) then
              (not state.props.placeNameExists && true)
            else
              false
          }
  ...
  case (trim input) of
    ...
    _ ->
      if (validTag state.data.savedTags (trim input) state.data.placeName) then
        ...
        continue state { props { isBtnActive = false, placeNameExists = true } }
