...
eval (TagSelected index) state = do
  ...
  if (isEmpty activeTag) then  continue state{props{isBtnActive = false}, data {activeIndex = Just index,  selectedTag = getTag index }}
    else
...
isValidLocation savedLocations excludeTag placeId = do
 map (\item -> do
    {locationName : item.tag, distanceDiff : 100.0}) (DA.filter (\x -> (if isEmpty placeId then false else (fromMaybe "" x.placeId) == placeId)) (DA.filter (\x -> (toLower x.tag) /= (toLower excludeTag)) savedLocations))
