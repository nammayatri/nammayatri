...
screen initialState =
  ...
  , eval:
      \action state -> do
        let
          _ = spy "AddNewAddressScreenState action " action
        let
          _ = spy "AddNewAddressScreenState state " state
        eval action state
  }
                  ...
                  [ linearLayout
                      ...
                      , textView
                          $ [ height WRAP_CONTENT
                            ...
                            , onClick
                                ( \action ->
                                    if item.tag == "CURRENT_LOCATION" then do
                                      _ <- push action
...
btnData state =
  [ { text: (getString SELECT_ON_MAP), imageUrl: fetchImage FF_ASSET "ny_ic_locate_on_map", action: SetLocationOnMap, tag: "LOCATE_ON_MAP" }
