...
import Accessor (_computedPrice, _contents, _driverName, _estimatedDistance, _id, _otpCode, _rideRating, _toLocation, _vehicleNumber, _stopLocation)
...
myRideListTransformer state listRes = filter (\item -> any (_ == item.status) ["COMPLETED", "CANCELLED", "CONFIRMED"]) (map (\(RideBookingRes ride) ->
  let
    (RideBookingAPIDetails rideApiDetails) = ride.bookingDetails
