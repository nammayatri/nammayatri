      ...
      , scrollView
          ...
          [ if (not hasApiFailed) then
              ...
              , visibility $ boolToVisibility (not hasApiFailed)
                  ...
                  , DropDownCard.view (push <<< IncludedCharges) (dropDownCard state.props.includedChargesOpen (getString LType.TRIP_INCLUSIONS) (includedChargesCard state push))
                  , if tripCategory.tag /= CTA.OneWay then DropDownCard.view (push <<< ExcludedCharges) (dropDownCard state.props.excludedChargesOpen (getString LType.TRIP_EXCLUSION) (excludedChargesCard state push)) else emptyLayout
                  , DropDownCard.view (push <<< Terms) (dropDownCard state.props.termsAndConditionOpen (getString LType.TERMS_AND_CONDITIONS_FULL) (termsAndConditionCard state push))
...
includedChargesBox state push =  
    let 
      ...
      (RateCardItem fare) = getFare state dummyFare SERVICE_CHARGE
      (RateCardItem governmentFare) = getFare state dummyFare GOVERNMENT_CHARGE
      (RateCardItem minFare) = getFare state dummyFare MIN_FARE
      (RateCardItem perHourFare) = getFare state dummyFare PER_HOUR_CHARGE
      (RateCardItem perMinFare) = getFare state dummyFare PER_MINUTE_CHARGE
      (RateCardItem unplannedKMFare) = getFare state dummyFare UNPLANNED_PER_KM_CHARGE
      (RateCardItem perHourKmFare) = getFare state dummyFare PER_HOUR_DISTANCE_KM
      (RateCardItem plannedKMFare) = getFare state dummyFare PLANNED_PER_KM_CHARGE
      (RateCardItem deadKMFare) = getFare state dummyFare DEAD_KILOMETER_FARE
      (RateCardItem waitPerMinFare) = getFare state dummyFare WAITING_CHARGE_PER_MIN
      (RateCardItem nightShiftFare) = getFare state dummyFare NIGHT_SHIFT_CHARGE
    ...
    in
      linearLayout
        ...
        [ linearLayout
          ...
          [ if tripCategory.tag==CTA.InterCity then chargesTile (getString LType.INCLUDED_DISTANCE )(formatDistance roundTrip includedDistance) true state push else chargesTile (getString LType.BASE_CHARGE_INFO) ("₹"<>(show perHourFare.price)) true state push
            , linearLayout [ width WRAP_CONTENT
                , height $ V 24
                , weight 1.0
                , visibility VISIBLE
                ][]
            , chargesTile ((getString LType.EXTRA_TIME_CHARGES) <> "*") (extraTimeFareVal) true state push 
            ]
        , linearLayout
            [ width WRAP_CONTENT
            , weight 1.0
        ...
        , linearLayout
            ...
            [ (case tripCategory.tag of 
                CTA.InterCity -> chargesTile (getString LType.INCLUDED_TIME)(formatDuration includedDuration) true state push
                CTA.Rental -> chargesTile (getString LType.ADDON_KM_CHARGE) ("₹"<>(show plannedKMFare.price)<>"/km") true state push
                _ -> chargesTile (getString LType.PICKUP_CHARGE) ("₹"<>(show deadKMFare.price)) true state push)
           , linearLayout 
                [ width WRAP_CONTENT
                , height $ V 24
                , weight 1.0
              ...
              ,  chargesTile ((getString LType.EXTRA_DISTANCE_FARE)<>"*") (extraDistanceFareVal) true state push
            ]
        ]
        where 
        ...
        formatDistance :: Boolean -> Int -> String  -- same for this
        formatDistance roundTrip duration =
          ...
          in  
            duration'
        getFare :: RideSummaryScreenState -> RateCardItem -> TitleTag -> RateCardItem
        getFare state dummyFare title = fromMaybe dummyFare ((filter (\(RateCardItem x) -> x.title == title) state.data.fareDetails) !! 0)
