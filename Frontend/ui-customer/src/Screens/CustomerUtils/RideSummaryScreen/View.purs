...
import Engineering.Helpers.Commons (screenWidth, convertUTCtoISC, getNewIDWithTag, safeMarginTop, getCurrentUTC, flowRunner)
...
screen initialState =  
  ...
  where 
      globalEventsFn push = do 
        ...
        if initialState.data.fromScreen ==(Screen.getScreen Screen.HOME_SCREEN) then do
            void $ launchAff $ flowRunner defaultGlobalState $ getBookingList GetBookingList CheckFlowStatusAction 10 1000.0 push initialState
        else if initialState.data.fromScreen == (Screen.getScreen Screen.MY_RIDES_SCREEN) then do 
            void $ launchAff $ flowRunner defaultGlobalState $ getBooking GetBooking CheckFlowStatusAction 10 1000.0 push initialState
        else pure unit            
        pure $ pure unit

      getBooking :: forall action. (RideBookingRes -> action) -> action -> Int -> Number -> (action -> Effect Unit) -> RideSummaryScreenState -> Flow GlobalState Unit
      ...
      getBookingList :: forall action. (RideBookingListRes -> action) -> action -> Int -> Number -> (action -> Effect Unit) -> RideSummaryScreenState -> Flow GlobalState Unit
      getBookingList action flowStatusAction count duration push state = do
        if(count > 0 && ((fromMaybe "" state.data.bookingId) == "") && state.data.fromScreen ==(Screen.getScreen Screen.HOME_SCREEN)) then do
          rideBookingListResponse <- Remote.rideBookingList "5" "0" "true"
...
includedChargesBox state push =  
    ...
    in
      linearLayout
        ...
        [ linearLayout
            [ width $ V $ (screenWidth unit) / 2 --  i think i need to check here later
            , height WRAP_CONTENT
            , orientation VERTICAL
            ]
        ...
        where 
        formatDuration :: Int -> String -- might move this to Utils or Common
    ...
    in
      linearLayout 
        ...
        [ textView $ 
            [ width WRAP_CONTENT
            , height WRAP_CONTENT
            , text $ if not state.props.isBookingAccepted then((getString LType.CONFIRM) <> " " <> (getString LType.INTER_CITY_)) else "Done"
...
driverInfoView push state = 
  ...
  linearLayout[
    ...
    imageView[
      ...
      imageWithFallback $ fetchImage FF_ASSET "ic_phone_green",
