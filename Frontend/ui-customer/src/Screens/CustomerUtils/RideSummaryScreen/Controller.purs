...
data Action = BackPressed String
              ...
              | CloseShowCallDialer
              | GetBookingList API.RideBookingListRes
              | CheckFlowStatusAction
              | GetBooking API.RideBookingRes
...
eval (GetBookingList apiResp) state =
  let (API.RideBookingListRes listResp) = apiResp
      (API.RideBookingRes resp) = fromMaybe HomeScreenData.dummyRideBooking $ head listResp.list
  in continue state {data{
          rideDetails = fetchRideDetails state (fromMaybe HomeScreenData.dummyRideBooking $ head listResp.list),
...
fetchRideDetails state apiResp =
  let
    ...
    rideDetails =  BookingAPIEntity{
      ...
      estimatedFare : "₹" <> show resp.estimatedFare, 
      fromLocation :(LocationInformation{
              ...
              id  : fromMaybe "" fromLocation.placeId,
      ...
      toLocation : Just (LocationInformation{
              ...
              id  : fromMaybe "" stopLocation.placeId,
