...
screen initialState =
  ...
  , globalEvents: [(\push -> pure $ globalEvents' push)]
  , eval:
      \action state -> do
        let
  ...
  where
  globalEvents' :: (Action -> Effect Unit) -> Effect Unit
        ...
        [ linearLayout
            ...
            [ linearLayout
                ...
                , textView $ 
                    [ weight 1.0
                    , height WRAP_CONTENT
                    , text if not (null state.data.addedImages)  then (show $ length state.data.addedImages) <> " " <> (if length state.data.addedImages > 1 then (getString IMAGES) else (getString IMAGE)) else (getString ADD_IMAGE)
  ...
  linearLayout
    ...
    , linearLayout
        ...
        , textView $ 
            ...
            , text if state.props.isResolved then getString ISSUE_RESOLVED_TEXT else getString ISSUE_SUBMITTED_TEXT
            , gravity CENTER
