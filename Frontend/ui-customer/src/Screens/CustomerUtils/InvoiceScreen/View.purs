  ...
  let cityStr = getValueToLocalStore CUSTOMER_LOCATION
      ...
      waitingCharges = 
        if state.data.selectedItem.vehicleVariant == Just VV.AUTO_RICKSHAW then
            ...
            bikeWaitingCharges
        else if state.data.selectedItem.vehicleVariant `DA.elem` [Just VV.AMBULANCE_TAXI, Just VV.AMBULANCE_TAXI_OXY, Just VV.AMBULANCE_AC, Just VV.AMBULANCE_AC_OXY, Just VV.AMBULANCE_VENTILATOR] then
