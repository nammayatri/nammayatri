...
view :: forall w . (Action -> Effect Unit) -> SelectFaqScreenState -> PrestoDOM (Effect Unit) w
view push state = 
...
allTopicsView :: SelectFaqScreenState -> (Action -> Effect Unit) -> Array CategoryListType -> forall w . PrestoDOM (Effect Unit) w
...
headingView :: SelectFaqScreenState -> String -> forall w . PrestoDOM (Effect Unit) w
...
apiFailureView :: forall w. SelectFaqScreenState -> (Action -> Effect Unit) -> PrestoDOM (Effect Unit ) w
apiFailureView state push=
  linearLayout
  [ height MATCH_PARENT
  , width MATCH_PARENT
