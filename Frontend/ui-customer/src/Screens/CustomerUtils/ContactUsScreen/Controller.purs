...
eval (PrimaryButtonActionController (PrimaryButton.OnClick)) state = do
  ...
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_write_to_us"
  if (state.props.isSubmitted) then
    updateAndExit state $ GoHome state
  else
    continue state { props { isSubmitted = true } }
