...
eval (CountDownShare seconds id status timerID) state = do
  ...
  if status == "EXPIRED" then do
    let
      newState = state { props { shareTimerValue = 5, shareTimerId = "" } }
    void $ pure $ EHC.clearCountDownTimer state.props.shareTimerId "ShareTimer"
...
eval (CountDown seconds id status timerID) state = do
  ...
  if status == "EXPIRED" then do
    ...
    void $ pure $ EHC.clearCountDownTimer state.props.timerId "RecordingTimer"
    continue newState
  else
