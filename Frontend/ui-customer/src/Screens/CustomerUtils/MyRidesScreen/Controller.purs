...
import Accessor (_amount, _computedPrice, _contents, _description, _driverName, _estimatedDistance, _id, _list, _rideRating, _toLocation, _vehicleNumber, _otpCode, _stopLocation)
...
myRideListTransformer state listRes = filter (\item -> (any (_ == item.status) ["COMPLETED", "CANCELLED", "REALLOCATED", "CONFIRMED" ])) (map (\(RideBookingRes ride) ->
  let
    ...
    rideStartTime : if isScheduled then rideStartTime else (convertUTCtoISC rideStartTime "h:mm A"),
