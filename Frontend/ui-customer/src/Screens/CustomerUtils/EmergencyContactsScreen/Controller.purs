...
eval (ContactsCallback allContacts) state = do
  ...
  if flag == "false"  then do
    ...
    let filteredContacts = map (\contactItem -> do 
                                ...
                                if contactIsSelected formattedContact state.data.emergencyContactsList then formattedContact{isSelected = true} else formattedContact ) $ getContactList updatedContactList
...
eval (NewContactActionController (NewContactController.ContactSelected index)) state = do
  ...
  if (((length state.data.selectedContacts) >= 3) && not item.isSelected ) then do
