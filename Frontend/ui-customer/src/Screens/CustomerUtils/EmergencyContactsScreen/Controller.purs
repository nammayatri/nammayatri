...
eval (PrimaryButtonActionControll PrimaryButton.OnClick) state =
  continueWithCmd state
    [ do
        ...
        let
          _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_add_emergency_contact_click"
        pure NoAction
    ]
...
eval (PopUpModalAction PopUpModal.OnButton2Click) state = do
  ...
  let
    newPrestoList = map (\x -> if ((fromProp x.number) <> (fromProp x.name) == state.data.removedContactDetail.number <> state.data.removedContactDetail.name) then contactTransformerProp state.data.removedContactDetail { isSelected = false } else x) state.data.prestoListArrayItems
...
eval (ContactsCallback allContacts) state = do
  ...
  if (flag == "false") then do
    ...
    let
      newContacts = sortedContactData $ getContactList updatedContactList
    localContacts <- pure $ getValueToLocalStore CONTACTS
    contactsInJson <- pure $ parseNewContacts localContacts
    let
      filteredContacts = map (\x -> if (contactIsSelected x contactsInJson) then x { isSelected = true } else x) newContacts
    let
      unionNewContacts = uniqueContacts [] filteredContacts
...
eval BackPressed state =
  if (state.props.showContactList) then do
    ...
    contactsInJson <- pure $ parseNewContacts localContacts
    let
      filteredContacts = map (\x -> if (contactIsSelected x contactsInJson) then x { isSelected = true } else x { isSelected = false }) state.data.contactsNewList
    let
      unionNewContacts = uniqueContacts [] filteredContacts
...
eval (ContactListScroll value) state = do
  let
    firstIndex = fromMaybe 0 (fromString (fromMaybe "0" ((split (Pattern ",") (value)) !! 0)))
  let
  ...
  let
    canScrollUp = if firstIndex == 0 then false else true
  let
  ...
  if loadMoreButton then
    continueWithCmd state [ do pure LoadMoreContacts ]
  else
    continue state { data { loadMoreDisabled = loadMoreButton } }

eval LoadMoreContacts state = do
  let
    contactsList = sliceContacts state
  let
    bufferCardDataPrestoList = ((contactListTransformerProp (contactsList)))
  let
    loaderBtnDisabled = if (length (contactsList) == 0) then true else false
  let
    offsetForContacts = if (loaderBtnDisabled) then state.data.offsetForEmergencyContacts else state.data.offsetForEmergencyContacts + length contactsList
...
eval (ContactTextChanged value) state = do
  let
    newArray = findContactsWithPrefix value state.data.contactsNewList
...
eval (NewContactActionController (NewContactController.ContactSelected index)) state = do
  let
    contact = fromMaybe { isSelected: false, name: "", number: "" } (state.data.contactsUpdatedNewList !! index)
  let
    item =
  ...
  if (((length state.data.contactsList) >= 3) && not item.isSelected) then do
    ...
    let
      contactListState = if not contact.isSelected then state { data { contactsList = (snoc state.data.contactsList item { isSelected = true }) } } else state { data { contactsList = filter (\x -> ((if DS.length contact.number == 10 then "" else "91") <> x.number <> x.name /= contact.number <> contact.name)) state.data.contactsList } }
    let
      newState =
        contactListState
          { data =
            contactListState.data
              { contactsNewList =
                map
                  ( \x ->
                      if ((x.number <> x.name == contact.number <> contact.name)) then
                        x { isSelected = not (x.isSelected) }
                      else
                        x
                  )
    ...
    let
      contactPropValue = contactTransformerProp contact { isSelected = not contact.isSelected }
    let
      updatedPrestoList =
        updatedNewState
          { data =
            updatedNewState.data
              { prestoListArrayItems =
                map
                  ( \x ->
                      if ((x.number == contactPropValue.number && x.name == contactPropValue.name)) then
                        contactPropValue
                      else
                        x
                  )
...
eval (ContactListPrimaryButtonActionController PrimaryButton.OnClick) state = do
  ...
  let
    selectedContacts = filter (_.isSelected) state.data.contactsNewList
  let
...
contactListTransformerProp contactList =
  ( map
      ( \(contact) ->
...
contactListTransformer contactList =
  ( map
      ( \(contact) ->
          { name: contact.name
...
sliceContacts config = do
  ...
  let
    lastIndex = if ((length (config.data.contactsUpdatedNewList)) < tempLastIndex) then length (config.data.contactsUpdatedNewList) else tempLastIndex
...
contactIsSelected contact tempContactList = do
  let
    ele = filter (\x -> ((if DS.length contact.number == 10 then "" else "91") <> x.number <> x.name == contact.number <> contact.name)) tempContactList
  length ele == 1
