...
eval (ContactsCallback allContacts) state = do
  ...
  if (flag == "false") then do
    ...
    let
      newContacts = getContactList updatedContactList
    localContacts <- pure $ getValueToLocalStore CONTACTS
    contactsInJson <- pure $ parseNewContacts localContacts
    let
      filteredContacts = filter (\x -> filter (\y -> (validContact x.number) <> x.name == (validContact y.number) <> y.name) contactsInJson == []) newContacts
    let
...
eval (ContactListAction ContactListController.ClearText) state =
...
eval (ContactListAction (ContactListController.ContactSelected contact)) state = do
  ...
  if ((state.data.contactsCount >= (3 - (length state.data.contactsList))) && (item.isSelected == false)) then do
          ...
          { data =
            state.data
              { contactsNewList =
                map
                  ( \x ->
                      if ((x.number <> x.name == contact.number <> contact.name) && (state.data.contactsCount < (3 - (length state.data.contactsList)) || x.isSelected == true)) then
                        x { isSelected = not (x.isSelected) }
                      else
                        x
                  )
    ...
    let
      selectedContactsCount = spy "contactsCount" (length $ filter (\x -> x.isSelected) newState.data.contactsNewList)
...
eval (ContactListAction (ContactListController.PrimaryButtonActionController PrimaryButton.OnClick)) state = do
  let
    selectedContacts = (filter (\x -> x.isSelected) state.data.contactsNewList) <> state.data.contactsList
  let
