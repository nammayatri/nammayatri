...
data ScreenOutput
  = UpdateProfile MyProfileScreenState
  | GoToHome MyProfileScreenState
...
eval (EditProfile fieldType) state = do
  case fieldType of
    ...
    _ -> pure unit
  let
    disability = case state.data.disabilityType of
      Just disabilityT -> Just (Constants.getDisabilityType disabilityT.tag state.data.disabilityOptions.disabilityOptionList)
      _ -> Nothing
  continue
    state
      { props { isBtnEnabled = (isNothing state.data.hasDisability), updateProfile = true, isEmailValid = true, isSpecialAssistList = false }
      , data
        { editedName = state.data.name
        , editedEmailId = state.data.emailId
        , editedGender = state.data.gender
...
eval (NameEditTextAction (PrimaryEditText.TextChanged id value)) state = do
  if value == state.data.name then
    continue state
  else if (length (trim value) < 3) then
    continue state { data { editedName = value, nameErrorMessage = checkError "name" (Just state.data.name) value }, props { isNameValid = false, isBtnEnabled = false } }
  else do
    let
      isBtnActive = state.data.emailErrorMessage == Nothing && state.props.isEmailValid && ((trim value) /= state.data.name) || (state.data.emailId /= state.data.editedEmailId)
...
eval (EmailIDEditTextAction (PrimaryEditText.TextChanged id value)) state = do
  let
    isButtonActive = (state.data.nameErrorMessage == Nothing && checkValid state.data.emailId value && (state.data.emailId /= Just value || state.data.name /= state.data.editedName))
  if value == fromMaybe "" state.data.emailId then
    continue state
  else if (state.data.emailId == Nothing) then
    continue
      state
        { data { editedEmailId = Just value, emailErrorMessage = checkError "email" state.data.emailId value }
    ...
    continue
      state
        { data { editedEmailId = Just value, emailErrorMessage = checkError "email" state.data.emailId value }
        , props
          { isEmailValid = checkValid state.data.emailId value
          , isBtnEnabled = isButtonActive && checkValid state.data.emailId value
...
eval (UpdateButtonAction (PrimaryButton.OnClick)) state = do
  ...
  if state.data.gender /= state.data.editedGender then do
    let
      _ = unsafePerformEffect $ logEvent state.data.logField $ if state.props.fromHomeScreen then "banner_gender_selected" else "profile_gender_selected"
    pure unit
  else
    pure unit
  if state.data.editedDisabilityOptions.activeIndex == 1 && state.props.changeAccessibility then
    ...
    let
      newState = state { data { editedDisabilityOptions { editedDisabilityReason = "", selectedDisability = Nothing, otherDisabilityReason = Nothing } } }
        ...
        { data
          { editedDisabilityOptions =
            if idx == 0 then
              ...
              state.data.editedDisabilityOptions { activeIndex = idx }
          }
        }
...
eval (SpecialAssistanceListAC action) state = do
  let
    editedDisabilityOptions = state.data.editedDisabilityOptions
  case action of
...
checkError inputType originalValue value = case inputType of
  "email" ->
    if (length value == 0 && originalValue /= Nothing) then
      Just EMAIL_CANNOT_BE_BLANK
    else if ((length value == 0 && originalValue == Nothing) || (validateEmail value)) then
      Nothing
    else
      Just INVALID_EMAIL
  "name" ->
    if (length (trim value) == 0) then
      Just NAME_CANNOT_BE_BLANK
    else if (length (trim value) < 3) then
      Just INVALID_NAME
    else
      Nothing
...
checkValid originalValue value =
  if (length value == 0 && originalValue == Nothing) then
    true
...
updateProfile (GetProfileRes profile) state = do
  let
    middleName = case profile.middleName of
      Just "" -> ""
      Just name -> (" " <> name)
      Nothing -> ""
    ...
    disabilityOptions =
      state.data.disabilityOptions
        { activeIndex = if hasDisability == Just true then 1 else 0
