...
screen initialState =
  ...
  , eval:
      \action state -> do
        let
          _ = spy "MyProfileScteen action " action
        let
          _ = spy "MyProfileScteen state " state
        eval action state
  }
...
personalDetailsArray state =
  ...
  , { title: (getString ASSISTANCE_REQUIRED)
    , text:
        case state.data.hasDisability of
          Just false -> (getString NO_DISABILITY)
          _ -> case state.data.disabilityType of
            Just disabilityType -> disabilityType.description
            _ -> (getString SET_NOW)
    , fieldType: ST.DISABILITY_TYPE
    , supportText: if (state.data.hasDisability == Just false || isNothing state.data.disabilityType) then Nothing else Just (getString $ LEARN_HOW_TEXT "LEARN_HOW_TEXT")
    }
  ]
  ...
  linearLayout
    ...
    , linearLayout
        ...
        [ linearLayout
            ...
            , height MATCH_PARENT
            , gravity
                $ case state.data.config.profileEditGravity of
                    "bottom" -> BOTTOM
                    _ -> CENTER
            , orientation VERTICAL
            ]
...
emailIdEditTextView push state = PrimaryEditText.view (push <<< EmailIDEditTextAction) (emailEditTextConfig state)
