...
rideInfoTransformer (RideBookingRes resp) message =
  let
    ...
    keyValuePairs =
      ...
      , { key: Pattern "{#FARE_CORRELATION#}", value: Replacement (if finalFare == estimatedFare then "remained same" else if finalFare > estimatedFare then ("was increased by R$" <> show fareDifference) else ("was decreased by R$" <> show fareDifference)) }
