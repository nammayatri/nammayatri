...
instance loggableAction :: Loggable Action where
  performLog action appId = case action of
    ...
    BackPressed flag -> do
      ...
      if flag then
        trackAppScreenEvent appId (getScreen SAVED_LOCATION_SCREEN) "in_screen" "backpress_in_request_delete_fav_location"
      else
...
eval (BackPressed flag) state = do
  if state.props.showDeleteLocationModel then
    continue state { props { showDeleteLocationModel = false }, data { deleteTag = Nothing } }
...
eval (SavedLocationListAPIResponseAction respList) state = do
  ...
  let
    home = (filter (\x -> (toLower x.tag) == "home") (getSavedLocation (respList ^. _list)))
  let
    work = (filter (\x -> (toLower x.tag) == "work") (getSavedLocation (respList ^. _list)))
  let
    otherLocation = (filter (\x -> not ((toLower x.tag) == "home" || (toLower x.tag) == "work")) (getSavedLocation (respList ^. _list)))
...
eval (PrimaryButtonAC (PrimaryButtonController.OnClick)) state = do
  if (length state.data.savedLocations >= 20) then do
    ...
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_add_favourite_click"
...
eval (ErrorModalAC (ErrorModalController.PrimaryButtonActionController PrimaryButtonController.OnClick)) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_add_favourite_click_error_model"
  updateAndExit state $ AddLocation state

eval _ state = continue state

getSavedLocation :: (Array SavedReqLocationAPIEntity) -> Array LocationListItemState
getSavedLocation (savedLocation) =
  ( map
...
getSavedLocationForAddNewAddressScreen (savedLocation) =
  ( map
      ( \(item) ->
