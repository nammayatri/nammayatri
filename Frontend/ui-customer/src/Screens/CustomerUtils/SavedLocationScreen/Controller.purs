...
import Engineering.Helpers.Utils (toggleLoader, isEmpty)
...
decodePlace (SavedReqLocationAPIEntity address )= 
  if ((isEmpty $ trim (fromMaybe "" address.area)) &&(isEmpty $ trim (fromMaybe "" address.street)) &&(isEmpty $ trim (fromMaybe "" address.door)) &&(isEmpty $ trim (fromMaybe "" address.building)) ) then
