...
eval AddUPI state = continue state { props { showUPIPopUp = true } }

eval EditUPI state = continue state { props { showUPIPopUp = true , isUpiUpdated = true} }

eval CloseUPI state = continue state { props { showUPIPopUp = false } }

eval (VpaTextChanged upi) state = continue state { data { vpa = upi, verificationStatus = UpiNotVerified } }

eval VerifyVPA state = updateAndExit state { data { verificationStatus = UpiVerifying } } $ VerifyVPAOut state

eval CopyToClipboard state = do
  let
    _ = copyToClipboard state.data.referralCode
