...
getDriverInfo vehicleVariant (RideBookingRes resp) isQuote =
  let
    (RideAPIEntity rideList) = fromMaybe dummyRideAPIEntity ((resp.rideList) DA.!! 0)
  in
    { otp: if isQuote then fromMaybe "" ((resp.bookingDetails) ^. _contents ^. _otpCode) else rideList.rideOtp
    , driverName:
        if length (fromMaybe "" ((split (Pattern " ") (rideList.driverName)) DA.!! 0)) < 4 then
          (fromMaybe "" ((split (Pattern " ") (rideList.driverName)) DA.!! 0)) <> " " <> (fromMaybe "" ((split (Pattern " ") (rideList.driverName)) DA.!! 1))
        else
    ...
    , vehicleVariant:
        if rideList.vehicleVariant /= "" then
          rideList.vehicleVariant
        else
...
encodeAddressDescription address tag placeId lat lon addressComponents = do
  let
    totalAddressComponents = DA.length $ split (Pattern ", ") address
...
getPlaceNameResp address placeId lat lon item = do
  ...
  getPlaceNameRes = case placeId of
  ...
  checkLatLon = case item.lat, item.lon of
...
getUpdatedLocationList locationList placeId =
  ( map
      ( \item ->
...
transformSavedLocations array = case DA.head array of
  Just item -> do
    case item.lat, item.lon, item.fullAddress.ward of
      Just 0.0, Just 0.0, Nothing -> updateSavedLocation item 0.0 0.0
      Just 0.0, Just 0.0, Just _ -> updateSavedLocation item 0.0 0.0
      Just lat, Just lon, Nothing -> updateSavedLocation item lat lon
      Nothing, Nothing, Nothing -> updateSavedLocation item 0.0 0.0
      _, _, _ -> pure unit
...
updateSavedLocation item lat lon = do
  ...
  let
    (PlaceName placeName) = (fromMaybe dummyLocationName (placeNameResp DA.!! 0))
  let
...
getSpecialZoneQuote quote index = case quote of
  Quotes body ->
    let
      ...
      mappedEstimates =
        map
          ( \(EstimateAPIEntity estimate) ->
              ...
              in
                { item: (EstimateAPIEntity estimate), order: orderNumber }
          )
          estimates
...
getFareFromEstimate (EstimateAPIEntity estimate) = do
  case estimate.totalFareRange of
    ...
    Just (FareRange fareRange) ->
      if fareRange.minFare == fareRange.maxFare then
        estimate.estimatedTotalFare
      else
        fareRange.minFare
    ...
    ChooseVehicle.config
      ...
      , price =
        case estimate.totalFareRange of
          ...
          Just (FareRange fareRange) ->
            if fareRange.minFare == fareRange.maxFare then
              currency <> (show estimate.estimatedTotalFare)
            else
              currency <> (show fareRange.minFare) <> " - " <> currency <> (show fareRange.maxFare)
      , activeIndex = 0
      , index = index
      , id = trim estimate.id
...
getNearByDrivers estimates = DA.nub (getCoordinatesFromEstimates [] estimates)
  ...
  getCoordinatesFromEstimate (EstimateAPIEntity estimate) =
    let
      latLngs = estimate.driversLatLong
    in
...
getEstimatesInfo estimates vehicleVariant state =
  let
    estimatedVarient = if vehicleVariant == "" then estimates else filter (\x -> x ^. _vehicleVariant == vehicleVariant) estimates
...
getSpecialTag specialTag = case specialTag of
  Just tag ->
    let
      zones = split (Pattern "_") tag
    ...
    in
      { sourceTag: sourceTag, destinationTag: destinationTag, priorityTag: priorityTag }
  Nothing -> dummyZoneType

getZoneType :: Maybe String -> ZoneType
getZoneType tag = case tag of
