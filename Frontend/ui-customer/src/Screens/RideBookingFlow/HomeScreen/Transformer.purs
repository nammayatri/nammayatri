      ...
      case quoteDetails.contents of 
        INTER_CITY (IntercityQuoteAPIDetails interCityObj) ->
          (if(not isRoundTrip) then 
            (maybe []  (\plannedPerKmRateOneWay -> [{key : (getString DISTANCE_FARE), val : (currency <> (show $  plannedPerKmRateOneWay ^. _amount ) <> ( " / km"))}]) interCityObj.plannedPerKmRateOneWay)
          else
            (maybe []  (\plannedPerKmRateRoundTrip -> [{key : (getString DISTANCE_FARE), val : (currency <> (show $  plannedPerKmRateRoundTrip ^. _amount ) <> ( " / km"))}]) interCityObj.plannedPerKmRateRoundTrip) )<> 
          (maybe []  (\perHourCharge -> [ { key: (getString DRIVER_ALLOWANCE_), val: (currency <> (show  $  perHourCharge ^. _amount) <> " / hr") } ]) interCityObj.perHourCharge) <>
