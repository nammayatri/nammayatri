...
import Data.Ord
import Data.Eq
import Data.Maybe (Maybe(..), fromMaybe, isJust, maybe, fromJust)
...
getEstimates (EstimateAPIEntity estimate) index isFareRange =
  ...
  in ChooseVehicle.config {
      ...
      , isOurQuote = false -- fromMaybe true estimate.valueAddNP -- this decides providers flow after search
      }
...
estimatesGroupbyProviderId :: Array EstimateAPIEntity -> EstimateAndQuoteConfig -> Map.Map String (Array CVC.Config)
estimatesGroupbyProviderId estimateArray config =
  let x = getEstimateList estimateArray config
      pairsArray = map (\obj -> Tuple obj.providerId [obj]) x
  in foldl (\acc (Tuple k v) -> Map.insertWith append k v acc) Map.empty pairsArray
...
-- mkProvidersQuoteList :: Array EstimateAPIEntity -> EstimateAndQuoteConfig -> Array ST.ProvidersQuote
-- mkProvidersQuoteList estimateArray config = do
--   let chooseVcArray = getEstimateList estimateArray config
--       pairsArray = map (\obj -> mkPl obj ) chooseVcArray
--   DA.nubBy (\a b -> (compare a.providerId b.providerId)) pairsArray
--   where
--     mkPl ob = {
...
mkProvidersQuoteList :: Array EstimateAPIEntity -> EstimateAndQuoteConfig -> Array ST.ProvidersQuote
mkProvidersQuoteList estimateArray config = do
  let chooseVcArray = getEstimateList estimateArray config --arr
      pairsArray = map (\obj -> mkPl obj ) chooseVcArray
