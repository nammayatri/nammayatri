...
cancelAppConfig state = let
  ...
  popUpConfig' = config'{
      ...
      coverImageConfig {
        imageUrl = fetchImage FF_ASSET $ if (fromMaybe dummyDriverInfo state.data.driverInfoCardState).distance <= 500
                    then if (fromMaybe dummyDriverInfo state.data.driverInfoCardState).vehicleVariant == "AUTO_RICKSHAW"  then "ny_ic_driver_near_auto" else "ny_ic_driver_near"
                    else if (fromMaybe dummyDriverInfo state.data.driverInfoCardState).vehicleVariant == "AUTO_RICKSHAW" then  "ny_ic_driver_started_auto" else "ny_ic_driver_started"
      , visibility = VISIBLE
      , margin = Margin 16 20 16 24
      , width = MATCH_PARENT
      , height = V 200
  ...
  in popUpConfig'
      where distanceString = getDistanceString (fromMaybe dummyDriverInfo state.data.driverInfoCardState).distance (fromMaybe 0 (fromMaybe dummyDriverInfo state.data.driverInfoCardState).initDistance) state.props.zoneType.priorityTag
...
chatViewConfig state = let
  ...
  chatViewConfig' = config {
    userConfig
        {
          userName = (fromMaybe dummyDriverInfo state.data.driverInfoCardState).driverName
        , appType = "Customer"
        }
      , messages = state.data.messages
      , messagesSize = state.data.messagesSize
      , sendMessageActive = state.props.sendMessageActive
      , vehicleNo = HU.makeNumber $ (fromMaybe dummyDriverInfo state.data.driverInfoCardState).registrationNumber     
      , suggestionsList = if showSuggestions state then if (metersToKm (fromMaybe dummyDriverInfo state.data.driverInfoCardState).distance state) == getString AT_PICKUP then getSuggestionsfromKey "customerInitialAP" else getSuggestionsfromKey "customerInitialBP" else state.data.suggestionsList
      ...
      , showAutoGeneratedText = (getValueToLocalStore NOTIFIED_CUSTOMER == "true") && (HU.secondsToHms (fromMaybe dummyDriverInfo state.data.driverInfoCardState).eta) /= ""
      , rideConfirmedAt = (fromMaybe dummyDriverInfo state.data.driverInfoCardState).startedAt
      , autoGeneratedText = state.data.config.notifyRideConfirmationConfig.autoGeneratedText <> (HU.secondsToHms (fromMaybe dummyDriverInfo state.data.driverInfoCardState).eta)
  }
  in chatViewConfig'
...
driverInfoTransformer state =
  let cardState = (fromMaybe dummyDriverInfo state.data.driverInfoCardState)
  in
    { otp : cardState.otp
...
rideCompletedCardConfig state = let
  ...
  config' = config{
        ...
        topCard {
          ...
          initalAmount = (fromMaybe dummyDriverInfo state.data.driverInfoCardState).price,
          fareUpdatedVisiblity = state.data.finalAmount /= (fromMaybe dummyDriverInfo state.data.driverInfoCardState).price && state.props.estimatedDistance /= Nothing,
          ...
          infoPill {
            ...
            imageVis = VISIBLE,
            visible = if state.data.finalAmount == (fromMaybe dummyDriverInfo state.data.driverInfoCardState).price || state.props.estimatedDistance == Nothing then GONE else VISIBLE
          },
