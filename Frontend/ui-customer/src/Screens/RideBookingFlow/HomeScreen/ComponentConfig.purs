...
import Screens.Types (DriverInfoCard, Stage(..), ZoneType(..), TipViewData, TipViewStage(..), TipViewProps, ReferralStatus(..), VehicleViewType(..))
...
logOutPopUpModelConfig state = case state.props.isPopUp of
  ...
  ST.TipsPopUp -> do
    let
      tipConfig = getTipConfig state.data.selectedEstimatesObject.vehicleVariant $ getValueToLocalStore CUSTOMER_LOCATION
...
chooseYourRideConfig state =
  let
    tipConfig = getTipConfig state.data.selectedEstimatesObject.vehicleVariant city
