...
waitTimeInfoCardConfig _ = let
  ...
  , imageConfig {
      imageUrl = "ny_ic_wait_timer," <> (getAssetStoreLink FunctionCall) <> "ny_ic_wait_timer.png",
      height = V 130,
      width = V 130,
...
driverInfoCardViewState state = { props:
                                  ...
                                  , showChatNotification : state.props.showChatNotification || (state.data.lastMessage.sentBy == "" && state.props.isMessagesLoaded)
                                  ...
                                  , sentQuickMessage : state.props.sentQuickMessage
                                  , isMessagesLoaded : state.props.isMessagesLoaded
                                  }
...
messagingViewConfig state = let
  ...
  messagingViewConfig' = config {
      ...
      , config = state.data.config
      , chatSheetState = state.props.chatSheetState
      , peekHeight = state.data.peekHeight
      , chatSheetSlide = state.props.chatSheetSlide
      , isExpanding = state.props.isChatExpanding
      , isKeyboardOpen = state.props.isKeyboardOpen
      , sheetState = state.props.bottomSheetState
  }
...
driverInfoTransformer state =
  ...
  in
    ...
    , chatSuggestions : []--if showSuggestions state then if (metersToKm state.data.driverInfoCardState.distance state) == getString AT_PICKUP then getSuggestionsfromKey "customerInitialAP" else getSuggestionsfromKey "customerInitialBP" else state.data.suggestionsList
    , quickMessage : state.data.lastSentMessage
    }
