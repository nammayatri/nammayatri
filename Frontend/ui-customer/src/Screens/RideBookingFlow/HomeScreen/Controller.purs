...
import Engineering.Helpers.Commons (clearTimer, flowRunner, getNewIDWithTag, os, getExpiryTime, convertUTCtoISC, getCurrentUTC, isPreviousVersion, getDeviceHeight, screenHeight, clearCountDownTimer)
...
eval (NotifyDriverStatusCountDown seconds id status timerID) state = do 
  if status == "EXPIRED" then do
    void $ pure $ clearCountDownTimer "notifyCustomer"
...
eval (RepeatRideCountDown seconds id status timerID) state = do
  if status == "EXPIRED" then do
    void $ pure $ clearCountDownTimer "repeatRide"
