...
eval (DriverInfoCardActionController (DriverInfoCardController.MessageDriver)) state = do
  if state.data.config.feature.enableChat && state.data.driverInfoCardState.providerType == CTP.ONUS && HU.disableChatForDelivery state.data.fareProductType then do
...
eval (DeliveryParcelImageOtpAction (DeliveryParcelImageAndOtp.CheckImageUploadStatus PrimaryButtonController.OnClick)) state = 
  -- continueWithCmd state [pure $ RefreshDelveryParcelImage]
