...
eval (UpdateLocation key lat lon) state = case key of
  "LatLon" -> do
    let isSourceManuallyMoved = state.props.hotSpot.manuallyMoved
        selectedSpot = (filter (\spots -> (getDistanceBwCordinates (fromMaybe 0.0 (NUM.fromString lat)) (fromMaybe 0.0 (NUM.fromString lon)) spots.lat spots.lng) * 1000.0 < 1.0 ) state.data.nearByPickUpPoints) !! 0
    exit $ UpdateLocationName state{props{defaultPickUpPoint = "", hotSpot{ manuallyMoved = if state.props.isSource == Just true then true else isSourceManuallyMoved, selectedSpot = selectedSpot }}} (fromMaybe 0.0 (NUM.fromString lat)) (fromMaybe 0.0 (NUM.fromString lon))
...
eval (UpdatePickupLocation  key lat lon) state = do
  case key of
    "LatLon" -> do
      let isSourceManuallyMoved = state.props.hotSpot.manuallyMoved
          selectedSpot = (filter (\spots -> (getDistanceBwCordinates (fromMaybe 0.0 (NUM.fromString lat)) (fromMaybe 0.0 (NUM.fromString lon)) spots.lat spots.lng) * 1000.0 < 1.0 ) state.data.nearByPickUpPoints) !! 0
      exit $ UpdatePickupName state{props{defaultPickUpPoint = "", hotSpot{ manuallyMoved = if state.props.isSource == Just true then true else isSourceManuallyMoved, selectedSpot = selectedSpot }}} (fromMaybe 0.0 (NUM.fromString lat)) (fromMaybe 0.0 (NUM.fromString lon))
    _ -> do
