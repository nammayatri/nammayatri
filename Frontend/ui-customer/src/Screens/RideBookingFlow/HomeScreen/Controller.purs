...
eval (ShowProviderInfo showProviderInfo) state =
  continue
    state
      { data
        { iopState
...
eval (UpdateFollowers (FollowRideRes resp)) state = do
  let
    followers = map (\(Followers follower) -> follower) resp
  continue
    state
      { data
...
eval (UpdateRepeatTrips rideList) state = do
  ...
  let
    shimmerState = state { props { showShimmer = false } }
  ...
  if not (null list) then do
    let
      updatedMap = updateMapWithPastTrips list shimmerState
...
eval (Scroll item) state = do
  let
    sheetState =
      if item == state.props.currSlideIndex then
        state.props.isHomescreenExpanded
      else
        item > state.props.currSlideIndex
...
eval ReAllocate state =
  if isLocalStageOn ReAllocated then do
    let
      updatedState = state { props { currentStage = FindingQuotes } }
...
eval (BannerCarousel (BannerCarousel.OnClick idx)) state =
  continueWithCmd state
    [ do
        let
          banners = if state.props.currentStage == HomeScreen then getBannerConfigs state BannerCarousel else getDriverInfoCardBanners state BannerCarousel
        case index banners idx of
          Just config -> do
            let
              _ = runFn2 updatePushInIdMap "bannerCarousel" false
            case config.type of
              ...
              BannerCarousel.Remote link -> do
                if os == "IOS" && STR.contains (STR.Pattern "vp=sedu&option=video") link -- To be removed after deep links are added in iOS then
...
eval SearchForSelectedLocation state = do
  let
    currentStage = if state.props.searchAfterEstimate then TryAgain else FindingEstimate
...
eval (NotifyDriverStatusCountDown seconds status timerID) state = do
  if status == "EXPIRED" then do
    ...
    let
      eta = fromMaybe 0 state.data.driverInfoCardState.eta
...
eval (RepeatRideCountDown seconds status timerID) state = do
  if state.props.currentStage == FindingQuotes then do
    ...
    let
      updatedState = state { data { rideHistoryTrip = Nothing }, props { repeatRideTimerId = "", repeateRideTimerStoped = true, searchExpire = (getSearchExpiryTime true) } }
    updateAndExit (updatedState) (GetQuotes updatedState)
  else
    continue state { props { repeatRideTimer = (show seconds), repeatRideTimerId = timerID, repeateRideTimerStoped = false } }
...
eval (AutoScrollCountDown seconds status timerID) state = do
  if status == "EXPIRED" then do
    ...
    void $ pure $ performHapticFeedback unit
    let
      updatedState = state { props { autoScroll = false, autoScrollTimerId = "", homeScreenSheetState = EXPANDED, isHomescreenExpanded = true, currSlideIndex = 1.0, autoScrollTimer = "" } }
    continue updatedState
  else
    continue state { props { autoScrollTimer = (show seconds), autoScrollTimerId = timerID } }
...
eval (UpdateCurrentStage stage (RideBookingRes resp)) state = do
  ...
  let
    ...
    searchResultType =
      if (fareProductType == FPT.ONE_WAY_SPECIAL_ZONE || otpCode /= Nothing) then
        QUOTES
      else if fareProductType == FPT.INTER_CITY then
        INTERCITY
      else if fareProductType == FPT.RENTAL then
        RENTALS
      else
        ESTIMATES
...
eval OnResumeCallback state =
  if (state.props.currentStage == RideAccepted || state.props.currentStage == RideStarted) && state.props.emergencyHelpModelState.waitingDialerCallback then
  ...
  else case getValueToLocalNativeStore LOCAL_STAGE of
    "FindingQuotes" -> do
      let
        secondsLeft = findingQuotesSearchExpired false true
      case (methodArgumentCount "startLottieProcess") == 1 of
        true -> do
          let
            findingQuotesProgress = 1.0 - (toNumber secondsLeft) / (toNumber (getSearchExpiryTime true))
    ...
    "QuoteList" -> do
      let
        findingQuotesProgress = 1.0 - 30.0 / (toNumber (getSearchExpiryTime true))
      void $ pure $ startLottieProcess lottieAnimationConfig { rawJson = "progress_loader_line", lottieId = (getNewIDWithTag "lottieLoaderAnimProgress"), minProgress = findingQuotesProgress, scaleType = "CENTER_CROP" }
      continue state
    "RideAccepted"
      | (state.data.fareProductType == FPT.ONE_WAY_SPECIAL_ZONE) -> exit $ Retry state
    "ConfirmingQuotes" -> do
      let
        findingQuotesProgress = 1.0 - 30.0 / (toNumber (getSearchExpiryTime false))
      void $ pure $ startLottieProcess lottieAnimationConfig { rawJson = "progress_loader_line", lottieId = (getNewIDWithTag "lottieLoaderAnimProgress"), minProgress = findingQuotesProgress, scaleType = "CENTER_CROP" }
      continue state
    _ -> continue state
...
eval (RideCompletedAC (RideCompletedCard.IssueReportIndex index)) state = case index of
  ...
  case last allMessages of
    Just value ->
      if value.message == "" then
        ...
        if value.sentBy == currentUser then
          updateMessagesWithCmd state { data { messages = allMessages, chatSuggestionsList = if state.props.isChatWithEMEnabled then getSuggestionsfromKey emChatSuggestion emergencyContactInitialChatSuggestionId else [], lastMessage = value, lastSentMessage = value }, props { canSendSuggestion = true, isChatNotificationDismissed = false } }
        else do
          let
...
eval (DriverInfoCardActionController DriverInfoCardController.RateCardInfo) state = case state.data.rateCardCache of
...
eval (MessagingViewActionController (MessagingView.Call)) state = do
  ...
  if state.props.isChatWithEMEnabled then do
    let
      filterContacts = filter (\item -> item.priority == 0) $ fromMaybe [] state.data.contactList
    ...
    callDriver state $ fromMaybe "ANONYMOUS" $ state.data.config.callOptions !! 0
...
eval (DriverInfoCardActionController (DriverInfoCardController.MessageDriver)) state = do
  if state.data.config.feature.enableChat && state.data.driverInfoCardState.providerType == CTP.ONUS then do
    if not state.props.chatcallbackInitiated || state.data.waitTimeInfo then
      continue state
    else do
      void $ pure $ performHapticFeedback unit
...
eval (MessageExpiryTimer seconds status timerID) state = do
  let
    newState = state { data { triggerPatchCounter = state.data.triggerPatchCounter + 1 } }
  if status == "EXPIRED" then do
    _ <- pure $ clearTimerWithId timerID
    let
      currentUser = if state.props.isChatWithEMEnabled then (getValueFromCache (show CUSTOMER_ID) getKeyInSharedPrefKeys) else "Customer"
...
eval (ScrollStateChanged scrollState) state = do
  let
    sheetState = case scrollState of
      "1" -> STATE_DRAGGING
      "2" -> STATE_SETTLING
      "3" -> STATE_EXPANDED
      "4" -> STATE_COLLAPSED
...
eval BackPressed state = do
  ...
  case state.props.currentStage of
    ...
    SettingPrice -> do
      ...
      void $ pure $ clearTimerWithId state.props.repeatRideTimerId
      let
        updatedState = state { props { repeatRideTimer = "", repeatRideTimerId = "", isSearchCancelled = false } }
    ...
    FindingEstimate -> do
      ...
      _ <- pure $ updateLocalStage SearchLocationModel
      let
        newState = state { props { rideRequestFlow = false, currentStage = SearchLocationModel, searchId = "", isSource = Just false, isSearchLocation = SearchLocation } }
    ...
    ChatWithDriver -> do
      if state.props.showCallPopUp then
        continue state { props { showCallPopUp = false } }
      else do
    ...
    _ -> do
      if state.props.isLocationTracking then
        continue state { props { isLocationTracking = false } }
      else if state.props.cancelSearchCallDriver then
        continue state { props { cancelSearchCallDriver = false } }
      else if state.props.showCallPopUp then
        continue state { props { showCallPopUp = false } }
      else if state.props.isCancelRide then
      ...
      else if state.props.showShareAppPopUp then
        continue state { props { showShareAppPopUp = false } }
      else if state.props.showMultipleRideInfo then
        continue state { props { showMultipleRideInfo = false } }
      ...
      else if state.props.callSupportPopUp then
        continue state { props { callSupportPopUp = false } }
      else if state.data.ratingViewState.openReportIssue then
        continue state { data { ratingViewState { openReportIssue = false } } }
      ...
      else if state.data.waitTimeInfo then
        continue state { data { waitTimeInfo = false } }
      else if state.props.showSpecialZoneInfoPopup then
        continue state { props { showSpecialZoneInfoPopup = false } }
      else if state.props.zoneOtpExpired then
        continue state { props { zoneOtpExpired = false } }
      else if state.props.showScheduledRideExistsPopUp then
        continue state { props { showScheduledRideExistsPopUp = false } }
      else do
        pure $ terminateApp state.props.currentStage true
        continue state
...
eval (MAPREADY key latitude longitude) state = case key of
  _ ->
    continueWithCmd state
      [ do
...
eval OpenSearchLocation state = do
  ...
  let
    _ = unsafePerformEffect $ Events.addEventData "External.Clicked.PickupSearch" "true"
  let
    srcValue = if state.data.source == "" then (getString CURRENT_LOCATION) else state.data.source
  _ <- pure $ updateLocalStage SearchLocationModel
...
eval (UpdateLocation key lat lon) state = do
  ...
  if os == "IOS" && not state.props.locateOnMapProps.cameraAnimatedToSource && (getDistanceBwCordinates latitude longitude state.props.sourceLat state.props.sourceLong) > 5.0 then do
    continueWithCmd state { props { locateOnMapProps { cameraAnimatedToSource = true } } }
      [ do
    ...
    let
      updatedState = state { props { locateOnMapProps { cameraAnimatedToSource = true } } }
    ...
    case key of
      "LatLon" -> do
        let
          selectedSpot = head (filter (\spots -> (getDistanceBwCordinates latitude longitude spots.lat spots.lng) * 1000.0 < (toNumber JB.locateOnMapConfig.thresholdDistToSpot)) updatedState.data.nearByPickUpPoints)
...
eval (UpdatePickupLocation key lat lon) state = do
  ...
  if os == "IOS" && not state.props.locateOnMapProps.cameraAnimatedToSource && (getDistanceBwCordinates latitude longitude state.props.sourceLat state.props.sourceLong) > 5.0 then do
    ...
    case key of
      "LatLon" -> do
        let
          selectedSpot = head (filter (\spots -> (getDistanceBwCordinates (fromMaybe 0.0 (NUM.fromString lat)) (fromMaybe 0.0 (NUM.fromString lon)) spots.lat spots.lng) * 1000.0 < (toNumber JB.locateOnMapConfig.thresholdDistToSpot)) updatedState.data.nearByPickUpPoints)
...
eval (CheckBoxClick autoAssign) state = do
  void $ pure $ performHapticFeedback unit
  let
    event = if autoAssign then "ny_user_pref_autoassigned" else "ny_user_pref_driveroffers"
  let
    _ = unsafePerformEffect $ logEvent state.data.logField event
...
eval (RatingCardAC (RatingCard.Rating index)) state = do
  let
    feedbackListArr = if index == state.data.rideRatingState.rating then state.data.rideRatingState.feedbackList else []
...
eval (RatingCardAC (RatingCard.SelectPill feedbackItem id)) state = do
  let
    newFeedbackList = updateFeedback id feedbackItem state.data.rideRatingState.feedbackList
...
eval (SettingSideBarActionController (SettingSideBarController.PastRides)) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_myrides_click"
...
eval (SettingSideBarActionController (SettingSideBarController.ChangeLanguage)) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_language"
...
eval (SettingSideBarActionController (SettingSideBarController.GoToAbout)) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_about"
...
eval (SettingSideBarActionController (SettingSideBarController.GoToMyTickets)) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_zoo_tickets"
...
eval (SettingSideBarActionController (SettingSideBarController.GoToMyMetroTickets)) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_metro_tickets"
  exit $ GoToMyMetroTickets state { data { settingSideBar { opened = SettingSideBarController.OPEN } } }

eval (SettingSideBarActionController (SettingSideBarController.ShareAppLink)) state = exit $ GoToReferral GIVE_REFERRAL state
...
eval (SearchLocationModelActionController (SearchLocationModelController.PrimaryButtonActionController PrimaryButtonController.OnClick)) state = do
  ...
  _ <- pure $ exitLocateOnMap ""
  let
    newState = state { props { isSource = Just false, isSearchLocation = SearchLocation, currentStage = SearchLocationModel, locateOnMap = false, defaultPickUpPoint = "" } }
...
eval (PrimaryButtonActionController (PrimaryButtonController.OnClick)) newState = do
  ...
  let
    state = newState { data { rentalsInfo = Nothing } }
  case state.props.currentStage of
    HomeScreen -> do
      void $ pure $ performHapticFeedback unit
      let
        _ = unsafePerformEffect $ Events.addEventData "External.Clicked.DestinationSearch" "true"
    ...
    ConfirmingLocation -> do
      ...
      let
        _ = unsafePerformEffect $ Events.addEventData "External.Clicked.ConfirmLocation" "true"
      let
        _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_confirm_pickup"
      let
        updatedState = state { props { currentStage = FindingEstimate, locateOnMap = false }, data { iopState { showMultiProvider = false } } }
    ...
    SettingPrice -> do
      ...
      void $ pure $ cacheRateCard state
      let
        updatedState = state { data { rideHistoryTrip = Nothing }, props { searchExpire = (getSearchExpiryTime true) } }
      updateAndExit (updatedState) (GetQuotes updatedState)
    _ -> continue state
...
eval (RideCompletedAC (RideCompletedCard.SkipButtonActionController (PrimaryButtonController.OnClick))) state = do
  if state.data.fareProductType == FPT.RENTAL then
    continue state { data { fareProductType = FPT.ONE_WAY } }
  else case state.data.ratingViewState.issueFacedView of
    true -> do
    ...
    false ->
      ...
      else if state.data.ratingViewState.selectedRating > 0 then
        updateAndExit state $ SubmitRating state { data { rideRatingState { rating = state.data.ratingViewState.selectedRating } } }
      else do
...
eval OpenSettings state = do
  _ <- pure $ hideKeyboardOnNavigation true
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_burger_menu"
...
eval (SearchExpireCountDown seconds status timerID) state = do
  if status == "EXPIRED" then do
    _ <- pure $ clearTimerWithId timerID
    let
      tipViewData = HomeScreenData.initData.props.tipViewProps
    ...
    let
      enableTips = isTipEnabled state
    if any (_ == state.props.currentStage) [ FindingQuotes, QuoteList ] then
      continue state { props { searchExpire = seconds, timerId = timerID, tipViewProps { isVisible = enableTips && (seconds <= (getSearchExpiryTime true) - state.data.config.tipDisplayDuration || state.props.tipViewProps.isVisible || state.props.tipViewProps.activeIndex >= 0) }, customerTip { enableTips = enableTips } } }
    else do
      _ <- pure $ clearTimerWithId timerID
...
eval CancelSearch state = case state.props.currentStage of
  FindingEstimate -> do
    ...
    _ <- pure $ updateLocalStage SearchLocationModel
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_estimate_cancel_search"
  ...
  _ <-
    pure
      $ if getValueToLocalStore DRIVER_ARRIVAL_ACTION == "TRIGGER_WAITING_ACTION" then
          setValueToLocalStore DRIVER_ARRIVAL_ACTION "WAITING_ACTION_TRIGGERED"
        else
          pure unit
...
eval (DriverInfoCardActionController (DriverInfoCardController.RideDurationTimer timerID timeInHHMM _)) state = continue state { props { rideDurationTimerId = timerID, rideDurationTimer = timeInHHMM } }

eval (RideDurationTimer timerID timeInHHMM _) state = continue state { props { rideDurationTimerId = timerID, rideDurationTimer = timeInHHMM } }
...
eval (SpecialZoneOTPExpiryAction seconds status timerID) state = do
  if status == "EXPIRED" then do
    ...
    let
      timeInMinutes = formatDigits $ seconds / 60
...
eval (CancelSearchAction PopUpModal.OnButton2Click) state = do
  let
    isAcCab = ServiceTierCard.showACDetails (fromMaybe "" state.data.driverInfoCardState.serviceTierName) Nothing
...
eval (DriverInfoCardActionController (DriverInfoCardController.CancelRide infoCard)) state =
  if (state.data.config.driverInfoConfig.showCancelPrevention && not state.props.isSpecialZone) || state.props.zoneType.sourceTag == METRO then
    ...
    continueWithCmd state [ pure $ CancelSearchAction PopUpModal.OnButton2Click ]
...
eval OpenEmergencyHelp state = do
  ...
  if state.props.isOffline then do
    ...
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "ny_ic_safety_center_clicked"
...
eval (DriverInfoCardActionController (DriverInfoCardController.ShareRide)) state =
  if state.props.isOffline then do
    ...
    if state.data.config.feature.shareWithEmergencyContacts then
      exit $ GoToShareRide state
    else
      continueWithCmd state [ pure ShareRide ]
...
eval (CancelRidePopUpAction (CancelRidePopUp.Button2 PrimaryButtonController.OnClick)) state = do
  let
    _ = unsafePerformEffect $ Events.addEventData ("External.Clicked.Search." <> state.props.searchId <> ".CancelRide") "true"
  _ <- pure $ performHapticFeedback unit
  if state.props.isOffline then do
    void $ pure $ toast (getString CHECK_YOUR_INTERNET_CONNECTION_AND_TRY_AGAIN)
    continue state
    ...
    case state.props.cancelRideActiveIndex of
      Just index ->
        if ((fromMaybe dummyCancelReason (state.props.cancellationReasons !! index)).reasonCode == "OTHER" || (fromMaybe dummyCancelReason (state.props.cancellationReasons !! index)).reasonCode == "TECHNICAL_GLITCH") then
          exit $ CancelRide state { props { cancelDescription = if (state.props.cancelDescription == "") then (fromMaybe dummyCancelReason (state.props.cancellationReasons !! index)).description else state.props.cancelDescription } } NORMAL_RIDE_CANCEL
        else
          exit $ CancelRide state { props { cancelDescription = (fromMaybe dummyCancelReason (state.props.cancellationReasons !! index)).description, cancelReasonCode = (fromMaybe dummyCancelReason (state.props.cancellationReasons !! index)).reasonCode } } NORMAL_RIDE_CANCEL
      Nothing -> continue state
...
eval (RideCompletedAC (RideCompletedCard.IssueReportPopUpAC (CancelRidePopUp.Button2 PrimaryButtonController.OnClick))) state = do
  let
    issue = (if state.props.nightSafetyFlow then safetyIssueOptions true else reportIssueOptions state) !! (fromMaybe 1 state.data.ratingViewState.issueReportActiveIndex)
...
eval (PredictionClickedAction (LocationListItemController.OnClick item)) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_prediction_list_item"
...
eval (SuggestedDestinationClicked item) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_sd_list_item"
  let
    _ = unsafePerformEffect $ Events.addEventData "External.Clicked.SuggestedDestination" "true"
...
eval (PredictionClickedAction (LocationListItemController.FavClick item)) state = do
  if (length state.data.savedLocations >= 20) then do
    ...
    exit $ CheckFavDistance state { data { saveFavouriteCard { address = item.description, selectedItem = item, tag = "", tagExists = false, isBtnActive = false }, selectedLocationListItem = Just item } }
...
eval (SaveFavouriteCardAction (SaveFavouriteCardController.PrimayEditTA (PrimaryEditTextController.TextChanged id val))) state = do
  let
    input = STR.trim val
  let
    updatedState = state { data { saveFavouriteCard { isBtnActive = ((STR.length input) >= 3), tagExists = not (validTag (getSavedTagsFromHome state.data.savedLocations) input ""), tag = input } } }
  continue updatedState
...
eval (FavouriteLocationModelAC (FavouriteLocationModelController.FavouriteLocationAC (SavedLocationCardController.CardClicked item))) state = do
  if state.props.isSource == Just true then do
    let
      newState = state { data { source = item.savedLocation, sourceAddress = item.fullAddress }, props { sourcePlaceId = item.placeId, sourceLat = fromMaybe 0.0 item.lat, sourceLong = fromMaybe 0.0 item.lon, rideSearchProps { sourceSelectType = ST.FAVOURITE } } }
    pure $ setText (getNewIDWithTag "SourceEditText") item.savedLocation
    exit $ LocationSelected item false newState
  else do
    let
      newState = state { data { destination = item.savedLocation, destinationAddress = item.fullAddress }, props { destinationPlaceId = item.placeId, destinationLat = fromMaybe 0.0 item.lat, destinationLong = fromMaybe 0.0 item.lon } }
    pure $ setText (getNewIDWithTag "DestinationEditText") item.savedLocation
    exit $ LocationSelected item false newState

eval (SavedAddressClicked (LocationTagBarController.TagClick savedAddressType arrItem)) state =
  if not state.props.isSrcServiceable then
    continue state
  else do
    _ <- pure $ firebaseLogEvent ("ny_user_savedLoc_" <> show savedAddressType)
    let
      _ = unsafePerformEffect $ Events.addEventData ("External.Clicked.SavedLocation." <> show savedAddressType) "true"
...
eval (SearchLocationModelActionController (SearchLocationModelController.LocationListItemActionController (LocationListItemController.OnClick item))) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_location_list_item"
  let
    condition = state.props.isSource == Just true && any (_ == item.locationItemType) [ Just RECENTS, Just SUGGESTED_DESTINATIONS ]
...
eval (SearchLocationModelActionController (SearchLocationModelController.SourceChanged input)) state = do
  let
    srcValue = if (state.data.source == "" || state.data.source == "Current Location") then true else false
  let
...
eval (SearchLocationModelActionController (SearchLocationModelController.SourceClear)) state = do
  ...
  if (state.props.isSearchLocation /= LocateOnMap) then do
    ...
    pure unit
  let
    predicArray = (updateLocListWithDistance state.data.recentSearchs.predictionArray state.props.sourceLat state.props.sourceLong true state.data.config.suggestedTripsAndLocationConfig.locationWithinXDist)
...
eval (SearchLocationModelActionController (SearchLocationModelController.SetCurrentLocation)) state = do
  ...
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_currentlocation_click"
...
eval (SearchLocationModelActionController (SearchLocationModelController.SetLocationOnMap)) state = do
  void $ pure $ performHapticFeedback unit
  let
    isSource = case state.props.isSource of
      Just true -> true
      _ -> false
...
eval (SearchLocationModelActionController (SearchLocationModelController.UpdateSource lat lng name)) state = do
  ...
  if state.props.isSource == Just true then do
    let
      newState = state { data { source = if state.data.source == "" then getString CURRENT_LOCATION else state.data.source, sourceAddress = encodeAddress name [] Nothing lat lng }, props { sourceLat = lat, sourceLong = lng, sourcePlaceId = Nothing, searchLocationModelProps { isAutoComplete = false } } }
    updateAndExit newState $ LocationSelected (fromMaybe dummyListItem newState.data.selectedLocationListItem) false newState
  else do
    let
      newState = state { data { destination = name, destinationAddress = encodeAddress name [] Nothing lat lng }, props { destinationLat = lat, destinationLong = lng, destinationPlaceId = Nothing } }
    updateAndExit newState $ LocationSelected (fromMaybe dummyListItem newState.data.selectedLocationListItem) false newState
...
eval (QuoteListModelActionController (QuoteListModelController.TipViewPrimaryButtonClick PrimaryButtonController.OnClick)) state = do
  let
    _ = unsafePerformEffect $ Events.addEventData ("External.Clicked.Search." <> state.props.searchId <> ".Tip") "true"
  let
  ...
  let
    tipViewData = state.props.tipViewProps { stage = TIP_ADDED_TO_SEARCH, onlyPrimaryText = true }
  let
...
eval (QuoteListModelActionController (QuoteListModelController.TipsViewActionController (TipsView.TipBtnClick index value))) state = do
  let
    check = index == state.props.tipViewProps.activeIndex
...
eval (QuoteListModelActionController (QuoteListModelController.QuoteListItemActionController QuoteListItemController.ConfirmRide)) state = do
  void $ pure $ performHapticFeedback unit
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_quote_confirm"
...
eval (QuoteListModelActionController (QuoteListModelController.QuoteListItemActionController (QuoteListItemController.CountDown seconds status id))) state = do
  if status == "EXPIRED" then do
    ...
    if (id == fromMaybe "" state.props.selectedQuote && autoSelecting && state.props.currentStage == QuoteList) then do
      let
        _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_auto_assign"
...
eval (PopUpModalAction (PopUpModal.OnButton1Click)) state = case state.props.isPopUp of
  TipsPopUp -> do
    void $ pure $ performHapticFeedback unit
    let
      _ = unsafePerformEffect $ logEvent state.data.logField if state.props.customerTip.isTipSelected then ("ny_added_tip_for_" <> (show state.props.currentStage)) else "ny_no_tip_added"
    _ <- pure $ clearTimerWithId state.props.timerId
    let
      tipViewData = state.props.tipViewProps { stage = RETRY_SEARCH_WITH_TIP, isVisible = not (state.props.customerTip.tipActiveIndex == 0), activeIndex = state.props.customerTip.tipActiveIndex, onlyPrimaryText = true }
    let
      newState = state { props { findingRidesAgain = true, searchExpire = (getSearchExpiryTime true), currentStage = RetryFindingQuote, isPopUp = NoPopUp, tipViewProps = tipViewData, rideSearchProps { sourceSelectType = ST.RETRY_SEARCH } } }
    _ <- pure $ setTipViewData (TipViewData { stage: tipViewData.stage, activeIndex: tipViewData.activeIndex, isVisible: tipViewData.isVisible })
  ...
  _ -> do
    ...
    if (isLocalStageOn FindingQuotes) then do
      ...
      _ <- pure $ clearTimerWithId state.props.timerId
      let
        newState = state { props { findingRidesAgain = true, searchExpire = (getSearchExpiryTime true), currentStage = RetryFindingQuote, isPopUp = NoPopUp, rideSearchProps { sourceSelectType = ST.RETRY_SEARCH } } }
...
eval (PopUpModalAction (PopUpModal.OnButton2Click)) state = case state.props.isPopUp of
  ...
  ConfirmBack -> do
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "ny_no_retry"
...
eval (PopUpModalAction (PopUpModal.DismissPopup)) state = do
  let
    newState = if (isLocalStageOn QuoteList) then state else state { props { isPopUp = NoPopUp, customerTip { tipActiveIndex = 1, tipForDriver = 10, isTipSelected = false } } }
  continue newState
...
eval (PickUpFarFromCurrentLocAC (PopUpModal.OnButton2Click)) state = do
  if (state.props.isShorterTrip) then do
    ...
    continueWithCmd state [ do pure $ ShortDistanceActionController (PopUpModal.OnButton2Click) ]
...
eval (GetEstimates (GetQuotesRes quotesRes) count) state = do
  ...
  let
    quotes = filterSpecialZoneAndInterCityQuotes quotesRes.quotes

    fareProductType = getFareProductType $ maybe "" extractFareProductType (head quotes)
  case null quotes of
    false -> do
      let
        _ = unsafePerformEffect $ Events.addEventData ("External.Search." <> state.props.searchId <> ".Type") "Quotes"
    ...
    true -> do
      let
        _ = unsafePerformEffect $ Events.addEventData ("External.Search." <> state.props.searchId <> ".Type") "Estimates"
...
eval (EstimatesTryAgain (GetQuotesRes quotesRes) count) state = do
  case (getMerchant FunctionCall) of
    ...
    _ -> do
      let
        _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_estimate_try_again"
      let
...
eval (GetQuotesList (SelectListRes resp)) state = do
  case spy "flowWithoutOffers" $ flowWithoutOffers WithoutOffers of
    ...
    false -> do
      let
        selectedQuotes = getQuoteList ((fromMaybe dummySelectedQuotes resp.selectedQuotes) ^. _selectedQuotes) state.props.city
      _ <- pure $ printLog "vehicle Varient " selectedQuotes
      let
        filteredQuoteList = filter (\a -> length (filter (\b -> a.id == b.id) state.data.quoteListModelState) == 0) selectedQuotes
      let
        removeExpired = filter (\a -> a.seconds > 0) filteredQuoteList
      _ <- pure $ spy "quotes" filteredQuoteList
      let
        quoteListModelState = state.data.quoteListModelState <> removeExpired
      if (getValueToLocalStore GOT_ONE_QUOTE == "FALSE") && (length quoteListModelState > 0) then do
        ...
        pure unit
      let
        newState = state { data { quoteListModelState = quoteListModelState }, props { isSearchLocation = NoView, isSource = Nothing, currentStage = QuoteList } }
      if isLocalStageOn QuoteList then do
        let
          updatedState = if isTipEnabled state then tipEnabledState newState { props { isPopUp = TipsPopUp, findingQuotesProgress = 0.0 } } else newState { props { isPopUp = ConfirmBack, findingQuotesProgress = 0.0 } }
        ...
        let
          id = (fromMaybe dummyQuoteList (newState.data.quoteListModelState !! 0)).id
        ...
        let
          quoteListEmpty = null newState.data.quoteListModelState
...
eval (ContinueWithoutOffers (SelectListRes resp)) state = do
  case resp.bookingId of
    ...
    Nothing -> do
      if isLocalStageOn QuoteList then do
        let
          onUs = state.data.selectedEstimatesObject.providerType == CTP.ONUS
...
eval (CurrentLocation lat lng) state = do
  ...
  if isLocalStageOn FindingEstimate then
    continue state
  else
    exit $ UpdatedState state { props { currentLocation { lat = fromMaybe 0.0 (NUM.fromString lat), lng = fromMaybe 0.0 (NUM.fromString lng) }, sourceLat = fromMaybe 0.0 (NUM.fromString lat), sourceLong = fromMaybe 0.0 (NUM.fromString lng) } } false
...
eval (RequestInfoCardAction RequestInfoCard.Close) state =
  continueWithCmd state { props { showMultipleRideInfo = false }, data { waitTimeInfo = false, iopState { providerPrefInfo = false } } }
    [ do
...
eval SpecialZoneInfoTag state = continue state { props { showSpecialZoneInfoPopup = true } }
...
eval (UpdateProfileButtonAC PrimaryButtonController.OnClick) state = do
  _ <- pure $ pauseYoutubeVideo unit
  let
    newState = state { props { showEducationalCarousel = false } }
...
eval (SkipAccessibilityUpdateAC PrimaryButtonController.OnClick) state = do
  _ <- pure $ pauseYoutubeVideo unit
  let
    _ = runFn2 updatePushInIdMap "bannerCarousel" true
  continue state { props { showEducationalCarousel = false } }
...
eval (CallSupportAction PopUpModal.OnButton2Click) state = do
  ...
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_ride_support_click"
  continue state { props { callSupportPopUp = false } }

eval (UpdateETA currentETA currentDistance) state = do
  let
    initDistance = state.data.driverInfoCardState.initDistance
  distance <- case initDistance of
    Just initDistance -> pure initDistance
    Nothing -> do
      let
        storedDistance = getValueToLocalStore PICKUP_DISTANCE
      if storedDistance == "0" || storedDistance == "__failed" || storedDistance == "(null)" then do
        ...
        pure $ fromMaybe 0 (fromString storedDistance)
  let
...
eval (RepeatRide index item) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_repeat_trip"
  let
    _ = unsafePerformEffect $ Events.addEventData "External.Clicked.RepeatRide" "true"
...
eval (ReferralFlowAction) state = continue state { props { referral { showAddReferralPopup = true }, referralComponentProps = HomeScreenData.initData.props.referralComponentProps } }
...
eval UpdateSourceFromPastLocations state = do
  let
    currLocationArr = (getNearestCurrentLocation state.props.sourceLat state.props.sourceLong state.data.previousCurrentLocations.pastCurrentLocations)
...
eval (UpdateLocAndLatLong lat lng) state = do
  let
    slat = fromMaybe 0.0 (NUM.fromString lat)

    slng = fromMaybe 0.0 (NUM.fromString lng)
  continueWithCmd state { props { currentLocation { lat = slat, lng = slng }, sourceLat = slat, sourceLong = slng, locateOnMapLocation { sourceLat = slat, sourceLng = slng, source = state.data.source, sourceAddress = state.data.sourceAddress } } }
    [ do
...
eval (MenuButtonActionController (MenuButtonController.OnClick config)) state = do
  continueWithCmd state { props { defaultPickUpPoint = config.id } }
    [ do
        let
          focusedIndex = findIndex (\item -> item.place == config.id) state.data.nearByPickUpPoints
...
eval (ChooseYourRideAction (ChooseYourRideController.ChooseVehicleAC (ChooseVehicleController.NoAction config))) state = do
  let
    height = (runFn1 getLayoutBounds $ getNewIDWithTag config.id).height
...
eval (ChooseYourRideAction (ChooseYourRideController.ChooseVehicleAC (ChooseVehicleController.ServicesOnClick config item))) state = do
  let
    updatedServices = if elem item config.selectedServices then delete item config.selectedServices else insert item config.selectedServices
      ...
      map
        ( \item ->
            if item.vehicleVariant == "BOOK_ANY" then
              item { selectedServices = updatedServices }
            else
              item
        )
...
eval (ChooseYourRideAction (ChooseYourRideController.ChooseVehicleAC (ChooseVehicleController.OnSelect config))) state = do
  let
    _ = unsafePerformEffect $ Events.addEventData ("External.Clicked.Search." <> state.props.searchId <> ".ChooseVehicle") "true"
  let
    updatedQuotes = map (\item -> item { activeIndex = config.index }) state.data.specialZoneQuoteList
  ...
  let
    updatedSpecialZOneQuotes = map (\item -> item { activeIndex = config.index }) state.data.specialZoneQuoteList
...
eval (ChooseYourRideAction (ChooseYourRideController.ChooseVehicleAC (ChooseVehicleController.ShowRateCard config))) state = do
  let
    _ = unsafePerformEffect $ Events.addEventData ("External.Clicked.Search." <> state.props.searchId <> ".RateCard") "true"
  continue
    state
...
eval (ChooseYourRideAction (ChooseYourRideController.PrimaryButtonActionController (PrimaryButtonController.OnClick))) state = do
  let
    _ = unsafePerformEffect $ Events.addEventData ("External.Clicked.Search." <> state.props.searchId <> ".BookNow") "true"
  ...
  if any (_ == state.data.fareProductType) [ FPT.ONE_WAY_SPECIAL_ZONE, FPT.INTER_CITY ] then do
    ...
    let
      customerTip = if state.props.tipViewProps.activeIndex == -1 then HomeScreenData.initData.props.customerTip else state.props.customerTip

      tipViewProps =
        if state.props.tipViewProps.activeIndex == -1 then
          HomeScreenData.initData.props.tipViewProps
        else if state.props.tipViewProps.stage == TIP_AMOUNT_SELECTED then
          state.props.tipViewProps { stage = TIP_ADDED_TO_SEARCH }
        else
          state.props.tipViewProps
...
eval (QuoteListModelActionController (QuoteListModelController.ProviderModelAC (PM.ButtonClick (PrimaryButtonController.OnClick)))) state = do
  ...
  let
    updatedState = state { props { searchExpire = (getSearchExpiryTime true) }, data { iopState { providerSelectionStage = false } } }
...
eval (QuoteListModelActionController (QuoteListModelController.ProviderModelAC (PM.FavClick item))) state = do
  let
    selectedItem = find (\quote -> quote.id == item.id) state.data.specialZoneQuoteList
...
eval (ProviderAutoSelected seconds status timerID) state = do
  if status == "EXPIRED" then do
    ...
    continue state { data { iopState { timerVal = show seconds, timerId = timerID } } } -- update timer in ui
...
eval MapReadyAction state = do
  continueWithCmd state
    [ do
        ...
        when (state.props.currentStage == HomeScreen)
          $ do
        ...
        let
          action =
            if (not internetCondition) then
              TriggerPermissionFlow INTERNET_ACTION
            else if (not (permissionConditionA && permissionConditionB)) then
              TriggerPermissionFlow LOCATION_DISABLED
            else
              CheckAndAskNotificationPermission
        pure action
    ]
...
eval (ChooseYourRideAction (ChooseYourRideController.OnIconClick autoAssign)) state = continue state { props { showMultipleRideInfo = not autoAssign } }
...
eval (DateTimePickerAction dateResp year month day timeResp hour minute) state = do
  if any (_ /= "SELECTED") [ dateResp, timeResp ] then
    continue state
  else
    let
    ...
    in
      if validDate && isAfterThirtyMinutes then do
        let
          maybeInvalidBookingDetails = invalidBookingTime selectedUTC Nothing
        if (isJust maybeInvalidBookingDetails) then do
          continue state { data { invalidBookingId = maybe Nothing (\invalidBookingTime -> Just invalidBookingTime.bookingId) maybeInvalidBookingDetails }, props { showScheduledRideExistsPopUp = true } }
        else
          continue newState
      else if isAfterThirtyMinutes then do
...
eval (BottomNavBarAction id) state = do
  let
    newState = state { props { focussedBottomIcon = id } }
  case id of
...
eval (ShareRideAction (PopupWithCheckboxController.ToggleSelect index)) state = do
  let
    contacts = fromMaybe [] state.data.contactList
  case contacts !! index of
    Just contactToUpdate -> do
      let
        updatedContactList = updateAt index contactToUpdate { isSelected = not contactToUpdate.isSelected } contacts
      continue
        state
          { data
...
eval (ReferralComponentAction componentAction) state = case componentAction of
  ...
  ReferralComponent.PopUpModalAction popUpAction -> case popUpAction of
    PopUpModal.OnButton1Click -> do
      case state.props.referral.referralStatus of
        ...
        _ -> continue state
    PopUpModal.OnButton2Click -> continue state { props { referral { referralStatus = NO_REFERRAL, showAddReferralPopup = false } } }
    _ -> continue state
  ReferralComponent.ApplyAction buttonAction -> case buttonAction of
    PrimaryButtonController.OnClick -> case state.props.referralComponentProps.referralCode of
      Just code -> exit $ UpdateReferralCode state { props { referralComponentProps { applyButtonActive = false } } } code
      Nothing -> continue state
    _ -> continue state
  ReferralComponent.SkipAction buttonAction -> case buttonAction of
    ...
    _ -> continue state
  ReferralComponent.OpenReferralProgramInfo -> continue state { props { referralComponentProps { showReferralProgramInfoPopup = true } } }
  ReferralComponent.ReferredUserInfo PopUpModal.OnButton2Click -> continue state { props { referralComponentProps { showReferredUserInfoPopup = false } } }
  ReferralComponent.ReferralProgramInfo PopUpModal.OnButton2Click -> continue state { props { referralComponentProps { showReferralProgramInfoPopup = false } } }
  _ -> continue state
...
eval UpdateRateCardCache state = do
  let
    (rateCard :: Maybe ST.RateCard) = handleRateCard $ runFn3 getFromCache (show RATE_CARD_INFO) Nothing Just
  ...
  handleRateCard rateCard = do
    case rateCard of
      Nothing -> do
        let
          stringifiedValue = getKeyInSharedPrefKeys (show RATE_CARD_INFO)
        if (any (_ == stringifiedValue) [ "__failed", "", "(null)" ]) then
          Nothing
...
updateFeedback feedbackId feedbackItem feedbackList =
  if hasFeedbackId feedbackId feedbackList then
    updateFeedbackAnswer feedbackId feedbackItem feedbackList
  else
    addFeedbackAnswer feedbackId feedbackItem feedbackList
  where
  ...
  updateFeedbackAnswer fid newItem list =
    map
      ( \feedback ->
          if feedback.questionId == fid then
            feedback { answer = if newItem `elem` (feedback.answer) then filter (\x -> x /= newItem) feedback.answer else feedback.answer <> [ newItem ] }
          else
            feedback
      )
      list
  ...
  addFeedbackAnswer fid newItem list = do
    let
      config = { questionId: fid, answer: [ newItem ] }
    list <> [ config ]
...
getEstimateId esimates config =
  let
    selectedEstimates = foldl (\acc item -> if elem (fromMaybe "" item.serviceTierName) config.selectedServices then acc <> [ item.id ] else acc) [] esimates

    estimateId = if config.vehicleVariant == "BOOK_ANY" then fromMaybe "" (head selectedEstimates) else config.id

    otherSelectedEstimates = fromMaybe [] $ tail $ selectedEstimates
  in
    (Tuple estimateId otherSelectedEstimates)
...
tagClickEvent savedAddressType arrItem state = do
  let
    stage' = if os == "IOS" && state.props.currentStage == HomeScreen then ConfirmingLocation else LoadMap
  case savedAddressType, arrItem of
    ...
    _, Nothing -> do
      if (length state.data.savedLocations >= 20) then do
        ...
        updateAndExit state { props { tagType = Just savedAddressType } } $ CheckFavDistance state { props { tagType = Just savedAddressType } }
    _, Just item -> do
      if state.props.isSource == Just true then do
        let
          newState = state { data { source = item.description, sourceAddress = item.fullAddress }, props { sourcePlaceId = item.placeId, sourceLat = fromMaybe 0.0 item.lat, sourceLong = fromMaybe 0.0 item.lon, rideSearchProps { sourceSelectType = ST.FAVOURITE } } }
        ...
        let
          newState = state { data { destination = item.description, destinationAddress = item.fullAddress }, props { destinationPlaceId = item.placeId, destinationLat = fromMaybe 0.0 item.lat, destinationLong = fromMaybe 0.0 item.lon } }
...
locationSelected item addToRecents state = do
  ...
  _ <- pure $ hideKeyboardOnNavigation true
  let
    favClick = if item.postfixImageUrl == "ny_ic_fav_red,https://assets.moving.tech/beckn/nammayatri/user/images/ny_ic_fav_red.png" then "true" else "false"
  if state.props.isSource == Just true then do
    let
      _ =
        unsafePerformEffect $ logEventWithMultipleParams state.data.logField "ny_user_pickup_select"
          $ [ { key: "Source", value: unsafeToForeign item.title }
    ...
    let
      _ =
        unsafePerformEffect $ logEventWithMultipleParams state.data.logField "ny_user_destination_select"
          $ [ { key: "Destination", value: unsafeToForeign item.title }
            , { key: "Favourite", value: unsafeToForeign favClick }
            ]
    let
      newState = state { data { destination = item.title, destinationAddress = encodeAddress (item.title <> ", " <> item.subTitle) [] item.placeId (fromMaybe 0.0 item.lat) (fromMaybe 0.0 item.lon) }, props { destinationPlaceId = item.placeId, destinationLat = fromMaybe 0.0 item.lat, destinationLong = fromMaybe 0.0 item.lon } }
...
getNearestCurrentLocation lat lon previousCurrentLocations =
  ( sortBy compareByDistance
      ( map
          ( \(item) ->
...
getNearestSavedLocation lat lon savedLocations =
  ( sortBy compareByDistance
      ( map
          ( \item ->
...
getSearchExpiryTime isNormalRide =
  let
    count = fromMaybe 0 (fromString (getValueToLocalStore $ if isNormalRide then TEST_POLLING_COUNT else CONFIRM_QUOTES_POLLING_COUNT))
...
isTipEnabled state =
  let
    ...
    customerTipArrayWithValues = tipConfig.customerTipArrayWithValues
  in
    not $ null customerTipArrayWithValues
...
quoteListFlow flowType estimatedQuotes state = do
  let
    quoteList = getQuotesTransformer estimatedQuotes state.data.config.estimateAndQuoteConfig
  ...
  updateStateWithQuotes flowType state quoteList defaultQuote =
    state
      { data
        { quoteList = quoteList
...
estimatesListFlow estimates state count = do
  let
    ...
    defaultQuote =
      fromMaybe ChooseVehicleController.config
        $ if newState.props.isRepeatRide then do
          ...
          else if showMultiProvider' then
            quoteList !! newState.data.selectedEstimatesObject.activeIndex
          else
            let
              nYQuotes = filter (\item -> item.providerType == ONUS) quoteList -- NY Quotes
            in
              nYQuotes !! newState.data.selectedEstimatesObject.activeIndex

    zoneType =
      getSpecialTag
        $ case head estimates of
  ...
  getSelectedEstimates quote quotes =
    let
      filteredEstimates = foldl (\acc item -> if elem (fromMaybe "" item.serviceTierName) quote.selectedServices then acc <> [ item.id ] else acc) [] quotes
    in
      (Tuple (fromMaybe "" $ head filteredEstimates) (fromMaybe [] $ tail filteredEstimates))
  ...
  case (null estimatedVarient) of
    ...
    false -> do
      if (estimatedPrice > state.data.selectedEstimatesObject.basePrice) then
        ...
        let
          updatedState = state { data { suggestedAmount = estimatedPrice }, props { estimateId = defaultQuote.id, searchExpire = (getSearchExpiryTime true) } }
        updateAndExit updatedState $ GetQuotes updatedState
...
getRateCardArray nightCharges lang baseFare extraFare additionalFare =
  ( [ { title: (getString $ MIN_FARE_UPTO "2 km") <> if nightCharges then " 🌙" else "", description: "₹" <> toStringJSON (baseFare) }
...
findingQuotesSearchExpired gotQuotes isNormalRide =
  let
    secondsPassed = getExpiryTime (getValueToLocalStore $ if isNormalRide then FINDING_QUOTES_START_TIME else CONFIRM_QUOTES_START_TIME) isNormalRide
...
callDriver state callType = do
  let
    newState = state { props { showCallPopUp = false } }
  ...
  updateWithCmdAndExit newState
    [ do
        ...
        let
          _ = unsafePerformEffect $ logEventWithTwoParams state.data.logField ("ny_user_" <> callType <> "_call_click") "trip_id" (state.props.bookingId) "user_id" (getValueToLocalStore CUSTOMER_ID)
        pure NoAction
    ]
...
getInfoCardPeekHeight state =
  let
    bottomSheetLayout = (runFn1 getLayoutBounds $ getNewIDWithTag (if state.data.fareProductType == FPT.ONE_WAY_SPECIAL_ZONE then "driverInfoViewSpecialZone" else "driverInfoView"))
    ...
    currentPeekHeight =
      if bottomSheetLayout.height == 0 || actionsView.height == 0 then
        0
      else
        bottomSheetLayout.height + if state.data.config.driverInfoConfig.footerVisibility then brandingBanner.height else 0 - (if state.data.fareProductType == FPT.RENTAL then (fareEstimate.height + fareEstimateViewPadding + driverDetailsView.height + driverDetailsViewPadding) else 0) + actionsView.height
...
getPeekHeight state =
  ...
  in
    if homescreenHeader == 0 then 500 else requiredPeekHeight
  where
  ...
  getPeekHeightForIos homescreenHeader scrHeight =
    ...
    in
      ceil ((((toNumber scrHeight) / displayZoomFactor) / iosScale) - (toNumber homescreenHeader))

  getPeekHeightForAndroid :: Int -> Int
  getPeekHeightForAndroid homescreenHeader =
    let
      androidPixels = runFn1 getPixels FunctionCall

      androidDensity = (runFn1 getDeviceDefaultDensity FunctionCall) / defaultDensity
    in
      (screenHeight unit) - (ceil (((toNumber homescreenHeader) / androidPixels) * androidDensity))
...
openLiveDashboard state = do
  ...
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_live_stats_dashboard"
...
cacheRateCard state = do
  ...
  if state.data.selectedEstimatesObject.vehicleVariant == "BOOK_ANY" then do
    let
      _ = JB.removeKeysInSharedPrefs $ show RATE_CARD_INFO
    void $ pure $ removeValueFromCache (show RATE_CARD_INFO)
  else
    void $ pure $ setValueToCache (show RATE_CARD_INFO) rateCard (\a -> stringifyJSON $ encode a)
