...
eval (NotifyDriverStatusCountDown seconds id status timerID) state = do 
  if status == "EXPIRED" then do
    ...
    if isLocalStageOn RideAccepted && (secondsToHms (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).eta) /= "" then 
      continue state{data{lastMessage = state.data.lastMessage{message = state.data.config.notifyRideConfirmationConfig.autoGeneratedText <> (secondsToHms (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).eta), sentBy = "Driver"}},props{unReadMessages = true, showChatNotification = true}}
      else continue state
    else continue state
...
eval (UpdateCurrentStage resp) state = do
  -- let newState =  case state.data.driverInfoCardState of
  --                   Nothing -> state{data {driverInfoCardState = getDriverInfo state.data.specialZoneSelectedVariant (RideBookingRes respBooking) (state.data.currentSearchResultType == QUOTES)}}
  --                   Just _ -> state
                ...
                , data
                  { rideRatingState =
                    dummyRideRatingState
                      { driverName = (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).driverName
                      , rideId = (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).rideId
                      }
                  }
                }
...
eval (DriverArrivedAction driverArrivalTime) state = do
  ...
  let newState = case state.data.driverInfoCardState of
        Nothing -> state
        Just driverInfo -> state { data { driverInfoCardState = Just $ driverInfo{driverArrived = true, driverArrivalTime = getExpiryTime driverArrivalTime true } } }
...
eval (WaitingTimeAction timerID timeInMinutes seconds) state = do
  ...
  let newState = case state.data.driverInfoCardState of
        Nothing -> state {props { waitingTimeTimerIds = union state.props.waitingTimeTimerIds [timerID] } }
        Just driverInfo -> state { data { driverInfoCardState = Just driverInfo { waitingTime = timeInMinutes} }, props { waitingTimeTimerIds = union state.props.waitingTimeTimerIds [timerID] } }
  continue newState
...
eval (DriverInfoCardActionController (DriverInfoCardController.OnNavigate)) state = do
  void $ pure $ openNavigation 0.0 0.0 (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).destinationLat (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).destinationLng "DRIVE"
  continue state

eval (DriverInfoCardActionController (DriverInfoCardController.OnNavigateToZone)) state = do
  void $ pure $ openNavigation 0.0 0.0 (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).sourceLat (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).sourceLng "WALK"
  continue state
...
eval (DriverInfoCardActionController (DriverInfoCardController.ShareRide)) state = do
  continueWithCmd state
        [ do
            _ <- pure $ shareTextMessage (getValueToLocalStore USER_NAME <> "is on a Namma Yatri Ride") $ "👋 Hey,\n\nI am riding with Namma Driver " <> ((fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).driverName) <> "! Track this ride on: " <> ("https://nammayatri.in/track/?id="<>(fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).rideId) <> "\n\nVehicle number: " <> ((fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).registrationNumber)
            pure NoAction
         ]
...
eval ( RideCompletedAC (RideCompletedCard.IssueReportPopUpAC (CancelRidePopUp.Button2 PrimaryButtonController.OnClick))) state = do
  ...
  exit $ ReportIssue state { data {
    ...
    rideRatingState {rideId = (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).rideId, feedback = ""}
    }}
...
eval (UpdateETA currentETA currentDistance) state = do
  let initDistance = (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).initDistance
  ...
  let
    newState = case state.data.driverInfoCardState of
      Nothing -> state
      Just driverInfo -> state { data { driverInfoCardState = Just $ driverInfo{ eta = currentETA, distance = currentDistance, initDistance = Just distance } } }
  continue newState
  ...
  continueWithCmd state{props{ showCallPopUp = false }}
    [ do
        let driverNumber = case callType of
                            "DIRECT" ->(fromMaybe (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).merchantExoPhone (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).driverNumber)
                            _ -> if (STR.take 1 (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).merchantExoPhone) == "0" then (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).merchantExoPhone else "0" <> (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).merchantExoPhone
...
getCustomerSuggestions state suggestions = case (length suggestions == 0) of
                                  true -> if (metersToKm (fromMaybe HomeScreenData.dummyDriverInfo state.data.driverInfoCardState).distance state) == (getString AT_PICKUP) then getSuggestionsfromKey "customerDefaultAP" else getSuggestionsfromKey "customerDefaultBP"
                                  false -> suggestions
