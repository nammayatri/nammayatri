...
eval (RepeatRideCountDown seconds id status timerID) state = do
  if status == "EXPIRED" then do
    ...
    let
      updatedState = state { data { rideHistoryTrip = Nothing }, props { repeatRideTimerId = "", currentStage = FindingQuotes, searchExpire = (getSearchExpiryTime "LazyCheck") } }
    updateAndExit (updatedState) (GetQuotes updatedState)
  else
    continue state { props { repeatRideTimer = (show seconds), repeatRideTimerId = timerID } }
...
eval OnResumeCallback state =
  if (state.props.currentStage == RideAccepted || state.props.currentStage == RideStarted) && state.props.emergencyHelpModelState.waitingDialerCallback then
    ...
    "FindingQuotes" -> do
      let
        secondsLeft = findingQuotesSearchExpired false
    ...
    "QuoteList" -> do
      let
        findingQuotesProgress = 1.0 - 30.0 / (toNumber (getSearchExpiryTime "LazyCheck"))
      void $ pure $ startLottieProcess lottieAnimationConfig { rawJson = "progress_loader_line", lottieId = (getNewIDWithTag "lottieLoaderAnimProgress"), minProgress = findingQuotesProgress, scaleType = "CENTER_CROP" }
      continue state
    "RideAccepted"
      | state.data.currentSearchResultType == QUOTES -> exit $ Retry state
    _ -> continue state
  ...
  case (last allMessages) of
    Just value ->
      if value.message == "" then
        ...
        updateMessagesWithCmd state { data { messages = allMessages, suggestionsList = [] }, props { canSendSuggestion = true, isChatNotificationDismissed = false } }
      else do
        let
...
eval (ChatViewActionController (ChatView.SendSuggestion chatSuggestion)) state = do
  if state.props.canSendSuggestion then do
    let
      message = getMessageFromKey chatSuggestion "EN_US"
...
eval BackPressed state = do
  ...
  case state.props.currentStage of
    ...
    SettingPrice -> do
      ...
      void $ pure $ clearCountDownTimer state.props.repeatRideTimerId
      let
        updatedState = state { props { repeatRideTimer = "", repeatRideTimerId = "" } }
    ...
    _ -> do
      ...
      else if state.props.emergencyHelpModelState.showContactSupportPopUp then
        continue state { props { emergencyHelpModelState { showContactSupportPopUp = false } } }
      else if state.props.emergencyHelpModelState.showCallPolicePopUp then
        continue state { props { emergencyHelpModelState { showCallPolicePopUp = false } } }
      else if state.props.emergencyHelpModelState.showCallSuccessfulPopUp then
        continue state { props { emergencyHelpModelState { showCallSuccessfulPopUp = false } } }
      ...
      else if state.props.emergencyHelpModal then
        continue state { props { emergencyHelpModal = false } }
...
eval (UpdateLocation key lat lon) state = case key of
  ...
  _ ->
    if length (filter (\(item) -> (item.place == key)) state.data.nearByPickUpPoints) > 0 then do
...
eval (UpdatePickupLocation key lat lon) state = case key of
  "LatLon" -> do
    exit $ UpdatePickupName state { props { defaultPickUpPoint = "" } } (fromMaybe 0.0 (NUM.fromString lat)) (fromMaybe 0.0 (NUM.fromString lon))
  _ -> do
    let
      focusedIndex = findIndex (\item -> item.place == key) state.data.nearByPickUpPoints
...
eval (SettingSideBarActionController (SettingSideBarController.GoToEmergencyContacts)) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_emergency_contacts"
...
eval (SettingSideBarActionController (SettingSideBarController.ShareAppLink)) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_share_app_menu"
...
eval (PrimaryButtonActionController (PrimaryButtonController.OnClick)) state = do
  ...
  case state.props.currentStage of
    ...
    ConfirmingLocation -> do
      ...
      let
        updatedState = state { props { currentStage = FindingEstimate, locateOnMap = false } }
      updateAndExit updatedState $ (UpdatedSource updatedState)
    SettingPrice -> do
      _ <- pure $ performHapticFeedback unit
      _ <- pure $ updateLocalStage FindingQuotes
      let
        updatedState = state { data { rideHistoryTrip = Nothing }, props { currentStage = FindingQuotes, searchExpire = (getSearchExpiryTime "LazyCheck") } }
      updateAndExit (updatedState) (GetQuotes updatedState)
    _ -> continue state
...
eval (RideCompletedAC (RideCompletedCard.SkipButtonActionController (PrimaryButtonController.OnClick))) state = case state.data.ratingViewState.issueFacedView of
  true -> do
    _ <- pure $ setValueToLocalStore REFERRAL_STATUS "HAS_TAKEN_RIDE"
    continue
      state
  ...
  _ ->
    if state.data.ratingViewState.selectedRating > 0 then
...
eval (SearchExpireCountDown seconds id status timerID) state = do
  if status == "EXPIRED" then do
    ...
    if any (_ == state.props.currentStage) [ FindingQuotes, QuoteList ] then
      continue state { props { searchExpire = seconds, timerId = timerID, tipViewProps { isVisible = enableTips && (seconds <= (getSearchExpiryTime "LazyCheck") - 30 || state.props.tipViewProps.isVisible) && (state.props.customerTip.tipActiveIndex > 0) }, customerTip { enableTips = enableTips } } }
    else do
      _ <- pure $ clearTimer timerID
      let
        tipViewData = HomeScreenData.initData.props.tipViewProps
...
eval (EmergencyHelpModalAC (EmergencyHelpController.CallEmergencyContact PopUpModal.OnButton2Click)) state = do
  ...
  let
    newState = state { props { emergencyHelpModelState { showCallContactPopUp = false, waitingDialerCallback = true } } }
...
eval (EmergencyHelpModalAC (EmergencyHelpController.CallSuccessful PopUpModal.OnButton1Click)) state = do
  let
    newState = state { props { emergencyHelpModelState { waitingDialerCallback = false, showCallSuccessfulPopUp = false, sosStatus = "NotResolved" } } }
...
eval (EmergencyHelpModalAC (EmergencyHelpController.CallSuccessful PopUpModal.OnButton2Click)) state = do
  let
    newState = state { props { emergencyHelpModelState { waitingDialerCallback = false, showCallSuccessfulPopUp = false, sosStatus = "Resolved" } } }
  updateAndExit newState $ UpdateSosStatus newState
...
eval (EmergencyHelpModalAC (EmergencyHelpController.CallPolice PopUpModal.OnButton2Click)) state = do
  ...
  let
    newState = state { props { emergencyHelpModelState { showCallPolicePopUp = false, waitingDialerCallback = true } } }
...
eval (EmergencyHelpModalAC (EmergencyHelpController.ContactSupport PopUpModal.OnButton2Click)) state = do
  ...
  let
    newState = state { props { emergencyHelpModelState { showContactSupportPopUp = false, waitingDialerCallback = true } } }
...
eval (CancelRidePopUpAction (CancelRidePopUp.Button2 PrimaryButtonController.OnClick)) state = do
  _ <- pure $ performHapticFeedback unit
  let
    newState = state { props { isCancelRide = false, currentStage = HomeScreen, rideRequestFlow = false, isSearchLocation = NoView } }
  case state.props.cancelRideActiveIndex of
    Just index ->
      if ((fromMaybe dummyCancelReason (state.props.cancellationReasons !! index)).reasonCode == "OTHER" || (fromMaybe dummyCancelReason (state.props.cancellationReasons !! index)).reasonCode == "TECHNICAL_GLITCH") then
        exit $ CancelRide newState { props { cancelDescription = if (newState.props.cancelDescription == "") then (fromMaybe dummyCancelReason (state.props.cancellationReasons !! index)).description else newState.props.cancelDescription } }
      else
        exit $ CancelRide newState { props { cancelDescription = (fromMaybe dummyCancelReason (state.props.cancellationReasons !! index)).description, cancelReasonCode = (fromMaybe dummyCancelReason (state.props.cancellationReasons !! index)).reasonCode } }
    Nothing -> continue state
...
eval (RideCompletedAC (RideCompletedCard.IssueReportPopUpAC (CancelRidePopUp.Button2 PrimaryButtonController.OnClick))) state = do
  let
    issue = (reportIssueOptions state) !! (fromMaybe 1 state.data.ratingViewState.issueReportActiveIndex)
  let
    reason = (fromMaybe dummyCancelReason issue).description
  exit
    $ ReportIssue
        state
          { data
...
eval (FavouriteLocationModelAC (FavouriteLocationModelController.FavouriteLocationAC (SavedLocationCardController.CardClicked item))) state = do
  if state.props.isSource == Just true then do
    let
      newState = state { data { source = item.savedLocation, sourceAddress = item.fullAddress }, props { sourcePlaceId = item.placeId, sourceLat = fromMaybe 0.0 item.lat, sourceLong = fromMaybe 0.0 item.lon, sourceSelectedOnMap = true } }
...
eval (SearchLocationModelActionController (SearchLocationModelController.SourceClear)) state = do
  ...
  if (state.props.isSearchLocation /= LocateOnMap) then do
    ...
    pure unit
  let
    predicArray = (recentDistance state.data.recentSearchs.predictionArray state.props.sourceLat state.props.sourceLong)
...
eval (SearchLocationModelActionController (SearchLocationModelController.UpdateSource lat lng name)) state = do
  ...
  if state.props.isSource == Just true then do
    let
      newState = state { data { source = (getString CURRENT_LOCATION), sourceAddress = encodeAddress name [] Nothing }, props { sourceLat = lat, sourceLong = lng, sourcePlaceId = Nothing, searchLocationModelProps { isAutoComplete = false } } }
    updateAndExit newState $ LocationSelected (fromMaybe dummyListItem newState.data.selectedLocationListItem) false newState
  else do
    let
      newState = state { data { destination = name, destinationAddress = encodeAddress name [] Nothing }, props { destinationLat = lat, destinationLong = lng, destinationPlaceId = Nothing } }
    updateAndExit newState $ LocationSelected (fromMaybe dummyListItem newState.data.selectedLocationListItem) false newState
...
eval (QuoteListModelActionController (QuoteListModelController.TipViewPrimaryButtonClick PrimaryButtonController.OnClick)) state = do
  ...
  let
    tipViewData = state.props.tipViewProps { stage = TIP_ADDED_TO_SEARCH }
  let
    newState = state { props { findingRidesAgain = true, searchExpire = (getSearchExpiryTime "LazyCheck"), currentStage = TryAgain, sourceSelectedOnMap = true, isPopUp = NoPopUp, tipViewProps = tipViewData, customerTip { tipForDriver = (fromMaybe 10 (state.props.tipViewProps.customerTipArrayWithValues !! state.props.tipViewProps.activeIndex)), tipActiveIndex = state.props.tipViewProps.activeIndex + 1, isTipSelected = true } }, data { nearByDrivers = Nothing } }
  _ <- pure $ setTipViewData (TipViewData { stage: tipViewData.stage, activeIndex: tipViewData.activeIndex, isVisible: tipViewData.isVisible })
...
eval (PopUpModalAction (PopUpModal.OnButton1Click)) state = case state.props.isPopUp of
  TipsPopUp -> do
    ...
    _ <- pure $ clearTimer state.props.timerId
    let
      tipViewData = state.props.tipViewProps { stage = RETRY_SEARCH_WITH_TIP, isVisible = not (state.props.customerTip.tipActiveIndex == 0), activeIndex = state.props.customerTip.tipActiveIndex - 1 }
    let
      newState = state { props { findingRidesAgain = true, searchExpire = (getSearchExpiryTime "LazyCheck"), currentStage = RetryFindingQuote, sourceSelectedOnMap = true, isPopUp = NoPopUp, tipViewProps = tipViewData } }
    _ <- pure $ setTipViewData (TipViewData { stage: tipViewData.stage, activeIndex: tipViewData.activeIndex, isVisible: tipViewData.isVisible })
  ...
  _ -> do
    ...
    if (isLocalStageOn FindingQuotes) then do
      ...
      _ <- pure $ clearTimer state.props.timerId
      let
        newState = state { props { findingRidesAgain = true, searchExpire = (getSearchExpiryTime "LazyCheck"), currentStage = RetryFindingQuote, sourceSelectedOnMap = true, isPopUp = NoPopUp } }
      updateAndExit newState $ RetryFindingQuotes true newState
  ...
  case flowWithoutOffers WithoutOffers of
    ...
    false -> do
      ...
      if isLocalStageOn QuoteList then do
        ...
        let
          value = null newState.data.quoteListModelState
...
eval (ContinueWithoutOffers (SelectListRes resp)) state = do
  case resp.bookingId of
    ...
    Nothing -> do
      if isLocalStageOn QuoteList then do
        let
          updatedState = if isTipEnabled state then tipEnabledState state { props { isPopUp = TipsPopUp, customerTip { enableTips = true } } } else state { props { isPopUp = ConfirmBack } }
        continue updatedState
      else
        continue state
...
eval (ChooseYourRideAction (ChooseYourRideController.ChooseVehicleAC ChooseVehicleController.NoAction)) state = continue state { props { defaultPickUpPoint = "" } }
...
eval (ChooseYourRideAction (ChooseYourRideController.PrimaryButtonActionController (PrimaryButtonController.OnClick))) state = do
  ...
  if state.data.currentSearchResultType == QUOTES then do
    ...
    _ <- pure $ updateLocalStage FindingQuotes
    let
      updatedState = state { props { currentStage = FindingQuotes, searchExpire = (getSearchExpiryTime "LazyCheck") } }
    updateAndExit (updatedState) (GetQuotes updatedState)

eval (ChooseYourRideAction ChooseYourRideController.NoAction) state = continue state { props { defaultPickUpPoint = "" } }

eval MapReadyAction state =
  continueWithCmd state
    [ do
...
tagClickEvent savedAddressType arrItem state = case savedAddressType, arrItem of
  OTHER_TAG, _ -> do
  ...
  _, Just item -> do
    if state.props.isSource == Just true then do
      let
        newState = state { data { source = item.description, sourceAddress = item.fullAddress }, props { sourcePlaceId = item.placeId, sourceLat = fromMaybe 0.0 item.lat, sourceLong = fromMaybe 0.0 item.lon, sourceSelectedOnMap = true } }
...
locationSelected item addToRecents state = do
  _ <- pure $ hideKeyboardOnNavigation true
  let
    favClick = if item.postfixImageUrl == "ny_ic_fav_red,https://assets.juspay.in/beckn/nammayatri/user/images/ny_ic_fav_red.png" then "true" else "false"
  if state.props.isSource == Just true then do
    let
      _ =
    ...
    let
      newState = state { data { source = item.title, sourceAddress = encodeAddress (item.title <> ", " <> item.subTitle) [] item.placeId }, props { sourcePlaceId = item.placeId, sourceLat = fromMaybe 0.0 item.lat, sourceLong = fromMaybe 0.0 item.lon, sourceSelectedOnMap = (item.tag /= "") } }
    ...
    let
      newState = state { data { destination = item.title, destinationAddress = encodeAddress (item.title <> ", " <> item.subTitle) [] item.placeId }, props { destinationPlaceId = item.placeId, destinationLat = fromMaybe 0.0 item.lat, destinationLong = fromMaybe 0.0 item.lon } }
...
getSearchExpiryTime dummy =
  let
    count = fromMaybe 0 (fromString (getValueToLocalStore TEST_POLLING_COUNT))
...
estimatesFlow estimatedQuotes state = do
  let
    estimatesInfo = getEstimatesInfo estimatedQuotes "AUTO_RICKSHAW" state
  ...
  if not (null estimatesInfo.estimatedVarient) && isLocalStageOn FindingEstimate then do
    ...
    exit
      $ SelectEstimate
          state
            { data
              { suggestedAmount = estimatesInfo.estimatedPrice
...
specialZoneFlow estimatedQuotes state = do
  let
    quoteList = getSpecialZoneQuotes estimatedQuotes state.data.config.estimateAndQuoteConfig
...
estimatesListFlow estimates state = do
  let
    estimatesInfo = getEstimatesInfo estimates "" state
  void $ pure $ setSelectedEstimatesObject estimatesInfo.defaultQuote
  ...
  case (null estimatedVarient) of
    ...
    false -> do
      if (estimatedPrice > state.data.selectedEstimatesObject.basePrice) then
        ...
        _ <- pure $ updateLocalStage FindingQuotes
        let
          updatedState = state { data { suggestedAmount = estimatedPrice }, props { estimateId = defaultQuote.id, currentStage = FindingQuotes, searchExpire = (getSearchExpiryTime "LazyCheck") } }
        updateAndExit updatedState $ GetQuotes updatedState
...
getRateCardArray nightCharges lang baseFare extraFare additionalFare =
  ( [ { title: (if (lang == "EN_US") then (getString MIN_FARE_UPTO) <> " 2 km" else "2 km " <> (getString MIN_FARE_UPTO)) <> if nightCharges then " 🌙" else "", description: "₹" <> toStringJSON (baseFare) }
...
findingQuotesSearchExpired gotQuotes =
  let
    secondsPassed = getExpiryTime (getValueToLocalStore FINDING_QUOTES_START_TIME) true
...
getPeekHeight state =
  ...
  in
    if homescreenHeader == 0 then 700 else requiredPeekHeight
  where
  ...
  getPeekHeightForAndroid homescreenHeader =
    let
      androidPixels = runFn1 getPixels ""
