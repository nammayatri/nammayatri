  ...
  case state.props.currentStage of
    ...
    _               -> do
                        if state.props.isLocationTracking then continue state{props{isLocationTracking = false}}
                          ...
                          else if state.props.showSafetyPlusInfoIntoPopup then continue state{ props{ showSafetyPlusInfoPopup = false } }
...
eval (ChooseYourRideAction (ChooseYourRideController.PreferSafetyPlus preferSafetyPlus' quoteList')) state = do
  let updatedQuoteList = if preferSafetyPlus' then mapQuoteList else quoteList'
      _ = spy "quote list mapped" $ show mapQuoteList
      _ = spy "quote list mapped og" $ show quoteList'
  continue $ 
...
eval (PopUpModalSafetyPlusIntroInfo PopUpModal.OnButton1Click) state = continue state {props{showSafetyPlusInfoIntoPopup = false}}

eval (PopUpModalSafetyPlusIntroInfo PopUpModal.DismissPopup) state = continue state {props{showSafetyPlusInfoIntoPopup = false}}
