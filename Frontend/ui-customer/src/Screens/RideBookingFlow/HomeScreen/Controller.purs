...
import Helpers.Utils (addToRecentSearches,  getCurrentLocationMarker, getDistanceBwCordinates, getExpiryTime, getLocationName, parseNewContacts, saveRecents, setText', updateInputString, withinTimeRange, convertUTCtoISC, getCurrentUTC)
...
eval (IsMockLocation isMock) state = do
  let val = isMock == "true"
      _ = unsafePerformEffect $ if val then  logEvent (state.data.logField) "ny_fakeGPS_enabled" else pure unit 
...
eval (SearchLocationModelActionController (SearchLocationModelController.SetLocationOnMap)) state = do
  ...
  _ <- pure $ hideKeyboardOnNavigation true
  let _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_clic k_set_location_on_map"
