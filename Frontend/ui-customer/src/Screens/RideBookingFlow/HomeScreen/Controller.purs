    ...
    [ do
        void $ launchAff $ EHC.flowRunner defaultGlobalState $ runExceptT $ runBackT
          $ do
            ...
            bbpsSessionRespOrError <- lift $ lift $ Remote.postBbpsSession mobileNumber deviceId
            case bbpsSessionRespOrError of
              Left error -> do
                void $ pure $ EHU.showToast $ getString SOMETHING_WENT_WRONG_PLEASE_TRY_AGAIN
                pure unit
              Right (API.BBPSSessionResp bbpsSessionResp) -> do
                liftFlowBT $ runEffectFn2 JB.initiateBBPS bbpsSessionResp.token deviceId
                liftFlowBT $ runEffectFn2 JB.startBBPSMicroApp (getNewIDWithTag "BBPSView") mobileNumber
        pure NoAction
    ]
...
eval (ServicesOnClick service) state = do
  ...
  case service.type of
    ...
    RC.DELIVERY -> do 
      ...
      continueWithCmd state [pure $ StartBBPS]
