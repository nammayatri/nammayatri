...
eval SearchForSelectedLocation state = updateAndExit state { props { isPopUp = NoPopUp } } $ LocationSelected (fromMaybe dummyListItem state.data.selectedLocationListItem) false state { props { currentStage = TryAgain, sourceSelectedOnMap = true, isPopUp = NoPopUp } }
...
eval (UpdateMessages message sender timeStamp size) state = do
  let
    newMessage = [ (ChatView.makeChatComponent message sender timeStamp) ]
  let
    messages = state.data.messages <> [ ((ChatView.makeChatComponent (getMessage message) sender timeStamp)) ]
  case (last newMessage) of
    Just value ->
      if value.sentBy == "Customer" then
        ...
        let
          unReadMessages = (if readMessages == 0 then true else (if (readMessages < (length messages) && state.props.currentStage /= ChatWithDriver) then true else false))
...
eval (ChatViewActionController (ChatView.TextChanged value)) state = do
  let
    sendMessageActive =
      if (STR.length (STR.trim value)) >= 1 then
        true
      else
        false
...
eval (ChatViewActionController (ChatView.SendSuggestion chatSuggestion)) state = do
  let
    suggestions = filter (\item -> (getString item) == chatSuggestion) (chatSuggestionsList "")
  let
    messageArr = map (\item -> (getEN item)) suggestions
  let
    message = fromMaybe "" (messageArr !! 0)
...
eval BackPressed state = do
  ...
  case state.props.currentStage of
    ...
    QuoteList -> do
      _ <- pure $ performHapticFeedback unit
      let
        newState = if state.props.customerTip.enableTips then (tipEnabledState state) else state
...
eval (SearchLocationModelActionController (SearchLocationModelController.PrimaryButtonActionController PrimaryButtonController.OnClick)) state = do
  ...
  _ <- pure $ exitLocateOnMap ""
  let
    newState = state { props { isSource = Just false, sourceSelectedOnMap = if (state.props.isSource == Just true) then true else state.props.sourceSelectedOnMap, isSearchLocation = SearchLocation, currentStage = SearchLocationModel, locateOnMap = false } }
...
eval (SearchLocationModelActionController (SearchLocationModelController.LocationListItemActionController (LocationListItemController.OnClick item))) state = do
  ...
  let
    condition = state.props.isSource == Just true && item.locationItemType == Just RECENTS
...
eval (SearchLocationModelActionController (SearchLocationModelController.PrimaryButtonActionController PrimaryButtonController.OnClick)) state = case state.props.selectedQuote, (null state.data.quoteListModelState), (getValueToLocalStore LOCAL_STAGE) of
...
eval (SearchLocationModelActionController (SearchLocationModelController.SetLocationOnMap)) state = do
  ...
  let
    newState = state { props { isSearchLocation = LocateOnMap, currentStage = SearchLocationModel, locateOnMap = true, isRideServiceable = true, showlocUnserviceablePopUp = false } }
...
eval (SearchLocationModelActionController (SearchLocationModelController.UpdateSource lat lng name)) state = do
  ...
  if state.props.isSource == Just true then do
    let
      newState = state { data { source = name, sourceAddress = encodeAddress name [] Nothing }, props { sourceLat = lat, sourceLong = lng, sourcePlaceId = Nothing } }
...
eval (GetQuotesList (SelectListRes resp)) state = do
  case flowWithoutOffers WithoutOffers of
    ...
    false -> do
      let
        selectedQuotes = getQuoteList ((fromMaybe dummySelectedQuotes resp.selectedQuotes) ^. _selectedQuotes)
      ...
      let
        newState = state { data { quoteListModelState = state.data.quoteListModelState <> removeExpired }, props { isSearchLocation = NoView, isSource = Nothing, currentStage = QuoteList } }
      if isLocalStageOn QuoteList then do
        _ <- pure $ spy "checking " "state"
        let
          updatedState = if newState.props.customerTip.enableTips then tipEnabledState newState { props { isPopUp = TipsPopUp } } else newState { props { isPopUp = ConfirmBack } }
...
eval (ContinueWithoutOffers (SelectListRes resp)) state = do
  case resp.bookingId of
    ...
    Nothing -> do
      if isLocalStageOn QuoteList then do
        let
          updatedState = if state.props.customerTip.enableTips then tipEnabledState state { props { isPopUp = TipsPopUp } } else state { props { isPopUp = ConfirmBack } }
        continue updatedState
      else
        continue state
...
eval (UpdateLocAndLatLong lat lng) state = do
  continueWithCmd state { props { sourceLat = (fromMaybe 0.0 (NUM.fromString lat)), sourceLong = (fromMaybe 0.0 (NUM.fromString lng)) } }
    [ do
        if os == "IOS" then do
...
eval (ChooseYourRideAction (ChooseYourRideController.ChooseVehicleAC (ChooseVehicleController.OnSelect config))) state = do
  ...
  continue
    $ if state.props.isSpecialZone then
        newState { data { specialZoneSelectedQuote = Just config.id } }
      else
        newState { props { estimateId = config.id }, data { selectedEstimatesObject = config } }
    ...
    launchAff $ flowRunner defaultGlobalState $ runExceptT $ runBackT
      $ do
          (GetPlaceNameResp locationName) <-
            Remote.placeNameBT
              ( Remote.makePlaceNameReq location.lat location.lng
...
isTipEnabled distance =
  distance < 5000
    && (getValueFromConfig "CUSTOMER_TIP")
    == "true"
...
specialZoneFlow estimatedQuotes state = do
  let
    quoteList = getSpecialZoneQuotes estimatedQuotes
