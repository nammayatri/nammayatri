...
eval (DriverInfoCardActionController (DriverInfoCardController.MessageDriver)) state = do
  if state.data.config.feature.enableChat then do
    if not state.props.chatcallbackInitiated || state.props.emergencyHelpModal || state.data.waitTimeInfo then continue state else do
      ...
      let allMessages = getChatMessages FunctionCall
      continue state {data{messages = allMessages}, props {currentStage = ChatWithDriver, sendMessageActive = false, unReadMessages = false, showChatNotification = false, isChatNotificationDismissed = null allMessages, sheetState = COLLAPSED}}
      -- continueWithCmd state {data{messages = allMessages}, props {currentStage = ChatWithDriver, sendMessageActive = false, unReadMessages = false, showChatNotification = false, isChatNotificationDismissed = null allMessages}} [ do
