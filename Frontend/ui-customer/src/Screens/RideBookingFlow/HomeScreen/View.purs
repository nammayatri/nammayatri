      ...
      , ( \push -> do
            ...
            case initialState.props.currentStage of
              ...
              FindingEstimate -> do
                ...
                if initialState.data.currentCityConfig.iopConfig.enable then do 
                  when (not $ getValueToLocalStore STARTED_ESTIMATE_SEARCH == "TRUE") do -- Check if estimate search is already started
                    ...
                    void $ pure $ spy "DEBUG: getEstimatePolling" $ (getValueToLocalStore STARTED_ESTIMATE_SEARCH)
                    void $ launchAff $ flowRunner defaultGlobalState $ getEstimatePolling (getValueToLocalStore TRACKING_ID) GetEstimates CheckFlowStatusAction 3 2000.0 push initialState
                     ...
                     void $ pure $ spy "DEBUG: getEstimatePolling" $ (estimatesPolling)
              ...
              RideAccepted -> do
                ...
                push LoadMessages
                when (not initialState.props.chatcallbackInitiated && initialState.data.fareProductType /= FPT.ONE_WAY_SPECIAL_ZONE && disableChatForDelivery initialState.data.fareProductType) $ do
...
messageWidgetView push state = 
  ...
  where disableSuggestions :: HomeScreenState -> Boolean
        disableSuggestions state = state.data.fareProductType == FPT.ONE_WAY_SPECIAL_ZONE || not state.data.config.feature.enableChat || not state.data.config.feature.enableSuggestions || disableChatForDelivery state.data.fareProductType
