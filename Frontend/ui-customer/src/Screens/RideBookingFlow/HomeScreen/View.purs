...
sosView push state =
  relativeLayout
    ...
    , height WRAP_CONTENT
    , gravity CENTER
    , visibility $ boolToVisibility $ Arr.elem state.props.currentStage [RideAccepted, RideStarted] && state.data.config.feature.enableSafetyFlow && (state.data.fareProductType /= FPT.DELIVERY || (state.props.currentStage == RideAccepted && state.data.driverInfoCardState.driverArrived) || state.data.driverInfoCardState.destinationReached )
