...
import Helpers.Utils (Merchant(..), decodeError, fetchAndUpdateCurrentLocation, getCurrentLocationMarker, getLocationName, getMerchant, getNewTrackingId, parseFloat, storeCallBackCustomer, storeCallBackLocateOnMap, storeOnResumeCallback, toString)
...
screen initialState =
  ...
  , globalEvents:
      [ ( \push -> do
            ...
            _ <- if (getMerchant FunctionCall == NAMMAYATRI) then isMockLocation push IsMockLocation else pure unit
        ...
        , afterRender
            (  \action -> do
                ...
                if(state.props.openChatScreen == true && state.props.currentStage == RideAccepted) then do 
                  if not state.props.isChatOpened then showAndHideLoader 5000.0 (getString LOADING) (getString PLEASE_WAIT_WHILE_IN_PROGRESS) defaultGlobalState else pure unit
                  push OpenChatScreen 
                else pure unit
            )
                ...
                , linearLayout
                    ...
                    , imageView
                        ...
                        , imageWithFallback $ case (state.props.currentStage == ConfirmingLocation) || state.props.isSource == (Just true) of
                            true  ->  "ny_ic_src_marker,https://assets.juspay.in/nammayatri/images/common/ny_ic_src_marker.png"
                            false ->  "ny_ic_dest_marker,https://assets.juspay.in/nammayatri/images/common/ny_ic_dest_marker.png"
...
rateExperienceView state push =
  ...
  , gravity CENTER
    ...
    , commonTextView state push ((getString RATE_YOUR_RIDE_WITH) <> state.data.driverInfoCardState.driverName) Color.black800 (FontStyle.h3 TypoGraphy) 8
...
completedRideDetails state push =
  ...
  , gravity CENTER
      ...
      , padding $ PaddingTop safeMarginTop
          ...
          [ height $ V 40
          , width $ V 40
          , imageWithFallback "ic_headphone_white,https://assets.juspay.in/beckn/yatri/user/images/ny_ic_headphone_white.png"
