...
rideInfoView push state =
  ...
  , orientation VERTICAL
  ][ (if state.data.currentSearchResultType == QUOTES || (getValueFromConfig "isChatEnabled") == "false" then 
...
messageNotificationView push state =
  ...
  , visibility $ if (((any (_ == state.props.currentStage)) [ RideAccepted, ChatWithDriver]) && state.data.currentSearchResultType /= QUOTES && (getValueFromConfig "isChatEnabled") == "true") then VISIBLE else GONE
  , cornerRadius 20.0
  ][linearLayout 
    [ height $ WRAP_CONTENT
    , width $ MATCH_PARENT
