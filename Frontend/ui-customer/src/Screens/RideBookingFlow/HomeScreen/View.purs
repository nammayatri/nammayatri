          ...
          Right (GetDriverLocationResp resp) -> do
            ...
            if ((getValueToLocalStore TRACKING_ID) == trackingId) then do
              if (getValueToLocalStore TRACKING_ENABLED) == "False" then do
                ...
                let rentalPoints = Nothing--spy "RentalPoints" if state.data.rideType == RideType.RENTAL_RIDE then points else Nothing
                    rentalRoute = route 
                ...
                liftFlow $ drawRoute (fromMaybe {points : []} points) (fromMaybe {points : []} Nothing) "LineString" "#323643" true markers.srcMarker markers.destMarker 8 "DRIVER_LOCATION_UPDATE" "" (metersToKm (fromMaybe 0 routeDistance) (state.props.currentStage == RideStarted)) specialLocationTag (getNewIDWithTag "CustomerHomeScreen")
    ...
    case resp of
      Right response -> do
        ...
        let (RideBookingRes resp) = response
            ...
            status = if any ( _ == fareProductType ) ["OneWaySpecialZoneAPIDetails" , "RENTAL"] then "CONFIRMED" else "TRIP_ASSIGNED"
            willRideListNull = any ( _ == fareProductType ) ["OneWaySpecialZoneAPIDetails" , "RENTAL"]
  ...
  relativeLayout
    ...
    , onAnimationEnd
            ( \action -> do
                _ <- push action
                _ <- getCurrentPosition push CurrentLocation
                void $ pure $ spy "INside showMap" "sdfjhgsdjhbjk"
