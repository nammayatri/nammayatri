      ...
      [ ( \push -> do
            ...
            case initialState.props.currentStage of
              ...
              FindingEstimate -> do
                ...
                pure unit
              FindingQuotes -> do
                when ((getValueToLocalStore FINDING_QUOTES_POLLING) == "false")
                  $ do
                      ...
                      let
                        pollingCount = ceil ((toNumber initialState.props.searchExpire) / ((fromMaybe 0.0 (NUM.fromString (getValueToLocalStore TEST_POLLING_INTERVAL))) / 1000.0))
              ...
              HomeScreen -> do
                when (isJust initialState.data.rideHistoryTrip)
                  $ do
              ...
              SettingPrice -> do
                _ <- pure $ removeMarker (getCurrentLocationMarker (getValueToLocalStore VERSION_NAME))
                when (initialState.props.isRepeatRide && initialState.data.config.estimateAndQuoteConfig.enableOnlyAuto)
                  $ do
                      if (os == "IOS") then
                        startTimerWithTime (show 5) "repeatRide" "1" push RepeatRideCountDown
                      else
                        countDown 5 "repeatRide" push RepeatRideCountDown
                pure unit
              PickUpFarFromCurrentLocation -> void $ pure $ removeMarker (getCurrentLocationMarker (getValueToLocalStore VERSION_NAME))
              RideAccepted -> do
                if (initialState.data.config.notifyRideConfirmationConfig.notify && any (_ == getValueToLocalStore NOTIFIED_CUSTOMER) [ "false", "__failed", "(null)" ]) then
                  if (os == "IOS") then
                    liftEffect $ startTimerWithTime (show 5) "notifyCustomer" "1" push NotifyDriverStatusCountDown
                  else
                    liftEffect $ countDown 5 "notifyCustomer" push NotifyDriverStatusCountDown
                else
                  pure unit
                _ <- pure $ enableMyLocation true
                if ((getValueToLocalStore DRIVER_ARRIVAL_ACTION) == "TRIGGER_WAITING_ACTION") then
                  ...
                  let
                    secondsLeft = initialState.data.config.driverInfoConfig.specialZoneQuoteExpirySeconds - (getExpiryTime initialState.data.driverInfoCardState.createdAt true)
...
view push state =
  let
    showLabel = if state.props.defaultPickUpPoint == "" then false else true
  in
    frameLayout
      [ height MATCH_PARENT
      , width MATCH_PARENT
              ...
              ( [ frameLayout
                    ...
                    , linearLayout
                        ...
                        [ textView
                            ...
                            , text
                                if DS.length state.props.defaultPickUpPoint > state.data.config.mapConfig.labelTextSize then
                        ...
                        , imageView
                            ...
                            , imageWithFallback $ fetchImage FF_COMMON_ASSET
                                $ case (state.props.currentStage == ConfirmingLocation) || state.props.isSource == (Just true) of
                ...
                , if state.props.repeatRideTimer /= "0" then
                    linearLayout
                      [ width MATCH_PARENT
                      , height MATCH_PARENT
...
disabilityPopUpView push state = PopUpModal.view (push <<< DisabilityPopUpAC) (CommonComponentConfig.accessibilityPopUpConfig state.data.disability state.data.config.purpleRideConfig)
            ...
            ( map
                ( \item ->
                    linearLayout
                      ...
                      , if (item.type == ANONYMOUS_CALLER) then
                          linearLayout
                            [ height $ V 1
                            , width MATCH_PARENT
...
recenterButtonView push state =
  (if os == "IOS" then PrestoAnim.animationSet [] else PrestoAnim.animationSet [ translateYAnimFromTop $ translateYAnimHomeConfig BOTTOM_TOP ])
    $ linearLayout
        ...
        [ -- linearLayout --   [ width WRAP_CONTENT
          --   , height WRAP_CONTENT
  ...
  PrestoAnim.animationSet (buttonLayoutAnimation state)
    $ linearLayout
        ...
        , linearLayout
            ...
            , if state.props.isSearchLocation == LocateOnMap then
                emptyLayout state
              else
                recentSearchesAndFavourites state push (null state.data.savedLocations) (null state.data.recentSearchs.predictionArray)
            ]
        ]
...
recentSearchesAndFavourites state push hideSavedLocsView hideRecentSearches =
  linearLayout
    ...
    ( [ if (not hideSavedLocsView) then savedLocationsView state push else linearLayout [ visibility GONE ] []
      ...
      , if (getValueToLocalStore DISABILITY_UPDATED == "false" && state.data.config.showDisabilityBanner) then
          updateDisabilityBanner state push
        else if (state.data.config.feature.enableZooTicketBookingFlow) then
            ...
            , linearLayout
                ...
                , textView
                    $ [ height WRAP_CONTENT
                      , width MATCH_PARENT
                      , text
                          if state.props.isSrcServiceable then
...
rideRequestFlowView push state =
  linearLayout
    ...
    [ -- TODO Add Animations -- PrestoAnim.animationSet
      ...
      relativeLayout
        ...
        [ PrestoAnim.animationSet [ fadeIn true ]
            $ if (state.props.currentStage == SettingPrice) then
                if (not state.data.config.estimateAndQuoteConfig.enableOnlyAuto || state.props.specialZoneType == "OneWaySpecialZoneAPIDetails") then
...
estimatedFareView push state =
  linearLayout
    ...
    , afterRender
        ( \action -> do
            let
              fareEstimate = if state.data.rateCard.additionalFare == 0 then "₹" <> (show state.data.suggestedAmount) else "₹" <> (show state.data.suggestedAmount) <> "-" <> "₹" <> (show $ (state.data.suggestedAmount + state.data.rateCard.additionalFare))
...
rideDetailsView push state =
  linearLayout
    ...
    , linearLayout
        ...
        [ let
            fareEstimate = if state.data.rateCard.additionalFare == 0 then state.data.config.currency <> (show state.data.suggestedAmount) else state.data.config.currency <> (show state.data.suggestedAmount) <> "-" <> state.data.config.currency <> (show $ (state.data.suggestedAmount + state.data.rateCard.additionalFare))
          in
            textView
              $ [ text $ fareEstimate
                , width WRAP_CONTENT
    ...
    , linearLayout
        ...
        [ linearLayout
            ...
            , imageView
                ...
                , accessibility DISABLE
                , imageWithFallback
                    $ if state.data.showPreferences then
                        fetchImage FF_COMMON_ASSET "ny_ic_chevron_up"
                      else
                        fetchImage FF_ASSET "ny_ic_chevron_down"
                ]
            ]
        , linearLayout
            [ width MATCH_PARENT
...
confirmPickUpLocationView push state =
  let
    zonePadding = if os == "IOS" then 0 else (ceil (toNumber (screenWidth unit)) / 8)
  in
    linearLayout
      [ orientation VERTICAL
      , height WRAP_CONTENT
      ...
      , linearLayout
          ...
          [ linearLayout
              ...
              , textView
                  ...
                  , textSize FontSize.a_14
                  , text
                      if state.props.confirmLocationCategory == "SureBlockedAreaForAutos" then
...
loaderView push state =
  linearLayout
    ...
    , PrestoAnim.animationSet [ fadeIn true ]
        $ textView
        $ [ accessibilityHint
              $ DS.replaceAll (DS.Pattern ".") (DS.Replacement "")
      ...
      case resp of
        ...
        Left err -> do
          ...
          if (err.code == 400 && codeMessage == "ACTIVE_BOOKING_ALREADY_PRESENT") then do
            ...
            if (count == 1) then do
              let
                response = GetQuotesRes { quotes: [], estimates: [], fromLocation: SearchReqLocationAPIEntity { lat: 0.0, lon: 0.0 }, toLocation: Nothing }
              _ <- pure $ updateLocalStage SearchLocationModel
              doAff do liftEffect $ push $ action response
            else do
              getEstimate action flowStatusAction (count - 1) duration push state
...
getQuotesPolling pollingId action retryAction count duration push state = do
  when (pollingId == (getValueToLocalStore TRACKING_ID) && (isLocalStageOn FindingQuotes))
    $ do
        internetCondition <- liftFlow $ isInternetAvailable unit
        when internetCondition
          $ do
              let
                gotQuote = (getValueToLocalStore GOT_ONE_QUOTE)
              let
                minimumPollingCount = fromMaybe 0 (fromString (getValueToLocalStore TEST_MINIMUM_POLLING_COUNT))
              let
                usableCount = if gotQuote == "TRUE" && count > minimumPollingCount then minimumPollingCount else count
              if (spy "USABLECOUNT :- " usableCount > 0) then do
                ...
                let
                  response = SelectListRes { selectedQuotes: Nothing, bookingId: Nothing }
              ...
              if (length respBooking.rideList) > 0 then do
                case (respBooking.rideList !! 0) of
                  Just (RideAPIEntity res) -> do
                    let
                      rideStatus = res.status
                    doAff do liftEffect $ push $ action rideStatus
                    if (os /= "IOS" && res.driverArrivalTime /= Nothing && (getValueToLocalStore DRIVER_ARRIVAL_ACTION) == "TRIGGER_DRIVER_ARRIVAL") then
      ...
      case response of
        Right (GetDriverLocationResp resp) -> do
          let
            ...
            specialLocationTag =
              if (any (_ == state.props.currentStage) [ RideAccepted, ChatWithDriver ]) then
              ...
              case state.data.route of
                Just (Route route) -> do
                  ...
                  if locationResp.isInPath then do
                    ...
                    let
                      specialLocationTag =
                        if (any (\stage -> isLocalStageOn stage) [ RideAccepted, ChatWithDriver ]) then
        ...
        ( mapWithIndex
            ( \index item ->
                linearLayout
                  ...
                  [ MenuButton.view (push <<< MenuButtonActionController) (menuButtonConfig state item) ]
            )
            state.data.nearByPickUpPoints
        )
    ]
...
genderBanner push state = Banner.view (push <<< GenderBannerModal) (genderBannerConfig state)
            ...
            [ bottomSheetLayout
                ...
                , onSlide
                    ( \action -> do
                        case action of
                          Scroll val ->
                            if ((state.props.currSlideIndex < truncate 1 val) && not state.props.isHomescreenExpanded) || ((state.props.currSlideIndex > truncate 1 val) && state.props.isHomescreenExpanded) then
                              push (Scroll (truncate 1 val))
                            else
                              pure unit
                          _ -> push action
                        pure unit
                    ...
                    [ linearLayout
                        ...
                        [ scrollView
                            ...
                            [ linearLayout
                                ...
                                , if (suggestionViewVisibility state) then
...
whereToButtonView push state =
  let
    whereToButtonConfig = state.data.config.homeScreen.whereToButton
  in
    linearLayout
      [ width MATCH_PARENT
      , height WRAP_CONTENT
    ...
    [ PrestoAnim.animationSet
        ...
        $ linearLayout
            ...
            [ linearLayout
                ...
                , visibility
                    if not state.data.config.terminateBtnConfig.visibility then
                      GONE
                    else if state.props.isHomescreenExpanded then
                      INVISIBLE
                    else
                      VISIBLE
                , onClick push (const TerminateApp)
    ...
    relativeLayout
      ...
      , afterRender
          ( \action -> do
              ...
              if state.props.openChatScreen && state.props.currentStage == RideAccepted then
                push OpenChatScreen
              else
                pure unit
              case state.props.currentStage of
...
getMapDimensions state =
  let
    mapHeight =
      if (any (_ == state.props.currentStage) [ RideAccepted, RideStarted, ChatWithDriver ] && os /= "IOS" && not state.props.locateOnMap) then
...
suggestionsView push state =
  linearLayout
    ...
    , if null state.data.tripSuggestions then
    ...
    , linearLayout
        ...
        , width MATCH_PARENT
        , visibility
            if (length state.data.tripSuggestions > 2) then
              VISIBLE
            else if length state.data.tripSuggestions > 0 then
              GONE
            else if length state.data.destinationSuggestions > 2 then
              VISIBLE
            else
              GONE
        , margin $ MarginTop 10
...
suggestedLocationCardView push state =
  let
    takeValue = if state.props.suggestionsListExpanded then 5 else 2
  in
    linearLayout
      [ height WRAP_CONTENT
      , width MATCH_PARENT
...
getMarginFromConfig marginConfig = Margin marginConfig.left marginConfig.top marginConfig.right marginConfig.bottom

getShadowFromConfig :: ShadowConfig -> Shadow
getShadowFromConfig shadowConfig = Shadow shadowConfig.x shadowConfig.y shadowConfig.blur shadowConfig.spread shadowConfig.color shadowConfig.opacity
