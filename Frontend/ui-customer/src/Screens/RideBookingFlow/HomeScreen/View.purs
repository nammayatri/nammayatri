...
import Data.Array (any, length, mapWithIndex, null,take, (!!),head, filter, cons, null)
  ...
  , globalEvents:
      [ ( \push -> do
            ...
            case initialState.props.currentStage of
              ...
              HomeScreen -> do
                if (isJust initialState.data.rideHistoryTrip) then 
       ...
       [ relativeLayout
            ...
            , if state.props.repeatRideTimer /= "0" 
              then linearLayout
                    ...
                    , clickable $ state.props.repeatRideTimerId /= "" 
     ...
     linearLayout
        ...
        , linearLayout
            ...
            [ linearLayout
                ...
                [ imageView
                    [ imageWithFallback $ fetchImage FF_ASSET $ if state.data.config.showDashboard && (checkVersion "LazyCheck") then "ic_menu_notify" else "ny_ic_hamburger"
                    , height $ V 24
                    , width $ V 24
  ...
  linearLayout
      ...
      , accessibility if state.data.settingSideBar.opened /= SettingSideBar.CLOSED || state.props.currentStage == ChatWithDriver || state.props.isCancelRide || state.props.isLocationTracking || state.props.callSupportPopUp || state.props.cancelSearchCallDriver || state.props.showCallPopUp || state.props.emergencyHelpModal || state.props.showRateCard then DISABLE_DESCENDANT else DISABLE
        ...
        linearLayout
          ...
          [ imageView
              [ imageWithFallback if (any (_ == state.props.currentStage) [ SettingPrice, ConfirmingLocation, PricingTutorial, DistanceOutsideLimits ]) then fetchImage FF_COMMON_ASSET "ny_ic_chevron_left" else if state.data.config.showDashboard && (checkVersion "LazyCheck") then fetchImage FF_ASSET "ic_menu_notify" else fetchImage FF_ASSET "ny_ic_hamburger"
              , height $ V 25
              , accessibility DISABLE
              , clickable true
              , onClick push $ if (any (_ == state.props.currentStage) [ SettingPrice, ConfirmingLocation, PricingTutorial, DistanceOutsideLimits ]) then const BackPressed else const OpenSettings
  ...
  linearLayout
    ...
    , linearLayout
        ...
        , gravity CENTER_VERTICAL
            ...
            , height WRAP_CONTENT
            , fontStyle $ FontStyle.bold LanguageStyle
            , onClick (\action -> if state.data.config.showRateCard then push action else pure unit ) $ const ShowRateCard
            ] <> FontStyle.body7 LanguageStyle
         , imageView
            ...
            , height $ V 40
            , accessibility DISABLE
            , visibility if state.data.config.showRateCard then VISIBLE else GONE
            , onClick (\action -> if state.data.config.showRateCard then push action else pure unit ) $ const ShowRateCard
            ]
        ]
    ]
...
bookingPreferencesView push state = 
 linearLayout
  ...
  , height WRAP_CONTENT
  , orientation VERTICAL
  , visibility if state.data.config.showBookingPreference && not state.props.isRepeatRide then VISIBLE else GONE
  ][ linearLayout
      [ width MATCH_PARENT
      , height $ V 1
          ...
          , margin (MarginBottom 8)
           ...
           , imageView
              ...
              , imageWithFallback if state.data.showPreferences
                                  then "ny_ic_chevron_up,https://assets.juspay.in/nammayatri/images/common/ny_ic_chevron_up.png"
                                  else "ny_ic_chevron_down,https://assets.juspay.in/nammayatri/images/user/ny_ic_down_arrow.png"
              ]
          ]
        , linearLayout
            [ width MATCH_PARENT
...
mapView push state = 
  ...
  relativeLayout
    ...
    , afterRender
        ...
        , clickable $ if isHomeScreenView state then false else true
        ][]
     , linearLayout 
        [ height WRAP_CONTENT
        , width MATCH_PARENT
