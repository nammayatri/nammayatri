      ...
      [ ( \push -> do
            ...
            case initialState.props.currentStage of
              ...
              RideAccepted -> do
                ...
                push LoadMessages
                if(not initialState.props.chatcallbackInitiated && initialState.data.fareProductType /= FPT.ONE_WAY_SPECIAL_ZONE) then do
                  _ <- clearChatMessages
       ...
       [ relativeLayout
            ...
            ([ frameLayout
                ...
                , linearLayout
                    ...
                    , height MATCH_PARENT
                    , background Color.transparent
                    , padding (PaddingBottom if os == "IOS" then 43 + (if showLabel then 20 else 0) else 70)
                    , gravity CENTER
                    , accessibility DISABLE
                    , orientation VERTICAL
                    ]
...
messageWidgetView push state = 
  let isWidgetVisible = ((any (_ == state.props.currentStage)) [ RideAccepted, ChatWithDriver]) && state.data.fareProductType /= FPT.ONE_WAY_SPECIAL_ZONE && state.data.config.feature.enableChat && state.data.config.feature.enableSuggestions && not state.props.removeNotification
