module Screens.RideBookingFlow.MeterSelectDestinationScreen.Controller where
...
import Screens.RideBookingFlow.MeterSelectDestinationScreen.ScreenData (initData)
import Timers
...
data ScreenOutput
  = GoBack ST.MeterSelectDestinationScreenState
  | SearchPlace String ST.MeterSelectDestinationScreenState 
  | GetPlaceName ST.MeterSelectDestinationScreenState String
  | UpdatedLocationName ST.MeterSelectDestinationScreenState Number Number
  | GoToMeterRideScreen ST.MeterSelectDestinationScreenState

eval :: Action -> ST.MeterSelectDestinationScreenState -> Eval Action ScreenOutput ST.MeterSelectDestinationScreenState
...
eval (VoiceToText text isSuccess) state = do 
  ...
  if isSuccess then do 
    continueWithCmd updatedState [do
      push <- getPushFn Nothing "MeterSelectDestinationScreen"
...
validateSearchInput :: ST.MeterSelectDestinationScreenState -> String -> Eval Action ScreenOutput ST.MeterSelectDestinationScreenState 
validateSearchInput state searchString =
