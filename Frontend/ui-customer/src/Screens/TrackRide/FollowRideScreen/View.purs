...
getSosOverlayConfig state status =
  let
    ...
    name = getFollwerName currentFollower state
  in
    ...
    relativeLayout
      ...
      [ PrestoAnim.animationSet [ fadeIn isRideData ]
          $ linearLayout
              ...
              [ sourceDestinationView push (getTripDetails state)
              ]
      , PrestoAnim.animationSet [ fadeOut isRideData ]
          $ linearLayout
              [ width MATCH_PARENT
...
headerView push state =
  ...
  in
    linearLayout
      ...
      [ textView
          $ [ text $ (getFollwerName currentFollower state) <> " " <> getString IS_ON_THE_WAY
            , weight 1.0
            , height WRAP_CONTENT
...
genericHeaderConfig state =
  let
    ...
    titleText = if isFollowingRide then getString $ FOLLOWING (getFollwerName currentFollower state) else getString CHOOSE_A_PERSON_TO_FOLLOW
  in
...
getFollwerName :: Followers -> FollowRideScreenState -> String
getFollwerName currentFollower state = if state.data.currentStage == MockFollowRide then getMockFollowerName "" else (fromMaybe currentFollower.mobileNumber currentFollower.name)

getTripDetails :: FollowRideScreenState -> TripDetails Action
...
updateMockData push = do 
  ...
  where
    ...
    drawDriverRoute ride (GetDriverLocationResp resp) = do
      ...
      void $ runExceptT $ runBackT $ drawMapRoute srcLat srcLon dstLat dstLon (normalRoute "") "NORMAL" (getString SOS_LOCATION) (getString DROP) Nothing "trip" (HSConfig.specialLocationConfig "" "" false getPolylineAnimationConfig)
      liftFlow $ addAndUpdateSOSRipples point
      liftFlow $ animateCamera srcLat srcLon 17.0 "ZOOM"
