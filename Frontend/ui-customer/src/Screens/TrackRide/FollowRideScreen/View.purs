          ...
          case state.data.driverInfoCardState of
            ...
            Just ride -> 
              when (not initialState.props.chatCallbackInitiated) $ do
                ...
                when (currentFollower.priority == 0) $ do
                  let customerId = getCustomerId personId
                  void $ liftFlow $ clearChatMessages ride.rideId
                  void $ liftFlow $ storeCallBackMessageUpdated push ride.rideId customerId UpdateMessages
                  void $ liftFlow $ storeCallBackOpenChatScreen push OenChaptScreen
                  void $ liftFlow $ startChatListenerService
                  void $ liftFlow $ scrollOnResume push ScrollToBottom
                  void $ liftFlow $ push InitializeChat
        )
      ]
  , eval:
      \action state -> do
...
chatView :: forall w. (Action -> Effect Unit) -> FollowRideScreenState -> Layout w
chatView push state = 
  relativeLayout
  [ height $ MATCH_PARENT
  , width $ MATCH_PARENT
...
messagingViewConfig state = 
  ...
  in 
    MessagingView.config {
      userConfig
      { userName = getCustomerId personId 
      , appType = "Customer"
      }
    , feature 
      { sendMessageActive = state.props.sendMessageActive
      , canSendSuggestion = state.props.canSendSuggestion
      , showAutoGeneratedText = false --(getValueToLocalStore NOTIFIED_CUSTOMER == "true") && isJust state.data.driverInfoCardState.eta && (HU.secondsToHms $ fromMaybe 0 state.data.driverInfoCardState.eta) /= "--"
      , enableSuggestions = state.data.config.feature.enableSuggestions
      }
    , messages = state.data.messages
    , messagesSize = state.data.messagesSize
    , vehicleNo = makeNumber $ ride.registrationNumber     
    , chatSuggestionsList = []--getChatSuggestions state
    , hint = (getString MESSAGE)
    , languageKey = (getLanguageLocale languageKey)
    , rideConfirmedAt = ride.startedAt
    , autoGeneratedText = ""
    , driverRating = show $ ride.rating
    , fareAmount = ""
