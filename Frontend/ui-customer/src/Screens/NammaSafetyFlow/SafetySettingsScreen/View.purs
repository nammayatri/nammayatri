...
view push state =
  screenAnimation
    $ linearLayout
        ...
        , background if state.props.isValueAddNP then Color.black900 else Color.white900
        , padding padding'
  ...
  headerConfig = if state.props.isValueAddNP then 
...
dashboardView state push =
  ...
  relativeLayout
    [ height MATCH_PARENT
    , width MATCH_PARENT
    , background if isValueAddNP then Color.black900 else Color.white900
    , orientation VERTICAL
...
imageWithTextView text' isActive state =
  linearLayout
    ...
    , padding $ PaddingHorizontal 16 16
    , margin $ MarginTop 12
    , background if state.props.isValueAddNP then Color.blackOpacity12 else Color.white900
    ]
    [ imageView
    ...
    , textView
        $ [ text text'
          , color if state.props.isValueAddNP then Color.white900 else Color.black800
          , weight 1.0
          , height WRAP_CONTENT
          , singleLine false
          ]
