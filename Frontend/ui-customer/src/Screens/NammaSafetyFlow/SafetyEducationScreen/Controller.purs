...
eval (StartNammaSafetyOnboarding PrimaryButtonController.OnClick) state = continue state { props { setupStage = SetDefaultEmergencyContacts } }
...
eval (ChangeEducationViewIndex index) state = do
  ...
  updateAndExit state { props { educationViewIndex = Nothing } } $ Refresh newState
