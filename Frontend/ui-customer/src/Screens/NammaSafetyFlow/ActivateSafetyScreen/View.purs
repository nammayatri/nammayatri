...
view push state =
  screenAnimation
    $ relativeLayout
        ...
        [ linearLayout
            ...
            [ if state.props.reportPastRide && not state.props.confirmTestDrill then Header.view (push <<< SafetyHeaderAction) $ postRideHeaderConfig state 
              else Header.view (push <<< SafetyHeaderAction) headerConfig
  ...
  headerConfig =
    (Header.config Language)
      ...
      , useLightColor = true
      , title = SU.getStringWithoutNewLine if not state.props.showCallPolice then SAFETY_CENTER else CALL_POLICE
...
sosButtonConfig state =
  {
    ...
    primaryContactAndEdit: case contactName of
      Just name -> Just imageWithTextConfig {text' = getString CALL <> " : " <> name, visibility = isJust contactName, useFullWidth = false}
      Nothing -> Nothing,
...
-- sosButtonAndDescriptionView :: NammaSafetyScreenState -> (Action -> Effect Unit) -> forall w. PrestoDOM (Effect Unit) w
-- sosButtonAndDescriptionView state push =
--   linearLayout
--     [ width MATCH_PARENT
--     , height WRAP_CONTENT
...
--     , imageWithTextView imageWithTextConfig {text' = getString RECEIVE_CALL_FROM_SAFETY_TEAM}
--     , imageWithTextView imageWithTextConfig {text' = getString NOTIFY_ALL_EMERGENCY_CONTACTS}
--     , linearLayout
--         [ height WRAP_CONTENT
--         , width MATCH_PARENT
--         , gravity CENTER_VERTICAL
--         ]
--         [ imageWithTextView imageWithTextConfig {text' = getString CALL <> " : " <> fromMaybe "" contactName, visibility = isJust contactName, useFullWidth = false}
--         , textView
--             $ [ text $ getString EDIT
--               , color Color.blue800
--               , gravity RIGHT
...
otherActionsView state push =
  linearLayout
    ...
    , linearLayout
        ...
        ( map
            ( \item ->
                SafetyActionTileView.view item.image item.text item.push item.backgroundColor item.strokeColor (V $ (EHC.screenWidth unit - 56) / 3) true
            )
            otherActions
        )
    ]
    where   
      otherActions =
        [ { text: getString RECORD_AUDIO, image: "ny_ic_microphone_white", backgroundColor: Color.blackOpacity12, strokeColor: Color.black800, push : push <<< RecordAudio }
        , { text: getString CALL_POLICE , image: "ny_ic_police" , backgroundColor: Color.redOpacity20, strokeColor: Color.redOpacity30, push : push <<< ShowPoliceView }
        , { text: getString CALL_SAFETY_TEAM, image: "ny_ic_police_alert", backgroundColor: Color.blackOpacity12, strokeColor: Color.black800, push : push <<< CallSafetyTeam }
        ]
...
triggeringSosView state push =
  linearLayout
    ...
    [ linearLayout 
      ...
      , width MATCH_PARENT
      , margin $ Margin 16 16 16 16
      ][ SafetyActionTileView.view sirenImage (getString SIREN) (push <<< ToggleSiren) sirenTileBackgroundColor sirenTileStrokeColor MATCH_PARENT false]
