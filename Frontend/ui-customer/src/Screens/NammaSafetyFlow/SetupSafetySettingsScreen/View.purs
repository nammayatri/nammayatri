...
import Screens.NammaSafetyFlow.SetupSafetySettingsScreen.Controller (Action(..), ScreenOutput, eval, checkForContactsAndSupportDisabled)
import Screens.Types (NammaSafetyScreenState, NammaSafetyStage(..), NewContacts, RecordingState(..), StepsHeaderModelState, SafetySetupStage(..))
...
settingUpContentViewData state = case state.props.setupStage of
  SetDefaultEmergencyContacts -> { title: getString SHARE_INFO_WITH_EMERGENCY_CONTACTS_TITLE, desc: getString SHARE_INFO_WITH_EMERGENCY_CONTACTS_DESC, image: "ny_ic_share", step: 0, isActive: state.data.shareToEmergencyContacts && length state.data.contactsList /= 0 }
  SetNightTimeSafetyAlert -> { title: getString ENABLE_NIGHT_TIME_SAFETY_ALERTS_TITLE, desc: getString ENABLE_NIGHT_TIME_SAFETY_ALERTS_DESC, image: "ny_ic_night_safety", step: getStepNumber 2, isActive: state.data.nightSafetyChecks }
  SetPersonalSafetySettings -> { title: getString ALMOST_DONE_TITLE, desc: getString ALMOST_DONE_DESC, image: "", step: getStepNumber 3, isActive: state.data.nightSafetyChecks }
