...
import Timers (clearTimerWithId, waitingCountdownTimerV2Impl)
...
eval (RecordAudio SafetyActionTileView.OnClick) state = continue state { props { isAudioRecordingActive = true } }

eval StartRecord state = 
  continueWithCmd state { props { recordingTimer = "00 : 00", audioRecordingStatus = ST.RECORDING  } }  [do
    ...
    if recordingStarted then do
      ...
      pure $ UpdateState state { props { recordingTimer = "00 : 00", audioRecordingStatus = ST.RECORDING } }
    else
      pure $ NoAction
  ]
...
eval StopRecord state = 
  continueWithCmd state { props { audioRecordingStatus = ST.RECORDED, recordingTimer = "00 : 00"  } } [do
...
eval (ShareAudio PrimaryButtonController.OnClick) state = continueWithCmd state [ do
    case state.props.recordedAudioUrl of
      Just url -> do
        void $ runEffectFn3 JB.uploadMultiPartData url (EndPoint.updateSosMedia state.data.sosId) "Audio"
      Nothing -> pure unit
    pure NoAction
  ]
...
eval (UploadMultiPartDataCallback _ fileId) state = do
  ...
  if DS.null fileId then do
    ...
    continue state {props { recordedAudioUrl = Nothing, audioRecordingStatus = ST.NOT_RECORDING } }

eval CancelAudioRecording state = 
  continueWithCmd state { props { audioRecordingStatus = ST.NOT_RECORDING, recordingTimer = "00 : 00", isAudioRecordingActive = false } } [do
    _   <- pure $ clearTimerWithId state.props.recordingTimerId
    res <- runEffectFn1 JB.stopAudioRecording ""
    pure NoAction
  ]
