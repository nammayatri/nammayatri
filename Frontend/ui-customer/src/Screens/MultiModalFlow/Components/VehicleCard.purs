...
specialAlertToggleView push config =
  linearLayout
    ...
    , visibility $ boolToVisibility $ not config.showSource
    ]
    [ linearLayout
        [ height WRAP_CONTENT
        , width WRAP_CONTENT
...
sourceAddressView :: forall w. (Action -> Effect Unit) -> Config -> PrestoDOM (Effect Unit) w
sourceAddressView push config =
  linearLayout
    [ width MATCH_PARENT
    , orientation HORIZONTAL
    , gravity CENTER
    , visibility $ boolToVisibility config.showSource
    ]
    [ imageView
        [ imageWithFallback $ HU.fetchImage HU.FF_ASSET "ny_ic_source_dot"
        , height $ V 14
        , width $ V 14
