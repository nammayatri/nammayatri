...
showEmergencyContact :: forall w. PrestoList.ListItem -> (Action -> Effect Unit) -> SelectContactsScreenState -> PrestoDOM (Effect Unit) w
showEmergencyContact listitem push state =
  linearLayout
    [ width MATCH_PARENT
    , background Color.blue600
    , weight 1.0
    ]
    [ showEmergencyContactData listitem push state
    ]

showEmergencyContactData :: forall w. PrestoList.ListItem -> (Action -> Effect Unit) -> SelectContactsScreenState -> PrestoDOM (Effect Unit) w
showEmergencyContactData listItem push state =
  Keyed.linearLayout
    [ height MATCH_PARENT
    , width MATCH_PARENT
