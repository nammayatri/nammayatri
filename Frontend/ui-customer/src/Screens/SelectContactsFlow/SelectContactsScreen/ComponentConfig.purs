  ...
  let
    ...
    genericHeaderConfig' =
      config
        ...
        , textConfig
          { text = titleText
          , accessibilityHint = if state.props.showContacts then (show (length state.data.contacts) <> " Of " <> (show state.data.contactSelectionLimit) <> " " <> (getString CONTACTS_SELECTED)) else  (getString EMERGENCY_CONTACTS)
