...
import Prelude (class Eq, class Show, bind, map, negate, pure, show, unit, ($), (&&), (*), (+), (<), (<>), (==), (>), (/=), discard, void, (||), not)
...
eval (GetRentalQuotes (GetQuotesRes quoteRes)) state = do 
  ...
  let filteredQuoteList = (getFilteredQuotes quoteRes.quotes state.data.config.estimateAndQuoteConfig)
  let _ = spy "quoteList" quoteList
  let rentalsQuoteList = spy "Array" (DA.mapWithIndex (\index quote -> 
  ...
  continue state { data{rentalsQuoteList = rentalsQuoteList}}

eval (CheckFlowStatusAction) state = continue state{data{currentStage = RENTAL_SELECT_PACKAGE}}
...
eval (PrimaryButtonActionController (PrimaryButtonController.OnClick)) state = 
  case state.data.currentStage of
    RENTAL_SELECT_PACKAGE ->
      let currentTime = unsafePerformEffect $ EHC.getUTCAfterNSeconds (EHC.getCurrentUTC "") 30
      ...
      in updateAndExit state $ DoRentalSearch newState
