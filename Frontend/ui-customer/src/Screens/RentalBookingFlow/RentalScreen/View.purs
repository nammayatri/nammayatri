...
getRentalScreenView push state = if state.data.currentStage == RENTAL_SELECT_PACKAGE then rentalPackageSelectionView push state 
        else if state.data.currentStage == RENTAL_SELECT_VARIANT then rentalVariantSelectionView push state
        else if state.data.currentStage == RENTAL_CONFIRMATION then fareBreakupView push state
        else textView[]
  ...
  linearLayout
    ...
    , scrollView
        ...
        [ linearLayout
          ...
          if null state.data.rentalsQuoteList then 
            [shimmerView push state ]
              
            else 
          ( map
            ( \quote ->
                ChooseVehicle.view (push <<< ChooseVehicleAC) quote.quoteDetails{showInfo = true}
            )
