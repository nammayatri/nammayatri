...
mapInputViewConfig state = 
  ...
  where 
    ...
    inputTextConfigArray isSelectPackageStage = 
      ...
      in
      [ { textValue : if isSelectPackageStage then pickUpText else getDateString state.data.selectedDateTimeConfig false -- formatDate "DD" <> " " <> formatDate "MMMM" <> " " <> formatDate "YYYY" <> ", " <> formatDate "HH"
        , isFocussed : false
