...
import PrestoDOM (Gravity(..), Length(..), Margin(..), Orientation(..), Visibility(..), Padding(..))
...
incrementDecrementConfig :: String -> RentalScreenState -> IncrementDecrement.Config
incrementDecrementConfig headingText state = let
  config = IncrementDecrement.config
  incrementDecrementConfig' = config
    { initialValue = case headingText of 
        "Duration" -> state.data.rentalBookingData.baseDuration
        _ -> state.data.rentalBookingData.baseDistance
    , minValue = 2
    , maxValue = 12
    , heading = case headingText of
        "Duration" -> getString SELECT_DURATION
        _ -> getString SELECT_DISTANCE
    , buttonTextConfig {
        color = Color.black800
      , background = Color.white900
      , cornerRadius = 20.0
    ...
    , width = 180
    , lineWidth = MATCH_PARENT
    , countSuffixText = case headingText of
        "Duration" -> "hr"
        _ -> "km"
    }
