...
getSDKPolling bookingId delayDuration state push action = do
  ...
  if state.props.currentStage == PaymentSDKPooling && localPoolingStatus == "true" then do
      ...
      -- (GetMetroBookingStatusResp getMetroStatusResp) -> do
      --   let (MetroTicketBookingStatus metroTicketStatusResp) = getMetroStatusResp
      --       orderResp = metroTicketStatusResp.payment >>= \(FRFSBookingPaymentAPI paymentInfo) -> paymentInfo.paymentOrder 
      --   case orderResp of
      --     Just (CreateOrderRes createOrderResp) -> doAff do liftEffect $ push $ action (CreateOrderRes createOrderResp)
      --     Nothing -> do
      --       void $ delay $ Milliseconds delayDuration
