...
import Helpers.Utils (fetchImage, FetchImageFrom(..), getMetroConfigFromCity, getCityFromString, CityMetroConfig(..))
import Prelude
  ...
  where
    getQuotes push = do
      let city = getCityFromString $ getValueToLocalStore CUSTOMER_LOCATION
          (CityMetroConfig cityMetroConfig) = getMetroConfigFromCity city
          withinTimeRange = JB.withinTimeRange cityMetroConfig.operationalHours.startTime cityMetroConfig.operationalHours.endTime $ 
