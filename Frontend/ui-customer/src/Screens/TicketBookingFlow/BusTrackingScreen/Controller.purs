...
eval (UpdateStops (API.GetMetroStationResponse metroResponse)) state = do
  let
    ...
    finalMap =
      DA.foldl
        ( \acc item@(API.GetMetroStationResp station) -> case acc.previousStop of
...
eval (UpdateTracking trackingData) state = do
  ...
  where
    processTrackingData = do
          ...
          DA.foldl
            ( \acc item -> do
                let
                  ...
                  _ = spy "acc" acc 
                  -- _ = spy "zoomIn" mbPreviousStop
                  -- _ = spy "zoomIn" (Mb.isNothing $ DT.snd acc)
                ...
                case mbPreviousStop of 
                  ...
                  Mb.Just (API.GetMetroStationResp previousStop) -> do
                    ...
                    DT.Tuple (DM.insert item.nextStop (currentValues <> [ travelDistancePercent ]) (DT.fst acc))
                      ( if (spy "condition" (zoomIn && ((Mb.isNothing $ DT.snd acc) || isBusNearer))) then
                          Mb.Just item
                        else
                          DT.snd acc
                      )
      ...
      let _ = spy "DT.snd f" ( finalMap)
      let _ = spy "DT.snd finalMap" (DT.snd finalMap)
      let nearByBusPosition = spy "nearByBusPosition" $ Mb.maybe Mb.Nothing (\item -> Mb.Just {lat : item.vehicleLat, lng : item.vehicleLon}) (DT.snd finalMap)
      continueWithCmd
        state
...
eval UserBoarded state = do
  let filterStopsIndex = DA.findIndex (\(API.GetMetroStationResp item) -> item.code == sourceCode) state.data.stopsList
...
drawDriverRoute resp state = do
  ...
  let
    route =
      Remote.walkCoordinates $ API.Snapped
        $ case response of
            Right (API.FrfsGetRouteResp routeResp) -> DA.reverse $ Mb.fromMaybe [] routeResp.waypoints
            Left err -> []

    destinationStation = DA.find (\(API.GetMetroStationResp item) -> item.code == destinationCode) resp
  filteredRoute <- case destinationStation of
    Mb.Just (API.GetMetroStationResp dest) -> do
  ...
  where
  processStop index (API.GetMetroStationResp item) = do
    let
