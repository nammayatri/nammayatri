...
import Engineering.Helpers.LogEvent (logEventWithMultipleParams, logEventWithParams)
...
eval (BookTicketButtonAction PrimaryButton.OnClick) state = do
  ...
  let _ = unsafePerformEffect $ logEvent state.data.logField "ny_bus_user_book_ticket_initiated"
...
userBoardedActions state vehicles vehicle = do
  ...
  -- Redundant code to filter specific route points might be needed in future
  -- filteredRoute <- case destinationStation, DS.null state.data.bookingId of
  --   Mb.Just (API.FRFSStationAPI dest), false -> do
