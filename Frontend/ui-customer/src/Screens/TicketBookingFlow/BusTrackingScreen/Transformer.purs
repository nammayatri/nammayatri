...
transformStationsForMap stations route srcCode destCode = do
  ...
  --     mbDest = maybe Nothing (\(API.FRFSStationAPI item) -> Just item) (res.dest)
      let stops = res.stops
          ...
          srcMarkerConfig = JB.defaultMarkerConfig  { markerId = s.code, pointerIcon = getMarkerImage s.code (-1) len, primaryText = s.name, position = sourcePosition, markerSize = if getMarkerImage s.code (-1) len == "ny_ic_stop_grey" then 20.0 else 90.0, useMarkerSize = true}
          destMarkerConfig = JB.defaultMarkerConfig { markerId = d.code, pointerIcon = getMarkerImage d.code len len, primaryText = d.name, position = destPosition, markerSize = if getMarkerImage d.code (len) len == "ny_ic_stop_grey" then 20.0 else 90.0, useMarkerSize = true}
