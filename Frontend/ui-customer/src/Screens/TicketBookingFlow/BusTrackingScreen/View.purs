...
verticalLineView push idTag showOnlyBullet vehicles =  -- state isDotted =
  ...
  busMarkerView percent = do
    let
      ...
      -- _ = spy "lineViewBounds" lineViewBounds
  ...
  linearLayout
    ...
    [ linearLayout
        ...
        [ linearLayout
            ...
            , textView
                $ [ text $ "Towards " <> (Mb.maybe "" (\(API.GetMetroStationResp item) -> MP.spaceSeparatedPascalCase item.name) (DA.last state.data.stopsList))
                  , margin $ MarginLeft 8
...
stopListView push state showOnlyBullet =
  linearLayout
    ...
    , linearLayout
        ...
        (DA.mapWithIndex (\index item@(API.GetMetroStationResp station) -> stopView item showOnlyBullet stopMarginTop state push index (getStopType station.code index state)) (if state.props.individualBusTracking && DA.length state.data.stopsList > 2 then stops else state.data.stopsList))
    ]
  where
...
--     [ 
--     ]
stopView :: forall w. API.GetMetroStationResp -> Boolean -> Int -> ST.BusTrackingScreenState -> (Action -> Effect Unit) -> Int -> StopType -> PrestoDOM (Effect Unit) w
stopView (API.GetMetroStationResp stop) showOnlyBullet marginTop state push index stopType =
  linearLayout
    [ height WRAP_CONTENT
    , width MATCH_PARENT
    , orientation VERTICAL
    ...
    [ linearLayout
        ...
        , if stopType == SOURCE_STOP && index /=0 
            then showETAView push state index (API.GetMetroStationResp stop) showOnlyBullet
            else 
              MP.noView
        ]
    , linearLayout
...
showETAView :: forall w. (Action -> Effect Unit) -> ST.BusTrackingScreenState -> Int -> API.GetMetroStationResp -> Boolean -> PrestoDOM (Effect Unit) w
showETAView push state index (API.GetMetroStationResp stop) showOnlyHeight =
  linearLayout
