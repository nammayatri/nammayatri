...
eval (AutoFill otp) state =
  updateAndExit
    ( state
        ...
        , data = state.data { otp = if (length otp <= 4) then otp else state.data.otp }
        }
    )
...
eval (CountDown seconds id status timerID) state = do
  ...
  if status == "EXPIRED" then do
    ...
    let
      newState = state { data { timer = "", timerID = "" }, props = state.props { resendEnable = true } }
    continue newState
  else
