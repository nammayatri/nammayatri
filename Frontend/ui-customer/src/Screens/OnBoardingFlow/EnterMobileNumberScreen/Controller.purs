...
data ScreenOutput
  = GoToAccountSetUp EnterMobileNumberScreenState
...
eval (MobileNumberButtonAction PrimaryButtonController.OnClick) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_otp_triggered"
  let
    newState = state { data { otpChannel = OTP.SMS }, props { btnActiveOTP = false } }
...
eval (WhatsAppOTPButtonAction PrimaryButtonController.OnClick) state = do
  let
    _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_otp_triggered"
  let
    newState = state { data { otpChannel = OTP.WHATSAPP } }
  pure $ hideKeyboardOnNavigation true
  pure $ setText (getNewIDWithTag "EnterOTPNumberEditText") ""
  exit $ GoToOTP newState
...
eval (MobileNumberEditTextAction (MobileNumberEditorController.TextChanged id value)) state = do
  _ <-
    if length value == mobileNumberMaxLength state.data.countryObj.countryShortCode then do
      pure $ hideKeyboardOnNavigation true
    else
      pure unit
  let
    validatorResp = mobileNumberValidator state.data.countryObj.countryCode state.data.countryObj.countryShortCode value
  let
    newState =
      state
        { props =
  ...
  if isValidMobileNumber validatorResp then do
    let
      _ = unsafePerformEffect $ logEvent state.data.logField "ny_user_mobnum_entry"
    pure unit
  else
    pure unit
  continue newState
...
eval (MobileNumberEditTextAction (MobileNumberEditorController.CountryCodeSelected country)) state = do
  _ <- pure $ hideKeyboardOnNavigation true
  let
    validatorResp = mobileNumberValidator country.countryCode country.countryShortCode state.data.mobileNumber
  let
    newState =
      state
...
eval (OTPEditTextAction (PrimaryEditTextController.TextChanged id value)) state = do
  let
    newState =
      state
        { props = state.props { btnActiveOTP = if length value == 4 then true else false, letterSpacing = PX if value == "" then 1.0 else 6.0, wrongOTP = if state.props.wrongOTP && value == "" then true else false }
...
eval Resend state = do
  let
    newState = state { data { attempts = if (state.data.attempts > 0) then state.data.attempts - 1 else state.data.attempts }, props { resendEnable = false } }
...
eval (AutoFill otp) state = do
  ...
  let
    newState = state { props = state.props { isReadingOTP = if length otp == 4 then false else true, capturedOtp = otp }, data = state.data { otp = if (length otp <= 4) then otp else state.data.otp } }
  updateAndExit newState $ GoToAccountSetUp (newState)
...
eval (BackPressed flag) state = do
  ...
  _ <- pure $ clearCountDownTimer state.data.timerID
  let
    newState = state { props { enterOTP = false, letterSpacing = PX 1.0 }, data { otp = "" } }
  _ <- pure $ hideKeyboardOnNavigation true
  if state.props.enterOTP then
    exit $ GoBack newState
  else if showCarouselScreen Lazy.FunctionCall then
    exit $ GoToWelcomeScreen newState { data { mobileNumber = "" } }
  else do
...
eval (CountDown seconds id status timerID) state = do
  ...
  if status == "EXPIRED" then do
    _ <- pure $ clearCountDownTimer state.data.timerID
    let
      newState = state { data { timer = 30, timerID = "" }, props = state.props { resendEnable = true } }
    continue newState
  else
