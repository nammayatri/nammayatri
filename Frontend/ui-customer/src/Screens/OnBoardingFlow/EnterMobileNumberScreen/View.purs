...
enterMobileNumberView  state lang push =
  let others = state.data.config.others
  in
  linearLayout
    [ height WRAP_CONTENT
    , width MATCH_PARENT
    ...
    , orientation VERTICAL
       ...
       ,  PrestoAnim.animationSet
          ...
          , Anim.fadeOut state.props.countryCodeOptionExpanded
            ...
            , commonTextView state " &nbsp; <u>T&Cs</u>" true (Just others.termsLink) push true true ( " By Clicking Continue: You Agree To Our Terms And Conditions" )
              ]
        , PrestoAnim.animationSet
          [ Anim.fadeIn $ not state.props.enterOTP 
