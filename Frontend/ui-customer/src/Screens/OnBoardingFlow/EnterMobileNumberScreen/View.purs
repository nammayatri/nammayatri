  ...
  , globalEvents:
      [ ( \push -> do
            ...
            if not initialState.props.resendEnable && initialState.data.attempts >= 0 && initialState.props.enterOTP then do
              _ <-
                launchAff $ EHC.flowRunner defaultGlobalState $ runExceptT $ runBackT $ lift $ lift
                  $ doAff do
                      if (EHC.os == "IOS") then
                        liftEffect $ JB.startTimerWithTime (show initialState.data.timer) "otp" "1" push CountDown
                      else
                        liftEffect $ EHC.countDown initialState.data.timer "otp" push CountDown
              pure unit
            else
              pure unit
            pure (pure unit)
        ...
        <> if EHC.os == "IOS" then [] else [ startOtpReciever AutoFill ]
  , eval:
      \action state -> do
        let
          _ = printLog "EnterMobileNumber state -----" state
        eval action state
  }
    ...
    linearLayout
      ...
      [ linearLayout
          ...
          [ PrestoAnim.animationSet
              ...
              $ StepsHeaderModel.view (push <<< StepsHeaderModelAC)
              $ (StepsHeaderModel.stepsHeaderData if state.props.enterOTP then 1 else 0) { config = state.data.config }
          , frameLayout
              [ width MATCH_PARENT
              , height MATCH_PARENT
