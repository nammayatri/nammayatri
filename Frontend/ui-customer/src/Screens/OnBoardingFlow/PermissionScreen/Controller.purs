...
data ScreenOutput
  = GoBack
  | Refresh
  | InternetCallBack PermissionScreenState
  | LocationCallBack PermissionScreenState
...
eval (LocationPermissionCallBackCustomer isLocationPermissionEnabled) state = do
  let
    status = getLocationPermissionStatus unit
  if isLocationPermissionEnabled then
    updateAndExit state (LocationCallBack state)
  else
    continue state { stage = (if status == "DENIED" then LOCATION_DENIED else NORMAL) }
...
eval (PrimaryButtonActionController PrimaryButtonController.OnClick) state = do
  let
    _ = unsafePerformEffect $ requestLocation unit
