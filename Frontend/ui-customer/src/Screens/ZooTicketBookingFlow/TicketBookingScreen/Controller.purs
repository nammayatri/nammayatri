...
data ScreenOutput
  = GoToHomeScreen TicketBookingScreenState
...
eval (ToggleTicketOption ticketID) state = do
  let
    updatedServicesInfo = map (updateExpandService ticketID) state.data.servicesInfo
...
eval (IncrementTicket ticketOption) state = do
  let
    { finalAmount, updateServicesInfo } = calculateTicketAmount ticketOption true state.data.servicesInfo
  continue state { data { totalAmount = finalAmount, servicesInfo = updateServicesInfo } }

eval (DecrementTicket ticketOption) state = do
  let
    { finalAmount, updateServicesInfo } = calculateTicketAmount ticketOption false state.data.servicesInfo
  continue state { data { totalAmount = finalAmount, servicesInfo = updateServicesInfo } }
...
eval (DatePicker _ resp year month date) state = do
  case resp of
    "SELECTED" -> do
      let
        validDate = (unsafePerformEffect $ runEffectFn2 compareDate ((show date) <> "/" <> (show (month + 1)) <> "/" <> (show year)) (getCurrentDate ""))
...
eval (UpdatePlacesData placeData Nothing) state = do
  let
    newState = state { data { placeInfo = placeData }, props { showShimmer = false } }
  continue newState

eval (UpdatePlacesData placeData (Just (TicketServicesResponse serviceData))) state = do
  let
    servicesInfo = mapWithIndex (\i it -> transformRespToStateData (i == 0) it) serviceData
  let
    newState = state { data { placeInfo = placeData, servicesInfo = servicesInfo }, props { showShimmer = false } }
  continue newState
...
eval BackPressed state = do
  case state.props.currentStage of
    ...
    BookingConfirmationStage ->
      if state.props.previousStage == ViewTicketStage then
        continue state { props { currentStage = state.props.previousStage } }
      else
        exit $ GoToHomeScreen state { props { currentStage = DescriptionStage, showShimmer = true } }
    _ -> continue state
...
eval (GetBookingInfo bookingShortId bookingStatus) state = do
  let
    newState = state { props { selectedBookingId = bookingShortId } }
  updateAndExit newState $ GoToGetBookingInfo newState bookingStatus

eval (ViewTicketAC (PrimaryButton.OnClick)) state = case state.props.paymentStatus of
...
eval (PaymentStatusAction status) state = case status of
...
updateTicketAmount ticketID servicesInfo actualAmount = do
  let
    mbService = head (filter (\service -> service.id == ticketID) servicesInfo)
  case mbService of
    Just service ->
      if service.isExpanded then
        actualAmount + sum (map calculateAmountPrice service.prices)
      else
        actualAmount - sum (map calculateAmountPrice service.prices)
...
calculateTicketAmount ticketOption isIncrement servicesInfo = do
  let
    updatedServices = map (updateService ticketOption) servicesInfo
  ...
  calculateAmountService service = sum (map (calculateAmountPrice service.isExpanded) service.prices)
