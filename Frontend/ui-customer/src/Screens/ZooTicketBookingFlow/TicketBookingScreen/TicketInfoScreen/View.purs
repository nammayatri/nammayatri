...
ticketHeaderView state push placeColor infoColor  =
  ...
  ,  linearLayout
     ...
     , height WRAP_CONTENT
     , orientation VERTICAL
     ][ textContentView state.data.selectedBookingInfo.ticketPlaceName placeColor (MarginBottom 0) (FontStyle.body3 TypoGraphy)
      , linearLayout
        [ width WRAP_CONTENT
        , height WRAP_CONTENT
        , orientation HORIZONTAL
...
pillView state push backgroudColor textColor =
  ...
  , padding $ Padding 10 10 10 10
     ...
     ] <> ( maybe [] getFormattedSlot activeItem.slot )
  ]
   where
...
individualPill state push backgroudColor textColor title desc =
  ...
  , margin $ MarginLeft 10
      ...
      ][  textContentView (title <> " " <> desc) textColor (MarginBottom 0) (FontStyle.subHeading1 TypoGraphy)]
  ]
