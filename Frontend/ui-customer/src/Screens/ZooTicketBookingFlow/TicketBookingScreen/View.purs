...
screen initialState =
  ...
  , eval:
      \action state -> do
        let
          _ = spy "ZooTicketBookingFlow action " action
        let
          _ = spy "ZooTicketBookingFlow state " state
        eval action state
  }
  where
  ...
  getPlaceDataEvent' push = do
    if (any (_ == initialState.props.currentStage) [ ST.DescriptionStage, ST.ViewTicketStage ]) then do
      ...
      let
        mFirstPlace = head placesResp -- TODO:: Remove this once Screen for choosing place is ready
      case mFirstPlace of
...
view push state =
  ...
  where
  actionsView state push = case state.props.currentStage of
  ...
  headerView state push = case state.props.currentStage of
    ST.BookingConfirmationStage -> linearLayout [ visibility GONE ] []
    _ -> GenericHeader.view (push <<< GenericHeaderAC) (genericHeaderConfig state)

  mainView state push =
    if (state.props.currentStage == ST.DescriptionStage) then case state.data.placeInfo of
    ...
    else if (state.props.currentStage == ST.ChooseTicketStage) then
      [ chooseTicketsView state push ]
    else if (state.props.currentStage == ST.BookingConfirmationStage) then
      [ bookingStatusView state push state.props.paymentStatus ]
    else if (state.props.currentStage == ST.ViewTicketStage) then
      [ ticketsListView state push ]
    else if (state.props.currentStage == ST.TicketInfoStage) then
...
convertServicesDataToViewData services = do
  ...
  createFeeSubtext price = (priceInfoMap price.attendeeType) <> ": ₹" <> (show price.pricePerUnit)
...
zooTicketView state push =
  let
    activeItem = state.props.activeListItem
  in
    PrestoAnim.animationSet
      [ translateInXForwardAnim true, translateInXForwardAnim true ]
      $ linearLayout
...
bookingConfirmationActions state push paymentStatus =
  ...
  where
  primaryButtonText = case paymentStatus of
...
paymentStatusHeader state push paymentStatus =
  let
    transcationConfig = getTransactionConfig paymentStatus
  in
    linearLayout
      [ width MATCH_PARENT
      , height WRAP_CONTENT
