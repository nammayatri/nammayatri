        ...
        case rideListItem of
          ...
          Just (RideAPIEntity _) ->
            if isJust otpCode then do
              setValueToLocalStore TRACKING_ENABLED "True"
              modifyScreenState $ HomeScreenStateType (\homeScreen → homeScreen { props { isSpecialZone = true, isInApp = true } })
            else
              pure unit
        ...
        case rideBookingListResponse of
          Right (RideBookingListRes listResp) -> do
            let
              ...
              lastRideDate =
                ( case currRideListItem.rideStartTime of
            ...
            if (lastRideDate /= currentDate) then do
              ...
              pure unit
            when (isNothing currRideListItem.rideRating)
              $ do
                  when (resp.status /= "CANCELLED" && length listResp.list > 0)
                    $ do
                        modifyScreenState
                          $ HomeScreenStateType
                              ( \homeScreen →
                                      ...
                                      { rideRatingState
                                        ...
                                        , rideStartTime =
                                          case currRideListItem.rideStartTime of
                                            ...
                                            Nothing -> ""
                                        , rideEndTime =
                                          case currRideListItem.rideEndTime of
...
getFlowStatusData dummy = case runExcept (decodeJSON (getValueToLocalStore FLOW_STATUS_DATA) :: _ FlowStatusData) of
  Right res -> Just res
  Left err -> Nothing
