...
import Services.API (Prediction, SavedReqLocationAPIEntity(..), GateInfoFull(..), MetroBookingConfigRes, FrfsQuote(..), FrfsGetRouteResp(..))
...
getMetroConfigFromCity :: City -> Maybe Boolean ->String -> CityMetroConfig
getMetroConfigFromCity city isEventOngoing vehicleType =
  case city of
    ...
    Chennai -> mkCityBasedConfig (if vehicleType == "BUS" then "ny_ic_kolkata_bus" else  "ny_ic_chennai_metro") (if vehicleType == "BUS" then "Tickets for Chennai Bus" else getString TICKETS_FOR_CHENNAI_METRO) "ny_ic_chennai_metro_map" "ny_ic_chennai_metro_banner" "#D8E2FF" Color.metroBlue "https://metro-terms.triffy.in/chennai/index.html" (if vehicleType == "BUS" then ["Cancellation of tickets is not applicable" , "The ticket is valid for only 30 minutes from the time of booking" ,"Fare is commission-free and determined by the MTC"] else [getString CHENNAI_METRO_TERM_2 , if isEventOngoing == Just true then getString CHENNAI_METRO_TERM_EVENT else getString CHENNAI_METRO_TERM_1, if isEventOngoing == Just true then getString FREE_TICKET_CASHBACK else ""  ]) (getString $ CHENNAI_METRO_TIME "04:30:00" "22:30:00") false
    Kolkata -> mkCityBasedConfig "ny_ic_kolkata_bus" "Tickets for kolkata Bus" "" "" "" "" "" ["Cancellation of tickets is not applicable" , "The ticket is valid for only 30 minutes from the time of booking" ,"Fare is commission-free and determined by the MTC"] "" false 
...
getFirstRoute :: FrfsQuote -> Maybe FrfsGetRouteResp
