...
addOrUpdateSuggestedDestination sourceGeohash destination suggestionsMap config =
  if member sourceGeohash suggestionsMap then
    update updateSuggestions sourceGeohash suggestionsMap
  else
    insertSuggestionInMap
      sourceGeohash
  ...
  updateDestination destinations =
    let
      ...
      updateExisting existingDestination =
        if destination.placeId == existingDestination.placeId then
          existingDestination
    ...
    in
      if destinationExists then
        ...
        (take (config.locationsToBeStored - 1) sortedDestinations)
          <> ( singleton
                destination
                  { recencyDate = (Just $ (getCurrentUTC ""))
                  , frequencyCount = Just 1
  ...
  updateTrip trips =
    let
      ...
      updateExisting existingTrip =
        if (getDistanceBwCordinates trip.sourceLat trip.sourceLong existingTrip.sourceLat existingTrip.sourceLong) < config.tripDistanceThreshold
          && (getDistanceBwCordinates trip.destLat trip.destLong existingTrip.destLat existingTrip.destLong)
          < config.tripDistanceThreshold then
          existingTrip
      ...
      tripExists =
        any
          ( \tripItem ->
              (getDistanceBwCordinates tripItem.sourceLat tripItem.sourceLong trip.sourceLat trip.sourceLong) < config.tripDistanceThreshold
                && (getDistanceBwCordinates tripItem.destLat tripItem.destLong trip.destLat trip.destLong)
                < config.tripDistanceThreshold
          )
          trips
    ...
    in
      if tripExists then
        ...
        (take (config.tripsToBeStored - 1) sortedTrips)
          <> ( singleton
                trip
                  { recencyDate = Just $ getCurrentUTC ""
                  , frequencyCount = Just 1
...
insertSuggestionInMap sourceGeohash suggestionItem suggestionsMap geohashLimit =
  let
    hashList = getValueToLocalStore SOURCE_GEOHASHES
  ...
  in
    insert sourceGeohash suggestionItem updatedMap
...
getSuggestionsMapFromLocal lazycheck = either (\err -> empty) (\val -> val) (fetchSuggestionsFromLocal (show SUGGESTIONS_MAP))
