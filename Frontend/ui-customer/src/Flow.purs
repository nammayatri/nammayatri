...
issueReportChatScreenFlow = do
  ...
  case flow of
    SELECT_ISSUE_OPTION updatedState -> do
      ...
      modifyScreenState $ ReportIssueChatScreenStateType (\_ -> updatedState { data {chats = (updatedState.data.chats <> chats'), options = getOptionsRes', chatConfig = updatedState.data.chatConfig{messages = (updatedState.data.chatConfig.messages <> messages')} }, props {isResolved = isResolved, showSubmitComp = (not isResolved && showSubmitComp)}})
      issueReportChatScreenFlow
