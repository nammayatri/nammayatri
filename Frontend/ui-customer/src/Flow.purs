...
homeScreenFlow = do
  ...
  case flow of
    ...
    SEARCH_LOCATION input state -> do
      ...
      (SearchLocationResp searchLocationResp) <- Remote.searchLocationBT (Remote.makeSearchLocationReq input state.props.sourceLat state.props.sourceLong cityConfig.geoCodeConfig.radius (EHC.getMapsLanguageFormat $ getLanguageLocale languageKey) "" cityConfig.geoCodeConfig.strictBounds)
      let event =
            case state.props.isSource of
...
addNewAddressScreenFlow input = do
  ...
  case flow of
    SEARCH_ADDRESS input state -> do
      (GlobalState newState) <- getState
      let cityConfig = getCityConfig state.data.config.cityConfig (getValueToLocalStore CUSTOMER_LOCATION)
      (SearchLocationResp searchLocationResp) <- Remote.searchLocationBT (Remote.makeSearchLocationReq input newState.homeScreen.props.sourceLat newState.homeScreen.props.sourceLong cityConfig.geoCodeConfig.radius (EHC.getMapsLanguageFormat (getLanguageLocale languageKey) ) "" cityConfig.geoCodeConfig.strictBounds)
