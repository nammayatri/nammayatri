  ...
  case flow of
    ...
    RETRY_FINDING_QUOTES showLoader estimateId -> do
      ...
      if withinTimeRange findingQuotesTime currentTime "22:00:00" || withinTimeRange findingQuotesTime currentTime "05:00:00" then do
        ...
        response <- lift $ lift $ Remote.selectEstimate (Remote.makeEstimateSelectReq (flowWithoutOffers WithoutOffers) (if state.props.customerTip.enableTips && state.props.customerTip.isTipSelected && state.props.customerTip.tipForDriver > 0 then Just state.props.customerTip.tipForDriver else Nothing) state.data.otherSelectedEstimates (state.data.config.isAdvancedBookingEnabled) state.data.deliveryDetailsInfo (Just state.data.selectedEstimatesObject.hasPetCharges)) (state.props.estimateId)
        case response of
          Right res -> do
    ...
    SELECT_ESTIMATE_AND_QUOTES state -> do
          ...
          case selectedEstimateOrQuote.searchResultType of
            ChooseVehicle.ESTIMATES -> do
              ...
              if valid then do
                void $ Remote.selectEstimateBT (Remote.makeEstimateSelectReq (flowWithoutOffers WithoutOffers) (if state.props.customerTip.enableTips && state.props.customerTip.isTipSelected && state.props.customerTip.tipForDriver > 0 then Just state.props.customerTip.tipForDriver else Nothing) state.data.otherSelectedEstimates (state.data.config.isAdvancedBookingEnabled) state.data.deliveryDetailsInfo (Just selectedEstimateOrQuote.hasPetCharges)) selectedEstimateOrQuote.id
                void $ pure $ setValueToLocalStore FINDING_QUOTES_START_TIME (getCurrentUTC "LazyCheck")
