...
checkVersion versioncodeAndroid versionName = do
  ...
  if os /= "IOS" && versioncodeAndroid < (getLatestAndroidVersion (getMerchant FunctionCall)) then do
    ...
    if any (_ == -1) [ majorUpdateIndex, minorUpdateIndex, patchUpdateIndex ] then
      pure unit
  ...
  case rideBookingListResponse of
    Right (RideBookingListRes listResp) -> do
      if not (null listResp.list) then do
        ...
        let
          (RideBookingRes resp) = (fromMaybe dummyRideBooking (listResp.list !! 0))
          ...
          newState =
            state
              { data
                { driverInfoCardState = getDriverInfo (RideBookingRes resp) state.props.isSpecialZone
        ...
        case rideBookingListResponse of
          Right (RideBookingListRes listResp) -> do
            ...
            let
              (RideBookingDetails contents) = bookingDetails.contents
            let
              (RideAPIEntity currRideListItem) = (fromMaybe dummyRideAPIEntity (resp.rideList !! 0))
...
currentFlowStatus = do
  ...
  if (not internetCondition) then
    permissionScreenFlow "INTERNET_ACTION"
  else if (permissionConditionA && os == "IOS") then
    pure unit
  else if (not (permissionConditionA && permissionConditionB)) then
    permissionScreenFlow "LOCATION_DISABLED"
  else
    pure unit
  ...
  verifyProfile dummy = do
    ...
    let
      dbBundleVersion = response.bundleVersion
    ...
    if isJust response.language then do
      when (getKeyByLanguage (fromMaybe "ENGLISH" response.language) /= (getValueToLocalNativeStore LANGUAGE_KEY))
        $ do
  ...
  goToFindingQuotesStage estimateId driverOfferedQuote = do
    if any (_ == (getValueToLocalStore FINDING_QUOTES_START_TIME)) [ "__failed", "" ] then do
      ...
      let
        secondsPassed = spy "secondsPassed" (getExpiryTime (getValueToLocalStore FINDING_QUOTES_START_TIME) true)
      let
        searchExpiryTime = getSearchExpiryTime "LazyCheck"
      let
        secondsLeft = case driverOfferedQuote of
  ...
  case flow of
    ...
    GO_TO_FIND_ESTIMATES state -> do
      ...
      case rideSearchRes.routeInfo of
        Just (Route response) -> do
          let
            distance = if response.distance < 1000 then toString (response.distance) <> " m" else parseFloat (INT.toNumber (response.distance) / 1000.0) 2 <> " km"
      ...
      void $ pure
        $ setFlowStatusData
            ( FlowStatusData
                { source: { lat: state.props.sourceLat, lng: state.props.sourceLong, place: state.data.source }
    ...
    LOCATION_SELECTED item addToRecents -> do
      ...
      if (not srcServiceable && (updateScreenState.props.sourceLat /= -0.1 && updateScreenState.props.sourceLong /= -0.1)) then do
        ...
        modifyScreenState $ HomeScreenStateType (\homeScreen -> updateScreenState { props { isRideServiceable = true, isSrcServiceable = true, isDestServiceable = true } })
      rideSearchFlow "NORMAL_FLOW"
    SEARCH_LOCATION input state -> do
      (SearchLocationResp searchLocationResp) <-
        Remote.searchLocationBT
          ( Remote.makeSearchLocationReq input (state.props.sourceLat) (state.props.sourceLong) getSearchRadius
    ...
    CONFIRM_RIDE state -> do
      ...
      let
        selectedQuote = if state.props.isSpecialZone then state.data.specialZoneSelectedQuote else state.props.selectedQuote
    ...
    FCM_NOTIFICATION notification state -> do
      ...
      case notification of
        "TRIP_STARTED" -> do -- OTP ENTERED
          ...
          _ <- pure $ clearWaitingTimer <$> state.props.waitingTimeTimerIds
          let
            newState = state { data { route = Nothing }, props { waitingTimeTimerIds = [], currentStage = RideStarted, forFirst = true, showShareAppPopUp = (INT.round $ (fromMaybe 0.0 (fromString (getValueToLocalStore SHARE_APP_COUNT)))) `mod` 4 == 0 } }
        ...
        "TRIP_FINISHED" -> do -- TRIP FINISHED
          ...
          if (state.props.bookingId /= "") then do
            ...
            let
              finalAmount = INT.round $ fromMaybe 0.0 (fromString (getFinalAmount (RideBookingRes resp)))
            let
        ...
        if state.homeScreen.props.sourceLat /= 0.0 && state.homeScreen.props.sourceLong /= 0.0 then do
          ...
          if (sourceServiceabilityResp.serviceable) then do
            if (isJust sourceServiceabilityResp.geoJson && (getMerchant FunctionCall) == JATRISAATHI) then do
              let
                (SpecialLocation specialLocation) = (fromMaybe (HomeScreenData.specialLocation) sourceServiceabilityResp.specialLocation)
              lift $ lift $ doAff do liftEffect $ drawPolygon (fromMaybe "" sourceServiceabilityResp.geoJson) (specialLocation.locationName)
            else
              lift $ lift $ doAff do liftEffect $ removeLabelFromMarker unit
    ...
    UPDATE_LOCATION_NAME state lat lon -> do
      (GetPlaceNameResp locationName) <-
        Remote.placeNameBT
          ( Remote.makePlaceNameReq lat lon
              ( case (getValueToLocalStore LANGUAGE_KEY) of
                  "HI_IN" -> "HINDI"
                  "KN_IN" -> "KANNADA"
                  "BN_IN" -> "BENGALI"
      ...
      let
        (PlaceName placeDetails) = fromMaybe HomeScreenData.dummyLocationName (locationName !! 0)
      modifyScreenState
        $ HomeScreenStateType
            ( \homeScreen ->
                homeScreen
    ...
    UPDATE_PICKUP_NAME state lat lon -> do
      ...
      let
        (PlaceName address) = (fromMaybe HomeScreenData.dummyLocationName (locationName !! 0))
      modifyScreenState
        $ HomeScreenStateType
            ( \homeScreen ->
                homeScreen
    ...
    UPDATE_SAVED_LOCATION -> do
      ...
      case savedLocationResp of
        Right (SavedLocationsListRes listResp) -> do
          ...
          let
            savedLocationWithHomeOrWorkTag = (filter (\listItem -> (listItem.prefixImageUrl == "ny_ic_home_blue,https://assets.juspay.in/nammayatri/images/user/ny_ic_home_blue.png" || (listItem.prefixImageUrl == "ny_ic_work_blue,https://assets.juspay.in/nammayatri/images/user/ny_ic_work_blue.png"))) (AddNewAddress.getSavedLocations listResp.list))
            ...
            updatedList =
              ( map
                  ( \item ->
                      item
                        { postfixImageUrl =
                          if not (checkPrediction item savedLocationsWithOtherTag) then
                            "ny_ic_fav_red,https://assets.juspay.in/nammayatri/images/user/ny_ic_fav_red.png"
                          else
                            "ny_ic_fav,https://assets.juspay.in/nammayatri/images/user/ny_ic_fav.png"
                        }
                  )
                  (recents)
              )
    ...
    GO_TO_INVOICE_ updatedState -> do
      let
        prevRideState = updatedState.data.previousRideRatingState
      let
    ...
    GO_TO_FETCH_CONTACTS state -> do
      ...
      let
        newContacts = transformContactList contactsInJson
...
rideSearchFlow flowType = do
  ...
  if (finalState.props.sourceLat /= 0.0 && finalState.props.sourceLong /= 0.0) && (finalState.props.destinationLat /= 0.0 && finalState.props.destinationLong /= 0.0) && (finalState.data.source /= "") && (finalState.data.destination /= "") then do
    case ((not finalState.props.isSpecialZone) && finalState.props.sourceSelectedOnMap) of
      ...
      true -> do
        ...
        void $ pure
          $ setFlowStatusData
              ( FlowStatusData
                  { source: { lat: finalState.props.sourceLat, lng: finalState.props.sourceLong, place: finalState.data.source }
    ...
    modifyScreenState $ HomeScreenStateType (\homeScreen -> homeScreen { props { isSource = Just false, isRideServiceable = true, isSrcServiceable = true, isDestServiceable = true, currentStage = SearchLocationModel } })
  homeScreenFlow
...
getFinalAmount (RideBookingRes resp) =
  ...
  in
    (show (fromMaybe 0 ride.computedPrice))
  ...
  case flow of
    ...
    POST_CONTACTS state -> do
      ...
      if state.props.showInfoPopUp then
        pure $ toast $ getString CONTACT_REMOVED_SUCCESSFULLY
      else
        pure $ toast $ getString EMERGENCY_CONTACS_ADDED_SUCCESSFULLY
      modifyScreenState $ EmergencyContactsScreenStateType (\emergencyContactsScreen -> state { props { showInfoPopUp = false } })
      (GlobalState globalState) <- getState
      if globalState.homeScreen.props.emergencyHelpModelState.isSelectEmergencyContact then do
...
permissionScreenFlow triggertype = do
  ...
  case flow of
    REFRESH_INTERNET -> do
      if (os == "IOS") then
        pure unit
      else if not internetCondition then
        permissionScreenFlow "INTERNET_ACTION"
      else
        currentFlowStatus
    ...
    UPDATE_USER_PROFILE state -> do
      ...
      case resp of
        ...
        Left err -> do
          ...
          let
            codeMessage = decodeErrorCode errResponse.errorMessage
          case codeMessage of
      ...
      let
        recents =
          map
            ...
            (differenceOfLocationLists (resp.predictionArray) ((AddNewAddress.getSavedLocations savedLocationResp.list)))
...
addNewAddressScreenFlow input = do
  ...
  case flow of
    SEARCH_ADDRESS input state -> do
      (SearchLocationResp searchLocationResp) <-
        Remote.searchLocationBT
          ( Remote.makeSearchLocationReq input (newState.homeScreen.props.sourceLat) (newState.homeScreen.props.sourceLong) getSearchRadius
              ( case (getValueToLocalStore LANGUAGE_KEY) of
                  "HI_IN" -> "HINDI"
                  "KN_IN" -> "KANNADA"
                  "BN_IN" -> "BENGALI"
    ...
    ADD_LOCATION state -> do
      ...
      if state.props.editSavedLocation then
        pure $ toast (getString FAVOURITE_UPDATED_SUCCESSFULLY)
      else
        pure $ toast (getString FAVOURITE_ADDED_SUCCESSFULLY)
      _ <- pure $ setValueToLocalStore RELOAD_SAVED_LOCATION "true"
      _ <- lift $ lift $ liftFlow $ reallocateMapFragment (getNewIDWithTag "CustomerHomeScreenMap")
...
drawDottedRoute state = do
  _ <- pure $ removeAllPolylines ""
  let
    destMarker = if state.props.currentStage == RideAccepted then "src_marker" else "dest_marker"
...
checkAndUpdateSavedLocations state = do
  if (getValueToLocalStore RELOAD_SAVED_LOCATION == "true") || (state.props.currentStage == HomeScreen) then do
    ...
    case savedLocationResp of
      Right (SavedLocationsListRes listResp) -> do
        let
          savedLocationWithHomeOrWorkTag = (filter (\listItem -> (listItem.prefixImageUrl == "ny_ic_home_blue,https://assets.juspay.in/nammayatri/images/user/ny_ic_home_blue.png" || (listItem.prefixImageUrl == "ny_ic_work_blue,https://assets.juspay.in/nammayatri/images/user/ny_ic_work_blue.png"))) (AddNewAddress.getSavedLocations listResp.list))
        let
          recent = (differenceOfLocationLists recentPredictionsObject.predictionArray savedLocationWithHomeOrWorkTag)
        let
          twoElements = catMaybes ([] <> [ recent !! 0 ] <> [ recent !! 1 ])
                  ...
                  homeScreen
                    { data
                      { recentSearchs
                        { predictionArray =
                          map
                            (\item -> item { postfixImageUrl = if not (checkPrediction item (AddNewAddress.getSavedLocations listResp.list)) then "ny_ic_fav_red,https://assets.juspay.in/nammayatri/images/user/ny_ic_fav_red.png" else "ny_ic_fav,https://assets.juspay.in/nammayatri/images/user/ny_ic_fav.png" })
                            twoElements
                        }
                      , savedLocations = (AddNewAddress.getSavedLocations listResp.list)
                      , locationList =
