            ...
            if not isBusQuoteSelected then do
              ...
              case response of
                Right (ConfirmRes resp) -> do
                  ...
                  homeScreenFlow
                Left err  ->
                  handleQuoteError state err
            else do
              _ <- pure $ spy "selected Quote " selectedQuote
              response  <- lift $ lift $ Remote.busConfirm (fromMaybe "" selectedQuote) quantity
              case response of
