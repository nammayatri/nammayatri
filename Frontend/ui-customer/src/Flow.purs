...
currentFlowStatus = do
  ...
  updateUserLanguage language =
    when (isNothing language || (getKeyByLanguage (fromMaybe "ENGLISH" language) /= (getValueToLocalNativeStore LANGUAGE_KEY)))
      ...
      $ lift
      $ lift
      $ Remote.updateProfile (Remote.mkUpdateProfileRequest FunctionCall)
  ...
  case flow of
    ...
    GO_TO_FIND_ESTIMATES updatedState -> do
      ...
      void $ liftFlowBT
        $ setFlowStatusData
            ( FlowStatusData
                { source: { lat: state.props.sourceLat, lng: state.props.sourceLong, place: state.data.source, address: Nothing, city: state.props.city }
    ...
    RETRY_FINDING_QUOTES showLoader -> do
      ...
      if withinTimeRange findingQuotesTime currentTime "22:00:00" || withinTimeRange findingQuotesTime currentTime "05:00:00" then do
        ...
        let
          tipViewData = if state.props.customerTip.isTipSelected then state.props.tipViewProps else HomeScreenData.initData.props.tipViewProps
    ...
    LOCATION_SELECTED item addToRecents -> do
      ...
      case state.props.sourceSelectedOnMap of
        true
          | state.props.isSource == Just true -> pure unit
        _ -> case state.props.isSource of
          Just true -> do
      ...
      let
        bothLocationChangedState = updatedState.homeScreen
      ...
      if (not srcServiceable && (updateScreenState.props.sourceLat /= -0.1 && updateScreenState.props.sourceLong /= -0.1) && (updateScreenState.props.sourceLat /= 0.0 && updateScreenState.props.sourceLong /= 0.0)) then do
        let
          recentList = recentDistance updateScreenState.data.recentSearchs.predictionArray updateScreenState.props.sourceLat updateScreenState.props.sourceLong
        ...
        $ HomeScreenStateType
            ( \homeScreen ->
                state
                  ...
                  , props { searchLocationModelProps { isAutoComplete = true, showLoader = false, findPlaceIllustration = false } }
                  }
            )
      homeScreenFlow
    GET_QUOTES state -> do
    ...
    CONFIRM_RIDE state -> do
      ...
      let
        selectedQuote = if state.props.isSpecialZone && state.data.currentSearchResultType == QUOTES then state.data.specialZoneSelectedQuote else state.props.selectedQuote
    ...
    CANCEL_RIDE_REQUEST state -> do
      ...
      liftFlowBT $ logEventWithMultipleParams logField_ "ny_user_rider_cancellation"
        $ [ { key: "Reason code", value: unsafeToForeign state.props.cancelReasonCode }
        ...
        "TRIP_STARTED" -> do -- OTP ENTERED
          ...
          when (homeScreenState.props.currentStage == RideStarted)
            $ do
                ...
                _ <- pure $ clearWaitingTimer <$> state.props.waitingTimeTimerIds
                let
                  newState = homeScreenState { data { route = Nothing }, props { isCancelRide = false, waitingTimeTimerIds = [], showShareAppPopUp = (INT.round $ (fromMaybe 0.0 (fromString (getValueToLocalStore SHARE_APP_COUNT)))) `mod` 4 == 0, showChatNotification = false, cancelSearchCallDriver = false } }
...
rideSearchFlow flowType = do
  ...
  if (finalState.props.sourceLat /= 0.0 && finalState.props.sourceLong /= 0.0) && (finalState.props.destinationLat /= 0.0 && finalState.props.destinationLong /= 0.0) && (finalState.data.source /= "") && (finalState.data.destination /= "") then do
    ...
    case finalState.props.sourceSelectedOnMap of
      ...
      true -> do
        ...
        void $ liftFlowBT
          $ setFlowStatusData
              ( FlowStatusData
                  { source: { lat: finalState.props.sourceLat, lng: finalState.props.sourceLong, place: finalState.data.source, address: Nothing, city: finalState.props.city }
        ...
        case finalState.props.currentStage of
          ...
          _ -> do
            let
              sourceSpecialTagIcon = specialLocationIcons finalState.props.zoneType.sourceTag
    ...
    let
      updatedLocationList = recentDistance finalState.data.destinationSuggestions finalState.props.sourceLat finalState.props.sourceLong
    in
  ...
  case flow of
    UPDATE_LANGUAGE state -> do
      liftFlowBT $ logEventWithMultipleParams logField_ "ny_user_lang_selected"
        $ [ { key: "Previous language", value: unsafeToForeign $ getValueToLocalStore LANGUAGE_KEY }
...
permissionScreenFlow = do
  ...
  case flow of
    ...
    TURN_ON_INTERNET -> case (getValueToLocalStore USER_NAME == "__failed") of
      true -> pure unit
      _ ->
        if os == "IOS" && not permissionConditionB then
          ...
          $ AddNewAddressScreenStateType
              ( \addNewAddressScreen ->
                  addNewAddressScreen
                    ...
                    , props
                      { isSpecialZone = (sourceServiceabilityResp.geoJson) /= Nothing
...
setSuggestionsMapInLocal item lat lon serviceability state = do
  ...
  when (serviceability && lat /= 0.0 && lon /= 0.0)
    $ do
        let
          currentSourceGeohash = runFn3 encodeGeohash currentState.homeScreen.props.sourceLat currentState.homeScreen.props.sourceLong state.data.config.suggestedTripsAndLocationConfig.geohashPrecision
...
fetchAndModifyLocationLists savedLocationResp = do
  ...
  recentPredictionsObject <- lift $ lift $ getObjFromLocal currentState.homeScreen
  let
    { savedLocationsWithOtherTag, recents, suggestionsMap, tripArrWithNeighbors, updateFavIcon } = getHelperLists savedLocationResp recentPredictionsObject currentState.homeScreen
    ...
    let
      ...
      currentGeoHash =
        if state.props.sourceLat /= 0.0 && state.props.sourceLong /= 0.0 then
          runFn3 encodeGeohash state.props.sourceLat state.props.sourceLong suggestionsConfig.geohashPrecision
        else
          runFn3 encodeGeohash (fromMaybe 0.0 $ fromString $ getValueToLocalNativeStore LAST_KNOWN_LAT) (fromMaybe 0.0 $ fromString $ getValueToLocalNativeStore LAST_KNOWN_LON) suggestionsConfig.geohashPrecision
              ...
              item
                { postfixImageUrl =
                  if not (checkPrediction item savedLocationsWithOtherTag) then
                    fetchImage FF_ASSET "ny_ic_fav_red"
                  else
                    fetchImage FF_ASSET "ny_ic_fav"
                }
          )
          sugestedFinalList
    in
      { savedLocationsWithOtherTag, recents, suggestionsMap, tripArrWithNeighbors, updateFavIcon }
...
updateUserInfoToState state = modifyScreenState $ HomeScreenStateType (\homeScreen -> HomeScreenData.initData { data { disability = state.data.disability, settingSideBar { gender = state.data.settingSideBar.gender, email = state.data.settingSideBar.email } }, props { isBanner = state.props.isBanner } })
  ...
  case flow of
    ...
    GET_BOOKING_INFO_SCREEN state bookingStatus -> do
      ...
      if (bookingStatus == Pending) then do
        ...
        let
          ticketBookingDetails = (ticketDetailsTransformer (GetBookingInfoRes resp))
        let
          dummyListItem = { ticketServiceShortId: "", ticketServiceName: "VideoPhotography", amount: 100.0, status: "Pending", verificationCount: 0, expiryDate: Nothing, prices: [ { pricePerUnit: 2.0, numberOfUnits: 3, attendeeType: "Adults" }, { pricePerUnit: 2.0, numberOfUnits: 2, attendeeType: "Mobile" } ] }
