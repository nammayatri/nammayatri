  ...
  where 
    ...
    addStopFlow state = do 
      ...
      if state.data.fromScreen == (Screen.getScreen Screen.HOME_SCREEN) then do 
        ...
        (GlobalState globalState) <- getState 
        let bookingId = globalState.rideScheduledScreen.bookingId 
            isEdit = isJust globalState.rideScheduledScreen.destination 
            destLoc = fromMaybe SearchLocationScreenData.dummyLocationInfo state.data.destLoc
            stopLocation = encodeAddress destLoc.address [] Nothing
        if isEdit then do 
          let req = Remote.makeEditStopReq ( fromMaybe 0.0 (destLoc.lat)) (fromMaybe 0.0 (destLoc.lon)) stopLocation
...
rentalScreenFlow = do
  ...
  case action of
    RentalScreenController.DoRentalSearch state -> do 
      ...
      (SearchRes rideSearchRes) <- Remote.rideSearchBT (Remote.mkRentalSearchReq (fromMaybe 0.0 state.data.pickUpLoc.lat)  (fromMaybe 0.0 state.data.pickUpLoc.lon) 0.0 0.0 (encodeAddress address.formattedAddress [] Nothing) (encodeAddress destAddress.formattedAddress [] Nothing) state.data.startTimeUTC state.data.rentalBookingData.baseDistance state.data.rentalBookingData.baseDuration)
      let _ = spy "Inside DoRentalSearch" rideSearchRes
    ...
    RentalScreenController.OnRentalRideConfirm updatedState -> do
      ...
      case response of 
        Right (ConfirmRes resp) -> do 
          -- void $ pure $ toast resp
          modifyScreenState $ RentalScreenStateType (\_ -> updatedState{data{bookingId = resp.bookingId}})
          modifyScreenState $ RideScheduledScreenStateType (\state -> state{bookingId = resp.bookingId})
          pure unit 
        Left err -> pure unit
      let currentTime = getCurrentUTC "" 
      -- let isNow = spy "cureentTimewjghhjsgdc" $ unsafePerformEffect $ runEffectFn2 compareDate (updatedState.data.startTimeUTC) currentTime
      -- if isNow then homeScreenFlow else rideScheduledFlow
      -- rentalScreenFlow
      rideScheduledFlow
      -- modifyScreenState $ RentalScreenStateType (\_ -> updatedState)
