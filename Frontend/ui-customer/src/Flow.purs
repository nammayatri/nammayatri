...
homeScreenFlow = do
  ...
  case flow of
    ...
    GO_TO_FIND_ESTIMATES updatedState -> do
      ...
      routeResponse <- Remote.drawMapRoute state.props.sourceLat state.props.sourceLong state.props.destinationLat state.props.destinationLong srcMarkerConfig destMarkerConfig "NORMAL" rideSearchRes.routeInfo "pickup" (specialLocationConfig "" "" false getPolylineAnimationConfig false false) 
    ...
    ONGOING_RIDE state -> do
      ...
      if spy "ONGOING_RIDEONGOING_RIDE CURRENT" state.props.currentStage == RideCompleted then
        do
          ...
          void $ Remote.drawMapRoute srcLat srcLon dstLat dstLon srcMarkerConfig destMarkerConfig "DRIVER_LOCATION_UPDATE" Nothing "pickup" (specialLocationConfig sourceSpecialTagIcon destSpecialTagIcon true getPolylineAnimationConfig false false) 
          homeScreenFlow
    ...
    EDIT_THE_LOCATION state -> do 
      res <-  lift $ lift $  Remote.editLocation (state.data.driverInfoCardState.rideId) $ Remote.mkEditLocationReq state.props.editedPickUpLocation.gps state.props.editedPickUpLocation.address (LatLong{ lat : state.data.driverInfoCardState.destinationLat, lon :  state.data.driverInfoCardState.destinationLng }) state.data.driverInfoCardState.destinationAddress
      case res of
        Right _  -> pure unit
        Left err -> do
          let errResp = err.response
              codeMessage = decodeError errResp.errorMessage "errorCode"
              _ = spy "Error Response edit Location " errResp 
              _ = spy "Error Code edit Location" codeMessage
          pure unit
    ...
    EDIT_LOCATION_FLOW finalState -> do 
      ...
      (ServiceabilityRes sourceServiceabilityResp) <- Remote.originServiceabilityBT (Remote.makeServiceabilityReq finalState.props.sourceLat finalState.props.sourceLong)
    ...
    UPDATE_PICKUP_NAME state lat lon -> do
      ...
      setValueToLocalStore CUSTOMER_LOCATION $ show cityName
      checkForSpecialZoneAndHotSpots state (ServiceabilityRes sourceServiceabilityResp) lat lon
      -- if (fromMaybe "" sourceServiceabilityResp.geoJson) /= "" && state.props.currentStage /= EditPickUpLocation && (fromMaybe "" sourceServiceabilityResp.geoJson) /= state.data.polygonCoordinates && pickUpPoints /= state.data.nearByPickUpPoints then do
      ...
      if isMoreThan20Meters then do
        ...
        case fullAddress of
          Just (PlaceName address) -> do
            ...
            modifyScreenState $ HomeScreenStateType (\homeScreen ->
            homeScreen {
              props{editedPickUpLocation {address = encodeAddress address.formattedAddress address.addressComponents Nothing lat lon}},
              data {
                source = address.formattedAddress ,
...
drawDottedRoute state = do
  ...
  let srcMarkerConfig = defaultMarkerConfig{ pointerIcon = "ny_ic_auto_map" }
      ...
      destLat = if state.props.currentStage == RideAccepted then state.data.driverInfoCardState.sourceLat else state.data.driverInfoCardState.destinationLat
      destLng = if state.props.currentStage == RideAccepted then state.data.driverInfoCardState.sourceLng else state.data.driverInfoCardState.destinationLng
  void $ liftFlowBT $ runEffectFn9 drawRoute (Remote.walkCoordinate srcLat srcLng destLat destLng) "DOT" "#323643" false srcMarkerConfig destMarkerConfig 8 "DRIVER_LOCATION_UPDATE" (specialLocationConfig "" "" false getPolylineAnimationConfig false false) 
...
checkForSpecialZoneAndHotSpots state (ServiceabilityRes serviceabilityResp) lat lon = do
  ...
  if not (DS.null geoJson) && not (null pickUpPoints) && state.props.currentStage /= EditPickUpLocation then do
    when (geoJson /= state.data.polygonCoordinates || pickUpPoints /= state.data.nearByPickUpPoints) $ do
      ...
      homeScreenFlow
  else if not (null serviceabilityResp.hotSpotInfo) && canUpdateHotSpots && state.data.config.mapConfig.locateOnMapConfig.hotSpotConfig.enableHotSpot && state.props.currentStage /= EditPickUpLocation then do
