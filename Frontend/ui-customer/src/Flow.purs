...
baseAppFlow gPayload callInitUI = do
  ...
  let hello3 = spy "I am here in" "baseAppFlow"
  -- -- checkVersion
  ...
  let hello2 = spy "I am here in" "baseAppFlow"
  handleDeepLinks (Just gPayload) false
  void $ lift $ lift $ toggleLoader false
  let hello = spy "I am here in" "baseAppFlow"
  _ <- pure $ backToReactNativeApp ""
  void $ lift $ lift $ delay $ Milliseconds 500.0
  liftFlowBT $ terminateUI $ Just "default"
  ...
  case mBGlobalPayload of
    Just globalPayload -> case globalPayload ^. _payload ^. _view_param of
      Just screen -> case screen of
        ...
        "rfrl" -> hideSplashAndCallFlow referralScreenFlow
        "smd" -> do
...
hideSplashAndCallFlow flow = do
  hideLoaderFlow
  flow
  -- setValueToLocalStore BACKPOINT_ENABLED "true"
  -- (App.BackT $ App.BackPoint <$> ( pure unit))
  -- let hello = spy "I am here in hideSplashAndCallFlow" "hideSplashAndCallFlow"
  -- if ((getValueToLocalStore BACKPOINT_ENABLED) == "true") then do
  --   setValueToLocalStore BACKPOINT_ENABLED "false"
  --   flow
  -- else
  --   pure unit
...
enterMobileNumberScreenFlow = do
  config <- getAppConfigFlowBT appConfig
  if config.reactNativeFlowEnabled 
    then pure unit 
  else do
...
homeScreenFlow = do
  config <- CP.getAppConfigFlowBT CP.appConfig
  let hello = spy "Hello world this is config" config
  let _ = spy "This is value" config.reactNativeFlowEnabled 
  if config.reactNativeFlowEnabled 
    then pure unit 
  else do
...
myRidesScreenFlow = do
  -- config <- CP.getAppConfigFlowBT CP.appConfig
  -- let _ = spy "This is the config" config
  logField_ <- lift $ lift $ getLogFields
  (GlobalState globalState) <- getState
