    ...
    verifyProfile dummy = do
      ...
      if isNothing (response ^. _firstName) 
        then do
          ...
          modifyScreenState $ HomeScreenStateType $ \homeScreen → homeScreen{data{ disability = Just {tag : tag, id : "", description: ""}, settingSideBar{name =fromMaybe ""  (response ^. _firstName),gender = response ^. _gender, email = response ^. _email}} , props {isBanner = false, followsRide = fromMaybe false (response ^. _followsRide)}}
...
updateUserInfoToState state =
  ...
  , tripSuggestions = state.data.tripSuggestions, followers = state.data.followers },props { isBanner = state.props.isBanner, followsRide = state.props.followsRide}})             
