...
makeVerifyOTPReq otp defaultId =
  let
    token = getValueToLocalNativeStore FCM_TOKEN
...
makeSearchLocationReq input lat lng radius language components =
  SearchLocationReq
    { "input": input
...
makeEstimateSelectReq isAutoAssigned tipForDriver =
  DEstimateSelect
...
makeFeedBackReq rating rideId feedback =
  FeedbackReq
    { "rating": rating
    , "rideId": rideId
...
mkDisabilityData selectedDisability otherDisabilityDescription =
  Disability
    ...
    , description:
        case selectedDisability.tag of
  ...
  case existingRoute of
    ...
    Nothing -> do
      ...
      let
        ios = (os == "IOS")
      let
        route = ((routeResponse) !! 0)
      callDrawRoute route
  where
  callDrawRoute :: Maybe Route -> FlowBT String (Maybe Route)
  callDrawRoute route = case route of
    Just (Route routes) ->
...
getRouteMarkers variant city trackingType =
  ...
  getMarker variant =
    if variant == "AUTO_RICKSHAW" then
      ...
      "ny_ic_vehicle_nav_on_map"

  mkDestMarker :: String
  mkDestMarker = case trackingType of
...
makeSendIssueReq email bookingId reason description =
  SendIssueReq
    { "contactEmail": email
...
makeServiceabilityReq latitude longitude =
  ServiceabilityReq
    { "location":
        LatLong
          { "lat": latitude
...
makeServiceabilityReqForDest latitude longitude =
...
makeRideFeedBackReq id feedbackList =
  RideFeedbackReq
    { "rideId": id
...
mkBookingTicketReq ticketBookingScreenData =
  ...
  createTicketServiceRequest service = catMaybes $ map createTicketServicePriceRequest service.prices
    where
