...
import Common.RemoteConfig (fetchRemoteConfigString, getCityBasedConfig, getAppBasedConfig, defaultRemoteConfig, defaultAppRemoteConfig, BundleLottieConfig, RemoteAC(..))
...
safetyVideoConfigData city language = do
  let
    config = fetchRemoteConfigString ("safety_videos_" <> language)
...
safetyBannerVideoConfigData city language = do
  let
    config = fetchRemoteConfigString ("safety_banner_videos_" <> language)
...
pickupInstructions placeName gateName language = do
  let
    config = fetchRemoteConfigString ("pickup_instructions_" <> language)
...
getFamousDestinations city = do
  let
    langConfig = fetchRemoteConfigString $ "famous_destinations" <> (getLanguage $ getLanguageLocale languageKey)

    config =
      if not $ DS.null langConfig then
        langConfig
      else
        fetchRemoteConfigString "famous_destinations_en"
...
getEstimatesOrder city = do
  let
    config = fetchRemoteConfigString "estimates_order"
...
getPreferredVariant city = do
  let
    config = fetchRemoteConfigString "preferred_estimate_order"
...
getBookAnyServices city = do
  let
    config = fetchRemoteConfigString "book_any_services"
...
getBookAnySelectedServices city = do
  let
    config = fetchRemoteConfigString "book_any_selected_services"
...
getLocationSuggestionsToExclude city = do
  let
    config = fetchRemoteConfigString "location_suggestions_to_exclude"
...
getEnabledServices city = do
  let
    config = fetchRemoteConfigString "enabled_services"
...
getTipConfigRC city = do
  let
    config = fetchRemoteConfigString "tip_config"
...
getMetroConfig city = do
    let config = fetchRemoteConfigString "metro_configs"
        value = decodeForeignObject (parseJSON config) $ defaultRemoteConfig defaultMetroConfig
    getCityBasedConfig value city
