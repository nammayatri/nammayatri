...
imageData =
  if os == "IOS" then
    { imageUrl: fetchImage FF_ASSET "ny_ic_wallet_rect", height: (V 15), width: (V 15) }
  else
    { imageUrl: fetchImage FF_ASSET "ny_ic_wallet", height: (V 24), width: (V 24) }
...
quoteListView state push =
  scrollView
    ...
    [ linearLayout
        ...
        ( map
            ( \item ->
                QuoteListItem.view (push <<< QuoteListItemActionController) item { appConfig = state.appConfig }
            )
...
continueWithTipButtonConfig state =
  let
