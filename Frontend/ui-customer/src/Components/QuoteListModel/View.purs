...
findingRidesView state push =
  let lottieRawJson = if (state.appConfig.autoVariantEnabled && getValueToLocalStore SELECTED_VARIANT == "AUTO_RICKSHAW") then (getAssetsBaseUrl FunctionCall) <> getAutoLottie state.city else if getValueToLocalStore SELECTED_VARIANT == "BIKE" || getValueToLocalStore SELECTED_VARIANT == "DELIVERY_BIKE" then (getAssetsBaseUrl FunctionCall) <> "lottie/finding_rides_loader_bike.json" else (getAssetsBaseUrl FunctionCall) <> "lottie/finding_rides_loader_without_text_cab.json"
  in
    linearLayout
    [ height MATCH_PARENT
    , width MATCH_PARENT
