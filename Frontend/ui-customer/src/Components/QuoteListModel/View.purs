...
import Screens.Types (Stage(..), QuoteListItemState(..), City(..),VehicleVariant(..),AmbulanceType(..))
import Storage 
...
findingRidesView state push =
  let lottieRawJson = if (state.appConfig.autoVariantEnabled && getValueToLocalStore SELECTED_VARIANT == "AUTO_RICKSHAW") 
                      ...
                      else case fetchVehicleVariant (getValueToLocalStore SELECTED_VARIANT) of 
                         Just (AMBULANCE_ _ ) -> (getAssetsBaseUrl FunctionCall) <> "lottie/finding_rides_loader_ambulance.json" 
