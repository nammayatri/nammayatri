...
import Helpers.Utils (fetchImage, FetchImageFrom(..), getAssetsBaseUrl, getPaymentMethod, getCityFromString, quoteModalVariantImage,fetchVehicleVariant)
...
import Screens.Types (Stage(..), QuoteListItemState(..), City(..),VehicleVariant(..))
import Storage 
...
findingRidesView state push =
  let lottieRawJson = if (state.appConfig.autoVariantEnabled && getValueToLocalStore SELECTED_VARIANT == "AUTO_RICKSHAW") 
                        then (getAssetsBaseUrl FunctionCall) <> getAutoLottie state.city 
                      else if getValueToLocalStore SELECTED_VARIANT == "BIKE" then (getAssetsBaseUrl FunctionCall) <> "lottie/finding_rides_loader_bike.json" 
                      else case  getValueToLocalStore SELECTED_VARIANT of 
                           "AMBULANCE_TAXI" -> (getAssetsBaseUrl FunctionCall) <> "lottie/finding_rides_loader_ambulance.json" 
                           "AMBULANCE_TAXI_OXY" -> (getAssetsBaseUrl FunctionCall) <> "lottie/finding_rides_loader_ambulance.json" 
                           "AMBULANCE_AC_OXY" -> (getAssetsBaseUrl FunctionCall) <> "lottie/finding_rides_loader_ambulance.json" 
                           "AMBULANCE_AC" -> (getAssetsBaseUrl FunctionCall) <> "lottie/finding_rides_loader_ambulance.json" 
                           "AMBULANCE_VENTILATOR" -> (getAssetsBaseUrl FunctionCall) <> "lottie/finding_rides_loader_ambulance.json"
                           _ -> (getAssetsBaseUrl FunctionCall) <> "lottie/finding_rides_loader_without_text_cab.json"
  in
    linearLayout
    [ height MATCH_PARENT
    , width MATCH_PARENT
