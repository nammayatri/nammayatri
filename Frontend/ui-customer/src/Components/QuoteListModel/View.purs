...
imageData =
  if os == "IOS" then
    { imageUrl: "ny_ic_wallet_rect,https://assets.juspay.in/nammayatri/images/user/ny_ic_wallet_rect.png", height: (V 15), width: (V 15) }
  else
    { imageUrl: "ny_ic_wallet,https://assets.juspay.in/nammayatri/images/user/ny_ic_wallet.png", height: (V 24), width: (V 24) }
...
quoteListView state push =
  scrollView
    ...
    [ linearLayout
        ...
        ( map
            ( \item ->
                QuoteListItem.view (push <<< QuoteListItemActionController) item
            )
            state.quoteListModel
        )
    ]
...
getPrice state =
  let
    selectQuoteArray = (filter (\x -> state.selectedQuote == Just x.id) state.quoteListModel)
...
checkVisibility state = case state.selectedQuote, (null state.quoteListModel) of
  Just _, _ -> VISIBLE
  Nothing, true -> VISIBLE
  Nothing, false -> GONE

setText :: QuoteListModelState -> String
setText state = case state.selectedQuote, (null state.quoteListModel) of
