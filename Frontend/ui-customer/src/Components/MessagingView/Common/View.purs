...
messageNotificationView push state =
  ...
  , visibility $ boolToVisibility $ (((any (_ == state.currentStage)) [ RideAccepted, ChatWithDriver]) && state.fareProductType /= FPT.ONE_WAY_SPECIAL_ZONE && state.config.feature.enableChat) && state.config.feature.enableSuggestions && not state.removeNotification
  , cornerRadius 20.0
  ][linearLayout 
    [ height $ WRAP_CONTENT
    , width $ MATCH_PARENT
