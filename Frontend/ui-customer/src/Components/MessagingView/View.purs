   ...
   , horizontalScrollView
     ...
     , disableKeyboardAvoidance true
        ...
        , accessibility DISABLE
            ...
            , accessibilityHint $ "Vehicle Number : " <>  (STR.replaceAll (STR.Pattern "") (STR.Replacement " ")  config.vehicleNo )
            , accessibility ENABLE
          ...
          , linearLayout
            ...
            , cornerRadius 4.0
            , accessibility ENABLE
            , accessibilityHint $ "O T P : " <> (STR.replaceAll (STR.Pattern "") (STR.Replacement " ")  config.otp)
            , margin $ MarginRight 6
            , padding $ Padding 4 2 4 2
            ][ textView $
              [ text $ "OTP "
...
suggestionsView config push =
  ...
  , width MATCH_PARENT
  , afterRender push $ const $ NoAction
  , visibility if (length config.suggestionsList == 0 ) || not config.canSendSuggestion || not config.enableSuggestions then GONE else VISIBLE
  , background Color.white900
  , accessibility DISABLE
...
getChatFooterHeight config = 
  ...
  in spy "getChatFooterHeight" (if height < 125 then (if null config.suggestionsList || not config.canSendSuggestion || not config.enableSuggestions then 80 else 125) else height)