...
getFavourites arrayItem = do
  let
    home = (filter (\x -> (toLower x.tag) == "home") (arrayItem))
  let
    work = (filter (\x -> (toLower x.tag) == "work") (arrayItem))
  let
    otherLocation = (filter (\x -> not ((toLower x.tag) == "home" || (toLower x.tag) == "work")) (arrayItem))
