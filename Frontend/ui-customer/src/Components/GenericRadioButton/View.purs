    ...
    , linearLayout
          ...
          , padding (Padding 16 2 16 2)
            ...
            ((if EHC.os == "ANDROID" then editText else multiLineEditText)
              ...
              ] <> (getFontStyle config.editTextConfig.textStyle TypoGraphy) <> ( if config.showEditText && config.isSelected then [id (EHC.getNewIDWithTag config.editTextConfig.id)] else []))
            ]]
