...
cardView push state =
  ...
  , orientation VERTICAL
  , cornerRadii $ Corners 14.0 true true false false
  , stroke $ "1,"<>Color.grey900
...
sourceAndDestination :: forall w . (Action -> Effect Unit) -> IndividualRideCardState -> PrestoDOM (Effect Unit) w
sourceAndDestination push state =
  linearLayout
  [ height WRAP_CONTENT
  , width MATCH_PARENT
  ...
  , padding $ if state.optionsVisibility then PaddingTop 16 else PaddingHorizontal 16 16
  , orientation VERTICAL
  ][ linearLayout
      [ orientation HORIZONTAL
      , height WRAP_CONTENT
