            ...
            $ [ height $ V 37
              ...
              , afterRender
                  ( \_ -> do
                      _ <-
                        pure
                          $ showKeyboard case state.isSource of
...
searchResultsView state push =
  let
    searchResultText =
      if state.isAutoComplete then
        getString SEARCH_RESULTS
      else if state.isSource == Just false then
        getString SUGGESTED_DESTINATION
      else
        getString PAST_SEARCHES
  in
    scrollView
      [ height WRAP_CONTENT
      , width MATCH_PARENT
...
destBtnData state = [ { text: (getString SELECT_LOCATION_ON_MAP), imageUrl: "ny_ic_locate_on_map,https://assets.juspay.in/nammayatri/images/user/ny_ic_locate_on_map.png", action: SetLocationOnMap, buttonType: "LocateOnMap" } ]
