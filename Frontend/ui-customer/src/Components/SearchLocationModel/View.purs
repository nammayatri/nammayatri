...
import JBridge (getBtnLoader, requestKeyboardShow, getCurrentPosition, firebaseLogEvent, startLottieProcess, lottieAnimationConfig)
import Language.Strings (getString)
import Language.Types (STR(..))
import MerchantConfig.Utils (Merchant(..), getMerchant)
import Prelude ((<>))
import Prelude (Unit, bind, const, map, pure, unit, ($), (&&), (+), (-), (/), (/=), (<<<), (<>), (==), (||), not, discard, not, (>=),(<), void)
...
searchLottieLoader push state =
  lottieAnimationView
  [ height $ if os == "IOS" then V 80 else V 200 
  , width $ if os == "IOS" then V 80 else V 200 
  ...
  , afterRender (\action -> do
    void $ pure $ startLottieProcess lottieAnimationConfig {rawJson = "primary_button_loader", lottieId = (getNewIDWithTag "lottieLoaderAnimProgress"), scaleType="CENTER_CROP", repeat = true, speed = 0.8}
    push action
    ) (const NoAction)
  ]
...
btnData2 :: SearchLocationModelState -> Array { text :: String, imageUrl :: String, action :: Action, buttonType :: String, color :: String }
btnData state =
