    ...
    $ linearLayout
        ...
        , afterRender
            ( \action -> do
                _ <- push action
                _ <-
                  launchAff $ flowRunner defaultGlobalState $ runExceptT $ runBackT $ lift $ lift
                    $ doAff do
                        if (os == "IOS") then
                          liftEffect $ startTimerWithTime (show state.seconds) state.id "1" push CountDown
                        else
                          liftEffect $ countDown state.seconds state.id push CountDown
                pure unit
            )
            (const NoAction)
...
autoAcceptingView state push =
  linearLayout
    ...
    [ frameLayout
        ...
        [ --PrestoAnim.animationSet [translateOutXForwardAnim ((getValueToLocalStore AUTO_SELECTING) == state.id)] $ linearLayout
            [ height MATCH_PARENT
            , width MATCH_PARENT
            , background "#ABC8F4"
            , alpha 0.3
