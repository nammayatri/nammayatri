...
driverDetailsView config uid =
 linearLayout
  ...
  , margin $ Margin 16 (if config.searchType == QUOTES OneWaySpecialZoneAPIDetails || config.isSpecialZone  then 12 else 0) 16 0
  , background Color.white900
  , cornerRadius 8.0
  , visibility $  boolToVisibility $ if config.searchType == QUOTES OneWaySpecialZoneAPIDetails || config.isSpecialZone then config.rideStarted else true
  , gravity BOTTOM
  ][  linearLayout
