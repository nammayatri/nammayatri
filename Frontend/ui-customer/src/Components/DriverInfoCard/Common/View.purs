 ...
 linearLayout
  ...
  , visibility $  boolToVisibility $ if config.fareProductType == FPT.ONE_WAY_SPECIAL_ZONE || config.isOtpRideFlow then config.rideStarted else true
      ...
      , padding $ PaddingRight 16
        ...
        , textView (
          [ text $ spaceSeparatedPascalCase $ config.vehicleColor <> " " <> if config.vehicleModel `Array.elem` ["Unkown", "Unknown"] then HU.getVariantRideType config.vehicleVariant else config.vehicleModel
