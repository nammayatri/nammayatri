...
view push state =
  ...
  , orientation VERTICAL
    ...
    , if state.props.quotesOrEstimates == QUOTES then driverInfoViewSpecialZone push state else driverInfoView push state
    ]
  ...
  , height WRAP_CONTENT
      ...
      , scrollBarY false
          ...
          , stroke $ "1," <> Color.grey900
            ...
            , separator (MarginHorizontal 16 16) (V 1) Color.grey900 (state.props.currentStage == RideStarted && (secondsToHms state.data.eta) /= "" && (state.props.quotesOrEstimates == QUOTES && (state.props.estimatedTime /= "--")))
...
titleAndETA push state =
  ...
  , visibility $ if ((state.props.currentStage /= RideAccepted && (secondsToHms state.data.eta) == "") || (state.props.currentStage == RideStarted && (state.props.estimatedTime == "--"))) then GONE else VISIBLE
      ...
      [ width MATCH_PARENT
      , height WRAP_CONTENT
      , text $ if state.props.currentStage == RideAccepted then getString BOARD_THE_FIRST_TAXI else "ETA: " <> if state.props.quotesOrEstimates == QUOTES then (state.props.estimatedTime) else (secondsToHms state.data.eta)
...
mapOptionsView push state =
  ...
  ][  if state.props.quotesOrEstimates == QUOTES && state.props.currentStage == RideAccepted then navigateView push state  else sosView push state
    , linearLayout
      [ height WRAP_CONTENT
      , weight 1.0
      , clickable false
...
waitTimeView push state =
 ...
 linearLayout
  ...
  , visibility case state.props.quotesOrEstimates == QUOTES of
      true -> VISIBLE
      false -> if state.data.driverArrived then VISIBLE else GONE
  ][ textView
      [ width WRAP_CONTENT
      , height WRAP_CONTENT
      , text $ if state.props.quotesOrEstimates == QUOTES then getString EXPIRES_IN else  getString WAIT_TIME <> ":"
      , textSize FontSize.a_14
    ...
    , textView
      ...
      , afterRender
            ( \action -> do
                if state.props.quotesOrEstimates == QUOTES && (isLocalStageOn RideAccepted) then do
         ...
         , stroke $ "1," <> Color.grey900
          ...
          , linearLayout
            ...
            , stroke $ "1," <> Color.grey900
              ...
              , if state.props.quotesOrEstimates == QUOTES  then headerTextView push state else contactView push state
              , otpAndWaitView push state
              ...
              , (if os == "IOS" then scrollView else linearLayout)
                ...
                , orientation VERTICAL
                ][ if state.props.quotesOrEstimates == QUOTES then destinationView push state else  sourceDistanceView push state
...
driverDetailsView push state =
 linearLayout
  ...
  , width MATCH_PARENT
  , visibility if state.props.quotesOrEstimates == QUOTES then (if state.props.currentStage == RideStarted then VISIBLE else GONE) else VISIBLE
  , gravity BOTTOM
  ][  linearLayout
      [ orientation VERTICAL
      , height MATCH_PARENT
