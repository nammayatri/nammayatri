...
mapOptionsView push state =
  ...
  , gravity CENTER_VERTICAL
  , padding $ PaddingHorizontal 16 16
  ][  if state.data.isSpecialZone && state.props.currentStage == RideAccepted then dummyView push else sosView push state
    , linearLayout
      [ height WRAP_CONTENT
      , weight 1.0
      , clickable false
    ...
    , linearLayout
      ...
      ]([ if state.data.isSpecialZone && state.props.currentStage == RideAccepted then navigateView push state else dummyView push
    ...
    , visibility if (Array.any (_ == state.props.currentStage) [ RideAccepted, ChatWithDriver ]) then VISIBLE else GONE
        ...
        , orientation if length state.data.driverName > 16 then VERTICAL else HORIZONTAL
          ...
          , textView (
            ...
            , visibility GONE  -- TODO: remove this line (temporary fix for yatri sathi)
            ] <> FontStyle.subHeading1 TypoGraphy)
          , textView (
            [ text case false of  -- TODO : text case of (state.data.distance > 1000 && (secondsToHms state.data.eta) /= "") of
                true -> getString AWAY
  ...
  , gravity LEFT
      ...
      , onClick push (const OnNavigate)
        ...
        , imageView
          ...
          , height $ V 20
          , margin (MarginLeft 6)
          , imageWithFallback $ "ny_ic_navigation,https://assets.juspay.in/beckn/jatrisaathi/driver/images/ny_ic_navigation.png"
          ]
      ]
  ]
