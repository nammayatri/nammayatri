  ...
  , visibility if state.props.showChatNotification then VISIBLE else GONE
    ...
    , linearLayout
      ...
      , onClick (\action -> do
                  ...
                  if not state.props.isChatOpened && state.props.chatcallbackInitiated then JB.showAndHideLoader delay (getString LOADING) (getString PLEASE_WAIT_WHILE_IN_PROGRESS) defaultGlobalState
                  else pure unit
                  push action
                ) (const MessageDriver)
      , gravity CENTER_VERTICAL
