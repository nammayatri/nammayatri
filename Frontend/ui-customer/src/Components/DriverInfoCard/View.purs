...
import Helpers.Utils (fetchImage, FetchImageFrom(..), userCommonAssetBaseUrl, getPaymentMethod, secondsToHms, makeNumber, getVariantRideType, getTitleConfig, getCityNameFromCode, getDefaultPixelSize, getDistanceBwCordinates,disableChatForDelivery)
  ...
  where 
    ...
    getTitleText = do
      case state.data.fareProductType of
        ...
        FPT.DELIVERY -> getString OUT_FOR_DELIVERY
...
chatButtonView push state = 
  ...
  where 
    feature = state.data.config.feature
    imageAsset = case feature.enableChat && disableChatForDelivery state.data.fareProductType, state.data.providerType of
