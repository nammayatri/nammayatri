                    ...
                    , orientation VERTICAL
                        ...
                        ( \index item -> do
                            let userCity = DS.toLower $ ST.getValueToLocalStore ST.CUSTOMER_LOCATION
                                ...
                                enableOffer = ratio > 0.0 && not isBookAny
                                finalPrice = if fromPrice /= toPrice && (fromPrice > 0.0) && (toPrice > 0.0) then "₹" <> EHC.parseFloat (fromPrice * ratio) 0 <> " - " <> "₹" <> EHC.parseFloat (toPrice * ratio) 0 else "₹" <> EHC.parseFloat (actualPrice * ratio) 0
                            ChooseVehicle.view (push <<< ChooseVehicleAC config.tipViewProps) (item{selectedEstimateHeight = config.selectedEstimateHeight, price = price, showInfo = true, capacity = capacity, singleVehicle = (length topProviderList == 1), currentEstimateHeight = config.currentEstimateHeight, services = services, activeIndex = config.activeIndex, actualPrice = finalPrice, enableOffer = enableOffer})
                        ) topProviderList)
