...
testOrder = do
  res <-
    runARDUFlow "Test ordering" $
      Q.getNearestDrivers Nothing pickupPoint 5000 org1 False Nothing <&> getIds
...
testInRadius = do
  res <-
    runARDUFlow "Test radius filtration" $
      Q.getNearestDrivers Nothing pickupPoint 800 org1 False Nothing <&> getIds
...
testNotInRadius = do
  res <-
    runARDUFlow "Test outside radius filtration" $
      Q.getNearestDrivers Nothing pickupPoint 10 org1 False Nothing <&> getIds
  res `shouldBe` []
