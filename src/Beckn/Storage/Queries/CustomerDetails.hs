...
import qualified Beckn.Types.Domain.CustomerDetails  as Domain
...
dbTable :: B.DatabaseEntity be DB.BecknDb (B.TableEntity Storage.CustomerDetailsT)
dbTable = DB._customerDetails DB.becknDb

create :: Domain.CustomerDetails -> L.Flow ()
create Domain.CustomerDetails{..} =
  DB.createOne dbTable (Storage.insertExpression Storage.CustomerDetails{..}) >>=
  either DB.throwDBError pure

findCustomerDetailsById :: CustomerDetailsId -> L.Flow (T.DBResult (Maybe Domain.CustomerDetails))
