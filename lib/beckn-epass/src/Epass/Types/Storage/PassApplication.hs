...
module Epass.Types.Storage.PassApplication where
...
data PassApplicationT f = PassApplication
...
instance B.Table PassApplicationT where
  data PrimaryKey PassApplicationT f
    = PassApplicationPrimaryKey
        ( B.C
            f
            PassApplicationId
        )
...
fieldEMod =
  B.setEntityName "pass_application"
    <> B.modifyTableFields
      B.tableModification
