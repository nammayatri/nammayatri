...
getDbTable :: (HasSchemaName m, Functor m) => m (B.DatabaseEntity be DB.AppDb (B.TableEntity SRCR.RideCancellationReasonT))
getDbTable =
  DB.rideCancellationReason . DB.appDb <$> getSchemaName
