...
getDbTable :: (HasSchemaName m, Functor m) => m (B.DatabaseEntity be DB.AppDb (B.TableEntity SBCR.RideBookingCancellationReasonT))
getDbTable =
  DB.bookingCancellationReason . DB.appDb <$> getSchemaName
