...
search url req = do
  ...
  case mGatewaySelector of
    Just "NSDL.BG.1" -> do
      ...
      callAPI' (Just signatureAuthManagerKey) nsdlBaseUrl (API.nsdlSearch req)
        >>= fromEitherM (ExternalAPICallError nsdlBaseUrl)
    Just "JUSPAY.BG.1" ->
      callAPI' (Just signatureAuthManagerKey) url (API.search req)
        >>= fromEitherM (ExternalAPICallError url)
    _ -> throwError GatewaySelectorNotSet

confirm :: BaseUrl -> ConfirmReq -> Flow AckResponse
confirm url req = do
  callAPI' (Just signatureAuthManagerKey) url (API.confirm req)
    >>= fromEitherM (ExternalAPICallError url)

location :: BaseUrl -> Text -> Flow GetLocationRes
location url req = do
  -- TODO: fix authentication
  callAPI' Nothing url (API.location req)
    >>= fromEitherM (ExternalAPICallError url)

track :: BaseUrl -> TrackTripReq -> Flow AckResponse
track url req = do
  callAPI' (Just signatureAuthManagerKey) url (API.trackTrip req)
    >>= fromEitherM (ExternalAPICallError url)

cancel :: BaseUrl -> CancelReq -> Flow AckResponse
cancel url req = do
  callAPI' (Just signatureAuthManagerKey) url (API.cancel req)
    >>= fromEitherM (ExternalAPICallError url)

status :: BaseUrl -> StatusReq -> Flow AckResponse
status url req = do
  callAPI' (Just signatureAuthManagerKey) url (API.status req)
    >>= fromEitherM (ExternalAPICallError url)

feedback :: BaseUrl -> FeedbackReq -> Flow AckResponse
feedback url req = do
  callAPI' (Just signatureAuthManagerKey) url (API.feedback req)
    >>= fromEitherM (ExternalAPICallError url)
