...
verifyTokenAction ::
  forall m r.
  (HasEsqReplica m r, Redis.HedisFlow m r, HasField "authTokenCacheExpiry" r Seconds) =>
...
verifyToken :: (HasEsqReplica m r, MonadThrow m, Log m) => RegToken -> m SR.RegistrationToken
verifyToken regToken = do
  QR.findByToken regToken
    >>= Utils.fromMaybeM (InvalidToken regToken)
    >>= validateToken

validateAdmin :: (HasEsqReplica m r, EncFlow m r) => RegToken -> m Person.Person
...
verifyPerson :: (HasEsqReplica m r, Redis.HedisFlow m r, HasField "authTokenCacheExpiry" r Seconds) => RegToken -> m (Id Person.Person)
...
validateAdminAction :: forall m r. (HasEsqReplica m r, EncFlow m r) => VerificationAction AdminVerifyToken m
validateAdminAction = VerificationAction validateAdmin

validateToken :: (HasEsqReplica m r, MonadThrow m, Log m) => SR.RegistrationToken -> m SR.RegistrationToken
