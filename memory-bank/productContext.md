...
- **Key Fields:**
    ...
    - `targetBppId`: Identifier of the Beckn Provider Platform (BPP) to which automated booking requests for this subscription will be sent.
    - `fixedPrice`: The agreed-upon fixed price for each instance of this subscription. This price is established through an initial interaction with the target BPP (identified by `targetBppId`). During this initial exchange, the BPP provides a comprehensive estimate that includes any conditional charges specific to NYRegular recurring services. This BPP-originated total amount is then stored here.
...
The "fixed price" referenced throughout this automated flow is pre-established for each `NyRegularSubscription`. It originates from an initial comprehensive estimate provided by the target BPP, which includes any conditional charges the BPP applies for regular, scheduled services. This agreed-upon price is stored in the `NyRegularSubscription` record.
