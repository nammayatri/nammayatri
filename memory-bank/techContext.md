...
- **Namma DSL (`alchemist`)**: YAML-based Domain Specific Language primarily used for defining API endpoints, data types (request/response models), and database storage schemas.
    - **Purpose**: To streamline and standardize the creation of backend services by allowing developers to define service contracts and data structures in a concise, declarative format. This promotes consistency across different services.
    - **`alchemist`**: This is likely the core engine or framework that interprets the Namma DSL (YAML files). It parses the DSL definitions and provides the foundation for code generation.
    - **`run-generator`**: This command-line tool is used to trigger the code generation process. It takes the Namma DSL definitions (processed by `alchemist`) as input and automatically generates Haskell source code files. These generated files typically include:
        - Servant API type definitions.
        - Aeson FromJSON/ToJSON instances for request/response data types.
        - Beam table definitions and related database schema code for PostgreSQL.
        - Handler function signatures that developers then implement with business logic.
    - **Benefits**:
        - **Rapid Development**: Automates the creation of boilerplate code for APIs and database interactions, allowing developers to focus on business logic.
        - **Consistency**: Ensures that APIs and data models across different services follow a uniform structure and naming conventions.
        - **Reduced Errors**: Minimizes common errors that can occur when manually writing repetitive code for API definitions and data serialization/deserialization.
        - **Single Source of Truth**: The DSL files serve as a single source of truth for the API contracts and data schemas, making it easier to understand and maintain the services.
        - **Type Safety**: By generating Haskell code, it leverages Haskell's strong type system to catch errors at compile time.
    - **Usage Pattern**: Developers define their service's API endpoints, request/response payloads, and database tables in YAML files following the Namma DSL syntax. They then execute `run-generator`, which reads these YAML files and outputs corresponding Haskell modules. These generated modules are then imported into the main service code, where developers implement the actual logic for the generated API handlers. Files generated by this process are often found in `src-read-only` directories, indicating they should not be manually edited.
