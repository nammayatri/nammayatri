...
instance IsBaseError SuspectFlaggedError where
  toMessage = \case
    ...
    SuspectNotFound dlOrVoterId -> Just $ "Suspect not found with dl or voterId" <> dlOrVoterId <> "."
...
instance IsHTTPError SuspectFlaggedError where
  toErrorCode = \case
    ...
    SuspectNotFound _ -> "SUSPECT_NOT_FOUND"
