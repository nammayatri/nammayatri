...
postChangeSuspectFlag tokenInfo req = do
  ...
  T.withTransactionStoring transaction $ do
    ...
    let receiverList = adminReceiversList <> merchantReceiversList
        notificationMetaData = encodeToText updatedSuspectList
    mapM_
      ( \receiver -> do
    ...
    mapM_
      ( \suspect -> do
          suspectStatusHistory <- DS.buildSuspectStatus tokenInfo merchant.shortId.getShortId (Just Domain.Types.SuspectFlagRequest.Approved) suspect
...
postAdminUploadSuspectBulk tokenInfo mbFlaggedStatus req = do
  ...
  T.withTransactionStoring transaction $ do
    ...
    when (length suspectsNeedToFlag > 0) $ do
      ...
      suspectFlagRequest <- mapM (\suspect -> DS.buildSuspectFlagRequest suspect tokenInfo merchant.shortId.getShortId flaggedBy Domain.Types.SuspectFlagRequest.Approved) suspectsNeedToFlag
      ...
      merchantReceiversList <- QP.findAllByRole merchantAdminRole.id
      let receiverList = adminReceiversList <> merchantReceiversList
          notificationMetaDataFromAdmin = encodeToText suspectList
      ...
      mapM_
        ( \receiver -> do
            notification <- DS.buildNotification tokenInfo.merchantId merchant.shortId.getShortId (length suspectFlagRequest) notificationType notificationMetaDataFromAdmin receiver.id.getId tokenInfo.personId.getId
            SQN.create notification
        )
        receiverList
      merchantConfigs <- SQMC.findByRequestWebHook True
      ...
      mapM_
        ( \flagReq -> do
            suspectStatusHistory <- DS.buildSuspectStatusHistory tokenInfo (Just flaggedBy) (Just Domain.Types.SuspectFlagRequest.Approved) (fromMaybe Domain.Types.Suspect.Flagged mbFlaggedStatus) flagReq
            SQSH.create suspectStatusHistory
        )
        suspectFlagRequest
