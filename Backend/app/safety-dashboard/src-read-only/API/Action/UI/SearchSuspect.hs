...
type API =
  DashboardAuth 'DASHBOARD_USER :> "search" :> "suspectList" :> ReqBody '[JSON] API.Types.UI.SearchSuspect.SearchSuspectReqList :> Post '[JSON] API.Types.UI.SearchSuspect.SuspectsList
    ...
    :<|> DashboardAuth 'MERCHANT_MAKER :> "merchant" :> "check" :> "suspectStatusHistory" :> ReqBody '[JSON] API.Types.UI.SearchSuspect.SearchSuspectReq :> Post '[JSON] API.Types.UI.SearchSuspect.CheckSuspectStatusHistoryResp
    ...
    :<|> DashboardAuth 'DASHBOARD_ADMIN :> "suspect" :> "list" :> QueryParam "dl" (Data.Text.Text) :> QueryParam "flaggedCategory" (Data.Text.Text) :> QueryParam "flaggedStatus" (Domain.Types.Suspect.FlaggedStatus) :> QueryParam "from" (Kernel.Prelude.UTCTime) :> QueryParam "limit" (Kernel.Prelude.Int) :> QueryParam "offset" (Kernel.Prelude.Int) :> QueryParam "partnerName" (Data.Text.Text) :> QueryParam "to" (Kernel.Prelude.UTCTime) :> QueryParam "voterId" (Data.Text.Text) :> Get '[JSON] API.Types.UI.SearchSuspect.SuspectsList
