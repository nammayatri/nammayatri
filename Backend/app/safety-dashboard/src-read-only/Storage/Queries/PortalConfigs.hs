...
updateByPrimaryKey (Domain.Types.PortalConfigs.PortalConfigs {..}) = do {_now <- getCurrentTime;
...
instance FromTType' Beam.PortalConfigs Domain.Types.PortalConfigs.PortalConfigs
    where fromTType' (Beam.PortalConfigsT {..}) = do pure $ Just Domain.Types.PortalConfigs.PortalConfigs{configName = configName,
                                                                                                          createdAt = createdAt,
                                                                                                          id = Kernel.Types.Id.Id id,
                                                                                                          updatedAt = updatedAt,
                                                                                                          value = value}
instance ToTType' Beam.PortalConfigs Domain.Types.PortalConfigs.PortalConfigs
    where toTType' (Domain.Types.PortalConfigs.PortalConfigs {..}) = do Beam.PortalConfigsT{Beam.configName = configName,
