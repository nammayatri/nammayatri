...
updateFlaggedCounterByVoterIdOrDl ::
  (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
  (Kernel.Prelude.Int -> Domain.Types.Suspect.FlaggedStatus -> [Domain.Types.Suspect.FlaggedBy] -> Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Prelude.Maybe Kernel.Prelude.Text -> m ())
updateFlaggedCounterByVoterIdOrDl flaggedCounter flaggedStatus flaggedBy voterId dl = do
  _now <- getCurrentTime
  updateWithKV
    [ Se.Set Beam.flaggedCounter flaggedCounter,
      Se.Set Beam.flaggedStatus flaggedStatus,
    ...
    [Se.Or [Se.Is Beam.voterId $ Se.Eq voterId, Se.Is Beam.dl $ Se.Eq dl]]
