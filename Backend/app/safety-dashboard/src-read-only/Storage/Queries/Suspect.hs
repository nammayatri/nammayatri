...
updateFlaggedCounterByDl :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateFlaggedCounterByDl flaggedCounter flaggedStatus flaggedBy dl = do {_now <- getCurrentTime;
                                                                         updateWithKV [Se.Set Beam.flaggedCounter flaggedCounter,
                                                                                       Se.Set Beam.flaggedStatus flaggedStatus,
                                                                                       Se.Set Beam.updatedAt _now,
                                                                                       Se.Set Beam.flaggedBy (convertFlaggedByToTable flaggedBy)] [Se.Is Beam.dl $ Se.Eq dl]}
updateFlaggedCounterByVoterId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                 (Kernel.Prelude.Int -> Domain.Types.Suspect.FlaggedStatus -> [Domain.Types.Suspect.FlaggedBy] -> Kernel.Prelude.Maybe Kernel.Prelude.Text -> m ())
updateFlaggedCounterByVoterId flaggedCounter flaggedStatus flaggedBy voterId = do {_now <- getCurrentTime;
                                                                                   updateWithKV [Se.Set Beam.flaggedCounter flaggedCounter,
                                                                                                 Se.Set Beam.flaggedStatus flaggedStatus,
                                                                                                 Se.Set Beam.updatedAt _now,
                                                                                                 Se.Set Beam.flaggedBy (convertFlaggedByToTable flaggedBy)] [Se.Is Beam.voterId $ Se.Eq voterId]}
...
updateFlaggedStatusById flaggedStatus id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.Suspect.Suspect {..}) = do {_now <- getCurrentTime;
                                                             updateWithKV [Se.Set Beam.dl dl,
                                                                           ...
                                                                           Se.Set Beam.updatedAt _now,
                                                                           Se.Set Beam.voterId voterId] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]}
