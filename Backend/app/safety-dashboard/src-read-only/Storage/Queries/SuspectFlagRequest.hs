...
findAllByDlAndAdminApprovalAndMerchantId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByDlAndAdminApprovalAndMerchantId dl adminApproval merchantId = do findAllWithKV [Se.And [Se.Is Beam.dl $ Se.Eq dl,
                                                                                                 Se.Is Beam.adminApproval $ Se.Eq adminApproval,
                                                                                                 Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId <$> merchantId)]]
findAllByMerchantIdAndDl :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByMerchantIdAndVoterId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                 (Kernel.Prelude.Maybe (Kernel.Types.Id.Id Domain.Types.Merchant.Merchant) -> Kernel.Prelude.Maybe Kernel.Prelude.Text -> m ([Domain.Types.SuspectFlagRequest.SuspectFlagRequest]))
...
findByDlOrVoterId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByMerchantIdAndAdminApprovalAndDl :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByMerchantIdAndAdminApprovalAndDl merchantId adminApproval dl = do findOneWithKV [Se.And [Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId <$> merchantId),
                                                                                              Se.Is Beam.adminApproval $ Se.Eq adminApproval,
                                                                                              Se.Is Beam.dl $ Se.Eq dl]]
findByMerchantIdAndAdminApprovalAndVoterId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                              (Kernel.Prelude.Maybe (Kernel.Types.Id.Id Domain.Types.Merchant.Merchant) -> Domain.Types.SuspectFlagRequest.AdminApproval -> Kernel.Prelude.Maybe Kernel.Prelude.Text -> m (Maybe Domain.Types.SuspectFlagRequest.SuspectFlagRequest))
findByMerchantIdAndAdminApprovalAndVoterId merchantId adminApproval voterId = do findOneWithKV [Se.And [Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId <$> merchantId),
                                                                                                        Se.Is Beam.adminApproval $ Se.Eq adminApproval,
                                                                                                        Se.Is Beam.voterId $ Se.Eq voterId]]
findByMerchantIdAndDl :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateAdminApprovalById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                           (Domain.Types.SuspectFlagRequest.AdminApproval -> Kernel.Types.Id.Id Domain.Types.SuspectFlagRequest.SuspectFlagRequest -> m ())
updateAdminApprovalById adminApproval id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.SuspectFlagRequest.SuspectFlagRequest {..}) = do {_now <- getCurrentTime;
                                                                                   updateWithKV [Se.Set Beam.adminApproval adminApproval,
