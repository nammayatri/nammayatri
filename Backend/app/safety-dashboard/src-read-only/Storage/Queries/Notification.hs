...
findByMerchantIdAndreadStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByMerchantIdAndreadStatus notificationCategory readStatus merchantId = do findOneWithKV [Se.And [Se.Is Beam.notificationCategory $ Se.Eq notificationCategory,
...
findByReceiverIdAndId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByReceiverIdAndReadStatus limit offset receiverId readStatus = do findAllWithOptionsKV [Se.And [Se.Is Beam.receiverId $ Se.Eq receiverId,
                                                                                                    Se.Is Beam.readStatus $ Se.Eq readStatus]] (Se.Desc Beam.createdAt) limit offset
...
instance FromTType' Beam.Notification Domain.Types.Notification.Notification
    where fromTType' (Beam.NotificationT {..}) = do pure $ Just Domain.Types.Notification.Notification{createdAt = createdAt,
