...
data ManagementAPIs = ManagementAPIs
  { healthCheckDSL :: API.Types.UnifiedDashboard.Management.HealthCheck.HealthCheckAPIs,
    mediaFileDocumentDSL :: API.Types.UnifiedDashboard.Management.MediaFileDocument.MediaFileDocumentAPIs,
    personDSL :: API.Types.UnifiedDashboard.Management.Person.PersonAPIs
  }
...
mkManagementAPIs merchantId city token = do
  ...
  let mediaFileDocumentDSL = API.Types.UnifiedDashboard.Management.MediaFileDocument.mkMediaFileDocumentAPIs mediaFileDocumentClientDSL
  let personDSL = API.Types.UnifiedDashboard.Management.Person.mkPersonAPIs personClientDSL
  (ManagementAPIs {..})
  where
    healthCheckClientDSL :<|> mediaFileDocumentClientDSL :<|> personClientDSL = Tools.Client.clientWithMerchantAndCity (Proxy :: Proxy API.UnifiedDashboard.ManagementDSLAPI) merchantId city token
