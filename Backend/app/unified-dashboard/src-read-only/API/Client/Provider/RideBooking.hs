...
module API.Client.Provider.RideBooking where
...
mkRideBookingAPIs merchantId city token = do {let {healthCheckDSL = API.Types.UnifiedDashboard.RideBooking.HealthCheck.mkHealthCheckAPIs healthCheckClientDSL}; (RideBookingAPIs {..})}
                      where healthCheckClientDSL = Tools.Client.clientWithMerchantAndCity (Proxy :: Proxy API.UnifiedDashboard.RideBookingDSLAPI) merchantId city token
...
callRideBookingAPI merchantId city = Tools.Client.callServerAPI @_ @m @r Domain.Types.AccessMatrix.DRIVER_OFFER_BPP_MANAGEMENT (mkRideBookingAPIs merchantId city) "callRideBookingAPI"
