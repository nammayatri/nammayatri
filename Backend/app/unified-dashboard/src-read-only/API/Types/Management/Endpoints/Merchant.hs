...
data ChangeMerchantEnableStateReq
    = ChangeMerchantEnableStateReq {enable :: Kernel.Prelude.Bool, merchantShortId :: Data.Text.Text}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
data CreateMerchantReq
    = CreateMerchantReq {defaultOperatingCity :: Kernel.Types.Beckn.Context.City,
    ...
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
data CreateMerchantWithAdminReq
    = CreateMerchantWithAdminReq {adminEmail :: Data.Text.Text,
    ...
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
data ListMerchantResp
    = ListMerchantResp {list :: [MerchantAPIEntity], summary :: Kernel.External.Verification.SafetyPortal.Types.Summary}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
data MerchantAPIEntity
    = MerchantAPIEntity {adminList :: [(Text, Data.Text.Text)],
...
data MerchantAPIs
    = MerchantAPIs {createMerchantWithAdmin :: (CreateMerchantWithAdminReq -> EulerHS.Types.EulerClient PersonAPIEntity),
...
mkMerchantAPIs merchantClient = (MerchantAPIs {..})
                   where createMerchantWithAdmin :<|> createMerchant :<|> listMerchants :<|> changeMerchantEnableState = merchantClient
data MerchantUserActionType
    = CREATE_MERCHANT_WITH_ADMIN | CREATE_MERCHANT | LIST_MERCHANTS | CHANGE_MERCHANT_ENABLE_STATE
