...
data PersonE e
    = Person {approvedBy :: Kernel.Prelude.Maybe (Kernel.Types.Id.Id Domain.Types.Person.Person),
...
instance EncryptedItem Person
    where type Unencrypted Person = (DecryptedPerson, HashSalt)
          encryptItem (entity, salt) = do {email_ <- encryptItem $ (, salt) <$> email entity;
                                           mobileNumber_ <- encryptItem $ (, salt) <$> mobileNumber entity;
                                           pure Person{approvedBy = approvedBy entity,
          ...
          decryptItem entity = do {email_ <- fmap fst <$> decryptItem (email entity);
