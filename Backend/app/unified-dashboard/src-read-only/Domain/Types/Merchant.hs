...
data MerchantE e
    = Merchant {authToken :: Kernel.Prelude.Maybe (Kernel.External.Encryption.EncryptedHashedField e Data.Text.Text),
...
instance EncryptedItem Merchant
    where type Unencrypted Merchant = (DecryptedMerchant, HashSalt)
          encryptItem (entity, salt) = do {authToken_ <- encryptItem $ (, salt) <$> authToken entity;
                                           pure Merchant{authToken = authToken_,
          ...
          decryptItem entity = do {authToken_ <- fmap fst <$> decryptItem (authToken entity);
                                   pure (Merchant{authToken = authToken_,
                                                  createdAt = createdAt entity,
                                                  defaultOperatingCity = defaultOperatingCity entity,
                                                  domain = domain entity,
                                         ...
                                         "")}
instance EncryptedItem' Merchant
    where type UnencryptedItem Merchant = DecryptedMerchant
          toUnencrypted a salt = (a, salt)
          fromUnencrypted = fst
data AvailableCitiesForMerchant
    = AvailableCitiesForMerchant {merchantShortId :: Kernel.Types.Id.ShortId Domain.Types.Merchant.Merchant, operatingCity :: [Kernel.Types.Beckn.Context.City]}
