...
updateEnableStatus enabled shortId = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.Merchant.Merchant {..}) = do {_now <- getCurrentTime;
                                                               updateWithKV [Se.Set Beam.authTokenEncrypted (((authToken <&> unEncrypted . (.encrypted)))),
