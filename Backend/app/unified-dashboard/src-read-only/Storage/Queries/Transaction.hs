...
fetchLastTransaction :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
fetchLastTransaction limit offset endpoint serverName = do findAllWithOptionsKV [Se.And [Se.Is Beam.endpoint $ Se.Eq endpoint,
                                                                                         Se.Is Beam.serverName $ Se.Eq serverName]] (Se.Desc Beam.createdAt) limit offset
...
updateByPrimaryKey (Domain.Types.Transaction.Transaction {..}) = do {_now <- getCurrentTime;
                                                                     updateWithKV [Se.Set Beam.commonDriverId commonDriverId,
