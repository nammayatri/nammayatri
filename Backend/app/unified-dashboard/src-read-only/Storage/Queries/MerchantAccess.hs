...
findByPersonIdAndMerchantId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByPersonIdAndMerchantId personId merchantId = do findAllWithKV [Se.And [Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId),
                                                                            Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId merchantId)]]
findByPersonIdAndMerchantIdAndCity :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByPersonIdAndMerchantIdAndCity personId merchantId operatingCity = do findOneWithKV [Se.And [Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId),
                                                                                                 Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId merchantId),
...
updateByPrimaryKey (Domain.Types.MerchantAccess.MerchantAccess {..}) = do {_now <- getCurrentTime;
                                                                           updateWithKV [Se.Set Beam.is2faEnabled is2faEnabled,
...
instance FromTType' Beam.MerchantAccess Domain.Types.MerchantAccess.MerchantAccess
    where fromTType' (Beam.MerchantAccessT {..}) = do pure $ Just Domain.Types.MerchantAccess.MerchantAccess{createdAt = createdAt,
                                                                                                             ...
                                                                                                             secretKey = secretKey,
                                                                                                             updatedAt = updatedAt}
instance ToTType' Beam.MerchantAccess Domain.Types.MerchantAccess.MerchantAccess
    where toTType' (Domain.Types.MerchantAccess.MerchantAccess {..}) = do Beam.MerchantAccessT{Beam.createdAt = createdAt,
