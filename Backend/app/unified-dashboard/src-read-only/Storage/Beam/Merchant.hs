...
data MerchantT f
    = MerchantT {authTokenEncrypted :: (B.C f (Kernel.Prelude.Maybe Data.Text.Text)),
...
instance B.Table MerchantT
    where data PrimaryKey MerchantT f = MerchantId (B.C f Data.Text.Text) deriving (Generic, B.Beamable)
