...
import Environment (AppCfg (..), AppEnv, buildAppEnv, releaseAppEnv)
...
import qualified Tools.Auth.Api as Auth

runService :: (AppCfg -> AppCfg) -> IO ()
runService configModifier = do
  appCfg <- readDhallConfigDefault "unified-dashboard" <&> configModifier
  ...
  where
    ...
    authTokenCacheKeyPrefix = "unified-dashboard:authTokenCacheKey:"
