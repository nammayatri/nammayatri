...
module Domain.Action.Provider.Operator.HealthCheck (getHealthCheckTest) where

import qualified API.Client.Provider.Operator
...
getHealthCheckTest merchantShortId opCity apiTokenInfo = do
  ...
  API.Client.Provider.Operator.callOperatorAPI checkedMerchantId opCity (.healthCheckDSL.getHealthCheckTest)
