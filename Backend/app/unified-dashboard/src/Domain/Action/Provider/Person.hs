...
module Domain.Action.Provider.Person
  ( postPersonCreate,
    postUserLoginSendOtp,
  )
where

import qualified API.Client.Provider
import qualified API.Types.Provider.Person
import qualified "dynamic-offer-driver-app" API.Types.Provider.Person as BPPPerson
  ...
  Domain.Action.Management.Transaction.withTransactionStoring transaction $ do
    ...
    personId <-
      if role.needsBppAccountCreation
        then do
          bppReq <-
            API.Client.Provider.callProviderAPI
              checkedMerchantId
              opCity
              ( \apis ->
