...
module Domain.Action.Provider.Fleet.HealthCheck (getHealthCheckTest) where

import qualified API.Client.Provider.Fleet
...
getHealthCheckTest merchantShortId opCity apiTokenInfo = do
  ...
  API.Client.Provider.Fleet.callFleetAPI checkedMerchantId opCity (.healthCheckDSL.getHealthCheckTest)
