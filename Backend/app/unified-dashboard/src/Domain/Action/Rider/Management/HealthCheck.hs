...
module Domain.Action.Rider.Management.HealthCheck (getHealthCheckTest) where

import qualified API.Client.Rider.Management
...
getHealthCheckTest merchantShortId opCity apiTokenInfo = do
  ...
  API.Client.Rider.Management.callManagementAPI checkedMerchantId opCity (.healthCheckDSL.getHealthCheckTest)
