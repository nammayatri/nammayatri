    ...
    (Just model, Just obj) -> do
      ...
      if shouldPushToDbOnly tableName _dontEnableForKafka || not isPushToKafka'
        then runUpdateQuery updateDataEntries (mbModel, mbObject, mbMappings, mbModelObject) tableName
          ...
          let updateObject = getDbUpdateDataJson model mbModelObject
          res <- EL.runIO $ createInKafka _kafkaConnection updateObject streamName
          case res of
            Left err -> do
...
getDbUpdateDataJson :: Text -> Maybe A.Object -> A.Value
getDbUpdateDataJson model a =
  A.object
    [ "contents" .= a,
      "tag" .= ((T.pack . pascal . T.unpack) (T.take (T.length model - 7) model) <> "Object"),
      "type" .= ("UPDATE" :: Text)
    ]
