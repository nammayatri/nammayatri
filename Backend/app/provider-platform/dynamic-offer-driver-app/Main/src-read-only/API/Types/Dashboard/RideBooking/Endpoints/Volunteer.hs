...
data AssignCreateAndStartOtpRideAPIReq
    = AssignCreateAndStartOtpRideAPIReq {bookingId :: Kernel.Types.Id.Id Dashboard.Common.Booking, driverId :: Kernel.Types.Id.Id Dashboard.Common.Driver, point :: Kernel.External.Maps.Types.LatLong}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
instance Kernel.Types.HideSecrets.HideSecrets AssignCreateAndStartOtpRideAPIReq
    where hideSecrets = Kernel.Prelude.identity
data BookingInfoResponse
    = BookingInfoResponse {bookingId :: Kernel.Types.Id.Id Dashboard.Common.Booking,
...
data VolunteerAPIs
    = VolunteerAPIs {getVolunteerBooking :: (Kernel.Prelude.Text -> EulerHS.Types.EulerClient BookingInfoResponse),
...
mkVolunteerAPIs volunteerClient = (VolunteerAPIs {..})
                    where getVolunteerBooking :<|> postVolunteerAssignStartOtpRide = volunteerClient
data VolunteerUserActionType
    = GET_VOLUNTEER_BOOKING | POST_VOLUNTEER_ASSIGN_START_OTP_RIDE
