...
data BookingWithVehicleAndPhoneReq
    = BookingWithVehicleAndPhoneReq {vehicleNumber :: Kernel.Prelude.Text,
...
instance Kernel.Types.HideSecrets.HideSecrets BookingWithVehicleAndPhoneReq
    where hideSecrets = Kernel.Prelude.identity
newtype BookingWithVehicleAndPhoneRes
  = BookingWithVehicleAndPhoneRes {driverId :: Kernel.Prelude.Text}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
instance Kernel.Types.HideSecrets.HideSecrets BookingWithVehicleAndPhoneRes
    where hideSecrets = Kernel.Prelude.identity
data CancelRideReq
    = CancelRideReq {reasonCode :: Dashboard.Common.Booking.CancellationReasonCode, additionalInfo :: Kernel.Prelude.Maybe Kernel.Prelude.Text}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
instance Kernel.Types.HideSecrets.HideSecrets CancelRideReq
    where hideSecrets = Kernel.Prelude.identity
data EndRideReq
    = EndRideReq {point :: Kernel.Prelude.Maybe Kernel.External.Maps.Types.LatLong, odometerReadingValue :: Kernel.Prelude.Maybe Kernel.Types.Common.Centesimal}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
instance Kernel.Types.HideSecrets.HideSecrets EndRideReq
    where hideSecrets = Kernel.Prelude.identity
data StartRideReq
    = StartRideReq {point :: Kernel.Prelude.Maybe Kernel.External.Maps.Types.LatLong, odometerReadingValue :: Kernel.Prelude.Maybe Kernel.Types.Common.Centesimal}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
instance Kernel.Types.HideSecrets.HideSecrets StartRideReq
    where hideSecrets = Kernel.Prelude.identity
...
data RideAPIs
    = RideAPIs {postRideStart :: (Kernel.Types.Id.Id Dashboard.Common.Ride -> StartRideReq -> EulerHS.Types.EulerClient Kernel.Types.APISuccess.APISuccess),
...
mkRideAPIs rideClient = (RideAPIs {..})
               where postRideStart :<|> postRideEnd :<|> getRideCurrentActiveRide :<|> postRideCancel :<|> postRideBookingWithVehicleNumberAndPhone = rideClient
data RideUserActionType
    = POST_RIDE_START | POST_RIDE_END | GET_RIDE_CURRENT_ACTIVE_RIDE | POST_RIDE_CANCEL | POST_RIDE_BOOKING_WITH_VEHICLE_NUMBER_AND_PHONE
