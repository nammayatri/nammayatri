...
data DriverInformationE e
    = DriverInformation {aadhaarNumber :: Kernel.Prelude.Maybe (Kernel.External.Encryption.EncryptedHashedField e Kernel.Prelude.Text),
...
instance EncryptedItem DriverInformation
    where type Unencrypted DriverInformation = (DecryptedDriverInformation, HashSalt)
          encryptItem (entity, salt) = do {aadhaarNumber_ <- encryptItem $ (, salt) <$> aadhaarNumber entity;
                                           ...
                                           pure DriverInformation{aadhaarNumber = aadhaarNumber_,
                                                                  ...
                                                                  nomineeRelationship = nomineeRelationship entity,
          ...
          decryptItem entity = do {aadhaarNumber_ <- fmap fst <$> decryptItem (aadhaarNumber entity);
                                   ...
                                   pure (DriverInformation{aadhaarNumber = aadhaarNumber_,
                                         ...
                                         "")}
instance EncryptedItem' DriverInformation
    where type UnencryptedItem DriverInformation = DecryptedDriverInformation
...
data DriverBankAccountDetails
    = DriverBankAccountDetails {accountNumber :: Kernel.Prelude.Maybe Kernel.Prelude.Text, ifscCode :: Kernel.Prelude.Maybe Kernel.Prelude.Text, nameAtBank :: Kernel.Prelude.Maybe Kernel.Prelude.Text}
    deriving (Generic, Show, ToJSON, FromJSON, ToSchema)
data DriverMissedOpp
    = DriverMissedOpp {cancellationRate :: Kernel.Prelude.Int,
    ...
    deriving (Generic, Show, ToJSON, FromJSON, ToSchema)
data DriverSummary
    = DriverSummary {bonusEarned :: Kernel.Types.Common.Money,
