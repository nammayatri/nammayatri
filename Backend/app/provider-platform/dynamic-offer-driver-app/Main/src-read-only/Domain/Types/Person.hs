...
data PersonE e
    = Person {alternateMobileNumber :: Kernel.Prelude.Maybe (Kernel.External.Encryption.EncryptedHashedField e Kernel.Prelude.Text),
...
instance EncryptedItem Person
    where type Unencrypted Person = (DecryptedPerson, HashSalt)
          encryptItem (entity, salt) = do {alternateMobileNumber_ <- encryptItem $ (, salt) <$> alternateMobileNumber entity;
                                           mobileNumber_ <- encryptItem $ (, salt) <$> mobileNumber entity;
                                           pure Person{alternateMobileNumber = alternateMobileNumber_,
          ...
          decryptItem entity = do {alternateMobileNumber_ <- fmap fst <$> decryptItem (alternateMobileNumber entity);
                                   ...
                                   pure (Person{alternateMobileNumber = alternateMobileNumber_,
                                         ...
                                         "")}
instance EncryptedItem' Person
    where type UnencryptedItem Person = DecryptedPerson
