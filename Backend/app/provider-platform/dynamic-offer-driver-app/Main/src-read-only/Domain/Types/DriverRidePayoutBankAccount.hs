...
data DriverRidePayoutBankAccountE e
    = DriverRidePayoutBankAccount {bankAccountNumber :: Kernel.Prelude.Maybe (Kernel.External.Encryption.EncryptedHashedField e Kernel.Prelude.Text),
...
instance EncryptedItem DriverRidePayoutBankAccount
    where type Unencrypted DriverRidePayoutBankAccount = (DecryptedDriverRidePayoutBankAccount, HashSalt)
          encryptItem (entity, salt) = do {bankAccountNumber_ <- encryptItem $ (, salt) <$> bankAccountNumber entity;
                                           bankIfscCode_ <- encryptItem $ (, salt) <$> bankIfscCode entity;
                                           pure DriverRidePayoutBankAccount{bankAccountNumber = bankAccountNumber_,
          ...
          decryptItem entity = do {bankAccountNumber_ <- fmap fst <$> decryptItem (bankAccountNumber entity);
                                   ...
                                   pure (DriverRidePayoutBankAccount{bankAccountNumber = bankAccountNumber_,
                                         ...
                                         "")}
instance EncryptedItem' DriverRidePayoutBankAccount
    where type UnencryptedItem DriverRidePayoutBankAccount = DecryptedDriverRidePayoutBankAccount
