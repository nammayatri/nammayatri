...
data RideDetailsE e
    = RideDetails {createdAt :: Kernel.Prelude.Maybe Kernel.Prelude.UTCTime,
...
instance EncryptedItem RideDetails
    where type Unencrypted RideDetails = (DecryptedRideDetails, HashSalt)
          encryptItem (entity, salt) = do {driverNumber_ <- encryptItem $ (, salt) <$> driverNumber entity;
                                           pure RideDetails{createdAt = createdAt entity,
          ...
          decryptItem entity = do {driverNumber_ <- fmap fst <$> decryptItem (driverNumber entity);
                                   pure (RideDetails{createdAt = createdAt entity,
                                                     defaultServiceTierName = defaultServiceTierName entity,
                                                     driverCountryCode = driverCountryCode entity,
                                                     driverName = driverName entity,
                                         ...
                                         "")}
instance EncryptedItem' RideDetails
    where type UnencryptedItem RideDetails = DecryptedRideDetails
