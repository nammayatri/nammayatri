...
data RiderDetailsE e
    = RiderDetails {bapId :: Kernel.Prelude.Maybe Kernel.Prelude.Text,
...
instance EncryptedItem RiderDetails
    where type Unencrypted RiderDetails = (DecryptedRiderDetails, HashSalt)
          encryptItem (entity, salt) = do {mobileNumber_ <- encryptItem (mobileNumber entity, salt);
                                           pure RiderDetails{bapId = bapId entity,
          ...
          decryptItem entity = do {mobileNumber_ <- fst <$> decryptItem (mobileNumber entity);
                                   pure (RiderDetails{bapId = bapId entity,
                                         ...
                                         "")}
instance EncryptedItem' RiderDetails
    where type UnencryptedItem RiderDetails = DecryptedRiderDetails
          toUnencrypted a salt = (a, salt)
          fromUnencrypted = fst
data PayoutFlagReason
    = ExceededMaxReferral | MinRideDistanceInvalid | MinPickupDistanceInvalid | CustomerExistAsDriver | MultipleDeviceIdExists | RideConstraintInvalid
