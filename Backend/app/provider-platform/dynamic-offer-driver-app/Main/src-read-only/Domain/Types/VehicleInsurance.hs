...
data VehicleInsuranceE e
    = VehicleInsurance {documentImageId :: Kernel.Types.Id.Id Domain.Types.Image.Image,
...
instance EncryptedItem VehicleInsurance
    where type Unencrypted VehicleInsurance = (DecryptedVehicleInsurance, HashSalt)
          encryptItem (entity, salt) = do {policyNumber_ <- encryptItem (policyNumber entity, salt);
                                           pure VehicleInsurance{documentImageId = documentImageId entity,
          ...
          decryptItem entity = do {policyNumber_ <- fst <$> decryptItem (policyNumber entity);
                                   pure (VehicleInsurance{documentImageId = documentImageId entity,
                                                          driverId = driverId entity,
                                                          id = id entity,
                                                          insuredName = insuredName entity,
                                         ...
                                         "")}
instance EncryptedItem' VehicleInsurance
    where type UnencryptedItem VehicleInsurance = DecryptedVehicleInsurance
