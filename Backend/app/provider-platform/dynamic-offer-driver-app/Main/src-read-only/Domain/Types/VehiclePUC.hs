...
data VehiclePUCE e
    = VehiclePUC {documentImageId :: Kernel.Types.Id.Id Domain.Types.Image.Image,
...
instance EncryptedItem VehiclePUC
    where type Unencrypted VehiclePUC = (DecryptedVehiclePUC, HashSalt)
          encryptItem (entity, salt) = do {pucNumber_ <- encryptItem $ (, salt) <$> pucNumber entity;
                                           pure VehiclePUC{documentImageId = documentImageId entity,
          ...
          decryptItem entity = do {pucNumber_ <- fmap fst <$> decryptItem (pucNumber entity);
                                   pure (VehiclePUC{documentImageId = documentImageId entity,
                                                    driverId = driverId entity,
                                                    id = id entity,
                                                    pucExpiry = pucExpiry entity,
                                         ...
                                         "")}
instance EncryptedItem' VehiclePUC
    where type UnencryptedItem VehiclePUC = DecryptedVehiclePUC
