...
instance EncryptedItem IdfyVerification
    where type Unencrypted IdfyVerification = (DecryptedIdfyVerification, HashSalt)
          encryptItem (entity, salt) = do {documentNumber_ <- encryptItem (documentNumber entity, salt);
                                           pure IdfyVerification{airConditioned = airConditioned entity,
          ...
          decryptItem entity = do {documentNumber_ <- fst <$> decryptItem (documentNumber entity);
                                   pure (IdfyVerification{airConditioned = airConditioned entity,
                                         ...
                                         "")}
instance EncryptedItem' IdfyVerification
    where type UnencryptedItem IdfyVerification = DecryptedIdfyVerification
