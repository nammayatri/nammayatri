...
data VehiclePermitE e
    = VehiclePermit {documentImageId :: Kernel.Types.Id.Id Domain.Types.Image.Image,
...
instance EncryptedItem VehiclePermit
    where type Unencrypted VehiclePermit = (DecryptedVehiclePermit, HashSalt)
          encryptItem (entity, salt) = do {permitNumber_ <- encryptItem (permitNumber entity, salt);
                                           pure VehiclePermit{documentImageId = documentImageId entity,
          ...
          decryptItem entity = do {permitNumber_ <- fst <$> decryptItem (permitNumber entity);
                                   pure (VehiclePermit{documentImageId = documentImageId entity,
                                                       driverId = driverId entity,
                                                       id = id entity,
                                                       issueDate = issueDate entity,
                                         ...
                                         "")}
instance EncryptedItem' VehiclePermit
    where type UnencryptedItem VehiclePermit = DecryptedVehiclePermit
