...
data DriverUdyamE e
    = DriverUdyam {driverId :: Kernel.Types.Id.Id Domain.Types.Person.Person,
...
instance EncryptedItem DriverUdyam
    where type Unencrypted DriverUdyam = (DecryptedDriverUdyam, HashSalt)
          encryptItem (entity, salt) = do {udyamNumber_ <- encryptItem (udyamNumber entity, salt);
                                           pure DriverUdyam{driverId = driverId entity,
          ...
          decryptItem entity = do {udyamNumber_ <- fst <$> decryptItem (udyamNumber entity);
                                   pure (DriverUdyam{driverId = driverId entity,
                                                     enterpriseName = enterpriseName entity,
                                                     enterpriseType = enterpriseType entity,
                                                     id = id entity,
                                         ...
                                         "")}
instance EncryptedItem' DriverUdyam
    where type UnencryptedItem DriverUdyam = DecryptedDriverUdyam
