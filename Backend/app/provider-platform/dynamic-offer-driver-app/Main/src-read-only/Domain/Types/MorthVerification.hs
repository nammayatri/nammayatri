...
data MorthVerificationE e
    = MorthVerification {docType :: Domain.Types.DocumentVerificationConfig.DocumentType,
...
instance EncryptedItem MorthVerification
    where type Unencrypted MorthVerification = (DecryptedMorthVerification, HashSalt)
          encryptItem (entity, salt) = do {documentNumber_ <- encryptItem (documentNumber entity, salt);
                                           pure MorthVerification{docType = docType entity,
          ...
          decryptItem entity = do {documentNumber_ <- fst <$> decryptItem (documentNumber entity);
                                   pure (MorthVerification{docType = docType entity,
                                         ...
                                         "")}
instance EncryptedItem' MorthVerification
    where type UnencryptedItem MorthVerification = DecryptedMorthVerification
