...
data DriverPanCardE e
    = DriverPanCard {consent :: Kernel.Prelude.Bool,
...
instance EncryptedItem DriverPanCard
    where type Unencrypted DriverPanCard = (DecryptedDriverPanCard, HashSalt)
          encryptItem (entity, salt) = do {panCardNumber_ <- encryptItem (panCardNumber entity, salt);
                                           pure DriverPanCard{consent = consent entity,
          ...
          decryptItem entity = do {panCardNumber_ <- fst <$> decryptItem (panCardNumber entity);
                                   pure (DriverPanCard{consent = consent entity,
                                                       consentTimestamp = consentTimestamp entity,
                                                       docType = docType entity,
                                                       documentImageId1 = documentImageId1 entity,
                                         ...
                                         "")}
instance EncryptedItem' DriverPanCard
    where type UnencryptedItem DriverPanCard = DecryptedDriverPanCard
