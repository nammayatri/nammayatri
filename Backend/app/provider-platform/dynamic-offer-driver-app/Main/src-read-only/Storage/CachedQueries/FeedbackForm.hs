  ...
  (Hedis.safeGet $ "driverOfferCachedQueries:FeedbackForm:" <> ":MerchantOperatingCityId-" <> Kernel.Types.Id.getId merchantOperatingCityId)
    >>= ( \case
            ...
            Nothing ->
              ( \dataToBeCached -> do
                  expTime <- fromIntegral <$> asks (.cacheConfig.configsExpTime)
                  Hedis.setExp ("driverOfferCachedQueries:FeedbackForm:" <> ":MerchantOperatingCityId-" <> Kernel.Types.Id.getId merchantOperatingCityId) dataToBeCached expTime
              )
  ...
  (Hedis.safeGet $ "driverOfferCachedQueries:FeedbackForm:" <> ":MerchantOperatingCityId-" <> Kernel.Types.Id.getId merchantOperatingCityId <> ":Rating-" <> show rating)
    >>= ( \case
            ...
            Nothing ->
              ( \dataToBeCached -> do
                  expTime <- fromIntegral <$> asks (.cacheConfig.configsExpTime)
                  Hedis.setExp ("driverOfferCachedQueries:FeedbackForm:" <> ":MerchantOperatingCityId-" <> Kernel.Types.Id.getId merchantOperatingCityId <> ":Rating-" <> show rating) dataToBeCached expTime
              )
  ...
  (Hedis.safeGet $ "driverOfferCachedQueries:FeedbackForm:" <> ":Rating-" <> show rating)
    >>= ( \case
            ...
            Nothing ->
              ( \dataToBeCached -> do
                  expTime <- fromIntegral <$> asks (.cacheConfig.configsExpTime)
                  Hedis.setExp ("driverOfferCachedQueries:FeedbackForm:" <> ":Rating-" <> show rating) dataToBeCached expTime
              )
