...
instance B.Table PayoutInstanceCalculationT where
  data PrimaryKey PayoutInstanceCalculationT f = PayoutInstanceCalculationId (B.C f Data.Text.Text) (B.C f Data.Text.Text) (B.C f Data.Text.Text) deriving (Generic, B.Beamable)
  primaryKey = PayoutInstanceCalculationId <$> fromVendorId <*> id <*> toVendorId

type PayoutInstanceCalculation = PayoutInstanceCalculationT Identity

$(enableKVPG (''PayoutInstanceCalculationT) [('fromVendorId), ('id), ('toVendorId)] [])
