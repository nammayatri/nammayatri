...
data SearchRequestT f
    = SearchRequestT {area :: (B.C f (Kernel.Prelude.Maybe Lib.Types.SpecialLocation.Area)),
...
instance B.Table SearchRequestT
    where data PrimaryKey SearchRequestT f = SearchRequestId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
