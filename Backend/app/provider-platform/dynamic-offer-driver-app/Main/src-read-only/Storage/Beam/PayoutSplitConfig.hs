...
instance B.Table PayoutSplitConfigT where
  data PrimaryKey PayoutSplitConfigT f
    = PayoutSplitConfigId (B.C f Lib.Types.SpecialLocation.Area) (B.C f Domain.Types.VehicleVariant.VehicleVariant) (B.C f Data.Text.Text)
    deriving (Generic, B.Beamable)
  primaryKey = PayoutSplitConfigId <$> area <*> vehicleVariant <*> vendorId

type PayoutSplitConfig = PayoutSplitConfigT Identity

$(enableKVPG (''PayoutSplitConfigT) [('area), ('vehicleVariant), ('vendorId)] [])
