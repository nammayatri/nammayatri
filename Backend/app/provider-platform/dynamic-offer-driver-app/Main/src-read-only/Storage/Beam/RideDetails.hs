...
data RideDetailsT f
    = RideDetailsT {createdAt :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.UTCTime)),
...
instance B.Table RideDetailsT
    where data PrimaryKey RideDetailsT f = RideDetailsId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
