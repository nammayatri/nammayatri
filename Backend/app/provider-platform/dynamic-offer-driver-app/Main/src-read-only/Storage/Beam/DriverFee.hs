...
data DriverFeeT f = DriverFeeT
  { addedToFeeId :: (B.C f (Kernel.Prelude.Maybe (Kernel.Prelude.Text))),
    ...
    dailyBaseAmount :: (B.C f (Kernel.Prelude.Maybe Kernel.Types.Common.HighPrecMoney)),
    dailyBaseCount :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.Int)),
    ...
    perRideBaseAmount :: (B.C f (Kernel.Prelude.Maybe Kernel.Types.Common.HighPrecMoney)),
...
$(enableKVPGWithPartialIndex (''DriverFeeT) [('id)] [[('driverId)]] [SKeyPartial [(('feeType), "CANCELLATION_PENALTY")], SKeyPartial [(('serviceName), "YATRI_RENTAL")]])
