...
data PayoutConfigT f
    = PayoutConfigT {batchLimit :: (B.C f Kernel.Prelude.Int),
...
instance B.Table PayoutConfigT
    where data PrimaryKey PayoutConfigT f = PayoutConfigId (B.C f Kernel.Prelude.Text) (B.C f Domain.Types.VehicleCategory.VehicleCategory) deriving (Generic, B.Beamable)
