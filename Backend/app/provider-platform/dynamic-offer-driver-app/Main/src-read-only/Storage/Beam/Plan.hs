...
data PlanT f
    = PlanT {allowStrikeOff :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.Bool)),
...
instance B.Table PlanT
    where data PrimaryKey PlanT f = PlanId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
