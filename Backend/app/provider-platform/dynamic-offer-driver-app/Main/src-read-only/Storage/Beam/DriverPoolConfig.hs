...
data DriverPoolConfigT f
    = DriverPoolConfigT {actualDistanceThreshold :: (B.C f (Kernel.Prelude.Maybe Kernel.Types.Common.Meters)),
...
instance B.Table DriverPoolConfigT
    where data PrimaryKey DriverPoolConfigT f = DriverPoolConfigId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
