...
data VehicleInsuranceT f
    = VehicleInsuranceT {documentImageId :: (B.C f Kernel.Prelude.Text),
...
instance B.Table VehicleInsuranceT
    where data PrimaryKey VehicleInsuranceT f = VehicleInsuranceId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
          primaryKey = VehicleInsuranceId . id
type VehicleInsurance = VehicleInsuranceT Identity

$(enableKVPG (''VehicleInsuranceT) [('id)] [[('documentImageId)], [('rcId)]])
