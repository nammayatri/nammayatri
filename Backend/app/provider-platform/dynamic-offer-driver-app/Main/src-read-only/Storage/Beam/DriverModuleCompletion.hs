...
data DriverModuleCompletionT f
    = DriverModuleCompletionT {completedAt :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.UTCTime)),
...
instance B.Table DriverModuleCompletionT
    where data PrimaryKey DriverModuleCompletionT f = DriverModuleCompletionId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
