...
data DriverGstinT f
    = DriverGstinT {address :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.Text)),
...
instance B.Table DriverGstinT
    where data PrimaryKey DriverGstinT f = DriverGstinId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
