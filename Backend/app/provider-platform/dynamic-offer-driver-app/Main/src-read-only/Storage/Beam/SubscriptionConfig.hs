...
data SubscriptionConfigT f
    = SubscriptionConfigT {allowDriverFeeCalcSchedule :: (B.C f Kernel.Prelude.Bool),
...
instance B.Table SubscriptionConfigT
    where data PrimaryKey SubscriptionConfigT f = SubscriptionConfigId (B.C f Domain.Types.Plan.ServiceNames) (B.C f (Kernel.Prelude.Maybe (Kernel.Prelude.Text))) deriving (Generic, B.Beamable)
