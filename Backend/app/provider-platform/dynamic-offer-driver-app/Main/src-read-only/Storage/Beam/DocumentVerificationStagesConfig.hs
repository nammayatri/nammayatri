...
instance B.Table DocumentVerificationStagesConfigT where
  data PrimaryKey DocumentVerificationStagesConfigT f
    ...
    deriving (Generic, B.Beamable)
  primaryKey = DocumentVerificationStagesConfigId <$> applicableTo <*> documentOnboardingStage <*> merchantOperatingCityId <*> vehicleCategory

type DocumentVerificationStagesConfig = DocumentVerificationStagesConfigT Identity

$(enableKVPG (''DocumentVerificationStagesConfigT) [('applicableTo), ('documentOnboardingStage), ('merchantOperatingCityId), ('vehicleCategory)] [])
