...
data DriverQuoteT f
    = DriverQuoteT {backendAppVersion :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.Text)),
...
instance B.Table DriverQuoteT
    where data PrimaryKey DriverQuoteT f = DriverQuoteId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
