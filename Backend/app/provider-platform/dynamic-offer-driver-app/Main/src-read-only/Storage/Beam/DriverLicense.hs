...
data DriverLicenseT f
    = DriverLicenseT {classOfVehicles :: (B.C f [Kernel.Prelude.Text]),
...
instance B.Table DriverLicenseT
    where data PrimaryKey DriverLicenseT f = DriverLicenseId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
