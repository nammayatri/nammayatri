...
data BookingLocationT f
    = BookingLocationT {area :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.Text)),
...
instance B.Table BookingLocationT
    where data PrimaryKey BookingLocationT f = BookingLocationId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
