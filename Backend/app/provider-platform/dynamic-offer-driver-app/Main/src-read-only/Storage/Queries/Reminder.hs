...
findAllPendingByDriverId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllPendingByDriverIdAndDocumentType :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllPendingByDriverIdAndDocumentType driverId status documentType = do findAllWithKV [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
...
updateByPrimaryKey (Domain.Types.Reminder.Reminder {..}) = do {_now <- getCurrentTime;
                                                               updateWithKV [Se.Set Beam.currentIntervalIndex currentIntervalIndex,
...
instance FromTType' Beam.Reminder Domain.Types.Reminder.Reminder
    where fromTType' (Beam.ReminderT {..}) = do pure $ Just Domain.Types.Reminder.Reminder{createdAt = createdAt,
                                                                                           ...
                                                                                           status = status,
                                                                                           updatedAt = updatedAt}
instance ToTType' Beam.Reminder Domain.Types.Reminder.Reminder
    where toTType' (Domain.Types.Reminder.Reminder {..}) = do Beam.ReminderT{Beam.createdAt = createdAt,
