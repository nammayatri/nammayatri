...
updateCancellationDues cancellationDues id = do {_now <- getCurrentTime;
...
updateDisputeChancesUsed disputeChancesUsed id = do {_now <- getCurrentTime;
...
updateDisputeChancesUsedAndCancellationDues :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                               (Kernel.Prelude.Int -> Kernel.Types.Common.HighPrecMoney -> Kernel.Types.Id.Id Domain.Types.RiderDetails.RiderDetails -> m ())
updateDisputeChancesUsedAndCancellationDues disputeChancesUsed cancellationDues id = do {_now <- getCurrentTime;
...
updateFirstRideIdAndFlagReason :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateFirstRideIdAndFlagReason firstRideId payoutFlagReason id = do {_now <- getCurrentTime;
...
updateFlagReasonAndIsDeviceIdExists :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateFlagReasonAndIsDeviceIdExists payoutFlagReason isDeviceIdExists id = do {_now <- getCurrentTime;
...
updateHasTakenValidRide :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateHasTakenValidRide hasTakenValidRide hasTakenValidRideAt id = do {_now <- getCurrentTime;
...
updateIsDeviceIdExists isDeviceIdExists id = do {_now <- getCurrentTime;
...
updateIsFlagConfirmed isFlagConfirmed id = do {_now <- getCurrentTime;
...
updateNightSafetyChecks nightSafetyChecks id = do {_now <- getCurrentTime;
...
updatePayoutFlagReason :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updatePayoutFlagReason payoutFlagReason id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.RiderDetails.RiderDetails {..}) = do {_now <- getCurrentTime;
                                                                       updateWithKV [Se.Set Beam.bapId bapId,
                                                                                     ...
                                                                                     Se.Set Beam.waivedOffAmount (Kernel.Prelude.Just waivedOffAmount)] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]}
