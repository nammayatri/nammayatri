...
findAllByMerchantOperatingCityAndVehicleType :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByMerchantOperatingCityAndVehicleType limit offset merchantOperatingCityId vehicleType code = do findAllWithOptionsKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                                      Se.Is Beam.vehicleType $ Se.Eq vehicleType,
                                                                                                                                      Se.Is Beam.code $ Se.In code]] (Se.Desc Beam.createdAt) limit offset
...
updateByPrimaryKey (Domain.Types.Route.Route {..}) = do {_now <- getCurrentTime;
                                                         updateWithKV [Se.Set Beam.code code,
