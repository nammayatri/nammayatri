...
findAllByCompletionId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByCompletionIdAndEntity :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByCompletionIdAndEntityAndStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByCompletionIdAndEntityAndStatus entity entityStatus completionId = do findAllWithKV [Se.And [Se.Is Beam.entity $ Se.Eq entity,
                                                                                                     Se.Is Beam.entityStatus $ Se.Eq entityStatus,
                                                                                                     Se.Is Beam.completionId $ Se.Eq (Kernel.Types.Id.getId completionId)]]
findByCompletionIdAndEntityAndEntityId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByCompletionIdAndEntityAndEntityId limit offset entity entityId completionId = do findAllWithOptionsKV [Se.And [Se.Is Beam.entity $ Se.Eq entity,
                                                                                                                    Se.Is Beam.entityId $ Se.Eq entityId,
                                                                                                                    Se.Is Beam.completionId $ Se.Eq (Kernel.Types.Id.getId completionId)]] (Se.Desc Beam.attempt) limit offset
...
findByPrimaryKey attempt completionId entity entityId = do findOneWithKV [Se.And [Se.Is Beam.attempt $ Se.Eq attempt,
                                                                                  Se.Is Beam.completionId $ Se.Eq (Kernel.Types.Id.getId completionId),
...
updateByPrimaryKey (Domain.Types.ModuleCompletionInformation.ModuleCompletionInformation {..}) = do {_now <- getCurrentTime;
                                                                                                     updateWithKV [Se.Set Beam.entityStatus entityStatus, Se.Set Beam.selectedEntityId selectedEntityId, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.attempt $ Se.Eq attempt,
                                                                                                                                                                                                                                              Se.Is Beam.completionId $ Se.Eq (Kernel.Types.Id.getId completionId),
                                                                                                                                                                                                                                              Se.Is Beam.entity $ Se.Eq entity,
                                                                                                                                                                                                                                              Se.Is Beam.entityId $ Se.Eq entityId]]}
...
instance FromTType' Beam.ModuleCompletionInformation Domain.Types.ModuleCompletionInformation.ModuleCompletionInformation
    where fromTType' (Beam.ModuleCompletionInformationT {..}) = do pure $ Just Domain.Types.ModuleCompletionInformation.ModuleCompletionInformation{attempt = attempt,
                                                                                                                                                    ...
                                                                                                                                                    updatedAt = updatedAt}
instance ToTType' Beam.ModuleCompletionInformation Domain.Types.ModuleCompletionInformation.ModuleCompletionInformation
    where toTType' (Domain.Types.ModuleCompletionInformation.ModuleCompletionInformation {..}) = do Beam.ModuleCompletionInformationT{Beam.attempt = attempt,
