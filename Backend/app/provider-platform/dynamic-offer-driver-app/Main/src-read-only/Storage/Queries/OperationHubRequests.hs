...
findByCreatorStatusAndType :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByCreatorStatusAndType creatorId requestStatus requestType = do findAllWithKV [Se.And [Se.Is Beam.creatorId $ Se.Eq (Kernel.Types.Id.getId creatorId),
                                                                                           Se.Is Beam.requestStatus $ Se.Eq requestStatus,
                                                                                           Se.Is Beam.requestType $ Se.Eq requestType]]
findOneByRequestStatusAndDriverId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findOneByRequestStatusAndRegistrationNo :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateStatusWithDetails :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateStatusWithDetails requestStatus remarks fulfilledAt operatorId id = do {_now <- getCurrentTime;
                                                                              updateOneWithKV [Se.Set Beam.requestStatus requestStatus,
...
updateByPrimaryKey (Domain.Types.OperationHubRequests.OperationHubRequests {..}) = do {_now <- getCurrentTime;
