...
findActiveManualInvoiceByFeeId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findActiveManualInvoiceByFeeId driverFeeId paymentMode invoiceStatus = do findAllWithKV [Se.And [Se.Is Beam.driverFeeId $ Se.Eq (Kernel.Types.Id.getId driverFeeId),
...
findByDriverFeePaymentModeAndInvoiceStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                              (Kernel.Types.Id.Id Domain.Types.DriverFee.DriverFee -> Domain.Types.Invoice.InvoicePaymentMode -> Domain.Types.Invoice.InvoiceStatus -> m ([Domain.Types.Invoice.Invoice]))
findByDriverFeePaymentModeAndInvoiceStatus driverFeeId paymentMode invoiceStatus = do findAllWithKV [Se.And [Se.Is Beam.driverFeeId $ Se.Eq (Kernel.Types.Id.getId driverFeeId),
                                                                                                             Se.Is Beam.paymentMode $ Se.Eq paymentMode,
                                                                                                             Se.Is Beam.invoiceStatus $ Se.Eq invoiceStatus]]
...
findByIdWithPaymenModeAndStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByIdWithPaymenModeAndStatus id paymentMode invoiceStatus = do findOneWithKV [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id),
                                                                                         Se.Is Beam.paymentMode $ Se.Eq paymentMode,
                                                                                         Se.Is Beam.invoiceStatus $ Se.Eq invoiceStatus]]
findInvoiceByFeeIdAndStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateBankErrorsByInvoiceId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateBankErrorsByInvoiceId bankErrorMessage bankErrorCode bankErrorUpdatedAt id = do {_now <- getCurrentTime;
                                                                                       updateWithKV [Se.Set Beam.bankErrorMessage bankErrorMessage,
...
updateByPrimaryKey (Domain.Types.Invoice.Invoice {..}) = do {_now <- getCurrentTime;
                                                             updateWithKV [Se.Set Beam.bankErrorCode bankErrorCode,
