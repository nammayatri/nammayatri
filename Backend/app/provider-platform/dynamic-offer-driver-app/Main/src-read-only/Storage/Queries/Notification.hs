...
updateNotificationStatusAndResponseInfoById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateNotificationStatusAndResponseInfoById status responseCode responseMessage id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.Notification.Notification {..}) = do {_now <- getCurrentTime;
                                                                       updateWithKV [Se.Set Beam.dateCreated dateCreated,
