...
findImagesByPersonAndType :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findImagesByPersonAndType limit offset merchantId personId imageType = do findAllWithOptionsKV [Se.And [Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId merchantId),
                                                                                                        Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId),
                                                                                                        Se.Is Beam.imageType $ Se.Eq imageType]] (Se.Desc Beam.createdAt) limit offset
...
updateDocumentExpiry documentExpiry id = do {_now <- getCurrentTime;
...
updateMerchantIdAndCityIdByPersonId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateMerchantIdAndCityIdByPersonId merchantId merchantOperatingCityId personId = do {_now <- getCurrentTime;
...
updateVerificationStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateVerificationStatus verificationStatus reviewerEmail workflowTransactionId = do {_now <- getCurrentTime;
...
updateVerificationStatusAndExpiry :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateVerificationStatusAndExpiry verificationStatus documentExpiry imageType id = do {_now <- getCurrentTime;
                                                                                       updateOneWithKV [Se.Set Beam.verificationStatus verificationStatus,
...
updateVerificationStatusAndFailureReasonByRcIdAndImageType :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateVerificationStatusAndFailureReasonByRcIdAndImageType verificationStatus failureReason rcId imageType = do {_now <- getCurrentTime;
                                                                                                                 updateWithKV [Se.Set Beam.verificationStatus verificationStatus, Se.Set Beam.failureReason failureReason, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.rcId $ Se.Eq rcId,
...
updateByPrimaryKey (Domain.Types.Image.Image {..}) = do {_now <- getCurrentTime;
                                                         updateWithKV [Se.Set Beam.documentExpiry documentExpiry,
