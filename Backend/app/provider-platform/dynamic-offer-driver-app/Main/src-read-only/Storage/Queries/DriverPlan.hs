...
findByDriverIdWithServiceName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByDriverIdWithServiceName driverId serviceName = do findOneWithKV [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
                                                                               Se.Is Beam.serviceName $ Se.Eq (Kernel.Prelude.Just serviceName)]]
findByMandateIdAndServiceName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByMandateIdAndServiceName mandateId serviceName = do findOneWithKV [Se.And [Se.Is Beam.mandateId $ Se.Eq (Kernel.Types.Id.getId <$> mandateId),
                                                                                Se.Is Beam.serviceName $ Se.Eq (Kernel.Prelude.Just serviceName)]]
updateEnableServiceUsageChargeByDriverIdAndServiceName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                                          (Kernel.Prelude.Bool -> Kernel.Types.Id.Id Domain.Types.Person.Person -> Domain.Types.Extra.Plan.ServiceNames -> m ())
updateEnableServiceUsageChargeByDriverIdAndServiceName enableServiceUsageCharge driverId serviceName = do {_now <- getCurrentTime;
                                                                                                           updateOneWithKV [Se.Set Beam.enableServiceUsageCharge (Kernel.Prelude.Just enableServiceUsageCharge),
                                                                                                                            Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId), Se.Is Beam.serviceName $ Se.Eq (Kernel.Prelude.Just serviceName)]]}
updateFreeTrialByDriverIdAndServiceName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                           (Kernel.Prelude.Bool -> Kernel.Types.Id.Id Domain.Types.Person.Person -> Domain.Types.Extra.Plan.ServiceNames -> m ())
updateFreeTrialByDriverIdAndServiceName isOnFreeTrial driverId serviceName = do {_now <- getCurrentTime;
                                                                                 updateOneWithKV [Se.Set Beam.isOnFreeTrial (Kernel.Prelude.Just isOnFreeTrial), Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
                                                                                                                                                                                                      Se.Is Beam.serviceName $ Se.Eq (Kernel.Prelude.Just serviceName)]]}
updateLastPaymentLinkSentAtDateByDriverIdAndServiceName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateLastPaymentLinkSentAtDateByDriverIdAndServiceName lastPaymentLinkSentAtIstDate driverId serviceName = do {_now <- getCurrentTime;
                                                                                                                updateOneWithKV [Se.Set Beam.lastPaymentLinkSentAtIstDate lastPaymentLinkSentAtIstDate, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
                                                                                                                                                                                                                                             Se.Is Beam.serviceName $ Se.Eq (Kernel.Prelude.Just serviceName)]]}
updateMandateIdByDriverIdAndServiceName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateMandateIdByDriverIdAndServiceName mandateId driverId serviceName = do {_now <- getCurrentTime;
                                                                             updateOneWithKV [Se.Set Beam.mandateId (Kernel.Types.Id.getId <$> mandateId), Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
                                                                                                                                                                                                Se.Is Beam.serviceName $ Se.Eq (Kernel.Prelude.Just serviceName)]]}
updateMandateSetupDateByDriverIdAndServiceName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                                  (Kernel.Prelude.Maybe Kernel.Prelude.UTCTime -> Kernel.Types.Id.Id Domain.Types.Person.Person -> Domain.Types.Extra.Plan.ServiceNames -> m ())
updateMandateSetupDateByDriverIdAndServiceName mandateSetupDate driverId serviceName = do {_now <- getCurrentTime;
                                                                                           updateOneWithKV [Se.Set Beam.mandateSetupDate mandateSetupDate, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
                                                                                                                                                                                                Se.Is Beam.serviceName $ Se.Eq (Kernel.Prelude.Just serviceName)]]}
...
updateMerchantIdAndCityIdByDriverId merchantId merchantOpCityId driverId = do {_now <- getCurrentTime;
                                                                               updateWithKV [Se.Set Beam.merchantId (Kernel.Prelude.Just (Kernel.Types.Id.getId merchantId)),
...
updatePaymentModeByDriverIdAndServiceName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                             (Domain.Types.Plan.PaymentMode -> Kernel.Types.Id.Id Domain.Types.Person.Person -> Domain.Types.Extra.Plan.ServiceNames -> m ())
updatePaymentModeByDriverIdAndServiceName planType driverId serviceName = do {_now <- getCurrentTime;
                                                                              updateOneWithKV [Se.Set Beam.planType planType, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
                                                                                                                                                                   Se.Is Beam.serviceName $ Se.Eq (Kernel.Prelude.Just serviceName)]]}
...
updateByPrimaryKey (Domain.Types.DriverPlan.DriverPlan {..}) = do {_now <- getCurrentTime;
                                                                   updateWithKV [Se.Set Beam.autoPayStatus autoPayStatus,
                                                                                 ...
                                                                                 Se.Set Beam.waiverOffPercentage (Kernel.Prelude.Just waiverOffPercentage)] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]]}
