...
findActiveRides ::
  ...
  (Kernel.Types.Id.Id Domain.Types.Person.Person -> Domain.Types.SharedRide.SharedRideStatus -> m ([Domain.Types.SharedRide.SharedRide]))
findActiveRides driverId status = do findAllWithKV [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId), Se.Is Beam.status $ Se.Eq status]]
...
findByTransactionId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => (Kernel.Prelude.Text -> m (Maybe Domain.Types.SharedRide.SharedRide))
...
updateTollCharges ::
  (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
  (Kernel.Prelude.Maybe Kernel.Types.Common.HighPrecMoney -> Kernel.Prelude.Maybe [Kernel.Prelude.Text] -> Kernel.Types.Id.Id Domain.Types.SharedRide.SharedRide -> m ())
updateTollCharges tollCharges tollNames id = do
  _now <- getCurrentTime
  updateOneWithKV [Se.Set Beam.tollCharges tollCharges, Se.Set Beam.tollNames tollNames, Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]
...
updateByPrimaryKey (Domain.Types.SharedRide.SharedRide {..}) = do
  _now <- getCurrentTime
  updateWithKV
    [ Se.Set Beam.chargeableDistance chargeableDistance,
      Se.Set Beam.createdAt createdAt,
      Se.Set Beam.driverId (Kernel.Types.Id.getId driverId),
      Se.Set Beam.estimatedTollCharges estimatedTollCharges,
      Se.Set Beam.estimatedTollNames estimatedTollNames,
      Se.Set Beam.isPickupOrDestinationEdited isPickupOrDestinationEdited,
      Se.Set Beam.merchantId (Kernel.Types.Id.getId merchantId),
      Se.Set Beam.merchantOperatingCityId (Kernel.Types.Id.getId merchantOperatingCityId),
      Se.Set Beam.numberOfDeviation numberOfDeviation,
      Se.Set Beam.numberOfOsrmSnapToRoadCalls numberOfOsrmSnapToRoadCalls,
      Se.Set Beam.numberOfSelfTuned numberOfSelfTuned,
      Se.Set Beam.numberOfSnapToRoadCalls numberOfSnapToRoadCalls,
      Se.Set Beam.passedThroughDestination passedThroughDestination,
      Se.Set Beam.rideEndTime rideEndTime,
      Se.Set Beam.rideEndedBy rideEndedBy,
      ...
      Se.Set Beam.traveledDistance traveledDistance,
