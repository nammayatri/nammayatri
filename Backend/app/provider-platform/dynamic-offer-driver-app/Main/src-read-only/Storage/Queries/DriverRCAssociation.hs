...
deactivateRCForDriver :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
deactivateRCForDriver isRcActive driverId rcId = do {_now <- getCurrentTime;
                                                     updateWithKV [Se.Set Beam.isRcActive isRcActive, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
...
findActiveAssociationByDriver :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findActiveAssociationByRC :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateByPrimaryKey (Domain.Types.DriverRCAssociation.DriverRCAssociation {..}) = do {_now <- getCurrentTime;
