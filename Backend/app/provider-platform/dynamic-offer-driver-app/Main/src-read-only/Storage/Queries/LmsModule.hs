...
getAllModules :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
getAllModulesWithModuleSection :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
getAllModulesWithModuleSection limit offset merchantOperatingCityId moduleSection = do findAllWithOptionsKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                     Se.Is Beam.moduleSection $ Se.Eq moduleSection]] (Se.Desc Beam.createdAt) limit offset
...
updateByPrimaryKey (Domain.Types.LmsModule.LmsModule {..}) = do {_now <- getCurrentTime;
                                                                 updateWithKV [Se.Set Beam.bonusCoinEventFunction bonusCoinEventFunction,
...
instance FromTType' Beam.LmsModule Domain.Types.LmsModule.LmsModule
    where fromTType' (Beam.LmsModuleT {..}) = do pure $ Just Domain.Types.LmsModule.LmsModule{bonusCoinEventFunction = bonusCoinEventFunction,
                                                                                              category = category,
...
instance ToTType' Beam.LmsModule Domain.Types.LmsModule.LmsModule
    where toTType' (Domain.Types.LmsModule.LmsModule {..}) = do Beam.LmsModuleT{Beam.bonusCoinEventFunction = bonusCoinEventFunction,
