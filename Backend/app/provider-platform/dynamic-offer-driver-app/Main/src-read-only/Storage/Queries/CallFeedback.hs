...
updateByPrimaryKey (Domain.Types.CallFeedback.CallFeedback {..}) = do {_now <- getCurrentTime;
                                                                       updateWithKV [Se.Set Beam.entityId entityId,
                                                                                     Se.Set Beam.optionIds optionIds,
                                                                                     Se.Set Beam.merchantId (Kernel.Types.Id.getId <$> merchantId),
                                                                                     Se.Set Beam.merchantOperatingCityId (Kernel.Types.Id.getId <$> merchantOperatingCityId),
                                                                                     Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.callId $ Se.Eq callId, Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]}



instance FromTType' Beam.CallFeedback Domain.Types.CallFeedback.CallFeedback
    where fromTType' (Beam.CallFeedbackT {..}) = do pure $ Just Domain.Types.CallFeedback.CallFeedback{callId = callId,
                                                                                                       entityId = entityId,
                                                                                                       ...
                                                                                                       createdAt = createdAt,
                                                                                                       updatedAt = updatedAt}
instance ToTType' Beam.CallFeedback Domain.Types.CallFeedback.CallFeedback
    where toTType' (Domain.Types.CallFeedback.CallFeedback {..}) = do Beam.CallFeedbackT{Beam.callId = callId,
