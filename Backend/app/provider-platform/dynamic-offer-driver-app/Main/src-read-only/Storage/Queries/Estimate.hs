...
findEligibleForCabUpgrade requestId eligibleForUpgrade = do findAllWithKVAndConditionalDB [Se.And [Se.Is Beam.requestId $ Se.Eq (Kernel.Types.Id.getId requestId),
                                                                                                   Se.Is Beam.eligibleForUpgrade $ Se.Eq (Kernel.Prelude.Just eligibleForUpgrade)]] Nothing
updateSupplyDemandRatioByReqIdAndServiceTier :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateSupplyDemandRatioByReqIdAndServiceTier supplyDemandRatioFromLoc supplyDemandRatioToLoc requestId vehicleServiceTier = do {_now <- getCurrentTime;
                                                                                                                                updateWithKV [Se.Set Beam.supplyDemandRatioFromLoc supplyDemandRatioFromLoc,
                                                                                                                                              Se.Set Beam.supplyDemandRatioToLoc supplyDemandRatioToLoc,
                                                                                                                                              Se.Set Beam.updatedAt (Just _now)] [Se.And [Se.Is Beam.requestId $ Se.Eq (Kernel.Types.Id.getId requestId),
...
updateByPrimaryKey (Domain.Types.Estimate.Estimate {..}) = do {_now <- getCurrentTime;
                                                               updateWithKV [Se.Set Beam.businessDiscount businessDiscount,
                                                                             ...
                                                                             Se.Set Beam.vehicleServiceTierName vehicleServiceTierName] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]}



instance FromTType' Beam.Estimate Domain.Types.Estimate.Estimate
    where fromTType' (Beam.EstimateT {..}) = do {farePolicy' <- (maybe (pure Nothing) ((Storage.Cac.FarePolicy.findById Nothing) . Kernel.Types.Id.Id)) farePolicyId;
                                                 fareParams' <- maybe (pure Nothing) (Storage.Queries.FareParameters.findById . Kernel.Types.Id.Id) fareParamsId;
                                                 pure $ Just Domain.Types.Estimate.Estimate{businessDiscount = businessDiscount,
...
instance ToTType' Beam.Estimate Domain.Types.Estimate.Estimate
    where toTType' (Domain.Types.Estimate.Estimate {..}) = do Beam.EstimateT{Beam.businessDiscount = businessDiscount,
