...
updateDiscountAmount discountAmount id = do {_now <- getCurrentTime;
                                             updateOneWithKV [Se.Set Beam.discountAmount discountAmount, Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]}
updateDriverCancellationPenalty :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateDriverCancellationPenalty driverCancellationPenaltyFeeId driverCancellationPenaltyAmount id = do {_now <- getCurrentTime;
                                                                                                        updateOneWithKV [Se.Set Beam.driverCancellationPenaltyFeeId driverCancellationPenaltyFeeId,
...
updateDriverCancellationPenaltyWaivedReasonAndAmount :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                                        (Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Prelude.Maybe Kernel.Types.Common.HighPrecMoney -> Kernel.Types.Id.Id Domain.Types.Ride.Ride -> m ())
updateDriverCancellationPenaltyWaivedReasonAndAmount driverCancellationPenaltyWaivedReason driverCancellationPenaltyAmount id = do {_now <- getCurrentTime;
                                                                                                                                    updateOneWithKV [Se.Set Beam.driverCancellationPenaltyWaivedReason driverCancellationPenaltyWaivedReason,
                                                                                                                                                     Se.Set Beam.driverCancellationPenaltyAmount driverCancellationPenaltyAmount,
                                                                                                                                                     Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]}
...
updateDriverGpsTurnedOff driverGpsTurnedOff id = do {_now <- getCurrentTime;
...
updateEstimatedEndTimeRange estimatedEndTimeRange id = do {_now <- getCurrentTime;
                                                           updateOneWithKV [Se.Set Beam.estimatedEndTimeRangeEnd (Kernel.Prelude.fmap (.end) estimatedEndTimeRange),
                                                                            ...
                                                                            Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]}
updateFinalFarePolicyId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                           (Kernel.Prelude.Maybe (Kernel.Types.Id.Id Domain.Types.FarePolicy.FarePolicy) -> Kernel.Types.Id.Id Domain.Types.Ride.Ride -> m ())
updateFinalFarePolicyId finalFarePolicyId id = do {_now <- getCurrentTime;
...
updateIsPickupOrDestinationEdited isPickupOrDestinationEdited id = do {_now <- getCurrentTime;
...
updatePreviousRideTripEndPosAndTime :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updatePreviousRideTripEndPosAndTime previousRideTripEndPos previousRideTripEndTime id = do {_now <- getCurrentTime;
                                                                                            updateOneWithKV [Se.Set Beam.previousRideTripEndLat (Kernel.Prelude.fmap (.lat) previousRideTripEndPos),
...
updateRideTags rideTags id = do {_now <- getCurrentTime;
...
updateSubscriptionPurchaseIds :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateSubscriptionPurchaseIds subscriptionPurchaseIds id = do {_now <- getCurrentTime;
