...
findBaseServiceTierTypeByCategoryAndCityId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findBaseServiceTierTypeByCategoryAndCityId vehicleCategory merchantOperatingCityId = do findOneWithKV [Se.And [Se.Is Beam.vehicleCategory $ Se.Eq vehicleCategory,
                                                                                                               Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                               Se.Is Beam.baseVehicleServiceTier $ Se.Eq (Just True),
                                                                                                               Se.Or [Se.Is Beam.isEnabled $ Se.Eq (Just True), Se.Is Beam.isEnabled $ Se.Eq Nothing]]]
findByMerchantOpCityIdForList :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                 (Kernel.Types.Id.Id Domain.Types.MerchantOperatingCity.MerchantOperatingCity -> m ([Domain.Types.VehicleServiceTier.VehicleServiceTier]))
...
findByServiceTierTypeAndCityId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByServiceTierTypeAndCityId serviceTierType merchantOperatingCityId = do findOneWithKV [Se.And [Se.Is Beam.serviceTierType $ Se.Eq serviceTierType,
                                                                                                   Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                   Se.Or [Se.Is Beam.isEnabled $ Se.Eq (Just True), Se.Is Beam.isEnabled $ Se.Eq Nothing]]]
findByServiceTierTypeAndCityIdForUpdate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                           (Domain.Types.Common.ServiceTierType -> Kernel.Types.Id.Id Domain.Types.MerchantOperatingCity.MerchantOperatingCity -> m (Maybe Domain.Types.VehicleServiceTier.VehicleServiceTier))
findByServiceTierTypeAndCityIdForUpdate serviceTierType merchantOperatingCityId = do findOneWithKV [Se.And [Se.Is Beam.serviceTierType $ Se.Eq serviceTierType,
                                                                                                            Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId)]]
...
updateByPrimaryKey (Domain.Types.VehicleServiceTier.VehicleServiceTier {..}) = do {_now <- getCurrentTime;
                                                                                   updateWithKV [Se.Set Beam.airConditionedThreshold airConditionedThreshold,
...
instance FromTType' Beam.VehicleServiceTier Domain.Types.VehicleServiceTier.VehicleServiceTier
    where fromTType' (Beam.VehicleServiceTierT {..}) = do {vehicleIconUrl' <- ((Kernel.Prelude.maybe (return Kernel.Prelude.Nothing) (Kernel.Prelude.fmap Kernel.Prelude.Just . parseBaseUrl))) vehicleIconUrl;
                                                           pure $ Just Domain.Types.VehicleServiceTier.VehicleServiceTier{airConditionedThreshold = airConditionedThreshold,
                                                                                                                          ...
                                                                                                                          createdAt = createdAt,
                                                                                                                          updatedAt = updatedAt}}
instance ToTType' Beam.VehicleServiceTier Domain.Types.VehicleServiceTier.VehicleServiceTier
    where toTType' (Domain.Types.VehicleServiceTier.VehicleServiceTier {..}) = do Beam.VehicleServiceTierT{Beam.airConditionedThreshold = airConditionedThreshold,
