...
findByPlanIdAndLanguage :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateByPrimaryKey (Domain.Types.PlanTranslation.PlanTranslation {..}) = do {_now <- getCurrentTime;
                                                                             updateWithKV [Se.Set Beam.description description, Se.Set Beam.name name, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.language $ Se.Eq language,
                                                                                                                                                                                            Se.Is Beam.planId $ Se.Eq (Kernel.Types.Id.getId planId)]]}



instance FromTType' Beam.PlanTranslation Domain.Types.PlanTranslation.PlanTranslation
    where fromTType' (Beam.PlanTranslationT {..}) = do pure $ Just Domain.Types.PlanTranslation.PlanTranslation{description = description,
                                                                                                                language = language,
                                                                                                                name = name,
                                                                                                                planId = Kernel.Types.Id.Id planId,
                                                                                                                createdAt = createdAt,
                                                                                                                updatedAt = updatedAt}
instance ToTType' Beam.PlanTranslation Domain.Types.PlanTranslation.PlanTranslation
    where toTType' (Domain.Types.PlanTranslation.PlanTranslation {..}) = do Beam.PlanTranslationT{Beam.description = description,
