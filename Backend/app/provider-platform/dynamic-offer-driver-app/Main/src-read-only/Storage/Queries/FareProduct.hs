...
findAllFareProductByMerchantOpCityId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllFareProductByMerchantOpCityId merchantOperatingCityId enabled = do findAllWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                 Se.Is Beam.enabled $ Se.Eq enabled]]
findAllUnboundedByMerchantOpCityIdVariantArea :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllUnboundedByMerchantOpCityIdVariantArea merchantOperatingCityId area tripCategory vehicleServiceTier timeBounds enabled searchSource = do findAllWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
...
findAllUnboundedFareProductForArea :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllUnboundedFareProductForArea merchantOperatingCityId area timeBounds enabled searchSource = do findAllWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                            Se.Is Beam.area $ Se.Eq area,
                                                                                                                            Se.Is Beam.timeBounds $ Se.Eq timeBounds,
                                                                                                                            Se.Is Beam.enabled $ Se.Eq enabled,
                                                                                                                            Se.Is Beam.searchSource $ Se.In searchSource]]
findUnboundedByMerchantOpCityIdVariantArea :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                              (Kernel.Types.Id.Id Domain.Types.MerchantOperatingCity.MerchantOperatingCity -> Lib.Types.SpecialLocation.Area -> Domain.Types.Common.TripCategory -> Domain.Types.Common.ServiceTierType -> Kernel.Types.TimeBound.TimeBound -> Kernel.Prelude.Bool -> [Domain.Types.FareProduct.SearchSource] -> m (Maybe Domain.Types.FareProduct.FareProduct))
findUnboundedByMerchantOpCityIdVariantArea merchantOperatingCityId area tripCategory vehicleServiceTier timeBounds enabled searchSource = do findOneWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                                                                    Se.Is Beam.area $ Se.Eq area,
                                                                                                                                                                    Se.Is Beam.tripCategory $ Se.Eq tripCategory,
                                                                                                                                                                    Se.Is Beam.vehicleVariant $ Se.Eq vehicleServiceTier,
                                                                                                                                                                    Se.Is Beam.timeBounds $ Se.Eq timeBounds,
...
updateByPrimaryKey (Domain.Types.FareProduct.FareProduct {..}) = do updateWithKV [Se.Set Beam.area area,
                                                                                  ...
                                                                                  Se.Set Beam.vehicleVariant vehicleServiceTier] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]
