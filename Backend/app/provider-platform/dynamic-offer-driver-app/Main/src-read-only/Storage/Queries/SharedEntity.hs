...
updateDriverQuoteIds ::
  (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
  (Kernel.Prelude.Maybe [Domain.Types.TrackedEntity.TrackedEntity] -> Kernel.Types.Id.Id Domain.Types.SharedEntity.SharedEntity -> m ())
updateDriverQuoteIds driverQuoteIds id = do
  _now <- getCurrentTime
  updateOneWithKV [Se.Set Beam.driverQuoteIds (Data.Aeson.toJSON <$> driverQuoteIds), Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]
...
updateSearchRequestForDriverIds ::
  (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
  (Kernel.Prelude.Maybe [Domain.Types.TrackedEntity.TrackedEntity] -> Kernel.Types.Id.Id Domain.Types.SharedEntity.SharedEntity -> m ())
updateSearchRequestForDriverIds searchRequestForDriverIds id = do
  _now <- getCurrentTime
  updateOneWithKV [Se.Set Beam.searchRequestForDriverIds (Data.Aeson.toJSON <$> searchRequestForDriverIds), Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]
...
updateByPrimaryKey (Domain.Types.SharedEntity.SharedEntity {..}) = do
  ...
  updateWithKV
    [ Se.Set Beam.bookingIds (Data.Aeson.toJSON <$> bookingIds),
      ...
      Se.Set Beam.searchRequestForDriverIds (Data.Aeson.toJSON <$> searchRequestForDriverIds),
