...
findAllFeeByTypeServiceStatusAndDriver :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllFeeByTypeServiceStatusAndDriver serviceName driverId feeType status = do findAllWithKV [Se.And [Se.Is Beam.serviceName $ Se.Eq ((Just serviceName)),
...
updateAmountPaidByCoins amountPaidByCoin id = do {_now <- getCurrentTime;
...
updateAutopayPaymentStageById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateAutopayPaymentStageById autopayPaymentStage stageUpdatedAt id = do {_now <- getCurrentTime;
...
updateFeeWithoutDiscount feeWithoutDiscount id = do {_now <- getCurrentTime;
...
updateNotificationRetryCountById notificationRetryCount id = do {_now <- getCurrentTime;
...
updateOfferAndPlanDetails :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateOfferAndPlanDetails offerId planOfferTitle planId planMode id = do {_now <- getCurrentTime;
                                                                          updateOneWithKV [Se.Set Beam.offerId offerId,
...
updateRetryCount schedulerTryCount id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.DriverFee.DriverFee {..}) = do {_now <- getCurrentTime;
                                                                 updateWithKV [Se.Set Beam.addedToFeeId (Kernel.Types.Id.getId <$> addedToFeeId),
                                                                               ...
                                                                               Se.Set Beam.vehicleNumber vehicleNumber] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]}
