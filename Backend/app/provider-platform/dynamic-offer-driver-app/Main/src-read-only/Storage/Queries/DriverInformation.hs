...
addReferralCode :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
addReferralCode referralCode referredByDriverId driverId = do {_now <- getCurrentTime;
                                                               updateOneWithKV [Se.Set Beam.referralCode referralCode,
...
incrementReferralCountByPersonId totalReferred driverId = do {_now <- getCurrentTime;
...
removeAcUsageRestriction :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
removeAcUsageRestriction airConditionScore acUsageRestrictionType acRestrictionLiftCount driverId = do {_now <- getCurrentTime;
                                                                                                        updateOneWithKV [Se.Set Beam.airConditionScore airConditionScore,
...
updateAadhaarVerifiedState aadhaarVerified driverId = do {_now <- getCurrentTime;
...
updateAcUsageRestrictionAndScore :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateAcUsageRestrictionAndScore acUsageRestrictionType airConditionScore driverId = do {_now <- getCurrentTime;
                                                                                         updateOneWithKV [Se.Set Beam.acUsageRestrictionType (Kernel.Prelude.Just acUsageRestrictionType),
...
updateAirConditionScore airConditionScore driverId = do {_now <- getCurrentTime;
...
updateCompAadhaarImagePath compAadhaarImagePath driverId = do {_now <- getCurrentTime;
...
updateDailyAndWeeklyCancellationRateBlockingCooldown :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateDailyAndWeeklyCancellationRateBlockingCooldown dailyCancellationRateBlockingCooldown weeklyCancellationRateBlockingCooldown driverId = do {_now <- getCurrentTime;
                                                                                                                                                 updateOneWithKV [Se.Set Beam.dailyCancellationRateBlockingCooldown dailyCancellationRateBlockingCooldown,
                                                                                                                                                                  Se.Set Beam.weeklyCancellationRateBlockingCooldown weeklyCancellationRateBlockingCooldown,
                                                                                                                                                                  Se.Set Beam.updatedAt _now] [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]}
updateDailyAndWeeklyExtraKms :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateDailyAndWeeklyExtraKms dailyExtraKms weeklyExtraKms driverId = do {_now <- getCurrentTime;
...
updateDailyCancellationRateBlockingCooldown dailyCancellationRateBlockingCooldown driverId = do {_now <- getCurrentTime;
...
updateDriverAddress :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                       (Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Prelude.Maybe Domain.Types.DriverInformation.AddressDocumentType -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateDriverAddress address addressDocumentType driverId = do {_now <- getCurrentTime;
...
updateDriverBankAccountDetails :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateDriverBankAccountDetails driverBankAccountDetails driverId = do {_now <- getCurrentTime;
...
updateDriverDob driverDob driverId = do {_now <- getCurrentTime;
...
updateDriverDobAndAddress :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                             (Kernel.Prelude.Maybe Kernel.Prelude.UTCTime -> Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Prelude.Maybe Domain.Types.DriverInformation.AddressDocumentType -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateDriverDobAndAddress driverDob address addressDocumentType driverId = do {_now <- getCurrentTime;
                                                                               updateOneWithKV [Se.Set Beam.driverDob driverDob,
...
updateDrunkAndDriveViolationCount drunkAndDriveViolationCount driverId = do {_now <- getCurrentTime;
...
updateExtraFareMitigation extraFareMitigationFlag driverId = do {_now <- getCurrentTime;
...
updateForwardBatchingEnabled forwardBatchingEnabled driverId = do {_now <- getCurrentTime;
...
updateIsInteroperable isInteroperable driverId = do {_now <- getCurrentTime;
...
updateIssueBreachCooldownTimes :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateIssueBreachCooldownTimes issueBreachCooldownTimes driverId = do {_now <- getCurrentTime;
...
updateLastACStatusCheckedAt lastACStatusCheckedAt driverId = do {_now <- getCurrentTime;
...
updateLatestScheduledBookingAndPickup :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateLatestScheduledBookingAndPickup latestScheduledBooking latestScheduledPickup driverId = do {_now <- getCurrentTime;
                                                                                                  updateOneWithKV [Se.Set Beam.latestScheduledBooking latestScheduledBooking,
...
updateOnRideAndLatestScheduledBookingAndPickup :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateOnRideAndLatestScheduledBookingAndPickup onRide latestScheduledBooking latestScheduledPickup driverId = do {_now <- getCurrentTime;
                                                                                                                  updateOneWithKV [Se.Set Beam.onRide onRide,
...
updatePayoutRegAmountRefunded payoutRegAmountRefunded driverId = do {_now <- getCurrentTime;
...
updatePayoutRegistrationOrderId payoutRegistrationOrderId driverId = do {_now <- getCurrentTime;
...
updatePayoutVpaAndStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updatePayoutVpaAndStatus payoutVpa payoutVpaStatus driverId = do {_now <- getCurrentTime;
...
updatePayoutVpaStatus payoutVpaStatus driverId = do {_now <- getCurrentTime;
...
updatePendingPayment paymentPending driverId = do {_now <- getCurrentTime;
...
updateReferredByFleetOwnerId referredByFleetOwnerId driverId = do {_now <- getCurrentTime;
...
updateReferredByOperatorId referredByOperatorId driverId = do {_now <- getCurrentTime;
...
updateRentalInterCityAndIntraCitySwitch :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                           (Kernel.Prelude.Bool -> Kernel.Prelude.Bool -> Kernel.Prelude.Bool -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateRentalInterCityAndIntraCitySwitch canSwitchToRental canSwitchToInterCity canSwitchToIntraCity driverId = do {_now <- getCurrentTime;
                                                                                                                   updateOneWithKV [Se.Set Beam.canSwitchToRental (Kernel.Prelude.Just canSwitchToRental),
                                                                                                                                    Se.Set Beam.canSwitchToInterCity (Kernel.Prelude.Just canSwitchToInterCity),
                                                                                                                                    Se.Set Beam.canSwitchToIntraCity (Kernel.Prelude.Just canSwitchToIntraCity),
                                                                                                                                    Se.Set Beam.updatedAt _now] [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]}
...
updateServicesEnabledForSubscription servicesEnabledForSubscription driverId = do {_now <- getCurrentTime;
                                                                                   updateOneWithKV [Se.Set Beam.servicesEnabledForSubscription (Kernel.Prelude.Just servicesEnabledForSubscription),
                                                                                                    Se.Set Beam.updatedAt _now] [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]}
updateSoftBlock :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateSoftBlock softBlockStiers softBlockExpiryTime softBlockReasonFlag driverId = do {_now <- getCurrentTime;
                                                                                       updateOneWithKV [Se.Set Beam.softBlockStiers softBlockStiers,
                                                                                                        Se.Set Beam.softBlockExpiryTime softBlockExpiryTime,
                                                                                                        Se.Set Beam.softBlockReasonFlag softBlockReasonFlag,
                                                                                                        Se.Set Beam.updatedAt _now] [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]}
updateSpecialLocWarriorInfo :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateSpecialLocWarriorInfo isSpecialLocWarrior preferredPrimarySpecialLocId preferredSecondarySpecialLocIds specialLocWarriorEnabledAt driverId = do {_now <- getCurrentTime;
                                                                                                                                                       updateOneWithKV [Se.Set Beam.isSpecialLocWarrior (Kernel.Prelude.Just isSpecialLocWarrior),
...
updateSubscription subscribed driverId = do {_now <- getCurrentTime;
...
updateTollRelatedIssueCount tollRelatedIssueCount driverId = do {_now <- getCurrentTime;
...
updateTollRouteBlockedTill tollRouteBlockedTill driverId = do {_now <- getCurrentTime;
...
updateTripEndLocation driverTripEndLocation driverId = do {_now <- getCurrentTime;
                                                           updateOneWithKV [Se.Set Beam.driverTripEndLocationLat (Kernel.Prelude.fmap (.lat) driverTripEndLocation),
...
updateUpgradedTiers ruleBasedUpgradeTiers driverId = do {_now <- getCurrentTime;
...
updateWeeklyCancellationRateBlockingCooldown weeklyCancellationRateBlockingCooldown driverId = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.DriverInformation.DriverInformation {..}) = do {_now <- getCurrentTime;
                                                                                 updateWithKV [Se.Set Beam.aadhaarNumberEncrypted (Storage.Queries.Transformers.FleetOwnerInformation.mkFieldEncrypted aadhaarNumber),
