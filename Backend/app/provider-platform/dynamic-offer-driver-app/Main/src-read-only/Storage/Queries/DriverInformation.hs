...
updateApprovedAndVerified :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => (Kernel.Prelude.Maybe Kernel.Prelude.Bool -> Kernel.Prelude.Bool -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateApprovedAndVerified approved verified driverId = do
  _now <- getCurrentTime
  updateOneWithKV [Se.Set Beam.approved approved, Se.Set Beam.verified verified, Se.Set Beam.updatedAt _now] [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]
