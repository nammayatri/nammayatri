...
updateBusinessLicenseImage businessLicenseImageId fleetOwnerPersonId = do {_now <- getCurrentTime;
...
updateFleetOwnerEnabledStatus enabled fleetOwnerPersonId = do {_now <- getCurrentTime;
...
updateFleetOwnerVerifiedStatus verified fleetOwnerPersonId = do {_now <- getCurrentTime;
...
updatePayoutRegistrationOrderId payoutRegistrationOrderId fleetOwnerPersonId = do {_now <- getCurrentTime;
                                                                                   updateOneWithKV [Se.Set Beam.payoutRegistrationOrderId payoutRegistrationOrderId, Se.Set Beam.updatedAt _now] [Se.Is Beam.fleetOwnerPersonId $ Se.Eq (Kernel.Types.Id.getId fleetOwnerPersonId)]}
...
updateReferredByOperatorId referredByOperatorId fleetOwnerPersonId = do {_now <- getCurrentTime;
...
updateRegistration registeredAt fleetOwnerPersonId = do {_now <- getCurrentTime;
...
updateTdsRate tdsRate fleetOwnerPersonId = do {_now <- getCurrentTime;
