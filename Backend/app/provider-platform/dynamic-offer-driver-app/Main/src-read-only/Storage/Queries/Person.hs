...
findAllByPersonIdsAndMerchantOpsCityId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByPersonIdsAndMerchantOpsCityId id merchantOperatingCityId = do findAllWithKV [Se.And [Se.Is Beam.id $ Se.In (Kernel.Types.Id.getId <$> id),
                                                                                              Se.Is Beam.merchantOperatingCityId $ Se.Eq (Just $ Kernel.Types.Id.getId merchantOperatingCityId)]]
findByEmailAndMerchant :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByIdAndRoleAndMerchantId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByIdAndRoleAndMerchantId id role merchantId = do findOneWithKV [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id),
                                                                            Se.Is Beam.role $ Se.Eq role,
                                                                            Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId merchantId)]]
findByIdentifierAndMerchant :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                               (Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Types.Id.Id Domain.Types.Merchant.Merchant -> m (Maybe Domain.Types.Person.Person))
...
updateDeviceToken :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateDriverTag driverTag id = do {_now <- getCurrentTime;
...
updateNyClubConsent nyClubConsent id = do {_now <- getCurrentTime;
...
updateTotalEarnedCoins totalEarnedCoins id = do {_now <- getCurrentTime;
...
updateWhatsappNotificationEnrollStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateWhatsappNotificationEnrollStatus whatsappNotificationEnrollStatus id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.Person.Person {..}) = do {_now <- getCurrentTime;
                                                           updateWithKV [Se.Set Beam.alternateMobileNumberEncrypted (((alternateMobileNumber <&> unEncrypted . (.encrypted)))),
                                                                         ...
                                                                         Se.Set Beam.whatsappNotificationEnrollStatus whatsappNotificationEnrollStatus] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]}
