...
findAllByMerchantOpCityIdAndRcIdAndType :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByMerchantOpCityIdAndRcIdAndType limit offset merchantOperatingCityId rcId mediaFileDocumentType = do findAllWithOptionsKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                                           Se.Is Beam.rcId $ Se.Eq (Kernel.Types.Id.getId rcId),
                                                                                                                                           Se.Is Beam.mediaFileDocumentType $ Se.Eq mediaFileDocumentType]] (Se.Desc Beam.createdAt) limit offset
findOneByCityRcTypeAndStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findOneByCityRcTypeAndStatus merchantOperatingCityId rcId mediaFileDocumentType status = do findOneWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                   Se.Is Beam.rcId $ Se.Eq (Kernel.Types.Id.getId rcId),
...
updateStatus status fileHash id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.MediaFileDocument.MediaFileDocument {..}) = do {_now <- getCurrentTime;
                                                                                 updateWithKV [Se.Set Beam.creatorId (Kernel.Types.Id.getId creatorId),
