...
findByModuleCompletionIdAndDriverIdAndModuleId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByModuleCompletionIdAndDriverIdAndModuleId moduleCompletionId driverId moduleId = do findOneWithKV [Se.And [Se.Is Beam.moduleCompletionId $ Se.Eq moduleCompletionId,
...
updateByPrimaryKey (Domain.Types.LmsCertificate.LmsCertificate {..}) = do {_now <- getCurrentTime;
...
instance FromTType' Beam.LmsCertificate Domain.Types.LmsCertificate.LmsCertificate
    where fromTType' (Beam.LmsCertificateT {..}) = do pure $ Just Domain.Types.LmsCertificate.LmsCertificate{driverId = Kernel.Types.Id.Id driverId,
                                                                                                             id = Kernel.Types.Id.Id id,
                                                                                                             moduleCompletionId = moduleCompletionId,
                                                                                                             moduleId = Kernel.Types.Id.Id moduleId,
                                                                                                             createdAt = createdAt,
                                                                                                             updatedAt = updatedAt}
instance ToTType' Beam.LmsCertificate Domain.Types.LmsCertificate.LmsCertificate
    where toTType' (Domain.Types.LmsCertificate.LmsCertificate {..}) = do Beam.LmsCertificateT{Beam.driverId = Kernel.Types.Id.getId driverId,
