...
deleteBySopTypeAndMerchantOperatingCityId sopType merchantOperatingCityId = do
  deleteWithKV
    [ Se.And
        [ Se.Is Beam.sopType $ Se.Eq sopType,
...
findAllBySopTypeAndMerchantOperatingCityId ::
...
findAllBySopTypeAndMerchantOperatingCityId limit offset sopType merchantOperatingCityId = do
...
updateByPrimaryKey (Domain.Types.KnowledgeCenter.KnowledgeCenter {..}) = do
  _now <- getCurrentTime
  updateWithKV
    [ Se.Set Beam.documentName documentName,
