...
findByRcIdAndDriverId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateVerificationStatusAndRejectReasonByRcId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateVerificationStatusAndRejectReasonByRcId verificationStatus rejectReason rcId = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.VehicleInsurance.VehicleInsurance {..}) = do {_now <- getCurrentTime;
                                                                               updateWithKV [Se.Set Beam.documentImageId (Kernel.Types.Id.getId documentImageId),
