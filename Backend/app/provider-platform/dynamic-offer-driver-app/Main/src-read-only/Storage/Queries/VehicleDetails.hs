...
findByMakeAndModelAndYear :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateByPrimaryKey (Domain.Types.VehicleDetails.VehicleDetails {..}) = do updateWithKV [Se.Set Beam.acAvailable acAvailable,
                                                                                        Se.Set Beam.capacity capacity,
                                                                                        Se.Set Beam.make make,
                                                                                        Se.Set Beam.model model,
                                                                                        Se.Set Beam.vehicleVariant vehicleVariant,
                                                                                        Se.Set Beam.year year] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]
