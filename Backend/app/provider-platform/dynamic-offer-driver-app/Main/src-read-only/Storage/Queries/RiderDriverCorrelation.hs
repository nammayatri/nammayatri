...
checkRiderFavDriver :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
checkRiderFavDriver riderDetailId driverId favourite = do findOneWithKV [Se.And [Se.Is Beam.riderDetailId $ Se.Eq (Kernel.Types.Id.getId riderDetailId),
                                                                                 Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
                                                                                 Se.Is Beam.favourite $ Se.Eq favourite]]
findByRiderIdAndDriverId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByRiderIdAndDriverId riderDetailId driverId = do findOneWithKV [Se.And [Se.Is Beam.riderDetailId $ Se.Eq (Kernel.Types.Id.getId riderDetailId),
                                                                            Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]]
findFavDriversForRider :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateFavouriteDriverForRider :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateFavouriteDriverForRider favourite riderDetailId driverId = do {_now <- getCurrentTime;
                                                                     updateOneWithKV [Se.Set Beam.favourite favourite, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.riderDetailId $ Se.Eq (Kernel.Types.Id.getId riderDetailId),
                                                                                                                                                            Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]]}
...
findByPrimaryKey driverId riderDetailId = do findOneWithKV [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
...
updateByPrimaryKey (Domain.Types.RiderDriverCorrelation.RiderDriverCorrelation {..}) = do {_now <- getCurrentTime;
                                                                                           updateWithKV [Se.Set Beam.favourite favourite,
                                                                                                         ...
                                                                                                         Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId), Se.Is Beam.riderDetailId $ Se.Eq (Kernel.Types.Id.getId riderDetailId)]]}



instance FromTType' Beam.RiderDriverCorrelation Domain.Types.RiderDriverCorrelation.RiderDriverCorrelation
    where fromTType' (Beam.RiderDriverCorrelationT {..}) = do pure $ Just Domain.Types.RiderDriverCorrelation.RiderDriverCorrelation{createdAt = createdAt,
                                                                                                                                     ...
                                                                                                                                     updatedAt = updatedAt}
instance ToTType' Beam.RiderDriverCorrelation Domain.Types.RiderDriverCorrelation.RiderDriverCorrelation
    where toTType' (Domain.Types.RiderDriverCorrelation.RiderDriverCorrelation {..}) = do Beam.RiderDriverCorrelationT{Beam.createdAt = createdAt,
