...
updateD2cReferralCount d2cReferralCount totalReferralCounts driverId = do {_now <- getCurrentTime;
                                                                           updateOneWithKV [Se.Set Beam.d2cReferralCount (Kernel.Prelude.Just d2cReferralCount),
...
updateD2dReferralCount d2dReferralCount totalReferralCounts driverId = do {_now <- getCurrentTime;
                                                                           updateOneWithKV [Se.Set Beam.d2dReferralCount (Kernel.Prelude.Just d2dReferralCount),
                                                                                            Se.Set Beam.totalReferralCounts (Kernel.Prelude.Just totalReferralCounts),
                                                                                            Se.Set Beam.updatedAt _now] [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]}
...
updateNumDriversOnboarded numDriversOnboarded driverId = do {_now <- getCurrentTime;
...
updateNumFleetsOnboarded numFleetsOnboarded driverId = do {_now <- getCurrentTime;
...
updateOnlineDuration onlineDuration driverId = do {_now <- getCurrentTime;
...
updatePayoutEarningsByDriverId totalPayoutEarnings driverId = do {_now <- getCurrentTime;
...
updateTotalPayoutAmountPaid totalPayoutAmountPaid driverId = do {_now <- getCurrentTime;
...
updateTotalReferralCount totalReferralCounts driverId = do {_now <- getCurrentTime;
...
updateTotalValidRidesAndPayoutEarnings :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                          (Kernel.Prelude.Int -> Kernel.Types.Common.HighPrecMoney -> Kernel.Types.Id.Id Domain.Types.Person.Driver -> m ())
updateTotalValidRidesAndPayoutEarnings totalValidActivatedRides totalPayoutEarnings driverId = do {_now <- getCurrentTime;
                                                                                                   updateOneWithKV [Se.Set Beam.totalValidActivatedRides (Kernel.Prelude.Just totalValidActivatedRides),
...
updateValidCustomerCancellationTagCount validCustomerCancellationTagCount driverId = do {_now <- getCurrentTime;
                                                                                         updateOneWithKV [Se.Set Beam.validCustomerCancellationTagCount (Kernel.Prelude.Just validCustomerCancellationTagCount),
                                                                                                          Se.Set Beam.updatedAt _now] [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]}
...
updateValidDriverCancellationTagCount validDriverCancellationTagCount driverId = do {_now <- getCurrentTime;
                                                                                     updateOneWithKV [Se.Set Beam.validDriverCancellationTagCount (Kernel.Prelude.Just validDriverCancellationTagCount),
                                                                                                      Se.Set Beam.updatedAt _now] [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]}
...
updateByPrimaryKey (Domain.Types.DriverStats.DriverStats {..}) = do {_now <- getCurrentTime;
                                                                     updateWithKV [Se.Set Beam.blacklistCoinEvents blacklistCoinEvents,
                                                                                   ...
                                                                                   Se.Set Beam.validDriverCancellationTagCount (Kernel.Prelude.Just validDriverCancellationTagCount)] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]]}
