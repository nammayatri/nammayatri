...
updateCallError :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateCallError callError callService merchantId id = do updateWithKV [Se.Set Beam.callError callError,
                                                                       Se.Set Beam.callService callService,
                                                                       Se.Set Beam.merchantId merchantId] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]
updateCallStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateCallStatus conversationDuration recordingUrl status callAttempt id = do updateWithKV [Se.Set Beam.conversationDuration conversationDuration,
                                                                                            Se.Set Beam.recordingUrl recordingUrl,
                                                                                            Se.Set Beam.status status,
                                                                                            Se.Set Beam.callAttempt callAttempt] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]
...
updateCallStatusInformation :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateCallStatusInformation dtmfNumberUsed merchantId callService id = do updateWithKV [Se.Set Beam.dtmfNumberUsed dtmfNumberUsed,
                                                                                        Se.Set Beam.merchantId merchantId,
                                                                                        Se.Set Beam.callService callService] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]
...
updateByPrimaryKey (Domain.Types.CallStatus.CallStatus {..}) = do updateWithKV [Se.Set Beam.aiCallAnalyzed aiCallAnalyzed,
                                                                                ...
                                                                                Se.Set Beam.recordingUrl recordingUrl,
                                                                                Se.Set Beam.status status] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]
