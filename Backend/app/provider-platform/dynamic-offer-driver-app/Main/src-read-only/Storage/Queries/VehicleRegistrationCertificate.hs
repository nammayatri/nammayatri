...
findAllByFleetOwnerId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByFleetOwnerIds :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                          (Maybe Int -> Maybe Int -> [Kernel.Prelude.Maybe Kernel.Prelude.Text] -> m ([Domain.Types.VehicleRegistrationCertificate.VehicleRegistrationCertificate]))
...
findByRCIdAndFleetOwnerId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateAirConditioned airConditioned id = do {_now <- getCurrentTime;
...
updateApproved :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                  (Kernel.Prelude.Maybe Kernel.Prelude.Bool -> Kernel.Types.Id.Id Domain.Types.VehicleRegistrationCertificate.VehicleRegistrationCertificate -> m ())
...
updateFleetOwnerId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateManufacturing :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateManufacturing mYManufacturing id = do {_now <- getCurrentTime;
...
updateMerchantIdAndCityIdById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateMerchantIdAndCityIdById merchantId merchantOperatingCityId id = do {_now <- getCurrentTime;
...
updateOxygen :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                (Kernel.Prelude.Maybe Kernel.Prelude.Bool -> Kernel.Types.Id.Id Domain.Types.VehicleRegistrationCertificate.VehicleRegistrationCertificate -> m ())
...
updateVehicleImageId vehicleImageId id = do {_now <- getCurrentTime;
...
updateVentilator :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                    (Kernel.Prelude.Maybe Kernel.Prelude.Bool -> Kernel.Types.Id.Id Domain.Types.VehicleRegistrationCertificate.VehicleRegistrationCertificate -> m ())
...
updateVerificationStatus verificationStatus documentImageId = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.VehicleRegistrationCertificate.VehicleRegistrationCertificate {..}) = do {_now <- getCurrentTime;
                                                                                                           updateWithKV [Se.Set Beam.airConditioned airConditioned,
