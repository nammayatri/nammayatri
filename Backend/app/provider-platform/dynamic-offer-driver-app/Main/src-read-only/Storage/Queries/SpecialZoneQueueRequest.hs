...
findActiveByDriverId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateResponse :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateResponse response status id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.SpecialZoneQueueRequest.SpecialZoneQueueRequest {..}) = do {_now <- getCurrentTime;
