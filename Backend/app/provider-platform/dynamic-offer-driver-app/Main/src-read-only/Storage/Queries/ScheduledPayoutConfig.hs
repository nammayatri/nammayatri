...
findAllByMerchantOpCityIdAndIsEnabled :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByMerchantOpCityIdAndIsEnabled merchantOperatingCityId isEnabled = do findAllWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                    Se.Is Beam.isEnabled $ Se.Eq isEnabled]]
findByMerchantOpCityIdAndCategory :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByMerchantOpCityIdAndCategory merchantOperatingCityId payoutCategory = do findOneWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                     Se.Is Beam.payoutCategory $ Se.Eq payoutCategory]]
findByPrimaryKey :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                    (Kernel.Types.Id.Id Domain.Types.MerchantOperatingCity.MerchantOperatingCity -> Lib.Payment.Domain.Types.Common.EntityName -> m (Maybe Domain.Types.ScheduledPayoutConfig.ScheduledPayoutConfig))
findByPrimaryKey merchantOperatingCityId payoutCategory = do findOneWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                    Se.Is Beam.payoutCategory $ Se.Eq payoutCategory]]
updateByPrimaryKey :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => (Domain.Types.ScheduledPayoutConfig.ScheduledPayoutConfig -> m ())
updateByPrimaryKey (Domain.Types.ScheduledPayoutConfig.ScheduledPayoutConfig {..}) = do {_now <- getCurrentTime;
                                                                                         updateWithKV [Se.Set Beam.batchSize batchSize,
                                                                                                       ...
                                                                                                       Se.Set Beam.vehicleCategory vehicleCategory] [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId), Se.Is Beam.payoutCategory $ Se.Eq payoutCategory]]}



instance FromTType' Beam.ScheduledPayoutConfig Domain.Types.ScheduledPayoutConfig.ScheduledPayoutConfig
    where fromTType' (Beam.ScheduledPayoutConfigT {..}) = do pure $ Just Domain.Types.ScheduledPayoutConfig.ScheduledPayoutConfig{batchSize = batchSize,
                                                                                                                                  createdAt = createdAt,
...
instance ToTType' Beam.ScheduledPayoutConfig Domain.Types.ScheduledPayoutConfig.ScheduledPayoutConfig
    where toTType' (Domain.Types.ScheduledPayoutConfig.ScheduledPayoutConfig {..}) = do Beam.ScheduledPayoutConfigT{Beam.batchSize = batchSize,
