...
findActiveByOwner :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByOwnerAndStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateReconciliationStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                              (Kernel.Prelude.Maybe Data.Aeson.Value -> Kernel.Types.Id.Id Domain.Types.SubscriptionPurchase.SubscriptionPurchase -> m ())
updateReconciliationStatus reconciliationStatus id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.SubscriptionPurchase.SubscriptionPurchase {..}) = do {_now <- getCurrentTime;
                                                                                       updateWithKV [Se.Set Beam.enableServiceUsageCharge enableServiceUsageCharge,
