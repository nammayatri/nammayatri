...
updatePanAadhaarLinkage :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => (Kernel.Prelude.Maybe Domain.Types.DriverPanCard.PanAadhaarLinkage -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updatePanAadhaarLinkage panAadhaarLinkage driverId = do {_now <- getCurrentTime;
                                                         updateOneWithKV [Se.Set Beam.panAadhaarLinkage panAadhaarLinkage, Se.Set Beam.updatedAt _now] [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId)]}
...
updateVerificationStatus verificationStatus driverId = do {_now <- getCurrentTime;
...
updateVerificationStatusByImageId verificationStatus documentImageId1 = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.DriverPanCard.DriverPanCard {..}) = do {_now <- getCurrentTime;
                                                                         updateWithKV [Se.Set Beam.consent consent,
