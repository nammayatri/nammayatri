...
updateByPrimaryKey (Domain.Types.ValueAddNP.ValueAddNP {..}) = do
  _now <- getCurrentTime
  updateWithKV [Se.Set Beam.enableOneShotAssign enableOneShotAssign, Se.Set Beam.enabled enabled, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.subscriberId $ Se.Eq subscriberId]]
    ...
    pure $
      Just
        Domain.Types.ValueAddNP.ValueAddNP
          { enableOneShotAssign = enableOneShotAssign,
            enabled = enabled,
...
instance ToTType' Beam.ValueAddNP Domain.Types.ValueAddNP.ValueAddNP where
  toTType' (Domain.Types.ValueAddNP.ValueAddNP {..}) = do
    Beam.ValueAddNPT
      { Beam.enableOneShotAssign = enableOneShotAssign,
