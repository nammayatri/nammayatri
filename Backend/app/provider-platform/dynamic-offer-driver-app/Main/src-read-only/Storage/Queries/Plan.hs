...
findByCityServiceAndVehicle :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByCityServiceAndVehicle merchantOpCityId serviceName vehicleCategory isDeprecated = do findAllWithKV [Se.And [Se.Is Beam.merchantOpCityId $ Se.Eq (Kernel.Types.Id.getId merchantOpCityId),
...
findByIdAndPaymentModeWithServiceName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByIdAndPaymentModeWithServiceName id paymentMode serviceName = do findOneWithKV [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id),
                                                                                             Se.Is Beam.paymentMode $ Se.Eq paymentMode,
                                                                                             Se.Is Beam.serviceName $ Se.Eq serviceName]]
findByMerchantOpCityAndServiceAndFleetOwnerPlan :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByMerchantOpCityAndServiceAndFleetOwnerPlan merchantOpCityId serviceName isFleetOwnerPlan = do findAllWithKV [Se.And [Se.Is Beam.merchantOpCityId $ Se.Eq (Kernel.Types.Id.getId merchantOpCityId),
                                                                                                                          Se.Is Beam.serviceName $ Se.Eq serviceName,
                                                                                                                          Se.Is Beam.isFleetOwnerPlan $ Se.Eq isFleetOwnerPlan]]
findByMerchantOpCityIdAndTypeWithServiceName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByMerchantOpCityIdAndTypeWithServiceName merchantOpCityId planType serviceName vehicleCategory isDeprecated = do findAllWithKV [Se.And [Se.Is Beam.merchantOpCityId $ Se.Eq (Kernel.Types.Id.getId merchantOpCityId),
                                                                                                                                            Se.Is Beam.planType $ Se.Eq planType,
                                                                                                                                            Se.Is Beam.serviceName $ Se.Eq serviceName,
                                                                                                                                            Se.Is Beam.vehicleCategory $ Se.Eq (Kernel.Prelude.Just vehicleCategory),
                                                                                                                                            Se.Is Beam.isDeprecated $ Se.Eq isDeprecated]]
findByMerchantOpCityIdAndTypeWithServiceNameAndVariant :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByMerchantOpCityIdAndTypeWithServiceNameAndVariant merchantOpCityId paymentMode serviceName vehicleVariant = do findAllWithKV [Se.And [Se.Is Beam.merchantOpCityId $ Se.Eq (Kernel.Types.Id.getId merchantOpCityId),
                                                                                                                                           Se.Is Beam.paymentMode $ Se.Eq paymentMode,
                                                                                                                                           Se.Is Beam.serviceName $ Se.Eq serviceName,
...
findByMerchantOpCityIdTypeServiceNameVehicle merchantOpCityId paymentMode serviceName vehicleCategory isDeprecated = do findAllWithKV [Se.And [Se.Is Beam.merchantOpCityId $ Se.Eq (Kernel.Types.Id.getId merchantOpCityId),
                                                                                                                                               Se.Is Beam.paymentMode $ Se.Eq paymentMode,
                                                                                                                                               Se.Is Beam.serviceName $ Se.Eq serviceName,
                                                                                                                                               Se.Is Beam.vehicleCategory $ Se.Eq (Kernel.Prelude.Just vehicleCategory),
                                                                                                                                               Se.Is Beam.isDeprecated $ Se.Eq isDeprecated]]
findByMerchantOpCityIdWithServiceName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByMerchantOpCityIdWithServiceName merchantOpCityId serviceName = do findAllWithKV [Se.And [Se.Is Beam.merchantOpCityId $ Se.Eq (Kernel.Types.Id.getId merchantOpCityId),
                                                                                               Se.Is Beam.serviceName $ Se.Eq serviceName]]
...
updateByPrimaryKey (Domain.Types.Plan.Plan {..}) = do updateWithKV [Se.Set Beam.allowStrikeOff (Kernel.Prelude.Just allowStrikeOff),
