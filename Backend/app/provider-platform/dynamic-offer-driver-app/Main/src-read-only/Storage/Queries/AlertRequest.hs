...
updateStatusWithReason :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateStatusWithReason status reason id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.AlertRequest.AlertRequest {..}) = do {_now <- getCurrentTime;
                                                                       updateWithKV [Se.Set Beam.body body,
...
instance FromTType' Beam.AlertRequest Domain.Types.AlertRequest.AlertRequest
    where fromTType' (Beam.AlertRequestT {..}) = do pure $ Just Domain.Types.AlertRequest.AlertRequest{body = body,
                                                                                                       createdAt = createdAt,
                                                                                                       ...
                                                                                                       title = title,
                                                                                                       updatedAt = updatedAt}
instance ToTType' Beam.AlertRequest Domain.Types.AlertRequest.AlertRequest
    where toTType' (Domain.Types.AlertRequest.AlertRequest {..}) = do Beam.AlertRequestT{Beam.body = body,
