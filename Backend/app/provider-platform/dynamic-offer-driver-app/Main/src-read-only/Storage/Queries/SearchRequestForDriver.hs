...
findAllActiveBySRId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllActiveBySTId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateDriverResponse :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateDriverResponse response status notificationSource renderedAt respondedAt id = do {_now <- getCurrentTime;
                                                                                        updateOneWithKV [Se.Set Beam.response response,
                                                                                                         ...
                                                                                                         Se.Set Beam.updatedAt (Just _now)] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]}
...
updateByPrimaryKey (Domain.Types.SearchRequestForDriver.SearchRequestForDriver {..}) = do {_now <- getCurrentTime;
                                                                                           updateWithKV [Se.Set Beam.acceptanceRatio acceptanceRatio,
                                                                                                         ...
                                                                                                         Se.Set Beam.vehicleVariant vehicleVariant] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]}
