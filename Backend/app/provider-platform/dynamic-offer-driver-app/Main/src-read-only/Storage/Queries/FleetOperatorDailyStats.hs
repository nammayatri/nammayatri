...
updateCustomerCancellationCountByFleetOperatorIdAndDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                                           (Kernel.Prelude.Maybe Kernel.Prelude.Int -> Kernel.Prelude.Text -> Kernel.Prelude.Text -> Data.Time.Calendar.Day -> m ())
updateCustomerCancellationCountByFleetOperatorIdAndDate customerCancellationCount fleetOperatorId fleetDriverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                                                                                        updateOneWithKV [Se.Set Beam.customerCancellationCount customerCancellationCount,
...
updateDistanceEarningAndCompletedRidesByFleetOperatorIdAndDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateDistanceEarningAndCompletedRidesByFleetOperatorIdAndDate totalDistance onlineTotalEarning cashTotalEarning totalCompletedRides cashPlatformFees onlinePlatformFees rideDuration fleetOperatorId fleetDriverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                                                                                                                                                                                            updateOneWithKV [Se.Set Beam.totalDistance (getTotalDistance totalDistance),
                                                                                                                                                                                                                                                             ...
                                                                                                                                                                                                                                                             Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.fleetOperatorId $ Se.Eq fleetOperatorId,
                                                                                                                                                                                                                                                                                                  Se.Is Beam.fleetDriverId $ Se.Eq fleetDriverId,
                                                                                                                                                                                                                                                                                                  Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}
updateDriverCancellationCountByFleetOperatorIdAndDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                                         (Kernel.Prelude.Maybe Kernel.Prelude.Int -> Kernel.Prelude.Text -> Kernel.Prelude.Text -> Data.Time.Calendar.Day -> m ())
updateDriverCancellationCountByFleetOperatorIdAndDate driverCancellationCount fleetOperatorId fleetDriverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                                                                                    updateOneWithKV [Se.Set Beam.driverCancellationCount driverCancellationCount, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.fleetOperatorId $ Se.Eq fleetOperatorId,
                                                                                                                                                                                                                                                       Se.Is Beam.fleetDriverId $ Se.Eq fleetDriverId,
                                                                                                                                                                                                                                                       Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}
updateDriverFirstSubscriptionByFleetOperatorIdAndDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                                         (Kernel.Prelude.Maybe Kernel.Prelude.Int -> Kernel.Prelude.Text -> Kernel.Prelude.Text -> Data.Time.Calendar.Day -> m ())
updateDriverFirstSubscriptionByFleetOperatorIdAndDate driverFirstSubscription fleetOperatorId fleetDriverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                                                                                    updateOneWithKV [Se.Set Beam.driverFirstSubscription driverFirstSubscription, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.fleetOperatorId $ Se.Eq fleetOperatorId,
                                                                                                                                                                                                                                                       Se.Is Beam.fleetDriverId $ Se.Eq fleetDriverId,
                                                                                                                                                                                                                                                       Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}
updateInspectionCompletedByFleetOperatorIdAndDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                                     (Kernel.Prelude.Maybe Kernel.Prelude.Int -> Kernel.Prelude.Text -> Kernel.Prelude.Text -> Data.Time.Calendar.Day -> m ())
updateInspectionCompletedByFleetOperatorIdAndDate inspectionCompleted fleetOperatorId fleetDriverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                                                                            updateOneWithKV [Se.Set Beam.inspectionCompleted inspectionCompleted, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.fleetOperatorId $ Se.Eq fleetOperatorId,
                                                                                                                                                                                                                                       Se.Is Beam.fleetDriverId $ Se.Eq fleetDriverId,
                                                                                                                                                                                                                                       Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}
updateOnlineDurationByFleetOperatorIdAndDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                                (Kernel.Prelude.Maybe Kernel.Types.Common.Seconds -> Kernel.Prelude.Text -> Kernel.Prelude.Text -> Data.Time.Calendar.Day -> m ())
updateOnlineDurationByFleetOperatorIdAndDate onlineDuration fleetOperatorId fleetDriverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                                                                  updateOneWithKV [Se.Set Beam.onlineDuration onlineDuration, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.fleetOperatorId $ Se.Eq fleetOperatorId,
                                                                                                                                                                                                                   Se.Is Beam.fleetDriverId $ Se.Eq fleetDriverId,
                                                                                                                                                                                                                   Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}
updateRequestCountsByFleetOperatorIdAndDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateRequestCountsByFleetOperatorIdAndDate rejectedRequestCount pulledRequestCount acceptationRequestCount totalRequestCount fleetOperatorId fleetDriverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                                                                                                                                    updateOneWithKV [Se.Set Beam.rejectedRequestCount rejectedRequestCount,
                                                                                                                                                                                                     ...
                                                                                                                                                                                                     Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.fleetOperatorId $ Se.Eq fleetOperatorId,
                                                                                                                                                                                                                                          Se.Is Beam.fleetDriverId $ Se.Eq fleetDriverId,
                                                                                                                                                                                                                                          Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}
updateTotalRatingCountAndTotalRatingScoreByFleetOperatorIdAndDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateTotalRatingCountAndTotalRatingScoreByFleetOperatorIdAndDate totalRatingCount totalRatingScore fleetOperatorId fleetDriverId merchantLocalDate = do {_now <- getCurrentTime;
...
findByPrimaryKey fleetDriverId fleetOperatorId merchantLocalDate = do findOneWithKV [Se.And [Se.Is Beam.fleetDriverId $ Se.Eq fleetDriverId,
...
updateByPrimaryKey (Domain.Types.FleetOperatorDailyStats.FleetOperatorDailyStats {..}) = do {_now <- getCurrentTime;
                                                                                             updateWithKV [Se.Set Beam.acceptationRequestCount acceptationRequestCount,
                                                                                                           ...
                                                                                                           Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.fleetDriverId $ Se.Eq fleetDriverId,
                                                                                                                                                Se.Is Beam.fleetOperatorId $ Se.Eq fleetOperatorId,
                                                                                                                                                Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}



instance FromTType' Beam.FleetOperatorDailyStats Domain.Types.FleetOperatorDailyStats.FleetOperatorDailyStats
    where fromTType' (Beam.FleetOperatorDailyStatsT {..}) = do pure $ Just Domain.Types.FleetOperatorDailyStats.FleetOperatorDailyStats{acceptationRequestCount = acceptationRequestCount,
                                                                                                                                        ...
                                                                                                                                        createdAt = createdAt,
                                                                                                                                        updatedAt = updatedAt}
instance ToTType' Beam.FleetOperatorDailyStats Domain.Types.FleetOperatorDailyStats.FleetOperatorDailyStats
    where toTType' (Domain.Types.FleetOperatorDailyStats.FleetOperatorDailyStats {..}) = do Beam.FleetOperatorDailyStatsT{Beam.acceptationRequestCount = acceptationRequestCount,
