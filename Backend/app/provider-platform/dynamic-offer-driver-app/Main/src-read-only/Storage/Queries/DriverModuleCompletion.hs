...
findAllByDriverIdAndModuleId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByDriverIdAndModuleId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByDriverIdAndStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateEntitiesCompleted :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateEntitiesCompleted completedAt entitiesCompleted completionId = do {_now <- getCurrentTime;
                                                                         updateOneWithKV [Se.Set Beam.completedAt completedAt,
                                                                                          Se.Set Beam.entitiesCompleted entitiesCompleted,
                                                                                          Se.Set Beam.updatedAt _now] [Se.Is Beam.completionId $ Se.Eq (Kernel.Types.Id.getId completionId)]}
updateExpiryTime :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateExpiryTime expiry completionId = do {_now <- getCurrentTime;
...
updatedCompletedAt :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updatedCompletedAt completedAt entitiesCompleted status ratingAtTheTimeOfCompletion completionId = do {_now <- getCurrentTime;
                                                                                                       updateOneWithKV [Se.Set Beam.completedAt completedAt,
                                                                                                                        Se.Set Beam.entitiesCompleted entitiesCompleted,
...
updateByPrimaryKey (Domain.Types.DriverModuleCompletion.DriverModuleCompletion {..}) = do {_now <- getCurrentTime;
                                                                                           updateWithKV [Se.Set Beam.completedAt completedAt,
                                                                                                         ...
                                                                                                         Se.Set Beam.status status,
                                                                                                         Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.completionId $ Se.Eq (Kernel.Types.Id.getId completionId)]]}



instance FromTType' Beam.DriverModuleCompletion Domain.Types.DriverModuleCompletion.DriverModuleCompletion
    where fromTType' (Beam.DriverModuleCompletionT {..}) = do pure $ Just Domain.Types.DriverModuleCompletion.DriverModuleCompletion{completedAt = completedAt,
                                                                                                                                     ...
                                                                                                                                     createdAt = createdAt,
                                                                                                                                     updatedAt = updatedAt}
instance ToTType' Beam.DriverModuleCompletion Domain.Types.DriverModuleCompletion.DriverModuleCompletion
    where toTType' (Domain.Types.DriverModuleCompletion.DriverModuleCompletion {..}) = do Beam.DriverModuleCompletionT{Beam.completedAt = completedAt,
