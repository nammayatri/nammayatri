...
updateByDriverId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateByDriverId totalEarnings numRides totalDistance tollCharges bonusEarnings totalRideTime driverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                                                                               updateOneWithKV [Se.Set Beam.totalEarnings (Kernel.Prelude.roundToIntegral totalEarnings),
                                                                                                                                                ...
                                                                                                                                                Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId), Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}
updateD2dReferralCount :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => (Kernel.Prelude.Int -> Kernel.Types.Id.Id Domain.Types.Person.Person -> Data.Time.Calendar.Day -> m ())
updateD2dReferralCount d2dReferralCounts driverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                          updateOneWithKV [Se.Set Beam.d2dReferralCounts (Kernel.Prelude.Just d2dReferralCounts), Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
...
updateNumDriversOnboardedByDriverId numDriversOnboarded driverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                                         updateOneWithKV [Se.Set Beam.numDriversOnboarded (Kernel.Prelude.Just numDriversOnboarded), Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
                                                                                                                                                                                                                          Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}
...
updateNumFleetsOnboardedByDriverId numFleetsOnboarded driverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                                       updateOneWithKV [Se.Set Beam.numFleetsOnboarded (Kernel.Prelude.Just numFleetsOnboarded), Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
                                                                                                                                                                                                                      Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}
...
updatePayoutOrderIdAndStatus payoutOrderId payoutStatus id = do {_now <- getCurrentTime;
...
updatePayoutStatusById payoutStatus id = do {_now <- getCurrentTime;
...
updateReferralCount referralCounts driverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                    updateOneWithKV [Se.Set Beam.referralCounts (Kernel.Prelude.Just referralCounts), Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
                                                                                                                                                                                           Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}
updateReferralStatsByDriverId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateReferralStatsByDriverId activatedValidRides referralEarnings payoutStatus driverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                                                                 updateOneWithKV [Se.Set Beam.activatedValidRides (Kernel.Prelude.Just activatedValidRides),
...
updateTipAmountByDriverId tipAmount driverId merchantLocalDate = do {_now <- getCurrentTime;
                                                                     updateOneWithKV [Se.Set Beam.tipAmount (Kernel.Prelude.Just tipAmount), Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.driverId $ Se.Eq (Kernel.Types.Id.getId driverId),
                                                                                                                                                                                  Se.Is Beam.merchantLocalDate $ Se.Eq merchantLocalDate]]}
...
updateByPrimaryKey (Domain.Types.DailyStats.DailyStats {..}) = do {_now <- getCurrentTime;
                                                                   updateWithKV [Se.Set Beam.activatedValidRides (Kernel.Prelude.Just activatedValidRides),
                                                                                 Se.Set Beam.bonusEarnings (Kernel.Prelude.Just bonusEarnings),
                                                                                 Se.Set Beam.cancellationCharges (Kernel.Prelude.Just cancellationCharges),
                                                                                 Se.Set Beam.currency (Kernel.Prelude.Just currency),
                                                                                 Se.Set Beam.d2dActivatedValidRides (Kernel.Prelude.Just d2dActivatedValidRides),
                                                                                 ...
                                                                                 Se.Set Beam.totalEarningsAmount (Kernel.Prelude.Just totalEarnings),
                                                                                 Se.Set Beam.totalRideTime (Kernel.Prelude.Just totalRideTime),
                                                                                 Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.id $ Se.Eq id]]}
