...
findAllTollsByMerchantOperatingCity :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateByPrimaryKey (Domain.Types.Toll.Toll {..}) = do {_now <- getCurrentTime;
                                                       updateWithKV [Se.Set Beam.isAutoRickshawAllowed isAutoRickshawAllowed,
...
instance FromTType' Beam.Toll Domain.Types.Toll.Toll
    where fromTType' (Beam.TollT {..}) = do pure $ Just Domain.Types.Toll.Toll{createdAt = createdAt,
...
instance ToTType' Beam.Toll Domain.Types.Toll.Toll
    where toTType' (Domain.Types.Toll.Toll {..}) = do Beam.TollT{Beam.createdAt = createdAt,
