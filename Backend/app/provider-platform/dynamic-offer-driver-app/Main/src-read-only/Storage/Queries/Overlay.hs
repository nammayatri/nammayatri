...
deleteByOverlayKeyMerchantOpCityIdUdf :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
deleteByOverlayKeyMerchantOpCityIdUdf merchantOperatingCityId overlayKey udf1 = do deleteWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                         Se.Is Beam.overlayKey $ Se.Eq overlayKey,
                                                                                                         Se.Is Beam.udf1 $ Se.Eq udf1]]
deleteByOverlayKeyMerchantOpCityIdUdfMbVehicleCategory :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
deleteByOverlayKeyMerchantOpCityIdUdfMbVehicleCategory merchantOperatingCityId overlayKey udf1 vehicleCategory = do deleteWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                                          Se.Is Beam.overlayKey $ Se.Eq overlayKey,
                                                                                                                                          Se.Is Beam.udf1 $ Se.Eq udf1,
                                                                                                                                          Se.Is Beam.vehicleCategory $ Se.Eq vehicleCategory]]
findAllByLanguage :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByLanguage merchantOperatingCityId language = do findAllWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                               Se.Is Beam.language $ Se.Eq language]]
findAllByLanguageVehicleCategory :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByLanguageVehicleCategory merchantOperatingCityId language vehicleCategory = do findAllWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                              Se.Is Beam.language $ Se.Eq language,
                                                                                                              Se.Is Beam.vehicleCategory $ Se.Eq vehicleCategory]]
...
findAllByOverlayKeyUdf :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByOverlayKeyUdf merchantOperatingCityId overlayKey udf1 = do findAllWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                           Se.Is Beam.overlayKey $ Se.Eq overlayKey,
                                                                                           Se.Is Beam.udf1 $ Se.Eq udf1]]
findAllByOverlayKeyUdfVehicleCategory :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByOverlayKeyUdfVehicleCategory merchantOperatingCityId overlayKey udf1 vehicleCategory = do findAllWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                          Se.Is Beam.overlayKey $ Se.Eq overlayKey,
                                                                                                                          Se.Is Beam.udf1 $ Se.Eq udf1,
                                                                                                                          Se.Is Beam.vehicleCategory $ Se.Eq vehicleCategory]]
findByMerchantOpCityIdPNKeyLangaugeUdf :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByMerchantOpCityIdPNKeyLangaugeUdf merchantOperatingCityId overlayKey language udf1 = do findOneWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                    Se.Is Beam.overlayKey $ Se.Eq overlayKey,
                                                                                                                    Se.Is Beam.language $ Se.Eq language,
                                                                                                                    Se.Is Beam.udf1 $ Se.Eq udf1]]
findByMerchantOpCityIdPNKeyLangaugeUdfVehicleCategory :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByMerchantOpCityIdPNKeyLangaugeUdfVehicleCategory merchantOperatingCityId overlayKey language udf1 vehicleCategory = do findOneWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                                                   Se.Is Beam.overlayKey $ Se.Eq overlayKey,
                                                                                                                                                   Se.Is Beam.language $ Se.Eq language,
                                                                                                                                                   Se.Is Beam.udf1 $ Se.Eq udf1,
                                                                                                                                                   Se.Is Beam.vehicleCategory $ Se.Eq vehicleCategory]]
...
instance FromTType' Beam.Overlay Domain.Types.Overlay.Overlay
    where fromTType' (Beam.OverlayT {..}) = do pure $ Just Domain.Types.Overlay.Overlay{actions = actions,
...
instance ToTType' Beam.Overlay Domain.Types.Overlay.Overlay
    where toTType' (Domain.Types.Overlay.Overlay {..}) = do Beam.OverlayT{Beam.actions = actions,
