...
updateMerchantIdByDriverId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                              (Kernel.Prelude.Maybe (Kernel.Types.Id.Id Domain.Types.Merchant.Merchant) -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateMerchantIdByDriverId merchantId driverId = do {_now <- getCurrentTime;
...
updateVerificationStatus verificationStatus documentImageId1 = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.DriverLicense.DriverLicense {..}) = do {_now <- getCurrentTime;
                                                                         updateWithKV [Se.Set Beam.classOfVehicles classOfVehicles,
