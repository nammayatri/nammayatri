...
findAllByRcId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateByPrimaryKey (Domain.Types.VehicleInfo.VehicleInfo {..}) = do updateWithKV [Se.Set Beam.answer answer, Se.Set Beam.question question] [Se.And [Se.Is Beam.questionId $ Se.Eq questionId,
                                                                                                                                                     Se.Is Beam.rcId $ Se.Eq (Kernel.Types.Id.getId rcId)]]



instance FromTType' Beam.VehicleInfo Domain.Types.VehicleInfo.VehicleInfo
    where fromTType' (Beam.VehicleInfoT {..}) = do pure $ Just Domain.Types.VehicleInfo.VehicleInfo{answer = answer, question = question, questionId = questionId, rcId = Kernel.Types.Id.Id rcId}
instance ToTType' Beam.VehicleInfo Domain.Types.VehicleInfo.VehicleInfo
    where toTType' (Domain.Types.VehicleInfo.VehicleInfo {..}) = do Beam.VehicleInfoT{Beam.answer = answer, Beam.question = question, Beam.questionId = questionId, Beam.rcId = Kernel.Types.Id.getId rcId}
