...
findAllByMessageTypeAndMerchantOpCityIdAndMerchantId messageType merchantOperatingCityId merchantId = do
  findAllWithKV
    [ Se.And
        [ Se.Is Beam.messageType $ Se.Eq messageType,
...
instance FromTType' Beam.MessageDictionary Domain.Types.MessageDictionary.MessageDictionary where
  fromTType' (Beam.MessageDictionaryT {..}) = do
    pure $
      Just
