...
findByDriverIdAndFleetOwnerId (Kernel.Types.Id.Id driverId) fleetOwnerId isActive = do
  findOneWithKV
    [ Se.And
...
updateFleetDriverActiveStatus isActive associatedTill (Kernel.Types.Id.Id driverId) fleetOwnerId = do
  _now <- getCurrentTime
  updateOneWithKV
