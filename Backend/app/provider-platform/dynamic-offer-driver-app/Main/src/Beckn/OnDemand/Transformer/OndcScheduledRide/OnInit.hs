...
-- | Patches Layer 1's on_init order: breakup titles, BPP_TERMS, ROUTE_INFO, fulfillment.type, vehicle.energy_type, and echoes back the selected add-ons.
...
ondcScheduledRideOnInitMessageBuild booking bapId bppConfig msg = do
  ...
  orderWithOverrides <- OSRCommon.applyOnInitOrderOverrides booking.isScheduled booking.transactionId booking.addOnData mbBapMetadata bppConfig orderWithBreakupTitles
