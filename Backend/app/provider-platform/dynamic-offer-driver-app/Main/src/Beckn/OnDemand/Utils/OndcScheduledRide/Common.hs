...
module Beckn.OnDemand.Utils.OndcScheduledRide.Common
  ( addTagGroup,
    ...
    applyOnCancelOrderOverrides,
    ensureFulfillmentVehicle,
    ensureQuoteBreakup,
    applyOnCancelOrderOverridesIfEnabled,
    remapBreakupTitle,
...
import BecknV2.OnDemand.Utils.Constructors (emptyVehicle)
...
applyOnInitOrderOverrides :: (EsqDBFlow m r, CacheFlow m r, MonadFlow m) => Bool -> Text -> [DAddOnConfig.AddOnData] -> Maybe DBapMetadata.BapMetadata -> DBC.BecknConfig -> Spec.Order -> m Spec.Order
applyOnInitOrderOverrides isScheduled transactionId addOnData mbBapMetadata bppConfig =
  join
    . fmap (patchOrderAddOns addOnData)
    . patchOrderRouteInfo transactionId
    . patchOrderVehicleEnergyType
...
-- | The ONDC on_cancel order patch. on_cancel was the one callback with no Layer 2 patch at all,
...
-- clause, so that NACKs. The booking's service tier gives the same category/variant pair on_init
-- announces. Only fills a missing vehicle -- a real assigned one is left untouched.
ensureFulfillmentVehicle :: DRB.Booking -> Spec.Order -> Spec.Order
ensureFulfillmentVehicle booking order =
  order {Spec.orderFulfillments = map fillVehicle <$> order.orderFulfillments}
  where
    (category, variant) = BUtils.castVariant (castServiceTierToVariant booking.vehicleServiceTier)
    fallbackVehicle = emptyVehicle {Spec.vehicleCategory = Just category, Spec.vehicleVariant = Just variant}
    fillVehicle fulfillment = case fulfillment.fulfillmentVehicle of
      Just _ -> fulfillment
      Nothing -> fulfillment {Spec.fulfillmentVehicle = Just fallbackVehicle}

-- | An application cancel with no cancellation dues leaves Layer 1's breakup empty, and ONDC requires
-- quote.breakup[*].price.currency / .value / .title with no "skip if absent" clause -- so an absent or
-- empty breakup NACKs. Substitute the booking's own fare breakup, which is what on_init sends and what
-- quote.price is computed from, so the two stay consistent. A non-empty breakup is left untouched.
ensureQuoteBreakup :: Bool -> DRB.Booking -> Spec.Order -> Spec.Order
ensureQuoteBreakup isValueAddNP booking order =
  order {Spec.orderQuote = fillBreakup <$> order.orderQuote}
  where
    fillBreakup quotation
      | maybe True null quotation.quotationBreakup =
        quotation {Spec.quotationBreakup = BUtils.mkQuotationBreakup isValueAddNP booking.fareParams}
      | otherwise = quotation

-- | Drops breakup lines whose price is missing either half of {currency, value} -- ONDC requires
-- both on every line -- and omits quote.breakup entirely rather than sending an empty array when
-- nothing survives (a booking cancelled with no cancellation dues produces no lines at all).
dropIncompleteOrderBreakup :: Spec.Order -> Spec.Order
dropIncompleteOrderBreakup order = order {Spec.orderQuote = fixQuote <$> Spec.orderQuote order}
  where
    -- Deliberately NOT collapsing an empty result to Nothing: ONDC requires quote.breakup to be
    -- present (its price/title checks carry no "skip if absent" clause), so dropping the field is a
    -- NACK rather than a fix. Beckn.ACL.OnCancel.tfQuotation guarantees a non-empty breakup instead.
    fixQuote quotation = quotation {Spec.quotationBreakup = filter hasCompletePrice <$> quotation.quotationBreakup}
    hasCompletePrice breakup = fromMaybe False $ do
      price <- breakup.quotationBreakupInnerPrice
      pure $ isJust price.priceCurrency && isJust price.priceValue

-- | Fetches the pilot gate and applies applyOnCancelOrderOverrides only when enabled, mirroring
-- applyOndcScheduledRideOrderOverridesIfEnabled for the on_cancel message.
applyOnCancelOrderOverridesIfEnabled ::
