...
remapBreakupTitle title
  ...
  -- The accepted Pre-Order Bid's adjustment to the ride fare, passed through under its own title rather than remapped or dropped: without this line the breakup no longer sums to the negotiated total the BAP was quoted, and no ONDC title carries a signed adjustment (a downward negotiation makes this negative).
  | title == show Enums.NEGOTIATED_FARE_DELTA = Just title
  | otherwise = Nothing
