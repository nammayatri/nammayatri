...
buildOnUpdateReqOrderV2 req' mbFarePolicy becknConfig = case req' of
  ...
  OU.EditDestinationUpdate OU.DEditDestinationUpdateReq {..} -> do
    ...
    -- items = Utils.tfItemsSoftUpdate booking merchant.shortId.getShortId bookingUpdateReqDetails.estimatedDistance farePolicy Nothing bookingUpdateReqDetails ride.id.getId
    ...
    -- updateDetailsTagGroup = if isValueAddNP then Utils.mkUpdatedDistanceTags bookingUpdateReqDetails.estimatedDistance else Nothing
