...
-- | Building: adds BPP_TERMS (STATIC_TERMS from beckn_config, OFFLINE_CONTRACT
-- from the requesting BAP's own bap_metadata row) to message.catalog.tags on
-- the already-built on_search reply, additive alongside whatever's already
-- there.
msilAddBppTerms :: Maybe DBapMetadata.BapMetadata -> DBC.BecknConfig -> Spec.OnSearchReq -> Spec.OnSearchReq
msilAddBppTerms mbBapMetadata bppConfig onSearchReq =
  onSearchReq {Spec.onSearchReqMessage = fixMessage <$> Spec.onSearchReqMessage onSearchReq}
  where
    fixMessage msg = msg {Spec.onSearchReqMessageCatalog = MSILTerms.patchCatalogTags mbBapMetadata bppConfig (Spec.onSearchReqMessageCatalog msg)}
