...
tfCatalog estimates res = do
  ...
  catalogProviders_ <- tfCatalogProviders estimates res <&> Data.List.singleton <&> Just
...
tfVehicle estInfo = do
  ...
  let vehicleVariant_ = Beckn.OnDemand.Utils.Common.castVariant estInfo.estimate.vehicleVariant & Just
