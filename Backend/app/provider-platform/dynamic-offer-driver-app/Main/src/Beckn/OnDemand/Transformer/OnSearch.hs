...
tfCatalogDescriptor res = do
  let descriptorCode_ = Nothing
      ...
      returnData = BecknV2.OnDemand.Types.Descriptor {descriptorLongDesc = Nothing, descriptorCode = descriptorCode_, descriptorName = descriptorName_, descriptorShortDesc = descriptorShortDesc_}
      allNothing = BecknV2.OnDemand.Utils.Common.allNothing returnData
  if allNothing
    then Nothing
    else Just returnData
...
tfProviderItems res isValueAddNP pricing = do
  let itemDescriptor_ = tfItemDescriptor pricing
      ...
      itemTags_ = Beckn.OnDemand.Utils.OnSearch.mkItemTags pricing isValueAddNP res.fareParametersInRateCard (not $ null res.stops)
