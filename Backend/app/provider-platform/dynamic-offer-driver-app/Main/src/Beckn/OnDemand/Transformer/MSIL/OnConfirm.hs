...
-- overrides and why. Gated behind enableOndcScheduledRideSupport initially
...
msilOnConfirmMessageBuild :: (CacheFlow m r, MonadFlow m) => Bool -> Text -> Maybe DBapMetadata.BapMetadata -> DBC.BecknConfig -> Spec.ConfirmReqMessage -> m Spec.ConfirmReqMessage
msilOnConfirmMessageBuild isScheduled transactionId mbBapMetadata bppConfig msg = do
  let orderWithBreakupTitles = overrideOrderBreakupTitles (Spec.confirmReqMessageOrder msg)
      ...
      orderWithTerms = MSILTerms.patchOrderTags True mbBapMetadata bppConfig orderWithCategoryIds
