  ...
  pure
    Spec.Order
      { orderId = Just $ booking.id.getId,
        orderStatus = Just "ACTIVE", -- TODO::Beckn, confirm mapping as we only have 5 states in v2 spec. #RITIKA
...
tfStartReqToOrder Common.DRideStartedReq {..} = do
  ...
  pure
    Spec.Order
      { orderId = Just $ booking.id.getId,
        orderStatus = Just "ACTIVE", -- TODO::Beckn, confirm mapping as we only have 5 states in v2 spec.
        orderFulfillments = Just [fulfillment],
        orderBilling = Nothing,
        orderCancellation = Nothing,
        orderCancellationTerms = Nothing,
...
tfCompleteReqToOrder Common.DRideCompletedReq {..} = do
  ...
  pure
    Spec.Order
      { orderId = Just $ booking.id.getId,
        orderStatus = Just "COMPLETE", -- TODO::Beckn, confirm mapping as we only have 5 states in v2 spec.
        orderFulfillments = Just [fulfillment],
...
tfCancelReqToOrder Common.DBookingCancelledReq {..} = do
  ...
  pure
    Spec.Order
      { orderId = Just $ booking.id.getId,
        orderStatus = Just "CANCELLED", -- TODO::Beckn, confirm mapping as we only have 5 states in v2 spec.
