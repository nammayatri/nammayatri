...
buildOnCancelMessageV2 merchant mbBapCity mbBapCountry cancelStatus (OC.BookingCancelledBuildReqV2 OC.DBookingCancelledReqV2 {..}) mbMsgId = do
  ...
  -- Soft cancel runs before any dues row exists, so the collection mode comes from the
  -- matrix row stashed on the ride by the soft-cancel quote (lets the BAP tell the rider
  -- whether the fee will be captured immediately or carried to the next ride)
  mbSoftCancelRow <-
    if isNothing mbCancellationDuesDetails && cancelStatus == show Enums.SOFT_CANCEL
      then maybe (pure Nothing) (QCCM.findByPrimaryKey . Id) (mbRide' >>= (.cancellationConsequenceRowIdIfCancelled))
      else pure Nothing
  let mbCollectionMode = (mbCancellationDuesDetails >>= (.cancellationCollectionMode)) <|> (show <$> (mbSoftCancelRow >>= (.collectionMode)))
...
tfQuotation :: DRB.Booking -> Maybe DCDD.CancellationDuesDetails -> Maybe Spec.Quotation
tfQuotation booking mbCancellationDuesDetails' =
  Just
    emptyQuotation
      { Spec.quotationBreakup = mkQuotationBreakup booking mbCancellationDuesDetails',
  ...
  Just
    [ emptyItem
        { Spec.itemDescriptor = BUtils.tfItemDescriptor booking,
          ...
          Spec.itemTags = BUtils.mkRateCardTag Nothing booking.fareParams.customerCancellationDues Nothing booking.estimatedFare booking.fareParams.congestionChargeViaDp (Just . FarePolicyD.fullFarePolicyToFarePolicy =<< mbFarePolicy) Nothing Nothing Nothing (not $ null booking.stops)
        }
    ]
