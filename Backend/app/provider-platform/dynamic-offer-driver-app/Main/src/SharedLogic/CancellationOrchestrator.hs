...
applyTerminalConsequences ::
  ...
  ConsequenceCtx ->
  -- | create finance ledger entries: base -> gst -> consumeRideCredit -> m ()
  (HighPrecMoney -> HighPrecMoney -> Bool -> m ()) ->
        ...
        whenJust mbOutcome $ \outcome ->
          whenJust outcome.fee $ \baseFee -> do
            ...
            SCD.applyCancellationCharge
              SCD.ApplyCancellationChargeReq
                { ride = ride,
                  ...
                  carryForwardEnabled = fromMaybe True (decision.consequenceRow >>= (.carryForwardDues))
                }
            ...
            when (isWalletEnabled && totalCharges > 0) $
              createLedgerEntries baseFee gst (fromMaybe False (decision.consequenceRow >>= (.consumeRideCreditOnCancellation)))
        pure mbOutcome
      case chargesE of
        Left err -> do
