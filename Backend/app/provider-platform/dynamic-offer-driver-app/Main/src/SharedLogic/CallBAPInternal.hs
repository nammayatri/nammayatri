...
assignFleetVehicle apiKey internalUrl request = do
  internalEndPointHashMap <- asks (.internalEndPointHashMap)
  EC.callApiUnwrappingApiError (identity @Error) Nothing (Just "BAP_INTERNAL_API_ERROR") (Just internalEndPointHashMap) internalUrl (fleetVehicleAssignmentClient (Just apiKey) request) "FleetVehicleAssignment" fleetVehicleAssignmentApi
