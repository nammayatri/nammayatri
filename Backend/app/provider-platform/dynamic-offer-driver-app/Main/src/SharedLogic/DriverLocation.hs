...
findById id =
  Hedis.get (makeDriverLocationKey id) >>= \case
    Just a ->
      return $ Just a
    Nothing ->
      flip whenJust cacheDriverLocation /=<< Esq.runInReplica (DLQueries.findById (Proxy @m) id)
...
updateOnRide :: forall m r. (CacheFlow m r, Esq.EsqDBFlow m r, EsqDBReplicaFlow m r) => Id Person.Driver -> Bool -> m ()
updateOnRide driverId onRide = do
  if onRide
    then do
      -- driver coming to ride, update location from db to redis
      driverLocation <- DLQueries.findById (Proxy @m) (cast driverId) >>= fromMaybeM (PersonNotFound driverId.getId)
      cacheDriverLocation driverLocation
    else do
      ...
      maybe
        ...
        ( \loc -> do
            ...
            void $ Esq.runTransaction $ DLQueries.upsertGpsCoord @m (cast driverId) latLong loc.coordinatesCalculatedAt
        )
        mDriverLocatation
