...
upsertFleetVehicleDailyStats fleetOwnerId rcId transporterConfig rideDistance rideDuration totalEarnings = do
  ...
  case res of
    ...
    Just s -> do
      let addMoney (HighPrecMoney a) (HighPrecMoney b) =
            (HighPrecMoney (a + b))
      let addMeters (Meters a) (Meters b) =
            (Meters (a + b))
      let addSeconds (Seconds a) (Seconds b) =
            (Seconds (a + b))
...
updateFleetVehicleDailyStats fleetOwnerId transporterConfig ride = do
  vehicle <- QV.findById ride.driverId >>= fromMaybeM (InvalidRequest $ "Vehicle not found for driver " <> ride.driverId.getId)
  rc <- VRCExtra.findLastVehicleRCFleet' vehicle.registrationNo fleetOwnerId >>= fromMaybeM (VehicleNotFound vehicle.registrationNo)
