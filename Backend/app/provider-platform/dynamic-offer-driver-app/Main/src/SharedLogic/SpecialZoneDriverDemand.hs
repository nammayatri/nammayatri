      ...
      if supplyCount >= minThreshold
        then logDebug $ "Supply already at/above minThreshold, skipping gateId=" <> gateId <> " variant=" <> variant
          ...
          when (needed > 0) $ do
            ...
            eligibleNearGate <- case readMaybe (T.unpack variant) :: Maybe DV.VehicleVariant of
              Just vehicleVariant -> filterByGateProximity merchantId gate vehicleVariant eligible
...
forceNotifyDriverDemand merchantOpCityId merchantId gate vehicleType needed mbPriorityDriverIds mbIsDemandHigh = do
  ...
  case mbVariant of
    Nothing -> do
      ...
      pure 0
    Just variant -> do
      eligiblePriorityNearGate <- filterByGateProximity merchantId gate variant eligiblePriority
...
filterByGateProximity ::
  ...
  DV.VehicleVariant -> -- vehicleVariant (LTS queue key, threaded through to manualQueueRemove for stale drivers)
  [Id DP.Person] ->
  m [Id DP.Person]
filterByGateProximity _ _ _ [] = pure []
filterByGateProximity merchantId targetGate variant driverIds = do
