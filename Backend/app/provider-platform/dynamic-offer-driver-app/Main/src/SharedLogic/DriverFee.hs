...
notificationSchedulerKey :: UTCTime -> UTCTime -> Id MOC.MerchantOperatingCity -> Text
notificationSchedulerKey startTime endTime merchantOpCityId = "NotificationScheduler:st:" <> show startTime <> ":et:" <> show endTime <> ":opCityid:" <> merchantOpCityId.getId

isNotificationSchedulerRunningKey :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => UTCTime -> UTCTime -> Id MOC.MerchantOperatingCity -> m (Maybe Bool)
isNotificationSchedulerRunningKey startTime endTime merchantOpCityId = Hedis.withCrossAppRedis $ Hedis.get (notificationSchedulerKey startTime endTime merchantOpCityId)

setIsNotificationSchedulerRunningKey :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => UTCTime -> UTCTime -> Id MOC.MerchantOperatingCity -> Bool -> m ()
setIsNotificationSchedulerRunningKey startTime endTime merchantOpCityId isNotificationSchedulerRunning = do
  Hedis.withCrossAppRedis $ Hedis.setExp (notificationSchedulerKey startTime endTime merchantOpCityId) isNotificationSchedulerRunning (3600 * 24) -- one day expiry
