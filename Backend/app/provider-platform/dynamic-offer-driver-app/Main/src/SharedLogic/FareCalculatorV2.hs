...
-- 2. Applies configurable charges (VAT, commission, toll tax) based on fare_policy configuration
-- 3. Returns FareParameters with new breakdown fields (rideVat, tollVat, commission)
--
...
calculateFareParametersV2 params = do
  ...
  -- Then, apply configurable charges based on fare_policy
  applyConfiguredCharges params.farePolicy baseFareParams
   ...
   in if T.null trimmed
        then Nothing
          ...
          if "%" `T.isSuffixOf` trimmed
            -- Percentage value: "14%" -> ParsedPercentage 0.14
            then
              let numeric = T.stripEnd $ T.dropEnd 1 trimmed
               in ParsedPercentage <$> parsePercentage numeric
            -- Fixed value: "50" -> ParsedFixed 50.0
            else ParsedFixed . HighPrecMoney . toRational <$> parseFixed trimmed
