...
initializeRide merchant driver booking mbOtpCode enableFrequentLocationUpdates mbClientId enableOtpLessRide mFleetOwnerId monitorPickupProgress bookingPreAssigned = do
  ...
  fork "BPP pre_ride pickup-distance histogram" $
    whenJust booking.distanceToPickup $ \pickupDistanceVal -> do
      (pickupZoneCat, dropZoneCat) <- SML.specialZoneCategoryLabels booking.area
      Metrics.observePickupDistanceMeters merchant.shortId.getShortId cityLabel (show booking.vehicleServiceTier) pickupZoneCat dropZoneCat "pre_ride" (realToFrac pickupDistanceVal)
