...
initializeRide merchantId driver booking mbOtpCode enableFrequentLocationUpdates mbClientId = do
  ...
  now <- getCurrentTime
  ride <- buildRide driver booking ghrId otpCode enableFrequentLocationUpdates mbClientId previousRideInprogress now
...
buildRide ::
  ...
  UTCTime ->
  Flow DRide.Ride
buildRide driver booking ghrId otp enableFrequentLocationUpdates clientId previousRide now = do
