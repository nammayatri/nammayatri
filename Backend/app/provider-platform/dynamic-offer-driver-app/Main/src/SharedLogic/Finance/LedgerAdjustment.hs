...
ledgerAdjustmentSubmit merchantShortId opCity requestorId requestorName req = ActorInfo.withDashboardPersonIdActorInfo (Id @DP.Person requestorId) $ do
  ...
  let personId = cast @Dashboard.Common.Person @DP.Person req.personId
      ...
      h = categoryHandlers category -- Per-category handler
  withLedgerAdjustmentLock lockKey $ do
    whenJust req.referenceId $ \referenceId -> do
      mbExisting <-
        QLedgerAdjustmentRequest.findByReferenceIdAndStatuses
          (Just referenceId)
          [DLA.PENDING_APPROVAL, DLA.POSTED] -- except REJECTED, DLA.POST_FAILED
    ...
    unless (h.expectedDirection == direction) $
    ...
    h.validateCategory merchantOpCity transporterConfig personId direction req
...
-- | Category-specific validate ledger adjustment action
type ValidateLedgerAdjustment =
...
categoryHandlers category = case category of
  ...
  where
    validateRideRelated _merchantOpCity = validateRideRelatedAdjustment
    validatePayoutRelated merchantOpCity _transporterConfig = validatePayoutRelatedAdjustment merchantOpCity
    validateTdsReimbursement merchantOpCity _transporterConfig = validateTdsReimbursementAdjustment merchantOpCity
    validateIncentive _merchantOpCity _transporterConfig = validateIncentiveAdjustment
    validateMiscellaneous _merchantOpCity _transporterConfig = validateMiscellaneousAdjustment
    validateTdsDeduction _merchantOpCity transporterConfig personId _direction = validateTdsDeductionAdjustment transporterConfig personId

mkCategoryHandlers ::
  DLA.AdjustmentDirection ->
  ValidateLedgerAdjustment ->
...
validateTdsReimbursementAdjustment merchantOpCity personId direction req = do
  ...
  void $
    assertTdsReimbursementClaimable tdsRequest referenceId $ \mappings -> do
      -- Frozen payable from WS8 submit: Σ invoice-mapping tdsCreditReceivable.
      let payableAmount = sum $ map (.tdsCreditReceivable) mappings
...
assertTdsReimbursementClaimable ::
  ...
  Text ->
  ([DTdsMap.FinanceTdsReimbursementInvoiceMapping] -> Flow ()) ->
...
assertTdsReimbursementClaimable tdsRequest referenceId afterStatusCheck = do
  ...
  mappingsWithInvoices <- TdsReimbursement.findInvoiceMappings tdsRequest.id mappings
...
-- | Submit/post TOCTOU guard: debit amount must not exceed current wallet balance.
validateWalletDebitAmountForPerson :: DP.Person -> HighPrecMoney -> Flow ()
validateWalletDebitAmountForPerson person debitAmount = do
  ...
  Wallet.validateWalletDebitAmount counterpartyType person.id.getId debitAmount
...
postTdsReimbursementAdjustment transporterConfig adjustmentRequest = do
  ...
  mappingsWithInvoices <- assertTdsReimbursementClaimable tdsRequest referenceId (\_mappings -> pure ())
...
postMiscellaneousAdjustment transporterConfig adjustmentRequest =
  ...
  postSimpleAdjustment
    ...
    ( \person -> do
        unless (person.role `elem` [DP.DRIVER, DP.FLEET_OWNER, DP.FLEET_BUSINESS]) $
          throwError (InvalidRequest "Miscellaneous adjustments are only supported for drivers and fleet owners")
        when (adjustmentRequest.direction == DLA.Debit) $
