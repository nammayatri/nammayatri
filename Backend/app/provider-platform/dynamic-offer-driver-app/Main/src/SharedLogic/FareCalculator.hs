...
fareSum fareParams =
  fareParams.baseFare
    ...
    + countFullFareOfParamsDetails fareParams.fareParametersDetails
...
calculateFareParameters params = do
  ...
  let isNightShiftChargeIncluded = isNightShift <$> fp.nightShiftBounds <*> Just params.rideTime
      ...
      fullRideCost {-without govtCharges and nightShift-} =
        baseFare
          + fromMaybe 0 fp.serviceCharge
          + fromMaybe 0 resultWaitingCharge
          + countFullFareOfParamsDetails fareParametersDetails
      resultNightShiftCharge = (\isCoefIncluded -> if isCoefIncluded then countNightShiftCharge fullRideCost <$> nightShiftCharge else Nothing) =<< isNightShiftChargeIncluded
      fullRideCostN {-without govtCharges-} =
        fullRideCost + fromMaybe 0 resultNightShiftCharge
      govtCharges =
...
countFullFareOfParamsDetails :: DFParams.FareParametersDetails -> Money
countFullFareOfParamsDetails = \case
  DFParams.ProgressiveDetails det -> fromMaybe 0 det.extraKmFare + det.deadKmFare
  DFParams.SlabDetails _ -> 0
