...
calculateFareParameters params = do
  ...
  where
    ...
    processFPRentalDetails DFP.FPRentalDetails {..} = do
      ...
      -- extendUpto = min (min extraPlannedKm 50) (120 - estimatedDurationInHr * includedKmPerHr.getMeters)
      -- when (extraPlannedKm > extendUpto) $ do
      -- logTagInfo "FareCalculator" $ "Fare parameters calculated: " +|| show extendUpto ||+ ""
      -- logInfo $ "extraPlannedKm is greater than extendUpto param. extraPlannedKm = " <> show extraPlannedKm <> " extendUpto = " <> show extendUpto -- not throwing error due to critical path
      let extraPlannedKmFare = extraPlannedKm * plannedPerKmRate.getMoney
      let baseFare_ = Money (estimatedDurationInHr * perHourCharge.getMoney + extraPlannedKmFare)
      ( baseFare_,
        nightShiftCharge,
        Nothing,
...
isNightAllowanceApplicable :: Maybe DFP.NightShiftBounds -> UTCTime -> UTCTime -> Seconds -> Bool
