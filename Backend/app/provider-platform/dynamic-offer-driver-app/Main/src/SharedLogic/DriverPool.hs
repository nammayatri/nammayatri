...
windowFromIntelligentPoolConfig merchantOpCityId windowKey = maybe defaultWindow windowKey <$> DIP.findByMerchantOpCityId merchantOpCityId 0 Nothing
  where
...
updateDriverSpeedInRedis merchantOpCityId driverId points timeStamp = Redis.withCrossAppRedis $ do
  locationUpdateSampleTime <- maybe 3 (.locationUpdateSampleTime) <$> DIP.findByMerchantOpCityId merchantOpCityId 0 Nothing
  now <- getCurrentTime
...
getDriverAverageSpeed merchantOpCityId driverId = Redis.withCrossAppRedis $ do
  intelligentPoolConfig <- DIP.findByMerchantOpCityId merchantOpCityId 0 Nothing
...
calculateGoHomeDriverPool CalculateGoHomeDriverPoolReq {..} merchantOpCityId = do
  ...
  goHomeRequests <-
    mapMaybeM
      ...
      randomDriverPool
  transporterConfig <- CTC.findByMerchantOpCityId merchantOpCityId 0 Nothing >>= fromMaybeM (TransporterConfigDoesNotExist merchantOpCityId.getId)
  ...
  where
    ...
    calculateActualDistanceCurrently driverToDestinationDistanceThreshold DriverPoolResultCurrentlyOnRide {..} = do
      ...
      if distanceFromDriverToDestination < driverToDestinationDistanceThreshold
        then do
          transporter <- CTC.findByMerchantOpCityId merchantOpCityId 0 Nothing >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
