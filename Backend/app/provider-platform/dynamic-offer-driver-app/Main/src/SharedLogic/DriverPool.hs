  ...
  where
    filterFunc threshold estDist distanceToPickup =
      case driverPoolCfg.thresholdToIgnoreActualDistanceThreshold of
        Just thresholdToIgnoreActualDistanceThreshold ->
          if distanceToPickup <= thresholdToIgnoreActualDistanceThreshold
            then True
            else getMeters estDist.actualDistanceToPickup <= fromIntegral threshold
