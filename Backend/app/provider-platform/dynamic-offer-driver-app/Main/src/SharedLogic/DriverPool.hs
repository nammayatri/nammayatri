...
calculateDriverPool ::
  ...
  UTCTime ->
  m [DriverPoolResult]
calculateDriverPool cityServiceTiers poolStage driverPoolCfg serviceTiers pickup merchantId onlyNotOnRide mRadiusStep isRental isInterCity now = do
...
calculateDriverPoolWithActualDist poolCalculationStage poolType driverPoolCfg serviceTiers pickup merchantId merchantOpCityId onlyNotOnRide mRadiusStep isRental isInterCity = do
  cityServiceTiers <- CQVST.findAllByMerchantOpCityId merchantOpCityId
  now <- getCurrentTime
  driverPool <- calculateDriverPool cityServiceTiers poolCalculationStage driverPoolCfg serviceTiers pickup merchantId onlyNotOnRide mRadiusStep isRental isInterCity now
  case driverPool of
    [] -> return []
    (a : pprox) -> do
...
calculateDriverPoolCurrentlyOnRide cityServiceTiers poolStage driverPoolCfg serviceTiers pickup merchantId mRadiusStep isRental isInterCity mbBatchNum now = do
  let radius = getRadius' mRadiusStep
  let coord = getCoordinates pickup
  approxDriverPool <-
    measuringDurationToLog INFO "calculateDriverPoolCurrentlyOnRide" $
      B.runInReplica $
        QP.getNearestDriversCurrentlyOnRide
          ...
          merchantId
          driverPoolCfg.driverPositionInfoExpiry
          driverPoolCfg.radiusShrinkValueForDriversOnRide ---- make it 0 in config
...
calculateDriverCurrentlyOnRideWithActualDist poolCalculationStage poolType driverPoolCfg serviceTiers pickup merchantId merchantOpCityId mRadiusStep isRental isInterCity batchNum = do
  cityServiceTiers <- CQVST.findAllByMerchantOpCityId merchantOpCityId
  now <- getCurrentTime
  (thresholdRadius, driverPool) <- calculateDriverPoolCurrentlyOnRide cityServiceTiers poolCalculationStage driverPoolCfg serviceTiers pickup merchantId mRadiusStep isRental isInterCity (Just batchNum) now
