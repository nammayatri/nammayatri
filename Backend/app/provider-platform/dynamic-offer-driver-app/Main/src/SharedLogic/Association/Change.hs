...
guardNoLiveRideByRC rcId = do
  ...
  rides <- QRide.getUpcomingOrActiveByDriverIds (map (.driverId) assocs)
  unless (null rides) $ throwError (InvalidRequest "Vehicle has a live ride, cannot change association")
...
guardNoLiveRideInFleet fleetOwnerId = do
  ...
  rides <- QRide.getUpcomingOrActiveByDriverIds driverIds
  unless (null rides) $ throwError (InvalidRequest "A driver in this fleet has a live ride, cannot change association")
