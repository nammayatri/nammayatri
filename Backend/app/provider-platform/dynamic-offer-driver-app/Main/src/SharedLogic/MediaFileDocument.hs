...
mediaFileDocumentUploadLink merchantShortId opCity _requestorId req = do
  ...
  pure $ Common.MediaFileUploadLinkResp {uploadUrl = uploadUrl, filePath = filePath, fileId = cast fileId}
...
mediaFileDocumentConfirm merchantShortId opCity _requestorId req = do
  let fileId = Id req.fileId :: Id DMF.MediaFile
  ...
  pure $ Common.ConfirmMediaFileResp {fileId = cast fileId}
  where
...
mediaFileDocumentDownloadLink merchantShortId _opCity fileId _requestorId = do
  ...
  pure $ Common.MediaFileDownloadResp {url = mbUrl}
