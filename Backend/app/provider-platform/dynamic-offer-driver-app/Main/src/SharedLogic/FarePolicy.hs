...
getFarePolicyByEstOrQuoteId :: (CacheFlow m r, EsqDBFlow m r, EsqDBReplicaFlow m r) => Maybe LatLong -> Maybe Text -> Maybe Meters -> Maybe Seconds -> Id DMOC.MerchantOperatingCity -> DTC.TripCategory -> DVST.ServiceTierType -> Maybe SL.Area -> Text -> Maybe UTCTime -> Maybe Bool -> Maybe CacKey -> m FarePolicyD.FullFarePolicy
getFarePolicyByEstOrQuoteId mbFromlocaton mbGeohash mbDistance mbDuration merchantOpCityId tripCategory vehicleServiceTier area estOrQuoteId mbBookingStartTime isDashboardRequest txnId = do
  Redis.safeGet (makeFarePolicyByEstOrQuoteIdKey estOrQuoteId) >>= \case
    Nothing -> do
      logWarning "Old Fare Policy Not Found, Hence using new fare policy."
      getFarePolicy mbFromlocaton mbGeohash mbDistance mbDuration merchantOpCityId (fromMaybe False isDashboardRequest) tripCategory vehicleServiceTier area mbBookingStartTime txnId
...
getFarePolicyOnEndRide :: (CacheFlow m r, EsqDBFlow m r, EsqDBReplicaFlow m r) => Maybe LatLong -> Maybe Text -> Maybe Meters -> Maybe Seconds -> LatLong -> Id DMOC.MerchantOperatingCity -> DTC.TripCategory -> DVST.ServiceTierType -> Maybe SL.Area -> Text -> Maybe UTCTime -> Maybe Bool -> Maybe CacKey -> m FarePolicyD.FullFarePolicy
getFarePolicyOnEndRide mbFromLocation mbGeohash mbDistance mbDuration toLocationLatLong merchantOpCityId tripCategory vehicleServiceTier area estOrQuoteId mbBookingStartTime isDashboardRequest txnId = do
  selectedFarePolicy <- case area of
    Just SL.Default -> handleFarePolicy
    Just (SL.Drop _) -> handleFarePolicy
    _ -> getFarePolicyByEstOrQuoteId mbFromLocation mbGeohash mbDistance mbDuration merchantOpCityId tripCategory vehicleServiceTier area estOrQuoteId mbBookingStartTime isDashboardRequest txnId
    ...
    handleFarePolicy = do
      ...
      selectedFarePolicy' <-
        if isJust isDropSpecialLocation && area /= isDropSpecialLocation
          then getFarePolicy mbFromLocation mbGeohash mbDistance mbDuration merchantOpCityId (fromMaybe False isDashboardRequest) tripCategory vehicleServiceTier isDropSpecialLocation mbBookingStartTime txnId
          else getFarePolicyByEstOrQuoteId mbFromLocation mbGeohash mbDistance mbDuration merchantOpCityId tripCategory vehicleServiceTier area estOrQuoteId mbBookingStartTime isDashboardRequest txnId
...
getFarePolicy :: (CacheFlow m r, EsqDBFlow m r, EsqDBReplicaFlow m r) => Maybe LatLong -> Maybe Text -> Maybe Meters -> Maybe Seconds -> Id DMOC.MerchantOperatingCity -> Bool -> DTC.TripCategory -> DVST.ServiceTierType -> Maybe SL.Area -> Maybe UTCTime -> Maybe CacKey -> m FarePolicyD.FullFarePolicy
getFarePolicy _mbFromlocaton mbGeohash mbDistance mbDuration merchantOpCityId isDashboard tripCategory serviceTier Nothing mbBookingStartTime txnId = do
  ...
  getFullFarePolicy mbGeohash mbDistance mbDuration txnId mbBookingStartTime fareProduct
getFarePolicy _mbFromlocaton mbGeohash mbDistance mbDuration merchantOpCityId isDashboard tripCategory serviceTier (Just area) mbBookingStartTime txnId = do
  let searchSources = FareProduct.getSearchSources isDashboard
  mbFareProduct <-
  ...
  case mbFareProduct of
    Just fareProduct -> getFullFarePolicy mbGeohash mbDistance mbDuration txnId mbBookingStartTime fareProduct
    Nothing -> do
      fareProduct <-
        FareProduct.getBoundedFareProduct merchantOpCityId searchSources tripCategory serviceTier SL.Default
          |<|>| QFareProduct.findUnboundedByMerchantVariantArea merchantOpCityId searchSources tripCategory serviceTier SL.Default
...
getAllFarePoliciesProduct :: (CacheFlow m r, EsqDBFlow m r, EsqDBReplicaFlow m r) => Id Merchant -> Id DMOC.MerchantOperatingCity -> Bool -> LatLong -> Maybe LatLong -> Maybe CacKey -> Maybe Text -> Maybe Meters -> Maybe Seconds -> DTC.TripCategory -> m FarePoliciesProduct
getAllFarePoliciesProduct merchantId merchantOpCityId isDashboard fromlocaton mbToLocation txnId mbGeohash mbDistance mbDuration tripCategory = do
  let searchSources = FareProduct.getSearchSources isDashboard
  ...
  farePolicies <- mapM (getFullFarePolicy mbGeohash mbDistance mbDuration txnId Nothing) allFareProducts.fareProducts
  return $
    FarePoliciesProduct
      { farePolicies,
...
getFullFarePolicy :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r, EsqDBReplicaFlow m r) => Maybe Text -> Maybe Meters -> Maybe Seconds -> Maybe CacKey -> Maybe UTCTime -> FareProduct.FareProduct -> m FarePolicyD.FullFarePolicy
getFullFarePolicy mbGeohash mbDistance mbDuration txnId mbBookingStartTime fareProduct = do
  ...
  _bookingStartTime <- maybe getCurrentTime return mbBookingStartTime
  let localTimeZoneSeconds = 19800 -- fix this
  congestionChargeMultiplierFromModel <-
    case fareProduct.tripCategory of
      DTC.OneWay _ -> do
        maybe (return Nothing) (checkGeoHashAndCalculate localTimeZoneSeconds whiteListedGeohashes blackListedGeohashes) mbGeohash
  ...
  where
    checkGeoHashAndCalculate localTimeZoneSeconds whiteListedGeohashes blackListedGeohashes geohash =
      if (elem geohash whiteListedGeohashes || not (elem geohash blackListedGeohashes))
        then getCongestionChargeMultiplierFromModel' localTimeZoneSeconds (Just geohash) fareProduct.vehicleServiceTier mbDistance mbDuration fareProduct.merchantOperatingCityId
        else return Nothing

updateCongestionChargeMultiplier :: FarePolicyD.FarePolicy -> Maybe FarePolicyD.CongestionChargeMultiplier -> Maybe Double -> FarePolicyD.FarePolicy
updateCongestionChargeMultiplier FarePolicyD.FarePolicy {..} congestionMultiplier congestionChargePerMin' = FarePolicyD.FarePolicy {congestionChargeMultiplier = congestionMultiplier, congestionChargePerMin = congestionChargePerMin', ..}
...
getCongestionChargeMultiplierFromModel' timeDiffFromUtc (Just geohash) serviceTier (Just (Meters distance)) (Just (Seconds duration)) merchantOperatingCityId = do
  ...
  mbSupplyDemandRatio <- Hedis.withCrossAppRedis $ Hedis.get $ mkSupplyDemandRatioKeyWithGeohash geohash serviceTier
