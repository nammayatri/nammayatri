...
getFullFarePolicy mbFromLocation mbToLocation mbFromLocGeohash mbToLocGeohash mbDistance mbDuration txnId mbBookingStartTime mbBaseVaraintCarPrice mbAppDynamicLogicVersion fareProduct = do
  ...
  congestionChargeMultiplierFromModel <-
    case fareProduct.tripCategory of
      DTC.OneWay v | (v /= MeterRide && v /= OneWayRideOtp) -> do
...
getCongestionChargeMultiplierFromModel' timeDiffFromUtc (Just fromLocation) (Just fromLocGeohash) toLocGeohash serviceTier vehicleCategory (Just (Meters distance)) (Just (Seconds duration)) (Just True) radius' (Just dynamicPricingLogicVersion) merchantOperatingCityId = do
  ...
  let dynamicPricingData = DynamicPricingData {serviceTier, speedKmh, distanceInKm, supplyDemandRatioFromLoc = fromMaybe 0.0 mbSupplyDemandRatioFromLoc, supplyDemandRatioToLoc = fromMaybe 0.0 mbSupplyDemandRatioToLoc, toss, actualQAR = fromMaybe 0 actualQAR, actualQARPast = fromMaybe 0 actualQARPast, congestionMultiplier = fromMaybe 0 congestionMultiplier, congestionMultiplierPast = fromMaybe 0 congestionMultiplierPast, rainStatus = mbRainStatus}
  if null allLogics
    then do
