...
getFarePolicyByEstOrQuoteId :: (CacheFlow m r, EsqDBFlow m r, EsqDBReplicaFlow m r) => Id DMOC.MerchantOperatingCity -> DTC.TripCategory -> DVST.ServiceTierType -> Maybe SL.Area -> Text -> Maybe Bool -> Maybe CacKey -> m FarePolicyD.FullFarePolicy
getFarePolicyByEstOrQuoteId merchantOpCityId tripCategory vehicleServiceTier area estOrQuoteId isDashboardRequest txnId = do
  Redis.safeGet (makeFarePolicyByEstOrQuoteIdKey estOrQuoteId) >>= \case
    Nothing -> do
      logWarning "Old Fare Policy Not Found, Hence using new fare policy."
      getFarePolicy merchantOpCityId (fromMaybe False isDashboardRequest) tripCategory vehicleServiceTier area txnId
...
getFarePolicy :: (CacheFlow m r, EsqDBFlow m r, EsqDBReplicaFlow m r) => Id DMOC.MerchantOperatingCity -> Bool -> DTC.TripCategory -> DVST.ServiceTierType -> Maybe SL.Area -> Maybe CacKey -> m FarePolicyD.FullFarePolicy
getFarePolicy merchantOpCityId isDashboard tripCategory serviceTier Nothing txnId = do
...
getFarePolicy merchantOpCityId isDashboard tripCategory serviceTier (Just area) txnId = do
  let searchSources = FareProduct.getSearchSources isDashboard
  mbFareProduct <-
...
mkFarePolicyBreakups mkValue mkBreakupItem mbDistance mbTollCharges farePolicy = do
  ...
  where
    ...
    mkAdditionalAmbulanceBreakups det = do
      let vehicleAgeSections = NE.sortBy (comparing (.vehicleAge)) det.slabs
      ...
      where
        ...
        mkPerKmStepFareItem perKmStepFareItems [s1] = do
          let perKmStepFareItem = mkBreakupItem (show $ Tags.PER_KM_STEP_FARE s1.vehicleAge Nothing) (mkValue $ highPrecMoneyToText s1.perKmRate)
          perKmStepFareItems <> [perKmStepFareItem]
        mkPerKmStepFareItem perKmStepFareItems (s1 : s2 : ss) = do
          let perKmStepFareItem = mkBreakupItem (show $ Tags.PER_KM_STEP_FARE s1.vehicleAge (Just s2.vehicleAge)) (mkValue $ highPrecMoneyToText s1.perKmRate)
        ...
        mkBaseStepFareItem baseStepFareItems [s1] = do
          let baseStepFareItem = mkBreakupItem (show $ Tags.BASE_STEP_FARE s1.vehicleAge Nothing) (mkValue $ highPrecMoneyToText s1.baseFare)
          baseStepFareItems <> [baseStepFareItem]
        mkBaseStepFareItem baseStepFareItems (s1 : s2 : ss) = do
          let baseStepFareCaption = show $ Tags.BASE_STEP_FARE s1.vehicleAge (Just s2.vehicleAge)
          ...
          case s1.nightShiftCharge of
            ...
            Just nightShiftCharge -> do
              let (mbNightShiftChargePercentage, mbConstantNightShiftCharge) = getNewNightShiftCharge nightShiftCharge
                  nightShiftPercentageStepFareItem = mkBreakupItem (show $ Tags.NIGHT_SHIFT_STEP_PERCENTAGE s1.vehicleAge Nothing) . mkValue <$> mbNightShiftChargePercentage
                  nightShiftConstantStepFareItem = mkBreakupItem (show $ Tags.CONSTANT_NIGHT_SHIFT_STEP_CHARGE s1.vehicleAge Nothing) . mkValue <$> mbConstantNightShiftCharge
        ...
        mkNightShiftStepFareItem nightShiftStepFareItems (s1 : s2 : ss) = do
          case s1.nightShiftCharge of
            ...
            Just nightShiftCharge -> do
              let (mbNightShiftChargePercentage, mbConstantNightShiftCharge) = getNewNightShiftCharge nightShiftCharge
                  nightShiftPercentageStepFareItem = mkBreakupItem (show $ Tags.NIGHT_SHIFT_STEP_PERCENTAGE s1.vehicleAge (Just s2.vehicleAge)) . mkValue <$> mbNightShiftChargePercentage
                  nightShiftConstantStepFareItem = mkBreakupItem (show $ Tags.CONSTANT_NIGHT_SHIFT_STEP_CHARGE s1.vehicleAge (Just s2.vehicleAge)) . mkValue <$> mbConstantNightShiftCharge
          ...
          case s1.platformFeeInfo of
            ...
            Just platformFeeInfo -> do
              let (mbProgressivePlatformFee, mbConstantPlatformFee) = getPlatformFee platformFeeInfo
                  ...
                  mbProgressivePlatformFeeStepFareItem = mkBreakupItem (show $ Tags.PLATFORM_FEE_STEP_FARE s1.vehicleAge Nothing) . mkValue <$> mbProgressivePlatformFee
                  mbConstantPlatformFeeItem = mkBreakupItem (show $ Tags.CONSTANT_PLATFORM_FEE_STEP_FARE s1.vehicleAge Nothing) . mkValue <$> mbConstantPlatformFee
                  mbPlatformFeeCgstItem = mkBreakupItem (show $ Tags.PLATFORM_FEE_CGST_STEP_FARE s1.vehicleAge Nothing) (mkValue $ show platformFeeInfo.cgst)
                  mbPlatformFeeSgstItem = mkBreakupItem (show $ Tags.PLATFORM_FEE_SGST_STEP_FARE s1.vehicleAge Nothing) (mkValue $ show platformFeeInfo.sgst)
          ...
          case s1.platformFeeInfo of
            ...
            Just platformFeeInfo -> do
              let (mbProgressivePlatformFee, mbConstantPlatformFee) = getPlatformFee platformFeeInfo
                  ...
                  mbProgressivePlatformFeeStepFareItem = mkBreakupItem (show $ Tags.PLATFORM_FEE_STEP_FARE s1.vehicleAge (Just s2.vehicleAge)) . mkValue <$> mbProgressivePlatformFee
                  mbConstantPlatformFeeItem = mkBreakupItem (show $ Tags.CONSTANT_PLATFORM_FEE_STEP_FARE s1.vehicleAge (Just s2.vehicleAge)) . mkValue <$> mbConstantPlatformFee
                  mbPlatformFeeCgstItem = mkBreakupItem (show $ Tags.PLATFORM_FEE_CGST_STEP_FARE s1.vehicleAge (Just s2.vehicleAge)) (mkValue $ show platformFeeInfo.cgst)
                  mbPlatformFeeSgstItem = mkBreakupItem (show $ Tags.PLATFORM_FEE_SGST_STEP_FARE s1.vehicleAge (Just s2.vehicleAge)) (mkValue $ show platformFeeInfo.sgst)
          ...
          case s1.waitingChargeInfo of
            ...
            Just waitingChargeInfo -> do
              let (_, mbWaitingChargePerMinFloat, mbConstantWaitingCharges) = getWaitingCharge waitingChargeInfo
                  ...
                  mbWaitingChargePerMinFloatStepFareItem = mkBreakupItem (show $ Tags.WAITING_CHARGE_PER_MIN_STEP_FARE s1.vehicleAge Nothing) . mkValue <$> mbWaitingChargePerMinFloat
                  mbConstantWaitingChargeStepFareItem = mkBreakupItem (show $ Tags.CONSTANT_WAITING_CHARGE_STEP_FARE s1.vehicleAge Nothing) . mkValue <$> mbConstantWaitingCharges
                  freeWaitingTimeStepFareItem = mkBreakupItem (show $ Tags.FREE_WAITING_TIME_STEP_MINUTES s1.vehicleAge Nothing) . mkValue . show $ waitingChargeInfo.freeWaitingTime.getMinutes
          ...
          case s1.waitingChargeInfo of
            ...
            Just waitingChargeInfo -> do
              let (_, mbWaitingChargePerMinFloat, mbConstantWaitingCharges) = getWaitingCharge waitingChargeInfo
                  ...
                  mbWaitingChargePerMinFloatStepFareItem = mkBreakupItem (show $ Tags.WAITING_CHARGE_PER_MIN_STEP_FARE s1.vehicleAge (Just s2.vehicleAge)) . mkValue <$> mbWaitingChargePerMinFloat
                  mbConstantWaitingChargeStepFareItem = mkBreakupItem (show $ Tags.CONSTANT_WAITING_CHARGE_STEP_FARE s1.vehicleAge (Just s2.vehicleAge)) . mkValue <$> mbConstantWaitingCharges
                  freeWaitingTimeStepFareItem = mkBreakupItem (show $ Tags.FREE_WAITING_TIME_STEP_MINUTES s1.vehicleAge (Just s2.vehicleAge)) . mkValue . show $ waitingChargeInfo.freeWaitingTime.getMinutes
