  ...
  case mbVehicleServiceTierItem of
    Just vehicleServiceTierItem -> do
      if vehicleServiceTierItem.vehicleCategory == Just DVC.CAR && isJust mbBaseVariantCarPrice && not (fromMaybe True vehicleServiceTierItem.baseVehicleServiceTier) && DTT.isOneWayTrip fareProduct.tripCategory
        then do
          let baseVariantPrice = fromMaybe 0.0 mbBaseVariantCarPrice
          estimatedFare <- calculateFareForFarePolicy fullFarePolicy mbDistance mbDuration fareProduct.merchantOperatingCityId
          if baseVariantPrice > estimatedFare
            then do
              logWarning $ "Fare with addition: " <> show baseVariantPrice <> " is greater than estimated fare: " <> show estimatedFare
              let fullFarePolicy' = fullFarePolicy{additionalCongestionCharge = baseVariantPrice - estimatedFare}
