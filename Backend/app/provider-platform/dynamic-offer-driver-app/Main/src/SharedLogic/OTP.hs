...
--   Maybe Text ->
--   m ()
-- sendOTP otpChannel otpCode personId merchantId merchantOperatingCityId mbCountryCode mbMobileNumber = do
--   smsCfg <- asks (.smsCfg)
...
--         Sms.sendSMS merchantId merchantOperatingCityId (Sms.SendSMSReq message phoneNumber sender templateId)
--           >>= Sms.checkSmsResult
--     WHATSAPP -> throwError $ InvalidRequest "WHATSAPP OTP channel is not supported for drivers"
--     EMAIL -> throwError $ InvalidRequest "EMAIL OTP channel is not supported for drivers"

-- | Send OTP based on identifier type (MOBILENUMBER, EMAIL, AADHAAR)
-- This function handles the channel selection and OTP dispatch
sendOTPByIdentifierType ::
  ( HasFlowEnv m r '["smsCfg" ::: SmsConfig, "kafkaProducerTools" ::: KafkaProducerTools],
    CacheFlow m r,
    EsqDBFlow m r,
