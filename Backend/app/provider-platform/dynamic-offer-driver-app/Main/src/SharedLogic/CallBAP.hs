...
callOnSelectV2 transporter searchRequest searchTry content = do
  ...
  fork "sending on select, pushing ondc logs" do
    void $ pushLogs "on_select" (toJSON (Spec.OnSelectReq context Nothing (Just content))) bppConfig.logsToken bppConfig.logsUrl
...
callOnConfirmV2 transporter context content bppConfig = do
  ...
  fork "sending on confirm, pushing ondc logs" do
    void $ pushLogs "on_confirm" (toJSON (Spec.OnConfirmReq context_ Nothing (Just content))) bppConfig.logsToken bppConfig.logsUrl
...
sendRideAssignedUpdateToBAP booking ride driver veh = do
  ...
  mbTransporterConfig <- CQTC.findByMerchantOpCityId booking.merchantOperatingCityId (Just booking.transactionId) (Just "transactionId") -- these two lines just for backfilling driver vehicleModel from idfy TODO: remove later
  ...
  fork "sending on update, pushing ondc logs" do
    void $ pushLogs "on_update" (toJSON rideAssignedMsgV2) bppConfig.logsToken bppConfig.logsUrl
...
sendRideStartedUpdateToBAP booking ride tripStartLocation = do
  ...
  fork "sending on status, pushing ondc logs" do
    void $ pushLogs "on_status" (toJSON rideStartedMsgV2) bppConfig.logsToken bppConfig.logsUrl
...
sendRideCompletedUpdateToBAP booking ride fareParams paymentMethodInfo paymentUrl tripEndLocation = do
  ...
  fork "sending on update, pushing ondc logs" do
    void $ pushLogs "on_update" (toJSON rideCompletedMsgV2) bppConfig.logsToken bppConfig.logsUrl
...
sendDriverArrivalUpdateToBAP booking ride arrivalTime = do
  ...
  fork "sending on update, pushing ondc logs" do
    void $ pushLogs "on_update" (toJSON driverArrivedMsgV2) bppConfig.logsToken bppConfig.logsUrl
...
sendStopArrivalUpdateToBAP booking ride driver vehicle = do
  ...
  when isValueAddNP $ do
    ...
    retryConfig <- asks (.shortDurationRetryCfg)
    fork "sending on update, pushing ondc logs" do
      void $ pushLogs "on_update" (toJSON stopArrivedMsgV2) bppConfig.logsToken bppConfig.logsUrl
...
sendNewMessageToBAP booking ride message = do
  ...
  when isValueAddNP $ do
    ...
    fork "sending on update, pushing ondc logs" do
      void $ pushLogs "on_update" (toJSON newMessageMsgV2) bppConfig.logsToken bppConfig.logsUrl
...
sendSafetyAlertToBAP booking ride reason driver vehicle = do
  ...
  when isValueAddNP $ do
    ...
    fork "sending on update, pushing ondc logs" do
      void $ pushLogs "on_update" (toJSON safetyAlertMsgV2) bppConfig.logsToken bppConfig.logsUrl
...
sendEstimateRepetitionUpdateToBAP booking ride estimateId cancellationSource driver vehicle = do
  ...
  when isValueAddNP $ do
    ...
    fork "sending on update, pushing ondc logs" do
      void $ pushLogs "on_update" (toJSON estimateRepMsgV2) bppConfig.logsToken bppConfig.logsUrl
