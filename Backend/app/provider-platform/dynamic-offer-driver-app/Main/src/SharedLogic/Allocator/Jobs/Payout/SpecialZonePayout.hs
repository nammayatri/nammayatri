          ...
          if amount <= 0
            then do
              ...
              let entityName = DLP.SPECIAL_ZONE_PAYOUT
                  createPayoutOrderReq = Payout.mkCreatePayoutServiceReq uid amount merchantOperatingCity.currency phoneNo person.email driverId.getId "Payout for Airport Ride" (Just person.firstName) mbVpa "FULFILL_ONLY" False payoutServiceFlow Nothing
              case merchantId of
                Nothing -> do
