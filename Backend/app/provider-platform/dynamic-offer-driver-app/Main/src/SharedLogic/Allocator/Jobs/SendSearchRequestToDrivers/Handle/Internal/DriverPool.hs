...
prepareDriverPoolBatch driverPoolCfg searchReq batchNum = withLogTag ("BatchNum-" <> show batchNum) $ do
  ...
  where
    prepareDriverPoolBatch' previousBatchesDrivers = do
      ...
      allNearbyDriversCurrentlyOnRide <- calcDriverCurrenlyOnRidePool radiusStep reduceRadiousValue
      let allNearbyDrivers = allNearbyDriversCurrentlyNotOnRide ++ allNearbyDriversCurrentlyOnRide
