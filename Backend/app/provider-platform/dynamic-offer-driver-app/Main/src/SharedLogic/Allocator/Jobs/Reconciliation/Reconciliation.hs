...
doReconciliationDsrVsLedger merchantId merchantOpCityId startTime endTime now = do
  ...
  let validEntries = catMaybes entries
      ...
      -- Check which bookings already have DSRvsLedger status set from domain-level reconciliation
      entriesNeedingStatusUpdate = filter (\e -> e.reconStatus /= ReconEntry.MATCHED) validEntries

  -- Update booking reconciliation statuses with JSON format
  forM_ entriesNeedingStatusUpdate $ \entry -> do
