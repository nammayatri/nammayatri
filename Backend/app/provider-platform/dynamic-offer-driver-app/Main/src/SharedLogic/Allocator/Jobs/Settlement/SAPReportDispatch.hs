  ...
  mbResult <- Hedis.whenWithLockRedisAndReturnValue lockKey lockTTLSeconds $ do
    ...
    case mbSAPConfig of
      ...
      Just sapCfg -> do
        ...
        tokenResp <- SAP.fetchSAPToken sapCfg
    ...
    dispatchEntry sapCfg token mId mocid maxRetries entryType reports = do
      ...
      case result of
        Left err -> do
          ...
          pure False
        Right resp -> do
          logInfo $ "SAP " <> label <> " dispatch response: " <> show resp
          saveSapJournalEntries req resp (toTransactionType entryType) (length reports) mId mocid
          pure True
...
buildJournalRequest sapCfg entryType reports = do
  ...
  headers <- forM reports $ \report -> do
    bId <- getNextBatchId
    pure $ buildHeader sapCfg entryType reqDate reqTime bId report
...
dispatchSubscriptionPurchase sapCfg token mId mocid maxRetries totals = do
  ...
  case result of
    ...
    Right resp -> do
      ...
      saveSapJournalEntries req resp SJE.SubscriptionPurchase 1 mId mocid
      pure True
...
saveSapJournalEntries ::
  ...
  Text ->
  m ()
saveSapJournalEntries req resp txnType txnCount mId mocid = do
  now <- getCurrentTime
  let reqHeaders = req.headers
      respHeaders = resp.responseHeaders
  forM_ (zip reqHeaders respHeaders) $ \(reqHeader, respHeader) -> do
    entryId <- generateGUID
    let (totalDebit, totalCredit) = computeDebitCreditTotals reqHeader.items
    QSJE.create
      SJE.SapJournalEntry
        { id = Id entryId,
          belnr = respHeader.belnr,
          ...
          sapMessage = respHeader.message,
          rawResponse = Just resp.rawXml,
          merchantId = mId,
...
computeDebitCreditTotals items =
  let parseAmt item = fromMaybe 0 (readMaybe (T.unpack item.amount) :: Maybe HighPrecMoney)
      debitTotal = sum [parseAmt item | item <- items, item.shkzg == "S"]
      creditTotal = sum [parseAmt item | item <- items, item.shkzg == "H"]
