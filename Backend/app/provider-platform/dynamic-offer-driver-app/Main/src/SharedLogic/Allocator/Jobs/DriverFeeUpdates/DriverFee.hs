...
createPercentageVendorFees subscriptionConfigs driverFee totalFeeWithCoinDeduction now = do
  case (subscriptionConfigs.enableVendorPercentageSplit, subscriptionConfigs.vendorSplitApplicableAt) of
    (Just True, Just DRIVER_FEE_CALCULATION) -> do
      ...
      if totalSplitPercentage > 100.0
        then do
          logError $ "Vendor split percentages exceed 100%. Total percentage: " <> show totalSplitPercentage <> "% for DriverFeeId: " <> driverFee.id.getId <> ", MerchantOpCityId: " <> driverFee.merchantOperatingCityId.getId <> ", VehicleVariant: " <> show vehicleVariant <> ". Skipping vendor fee creation."
