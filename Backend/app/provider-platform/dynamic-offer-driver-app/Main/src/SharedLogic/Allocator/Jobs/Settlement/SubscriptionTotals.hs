...
fetchPGSettlementTotals merchantId merchantOperatingCityId fromTime toTime = do
  ...
  let totals =
      ...
      orderRows = filter (\r -> r.txnType == PgDom.ORDER) txnRows
      refundRows = filter (\r -> r.txnType == PgDom.REFUND) txnRows
      chargebackRows = filter (\r -> r.txnType == PgDom.CHARGEBACK) txnRows
  ...
  where
    toTransactionRow (txnAmt, refAmt, cbAmt, tt, ts, spId) =
      let amt = case tt of
       ...
       in PGSettlementTransactionRow amt tt (show ts) spId
