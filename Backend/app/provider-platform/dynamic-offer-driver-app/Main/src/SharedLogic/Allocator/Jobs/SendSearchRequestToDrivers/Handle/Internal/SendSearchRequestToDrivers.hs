...
sendSearchRequestToDrivers isAllocatorBatch tripQuoteDetails oldSearchReq searchTry driverPoolConfig driverPool prevBatchDrivers goHomeConfig = do
  ...
  -- This is a cache for coin configurations by service tier type; computed once and shared with
  -- attemptPriorityDirectAssign below so a gold-tier batch doesn't look it up twice.
  coinConfigCache <-
...
attemptPriorityDirectAssign merchant searchReq searchTry tripQuoteDetails cityServiceTiers driverPoolCfg batchNum transporterConfig coinConfigCache batch = do
  if null sortedPriority
    then pure batch
      ...
      -- Closes over transporterConfig/tripQuoteDetailsHashMap/coinConfigCache/etc. above instead of
      -- threading them through every recursive call -- only the remaining candidate list varies.
      let tryAssign :: [SDP.DriverPoolWithActualDistResult] -> m Bool
          tryAssign [] = pure False
          tryAssign (dp : rest) = do
            ...
            if not locked
              then tryAssign rest
                ...
                result :: Either SomeException Bool <- C.try $ do
                  ...
                  walletOk <- case autoAcceptanceConfigForTier dp.driverPoolResult.serviceTier >>= (.minWalletBalance) of
                    Nothing -> pure True
                    Just minBalance -> do
                  ...
                  -- Per review: offerQuoteLockKeyWithCoolDown above is trusted to rule out an active quote elsewhere.
                  if onRide || not isStillLive
                    then pure False
                    else do
                ...
                case result of
                  ...
                  _ -> unlockThisDriver >> tryAssign rest
      assigned <- tryAssign sortedPriority
