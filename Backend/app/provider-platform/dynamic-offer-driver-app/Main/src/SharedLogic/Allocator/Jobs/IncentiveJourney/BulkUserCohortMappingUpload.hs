...
runBulkUserCohortMappingUploadJob Job {id, jobInfo} = withLogTag ("JobId-" <> id.getId <> " RunId-" <> jobInfo.jobData.runId) $ do
  ...
  -- Best-effort: progress table must never fail the upload job.
  safeUpdateProgress runId DBulkRun.Running offset (Just total) 0 0 (Just id.getId)
  ...
  forM_ distinctUserIds $ \uid ->
    CQAssignment.clearCacheByPersonId IJ.DriverActor uid
    ...
    then do
      ...
      void $
        JC.createJobIn @_ @'BulkUserCohortMappingUpload
          ...
          nextJobData
      safeUpdateProgress runId DBulkRun.Running nextOffset (Just total) insertedCount skipped (Just id.getId)
      logInfo $
      ...
      safeUpdateProgress runId DBulkRun.Succeeded nextOffset (Just total) insertedCount skipped (Just id.getId)
      logInfo $
...
-- | Never lets progress-table failures abort CSV processing / scheduler chaining.
safeUpdateProgress ::
  (BeamFlow m r) =>
  Id DBulkRun.BulkUserCohortMappingRun ->
  DBulkRun.BulkUserCohortMappingRunStatus ->
  Int ->
  Maybe Int ->
  Int ->
  Int ->
  Maybe Text ->
  m ()
safeUpdateProgress runId status offset totalRows insertedDelta skippedDelta mbSchedulerJobId = do
  result <-
    try @_ @SomeException $
      QBulkRunExtra.updateProgress runId status offset totalRows insertedDelta skippedDelta mbSchedulerJobId
  case result of
    Right _ -> pure ()
    Left err ->
      logWarning $
        "BulkUserCohortMappingUpload: progress update failed runId="
          <> getId runId
          <> " err="
          <> show err
