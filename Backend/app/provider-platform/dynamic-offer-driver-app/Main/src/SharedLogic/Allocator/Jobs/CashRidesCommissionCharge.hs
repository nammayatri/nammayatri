module SharedLogic.Allocator.Jobs.CashRidesCommissionCharge
  ( cashRidesCommissionCharge,
    cashRidesCommissionChargeProccessingKey,
  )
where
...
import qualified Kernel.External.Payment.Stripe.Types.Transfer as Tranfer
...
cashRidesCommissionChargeForSinglePerson' jobData personRides personId isFleetOwner = do
  ...
  Redis.whenWithLockRedis (cashRidesCommissionChargeProccessingKey personId) 60 $ do
              ...
              updCashRidesCommission <-
                SPayment.makeStripeTransfer merchantId merchantOperatingCityId personBankAccount.paymentMode DTransferTransaction.CASH_RIDES_COMMISSION entityId createTranferReq >>= \case
                  Left err -> do
                    ...
                    pure cashRidesCommission{status = Tranfer.TRANSFER_FAILED}
                  Right resp -> do
      ...
      pure
        DCashRidesCommission.CashRidesCommission
          { id,
            ...
            status = Tranfer.TRANSFER_PENDING,
            merchantId = jobData.merchantId,
            merchantOperatingCityId = jobData.merchantOperatingCityId,
            paymentMode,
            createdAt = now,
...
cashRidesCommissionChargeProccessingKey :: Kernel.Types.Id.Id DP.Person -> Text
cashRidesCommissionChargeProccessingKey personId = "CashRidesCommissionCharge:Processing:PersonId" <> personId.getId
