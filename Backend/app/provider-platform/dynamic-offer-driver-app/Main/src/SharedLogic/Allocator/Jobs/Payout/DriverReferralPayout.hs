        ...
        Just vpa -> do
          ...
          if referralEarnings <= payoutConfig.thresholdPayoutAmountPerPerson
            then do
              ...
              mbPayoutOrderResp <- try @_ @SomeException $ Payout.createPayoutService (cast person.merchantId) (cast driverId) (Just [id]) (Just entityName) Nothing (show merchantOperatingCity.city) createPayoutOrderReq createPayoutOrderCall
