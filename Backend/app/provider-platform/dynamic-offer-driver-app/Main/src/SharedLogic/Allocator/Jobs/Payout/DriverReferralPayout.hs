            ...
            then pure ()
              ...
              if directBase <= payoutConfig.thresholdPayoutAmountPerPerson
                then do
                  ...
                  mbPayoutOrderResp <- withTryCatch "createPayoutService:callPayout" $ Payout.createPayoutService (cast person.merchantId) (cast <$> merchantOperatingCityId) (cast driverId) (Just [id]) (Just entityName) (show merchantOperatingCity.city) createPayoutOrderReq createPayoutOrderCall Nothing (Just afterPayoutOrderCreated)
          ...
          when gotLock $ do
            ...
            whenJust mbPayoutConfig $ \payoutConfig -> do
              ...
              when (registrationAmount > 0) $ do
                ...
                result <- try @_ @SomeException $ Payout.createPayoutService (cast person.merchantId) (Just $ cast driverFee.merchantOperatingCityId) (cast driverId) Nothing (Just entityName) (show merchantOperatingCity.city) createPayoutOrderReq createPayoutOrderCall Nothing (Just afterPayoutOrderCreated)
                case result of
                  Right _ -> pure ()
                  Left err -> do
