...
sendOverlayToDriver (Job {id, jobInfo}) = withLogTag ("JobId-" <> id.getId) do
  ...
  driverIds <- nub <$> getBatchedDriverIds merchantId jobId jobData.condition jobData.freeTrialDays jobData.timeDiffFromUtc jobData.driverPaymentCycleDuration jobData.driverPaymentCycleStartTime jobData.overlayBatchSize serviceName
...
getBatchedDriverIds ::
  ...
  DPlan.ServiceNames ->
  m [Id DP.Person]
getBatchedDriverIds merchantId jobId condition freeTrialDays timeDiffFromUtc driverPaymentCycleDuration driverPaymentCycleStartTime overlayBatchSize serviceName = do
  case condition of
    DOverlay.InvoiceGenerated paymentMode -> do
      ...
      driverFees <- QDF.findWindowsAndServiceNameWithFeeTypeAndLimitAndServiceName merchantId startTime endTime (getFeeType paymentMode) overlayBatchSize serviceName
