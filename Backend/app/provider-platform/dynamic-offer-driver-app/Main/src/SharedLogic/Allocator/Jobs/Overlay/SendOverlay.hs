  ...
  case condition of
    DOverlay.InvoiceGenerated paymentMode -> do
      ...
      driverFees <- case shardNum of
        ...
        Just shard -> do
          activeDriverIds <- ADL.getNextDriverBatch ("SendOverlay:" <> merchantId.getId <> ":" <> merchantOperatingCityId.getId <> ":" <> show serviceName <> ":" <> show paymentMode) shard startTime (secondsToNominalDiffTime timeDiffFromUtc) overlayBatchSize
