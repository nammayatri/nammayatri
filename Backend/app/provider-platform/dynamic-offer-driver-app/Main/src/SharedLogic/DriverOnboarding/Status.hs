...
--   whose deps may not be mandatory). Extracted from checkAllVehicleDocsVerifiedForRC so callers that
...
-- | Thin Bool wrapper over fetchVehicleDocStatusesForRC (mandatory set) — "are all mandatory vehicle
--   docs for this RC VALID?". Used by vehicle inspection approval; behaviour unchanged from before the
--   fetch was extracted. mbPerson = Nothing for vehicle inspection (no driver linked).
checkAllVehicleDocsVerifiedForRC ::
  RC.VehicleRegistrationCertificate ->
  DMOC.MerchantOperatingCity ->
  DTC.TransporterConfig ->
...
checkAllVehicleDocsVerifiedForRC rc merchantOperatingCity transporterConfig language reqRegistrationNo = do
  ...
  -- Ops-approve verified check: uses isMandatory only (enabling-only docs like MSDS/Medical excluded).
  pure $ checkAllVehicleMandatoryDocsValid allDocumentVerificationConfigs vehicleDoc makeSelfieAadhaarPanMandatory

-- | Thin Bool wrapper over fetchDriverDocStatusesForPerson (mandatory set) — "are all mandatory driver
--   docs VALID?". Used by driver inspection approval; behaviour unchanged from before the fetch was
--   extracted (under enableBotFlow it applies the applicableTo filter, as before).
checkAllDriverDocsVerifiedForDriver ::
  DP.Person ->
  DMOC.MerchantOperatingCity ->
  DTC.TransporterConfig ->
...
checkAllDriverDocsVerifiedForDriver person merchantOperatingCity transporterConfig language = do
  ...
  let makeSelfieAadhaarPanMandatory = Nothing
  -- Under enableBotFlow, apply the applicableTo filter (via fleet-linkage); legacy passes Nothing.
  mbIsFleetDriver <-
    if transporterConfig.enableBotFlow == Just True
      then Just . isJust <$> QFDA.findByDriverId person.id True
      else pure Nothing
  pure $ checkAllDriverDocsVerified' ForVerified mbIsFleetDriver allDocVerificationConfigs person.role driverDocuments vehicleCategory makeSelfieAadhaarPanMandatory
...
--   default list (needed e.g. for the dependency check). Extracted from checkAllDriverDocsVerifiedForDriver
    ...
    if enableBotFlow
      then do
        ...
        when (isFleetRole person.role) $
          recomputeFleetVerifiedAndEnabled person allDocVerificationConfigs driverDocuments vehicleCategory makeSelfieAadhaarPanMandatory
        when (person.role == DP.DRIVER) $
          recomputeDriverVerifiedAndEnabled merchantOpCityId merchantId person allDocVerificationConfigs driverDocuments vehicleCategory makeSelfieAadhaarPanMandatory (mDL >>= (.driverName)) onboardingVehicleCategory transporterConfig
    ...
    getVehicleDocuments driverDocConfs role vehicleDocumentsUnverified requiresOnboardingInspection vehicleCategoryExcludedFromVerification separateEnablement driverDocuments merchantOpCityId = do
      ...
      vehicleDocumentsUnverified `forM` \vehicleDoc@VehicleDocumentItem {..} -> do
        ...
        when enableBotFlow $ do
          let allVehicleMandatoryDocsValid = checkAllVehicleMandatoryDocsValid driverDocConfs vehicleDoc makeSelfieAadhaarPanMandatory
  ...
  case allDocVerificationConfigs of
    Left fleetConfigs -> do
      ...
      let exactRoleConfigs = filter (\config -> config.role == role) fleetConfigs
          ...
          -- Use isMandatoryForEnabling (falling back to isMandatory) so enabling-only docs
          -- (e.g. fleet OperatorCode) are fetched too — needed for the fleet `enabled` computation.
          mandatoryDocTypes = nub $ map (.documentType) $ filter (\c -> fromMaybe c.isMandatory c.isMandatoryForEnabling) effectiveConfigs
...
-- | Legacy "all vehicle docs valid" check — uses isMandatoryForEnabling. Drives enable / RC activation.
checkAllVehicleDocsVerified ::
  [DVC.DocumentVerificationConfig] ->
  VehicleDocumentItem ->
  Maybe Bool ->
  Bool
checkAllVehicleDocsVerified = checkAllVehicleDocsVerified' ForEnabling

-- | Are all isMandatory vehicle docs VALID? Excludes isMandatoryForEnabling-only docs. Drives
--   @verified@. (Sibling checkAllVehicleDocsVerified uses the isMandatoryForEnabling set → @enabled@.)
checkAllVehicleMandatoryDocsValid ::
  [DVC.DocumentVerificationConfig] ->
  VehicleDocumentItem ->
  Maybe Bool ->
  Bool
checkAllVehicleMandatoryDocsValid = checkAllVehicleDocsVerified' ForVerified
...
checkAllVehicleDocsVerified' ::
  ...
  Maybe Bool ->
  Bool
checkAllVehicleDocsVerified' mode driverDocConfs vehicleDoc makeSelfieAadhaarPanMandatory = do
...
-- | Legacy "all driver docs valid" check — uses isMandatoryForEnabling, no applicableTo filtering. Drives enable.
checkAllDriverDocsVerified ::
  ...
  Maybe Bool ->
  Bool
checkAllDriverDocsVerified = checkAllDriverDocsVerified' ForEnabling Nothing

-- | Are all isMandatory driver docs VALID? Excludes isMandatoryForEnabling-only docs. Drives
--   @verified@. (Sibling checkAllDriverDocsVerified uses the isMandatoryForEnabling set → @enabled@.)
checkAllDriverMandatoryDocsValid ::
  DocVerificationConfigs ->
  DP.Role ->
  [DocumentStatusItem] ->
  DVC.VehicleCategory ->
  Maybe Bool ->
  Bool
checkAllDriverMandatoryDocsValid = checkAllDriverDocsVerified' ForVerified Nothing
...
checkAllDriverDocsVerified' ::
  ...
  Maybe Bool ->
  Bool
checkAllDriverDocsVerified' mode mbIsFleetDriver allDocVerificationConfigs role driverDocuments vehicleCategory makeSelfieAadhaarPanMandatory = do
...
recomputeDriverVerifiedAndEnabled merchantOpCityId merchantId person allDocVerificationConfigs driverDocuments vehicleCategory makeSelfieAadhaarPanMandatory driverName onboardingVehicleCategory transporterConfig = do
  -- A fleet driver (active fleet association) skips INDIVIDUAL-only docs like OperatorCode, via applicableTo.
  isFleetDriver <- isJust <$> QFDA.findByDriverId person.id True
  let allMandatoryDocsValid = checkAllDriverDocsVerified' ForVerified (Just isFleetDriver) allDocVerificationConfigs person.role driverDocuments vehicleCategory makeSelfieAadhaarPanMandatory
      allEnablingDocsValid = checkAllDriverDocsVerified' ForEnabling (Just isFleetDriver) allDocVerificationConfigs person.role driverDocuments vehicleCategory makeSelfieAadhaarPanMandatory
...
--     verified = all isMandatory fleet docs VALID            (excludes OperatorCode)
--     enabled  = all isMandatoryForEnabling fleet docs VALID (incl. OperatorCode — the BOT-set enable gate)
...
recomputeFleetVerifiedAndEnabled person allDocVerificationConfigs driverDocuments vehicleCategory makeSelfieAadhaarPanMandatory = do
  let allFleetMandatoryDocsValid = checkAllDriverMandatoryDocsValid allDocVerificationConfigs person.role driverDocuments vehicleCategory makeSelfieAadhaarPanMandatory
      allFleetEnablingDocsValid = checkAllDriverDocsVerified allDocVerificationConfigs person.role driverDocuments vehicleCategory makeSelfieAadhaarPanMandatory
...
getProcessedDriverDocuments role driverId entityImagesInfo docType useHVSdkForDL = do
  ...
  case docType of
    ...
    DVC.OperatorCode -> do
      status <- getOperatorCodeStatus role driverId
      return (status, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
...
-- | OperatorCode status by role: fleet reads fleet_operator_association, driver reads driver_operator_association.
getOperatorCodeStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => DP.Role -> Id DP.Person -> m (Maybe ResponseStatus)
getOperatorCodeStatus role personId
  | isFleetRole role = getOperatorCodeStatusForFleet personId
  | otherwise = getOperatorCodeStatusForDriver personId

-- | Driver OperatorCode status, derived from the active driver-operator association
--   (activated by postOperatorConsent).
getOperatorCodeStatusForDriver :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => Id DP.Person -> m (Maybe ResponseStatus)
getOperatorCodeStatusForDriver driverId = do
  mbAssoc <- QDOA.findByDriverId driverId True
  pure $ VALID <$ mbAssoc

-- | Fleet OperatorCode status from the active fleet-operator association.
getOperatorCodeStatusForFleet :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => Id DP.Person -> m (Maybe ResponseStatus)
getOperatorCodeStatusForFleet fleetOwnerId = do
...
getInProgressDriverDocuments role driverId entityImagesInfo docType possibleVehicleCategories allDocVerificationConfigs = do
  ...
  (status, mbReason, mbUrl) <- case docType of
    ...
    DDVC.OperatorCode -> do
      mbStatus <- getOperatorCodeStatus role driverId
