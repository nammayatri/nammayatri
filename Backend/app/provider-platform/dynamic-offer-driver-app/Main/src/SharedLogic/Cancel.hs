...
reAllocateBookingIfPossible isValueAddNP userReallocationEnabled merchant booking ride driver vehicle bookingCReason isForceReallocation = do
  ...
  where
    ...
    handleDriverSearchBatch driverSearchBatchInput newBooking estimateId isStatic singleBooking = do
      -- Reused booking keeps booking.id, so the triggering cancel's marker would block the next driver's accept.
      -- A rider cancel is consumed (kept present to reject a repeat reassign); only rider cancels ever set one.
      when singleBooking $
        if bookingCReason.source == SBCR.ByUser
          then CS.markBookingReallocationStarted booking.id
          else CS.markBookingCancellationCompleted booking.id
      ...
      if not isValueAddNP && not singleBooking
        then cancelRideTransactionForNonReallocation mbBookingToCancel (Just estimateId)
        else do
          -- value-add NP sends the repetition update up-front; single-booking reallocation defers to the on-success update below
          unless singleBooking $
          ...
          case result of
            Right _ -> do
              when singleBooking $ BP.sendReallocationUpdateToBAP booking ride bookingCReason.source driver vehicle
            ...
            Left err -> do
              ...
              cancelRideTransactionForNonReallocation (if singleBooking then Just newBooking else mbBookingToCancel) (Just estimateId)
    ...
    checkIfRepeatSearch searchTry driverArrivalTime isReallocationEnabled now isScheduled transporterConfig = do
      ...
      return $
        searchTry.searchRepeatCounter < searchRepeatLimit
          -- userReallocationEnabled is BAP-supplied, so isValueAddNP keeps rider-cancel reallocation to our own BAPs.
          && (bookingCReason.source == SBCR.ByDriver || (bookingCReason.source == SBCR.ByFleetOwner && scheduleReallocationAllowed) || (bookingCReason.source == SBCR.ByUser && userReallocationEnabled && isValueAddNP))
