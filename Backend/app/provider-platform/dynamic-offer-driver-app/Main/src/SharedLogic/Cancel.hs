...
reAllocateBookingIfPossible isValueAddNP userReallocationEnabled merchant booking ride driver vehicle bookingCReason isForceReallocation = do
  ...
  where
    reallocateDynamicOffer = do
      ...
      isRepeatSearch <- checkIfRepeatSearch searchTry ride.driverArrivalTime searchReq.isReallocationEnabled now booking.isScheduled transporterConfig now
      if isRepeatSearch
    ...
    reallocateStaticOffer = do
      ...
      transporterConfig <- QTC.findByMerchantOpCityId booking.merchantOperatingCityId (Just (TransactionId $ Id booking.transactionId)) >>= fromMaybeM (TransporterConfigNotFound booking.merchantOperatingCityId.getId)
      isRepeatSearch <- checkIfRepeatSearch searchTry ride.driverArrivalTime searchReq.isReallocationEnabled now searchReq.isScheduled transporterConfig now
    ...
    checkIfRepeatSearch searchTry driverArrivalTime isReallocationEnabled now isScheduled transporterConfig now = do
      ...
      return $
        searchTry.searchRepeatCounter < searchRepeatLimit
          ...
          && (driverHasNotArrived || diffUTCTime searchTry.startTime now > 1800) -- allow reallocation if driver reaches and cancels before 30 mins
