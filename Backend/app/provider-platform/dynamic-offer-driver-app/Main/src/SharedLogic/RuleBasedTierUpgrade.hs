...
computeEligibleUpgradeTiers ride transporterConfig =
  when (isJust transporterConfig.upgradeTierDropRetentionTime) $
    fork ("computeEligibleUpgradeTiers for driver: " <> ride.driverId.getId) $ do
      ...
      nammaTags <- withTryCatch "computeNammaTags:UpgradeTier" (LYDL.computeNammaTagsWithDebugLog LYDL.Driver (Id.cast ride.merchantOperatingCityId) Yudhishthira.UpgradeTier (Nothing :: Maybe Text) upgradeTierTagData)
