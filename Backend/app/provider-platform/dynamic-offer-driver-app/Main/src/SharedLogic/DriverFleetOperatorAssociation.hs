    ...
    if merchant.overwriteAssociation == Just True
      then forM_ existingFDAssociations $ \existingAssociation -> do
        ...
        when (transporterConfig.allowAnalytics == Just True) $ do
          ...
          whenJust mbOperator $ \operator -> Analytics.decrementOperatorAnalyticsApplicationCount transporterConfig operator.operatorId
  ...
  unless (null existingDOAssociations) $ do
    if merchant.overwriteAssociation == Just True
      then forM_ existingDOAssociations $ \existingAssociation -> do
        ...
        when (transporterConfig.allowAnalytics == Just True) $ Analytics.decrementOperatorAnalyticsApplicationCount transporterConfig existingAssociation.operatorId
        when (transporterConfig.allowCacheDriverFlowStatus == Just True) $ do
          driverInfo <- QDI.findById existingAssociation.driverId >>= fromMaybeM (DriverNotFound existingAssociation.driverId.getId)
