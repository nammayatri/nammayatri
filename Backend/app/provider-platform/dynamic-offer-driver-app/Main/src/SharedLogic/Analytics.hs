...
fallbackToClickHouseAndUpdateRedisForAllTimeFleet ::
  ...
  [Text] ->
  m CommonAllTimeFallbackRes
fallbackToClickHouseAndUpdateRedisForAllTimeFleet fleetOwnerId _fleetAllTimeKeysData = do
  ...
  whenJust mbActiveDriverCount $ \activeDriverCount -> Redis.set (makeFleetAnalyticsKey fleetOwnerId ACTIVE_DRIVER_COUNT) activeDriverCount
  whenJust mbActiveVehicleCount $ \activeVehicleCount -> Redis.set (makeFleetAnalyticsKey fleetOwnerId ACTIVE_VEHICLE_COUNT) activeVehicleCount
