...
getConfigFromInMemory id mbvt tripCategory dist srId = do
  ...
  bool
    ( maybe
        ( getDriverPoolConfigFromCAC id mbvt tripCategory dist Nothing
            >>= ( \config -> do
    ...
    (getDriverPoolConfigFromCAC id mbvt tripCategory dist srId)
    isExp
