...
data DriverPoolDataUpdate = DriverPoolDataUpdate
  { -- Class 2 fields (sync, LTS authoritative for dispatch)
    ...
    enableCashRide :: SetField (Maybe Bool),
...
syncDriverPoolDataToLTS driverId update = do
  ...
  Redis.withWaitAndLockRedis (driverPoolSyncLockKey driverId) 3 50000 $ do
...
applyUpdate now u d =
  d
    { DPD.lastUpdatedAt = Just now,
      ...
      DPD.enableCashRide = applyField u.enableCashRide d.enableCashRide,
