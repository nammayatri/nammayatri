...
-- summed over the `add_on_config` rows they point at. `Nothing` when none of
-- them is priced (a free opt-in), so an unpriced add-on leaves the fare alone.
--
-- Called once, where the selection is persisted, and the result is frozen into
-- `FareParameters.addOnCharges` -- the fare parameters are the record of what
-- was charged, so a later catalogue price edit can't change an existing quote's
-- or booking's fare.
addOnChargesTotal :: (EsqDBFlow m r, CacheFlow m r) => [DAddOnConfig.AddOnData] -> m (Maybe HighPrecMoney)
addOnChargesTotal [] = pure Nothing
addOnChargesTotal addOnData = do
  cfgs <- QAddOnConfig.findAllByIds (map (.configId) addOnData)
  let priceById = M.fromList [(cfg.id, price) | cfg <- cfgs, price <- maybeToList cfg.pricePerQuantity]
      charges = mapMaybe (\d -> (* fromIntegral d.selectedQuantity) <$> M.lookup d.configId priceById) addOnData
  pure $ if null charges then Nothing else Just (sum charges)
