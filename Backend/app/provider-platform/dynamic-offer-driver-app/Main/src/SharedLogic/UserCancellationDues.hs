...
data UserCancellationDuesData = UserCancellationDuesData
  { cancelledBy :: DCT.CancellationType,
    ...
    driverWaitingTime :: Int,
    callAttemptByDriver :: Bool,
    actualCoveredDistance :: Meters,
    expectedCoveredDistance :: Meters,
    cancellationDues :: HighPrecMoney,
    cancelledRides :: Int,
    totalBookings :: Int,
    estimatedBookingFare :: HighPrecMoney,
    completedRides :: Int,
  ...
  def =
    UserCancellationDuesData
      { cancelledBy = DCT.CancellationByCustomer,
        ...
        actualCoveredDistance = 0,
        expectedCoveredDistance = 0,
        cancellationDues = 0,
        cancelledRides = 0,
        totalBookings = 0,
        estimatedBookingFare = 0,
        completedRides = 0,
...
data UserCancellationDuesResult = UserCancellationDuesResult
  { cancellationCharges :: HighPrecMoney,
    cancellationChargesTax :: HighPrecMoney
  }
  deriving (Generic, Show, FromJSON, ToJSON)

instance Default UserCancellationDuesResult where
  def = UserCancellationDuesResult {cancellationCharges = 0, cancellationChargesTax = 0}
