...
buildEstimate merchant transactionId startTime dist farePolicy = do
  fareParams <- calculateFare merchant.id (Left farePolicy) dist startTime Nothing Nothing 0 Nothing Nothing Nothing
...
buildEstimateFromSlabFarePolicy merchant transactionId startTime dist slabFarePolicy = do
  fareParams <- calculateFare merchant.id (Right slabFarePolicy) dist startTime Nothing Nothing 0 Nothing Nothing Nothing
  let baseFare = fareSum fareParams
