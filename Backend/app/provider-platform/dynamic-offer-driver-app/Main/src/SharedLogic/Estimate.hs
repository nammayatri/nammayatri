...
buildEstimate org startTime dist (farePolicy, filteredPoolByVehVariant) = do
  fareParams <- calculateFare org.id (Left farePolicy) dist startTime Nothing Nothing Nothing
...
buildEstimateFromSlabFarePolicy org startTime dist (slabFarePolicy, filteredPoolByVehVariant) = do
  fareParams <- calculateFare org.id (Right slabFarePolicy) dist startTime Nothing Nothing Nothing
  let baseFare = fareSum fareParams
      currency = "INR"
