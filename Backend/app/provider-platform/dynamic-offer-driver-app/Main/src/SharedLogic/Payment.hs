...
import Domain.Types.SplitDetails as SD
...
createOrder (driverId, merchantId, opCity) serviceName (driverFees, driverFeesToAddOnExpiry) mbMandateOrder invoicePaymentMode existingInvoice splitDetails mbDeepLinkData = do
  ...
  let driverEmail = fromMaybe "test@juspay.in" driver.email
      ...
      splitSettlementDetails = if null splitDetails then Nothing else mkSplitSettlementDetails splitDetails amount
...
mkSplitSettlementDetails :: [SD.SplitDetails] -> HighPrecMoney -> Maybe SplitSettlementDetails
mkSplitSettlementDetails splitDetails totalAmount = do
  let vendorSplits =
        [ Split
            { amount = case sd.fixedAmount of
                Just fixed -> HighPrecMoney (toRational fixed)
                Nothing -> HighPrecMoney $ roundToTwoDecimalPlaces (totalAmount.getHighPrecMoney * toRational (sd.amountPercentage / 100)),
