...
mkPersonBasic :: (Id DP.Person, Text, Maybe Text) -> PersonBasic
mkPersonBasic (personId', firstName', lastName') =
  PersonBasic
...
findPersonsByIds personIds mbNameFilter mbMobileHash = do
  res <-
    CH.findAll $
      CH.select_
        (\person -> CH.notGrouped (person.id, person.firstName, person.lastName))
        $ CH.filter_
          ( \person ->
  ...
  where
    nameFilterPredicate person trimmedName =
      let searchPattern = "%" <> trimmedName <> "%"
       in CH.like_ person.firstName searchPattern
