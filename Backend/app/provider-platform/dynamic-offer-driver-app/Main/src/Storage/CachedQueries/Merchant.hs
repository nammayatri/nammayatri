...
findById id =
  Hedis.withCrossAppRedis (Hedis.safeGet $ makeIdKey id) >>= \case
    ...
    Nothing -> flip whenJust cacheMerchant /=<< Queries.findById (Proxy @m) id
...
update :: (HasCacheConfig r, HedisFlow m r) => Merchant -> Esq.SqlDB m ()
update merchant = do
  Queries.update merchant
  Esq.finalize $ cacheMerchant merchant

loadAllProviders :: Esq.Transactionable ma m => Proxy ma -> m [Merchant]
