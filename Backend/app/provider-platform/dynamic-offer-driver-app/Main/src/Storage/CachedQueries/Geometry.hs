...
findGeometryByStateAndCity city state =
  Hedis.withCrossAppRedis (Hedis.safeGet $ makeStateAndCityKey city state) >>= \case
...
clearCacheByStateAndCity city state = Hedis.withCrossAppRedis $ Hedis.del (makeStateAndCityKey city state)
...
cacheGeometry city state _geo = do
  ...
  Hedis.withCrossAppRedis $ Hedis.setExp (makeStateAndCityKey city state) _geo expTime

makeStateAndCityKey :: Context.City -> Context.IndianState -> Text
makeStateAndCityKey city state = "driver-offer:CachedQueries:Geometry:City-" <> show city <> ":State-" <> show state
