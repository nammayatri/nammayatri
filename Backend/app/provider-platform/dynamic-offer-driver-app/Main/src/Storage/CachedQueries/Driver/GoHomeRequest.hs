...
getDriverGoHomeRequestInfo driverId merchantOpCityId goHomeCfg = do
  ...
  currTime <- getLocalCurrentTime =<< ((getConfig (TransporterDimensions {merchantOperatingCityId = merchantOpCityId.getId}) >>= fromMaybeM (InternalError "Transporter config for timezone not found")) <&> (.timeDiffFromUtc))
  withCrossAppRedis $
