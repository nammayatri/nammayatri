...
deactivateDriverGoHomeRequest merchantId driverId stat ghInfo = do
  ...
  case stat of
    SUCCESS -> do
      QDGR.finishWithStatus driverGoHomeReqId DDGR.SUCCESS
      Hedis.setExp ghKey (templateGoHomeData Nothing (ghInfo.cnt - 1) Nothing Nothing False (Just ghInfo.goHomeReferenceTime) currTime) expTime
    FAILED -> do
      logDebug "Here:- inside deactivate driver go home request"
      QDGR.finishWithStatus driverGoHomeReqId DDGR.FAILED
      Hedis.setExp ghKey (templateGoHomeData Nothing ghInfo.cnt Nothing Nothing False (Just ghInfo.goHomeReferenceTime) currTime) expTime
