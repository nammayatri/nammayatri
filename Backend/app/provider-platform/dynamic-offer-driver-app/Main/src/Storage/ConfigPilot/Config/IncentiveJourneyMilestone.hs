...
fetchMilestones a =
  case a.journeyId of
    Just journeyId -> SQMilestone.findByJourneyId IJ.DriverActor journeyId
...
fetchAllMilestonesForCity merchantOpCityId = do
  journeys <- CQJourney.findByMerchantOperatingCityId IJ.DriverActor (cast merchantOpCityId)
  concat <$> mapM (SQMilestone.findByJourneyId IJ.DriverActor . (.id)) journeys
