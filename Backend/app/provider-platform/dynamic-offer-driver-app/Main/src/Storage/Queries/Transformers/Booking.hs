...
fromAndToLocation mappings tripCategory id fromLocationId toLocationId providerId merchantOperatingCityId = do
  case (mappings, tripCategory) of
    ([], Nothing) -> do
      ...
      pickupLoc <- upsertLocationForOldData (Id providerId) (Id <$> fromLocationId) id
      ...
      dropLoc <- upsertLocationForOldData (Id providerId) (Id <$> toLocationId) id
...
buildLocation :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => Id DM.Merchant -> DBBL.BookingLocation -> m DL.Location
buildLocation merchantId DBBL.BookingLocation {..} =
  return $
    DL.Location
      { id = cast id,
...
upsertLocationForOldData :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => Id DM.Merchant -> Maybe (Id DBBL.BookingLocation) -> Text -> m DL.Location
upsertLocationForOldData merchantId locationId bookingId = do
  loc <- QBBL.findById `mapM` locationId >>= fromMaybeM (InternalError "Location Id Not Found in Booking Location Table")
  location <- maybe (throwError $ InternalError ("Location Not Found in Booking Location Table for BookingId : " <> bookingId)) (buildLocation merchantId) loc
