...
getInProgressByFleetOwnerId fleetOwnerId =
  findAllWithKV [Se.And [Se.Is BeamR.fleetOwnerId $ Se.Eq (Just fleetOwnerId), Se.Is BeamR.status $ Se.Eq Ride.INPROGRESS]]
...
findAllByDriverIdAndStatuses :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => Id Person -> [Ride.RideStatus] -> m [Ride]
