...
import qualified Storage.Queries.QueriesExtra.SearchRequestLite as QRLite
...
getMerchantOpCId merchantOperatingCityId merchantId requestId = do
  merchant <- case merchantId of
    Nothing -> do
      searchReq <- QRLite.findByIdLite (Id requestId) >>= fromMaybeM (InternalError $ "Search request not found : " <> requestId)
