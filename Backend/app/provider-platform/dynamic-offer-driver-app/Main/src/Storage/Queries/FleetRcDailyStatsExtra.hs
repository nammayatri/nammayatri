...
data VehicleAggregatedStats = VehicleStats
  { fleetOwnerId :: Text,
    rcId :: Text,
...
makeVehicleAggregatedStats :: Text -> Text -> HighPrecMoney -> Int -> Meters -> Seconds -> VehicleAggregatedStats
makeVehicleAggregatedStats fleetOwnerId rcId totalEarnings totalCompletedRides totalDistance totalDuration =
  VehicleStats {..}
...
sumVehicleStatsByFleetOwnerIdAndDateRange ::
  ...
  DTC.Day ->
  DTC.Day ->
  m [VehicleAggregatedStats]
sumVehicleStatsByFleetOwnerIdAndDateRange fleetOwnerId mbVehicleNo mbLimit mbOffset fromDay toDay = do
  dbConf <- getReplicaBeamConfig
  mbRcId <- case mbVehicleNo of
    Just vehicleNo -> do
      mbRc <- VRCExtra.findLastVehicleRCFleet' vehicleNo fleetOwnerId
      pure $ (.id.getId) <$> mbRc
  ...
  case res of
    Right result -> pure $ map (\(fleetOwnerId', rcId, totalEarnings, totalCompletedRides, totalDistance, totalDuration) -> makeVehicleAggregatedStats fleetOwnerId' rcId totalEarnings totalCompletedRides totalDistance totalDuration) result
    Left _ -> pure []
