...
sumVehicleStatsByFleetOwnerIdAndDateRange mbFleetOwnerId mbRcId limit offset fromDay toDay mbMerchantId = do
                ...
                $ B.filter_'
                  ( \stats ->
                      ( case mbFleetOwnerId of
                        ...
                        B.&&?. maybe (B.sqlBool_ $ B.val_ True) (\mId -> B.sqlBool_ (Beam.merchantId stats B.==. B.val_ (Just mId))) mbMerchantId
    ...
    L.runDB dbConf $
      L.findRows $
        B.select $
          B.limit_ (fromIntegral limit) $
            B.offset_ (fromIntegral offset) $
              B.orderBy_ (\(_, _, _, totalCompletedRides, _, _) -> B.desc_ totalCompletedRides) $
                B.aggregate_
                  ( \stats ->
                      ( B.group_ (Beam.fleetOwnerId stats),
                        B.group_ (Beam.rcId stats),
