...
findAllActiveDriverByFleetOwnerIdWithDriverInfoMF :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r, EncFlow m r) => [Text] -> Text -> Text -> Int -> Int -> Maybe DbHash -> Maybe Text -> Maybe Text -> Maybe Bool -> Maybe DI.DriverMode -> Maybe Bool -> Maybe Bool -> Maybe UTCTime -> Maybe UTCTime -> Maybe Bool -> Maybe DDVS.DocsVerificationStatus -> m [(FleetDriverAssociation, Person, DriverInformation)]
findAllActiveDriverByFleetOwnerIdWithDriverInfoMF fleetOwnerIds merchantId merchantOperatingCityId limit offset mbMobileNumberSearchStringHash mbName mbSearchString mbIsActive mbMode mbHasRequestReason mbEnabled mbFrom mbTo mbApproved mbDocsVerificationStatus = do
  now <- getCurrentTime
  dbConf <- getReplicaBeamConfig
  encryptedMobileNumberHash <- mapM getDbHash mbSearchString
