...
findAllByDriverIds driverIds = do
  now <- getCurrentTime
  findAllWithKV [Se.And [Se.Is BeamDOA.driverId $ Se.In (getId <$> driverIds), Se.Is BeamDOA.isActive $ Se.Eq True, Se.Is BeamDOA.associatedTill (Se.GreaterThan $ Just now)]]
