...
getDriverInfosWithCond :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => [Id DP.Person] -> Bool -> Bool -> Bool -> Bool -> Maybe HighPrecMoney -> Maybe HighPrecMoney -> Maybe HighPrecMoney -> Maybe MP.PaymentInstrument -> m [DriverInfo.DriverInformation]
getDriverInfosWithCond driverLocs onlyNotOnRide onlyOnRide isRental isInterCity prepaidSubscriptionThreshold rideFare minimumWalletPayoutAmount paymentInstrument = do
  now <- getCurrentTime
  findAllWithKV
    [ Se.And
        ( [ Se.Is BeamDI.driverId $ Se.In personsKeys,
            ...
            <> [Se.Is BeamDI.walletBalance $ Se.GreaterThan minimumWalletPayoutAmount | isJust minimumWalletPayoutAmount && (isNothing paymentInstrument || paymentInstrument == Just MP.Cash)]
