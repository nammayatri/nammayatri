...
getDriverInfosWithCond driverLocs onlyNotOnRide onlyOnRide isRental isInterCity prepaidSubscriptionThreshold rideFare minWalletAmountForCashRides paymentInstrument = do
  now <- getCurrentTime
  findAllWithKV
    [ Se.And
