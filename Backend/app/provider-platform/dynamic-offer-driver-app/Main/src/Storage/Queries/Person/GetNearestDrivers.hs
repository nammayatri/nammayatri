          ...
          Just $
            NearestDriversResult
              { driverId = cast person.id,
                ...
                driverTags = Yudhishthira.convertTags $ LYT.TagNameValueExpiry "NormalDriver#true" : (map LYT.TagNameValueExpiry (fold vehicle.vehicleTags) ++ fold person.driverTag),
                score = Nothing,
