...
updatesubscriptionServiceRelatedDataInDriverPlan :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => Id Person -> Domain.SubscriptionServiceRelatedData -> DPlan.ServiceNames -> m ()
updatesubscriptionServiceRelatedDataInDriverPlan driverId subscriptionServiceRelatedData serviceName = do
  ...
  now <- getCurrentTime
  updateOneWithKV
    [ Se.Set BeamDF.commodityData (Just commodityDataRaw),
      Se.Set BeamDF.updatedAt now
    ]
    [ Se.And
        [ Se.Is BeamDF.driverId (Se.Eq (getId driverId)),
...
instance ToTType' BeamDF.DriverPlan DriverPlan where
  toTType' DriverPlan {..} = do
    let commodityData = A.toJSON subscriptionServiceRelatedData :: A.Value
    BeamDF.DriverPlanT
      { BeamDF.driverId = getId driverId,
        ...
        BeamDF.commodityData = Just commodityData
      }
