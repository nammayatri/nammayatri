...
getNearestDriversCurrentlyOnRide NearestDriversOnRideReq {..} = do
  ...
  driverInfos <- Int.getDriverInfosWithCond (driverLocs <&> (.driverId)) False True isRental isInterCity prepaidSubscriptionThreshold rideFare minimumWalletPayoutAmount paymentInstrument
