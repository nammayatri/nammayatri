...
setInactiveBySTId :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r, HedisFlow m r) => Maybe [SearchRequestForDriver] -> Text -> m ()
setInactiveBySTId mbSrfds searchTryId = do
  srfds <-
    case mbSrfds of
...
findByDriver (Id driverId) = do
  ...
  if isDriverValid
    then do
      ...
      srfdIds <- Hedis.runInMultiCloudRedisForList $ Hedis.withCrossAppRedis $ Hedis.zRangeByScore (searchReqestForDriverkey driverId) (utcToMilliseconds startTime) (utcToMilliseconds now)
