...
findByDriver (Id driverId) = do
  ...
  if isDriverValid
    then do
      ...
      srfdIds <- Hedis.runInMultiCloudRedis $ Hedis.withCrossAppRedis $ Hedis.zRangeByScore (searchReqestForDriverkey driverId) (utcToMilliseconds startTime) (utcToMilliseconds now)
