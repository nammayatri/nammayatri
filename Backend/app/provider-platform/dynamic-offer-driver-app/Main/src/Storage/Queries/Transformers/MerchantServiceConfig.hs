...
getConfigJSON = \case
  ...
  Domain.InsuranceSdkServiceConfig aarokyaCfg -> toJSON aarokyaCfg
...
getServiceName = \case
  ...
  Domain.InsuranceSdkServiceConfig _ -> Domain.InsuranceSdkService Domain.Aarokya
  Domain.SettlementServiceConfig settlementCfg -> case settlementCfg of
...
mkServiceConfig configJSON serviceName = either (\err -> throwError $ InternalError ("Unable to decode MerchantServiceConfigT.configJSON: " <> show configJSON <> " Error:" <> err)) return $ case serviceName of
  ...
  Domain.InsuranceSdkService Domain.Aarokya -> Domain.InsuranceSdkServiceConfig <$> eitherValue configJSON
