...
create :: KvDbFlow m r => Domain.Mandate -> m ()
create = createWithKV

findById :: KvDbFlow m r => Id Domain.Mandate -> m (Maybe Domain.Mandate)
findById (Id mandateId) = findOneWithKV [Se.Is BeamM.id $ Se.Eq mandateId]

findByStatus :: KvDbFlow m r => Text -> [MandateStatus] -> m (Maybe Domain.Mandate)
findByStatus mandateId status = findOneWithKV [Se.And [Se.Is BeamM.id $ Se.Eq mandateId, Se.Is BeamM.status $ Se.In status]]

updateMandateDetails :: KvDbFlow m r => Id Domain.Mandate -> MandateStatus -> Maybe Text -> Maybe Text -> Maybe Text -> Maybe Text -> m ()
...
updateStatus :: KvDbFlow m r => Id Domain.Mandate -> MandateStatus -> m ()
