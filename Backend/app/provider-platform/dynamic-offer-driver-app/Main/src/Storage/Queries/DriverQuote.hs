...
findActiveQuoteByDriverIdAndVehVar :: (Transactionable m) => Id Person -> VehVar.Variant -> m (Maybe Domain.DriverQuote)
findActiveQuoteByDriverIdAndVehVar driverId vehicleVariant = do
  buildDType $ do
    res <- Esq.findOne' $ do
      (dQuote :& farePars) <-
        from baseDriverQuoteQuery
