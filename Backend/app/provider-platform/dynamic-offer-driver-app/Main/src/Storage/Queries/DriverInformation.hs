...
updateDriverInformation :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => Id Person -> Bool -> Bool -> Bool -> Bool -> Bool -> Maybe Text -> m ()
updateDriverInformation (Id driverId) canDowngradeToSedan canDowngradeToHatchback canDowngradeToTaxi canSwitchToRental canSwitchToIntercity availableUpiApps = do
  now <- getCurrentTime
  updateOneWithKV
