...
updateShareTripWithEmergencyContactOptions :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Id Person -> Maybe SafetyCommon.RideShareOptions -> m ()
...
findpersonENListWithFallBack personId mbPerson = do
  ...
  if isUpdated
    then return personENList
      ...
      void $ maybe (runInReplica $ QPerson.findById personId >>= fromMaybeM (PersonNotFound personId.getId)) return mbPerson
      -- Driver Person does not have shareTripWithEmergencyContactOption; use default
      let defaultShareOption = Nothing :: Maybe SafetyCommon.RideShareOptions
      updateWithKV
        [ Se.Set BeamPDEN.shareTripWithEmergencyContactOption defaultShareOption
        ]
        [ Se.Is BeamPDEN.personId $ Se.Eq personId.getId
        ]
      return $ fmap (\personEN -> personEN {shareTripWithEmergencyContactOption = defaultShareOption}) personENList
