...
create :: KvDbFlow m r => DDGR.DriverGoHomeRequest -> m ()
...
findById :: KvDbFlow m r => Id DDGR.DriverGoHomeRequest -> m (Maybe DDGR.DriverGoHomeRequest)
findById (Id.Id driverGoHomeRequestId) = findOneWithKV [Se.Is BeamDHR.id $ Se.Eq driverGoHomeRequestId]

findActive :: (KvDbFlow m r, Log m) => Id Driver -> m (Maybe DDGR.DriverGoHomeRequest)
...
finishWithStatus :: KvDbFlow m r => Id DDGR.DriverGoHomeRequest -> DDGR.DriverGoHomeRequestStatus -> Maybe Bool -> m ()
...
todaySuccessCount :: KvDbFlow m r => Id Driver -> m Int
...
updateCancellationCount :: KvDbFlow m r => Id DDGR.DriverGoHomeRequest -> Int -> m ()
