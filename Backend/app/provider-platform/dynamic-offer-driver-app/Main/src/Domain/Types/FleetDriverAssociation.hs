...
makeFleetVehicleDriverAssociation :: (MonadFlow m) => Id Person -> Text -> Maybe UTCTime -> m FleetDriverAssociation
makeFleetVehicleDriverAssociation driverId fleetOwnerId end = do
  id <- generateGUID
  now <- getCurrentTime
  return $
