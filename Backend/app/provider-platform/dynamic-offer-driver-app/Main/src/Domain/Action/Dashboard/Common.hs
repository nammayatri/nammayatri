...
castVehicleVariantDashboard = \case
  ...
  Just DV.AUTO_LITE -> Just Common.AUTO_LITE
  Just DV.PINK_AUTO -> Just Common.PINK_AUTO
  Just DV.EV_HATCHBACK -> Just Common.EV_HATCHBACK
  Just DV.EV_SEDAN -> Just Common.EV_SEDAN
  Just DV.EV_SUV -> Just Common.EV_SUV
  _ -> Nothing
  ...
  let rcReq =
        DomainRC.DriverRCReq
          { vehicleRegistrationCertNumber = req.registrationNo,
            ...
            previousRcNumber = req.previousRcNumber
          }
...
notifyYatriRentalEventsToDriver vehicleId messageKey personId transporterConfig mbReason channel = do
  ...
  withLogTag ("personId_" <> personId.getId) $ do
    case channel of
      SMS -> do
        (mbSender, message, templateId, messageType) <- MessageBuilder.buildGenericMessage merchantOpCityId mkey Nothing MessageBuilder.BuildGenericMessageReq {}
        let sender = fromMaybe smsCfg.sender mbSender
        Sms.sendSMS driver.merchantId merchantOpCityId (Sms.SendSMSReq message phoneNumber sender templateId messageType)
          >>= Sms.checkSmsResult
      WHATSAPP -> do
        merchantMessage <-
...
castDashboardVehicleVariantToDomain = \case
  ...
  Common.AUTO_LITE -> DV.AUTO_LITE
  Common.PINK_AUTO -> DV.PINK_AUTO
