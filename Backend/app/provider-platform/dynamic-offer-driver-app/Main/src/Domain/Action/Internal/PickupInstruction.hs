...
sendPickupInstruction driverId apiKey req = do
  logDebug $ "PickupInstruction: Received pickup instruction for driver: " <> driverId

  -- Validate driver exists
  driver <- QPerson.findById (Id driverId) >>= fromMaybeM (PersonNotFound driverId)
  logDebug $ "PickupInstruction: Driver found: " <> driver.firstName
  ...
  logDebug $ "PickupInstruction: API key validated for merchant: " <> merchant.name

  -- Log the instruction details
  case req.instruction of
    Just instruction -> logDebug $ "PickupInstruction: Text instruction - " <> instruction
    Nothing -> logDebug "PickupInstruction: No text instruction provided"

  case req.audioUrl of
    Just _ -> logDebug "PickupInstruction: Audio instruction URL provided"
    Nothing -> logDebug "PickupInstruction: No audio instruction URL provided"
  ...
  logDebug $ "PickupInstruction: Sending FCM notification to driver: " <> driverId
  Notifications.sendPickupInstructionNotification driver.merchantOperatingCityId driver entityData

  logDebug $ "PickupInstruction: Successfully sent pickup instruction notification to driver: " <> driverId
  pure Success
