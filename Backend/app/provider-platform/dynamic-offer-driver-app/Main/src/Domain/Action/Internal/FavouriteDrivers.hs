...
getFavouriteDrivers merchantId apiKey GetFavouriteDriverInfoReq {..} = do
  ...
  rider <- QRD.findByMobileNumberHashAndMerchant numberHash merchant.id >>= fromMaybeM (InternalError "Rider does not exist")
  let favDrivers = rider.favDriverList
  favDriverResponse <- forM favDrivers $ \favDriver -> do
    person <- QP.findById (Id favDriver) >>= fromMaybeM (InternalError "Person not found")
    personStat <- QDS.findById (Id favDriver) >>= fromMaybeM (InternalError "Driver stats not found")
