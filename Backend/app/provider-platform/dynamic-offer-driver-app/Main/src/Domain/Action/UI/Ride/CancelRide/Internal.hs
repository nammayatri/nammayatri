  ...
  if isLocked
    then do
        ...
        ( do
            ...
            chargesOutcome <- applyTerminalConsequences consequenceCtx (\base gst consumeRideCredit -> createCancellationLedgerEntries booking ride base gst transporterConfig consumeRideCredit)
...
createCancellationLedgerEntries ::
  ...
  Bool ->
  m ()
createCancellationLedgerEntries booking ride baseCancellation gstOnCancellation transporterConfig consumeRideCredit = do
