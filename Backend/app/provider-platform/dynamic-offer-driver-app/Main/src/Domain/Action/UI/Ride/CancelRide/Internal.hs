      ...
      DTC.OneWay DTC.OneWayOnDemandDynamicOffer -> do
        ...
        if isRepeatSearch
          then do
            ...
            result <- try @_ @SomeException (initiateDriverSearchBatch merchant searchReq driverQuote.tripCategory searchTry.vehicleVariant searchTry.estimateId searchTry.customerExtraFee searchTry.messageId)
            case result of
