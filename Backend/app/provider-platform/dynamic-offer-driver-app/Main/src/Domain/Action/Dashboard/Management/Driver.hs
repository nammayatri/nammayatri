...
postDriverDisable merchantShortId opCity reqDriverId = do
  ...
  let isFleetOwnerPerson = SDO.isFleetRole driverPerson.role
      disableTarget = if isFleetOwnerPerson then SGuard.TargetFleetOwner personId else SGuard.TargetDriver personId
      disabledReason = if isFleetOwnerPerson then DrInfo.AdminDisabled else DrInfo.DriverDisabled
  SGuard.withOnboardingAction tcForDisable SGuard.None SGuard.Disable disableTarget $
    SFlags.markEnableDisableReasonFlags (tcForDisable.unifiedOnboardingFlagsRecompute == Just True) driverPerson (SFlags.AdminDisable disabledReason)
...
postDriverUpdateVehicleVariant merchantShortId opCity _ req = do
  ...
  SGuard.withOnboardingAction transporterConfig SGuard.None SGuard.Approve (SGuard.TargetVehicle rcNumber) $
  ...
  where
    ...
    isAssociationWithDriver requestedPersonDetails driverDetails =
      case (requestedPersonDetails.role, driverDetails.role) of
        (DP.FLEET_OWNER, DP.DRIVER) -> SA.checkFleetDriverAssociation driverDetails.id requestedPersonDetails.id
        (DP.OPERATOR, DP.DRIVER) -> SA.checkDriverOperatorAssociation driverDetails.id requestedPersonDetails.id
        _ -> pure (requestedPersonDetails.id == driverDetails.id)
