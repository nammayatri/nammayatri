    ...
    PersonRequestorId personId -> do
      ...
      (rideCancellationReason, mbCancellationCnt, isGoToDisabled) <- case authPerson.role of
        ...
        DP.DRIVER -> do
          ...
          fork "DriverRideCancelledCoin Event : " $ DC.driverCoinsEvent driverId driver.merchantId (DCT.Cancellation ride.createdAt booking.distanceToPickup disToPickup)
