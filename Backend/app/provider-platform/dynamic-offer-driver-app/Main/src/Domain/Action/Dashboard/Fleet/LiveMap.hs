...
getLiveMapDrivers merchantShortId _opCity radius requestorId mbFleetOwnerId mbDriverIdForRadius mbPoint = do
  ...
  catMaybes <$> catMaybes <$> mapM (traverse buildMapDriverInfo) mbPositionAndDriverInfoLs
  where
...
buildMapDriverInfo (mbRideId, position, driverInformation) = do
  ...
  mbVehicle <- QV.findById driverId -- >>= fromMaybeM (VehicleNotFound $ "driverId:-" <> driverId.getId)
  case mbVehicle of
    Just vehicle -> do
    ...
    Nothing -> do
      logError $ "Vehicle not found for driverId: " <> driverId.getId
      pure Nothing
