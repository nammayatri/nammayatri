...
postOperatorRegister merchantShortId opCity req = do
  ...
  case personOpt of
    ...
    Nothing -> do
      ...
      -- should we load any docs as for fleet owner?
...
createOperatorDetails authReq merchantId merchantOpCityId isDashboard deploymentVersion mbIsGenerateRefEntry = do
  ...
  -- createFleetOwnerInfo person.id merchantId mbfleetType mbgstNumber mbReferredOperatorId -- do we need operator_info table the same as fleet_owner_info ?
