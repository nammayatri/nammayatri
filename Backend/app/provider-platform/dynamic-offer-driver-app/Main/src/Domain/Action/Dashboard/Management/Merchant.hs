...
_isBaseFareBelowMin :: Maybe HighPrecMoney -> HighPrecMoney -> Bool
_isBaseFareBelowMin Nothing _ = False
_isBaseFareBelowMin (Just minVal) baseFare = baseFare < minVal

_getAllBaseFaresFromFarePolicy :: FarePolicy.FarePolicy -> [HighPrecMoney]
_getAllBaseFaresFromFarePolicy fp = case fp.farePolicyDetails of
    ...
    processFarePolicyGroup merchantOpCity allCityFareProducts (errors, boundedAlreadyDeletedMap, farePolicyUsageCount) (x : xs) = do
      ...
      if city /= opCity
        then return (errors <> ["Can't process fare policy for different city: " <> show city <> ", please login with this city in dashboard"], boundedAlreadyDeletedMap, farePolicyUsageCount)
          ...
          return (errors, newBoundedAlreadyDeletedMap, newFarePolicyUsageCount)
