...
postSubmitApplication (mbDriverId, merchantId, merchantOperatingCityId) req = do
  ...
  -- Call createOrderService directly (inline createOrderV2 logic)
  ...
  defaultPaymentServiceName <- pure (DMSC.MembershipPaymentService PaymentService.Juspay)
  ...
  entityName <- pure DPayment.DRIVER_STCL
