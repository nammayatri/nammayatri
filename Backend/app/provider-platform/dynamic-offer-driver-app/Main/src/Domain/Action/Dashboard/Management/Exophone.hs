...
getExophone ::
  ...
  Text ->
  Environment.Flow Common.ExophoneRes
getExophone _merchantShortId _opCity exophoneIdText = do
...
postExophoneUpdate merchantShortId opCity exophoneIdText req = do
  ...
  -- Check if updated phone numbers are already in use (if being changed)
  whenJust req.exophonePrimaryPhone $ \newPrimaryPhone -> do
    when (newPrimaryPhone /= exophone.primaryPhone) $ do
      allExophones <- CQExophone.findAllExophones
      let alreadyUsedPhones = getAllPhones $ filter (\e -> e.id /= exophoneId) allExophones
      when (newPrimaryPhone `elem` alreadyUsedPhones) $
        throwError $ InvalidRequest $ "Primary phone already in use: " <> newPrimaryPhone

  whenJust req.exophoneBackupPhone $ \newBackupPhone -> do
    when (newBackupPhone /= exophone.backupPhone) $ do
      allExophones <- CQExophone.findAllExophones
      let alreadyUsedPhones = getAllPhones $ filter (\e -> e.id /= exophoneId) allExophones
      when (newBackupPhone `elem` alreadyUsedPhones) $
        throwError $ InvalidRequest $ "Backup phone already in use: " <> newBackupPhone
