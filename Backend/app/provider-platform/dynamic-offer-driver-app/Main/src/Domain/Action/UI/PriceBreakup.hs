...
getPriceBreakup (_, _, merchantOpCityId) rideId = do
  ...
  case quote of
    Just quote' -> do
      let fareDetails_ = catMaybes $ maybe [] (mkFarePolicyBreakups Prelude.id (mkBreakupItem booking.currency) booking.estimatedDistance booking.fareParams.customerCancellationDues Nothing booking.estimatedFare quote'.fareParams.congestionChargeViaDp govtChargesRate (not $ null booking.stops)) quote'.farePolicy
      pure fareDetails_
    _ -> pure []
  where
