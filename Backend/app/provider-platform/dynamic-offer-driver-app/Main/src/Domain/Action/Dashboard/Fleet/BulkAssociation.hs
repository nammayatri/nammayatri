...
-- Basic device identifier (mobile number) validation for bulk upload
validateDeviceIdentifier :: Text -> Either Text Text
validateDeviceIdentifier deviceId =
  let cleaned = T.strip deviceId
      len = T.length cleaned
   in case () of
        _ | T.null cleaned -> Left "Device identifier cannot be empty"
        _ | len /= 10 -> Left "Device identifier must be exactly 10 digits"
        _ | not (T.all isDigit cleaned) -> Left "Device identifier can only contain digits"
        _ | T.head cleaned `notElem` ['6', '7', '8', '9'] -> Left "Device identifier must start with 6, 7, 8, or 9"
        _ -> Right cleaned
...
handleBulkRow merchantShortId opCity i row = do
  ...
  case validateDeviceIdentifier row.device_identifier of
