    ...
    processWalletTopupWebhook driver order transactionStatus = do
      when (transactionStatus == Payment.CHARGED) $ do
        ...
        when (null existing) $ do
          let ctx =
                FinanceCtx
                  { merchantId = order.merchantId.getId,
                    merchantOpCityId = driver.merchantOperatingCityId.getId,
                    currency = order.currency,
                    counterpartyType = FAccount.DRIVER,
                    counterpartyId = order.personId.getId,
                    referenceId = order.id.getId,
          ...
          result <- runFinance ctx $ transfer PlatformAsset OwnerLiability order.amount walletReferenceTopup
