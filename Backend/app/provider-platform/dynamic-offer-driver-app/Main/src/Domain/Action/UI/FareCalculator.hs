      ...
      CQVST.findByServiceTierTypeAndCityIdInRideFlow fp.vehicleServiceTier merchanOperatingCityId configsInExperimentVersions
        >>= \case
          Just vehicleServiceTierItem -> do
            estimate <- DBS.buildEstimate merchantId merchanOperatingCityId currency Meter Nothing now False Nothing False mbDistance Nothing Nothing mbTollCharges mbTollNames mbTollIds Nothing Nothing 0 mbDuration tConfig mempty False vehicleServiceTierItem fp
