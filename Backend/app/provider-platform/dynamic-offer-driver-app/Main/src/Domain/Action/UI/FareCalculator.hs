...
getCalculateFare (_, merchantId, merchanOperatingCityId) distanceWeightage dropLatLong pickupLatlong = do
  ...
  fareProducts <- FP.getAllFarePoliciesProduct merchantId merchanOperatingCityId False pickupLatlong (Just dropLatLong) Nothing Nothing mbDistance mbDuration (DTC.OneWay DTC.OneWayOnDemandDynamicOffer)
