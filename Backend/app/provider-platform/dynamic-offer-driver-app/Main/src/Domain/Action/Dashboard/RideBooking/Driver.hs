...
postDriverEnable merchantShortId opCity reqDriverId = do
  ...
  transporterConfig <- getOneConfig (TransporterConfigDimensions {merchantOperatingCityId = merchantOpCityId.getId}) Nothing >>= fromMaybeM (InvalidRequest "TransporterConfig not found")
  ...
  pure
    Common.DriverInfoRes
      { driverId = cast @DP.Person @Common.Driver person.id,
        ...
        isPetModeEnabled = Just info.isPetModeEnabled,
        tripDistanceMinThreshold = info.tripDistanceMinThreshold,
        tripDistanceMaxThreshold = info.tripDistanceMaxThreshold,
        maxPickupRadius = info.maxPickupRadius,
        isSilentModeEnabled = info.isSilentModeEnabled,
