...
verifyAadhaar verifyBy mbMerchant (personId, merchantId, merchantOpCityId) req adminApprovalRequired = do
  ...
  whenJust mbMerchant $ \merchant -> do
    unless (merchant.id == person.merchantId) $ throwError (PersonNotFound personId.getId)
  let isDashboard = verifyBy /= DPan.FRONTEND_SDK
      runVerify = verifyAadhaarCommon isDashboard verifyBy person transporterConfig merchantId merchantOpCityId (AadhaarExtractInput req.aadhaarNumber req.aadhaarFrontImageId req.aadhaarBackImageId req.consent req.aadhaarName)
  -- verifyAadhaarCommon returns the aadhaar number, so use it directly instead of re-reading + decrypting the row.
  mbAadhaarNumber <-
    if isNameCompareRequired transporterConfig verifyBy
      then Redis.withLockRedisAndReturnValue (makeDocumentVerificationLockKey personId.getId) 10 runVerify
      else runVerify
  triggerPanAadhaarLinkageWhenPanAndAadhaarExist person merchantOpCityId mbAadhaarNumber
