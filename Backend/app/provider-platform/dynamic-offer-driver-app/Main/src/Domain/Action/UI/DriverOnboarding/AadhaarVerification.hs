...
generateAadhaarOtp isDashboard mbMerchant personId merchantOpCityId req = do
  ...
  transporterConfig <- CTC.findByMerchantOpCityId merchantOpCityId (Just driverInfo.driverId.getId) >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
...
uploadCompressedAadhaarImage person merchantOpCityId image imageType = do
  transporterConfig <- CTC.findByMerchantOpCityId merchantOpCityId (Just person.id.getId) >>= fromMaybeM (TransporterConfigNotFound (merchantOpCityId.getId))
