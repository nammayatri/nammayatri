...
arrivedAtStop rideId pt = do
  ...
  case booking.stopLocationId of
    ...
    Just nextStopId -> do
      ...
      transporterConfig <- QMTC.findByMerchantOpCityId booking.merchantOperatingCityId (Just booking.transactionId) >>= fromMaybeM (TransporterConfigNotFound booking.merchantOperatingCityId.getId)
...
uploadOdometerReading merchantOpCityId rideId UploadOdometerReq {..} = do
  ...
  config <- QMTC.findByMerchantOpCityId merchantOpCityId (Just booking.transactionId) >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
