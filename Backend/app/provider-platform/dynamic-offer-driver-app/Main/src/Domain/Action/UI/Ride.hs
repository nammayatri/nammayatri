...
arrivedAtStop rideId pt = do
  ...
  case booking.stopLocationId of
    ...
    Just nextStopId -> do
      ...
      transporterConfig <- QMTC.findByMerchantOpCityId booking.merchantOperatingCityId (Just ride.driverId) >>= fromMaybeM (TransporterConfigNotFound booking.merchantOperatingCityId.getId)
...
uploadOdometerReading merchantOpCityId rideId UploadOdometerReq {..} = do
  ...
  config <- QMTC.findByMerchantOpCityId merchantOpCityId (Just ride.driverId) >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
