...
getTripRoutes (personId, merchantId, merchantOpCityId) enityId req = do
    ...
    Just ride -> do
      ...
      case mbRouteInfo of
        Just routeInfo -> do
          ...
          let selectedWaypoints = getSelectedWaypoints src points
...
getSelectedWaypoints :: Maps.LatLong -> [Maps.LatLong] -> [Maps.LatLong]
getSelectedWaypoints src waypoints = do
  case waypoints of
    (pt : pts) -> do
      let distance = distanceBetweenInMeters src pt
      if distance < (25 :: HighPrecMeters)
        then pickNWayPoints 5 pts
        else []
    [] -> []
