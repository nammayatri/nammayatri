...
handler merchantId req ride = do
  ...
  case mbBooking of
    Just booking -> do
      whenJust (liftM2 (,) req.shouldFavDriver (getId <$> booking.riderId)) $ \(_, riderId) -> do
        SQR.addToFavDriverList riderId ride.driverId
        SQD.addToFavRiderList riderId ride.driverId
    Nothing -> do
