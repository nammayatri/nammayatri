  ...
  case mbBooking of
    Just booking -> do
      ...
      fork "DriverCoinRating Event" $ do
        DC.driverCoinsEvent driverId Nothing merchantId ride.merchantOperatingCityId (DCT.Rating ratingValue ride) (Just ride.id.getId) ride.vehicleVariant (Just booking.vehicleServiceTier) (Just booking.configInExperimentVersions) Nothing
    Nothing -> do
