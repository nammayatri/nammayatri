...
endRideTransaction driverId booking ride mbFareParams mbRiderDetailsId newFareParams thresholdConfig = do
  ...
  newTotalRides <- QDriverStats.incrementTotalRidesAndTotalDistAndIdleTime (cast ride.driverId) (fromMaybe 0 ride.chargeableDistance)
  when (thresholdConfig.allowAnalytics == Just True) $ Analytics.updateOperatorAnalyticsTotalRideCount thresholdConfig driverId newTotalRides
