            ...
            if now >= ldFee.startTime && now < ldFee.endTime
              then do
                ...
                let perRideCountSoFar = max 0 (calcNumRides ldFee transporterConfig - ldFee.dailyBaseCount)
                    baseAccrual = basePlanAccrual planForAccrual isSpecialZoneCharge transporterConfig.considerSpecialZoneRidesForPlanCharges perRideCountSoFar ldFee.perRideBaseAmount ldFee.dailyBaseCount ldFee.dailyBaseAmount
...
mkDriverFee serviceName now startTime' endTime' merchantId driverId rideFare govtCharges platformFee cgst sgst currency transporterConfig _mbBooking isSpecialZoneCharge currentVehicleCategory subsConfig = do
  ...
  let (perRideBaseAmount, dailyBaseCount, dailyBaseAmount) = basePlanAccrual plan isSpecialZoneCharge transporterConfig.considerSpecialZoneRidesForPlanCharges 0 0 0 0
  return $
    DF.DriverFee
      { status = DF.ONGOING,
        collectedBy = Nothing,
