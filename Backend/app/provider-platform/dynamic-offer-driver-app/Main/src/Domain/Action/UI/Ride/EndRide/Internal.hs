...
import qualified Storage.Queries.VendorSplitDetailsExtra as QVSD
import Tools.Error
import Tools.Event
    ...
    when (toUpdateOrCreateDriverfee && isEligibleForCharge transporterConfig isOnFreeTrial isSpecialZoneCharge) $ do
      ...
      fork "Updating vendor fees" $
        when (fromMaybe False (subscriptionConfig >>= (.isVendorSplitEnabled))) $ do
          ...
          let vendorAmounts = map (\vendor -> (vendor.vendorId, toRational vendor.splitValue)) vendorSplitDetails
              vendorFees = map (mkVendorFee (maybe driverFee.id (.id) lastDriverFee) now) vendorAmounts
