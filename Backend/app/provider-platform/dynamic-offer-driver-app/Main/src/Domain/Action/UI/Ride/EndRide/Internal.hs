...
endRideTransaction driverId booking ride mbFareParams mbRiderDetailsId newFareParams thresholdConfig = do
  ...
  fork "BPP completed-stage ride-value histograms" $ do
    Metrics.observeRideDistanceMeters merchantLabel cityLabel vstLabel distBucketLabel "completed" chargeableMeters
    when (chargeableMeters > 0) $
      Metrics.observePricePerKm merchantLabel cityLabel vstLabel distBucketLabel "completed" (rideFareValue / (chargeableMeters / 1000))
    whenJust newFareParams.congestionCharge $ \congestionChargeVal ->
      Metrics.observeCongestionCharge merchantLabel cityLabel vstLabel distBucketLabel "completed" (realToFrac congestionChargeVal)
    whenJust ride.distanceToPickup $ \pickupDistanceVal ->
      Metrics.observePickupDistanceMeters merchantLabel cityLabel vstLabel distBucketLabel "completed" (realToFrac pickupDistanceVal)
  ...
  when (fromMaybe False merchant.prepaidSubscriptionAndWalletEnabled) $
...
processEndRideFinance merchant ride booking newFareParams driverId driverInfo thresholdConfig = do
  ...
  where
    ...
    getPrepaidRevenueAmount fare mbVC = do
      ...
      mbPurchase <- latestSubscriptionPurchase <$> QSPE.findAllActiveByOwnerAndServiceName ownerId' ownerType' PREPAID_SUBSCRIPTION mbVC
