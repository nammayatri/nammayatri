...
postPenaltyCheck (mbPersonId, _merchantId, _merchantOpCityId) req = do
  ...
  (isApplicable, penaltyAmount) <- case booking.fareParams.driverCancellationPenaltyAmount of
    Just penaltyAmount -> do
      ...
      tagsE <- withTryCatch "computeNammaTags:PenaltyCheck" $ LYDL.computeNammaTagsWithDebugLog LYDL.Driver (Kernel.Types.Id.cast booking.merchantOperatingCityId) YA.PenaltyCheck (Nothing :: Maybe Text) tagData
  ...
  cancellationValidity <- do
    ...
    tagsE <- withTryCatch "computeNammaTags:RideCancel" $ LYDL.computeNammaTagsWithDebugLog LYDL.Driver (Kernel.Types.Id.cast booking.merchantOperatingCityId) YA.RideCancel (Nothing :: Maybe Text) tagData
    let tags = fromMaybe [] $ eitherToMaybe tagsE
