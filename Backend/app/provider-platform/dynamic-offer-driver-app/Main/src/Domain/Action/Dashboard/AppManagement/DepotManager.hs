...
postDepotManagerUpsertOne merchantShortId opCity req = do
  ...
  result <- withTryCatch "postDepotManagerUpsertOne" $ do
    personId <- findOrCreatePerson merchant merchantOperatingCity.id req
...
postDepotManagerUpsertMany merchantShortId opCity req = do
  ...
  results <- forM req.items $ \item -> do
    result <- withTryCatch "postDepotManagerUpsertMany" $ do
      personId <- findOrCreatePerson merchant merchantOperatingCity.id item
...
buildDepotManagerInfos depotManagers = do
  persons <- QP.findAllByPersonIds (map (getId . (.personId)) depotManagers)
...
findOrCreatePerson ::
  ...
  API.DepotManagerDetail ->
  Flow (Id DP.Person)
findOrCreatePerson merchant merchantOperatingCityId req = do
  ...
  case mbPerson of
    ...
    Nothing -> Registration.createPersonWithPhoneNumber merchant merchantOperatingCityId req.mobileNumber req.countryCode
