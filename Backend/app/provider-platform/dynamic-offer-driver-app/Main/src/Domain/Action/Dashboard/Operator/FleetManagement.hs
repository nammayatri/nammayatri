...
getFleetManagementFleets merchantShortId opCity mbIsActive mbVerified mbEnabled mbLimit mbOffset mbSearchString requestorId = do
  ...
  where
    createFleetInfo (foa, person, FOI.FleetOwnerInformation {..}) = do
      ...
      statusRes <-
        castStatusRes
          <$> SStatus.statusHandler' person entityImagesInfo Nothing Nothing Nothing Nothing (Just True) shouldActivateRc Nothing skipMessages Nothing
      pure $
        Common.FleetInfo
...
postFleetManagementFleetLinkSendOtpUtil merchantShortId opCity requestorId req skipOtpVerification mbDashboardPersonId = do
  ...
  if skipOtpVerification
    then SGuard.withOnboardingAction transporterConfig (SGuard.ActorFleet operator.id) SGuard.LinkToOperator (SGuard.TargetFleetOwner fleetOwner.id) $ do
...
postFleetManagementFleetLinkVerifyOtp merchantShortId opCity requestorId req = do
  ...
  SGuard.withOnboardingAction transporterConfig (SGuard.ActorFleet operator.id) SGuard.LinkToOperator (SGuard.TargetFleetOwner fleetOwner.id) $ do
