...
getIncentiveJourneyHistory (mbPersonId, _merchantId, merchantOpCityId) mbDate mbLimit mbOffset = do
  ...
  let historyDay = fromMaybe (utctDay localTime) (mbDate >>= parseDateText)
      ...
      dailyPeriodKey = SLJourney.mkDailyPeriodKey historyDayUtc
      weeklyPeriodKey = SLJourney.mkWeeklyPeriodKey historyDayUtc
      monthlyPeriodKey = SLJourney.mkMonthlyPeriodKey historyDayUtc
      limitVal = fromMaybe 20 mbLimit
      offsetVal = fromMaybe 0 mbOffset
  dailyStatsRows <- QStats.findByDriverIdAndPeriodKey driverId dailyPeriodKey
