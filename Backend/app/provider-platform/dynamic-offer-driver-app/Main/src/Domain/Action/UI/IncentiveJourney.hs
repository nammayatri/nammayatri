...
getIncentiveJourneyList (mbPersonId, merchantId, merchantOpCityId) mbActive mbDate _mbLimit _mbOffset = do
  ...
  if null journeyTags
    then pure API.IncentiveJourneyListRes {journeys = []}
      ...
      enabledJourneys <-
        if onlyEnabled
          then CQJourney.findEnabledByMerchantOperatingCityId merchantOpCityId
          else CQJourney.findByMerchantOperatingCityId merchantOpCityId
      let matching =
            filter
              ( \j ->
      ...
      case SLJourney.selectPreferredJourney localTime matching of
        ...
        Just journey -> do
          milestones <- SLJourney.loadJourneyMilestones journey.id
  ...
  expanded <-
    concat
      <$> forM
        journeyIds
        ( \journeyId -> do
            mbJourney <- CQJourney.findById journeyId
            milestones <- SLJourney.loadJourneyMilestones journeyId
            specialLocationNames <- buildSpecialLocationNames milestones
