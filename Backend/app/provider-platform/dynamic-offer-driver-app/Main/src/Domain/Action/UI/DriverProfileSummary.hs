...
getDriverProfileSummary :: (Esq.EsqDBReplicaFlow m r, EncFlow m r) => (Id SP.Person, Id DM.Merchant) -> m DriverProfleSummaryRes
  ...
  let missedOpp =
        DriverInfo.DriverMissedOpp
          { cancellationRate = div ((fromMaybe 0 driverStats.ridesCancelled) * 100 :: Int) (nonZero driverStats.totalRidesAssigned :: Int),
            ridesCancelled = driverStats.ridesCancelled,
