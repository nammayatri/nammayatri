...
verifyDLFlow person merchantOpCityId documentVerificationConfig dlNumber driverDateOfBirth imageId1 imageId2 dateOfIssue nameOnCard mbVehicleCategory = do
  ...
  case verifyRes.requestor of
    ...
    VT.HyperVergeRCDL -> HVQuery.create =<< mkHyperVergeVerificationEntity verifyRes.requestId now imageExtractionValidation encryptedDL
  ...
  where
    ...
    mkHyperVergeVerificationEntity requestId now imageExtractionValidation encryptedDL = do
      id <- generateGUID
      return $
    ...
    linkDl person = do
      if verificationReq.imageExtractionValidation == Domain.Skipped
        ...
        then do
          ...
          onVerifyDLHandler person output.licenseNumber (output.t_validity_to <|> output.nt_validity_to) output.covs output.driverName output.dob documentVerificationConfig verificationReq.documentImageId1 verificationReq.documentImageId2 verificationReq.nameOnCard Nothing
          pure Ack
