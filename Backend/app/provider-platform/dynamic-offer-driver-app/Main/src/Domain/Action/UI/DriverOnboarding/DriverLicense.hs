    ...
    linkDl person = do
      if verificationReq.imageExtractionValidation == Domain.Skipped
        ...
        then do
          ...
          now <- getCurrentTime
          id <- generateGUID
          onboardingDocumentConfig <- QODC.findByMerchantOpCityIdAndDocumentType verificationReq.merchantOperatingCityId DTO.DL >>= fromMaybeM (OnboardingDocumentConfigNotFound person.merchantId.getId (show DTO.DL))
...
dlNotFoundFallback issueDate (extractedDL, operatingCity) dob verificationReq person = do
  ...
  void $ verifyDL False Nothing (person.id, person.merchantId, verificationReq.merchantOperatingCityId) dlreq
  return Ack
