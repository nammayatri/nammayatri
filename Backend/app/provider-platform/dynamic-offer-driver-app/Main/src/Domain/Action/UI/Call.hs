...
-- import qualified EulerHS.Prelude as EP hiding (elem)
...
sendFCMToBPPOnFailedCallStatus callStatus idInfo = do
  if isOneOfFailingStatus callStatus
    then do
      (ride, booking) <- case idInfo of
        Left driverId -> do
          ride <- runInReplica $ QRide.getActiveByDriverId driverId >>= fromMaybeM (RideForDriverNotFound $ getId driverId)
  ...
  where
    ...
    isOneOfFailingStatus status = status `elem` [CallTypes.BUSY, CallTypes.NO_ANSWER, CallTypes.FAILED, CallTypes.CANCELED]
