  ...
  if isOneOfFailingStatus callStatus || isOneOfIgnoredCallStatus callStatus
    then do
      ...
      when (DM.isNothing booking.exotelDeclinedCallStatusReceivingTime) $ do
        ...
        runWithServiceConfigForProviders (.notifyExotelCallStatusToDriver) booking.merchantOperatingCityId driver.clientId driver.clientDevice notification EulerHS.Prelude.id (clearDeviceToken ride.driverId)
