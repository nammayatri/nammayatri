...
validateStreakEndRewardOnMapping ::
  ...
  Environment.Flow ()
validateStreakEndRewardOnMapping mbRewardType mbRewardValue mbWaiveOff =
...
postIncentiveJourneyCohortJourneyCreate merchantShortId opCity req = do
  ...
  when ((toDomainRewardType <$> req.streakEndRewardType) == Just DIJC.SubscriptionWaiveOff && isNothing req.streakEndSubscriptionWaiveOff) $
    throwError (InvalidRequest "streakEndSubscriptionWaiveOff is required when streakEndRewardType is SubscriptionWaiveOff")
  validateStreakEndRewardOnMapping (toDomainRewardType <$> req.streakEndRewardType) req.streakEndRewardValue req.streakEndSubscriptionWaiveOff
  cjm <-
    QCJMExtra.createCohortJourneyMapping
      ...
      (toLibSubscriptionWaiveOff req.streakEndSubscriptionWaiveOff)
...
putIncentiveJourneyCohortJourneyUpdate merchantShortId opCity req = do
  ...
  when (streakEndRewardType == Just DIJC.SubscriptionWaiveOff) $
    case streakEndWaiveOff of
      Nothing -> throwError (InvalidRequest "streakEndSubscriptionWaiveOff is required when streakEndRewardType is SubscriptionWaiveOff")
      Just cfg -> validateSubscriptionWaiveOffConfig cfg
  validateStreakEndRewardOnMapping streakEndRewardType streakEndRewardValue streakEndWaiveOff
  void $
    QCJMExtra.updateCohortJourneyMappingFields
      cjm{DCJM.startDate = startDate,
          ...
          DCJM.streakEndSubscriptionWaiveOffPercentage = (.percentage) <$> streakEndWaiveOff,
          DCJM.streakEndSubscriptionWaiveOffDaysValidFor = (.daysValidFor) <$> streakEndWaiveOff,
          DCJM.streakEndSubscriptionWaiveOffServiceName = (.serviceName) <$> streakEndWaiveOff,
          DCJM.streakEndSubscriptionWaiveOffMode = (.waiveOffMode) <$> streakEndWaiveOff
         }
  pure Success
...
requireSubscriptionWaiveOffConfig :: Maybe Common.SubscriptionWaiveOffConfig -> Environment.Flow ()
...
validateSubscriptionWaiveOffConfig :: Common.SubscriptionWaiveOffConfig -> Environment.Flow ()
...
-- Partial config is rejected.
validateStoredSubscriptionWaiveOff :: Maybe Int -> Maybe Int -> Maybe Text -> Maybe Text -> Environment.Flow ()
validateStoredSubscriptionWaiveOff percentage daysValidFor serviceName waiveOffMode =
  case (percentage, daysValidFor, serviceName, waiveOffMode) of
    (Nothing, Nothing, Nothing, Nothing) -> pure ()
    (Just p, Just d, Just s, Just m) ->
    ...
    _ ->
      throwError (InvalidRequest "subscriptionWaiveOff must include percentage, daysValidFor, serviceName, and waiveOffMode")

toApiSubscriptionWaiveOff :: Maybe Int -> Maybe Int -> Maybe Text -> Maybe Text -> Maybe Common.SubscriptionWaiveOffConfig
toApiSubscriptionWaiveOff percentage daysValidFor serviceName waiveOffMode =
  Common.SubscriptionWaiveOffConfig
    <$> percentage
    <*> daysValidFor
    <*> serviceName
...
toLibSubscriptionWaiveOff :: Maybe Common.SubscriptionWaiveOffConfig -> Maybe IJ.SubscriptionWaiveOffSpec
toLibSubscriptionWaiveOff =
  fmap $ \cfg ->
    IJ.SubscriptionWaiveOffSpec
      { percentage = cfg.percentage,
        daysValidFor = cfg.daysValidFor,
        serviceName = cfg.serviceName,
        waiveOffMode = cfg.waiveOffMode
      }
