...
import SharedLogic.Allocator.Jobs.IncentiveJourney.BulkUserCohortMappingUpload
  ( clampBatchSize,
    defaultBatchSize,
...
getIncentiveJourneyDriverAssignments merchantShortId opCity driverId = do
  merchant <- findMerchantByShortId merchantShortId
  merchantOpCityId <- CQMOC.getMerchantOpCityId Nothing merchant (Just opCity)
  let driverPersonId = ID.cast @Dashboard.Common.Driver @DP.Person driverId
  assignments <- SLJourney.findAssignmentsByUserId driverPersonId
  items <-
    mapMaybeM
...
postIncentiveJourneyAssign merchantShortId opCity req = do
  ...
  let cjmId = ID.cast @Dashboard.Common.CohortJourneyMapping @DCJM.CohortJourneyMapping req.cohortJourneyMappingId
      ...
      driverPersonId = ID.cast @DIJC.Person @DP.Person driverId
  driver <- QPerson.findById driverPersonId >>= fromMaybeM (InvalidRequest "Driver not found")
  ...
  CQAssignment.clearCacheByPersonId driverPersonId
  pure Success
...
postIncentiveJourneyAssignBulkFromS3 merchantShortId opCity req = do
  merchant <- findMerchantByShortId merchantShortId
  merchantOpCityId <- CQMOC.getMerchantOpCityId Nothing merchant (Just opCity)
  ...
  let batchSize = clampBatchSize $ fromMaybe defaultBatchSize req.batchSize
      delaySecs = max 0 $ fromMaybe defaultRescheduleDelaySeconds req.rescheduleDelaySeconds
  runId <- generateGUIDText
  let jobData =
        BulkUserCohortMappingUploadJobData
          { merchantId = merchant.id,
            merchantOperatingCityId = merchantOpCityId,
            s3FilePath = T.strip req.s3FilePath,
            offset = 0,
            batchSize = batchSize,
            rescheduleDelaySeconds = delaySecs,
            runId = runId
          }
  createJobByTime @_ @'BulkUserCohortMappingUpload (Just merchant.id) (Just merchantOpCityId) req.scheduledAt jobData
  pure
...
toDriverAssignmentItem ::
  ...
  Environment.Flow (Maybe Common.IncentiveJourneyDriverAssignmentItem)
...
validateStoredSubscriptionWaiveOff :: Maybe Int -> Maybe Int -> Maybe Value -> Environment.Flow ()
