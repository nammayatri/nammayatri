...
handler transporterId req = do
  ...
  --booking <- B.runInReplica $ QRB.findById req.bookingId >>= fromMaybeM (BookingNotFound req.bookingId.getId) # ---------To do check with Shreyansh and remove bookingId from StatusReq #RITIKA
    ...
    Just ride -> do
      case ride.status of
        DRide.NEW -> do
          ...
          let isDriverBirthDay = False -----TODO FIX ------#RITIKA
          let isFreeRide = False -----TODO FIX ------#RITIKA
