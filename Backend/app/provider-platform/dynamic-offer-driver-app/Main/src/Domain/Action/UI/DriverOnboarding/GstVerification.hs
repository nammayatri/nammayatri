...
verifyGstin ::
  ...
  Maybe Bool ->
  Flow Bool
verifyGstin isDashboard mbMerchant (personId, _, merchantOpCityId) req adminApprovalRequired = do
  ...
  res <- case person.role of
    ...
    role | isFleetOwnerRole role -> do
      enableFleet <- DFR.enableFleetIfPossible person.id adminApprovalRequired (DFR.castRoleToFleetType person.role)
      pure enableFleet
    _ -> pure False
  pure res
  where
