...
postInternalFrfsBookingRating mbApiKey req = do
  ...
  mbDriver <- case req.driverBadgeToken of
    Nothing -> pure Nothing
    Just badgeToken -> do
      mbPerson <- QPerson.findByOperatorBadgeTokenAndMerchantId (Just badgeToken) merchantId
      when (isNothing mbPerson) $
        if isJust req.driverRating
          then throwError (PersonNotFound badgeToken)
          else logWarning $ "FRFS rating: badge token " <> badgeToken <> " did not resolve for booking " <> req.bookingId <> "; continuing with the fleet rating only."
      pure mbPerson
...
getInternalFrfsBookingRating mbMerchantId mbDriverBadgeToken mbFleetNumber mbGtfsId mbApiKey = do
  ...
  mbDriverStats <- case mbDriverBadgeToken of
    Nothing -> pure Nothing
    Just badgeToken -> do
      mbDriver <- QPerson.findByOperatorBadgeTokenAndMerchantId (Just badgeToken) merchantId
      maybe (pure Nothing) (QDriverStats.findById . (.id)) mbDriver
