    ...
    Nothing -> do
      ...
      fork "Uploading GST Image" $ do
        whenJust req.gstCertificateImage $ \gstImage -> do
          ...
          image <- Image.validateImage Nothing True Nothing Nothing (person.id, merchant.id, merchantOpCityId) req'
...
createPanInfo personId merchantId merchantOperatingCityId (Just img1) _ (Just panNo) = do
  ...
  image <- Image.validateImage Nothing True Nothing Nothing (personId, merchantId, merchantOperatingCityId) req'
