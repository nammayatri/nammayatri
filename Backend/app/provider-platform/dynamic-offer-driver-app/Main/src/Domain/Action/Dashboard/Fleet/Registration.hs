...
createFleetOwnerDetails authReq merchantId merchantOpCityId isDashboard deploymentVersion mbfleetType mbFleetName mbEnabled mbgstNumber mbReferredOperatorId mbTicketPlaceId = do
  ...
  createFleetOwnerInfo person.id merchantId mbfleetType mbFleetName mbEnabled' mbgstNumber mbReferredOperatorId mbTicketPlaceId (Just $ merchantOperatingCity.id) ((.rate) <$> transporterConfig.taxConfig.defaultTdsRate) ((.thresholdAmount) =<< transporterConfig.taxConfig.defaultTdsRate) defaultDocsVerificationStatus
...
createFleetOwnerInfo :: Id DP.Person -> Id DMerchant.Merchant -> Maybe FOI.FleetType -> Maybe Text -> Maybe Bool -> Maybe Text -> Maybe Text -> Maybe Text -> Maybe (Id DMOC.MerchantOperatingCity) -> Maybe Double -> Maybe HighPrecMoney -> Maybe DDVS.DocsVerificationStatus -> Flow ()
createFleetOwnerInfo personId merchantId mbFleetType mbFleetName mbEnabled mbGstNumber mbReferredByOperatorId mbTicketPlaceId mbMerchantOperatingCityId mbTdsRate mbTdsThresholdAmount mbDocsVerificationStatus = do
  now <- getCurrentTime
      ...
      fleetOwnerInfo =
        FOI.FleetOwnerInformation
          { fleetOwnerPersonId = personId,
            ...
            tdsRate = mbTdsRate,
            tdsThresholdAmount = if fleetType == NORMAL_FLEET then mbTdsThresholdAmount else Nothing,
