    ...
    Just (counterpartyType, ownerId) -> do
      ...
      case mbAccount of
        ...
        Just account -> do
          ...
          lockedBalance <- WalletService.getNonRedeemableBalance (fromMaybe False transporterConfig.driverWalletConfig.nonRedeemableBalanceConsiderCreditAndDebit) account.id timeDiff cutOffDays =<< getCurrentTime
...
postFinanceManagementTdsReimbursementReject merchantShortId opCity requestId requestorId requestorName req = do
  ...
  let lockKey = LedgerAdjustment.ledgerAdjustmentLockKey (Just requestId.getId) (cast @Dashboard.Common.FinanceTdsReimbursementRequest @DLAR.LedgerAdjustmentRequest requestId)
