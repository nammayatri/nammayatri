...
postOperatorConsent (mbDriverId, merchantId, merchantOperatingCityId) = do
  ...
  when (transporterConfig.allowAnalytics == Just True) $ Analytics.incrementOperatorAnalyticsApplicationCount operator.id.getId
  ...
  unless (transporterConfig.requiresOnboardingInspection == Just True) $ do
    when (transporterConfig.allowAnalytics == Just True) $ Analytics.incrementOperatorAnalyticsDriverEnabled driverId
