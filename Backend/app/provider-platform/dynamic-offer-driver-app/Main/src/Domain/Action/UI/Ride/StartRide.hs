...
driverStartRide ::
  (MonadThrow m, Log m, EsqLocDBFlow m r, MonadTime m, CacheFlow m r, EsqDBFlow m r, CoreMetrics m, MonadReader r m, HasField "enableAPILatencyLogging" r Bool, HasField "enableAPIPrometheusMetricLogging" r Bool, CacheFlow m r, EsqDBFlow m r) =>
  ServiceHandle m ->
  Id DRide.Ride ->
  DriverStartRideReq ->
...
startRide ::
  (MonadThrow m, Log m, EsqLocDBFlow m r, CacheFlow m r, EsqDBFlow m r, MonadTime m, CoreMetrics m, MonadReader r m, HasField "enableAPILatencyLogging" r Bool, HasField "enableAPIPrometheusMetricLogging" r Bool, CacheFlow m r, EsqDBFlow m r) =>
  ServiceHandle m ->
  Id DRide.Ride ->
  StartRideReq ->
  m APISuccess.APISuccess
