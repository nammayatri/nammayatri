...
startRide ServiceHandle {..} rideId req = withLogTag ("rideId-" <> rideId.getId) $ do
  ...
  openMarketAllow <-
    maybe
      (pure False)
      ( \_ -> do
          transporterConfig <- QTC.findByMerchantOpCityId ride.merchantOperatingCityId (Just driverId) >>= fromMaybeM (TransporterConfigNotFound (getId ride.merchantOperatingCityId))
