...
handler merchantId req initReq = do
    ...
    InitSpecialZoneReq -> do
      case initReq of
        SPECIAL_QUOTE (specialZoneQuote, searchRequest) -> do
          (mbPaymentMethod, paymentUrl) <- fetchPaymentMethodAndUrl searchRequest.merchantOperatingCityId
          booking <- buildBookingSpecialzone specialZoneQuote searchRequest specialZoneQuote.id.getId searchRequest.startTime DRB.SpecialZoneBooking now (mbPaymentMethod <&> (.id)) paymentUrl Nothing searchRequest.merchantOperatingCityId
          _ <- QRB.createBooking booking
    ...
    InitRentalReq -> do
      case initReq of
        RENTAL_QUOTE (rentalQuote, searchRequest) -> do
          ...
          (mbPaymentMethod, paymentUrl) <- fetchPaymentMethodAndUrl searchRequest.merchantOperatingCityId
          booking <- buildRentalBooking updatedRentalQuotes searchRequest req.startTime DRB.RentalBooking now (mbPaymentMethod <&> (.id)) paymentUrl searchRequest.disabilityTag searchRequest.merchantOperatingCityId
          _ <- QRB.createBooking booking
  ...
  where
    buildNormalBooking ::
      ...
      m DRB.Booking
    buildNormalBooking searchRequest driverQuote quoteId startTime bookingType now mbPaymentMethodId paymentUrl disabilityTag merchantOpCityId = do
      id <- Id <$> generateGUID
