...
instance FromJSON PanInputDetails where
  parseJSON = withObject "PanInputDetails" $ \o ->
    PanInputDetails
      <$> o .: "input_pan_number"
      <*> o .: "input_name"
      <*> o .: "input_dob"

parsePanInputDetails :: Maybe Value -> Maybe PanInputDetails
parsePanInputDetails mval = mval >>= A.parseMaybe parseJSON
...
onVerifyPanHandler person imageId1 imageId2 output = do
  case parsePanInputDetails output.inputDetails of
    Just details -> do
      mEncryptedPanNumber <- mapM encrypt (Just $ inputPanNumber details)
