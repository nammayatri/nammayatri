        ...
        case status of
          ...
          Enums.CONFIRM_UPDATE -> do
            ...
            if transporterConfig.editLocDriverPermissionNeeded
              ...
              else void $ EditBooking.postEditResultInner (Just person.id, merchantOperatingCity.merchantId, merchantOperatingCity.id) bookingUpdateReq.id (EditBooking.EditBookingRespondAPIReq {action = EditBooking.ACCEPT})
    ...
    Enums.CONFIRM_UPDATE -> do
      ...
      if transporterConfig.editLocDriverPermissionNeeded
        then do
          ...
          let stopLocs = map (mkLocation booking.merchantOperatingCityId) newStopLocs'
          let entityData =
                Notify.UpdateLocationNotificationReq
                  { rideId = ride.id,
                    origin = Nothing,
