      ...
      then sendUpdateEditDestErrToBAP booking bapBookingUpdateRequestId "Trip Update Request Not Available" "Driver has an upcoming ride near your drop location. "
        ...
        case status of
          Enums.SOFT_UPDATE -> do
            ...
            searchReq <- SQSR.findByTransactionIdAndMerchantId transactionId person.merchantId >>= fromMaybeM (SearchRequestDoesNotExist transactionId)
            fareProducts <- getAllFarePoliciesProduct merchantOperatingCity.merchantId merchantOperatingCity.id False srcPt (Just dropLatLong) (Just (TransactionId (Id booking.transactionId))) searchReq.fromLocGeohash (Just estimatedDistance) (Just duration) booking.tripCategory
            farePolicy <- getFarePolicy (Just srcPt) searchReq.fromLocGeohash (Just estimatedDistance) (Just duration) merchantOperatingCity.id False booking.tripCategory booking.vehicleServiceTier (Just fareProducts.area) (Just booking.startTime) (Just (TransactionId (Id booking.transactionId)))
