  ...
  let params =
        CalculateFareParametersParams
          { farePolicy = fullFarePolicy,
            ...
            negativeFareAdjustment = origFareParams.negativeFareAdjustment,
            petCharges = origFareParams.petCharges,
            addOnCharges = origFareParams.addOnCharges,
            negotiatedFareDelta = origFareParams.negotiatedFareDelta,
...
handler (UEditLocationReq EditLocationReq {..}) = do
  ...
  whenJust mbRide $ \ride -> unless (ride.bookingId == booking.id) $ throwError (InvalidRequest $ "Ride " <> ride.id.getId <> " does not belong to bookingId " <> booking.id.getId)
      ...
      then sendUpdateEditDestErrToBAP booking bapBookingUpdateRequestId "Trip Update Request Not Available" "Driver has an upcoming ride near your drop location. "
        ...
        case status of
          Enums.SOFT_UPDATE -> do
            ...
            let routeInfo = RR.RouteInfo {distance = Just estimatedDistance, distanceWithUnit = Just (convertMetersToDistance booking.distanceUnit estimatedDistance), duration = Just duration, points = Just shortestRoute.points, routeToken = shortestRoute.routeToken, trafficSegments = shortestRoute.trafficSegments}
            ...
            fareProducts <- getAllFarePoliciesProduct merchantOperatingCity.merchantId merchantOperatingCity.id False srcPt (Just dropLatLong) Nothing Nothing (Just (TransactionId (Id booking.transactionId))) booking.fromLocGeohash booking.toLocGeohash (Just estimatedDistance) (Just duration) booking.dynamicPricingLogicVersion booking.tripCategory booking.configInExperimentVersions Nothing
              ...
              FC.calculateFareParameters
                CalculateFareParametersParams
                  { farePolicy = farePolicy',
                    ...
                    estimatedRideStaticDuration = shortestRoute.staticDuration,
                    timeDiffFromUtc = Nothing,
          ...
          Enums.CONFIRM_UPDATE -> do
            ...
            unless (bookingUpdateReq.bookingId == booking.id) $ throwError (InvalidRequest $ "BookingUpdateRequest " <> bapBookingUpdateRequestId <> " does not belong to bookingId " <> booking.id.getId)
