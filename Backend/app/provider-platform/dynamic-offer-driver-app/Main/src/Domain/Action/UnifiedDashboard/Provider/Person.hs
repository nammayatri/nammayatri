...
module Domain.Action.UnifiedDashboard.Provider.Person
  ( postPersonCreate,
...
postPersonCreate :: (Kernel.Types.Id.ShortId DM.Merchant -> Kernel.Types.Beckn.Context.City -> API.Types.Provider.Person.CreatePersonReq -> Environment.Flow API.Types.Provider.Person.CreatePersonResp)
postPersonCreate merchantShortId opCity req = do
  ...
  QP.create person
  pure $ API.Types.Provider.Person.CreatePersonResp {personId = Kernel.Types.Id.cast personId}

postUserLoginSendOtp ::
  ( Kernel.Types.Id.ShortId DM.Merchant ->
    Kernel.Types.Beckn.Context.City ->
    API.Types.Provider.Person.SendOtpReq ->
    Environment.Flow API.Types.Provider.Person.SendOtpResp
  )
postUserLoginSendOtp merchantShortId opCity req = do
  ...
  pure $ API.Types.Provider.Person.SendOtpResp {otp = otp}
