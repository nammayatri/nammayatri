...
module Domain.Action.Dashboard.Management.SosMedia
  ( getSosMediaSosMedia,
  )
where

import qualified API.Types.ProviderPlatform.Management.SosMedia as Common
...
getSosMediaSosMedia _merchantShortId _opCity driverId = do
  let personId = cast @Dashboard.Common.Driver @DP.Person driverId
