...
getAllDriverForFleet merchantShortId fleetOwnerId mbLimit mbOffset = do
  ...
  driversInfo <- QPerson.findAllDriversByIdsFirstNameAsc merchant.id driverIdList
  let fleetDriversInfos = map convertToDriverAPIEntity driversInfo
  return $ Common.FleetListDriverRes fleetDriversInfos

convertToDriverAPIEntity :: QPerson.FullDriver -> Common.FleetDriversAPIEntity
convertToDriverAPIEntity QPerson.FullDriver {..} =
  Common.FleetDriversAPIEntity
    { driverId = cast @DP.Person @Common.Driver person.id,
      firstName = person.firstName,
      middleName = person.middleName,
      lastName = person.lastName,
      mobileNumber = person.unencryptedMobileNumber,
      mobileCountryCode = person.mobileCountryCode
    }
