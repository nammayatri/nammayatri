...
addVehicleForFleet merchantShortId reqDriverPhoneNo mbMobileCountryCode fleetOwnerId req = do
  ...
  vehicle <- RCQuery.findLastVehicleRCWrapper req.registrationNo >>= fromMaybeM (VehicleDoesNotExist req.registrationNo)
  unless (isJust vehicle.fleetOwnerId && vehicle.fleetOwnerId == Just fleetOwnerId) $ throwError VehicleBelongsToAnotherFleet
...
getFleetDriverVehicleAssociation _merchantShortId fleetOwnerId mbLimit mbOffset = do
  ...
  where
    ...
    createDriverVehicleAssociationListItem (vehicleRC, _fda, drca) = do
      ...
      completedRides <- QRD.totalRidesByFleetOwnerPerVehicleAndDriver (Just fleetOwnerId) vehicle.registrationNo driverId.getId
      earning <- QRide.totalEarningsByFleetOwnerPerVehicleAndDriver (Just fleetOwnerId) vehicle.registrationNo driverId.getId
      pure $
...
-- getAllDriverForFleet merchantShortId fleetOwnerId mbLimit mbOffset = do
--   merchant <- findMerchantByShortId merchantShortId
--   let limit = fromMaybe 10 mbLimit
--       offset = fromMaybe 0 mbOffset
--   driverList <- FDV.findAllActiveDriverByFleetOwnerId  fleetOwnerId limit offset
--   driversInfo <- QPerson.findAllDriversByIdsFirstNameAs merchant.id (coerce $ map (coerce . driverId) driverList)
--   drivers <- traverse convertToDriverAPIEntity driversInfo
--   return $ Common.FleetListDriverRes drivers

-- convertToDriverAPIEntity :: DP.Person -> Common.DriversAPIEntity
-- convertToDriverAPIEntity DP.Person {..} =
--   Common.DriverAPIEntity
...
fleetDriverEarning :: ShortId DM.Merchant -> Text -> Text -> Flow Common.FleetEarningRes
fleetDriverEarning _merchantShortId fleetOwnerId driverId = do
  let dId = Id driverId
  driver <- QPerson.findById dId >>= fromMaybeM (PersonNotFound dId.getId)
  let encryptedDriverNo = fromMaybe " " driver.unencryptedMobileNumber
  phoneNumberHash <- getDbHash (T.toLower encryptedDriverNo)
  let driverName = driver.firstName <> " " <> fromMaybe "" driver.lastName
  totalRides <- QRD.totalRidesByFleetOwnerPerDriver (Just fleetOwnerId) phoneNumberHash -- shrey00 check function names
  totalEarning <- QRide.totalEarningsByFleetOwnerPerDriver (Just fleetOwnerId) driverId -- shrey00 check function names
  driverInfo' <- QDriverInfo.findById (cast driverId) >>= fromMaybeM DriverInfoNotFound
  pure $
    Common.FleetEarningRes
      { driverId = Just driverId,
