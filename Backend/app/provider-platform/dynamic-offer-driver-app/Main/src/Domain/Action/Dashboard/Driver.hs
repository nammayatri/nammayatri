...
setVehicleDriverRcStatusForFleet merchantShortId opCity reqDriverId fleetOwnerId req = do
  ...
  case (mbSerivceName, req.isActivate) of
    (Just YATRI_RENTAL, True) -> do
      void $ toggleDriverSubscriptionByService (driver.id, driver.merchantId, driver.merchantOperatingCityId) YATRI_RENTAL (Id <$> req.planToAssociate) True req.rcNo
    (Just YATRI_RENTAL, False) -> do
      void $ toggleDriverSubscriptionByService (driver.id, driver.merchantId, driver.merchantOperatingCityId) YATRI_RENTAL (Id <$> req.planToAssociate) False req.rcNo
    _ -> pure ()
  logTagInfo "dashboard -> addVehicle : " (show driver.id)
  pure Success
...
getFleetDriverAssociation _merchantShortId _opCity fleetOwnerId mbLimit mbOffset mbDriverName mbDriverPhNo isSearch = do
  let limit = fromMaybe 10 mbLimit
      offset = fromMaybe 0 mbOffset
  listOfAllDrivers <- if isSearch then FDV.findAllDriverByFleetOwnerId' fleetOwnerId else FDV.findAllDriverByFleetOwnerId fleetOwnerId limit offset
...
getFleetVehicleAssociation ::
  ...
  Bool ->
  Flow Common.DrivertoVehicleAssociationRes
getFleetVehicleAssociation _merchantShortId _opCity fleetOwnerId mbLimit mbOffset mbVehicleNumber mbDriverPhNo isSearch = do
  let limit = fromMaybe 10 mbLimit
      offset = fromMaybe 0 mbOffset
  listOfAllVehicle <- if isSearch then RCQuery.findAllByFleetOwnerId' fleetOwnerId else RCQuery.findAllByFleetOwnerId fleetOwnerId limit offset
...
toggleDriverSubscriptionByService ::
  ...
  Text ->
  Flow ()
toggleDriverSubscriptionByService (driverId, mId, mOpCityId) serviceName mbPlanToAssign flag vehicleNo = do
  ...
  if flag
    then do
      ...
      case autoPayStatus of
        ...
        Just DrInfo.ACTIVE -> QDP.updateEnableServiceUsageChargeByDriverIdAndServiceName driverId flag serviceName
        _ -> pure ()
      ...
      when (isJust driverPlan && vehicleLinkedWithDPlan == Just vehicleNo) $ do
        QDP.updateEnableServiceUsageChargeByDriverIdAndServiceName driverId flag serviceName
    ...
    case channel of
      ...
      WHATSAPP -> do
        merchantMessage <-
          QMM.findByMerchantOpCityIdAndMessageKey merchantOpCityId mkey
            >>= fromMaybeM (MerchantMessageNotFound merchantOpCityId.getId (show mkey))
        result <- Whatsapp.whatsAppSendMessageWithTemplateIdAPI driver.merchantId merchantOpCityId (Whatsapp.SendWhatsAppMessageWithTemplateIdApIReq phoneNumber merchantMessage.templateId (Just vehicleId) (Just timeStamp) mbReason Nothing)
        when (result._response.status /= "success") $ throwError (InternalError "Unable to send Whatsapp message via dashboard")
      _ -> pure ()
