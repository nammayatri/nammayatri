...
getOperatingCity merchantShortId _ mbMobileCountryCode mbMobileNumber mbRideId = do
  ...
  case (mbRideId, mbMobileNumber) of
    ...
    (_, Just mobileNumber) -> do
      ...
      driver <- QPerson.findByMobileNumberAndMerchant (fromMaybe "+91" (appendPlusInMobileCountryCode mbMobileCountryCode)) mobileNumberHash merchant.id >>= fromMaybeM (InvalidRequest "Person not found")
