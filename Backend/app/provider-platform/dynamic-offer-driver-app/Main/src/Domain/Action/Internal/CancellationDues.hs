  ...
  canWaiveOffResult <-
    if transporterConfig.canAddCancellationFee
      then do
        ...
        response <- withTryCatch "runLogics:canWaiveOffResult" $ LYDL.runLogicsWithDebugLog LYDL.Driver (cast ride.merchantOperatingCityId) LYT.USER_CANCELLATION_DUES_WAIVE_OFF (Nothing :: Maybe Text) allLogics logicInput
        case response of
          Left e -> do
