    ...
    Just person -> do
      withLogTag ("personId_" <> getId person.id) $ do
        ...
        whenNothing_ useFakeOtpM $ do
          ...
          Sms.sendSMS person.merchantId merchantOpCityId (Sms.SendSMSReq message phoneNumber sender (Just templateId)) >>= Sms.checkSmsResult
  ...
  case mbAuthId of
    Just authId -> do
      ...
      withLogTag ("personId_" <> getId person.id) $ do
        ...
        let sender = fromMaybe smsCfg.sender mbSender
        Sms.sendSMS person.merchantId merchantOpCityId (Sms.SendSMSReq message phoneNumber sender (Just templateId))
          >>= Sms.checkSmsResult
    Nothing -> do
  ...
  where
    ...
    sendDeepLinkForAuth person mobileNumber merchantId merchantOpCityId fleetOwner = do
      ...
      withLogTag ("sending Deeplink Auth SMS" <> getId person.id) $ do
        ...
        let sender = fromMaybe smsCfg.sender mbSender
        Sms.sendSMS merchantId merchantOpCityId (Sms.SendSMSReq message phoneNumber sender (Just templateId)) >>= Sms.checkSmsResult
