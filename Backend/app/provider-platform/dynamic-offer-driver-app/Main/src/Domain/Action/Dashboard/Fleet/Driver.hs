...
postDriverFleetAddVehicle merchantShortId opCity reqDriverPhoneNo fleetOwnerId mbMobileCountryCode req = do
  ...
  isFleetDriver <- QFDV.findByDriverIdAndFleetOwnerId driver.id fleetOwnerId True
...
checkRCAssociationForFleet fleetOwnerId vehicleRC = do
  ...
  forM_ rcAssociatedDriverIds $ \driverId -> do
    isFleetDriver <- QFDV.findByDriverIdAndFleetOwnerId driverId fleetOwnerId True
...
getDriverFleetGetAllDriver _merchantShortId _opCity fleetOwnerId mbLimit mbOffset mbMobileNumber mbName = do
  ...
  driverList <- QFDV.findAllActiveDriverByFleetOwnerId fleetOwnerId limit offset mbMobileNumberHash mbName
...
postDriverFleetUnlink merchantShortId _opCity fleetOwnerId reqDriverId vehicleNo = do
  ...
  driver <-
    QPerson.findById personId
      >>= fromMaybeM (PersonDoesNotExist personId.getId)
  isFleetDriver <- QFDV.findByDriverIdAndFleetOwnerId personId fleetOwnerId True
  case isFleetDriver of
...
postDriverFleetRemoveVehicle _merchantShortId _ fleetOwnerId_ vehicleNo = do
  ...
  whenJust vehicle $ \veh -> do
    isFleetDriver <- QFDV.findByDriverIdAndFleetOwnerId veh.driverId fleetOwnerId_ True
  ...
  forM_ associations $ \assoc -> do
    isFleetDriver <- QFDV.findByDriverIdAndFleetOwnerId assoc.driverId fleetOwnerId_ True
...
postDriverFleetRemoveDriver _merchantShortId _ fleetOwnerId driverId = do
  ...
  QFDV.endFleetDriverAssociation fleetOwnerId personId
  pure Success
...
getDriverFleetDriverEarning merchantShortId _ fleetOwnerId mbMobileCountryCode mbDriverPhNo mbLimit mbOffset mbFrom mbTo mbSortDesc mbSortOn = do
  ...
  driverId <- case mbDriverPhNo of
    Just driverPhNo -> do
      ...
      fleetDriverAssociation <- QFDV.findByDriverIdAndFleetOwnerId driver.id fleetOwnerId True
  ...
  case mbDriverPhNo of
    ...
    Nothing -> do
      let limit = min 10 $ fromMaybe 5 mbLimit
          offset = fromMaybe 0 mbOffset
      case mbMode of
        Just mode -> QFDV.findAllDriversByFleetOwnerIdByMode fleetOwnerId (castDashboardDriverStatus mode) mbIsActive (fromIntegral limit) (fromIntegral offset)
        _ -> QFDV.findAllDriverByFleetOwnerIdAndMbIsActive fleetOwnerId mbIsActive limit offset
    ...
    Nothing -> do
      ...
      case checkedAssociation of
        ...
        OperatorAssociation operator -> do
          ...
          assoc <- DOA.makeDriverOperatorAssociation person.id operator.id.getId (DomainRC.convertTextToUTC (Just "2099-12-12"))
    ...
    Common.FLEET_ASSOCIATION -> do
      case requestor.role of
        ...
        DP.OPERATOR -> do
          ...
          association <-
            QFleetOperatorAssociation.findByfleetOperatorIdAndFleetOwnerId requestor.id.getId fleetOwner.id.getId True
          ...
          QDriverOperatorAssociation.checkDriverOperatorAssociation person.id operator.id
            >>= \isAssociated -> unless isAssociated $ do
              fork "Sending Operator Consent SMS to Driver" $ do
                QDriverOperatorAssociation.createDriverOperatorAssociationIfNotExists person.id operator.id req_.driverOnboardingVehicleCategory False
