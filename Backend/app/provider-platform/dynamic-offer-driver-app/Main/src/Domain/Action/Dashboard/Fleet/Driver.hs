...
unlinkVehicleFromDriver merchant personId vehicleNo opCity role = do
  ...
  merchantOpCityId <- CQMOC.getMerchantOpCityId Nothing merchant (Just opCity)
  transporterConfig <- getOneConfig (TransporterConfigDimensions {merchantOperatingCityId = merchantOpCityId.getId}) Nothing >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
  SGuard.withOnboardingAction transporterConfig (maybe (SGuard.ActorDriver personId) (\fleetOwnerId -> SGuard.ActorFleetAndDriver (Id fleetOwnerId) personId) rc.fleetOwnerId) SGuard.UnlinkVehicle (SGuard.TargetDriver personId) $ do
...
postDriverFleetVehicleDriverRcStatus merchantShortId opCity reqDriverId requestorId mbFleetOwnerId req = do
  ...
  SGuard.withOnboardingAction transporterConfig actor (if req.isActivate then SGuard.ActivateVehicle else SGuard.DeactivateVehicle) (SGuard.TargetDriverVehicle personId req.rcNo) $
...
postDriverUpdateFleetOwnerInfo merchantShortId opCity driverId req = do
  ...
  merchantOpCity <- CQMOC.findById merchantOpCityId >>= fromMaybeM (MerchantOperatingCityNotFound merchantOpCityId.getId)
  transporterConfig <- getOneConfig (TransporterConfigDimensions {merchantOperatingCityId = merchantOpCityId.getId}) Nothing >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
  let validateFn = if transporterConfig.isStrongNameCheckRequired then Common.validateUpdateFleetOwnerInfoReq else Common.validateUpdateFleetOwnerInfoReqWithLooseCheck
...
postDriverFleetLinkRCWithDriver merchantShortId opCity fleetOwnerId mbRequestorId req = do
  ...
  when (not isValidAssociation) $ do
    ...
    SGuard.withOnboardingAction transporterConfig (SGuard.ActorFleetAndDriver (Id fleetOwnerId) driver.id) SGuard.LinkVehicle (SGuard.TargetVehicleById rc.id) $
...
postDriverFleetAddDrivers merchantShortId opCity mbRequestorId req = do
  ...
  where
    ...
    processDriverByFleetOwner merchant moc transporterConfig fleetOwner req_ = do
      validateDriverName req_.driverName transporterConfig.isDriverNameMandatoryInBulkUpload transporterConfig.isStrongNameCheckRequired
...
postDriverFleetApproveDriver merchantShortId opCity fleetOwnerId req = do
  ...
  (pnType, messageBuilder) <- case req.approve of
    Just True -> do
      SGuard.withOnboardingAction transporterConfig (SGuard.ActorFleetAndDriver (Id fleetOwnerId) driverId) SGuard.ActivateToFleet (SGuard.TargetDriver driverId) $ do
