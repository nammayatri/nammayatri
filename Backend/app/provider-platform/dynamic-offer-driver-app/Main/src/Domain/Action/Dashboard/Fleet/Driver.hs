...
postDriverFleetRemoveDriver merchantShortId opCity requestorId driverId mbFleetOwnerId = do
  ...
  case entityRole of
    ...
    DP.OPERATOR -> do
      ...
      when (transporterConfig.allowAnalytics == Just True) $ Analytics.decrementOperatorAnalyticsApplicationCount transporterConfig entityId
      when (transporterConfig.allowCacheDriverFlowStatus == Just True) $ do
        driverInfo <- QDriverInfo.findById personId >>= fromMaybeM (DriverNotFound personId.getId)
  ...
  case mbAuthId of
    ...
    Nothing -> do
      ...
      when (transporterConfig.allowAnalytics == Just True) $ do
        ...
        whenJust mOperator $ \operator -> Analytics.incrementOperatorAnalyticsApplicationCount transporterConfig operator.operatorId
      when (transporterConfig.allowCacheDriverFlowStatus == Just True) $ do
