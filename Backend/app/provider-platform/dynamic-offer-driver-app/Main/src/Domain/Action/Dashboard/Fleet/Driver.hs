...
getDriverFleetDriverAssociation :: ShortId DM.Merchant -> Context.City -> Maybe Bool -> Maybe Int -> Maybe Int -> Maybe Text -> Maybe Text -> Maybe Bool -> Maybe UTCTime -> Maybe UTCTime -> Maybe Common.DriverMode -> Maybe Text -> Maybe Text -> Maybe Text -> Maybe Text -> Maybe Bool -> Maybe Bool -> Maybe Bool -> Maybe Text -> Maybe Bool -> Maybe Bool -> Flow Common.DrivertoVehicleAssociationResT
getDriverFleetDriverAssociation merchantShortId opCity mbIsActive mbLimit mbOffset mbCountryCode mbDriverPhNo mbStats mbFrom mbTo mbMode mbName mbSearchString mbFleetOwnerId mbRequestorId hasFleetMemberHierarchy isRequestorFleerOwner mbHasRequestReason mbServiceTier mbEnabled mbApproved = do
  ...
  listOfAllDrivers <- getListOfDriversMultiFleet mbCountryCode mbDriverPhNo fleetOwnerIds merchant.id merchantOpCity.id mbIsActive mbLimit mbOffset mbMode mbName mbSearchString mbHasRequestReason mbEnabled mbFrom mbTo mbApproved
...
getDriverFleetVehicleAssociation merchantShortId opCity mbLimit mbOffset mbVehicleNumber mbIncludeStats mbFrom mbTo mbStatus mbSearchString mbStatusAwareVehicleNo mbFleetOwnerId mbRequestorId hasFleetMemberHierarchy isRequestorFleerOwner mbApproved = do
  requestorId <- mbRequestorId & fromMaybeM (InvalidRequest "Requestor ID is required")
  let effectiveFleetOwnerId =
        if isRequestorFleerOwner == Just True
          then Just requestorId
  ...
  when (fromMaybe False merchant.fleetOwnerEnabledCheck && not (null fleetOwnerIds)) $ mapM_ (\info -> DCommon.checkFleetOwnerVerification info.fleetOwnerId merchant.fleetOwnerEnabledCheck) fleetOwnersInfo
  let fleetOwnerNameMapFromRequest = Map.fromList $ map (\info -> (info.fleetOwnerId, (info.fleetOwnerName, info.fleetName))) fleetOwnersInfo
  listOfAllVehicle <- getListOfVehiclesMultiFleet mbVehicleNumber fleetOwnerIds mbLimit mbOffset mbStatus merchant.id merchantOpCity.id mbSearchString mbStatusAwareVehicleNo mbApproved mbFrom mbTo
...
postDriverFleetVehicleEdit merchantShortId opCity requestorId _mbFleetOwnerId _mbDriverId mbVehicleNo mbRcId Common.EditVehicleReq {..} = do
  ...
  whenJust rc.fleetOwnerId $ \fleetOwnerId -> do
    case mbRequestor of
      ...
      Just requestor -> do
        (role, entityId) <- validateRequestorRoleAndGetEntityId requestor.id.getId (Just fleetOwnerId)
        case role of
          DP.FLEET_OWNER -> DCommon.checkFleetOwnerVerification entityId merchant.fleetOwnerEnabledCheck
          DP.OPERATOR -> pure ()
          _ -> throwError (InvalidRequest "Invalid Data")
  now <- getCurrentTime
...
getListOfVehiclesMultiFleet :: Maybe Text -> [Text] -> Maybe Int -> Maybe Int -> Maybe Common.FleetVehicleStatus -> Id DM.Merchant -> Id DMOC.MerchantOperatingCity -> Maybe Text -> Maybe Text -> Maybe Bool -> Maybe UTCTime -> Maybe UTCTime -> Flow [DVRC.VehicleRegistrationCertificate]
getListOfVehiclesMultiFleet mbVehicleNo fleetOwnerIds mbLimit mbOffset mbStatus merchantId merchantOperatingCityId mbSearchString statusAwareVehicleNo mbApproved mbFrom mbTo = do
  let limit = fromIntegral $ min 10 $ fromMaybe 5 mbLimit
      offset = fromIntegral $ fromMaybe 0 mbOffset
  case mbVehicleNo of
    ...
    Nothing -> do
      case mbStatus of
        ...
        Just Common.Valid -> RCQuery.findAllRCByStatusForFleetMF fleetOwnerIds (Just $ castFleetVehicleStatus mbStatus) limit offset merchantId merchantOperatingCityId.getId statusAwareVehicleNo mbApproved mbFrom mbTo
        Just Common.Invalid -> RCQuery.findAllRCByStatusForFleetMF fleetOwnerIds (Just $ castFleetVehicleStatus mbStatus) limit offset merchantId merchantOperatingCityId.getId statusAwareVehicleNo mbApproved mbFrom mbTo
        Just Common.Pending -> RCQuery.findAllRCByStatusForFleetMF fleetOwnerIds (Just $ castFleetVehicleStatus mbStatus) limit offset merchantId merchantOperatingCityId.getId statusAwareVehicleNo mbApproved mbFrom mbTo
        Nothing -> RCQuery.findAllRCByStatusForFleetMF fleetOwnerIds Nothing limit offset merchantId merchantOperatingCityId.getId statusAwareVehicleNo mbApproved mbFrom mbTo

getListOfDriversMultiFleet :: Maybe Text -> Maybe Text -> [Text] -> Id DM.Merchant -> Id DMOC.MerchantOperatingCity -> Maybe Bool -> Maybe Int -> Maybe Int -> Maybe Common.DriverMode -> Maybe Text -> Maybe Text -> Maybe Bool -> Maybe Bool -> Maybe UTCTime -> Maybe UTCTime -> Maybe Bool -> Flow ([FleetDriverAssociation], [DP.Person], [DI.DriverInformation])
getListOfDriversMultiFleet _ mbDriverPhNo fleetOwnerIds merchantId merchantOperatingCityId mbIsActive mbLimit mbOffset mbMode mbName mbSearchString mbHasRequestReason mbEnabled mbFrom mbTo mbApproved = do
  ...
  let mode = castDashboardDriverStatus <$> mbMode
  driverAssociationAndInfo <- FDV.findAllActiveDriverByFleetOwnerIdWithDriverInfoMF fleetOwnerIds merchantId.getId merchantOperatingCityId.getId limit offset mobileNumberHash mbName mbSearchString mbIsActive mode mbHasRequestReason mbEnabled mbFrom mbTo mbApproved
  let (fleetDriverAssociation, person, driverInformation) = unzip3 driverAssociationAndInfo
  return (fleetDriverAssociation, person, driverInformation)
...
getDriverFleetVehicleListStats merchantShortId opCity requestorId mbFleetOwnerId mbVehicleNo mbLimit mbOffset fromDay toDay = do
  ...
  agg <- case fleetOwnerIds of
    [singleFoId] -> do
      mbRcId <- case mbVehicleNo of
        Just vehicleNo -> do
          mbRc <- VRCQuery.findLastVehicleRCFleet' vehicleNo singleFoId
          pure $ (.id.getId) <$> mbRc
        Nothing -> pure Nothing
      case transporterConfig.analyticsConfig.useDbForEarningAndMetrics of
        True -> QFRDSExtra.sumVehicleStatsByFleetOwnerIdAndDateRange singleFoId mbRcId limit offset fromDay toDay
        _ -> CFRDSExtra.aggerateVehicleStatsByFleetOwnerIdAndDateRange singleFoId mbRcId limit offset fromDay toDay
    multipleIds -> do
      mbRcId <- case mbVehicleNo of
        Just vehicleNo -> do
        ...
        Nothing -> pure Nothing
      case transporterConfig.analyticsConfig.useDbForEarningAndMetrics of
        True -> QFRDSExtra.sumVehicleStatsByFleetOwnerIdsAndDateRange multipleIds mbRcId limit offset fromDay toDay
        _ -> CFRDSExtra.aggerateVehicleStatsByFleetOwnerIdsAndDateRange multipleIds mbRcId limit offset fromDay toDay
  ...
  pure $ Common.FleetVehicleStatsRes {fleetOwnerId = mbFleetOwnerId, listItem = agg', summary = Common.Summary {totalCount = length agg, count = length agg}}
  where
    maxLimit = 10
    defaultLimit = 5
...
getDriverFleetDriverOnboardedDriversAndUnlinkedVehicles merchantShortId opCity fleetOwnerId mbLimit mbOffset = do
  merchant <- findMerchantByShortId merchantShortId
  merchantOpCity <- CQMOC.findByMerchantIdAndCity merchant.id opCity >>= fromMaybeM (MerchantOperatingCityNotFound $ "merchantShortId: " <> merchantShortId.getShortId <> " ,city: " <> show opCity)
  listOfAllVehicle <- RCQuery.findAllRCByStatusForFleetMF [fleetOwnerId] Nothing (fromIntegral $ fromMaybe 100000 mbLimit) (fromIntegral $ fromMaybe 0 mbOffset) merchant.id merchantOpCity.id.getId Nothing Nothing Nothing Nothing
