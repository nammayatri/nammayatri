...
getDriverFleetDriverVehicleAssociation merchantShortId _opCity fleetOwnerId mbLimit mbOffset mbCountryCode mbPhoneNo mbVehicleNo mbStatus mbFrom mbTo = do
  ...
  pure $ Common.DrivertoVehicleAssociationRes {fleetOwnerId = Just fleetOwnerId, listItem = filteredItems, summary = summary}
  where
...
buildFleetOwnerNameMap :: Map.Map Text Text -> [Text] -> Flow (Map.Map Text Text)
buildFleetOwnerNameMap currentMap ownerIds = do
  ...
  let missingOwnerMap = Map.fromList $ map (\fleetOwner -> (fleetOwner.id.getId, fleetOwner.firstName <> maybe "" (" " <>) fleetOwner.lastName)) missingOwners
...
getDriverFleetDriverAssociation merchantShortId opCity mbIsActive mbLimit mbOffset mbCountryCode mbDriverPhNo mbStats mbFrom mbTo mbMode mbName mbSearchString mbFleetOwnerId mbRequestorId hasFleetMemberHierarchy isRequestorFleerOwner mbHasRequestReason mbServiceTier = do
  ...
  pure $
    Common.DrivertoVehicleAssociationResT
      { fleetOwnerId = Just requestorId, -- Kept for Backward Compatibility, Don't use this Fleet Owner Id for Multiple Fleets
        listItem = listItems,
        summary = summary
      }
  where
    ...
    createFleetDriverAssociationListItem fleetOwnerNameMap (fdaList, personList, driverInfoList) = do
      ...
      forM (zip driverListWithInfo fdaList) $ \((driver, driverInfo'), fda) -> do
        let fleetOwnerIdVal = fda.fleetOwnerId
            fleetOwnerNameVal = fromMaybe "" (Map.lookup fleetOwnerIdVal fleetOwnerNameMap)
            mbFleetOwnerName = if T.null fleetOwnerNameVal then Nothing else Just fleetOwnerNameVal
        driverRCAssociation <- QRCAssociation.findAllActiveAndInactiveAssociationsByDriverId driver.id
        let rcAssociatedWithFleet = filter (\(_, rc) -> rc.fleetOwnerId == Just fleetOwnerIdVal) driverRCAssociation
        ...
        (completedRides, earning) <- case mbStats of
          Just True -> do
            rides <- CQRide.totalRidesByFleetOwnerPerDriver (Just fleetOwnerIdVal) driver.id from to
            earnings <- CQRide.totalEarningsByFleetOwnerPerDriver (Just fleetOwnerIdVal) driver.id from to
            pure (rides, earnings)
          _ -> pure (0, 0)
        ...
        (isDriverOnPickup, isDriverOnRide, routeCode) <-
          if driverInfo'.onRide
            then do
              currentTripTransaction <- WMB.findNextActiveTripTransaction fleetOwnerIdVal driver.id
        ...
        let ls =
              Common.DriveVehicleAssociationListItemT
                { driverName = driverName,
                  ...
                  fleetOwnerId = Just fleetOwnerIdVal,
...
getDriverFleetVehicleAssociation merchantShortId opCity mbLimit mbOffset mbVehicleNumber mbIncludeStats mbFrom mbTo mbStatus mbSearchString mbStatusAwareVehicleNo mbFleetOwnerId mbRequestorId hasFleetMemberHierarchy isRequestorFleerOwner = do
  requestorId <- mbRequestorId & fromMaybeM (InvalidRequest "Requestor ID is required")
  let effectiveFleetOwnerId =
        if isRequestorFleerOwner == Just True
          then Just requestorId
  ...
  when (fromMaybe False merchant.fleetOwnerEnabledCheck && not (null fleetOwnerIds)) $ mapM_ (\info -> DCommon.checkFleetOwnerVerification info.fleetOwnerId merchant.fleetOwnerEnabledCheck) fleetOwnersInfo
  let fleetOwnerNameMapFromRequest = Map.fromList $ map (\info -> (info.fleetOwnerId, info.fleetOwnerName)) fleetOwnersInfo
  listOfAllVehicle <- getListOfVehiclesMultiFleet mbVehicleNumber fleetOwnerIds mbLimit mbOffset mbStatus merchant.id merchantOpCity.id mbSearchString mbStatusAwareVehicleNo
  let fleetOwnerIdsFromResponse = nub $ mapMaybe (.fleetOwnerId) listOfAllVehicle
  fleetOwnerNameMap <- buildFleetOwnerNameMap fleetOwnerNameMapFromRequest fleetOwnerIdsFromResponse
  let listOfAllVehicleForResponse =
        if isNothing effectiveFleetOwnerId
          then listOfAllVehicle
          else filter (isJust . (.fleetOwnerId)) listOfAllVehicle
  let listOfAllVehicleWithFleetInfo =
        map
          ( \vrc ->
              let mbFleetOwnerIdVal = vrc.fleetOwnerId
                  mbFleetOwnerNameVal = mbFleetOwnerIdVal <&> \fleetOwnerIdVal -> fromMaybe "" (Map.lookup fleetOwnerIdVal fleetOwnerNameMap)
               in (vrc, mbFleetOwnerIdVal, mbFleetOwnerNameVal)
          )
          listOfAllVehicleForResponse
  ...
  where
    createFleetVehicleAssociationListItem :: [(DVRC.VehicleRegistrationCertificate, Maybe Text, Maybe Text)] -> Flow [Common.DriveVehicleAssociationListItemT]
    createFleetVehicleAssociationListItem vrcListWithFleetInfo = do
      now <- getCurrentTime
      forM vrcListWithFleetInfo $ \(vrc, mbFleetOwnerIdVal, mbFleetOwnerNameVal) -> do
        let mbFleetOwnerName = mbFleetOwnerNameVal >>= \fleetOwnerNameVal -> if T.null fleetOwnerNameVal then Nothing else Just fleetOwnerNameVal
        decryptedVehicleRC <- decrypt vrc.certificateNumber
        let defaultFrom = UTCTime (utctDay now) 0
            from = fromMaybe defaultFrom mbFrom
            to = fromMaybe now mbTo
        stats <- case mbIncludeStats of
          Just _ -> do
            completedRides <- CQRide.totalRidesByFleetOwnerPerVehicle mbFleetOwnerIdVal decryptedVehicleRC from to
            earning <- CQRide.totalEarningsByFleetOwnerPerVehicle mbFleetOwnerIdVal decryptedVehicleRC from to
        ...
        ((driverName, conductorName, driverId, driverPhoneNo, driverStatus, isDriverOnPickup, isDriverOnRide, routeCode), mbAssociatedOn) <- case rcActiveAssociation of
          Just activeAssociation -> do
            driverInfo <- getFleetDriverInfoMaybe mbFleetOwnerIdVal activeAssociation.driverId False
          ...
          Nothing -> do
            ...
            case latestAssociation of
              Just latestAssoc -> do
                driverInfo <- getFleetDriverInfoMaybe mbFleetOwnerIdVal latestAssoc.driverId False
        ...
        let ls =
              Common.DriveVehicleAssociationListItemT
                { driverName = driverName,
                  ...
                  upcomingRouteCode = routeCode,
                  fleetOwnerId = mbFleetOwnerIdVal,
                  fleetOwnerName = mbFleetOwnerName,
...
getDriverFleetDriverListStats merchantShortId opCity requestorId mbFrom mbTo mbFleetOwnerId mbSearch mbLimit mbOffset sortDesc sortOnField mbResponseType = do
  ...
  (driverIdObjs, fleetOwnerId) <-
    case (mbRole, mbFleetOwnerId) of
      ...
      (_, Just fleetOwnerIdVal) -> do
        void $ FleetAccess.checkRequestorAccessToFleet False (Just requestorId) fleetOwnerIdVal
        ids <-
          if useDBForAnalytics
            then QFDAExtra.getActiveDriverIdsByFleetOwnerId fleetOwnerIdVal
            else do
              maybeDriverIds <- CFDA.getDriverIdsByFleetOwnerId fleetOwnerIdVal
              pure $ fromMaybe [] maybeDriverIds
        pure (ids, fleetOwnerIdVal)
      (_, Nothing) -> do
...
getFleetDriverInfoMaybe :: Maybe Text -> Id DP.Person -> Bool -> Flow (Maybe Text, Maybe Text, Maybe Text, Maybe Text, Maybe DrInfo.DriverMode, Maybe Bool, Maybe Bool, Maybe Text)
getFleetDriverInfoMaybe mbFleetOwnerId driverId isDriver =
  case mbFleetOwnerId of
    Just fleetOwnerId -> getFleetDriverInfo fleetOwnerId driverId isDriver
    Nothing -> do
      mbDriver <- QPerson.findById driverId
      mbDriverInfo' <- QDriverInfo.findById driverId
      case (mbDriverInfo', mbDriver) of
        (Just driverInfo', Just driver) -> do
          ...
          mobileNumber <- mapM decrypt driver.mobileNumber
          pure (Just driver.firstName, driver.lastName, Just driver.id.getId, mobileNumber, mode, Just False, Just False, Nothing)
        (_, _) -> pure (Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing)
...
getListOfVehiclesMultiFleet :: Maybe Text -> [Text] -> Maybe Int -> Maybe Int -> Maybe Common.FleetVehicleStatus -> Id DM.Merchant -> Id DMOC.MerchantOperatingCity -> Maybe Text -> Maybe Text -> Flow [DVRC.VehicleRegistrationCertificate]
getListOfVehiclesMultiFleet mbVehicleNo fleetOwnerIds mbLimit mbOffset mbStatus merchantId merchantOperatingCityId mbSearchString statusAwareVehicleNo = do
  let limit = fromIntegral $ min 10 $ fromMaybe 5 mbLimit
      offset = fromIntegral $ fromMaybe 0 mbOffset
  case mbVehicleNo of
    Just vehicleNo -> RCQuery.partialFindLastVehicleRCFleetMF vehicleNo merchantId merchantOperatingCityId.getId fleetOwnerIds limit offset
    Nothing -> do
      case mbStatus of
        -- This Status is Associated with Driver and
        Just Common.Active -> RCQuery.findAllActiveRCForFleetByLimitOffsetMF fleetOwnerIds merchantId merchantOperatingCityId.getId limit offset mbSearchString statusAwareVehicleNo
        Just Common.InActive -> RCQuery.findAllInactiveRCForFleetMF fleetOwnerIds limit offset merchantId merchantOperatingCityId.getId statusAwareVehicleNo
        -- This Status is only Associated purely with RCs and Not Associated with any Driver

        -- make changes here for onride and tripassigned
        Just Common.OnRide -> RCQuery.findAllVehicleByStatusForFleetByLimitOffsetMF fleetOwnerIds merchantId merchantOperatingCityId.getId limit offset mbSearchString statusAwareVehicleNo DTT.IN_PROGRESS
        Just Common.TripAssigned -> RCQuery.findAllVehicleByStatusForFleetByLimitOffsetMF fleetOwnerIds merchantId merchantOperatingCityId.getId limit offset mbSearchString statusAwareVehicleNo DTT.TRIP_ASSIGNED
        Just Common.Valid -> RCQuery.findAllRCByStatusForFleetMF fleetOwnerIds (Just $ castFleetVehicleStatus mbStatus) limit offset merchantId merchantOperatingCityId.getId statusAwareVehicleNo
        Just Common.Invalid -> RCQuery.findAllRCByStatusForFleetMF fleetOwnerIds (Just $ castFleetVehicleStatus mbStatus) limit offset merchantId merchantOperatingCityId.getId statusAwareVehicleNo
...
getDriverFleetDriverOnboardedDriversAndUnlinkedVehicles merchantShortId opCity fleetOwnerId mbLimit mbOffset = do
  merchant <- findMerchantByShortId merchantShortId
  merchantOpCity <- CQMOC.findByMerchantIdAndCity merchant.id opCity >>= fromMaybeM (MerchantOperatingCityNotFound $ "merchantShortId: " <> merchantShortId.getShortId <> " ,city: " <> show opCity)
  listOfAllVehicle <- RCQuery.findAllRCByStatusForFleetMF [fleetOwnerId] Nothing (fromIntegral $ fromMaybe 100000 mbLimit) (fromIntegral $ fromMaybe 0 mbOffset) merchant.id merchantOpCity.id.getId Nothing
