...
postDriverFleetRemoveVehicle merchantShortId _ fleetOwnerId_ vehicleNo mbRequestorId = do
  ...
  -- Mark the fleet-vehicle association ended (associatedTill=now); the stale inactive row is
  -- removed from Postgres by the ClickHouse ETL, not by a hard delete here (hard deletes are
  -- invisible to Kafka/ClickHouse). Mirrors the operator/fleet-operator/fleet-driver flows.
