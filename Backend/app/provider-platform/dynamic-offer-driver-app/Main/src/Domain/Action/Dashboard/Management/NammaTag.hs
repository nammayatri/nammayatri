...
postNammaTagAppDynamicLogicVerify merchantShortId opCity req = do
  ...
  resp <- case req.domain of
    LYT.POOLING -> do
      ...
      YudhishthiraFlow.verifyAndUpdateDynamicLogic mbMerchantId (cast merchantOpCityId) (Proxy :: Proxy TaggedDriverPoolInput) transporterConfig.referralLinkPassword req (TaggedDriverPoolInput driversData False 0 (Just 0) Nothing Nothing Nothing Nothing)
    ...
    LYT.DRIVER_CONFIG LYT.IncentiveJourneyConfigDriver -> do
    ...
    LYT.DRIVER_CONFIG LYT.IncentiveJourneyMilestoneConfigDriver -> do
...
postNammaTagConfigPilotGetConfigWithDimensions _merchantShortId _opCity req = do
  ...
  case req.configType of
    ...
    LYT.CoinsConfig -> do
      ...
      pure LYT.TableDataResp {configs = map A.toJSON cfgs}
    LYT.IncentiveJourneyConfigDriver -> do
      cfgs <- getConfig (IncentiveJourneyDimensions {merchantOperatingCityId = mocId, merchantId = dimLookup "merchantId" dims, journeyId = dimLookup "journeyId" dims, enabled = dimLookup "enabled" dims, journeyType = dimLookup "journeyType" dims}) (Just (SQIJ.findByMerchantOperatingCityId Nothing Nothing (cast merchantOpCityId)))
      pure LYT.TableDataResp {configs = map A.toJSON cfgs}
    LYT.IncentiveJourneyMilestoneConfigDriver -> do
...
getNammaTagConfigPilotGetDimensionSchema _merchantShortId _opCity configType =
  case configType of
    ...
    LYT.IncentiveJourneyConfigDriver -> pure $ mkDimSchema (Proxy @IncentiveJourneyDimensions)
    LYT.IncentiveJourneyMilestoneConfigDriver -> pure $ mkDimSchema (Proxy @IncentiveJourneyMilestoneDimensions)
...
postNammaTagConfigPilotCreateRow _merchantShortId _opCity req = do
  ...
  case req.configType of
    ...
    LYT.IncentiveJourneyConfigDriver -> do
      ...
      SQIJ.create cfg
      invalidateConfigInMem LYT.IncentiveJourneyConfigDriver
    LYT.IncentiveJourneyMilestoneConfigDriver -> do
      cfg :: DIJM.IncentiveJourneyMilestone <- parseConfigData req.configData
      SQIJM.create cfg
      invalidateConfigInMem LYT.IncentiveJourneyMilestoneConfigDriver
