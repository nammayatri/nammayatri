        ...
        Lib.Yudhishthira.Types.ApplicationTag Lib.Yudhishthira.Types.NammaTagApplication {..} -> do
          case tagRule of
            C.RuleEngine rule -> do
              events <-
                tagStages <|> (NE.singleton <$> tagStage)
                  & fromMaybeM (InvalidRequest "At least one tagStage should be provided for application tag")
              unless (length events == length (NE.nub events)) $
                throwError (InvalidRequest "Tag stages should be unique")
              createTagResponses <- forM events \event -> do
                ...
                case defVal of
                  ...
                  Just defaultVal -> do
                    ...
                    pure $ Lib.Yudhishthira.Types.ApplicationTagRes (Lib.Yudhishthira.Types.CreateNammaApplicationTagResponse result defaultVal)
              pure $ Lib.Yudhishthira.Types.CreateNammaTagResponse {result = NE.head createTagResponses, results = createTagResponses}
            _ -> throwError $ InvalidRequest "LLMContext not supported yet"
        _ -> pure $ Lib.Yudhishthira.Types.CreateNammaTagResponse {result = Lib.Yudhishthira.Types.Success, results = NE.singleton Lib.Yudhishthira.Types.Success}
