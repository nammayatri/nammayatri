...
handler merchant req validatedQuote = do
  ...
  where
    handleDynamicOfferFlow isNewRider driver driverQuote booking riderDetails = do
      ...
      (ride, _, vehicle) <- initializeRide merchant driver uBooking Nothing (Just req.enableFrequentLocationUpdates) driverQuote.clientId
...
validateRequest subscriber transporterId req now = do
  ...
  case booking.tripCategory of
    ...
    DTC.Ambulance DTC.OnDemandStaticOffer -> getStaticQuoteDetails booking transporter
    DTC.Ambulance DTC.RideOtp -> getRideOtpQuoteDetails booking transporter
  where
