...
import qualified "dashboard-helper-api" Dashboard.ProviderPlatform.Ride as DCommon
  ...
  where
    ...
    processIssueReport iConfig currTime identifier_ language iReport = do
      ...
      if iReport.status == Domain.AWAIT && timeDiff > iConfig.autoMarkIssueResolveDuration
        then do
          QIR.updateStatusAssignee iReport.id (Just Domain.RESOLVED) iReport.assignee
          ...
          mkIssueReport iReport (Just Domain.RESOLVED) language
...
issueMediaUpload (personId, merchantId) Common.IssueMediaUploadReq {..} = do
  ...
  transporterConfig <- CQTC.findByMerchantId (cast merchantId) >>= fromMaybeM (TransporterConfigNotFound merchantId.getId)
...
createIssueReport (personId, merchantId) mbLanguage Common.IssueReportReq {..} identifier = do
  ...
  where
    ...
    mkLocation DCommon.LocationAPIEntity {..} = TIT.Location {..}
...
issueInfo issueReportId (personId, _) mbLanguage identifier = do
  ...
  where
    ...
    mediaTypeToMessageType :: D.MediaType -> Domain.MessageType
