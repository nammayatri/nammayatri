...
makeSearchRequestForDriverAPIEntity :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => SearchRequestForDriver -> DSR.SearchRequest -> DST.SearchTry -> Maybe DSM.BapMetadata -> Seconds -> Maybe HighPrecMoney -> Seconds -> DVST.ServiceTierType -> Bool -> Bool -> Bool -> Maybe HighPrecMoney -> Maybe HighPrecMoney -> m SearchRequestForDriverAPIEntity
makeSearchRequestForDriverAPIEntity nearbyReq searchRequest searchTry bapMetadata delayDuration mbDriverDefaultExtraForSpecialLocation keepHiddenForSeconds requestedVehicleServiceTier isTranslated isValueAddNP useSilentFCMForForwardBatch driverPickUpCharges parkingCharge = do
  isFavourite <-
    case searchRequest.riderId of
      Just riderId -> do
        mbCorrelationRes <- findByRiderIdAndDriverId riderId nearbyReq.driverId
        return (mbCorrelationRes <&> (.favourite))
      _ -> pure Nothing
