...
-- DB: 1 SELECT (active siblings) + 1 bulk UPDATE; BAP notifications fan out concurrently.
...
postEditResult ctx@(_, _, _) bookingUpdateReqId req = do
  ...
  ( case bookingUpdateReq.updateType of
      Just DBUR.STOPS -> do
        ...
        postEditResultInner ctx bookingUpdateReqId req `finally` void (Redis.unlockRedis stopKey)
      _ ->
        postEditResultInner ctx bookingUpdateReqId req
    )
...
postEditResultInner (mbPersonId, _, _) bookingUpdateReqId EditBookingRespondAPIReq {..} = do
      ...
      if action == ACCEPT
        then do
          ...
          do
            ...
            case bookingUpdateReq.updateType of
              --   Just DBUR.STOPS -> Notify.notifyEditStops ride.merchantOperatingCityId driverId driver.deviceToken
              _ -> Notify.notifyEditDestination ride.merchantOperatingCityId driverId driver.deviceToken
            return Success
        else do
