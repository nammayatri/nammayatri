...
verify authId mbFleet fleetOwnerId req = do
  ...
  when mbFleet $ do
    ...
    assoc <- FDV.makeFleetDriverAssociation res.person.id fleetOwnerId Nothing (DomainRC.convertTextToUTC (Just "2099-12-12")) -- TODO , to be handled by new Fleet RBAC
    QFDV.create assoc
  pure Success
