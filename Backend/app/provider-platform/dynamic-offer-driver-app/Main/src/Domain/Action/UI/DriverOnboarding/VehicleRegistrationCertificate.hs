...
verifyRCFlow person merchantOpCityId imageExtraction rcNumber imageId dateOfRegistration multipleRC mbVariant = do
  ...
  verifyRes <-
    Verification.verifyRCAsync person.merchantId merchantOpCityId $
...
onVerifyRC verificationReq output = do
  ...
  if verificationReq.imageExtractionValidation == Domain.Skipped
    ...
    then IVQuery.updateExtractValidationStatus verificationReq.requestId Domain.Failed >> return Ack
      ...
      now <- getCurrentTime
      id <- generateGUID
      rCConfigs <- SCO.findByMerchantOpCityIdAndDocumentType verificationReq.merchantOperatingCityId ODC.RC >>= fromMaybeM (OnboardingDocumentConfigNotFound person.merchantId.getId (show ODC.RC))
      rCInsuranceConfigs <- SCO.findByMerchantOpCityIdAndDocumentType verificationReq.merchantOperatingCityId ODC.RCInsurance >>= fromMaybeM (OnboardingDocumentConfigNotFound person.merchantId.getId (show ODC.RCInsurance))
