...
verifyDocumentImageMatch ::
  ...
  Maybe Text ->
  Flow ()
verifyDocumentImageMatch person merchantOpCityId verifyingDocumentType mbImageId1 DriverDocument {..} mbProfileImage = do
  case verifyingDocumentType of
    ODC.AadhaarCard -> do
      imageId1 <- fromMaybeM (InvalidRequest $ "ImageId is required for " <> show verifyingDocumentType) mbImageId1
      image1 <- getImageByPersonIdAndImageId person.id imageId1 verifyingDocumentType
      profilePhoto <- getPersonImageByDocType person.id ODC.ProfilePhoto Image.APPROVED

      profileChecked <-
        maybe
          (pure False)
          (\image2 -> isFaceCompareValid person merchantOpCityId image1 image2)
          profilePhoto
