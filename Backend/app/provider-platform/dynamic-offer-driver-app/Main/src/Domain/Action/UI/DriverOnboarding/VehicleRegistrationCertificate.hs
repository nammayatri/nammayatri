  ...
  if verificationReq.imageExtractionValidation == Domain.Skipped
    ...
    then IVQuery.updateExtractValidationStatus verificationReq.requestId Domain.Failed >> return Ack
      ...
      fleetOwnerId <- case output.registration_number of
        ...
        _ -> return Nothing
      let mVehicleRC = createRC rCConfigs rCInsuranceConfigs output id verificationReq.documentImageId1 now verificationReq.dashboardPassedVehicleVariant <$> mEncryptedRC <*> mbFitnessEpiry <*> fleetOwnerId
      case mVehicleRC of
        Just vehicleRC -> do
...
createRC rcconfigs rcInsurenceConfigs output id imageId now mbVariant edl expiry mbFleetOwnerId = do
