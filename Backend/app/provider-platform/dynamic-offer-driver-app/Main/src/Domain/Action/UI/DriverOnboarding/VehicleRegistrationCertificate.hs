...
verifyPan isDashboard mbMerchant (personId, _, merchantOpCityId) req = do
  person <- Person.findById personId >>= fromMaybeM (PersonNotFound personId.getId)
  --transporterConfig <- SCTC.findByMerchantOpCityId merchantOpCityId (Just (DriverId (cast person.id.g))) >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
  -- documentVerificationConfig <- QODC.findByMerchantOpCityIdAndDocumentTypeAndCategory merchantOpCityId DTO.PanCard (fromMaybe CAR req.vehicleCategory) >>= fromMaybeM (DocumentVerificationConfigNotFound merchantOpCityId.getId (show DTO.DriverLicense))
  blocked <- case person.role of
    Person.FLEET_OWNER -> do
      res <- FOI.findByPrimaryKey person.id >>= fromMaybeM (PersonNotFound personId.getId)
      return res.blocked
      ...
      case resp.extractedPan of
        Just extractedPan -> do
          ...
          unless (extractedPanNo == Just req.panNumber) $
            throwError $ InvalidRequest "Inavlid Image"
          DPQuery.updateVerificationStatus Documents.VALID person.id
        Nothing -> throwError $ InvalidRequest "Invalid Image"
    Nothing -> do
      image1 <- getImage req.imageId
      resp <-
        Verification.extractPanImage person.merchantId merchantOpCityId $
