...
getInformation (personId, merchantId, merchantOpCityId) mbToss = do
  ...
  makeDriverInformationRes merchantOpCityId driverEntity organization driverReferralCode driverStats driverGoHomeInfo (Just currentDues) (Just manualDues) mbMd5Digest
...
activateGoHomeFeature (driverId, _merchantId, merchantOpCityId) driverHomeLocationId driverLocation = do
  goHomeConfig <- CQGHC.findByMerchantOpCityId merchantOpCityId (Just driverId)
...
deactivateGoHomeFeature (personId, _, merchantOpCityId) = do
  goHomeConfig <- CQGHC.findByMerchantOpCityId merchantOpCityId (Just personId)
  unless (goHomeConfig.enableGoHome) $ throwError GoHomeFeaturePermanentlyDisabled
  let driverId = cast personId
  driverInfo <- QDriverInformation.findById driverId >>= fromMaybeM DriverInfoNotFound
  unless driverInfo.enabled $ throwError DriverAccountDisabled
...
addHomeLocation (driverId, merchantId, merchantOpCityId) req = do
  cfg <- CQGHC.findByMerchantOpCityId merchantOpCityId (Just driverId)
...
buildDriverEntityRes (person, driverInfo) = do
  transporterConfig <- CQTC.findByMerchantOpCityId person.merchantOperatingCityId (Just person.id) >>= fromMaybeM (TransporterConfigNotFound person.merchantOperatingCityId.getId)
...
getNearbySearchRequests (driverId, _, merchantOpCityId) = do
  ...
  transporterConfig <- CQTC.findByMerchantOpCityId merchantOpCityId (Just driverId) >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
    ...
    driverFCMPulledList <-
      case req.response of
        ...
        Accept -> do
          ...
          farePolicy <- getFarePolicy merchantOpCityId sReqFD.vehicleVariant searchReq.area (Just searchReq.transactionId)
...
getDriverPaymentsHistoryV2 (driverId, _, merchantOpCityId) mPaymentMode mbLimit mbOffset = do
  ...
  transporterConfig <- CQTC.findByMerchantOpCityId merchantOpCityId (Just driverId) >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId) -- check if there is error type already for this
...
getHistoryEntryDetailsEntityV2 (driverId, _, merchantOpCityId) invoiceShortId = do
