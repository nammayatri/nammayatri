...
updateDriver (personId, _, merchantOpCityId) mbBundleVersion mbClientVersion mbConfigVersion mbDevice req = do
  ...
  whenJust mVehicle $ \vehicle -> do
    when (isJust req.canDowngradeToSedan || isJust req.canDowngradeToHatchback || isJust req.canDowngradeToTaxi || isJust req.canSwitchToRental || isJust req.canSwitchToInterCity || isJust req.isPetModeEnabled || isJust req.tripDistanceMaxThreshold || isJust req.tripDistanceMinThreshold || isJust req.maxPickupRadius || isJust req.isSilentModeEnabled || isJust req.rideRequestVolume) $ do
      ...
      QDriverInformation.updateDriverInformation canDowngradeToSedan canDowngradeToHatchback canDowngradeToTaxi canSwitchToRental canSwitchToInterCity canSwitchToIntraCity availableUpiApps isPetModeEnabled tripDistanceMaxThreshold tripDistanceMinThreshold maxPickupRadius isSilentModeEnabled rideRequestVolume person.id
...
respondQuote (driverId, merchantId, merchantOpCityId) clientId mbBundleVersion mbClientVersion mbConfigVersion mbDevice req = do
  ...
  case req.response of
        ...
        if lockRespondQuote && lockEditDestination
          then do
            ...
            driverFCMPulledList <- case DTC.tripCategoryToPricingPolicy searchTry.tripCategory of
              DTC.EstimateBased _ -> acceptDynamicOfferDriverRequest merchant searchTry searchReq driver sReqFD mbBundleVersion mbClientVersion mbConfigVersion mbDevice reqOfferedValue driverStats
            ...
            when (transporterConfig.allowAnalytics == Just True) $ Analytics.updateOperatorAnalyticsAcceptationAndTotalRequestCount driverId (Just acceptationCount) (Just totalRequestCount)
    ...
    Reject -> do
      ...
      when (transporterConfig.allowAnalytics == Just True) $ Analytics.updateOperatorAnalyticsAcceptationAndTotalRequestCount driverId Nothing totalRequestCount
  ...
  where
    ...
    acceptDynamicOfferDriverRequest :: DM.Merchant -> DST.SearchTry -> DSR.SearchRequest -> SP.Person -> SearchRequestForDriver -> Maybe Version -> Maybe Version -> Maybe Version -> Maybe Text -> Maybe HighPrecMoney -> DStats.DriverStats -> Flow [SearchRequestForDriver]
    acceptDynamicOfferDriverRequest merchant searchTry searchReq driver sReqFD mbBundleVersion' mbClientVersion' mbConfigVersion' mbDevice' reqOfferedValue driverStats = do
