...
clearDriverDues (personId, _merchantId, opCityId) serviceName mbDeepLinkData = do
  ...
  case sortedInvoices of
    [] -> do mkClearDuesResp <$> SPayment.createOrder (personId, _merchantId, opCityId) paymentService (dueDriverFees, []) Nothing INV.MANUAL_INVOICE Nothing vendorFees mbDeepLinkData
