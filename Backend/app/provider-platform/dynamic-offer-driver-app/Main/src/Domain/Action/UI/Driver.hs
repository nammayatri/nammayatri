...
import qualified SharedLogic.Allocator.Jobs.SendSearchRequestToDrivers as SSRD
...
import SharedLogic.CallBAP (sendRideAssignedUpdateToBAP)
...
respondQuote (driverId, merchantId, merchantOpCityId) clientId mbBundleVersion mbClientVersion mbConfigVersion mbReactBundleVersion mbDevice req = do
  ...
  where
    ...
    tryTopUpOrAdvanceBatchEarly searchTry sReqFD batchRejectCount = do
      mbSentCount <- DP.getBatchSentCount searchTry.id sReqFD.batchNumber
      hasReserve <- DP.hasDriverReserveList searchTry.id
      let isFullReject = maybe False (\sentCount -> sentCount > 0 && batchRejectCount >= sentCount) mbSentCount
      when (isFullReject || hasReserve) $ do
        searchReq <- QSR.findById searchTry.requestId >>= fromMaybeM (SearchRequestNotFound searchTry.requestId.getId)
        driverPoolConfig <-
          SCDPC.getDriverPoolConfig
            ...
            searchReq
        if DP.isContinuousBatchingEnabled driverPoolConfig
          then do
            logInfo $
              "ContinuousBatchingTopUp: searchTryId=" <> searchTry.id.getId
                <> " batchNum="
                <> show sReqFD.batchNumber
            void $
              SSRD.processSendSearchRequestJob ("inline-topup-" <> searchTry.id.getId) $
            ...
            when (isFullReject && isEnabled && hasBatchesLeft && batchOldEnough) $ do
    ...
    then pure $ ScheduledBookingRes []
      ...
      case (mbFromDay, mbToDay, mbDLoc') of
        (Just from, Just to, Just dLoc) -> do
          ...
          -- no commitment -> full board; at hold cap -> empty; else filter per-booking feasibility vs committed set
          mbCommittedForBoard <-
            if isNothing driverInfo.latestScheduledBooking && not driverInfo.onRide
              then pure (Just [])
              else do
              ...
              feasibleBookings <-
                if null committed
                  then pure (catMaybes scheduledBookings)
                    ...
                    let committedIntervals = SBOC.mkCommittedIntervals now Nothing committed
                        isBookingFeasibleForBoard scheduledBooking =
                          either (const False) identity
                            <$> withTryCatch "boardScheduledOverlap" (SBOC.isCandidateFeasible driver.merchantId cityId transporterConfig (SBOC.mkCandidateFromBooking scheduledBooking) committedIntervals)
                    filterM isBookingFeasibleForBoard (catMaybes scheduledBookings)
              bookings <- mapM (buildBookingAPIEntityFromBooking mbDLoc) (filter isBookingVisibleToDriver feasibleBookings)
    ...
    when payoutVpaValid $ do
      ...
      (_, mbPayoutOrder) <- do
        if createPayoutOrderReq.amount > 0.0
          then DPayment.createPayoutService (cast person.merchantId) (Just $ cast person.merchantOperatingCityId) (cast personId) (Just $ map ((.getId) . (.id)) driverFeeToPayout) (Just entityName) (show merchantOperatingCity.city) createPayoutOrderReq createPayoutOrderCall Nothing (Just afterPayoutOrderCreated)
