...
validate (personId, _, merchantOpCityId) phoneNumber = do
  ...
  whenNothing_ useFakeOtpM $ do
    ...
    withLogTag ("personId_" <> getId person.id) $ do
      ...
      Sms.sendSMS person.merchantId merchantOpCityId (Sms.SendSMSReq message altPhoneNumber sender (Just templateId))
        >>= Sms.checkSmsResult
  let verified = False
...
resendOtp (personId, merchantId, merchantOpCityId) req = do
  ...
  withLogTag ("personId_" <> getId personId) $ do
    ...
    let sender = fromMaybe smsCfg.sender mbSender
    Sms.sendSMS merchantId merchantOpCityId (Sms.SendSMSReq message altphoneNumber sender (Just templateId))
      >>= Sms.checkSmsResult
