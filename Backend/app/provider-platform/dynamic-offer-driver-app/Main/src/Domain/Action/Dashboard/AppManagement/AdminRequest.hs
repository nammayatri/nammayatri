  ...
  Redis.withWaitOnLockRedisWithExpiry (SFW.makeWalletRunningBalanceLockKey adminRequest.personId.getId) 10 10 $ do
    ...
    additionalAdjustments <- case adjustmentReferenceType of
      BookingReference bookingReferenceType -> do
        ...
        case bookingReferenceType of
          BaseRide -> rideRelatedAdjustment ledgerAdjustmentParams
          CustomerCancellationCharges -> userCancellationRelatedAdjustment ledgerAdjustmentParams
          DriverCancellationCharges -> driverCancellationRelatedAdjustment ledgerAdjustmentParams
...
rideRelatedAdjustment :: FinanceFlow m r => LedgerAdjustmentParams -> m [(Text, Maybe HighPrecMoney)]
rideRelatedAdjustment LedgerAdjustmentParams {..} = do
  ...
  let mbParkingAmount = Nothing -- TODO add later
      isOnline = True
  ctx <- SFW.buildFinanceCtx booking ride (Just driver) mbPanCard (Just driverInfo) transporterConfig isOnline
...
userCancellationRelatedAdjustment :: FinanceFlow m r => LedgerAdjustmentParams -> m [(Text, Maybe HighPrecMoney)]
userCancellationRelatedAdjustment LedgerAdjustmentParams {..} = do
...
driverCancellationRelatedAdjustment :: FinanceFlow m r => LedgerAdjustmentParams -> m [(Text, Maybe HighPrecMoney)]
driverCancellationRelatedAdjustment LedgerAdjustmentParams {..} = do
  let penaltyAdjustment = adminRequestAmount
      isOnline = True
  ctx <- SFW.buildFinanceCtx booking ride (Just driver) mbPanCard (Just driverInfo) transporterConfig isOnline
  result <- Finance.runFinance ctx $ do
    -- Note: Here Credit means increase driver balance, and hence reduce driver penalty (vice versa for Debit)
    adjustment adjustmentType Finance.OwnerRevenue Finance.OwnerLiability penaltyAdjustment SFW.walletReferenceDriverCancellationCharges
  case result of
