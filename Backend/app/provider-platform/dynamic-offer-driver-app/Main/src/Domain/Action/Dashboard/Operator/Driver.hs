...
postDriverOperatorVerifyJoiningOtp merchantShortId opCity mbAuthId requestorId req = do
  ...
  case mbAuthId of
    Just authId -> do
      ...
      verifyAndAssociateDriverWithOperator merchant merchantOpCityId operator res.person (transporterConfig.allowCacheDriverFlowStatus == Just True) (transporterConfig.allowAnalytics == Just True)
    ...
    Nothing -> do
      ...
      verifyAndAssociateDriverWithOperator merchant merchantOpCityId operator person (transporterConfig.allowCacheDriverFlowStatus == Just True) (transporterConfig.allowAnalytics == Just True)

  pure Success
  where
    verifyAndAssociateDriverWithOperator merchant merchantOpCityId operator person allowCache allowAnalytics = do
      checkAssocOperator <-
        B.runInReplica $
      ...
      QDOA.create assoc
      when allowAnalytics $ Analytics.incrementOperatorAnalyticsApplicationCount operator.id.getId
      when allowCache $ do
      ...
      (totalApplicationCount, driverEnabled, greaterThanOneRide, greaterThanTenRide, greaterThanFiftyRide) <- do
        if all isJust mbPeriodKeysRes
          then do
            ...
            res <- Analytics.handleCacheMissForOperatorAnalyticsPeriod operator.id.getId periodKeysData period fromDay toDay
            logTagInfo "PeriodFallbackRes" (show res)
            pure (res.totalApplicationCount, res.driverEnabled, res.greaterThanOneRide, res.greaterThanTenRide, res.greaterThanFiftyRide)
