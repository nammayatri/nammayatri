...
---------------------------------------------------------------------
merchantUpdate :: ShortId DM.Merchant -> City.City -> Common.MerchantUpdateReq -> Flow Common.MerchantUpdateRes
...
---------------------------------------------------------------------
merchantCommonConfig :: ShortId DM.Merchant -> City.City -> Flow Common.MerchantCommonConfigRes
...
---------------------------------------------------------------------
merchantCommonConfigUpdate :: ShortId DM.Merchant -> City.City -> Common.MerchantCommonConfigUpdateReq -> Flow APISuccess
...
---------------------------------------------------------------------
driverPoolConfig :: ShortId DM.Merchant -> City.City -> Maybe Meters -> Flow Common.DriverPoolConfigRes
...
driverPoolConfigUpdate ::
  ...
  Flow APISuccess
driverPoolConfigUpdate merchantShortId opCity tripDistance req = do
  runRequestValidation Common.validateDriverPoolConfigUpdateReq req
  merchant <- findMerchantByShortId merchantShortId
  merchantOpCityId <- CQMOC.getMerchantOpCityId Nothing merchant (Just opCity)
  config <- CQDPC.findByMerchantOpCityIdAndTripDistance merchantOpCityId tripDistance >>= fromMaybeM (DriverPoolConfigDoesNotExist merchant.id.getId tripDistance)
  let updConfig =
...
driverPoolConfigCreate ::
  ...
  Flow APISuccess
driverPoolConfigCreate merchantShortId opCity tripDistance req = do
  runRequestValidation Common.validateDriverPoolConfigCreateReq req
  merchant <- findMerchantByShortId merchantShortId
  merchantOpCityId <- CQMOC.getMerchantOpCityId Nothing merchant (Just opCity)
  mbConfig <- CQDPC.findByMerchantOpCityIdAndTripDistance merchantOpCityId tripDistance
  whenJust mbConfig $ \_ -> throwError (DriverPoolConfigAlreadyExists merchant.id.getId tripDistance)
  newConfig <- buildDriverPoolConfig merchant.id merchantOpCityId tripDistance req
...
buildDriverPoolConfig merchantId merchantOpCityId tripDistance Common.DriverPoolConfigCreateReq {..} = do
  now <- getCurrentTime
  pure
...
---------------------------------------------------------------------
driverIntelligentPoolConfig :: ShortId DM.Merchant -> City.City -> Flow Common.DriverIntelligentPoolConfigRes
driverIntelligentPoolConfig merchantShortId opCity = do
  merchant <- findMerchantByShortId merchantShortId
  merchantOpCityId <- CQMOC.getMerchantOpCityId Nothing merchant (Just opCity)
  config <- CQDIPC.findByMerchantOpCityId merchantOpCityId >>= fromMaybeM (DriverIntelligentPoolConfigNotFound merchant.id.getId)
...
---------------------------------------------------------------------
onboardingDocumentConfig :: ShortId DM.Merchant -> City.City -> Maybe Common.DocumentType -> Flow Common.OnboardingDocumentConfigRes
...
serviceUsageConfig merchantShortId opCity = do
  merchant <- findMerchantByShortId merchantShortId
  merchantOpCityId <- CQMOC.getMerchantOpCityId Nothing merchant (Just opCity)
  config <- CQMSUC.findByMerchantOpCityId merchantOpCityId >>= fromMaybeM (MerchantServiceUsageConfigNotFound merchant.id.getId)
...
createFPDriverExtraFee :: ShortId DM.Merchant -> City.City -> Id FarePolicy.FarePolicy -> Meters -> Common.CreateFPDriverExtraFeeReq -> Flow APISuccess
...
---------------------------------------------------------------------
updateFPDriverExtraFee :: ShortId DM.Merchant -> City.City -> Id FarePolicy.FarePolicy -> Meters -> Common.CreateFPDriverExtraFeeReq -> Flow APISuccess
