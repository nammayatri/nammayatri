...
getIsInterCity merchantId apiKey IsIntercityReq {..} = do
  ...
  let bapCity = nearestOperatingCity.city
  merchantOpCity <- CQMOC.getMerchantOpCity merchant (Just bapCity)
  transporterConfig <- getConfig (TransporterDimensions {merchantOperatingCityId = merchantOpCity.id.getId}) >>= fromMaybeM (TransporterConfigDoesNotExist merchantOpCity.id.getId)
