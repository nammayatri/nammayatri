...
handler ValidatedDSearchReq {..} sReq = do
  ...
  allFarePoliciesProduct <- combineFarePoliciesProducts <$> ((getAllFarePoliciesProduct merchant.id merchantOpCityId sReq.isDashboardRequest sReq.pickupLocation sReq.dropLocation (Just (TransactionId (Id sReq.transactionId))) fromLocGeohash mbDistance mbDuration) `mapM` possibleTripOption.tripCategories)
...
buildSearchRequest ::
  ...
  Maybe Text ->
  m DSR.SearchRequest
buildSearchRequest DSearchReq {..} bapCity mbSpecialZoneGateId mbDefaultDriverExtra startTime isScheduled providerId merchantOpCityId fromLocation mbToLocation mbDistance mbDuration specialLocationTag area tollCharges tollNames isCustomerPrefferedSearchRoute isBlockedRoute currency distanceUnit fromLocGeohash = do
  uuid <- generateGUID
  now <- getCurrentTime
  ...
  pure
    DSR.SearchRequest
      { id = Id uuid,
        ...
        dpVersion = Nothing, -------------needs to be fixed RITIKA
        ..
      }
