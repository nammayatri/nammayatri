...
handler ValidatedDSearchReq {..} sReq = do
  ...
  where
    ...
    addNammaTags tagData = do
      newSearchTags <- withTryCatch "computeNammaTags:Search" (LYDL.computeNammaTagsWithDebugLog LYDL.Driver (cast tagData.searchRequest.merchantOperatingCityId) (cast tagData.searchRequest.merchantOperatingCityId) Yudhishthira.Search tagData)
