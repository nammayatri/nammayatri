        ...
        listOfSpecialZoneQuotes <- case org.farePolicyType of
          SLAB -> do
            ...
            for listOfVehicleVariants $ \slabFarePolicy -> do
              fareParams <- calculateFare org.id (Right slabFarePolicy) result.distance sReq.pickupTime Nothing Nothing 0 Nothing Nothing Nothing
          ...
          NORMAL -> do
            ...
            for listOfVehicleVariants $ \farePolicy -> do
              fareParams <- calculateFare org.id (Left farePolicy) result.distance sReq.pickupTime Nothing Nothing 0 Nothing Nothing Nothing
              buildSpecialZoneQuote
                searchRequestSpecialZone
                fareParams
                org.id
