...
handler merchant sReq searchReq estimates = do
  now <- getCurrentTime
  mbRiderId <- case sReq.customerPhoneNum of
    Just number -> do
  ...
  let searchReq' = searchReq {DSR.isAdvanceBookingEnabled = sReq.isAdvancedBookingEnabled, DSR.riderId = mbRiderId}
