...
handler merchant sReq searchReq estimates = do
  ...
  QSR.updateSafetyPlus sReq.peferSafetyPlus searchReq.id
  tripQuoteDetails <-
  ...
  let searchReq' = searchReq {DSR.isAdvanceBookingEnabled = sReq.isAdvancedBookingEnabled, DSR.riderId = riderId, DSR.peferSafetyPlus = sReq.peferSafetyPlus}
  ...
  where
    ...
    filterChargesByApplicability conditionalCharges = do
      let safetyCharges = if sReq.peferSafetyPlus then find (\ac -> (ac.chargeCategory) == DAC.SAFETY_PLUS_CHARGES) conditionalCharges else Nothing
