...
applyNegotiatedFare merchantOpCityId quoteId sReq =
  ...
  where
    ...
    -- A /select carrying no add-ons keeps the ones an earlier /select persisted, rather than blanking them (and their charge).
    computeAddOnData quote
      | Kernel.Prelude.null sReq.addOns = pure []
