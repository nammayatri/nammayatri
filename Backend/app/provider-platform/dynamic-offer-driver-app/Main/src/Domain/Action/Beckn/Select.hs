...
handler merchantId sReq = do
  ...
  (fareParams, driverExtraFare) <- case merchant.farePolicyType of
    DFareParams.SLAB -> do
      ...
      fareParams <- calculateFare merchantId (Right slabFarePolicy) distance sReq.pickupTime Nothing sReq.customerExtraFee 0 Nothing Nothing Nothing
    ...
    DFareParams.NORMAL -> do
      ...
      fareParams <- calculateFare merchantId (Left farePolicy) distance sReq.pickupTime Nothing sReq.customerExtraFee 0 Nothing Nothing Nothing
