...
handler merchant sReq searchReq estimates = do
  ...
  -- Rider-acceptance funnel counter (BPP_rider_acceptance_count). Fires once per select
  -- to keep RA = rider_acceptance / search_request bounded; OTP/special-zone rides skip
  -- Select entirely, so this captures the "normal" acceptances only. Tier is taken from
  -- the primary selected estimate (validateRequest guarantees the list is non-empty).
