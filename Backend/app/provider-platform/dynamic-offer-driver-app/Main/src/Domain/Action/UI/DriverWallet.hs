    ...
    when (createPayoutOrderReq.amount > 0.0) $ do
      ...
      whenJust mbPayoutOrder $ \payoutOrder -> do
        SDW.withDriverWalletAndFYEarningsStore driverId transporterConfig SDW.emptyDriverWalletCollection $
          mkDriverWallet walletBalance payoutOrder
...
mkDriverWallet ::
  ...
  DW.DriverWallet
mkDriverWallet walletBalance payoutOrder _lastTransaction SDW.DriverWalletCollection {..} SDW.DriverWalletInfo {..} = do
  DW.DriverWallet
    { rideId = Nothing,
      transactionType = DW.PAYOUT,
      merchantPayable = Nothing,
      driverPayable = Just (negate walletBalance),
      runningBalance = 0,
