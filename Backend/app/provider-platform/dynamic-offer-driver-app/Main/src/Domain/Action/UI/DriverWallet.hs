  ...
  where
    doWalletTopup mbP mId mocId0 r =
      do
        ...
        transporterConfig <- findByMerchantOpCityId mocId0 Nothing >>= fromMaybeM (TransporterConfigNotFound mocId0.getId)
...
recordAirportCashRecharge ::
  ...
  m ()
recordAirportCashRecharge (driverId, merchantId, mocId) amount currency referenceId = do
  ...
  when (null existing) $ do
    ...
    result <- runFinance ctx $ transfer PlatformAsset OwnerLiability amount walletReferenceAirportCashRecharge
