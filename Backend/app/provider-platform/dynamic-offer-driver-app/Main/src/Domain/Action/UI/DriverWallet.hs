...
postWalletPayout (mbPersonId, _, mocId) = do
  ...
  Redis.withWaitOnLockRedisWithExpiry (makeWalletRunningBalanceLockKey driverId.getId) 10 10 $ do
    ...
    let istTime = addUTCTime (secondsToNominalDiffTime transporterConfig.timeDiffFromUtc) utcTime
        today = Data.Time.utctDay istTime
    whenJust transporterConfig.maxWalletPayoutsPerDay $ \maxPayoutsPerDay -> do
      payoutsToday <- QDW.findAllByDriverIdRangeAndTransactionType driverId (Data.Time.UTCTime today 0) (Data.Time.UTCTime today 86399) (Just DW.PAYOUT) (maxPayoutsPerDay + 1) 0
