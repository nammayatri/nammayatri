...
postPayoutCreateOrder (mbPersonId, merchantId, merchantOpCityId) req = do
  ...
  void $ Payout.createPayoutService (Kernel.Types.Id.cast merchantId) (Kernel.Types.Id.cast personId) (Just [personId.getId]) (Just entityName) Nothing (show merchantOperatingCity.city) req createPayoutOrderCall
  pure Kernel.Types.APISuccess.Success
