...
approveAndUpdateAadhaar req mId mOpCityId = do
  ...
  if SDO.isFullAadhaar req.aadhaarNumber
    then when (transporterConfig.allowDuplicateAadhaar == Just False) $ do
      ...
      conflicts <- QAadhaarCard.findActiveConflictsByAadhaarHash person.merchantId person.id (Just aadhaarHash)
  ...
  now <- getCurrentTime
  let maskedNumber = SDO.maskAadhaarNumber req.aadhaarNumber
  case mbAadhaar of
    Just aadhaar -> do
      let updatedAadhaar =
            aadhaar
