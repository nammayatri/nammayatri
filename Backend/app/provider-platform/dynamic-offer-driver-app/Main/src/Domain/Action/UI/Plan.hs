...
class Subscription a where
  getSubcriptionStatusWithPlan :: KvDbFlow m r => a -> Id SP.Person -> m (Maybe DI.DriverAutoPayStatus, Maybe DriverPlan)
  updateSubscriptionStatus :: KvDbFlow m r => a -> (Id SP.Person, Id DM.Merchant, Id DMOC.MerchantOperatingCity) -> Maybe DI.DriverAutoPayStatus -> Maybe Text -> m ()
