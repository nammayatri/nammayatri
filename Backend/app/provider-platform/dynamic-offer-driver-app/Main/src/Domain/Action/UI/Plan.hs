...
convertPlanToPlanEntity driverId applicationDate isCurrentPlanEntity driverPlan subscriptionConfig serviceNameParam plan@Plan {..} = do
  ...
  transporterConfig_ <- getOneConfig (TransporterConfigDimensions {merchantOperatingCityId = merchantOpCityId.getId}) Nothing >>= fromMaybeM (TransporterConfigNotFound merchantOpCityId.getId)
  currency <- SMerchant.getCurrencyByMerchantOpCity merchantOpCityId
...
fareCategories planId mbPlanName (driverId, _merchantId, merchantOpCityId) = do
  -- name is an optional QueryParam on the route -- when the caller has it (the common case, since
  -- the plan list/detail screens already carry it), pin the exact row for a clearer 404 on
  -- mismatch; when omitted, planId alone is enough (id is not globally unique across
  -- PaymentMode, but fare-category data doesn't vary by that, so any matching row works).
  plan <- case mbPlanName of
    Just planName -> QPlanExtra.findByIdAndName planId planName >>= fromMaybeM (PlanNotFound planId.getId)
    Nothing -> QPlanExtra.findById planId >>= fromMaybeM (PlanNotFound planId.getId)
  now <- getCurrentTime
          ...
          then Nothing
            ...
            Just $
              DL.sortOn (.minDistanceMeters) $
                map
                  ( \s ->
                      -- GST-inclusive, matching how FareCalculator actually charges this tier
                      -- (platformFee + platformFee * cgstPercentage + platformFee * sgstPercentage)
                      -- -- keeps this consistent with the flatAmt fallback above, which also
                      -- includes cgst/sgst.
                      let tierAmount = s.platformFee + s.platformFee * s.cgstPercentage + s.platformFee * s.sgstPercentage
                       in FareTierEntity
                            { minDistanceMeters = s.minDistanceMeters,
                              maxDistanceMeters = s.maxDistanceMeters,
                              amount = tierAmount,
                              amountWithCurrency = PriceAPIEntity tierAmount currency
                            }
                  )
                  slabs
