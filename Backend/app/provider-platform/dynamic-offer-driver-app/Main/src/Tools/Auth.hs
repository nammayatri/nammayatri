...
verifyTokenAction ::
  forall m r.
  (HasEsqEnv m r, Redis.HedisFlow m r, HasField "authTokenCacheExpiry" r Seconds, MonadFlow m, Utils.KvDbFlow m r) =>
...
verifyToken :: (HasEsqEnv m r, MonadFlow m, Utils.KvDbFlow m r) => RegToken -> m SR.RegistrationToken
verifyToken regToken = do
  QR.findByToken regToken
    >>= Utils.fromMaybeM (InvalidToken regToken)
    >>= validateToken

validateAdmin :: (HasEsqEnv m r, EncFlow m r, Utils.KvDbFlow m r) => RegToken -> m Person.Person
...
verifyPerson :: (HasEsqEnv m r, Redis.HedisFlow m r, HasField "authTokenCacheExpiry" r Seconds, MonadFlow m, Utils.KvDbFlow m r) => RegToken -> m (Id Person.Person, Id Merchant.Merchant, Id DMOC.MerchantOperatingCity)
...
validateAdminAction :: forall m r. (HasEsqEnv m r, EncFlow m r, Utils.KvDbFlow m r) => VerificationAction AdminVerifyToken m
...
clearDriverSession :: (HasEsqEnv m r, Redis.HedisFlow m r, MonadFlow m, Utils.KvDbFlow m r) => Id Person.Person -> m ()
