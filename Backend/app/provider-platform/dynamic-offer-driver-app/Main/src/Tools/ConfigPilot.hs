...
handleConfigDBUpdate merchantOpCityId concludeReq baseLogics mbMerchantId opCity = do
  case concludeReq.domain of
    ...
    LYT.DRIVER_CONFIG (LYT.UiConfig dt pt) -> do
      let uiConfigReq = LYT.UiConfigRequest {os = dt, platform = pt, merchantId = if isNothing mbMerchantId then "" else getId (fromJust mbMerchantId), city = opCity, language = Nothing, bundle = Nothing, toss = Nothing}
  ...
  where
    ...
    handleConfigUpdateWithExtraDimensionsUi ::
      (MonadFlow m, FromJSON a, ToJSON a, Eq a, Show a) =>
      (LYT.UiConfigRequest -> Id MerchantOperatingCity -> m (Maybe a, Maybe Int)) -> -- Fetch function
      m () -> -- Cache clearing function
      (a -> m ()) -> -- Update function
      Id MerchantOperatingCity ->
      LYT.UiConfigRequest ->
