...
notifyPaymentFailure driverId paymentMode mbBankErrorCode serviceName = do
  ...
  dueDriverFees <- B.runInReplica $ QDF.findDriverFeeByTypeStatusAndServiceName (cast driverId) [DF.RECURRING_INVOICE, DF.RECURRING_EXECUTION_INVOICE] [DF.PAYMENT_PENDING, DF.PAYMENT_OVERDUE] serviceName
