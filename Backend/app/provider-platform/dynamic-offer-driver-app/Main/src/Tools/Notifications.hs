...
notifyOnNewSearchRequestAvailable merchantOpCityId personId mbDeviceToken entityData = do
  transporterConfig <- findByMerchantOpCityId merchantOpCityId >>= fromMaybeM (MerchantServiceUsageConfigNotFound merchantOpCityId.getId)
...
sendOverlay ::
  ...
  Value ->
  m ()
sendOverlay merchantOpCityId personId mbDeviceToken mbTitle description imageUrl okButtonText cancelButtonText actions link endPoint method reqBody = do
  transporterConfig <- findByMerchantOpCityId merchantOpCityId >>= fromMaybeM (MerchantServiceUsageConfigNotFound merchantOpCityId.getId)
  FCM.notifyPersonWithPriority transporterConfig.fcmConfig (Just FCM.HIGH) False notificationData $ FCMNotificationRecipient personId.getId mbDeviceToken
  where
