...
verifyRC _ merchantOptCityId mbRemPriorityList req = do
  ...
  where
    getOrCreateTtenToken :: (ServiceFlow m r, CoreMetrics m) => Id DMOC.MerchantOperatingCity -> m Text
      ...
      case mbToken of
        ...
        Nothing -> do
          ...
          case tokenResp of
            ...
            Right tokenResp' -> do
              now <- getCurrentTime
              let expiryTime = fromMaybe (addUTCTime (secondsToNominalDiffTime 86400) now) tokenResp'.expiresAt
