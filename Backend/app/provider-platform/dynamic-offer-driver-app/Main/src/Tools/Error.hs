...
data NegotiatedFareError
  = -- | quoteId, offered fare, minimum acceptable, maximum acceptable
    NegotiatedFareNotAcceptable Text HighPrecMoney HighPrecMoney HighPrecMoney
  deriving (Eq, Show)
...
instance IsBaseError NegotiatedFareError where
  toMessage = \case
    NegotiatedFareNotAcceptable quoteId offered minAcceptable maxAcceptable ->
      Just $ "Negotiated fare " <> show offered <> " for quoteId \"" <> quoteId <> "\" is outside the acceptable range [" <> show minAcceptable <> ", " <> show maxAcceptable <> "]."

instance IsHTTPError NegotiatedFareError where
  toErrorCode = \case
    NegotiatedFareNotAcceptable {} -> "NEGOTIATED_FARE_NOT_ACCEPTABLE" -- TODO: swap for ONDC spec numeric code once confirmed
  toHttpCode = \case
    NegotiatedFareNotAcceptable {} -> E400
