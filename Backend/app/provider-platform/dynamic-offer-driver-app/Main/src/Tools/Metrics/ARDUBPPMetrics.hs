...
observePricePerKm :: (MonadIO m, HasBPPMetrics m r) => Text -> Text -> Text -> Text -> Text -> Double -> m ()
observePricePerKm merchantId merchantOpCityId vehicleServiceTier distanceBucket stage value = do
  bmContainer <- asks (.bppMetrics)
  version <- asks (.version)
  liftIO $ P.withLabel bmContainer.pricePerKmHist (merchantId, merchantOpCityId, vehicleServiceTier, distanceBucket, version.getDeploymentVersion, stage) (`P.observe` value)

observeCongestionCharge :: (MonadIO m, HasBPPMetrics m r) => Text -> Text -> Text -> Text -> Text -> Double -> m ()
observeCongestionCharge merchantId merchantOpCityId vehicleServiceTier distanceBucket stage value = do
  bmContainer <- asks (.bppMetrics)
  version <- asks (.version)
  liftIO $ P.withLabel bmContainer.congestionChargeHist (merchantId, merchantOpCityId, vehicleServiceTier, distanceBucket, version.getDeploymentVersion, stage) (`P.observe` value)

observeRideDistanceMeters :: (MonadIO m, HasBPPMetrics m r) => Text -> Text -> Text -> Text -> Text -> Double -> m ()
observeRideDistanceMeters merchantId merchantOpCityId vehicleServiceTier distanceBucket stage value = do
  bmContainer <- asks (.bppMetrics)
  version <- asks (.version)
  liftIO $ P.withLabel bmContainer.rideDistanceHist (merchantId, merchantOpCityId, vehicleServiceTier, distanceBucket, version.getDeploymentVersion, stage) (`P.observe` value)

observePickupDistanceMeters :: (MonadIO m, HasBPPMetrics m r) => Text -> Text -> Text -> Text -> Text -> Double -> m ()
observePickupDistanceMeters merchantId merchantOpCityId vehicleServiceTier distanceBucket stage value = do
  bmContainer <- asks (.bppMetrics)
  version <- asks (.version)
  liftIO $ P.withLabel bmContainer.pickupDistanceHist (merchantId, merchantOpCityId, vehicleServiceTier, distanceBucket, version.getDeploymentVersion, stage) (`P.observe` value)
