...
getDistancesForScheduledRides ::
  ...
  GetDistancesReq a b ->
  m (GetDistancesResp a b)
getDistancesForScheduledRides = runWithServiceConfig Maps.getDistances (.getDistancesForScheduledRides)
...
snapToRoadWithFallback rectifyDistantPointsFailureUsing _merchantId merchantOperatingCityId includeRectifiedDistance entityId = Maps.snapToRoadWithFallback entityId rectifyDistantPointsFailureUsing includeRectifiedDistance handler
  where
    ...
    getConfidenceThreshold = do
      transporterConfig <- getOneConfig (TransporterConfigDimensions {merchantOperatingCityId = merchantOperatingCityId.getId}) Nothing >>= fromMaybeM (MerchantNotFound merchantOperatingCityId.getId)
      pure $ transporterConfig.snapToRoadConfidenceThreshold

    getProvidersList = do
      merchantConfig <- getOneConfig (MerchantServiceUsageConfigDimensions {merchantOperatingCityId = merchantOperatingCityId.getId}) Nothing >>= fromMaybeM (MerchantServiceUsageConfigNotFound merchantOperatingCityId.getId)
    ...
    getProviderConfig provider = do
      merchantMapsServiceConfig <-
        getOneConfig (MerchantServiceConfigDimensions {merchantOperatingCityId = merchantOperatingCityId.getId, merchantId = Nothing, serviceName = Just (DOSC.MapsService provider)}) Nothing
...
getServiceConfigForRectifyingSnapToRoadDistantPointsFailure _merchantId merchantOpCityId = do
  orgMapsConfig <- getOneConfig (MerchantServiceUsageConfigDimensions {merchantOperatingCityId = merchantOpCityId.getId}) Nothing >>= fromMaybeM (MerchantServiceUsageConfigNotFound merchantOpCityId.getId)
  orgMapsServiceConfig <-
    getOneConfig (MerchantServiceConfigDimensions {merchantOperatingCityId = merchantOpCityId.getId, merchantId = Nothing, serviceName = Just (DOSC.MapsService orgMapsConfig.rectifyDistantPointsFailure)}) Nothing
