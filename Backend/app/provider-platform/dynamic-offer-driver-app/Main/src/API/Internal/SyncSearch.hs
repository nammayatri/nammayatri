...
syncSearch merchantIdRaw mbToken mbIsShadowSearch mbParentTransactionId reqV2 = withFlowHandlerAPI $
  withTimeAPI "syncSearch" "total" $ do
    ...
    Utils.withTransactionIdLogTag transactionId $ do
      ...
      if not isFirst
        then do
          logTagInfo "SyncSearchV2 Internal Flow" $ "Duplicate search transaction, returning empty OnSearch: " <> transactionId
          pure $ Spec.OnSearchReq context Nothing Nothing
        else do
