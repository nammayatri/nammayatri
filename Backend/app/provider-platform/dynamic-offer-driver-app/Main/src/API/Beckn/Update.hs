...
update merchantId (SignatureAuthResult _ subscriber) req = withFlowHandlerBecknAPI $ do
  ...
  Utils.withTransactionIdLogTag transactionId $ do
    ...
    ( classifyForLocks dUpdateReq >>= \case
...
classifyForLocks :: DUpdate.DUpdateReq -> Flow UpdateClassification
classifyForLocks = \case
  DUpdate.UEditRideStopsReq r
    | r.status == Enums.CONFIRM_UPDATE -> do
      n <- r.preservedPrefixStops & fromMaybeM (InvalidRequest "preservedPrefixStops is required for CONFIRM_UPDATE on editRideStops")
      pure $ ConfirmEditStops n
  DUpdate.UEditLocationReq r
    | r.status == Enums.CONFIRM_UPDATE -> pure ConfirmEditDest
  _ -> pure AsyncForked
