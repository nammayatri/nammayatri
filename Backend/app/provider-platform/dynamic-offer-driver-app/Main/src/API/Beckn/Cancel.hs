        ...
        then do
          ...
          case cancelStatus of
            ...
            Just Enums.SOFT_CANCEL -> do
              mbRide <- QRide.findActiveByRBId booking.id
              (mbChargesOutcome, mbLogicVersion) <- maybe (return (Nothing, Nothing)) (\ride -> DCancel.getCancellationCharges booking ride DCT.CancellationByCustomer (DTCR.CancellationReasonCode <$> cancelRideReq.cancellationReason) Nothing) mbRide
