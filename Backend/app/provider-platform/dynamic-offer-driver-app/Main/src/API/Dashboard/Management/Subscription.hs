...
collectManualPayments :: ShortId DM.Merchant -> Context.City -> Id DP.Driver -> CollectManualPaymentsReq -> FlowHandler APISuccess
collectManualPayments merchantShortId opCity driverId req = withFlowHandlerAPI $ do
  m <- findMerchantByShortId merchantShortId
  mOCityId <- CQMOC.getMerchantOpCityId Nothing m (Just opCity)
  let dataClearManualSelectedDues =
        Driver.ClearManualSelectedDues
          { driverFeeIds = fromMaybe [] req.paymentIds,
            messageKey = Just WHATSAPP_MANUAL_PAYMENT_LINK
          }
  _ <- Driver.clearDriverDues (cast driverId, m.id, mOCityId) req.serviceName (Just dataClearManualSelectedDues) Nothing
  return Success
