...
buildRideInterpolationHandler :: Id DM.Merchant -> Id DMOC.MerchantOperatingCity -> Maybe (Id DRide.Ride) -> Bool -> Maybe Integer -> Flow (RideInterpolationHandler Person Flow)
buildRideInterpolationHandler merchantId merchantOpCityId rideId isEndRide mbBatchSize = do
  ...
  let snapToRoad' shouldRectifyDistantPointsFailure =
        if transportConfig.useWithSnapToRoadFallback
          then TMaps.snapToRoadWithFallback shouldRectifyDistantPointsFailure merchantId merchantOpCityId rideId
          else snapToRoadWithService rideId
