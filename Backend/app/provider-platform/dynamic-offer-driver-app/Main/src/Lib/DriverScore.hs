...
driverScoreEventHandler :: forall a m r. (DP.HasDriverPoolConfig r, Log m, Monad m, EventPayload a, Redis.HedisFlow m r, EsqDBFlow m r, CacheFlow m r, Show a, Typeable a) => Event a -> m ()
driverScoreEventHandler event = fork "DRIVER_SCORE_EVENT_HANDLER" do
...
instance EventPayload OnDriverAcceptingSearchRequest where
  handleEvent OnDriverAcceptingSearchRequest {..} =
...
instance EventPayload OnNewRideAssigned where
  handleEvent OnNewRideAssigned {..} = DP.incrementQuoteAcceptedCount merchantId driverId
