...
validateCancellation rideId rideStartTime initialDisToPickup cancellationDisToPickup transporterConfig cancelledBy = do
  ...
  rideInfo <- case rideId of
    Nothing -> throwError $ InternalError "RideId is not present"
    Just rideIdText -> do
  ...
  let timeOfDriverCancellation = round $ diffUTCTime now rideStartTime
  let timeOfCustomerCancellation = round $ diffUTCTime now rideStartTime
  let driverWaitingTime = round $ diffUTCTime now rideStartTime

  let logicInput =
        CancellationCoins.CancellationCoinData
          { cancelledBy = cancelledBy,
            timeOfDriverCancellation = timeOfDriverCancellation,
            timeOfCustomerCancellation = timeOfCustomerCancellation,
