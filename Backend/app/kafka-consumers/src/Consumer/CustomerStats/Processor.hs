...
import "rider-app" Storage.Queries.PersonStats as QPS
      ...
      case mbOperatingCityId of
        ...
        Just merchantOperatingCityId -> do
          (personStats, isAlreadyBackfilled) <-
            QPS.findByPersonId personId >>= \case
              Nothing -> do
                ...
                _ <- QPS.createPersonStats personData
                pure (personData, True)
              Just ps | (isNotBackfilled ps || (ps.isBackfilled /= Just True)) -> do
                personData <- getBackfillPersonStatsData personId merchantOperatingCityId
                QPS.updateRideData personData
                pure (personData, True)
                    ...
                    RideEnded -> do
                      case payload.rs of
                        DDR.COMPLETED -> do
                          ...
                          QPS.incrementCompletedRidesEventCount personId ifIsWeekend isMorningPeak isEveningPeak isWeekendPeak isAnyPeak
                        _ -> pure ()
                    BookingCancelled -> do
                      ...
                      -- Esq.runNoTransaction $ do
                      when (bookingCancellationReason.source == SBCR.ByUser) do QPS.incrementUserCancelledRidesCount personId
                      when (bookingCancellationReason.source == SBCR.ByDriver) do QPS.incrementDriverCancelledRidesCount personId
                    _ -> do
