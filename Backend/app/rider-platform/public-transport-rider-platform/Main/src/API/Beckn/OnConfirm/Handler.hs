...
handler _ onConfirmCb = withFlowHandlerAPI . withTransactionIdLogTag onConfirmCb $ do
  now <- getCurrentTime
  isExp <- maybe (pure False) (\ttl -> isTtlExpired (Just ttl) onConfirmCb.context.timestamp now) onConfirmCb.context.ttl
  if isExp
    then getTtlExpiredRes
      ...
      case onConfirmCb.contents of
        Right msg -> do
          handleOnConfirm $ mkDomainOnConfirm (Id onConfirmCb.context.transaction_id) msg
