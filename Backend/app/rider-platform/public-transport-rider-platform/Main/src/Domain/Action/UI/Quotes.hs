...
getQuotesHandler :: forall m r. EsqDBReplicaFlow m r => Id DSearch.Search -> m GetQuotesRes
getQuotesHandler searchId = do
  quoteAggregates <- runInReplica $ QQuote.findAllAggregatesBySearchId searchId (Proxy @m)
