...
buildOnSelectErrorReq :: OnSearch.SellerIdentity -> UTCTime -> Spec.Context -> Text -> Text -> Spec.OnSelectReq
buildOnSelectErrorReq self now ctx code message =
  Spec.OnSelectReq
    { onSelectReqContext = mkCallbackContext self now ctx,
      onSelectReqError =
        Just
...
mkOrder selected =
  Spec.Order
    { orderBilling = Nothing,
      ...
      orderStatus = Nothing,
      orderTags = OnSearch.operatingHoursTags selected.operatingHours,
