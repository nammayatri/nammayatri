...
buildTrackReq res = do
  ...
  context <- buildContext Context.MOBILITY Context.TRACK messageId (Just res.transactionId) res.merchant.bapId bapUrl (Just res.bppId) (Just res.bppUrl) res.merchant.defaultCity res.merchant.country False
