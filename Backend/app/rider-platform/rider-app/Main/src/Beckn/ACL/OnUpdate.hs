...
parseEstimateRepetitionEvent transactionId order = do
  ...
  cancellationSource <- Utils.getTagV2 "previous_cancellation_reasonsbitc" "cancellation_reason" tagGroups & fromMaybeM (InvalidRequest "previous_cancellation_reasons tag is not present in EstimateRepetition Event.")
  return $
    DOnUpdate.EstimateRepetitionReq
      { searchRequestId = Id transactionId,
