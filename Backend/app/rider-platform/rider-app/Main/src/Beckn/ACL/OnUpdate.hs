...
parseEventV2 transactionId order = do
  ...
  case eventType of
    "RIDE_ASSIGNED" -> do
      assignedReq <- Common.parseRideAssignedEvent order
      return $ DOnUpdate.OURideAssignedReq assignedReq
    "RIDE_ARRIVED_PICKUP" -> do
      arrivedReq <- Common.parseDriverArrivedEvent order
      return $ DOnUpdate.OUDriverArrivedReq arrivedReq
    "RIDE_STARTED" -> do
      startedReq <- Common.parseRideStartedEvent order
      return $ DOnUpdate.OURideStartedReq startedReq
    "RIDE_ENDED" -> do
      completedReq <- Common.parseRideCompletedEvent order
      return $ DOnUpdate.OURideCompletedReq completedReq
    "RIDE_CANCELLED" -> do
      cancelledReq <- Common.parseBookingCancelledEvent order
...
--   registeredAt :: UTCTime <- fromMaybeM (InvalidRequest "personTags is not present in RideAssigned Event.") $ readMaybe . T.unpack =<< Utils.getTagV2 "driver_details" "registered_at" tagGroups
