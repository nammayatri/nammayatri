...
data SellerIdentity = SellerIdentity
  { subscriberId :: Text,
    subscriberUrl :: Text,
    -- | How long the callback we are about to send stays valid, and the core version we
    -- stamp on it. Both come from config: the TTL from the beckn_config column for this
    -- action, the version from operator_config.
    callbackTtl :: Text,
    contextVersion :: Text
  }
  deriving (Show, Eq)
...
mkSellerPayment catalog =
  Spec.Payment
    { paymentCollectedBy = Just "BAP",
      ...
      paymentTags = Just (sellerPaymentTags catalog.courtJurisdiction catalog.businessTermsUrl catalog.maxPaidAreaMinutes Nothing Nothing catalog.settlementWindow catalog.buyerFinderFeePercentage),
      paymentType = Nothing
    }
...
sellerPaymentTags :: Text -> Text -> Maybe Int -> Maybe Text -> Maybe Text -> Maybe Text -> Maybe Text -> [Spec.TagGroup]
sellerPaymentTags courtJurisdiction' businessTermsUrl' mbMaxPaidAreaMinutes mbSettlementAmount mbSettlementType mbSettlementWindow mbBuyerFinderFee =
  ...
  where
    ...
    buyerFinderFees =
      group
        "BUYER_FINDER_FEES"
        Nothing
        [ mkTag "BUYER_FINDER_FEES_PERCENTAGE" (fromMaybe "0" mbBuyerFinderFee),
    ...
    settlementTerms =
      group
        "SETTLEMENT_TERMS"
        Nothing
        ( [ mkTag "SETTLEMENT_WINDOW" (fromMaybe "PT1D" mbSettlementWindow),
