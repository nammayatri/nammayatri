...
buildQuoteInfo driverId fulfillment quote contextTime item = do
  quoteDetails <- case fulfillment._type of
    "RIDE" -> buildDriverOfferQuoteDetails item fulfillment quote contextTime driverId
    _ -> throwError $ InvalidRequest "select not supported for without ride trip"
...
buildQuoteInfoV2 driverId fulfillment quote contextTime order item = do
  ...
  let descriptions = []
      specialLocationTag = Utils.getTagV2 Tag.INFO Tag.SPECIAL_LOCATION_TAG =<< item.itemTags
  ...
  where
    castVehicleVariant mbCategory mbVariant =
      case (mbCategory, mbVariant) of
        (Just "CAB", Just "SEDAN") -> Just SEDAN
        (Just "CAB", Just "SUV") -> Just SUV
        (Just "CAB", Just "HATCHBACK") -> Just HATCHBACK
        (Just "AUTO_RICKSHAW", Just "AUTO_RICKSHAW") -> Just AUTO_RICKSHAW
        (Just "CAB", Just "TAXI") -> Just TAXI
        (Just "CAB", Just "TAXI_PLUS") -> Just TAXI_PLUS
        _ -> Nothing
...
buildDriverOfferQuoteDetailsV2 item fulfillment quote timestamp driverId = do
  ...
  bppQuoteId <- (Utils.getTagV2 Tag.GENERAL_INFO Tag.BPP_QUOTE_ID =<< item.itemTags) & fromMaybeM (InvalidRequest "Missing bpp quoteId select item")
  pure $
    DOnSelect.DriverOfferQuoteDetails
      { distanceToPickup = realToFrac distanceToPickup',
        bppDriverQuoteId = bppQuoteId,
