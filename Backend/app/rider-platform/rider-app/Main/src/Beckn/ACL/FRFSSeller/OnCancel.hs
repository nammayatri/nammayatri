    ...
    withRefundBreakup quote =
      quote
        { Spec.quotationPrice = Just (OnInit.mkPrice order.currency outcome.cancellationCharges),
          -- BASE_FARE (F) + REFUND (-refund) already sums to the retained charge (F - refund =
          -- cancellationCharges), which is the top-level quotationPrice. A separate
          -- CANCELLATION_CHARGES line would double-count it (sum = 2*charges), breaking the Beckn
          -- "breakup sums to price" invariant for any operator that charges a cancellation fee.
          Spec.quotationBreakup = (<> [refundLine]) <$> quote.quotationBreakup
        }
