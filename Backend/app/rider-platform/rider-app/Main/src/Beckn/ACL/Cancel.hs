...
buildCancelReq res = do
  ...
  context <- buildTaxiContext Context.CANCEL messageId (Just res.transactionId) bapIDs.cabs bapURIs.cabs (Just res.bppId) (Just res.bppUrl)
...
buildCancelSearchReq res = do
  ...
  context <- buildTaxiContext Context.CANCEL messageId (Just res.searchReqId.getId) bapIDs.cabs bapURIs.cabs (Just res.providerId) (Just res.providerUrl)
