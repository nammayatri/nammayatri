...
buildCancelReq res = do
  ...
  context <- buildTaxiContext Context.CANCEL messageId res.transactionId res.merchant.bapId bapUrl (Just res.bppId) (Just res.bppUrl) res.merchant.city res.merchant.country False Nothing
...
buildCancelSearchReq res = do
  ...
  bapUrl <- asks (.nwAddress) <&> #baseUrlPath %~ (<> "/" <> T.unpack res.merchant.id.getId)
  context <- buildTaxiContext Context.CANCEL messageId res.searchReqId.getId res.merchant.bapId bapUrl (Just res.providerId) (Just res.providerUrl) res.merchant.city res.merchant.country False Nothing
