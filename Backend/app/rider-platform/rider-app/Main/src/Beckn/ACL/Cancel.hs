...
buildCancelReq res = do
  ...
  context <- buildContext Context.MOBILITY Context.CANCEL messageId (Just res.transactionId) res.merchant.bapId bapUrl (Just res.bppId) (Just res.bppUrl) res.merchant.city res.merchant.country False
...
buildCancelSearchReq res = do
  ...
  bapUrl <- asks (.nwAddress) <&> #baseUrlPath %~ (<> "/" <> T.unpack res.merchant.id.getId)
  context <- buildContext Context.MOBILITY Context.CANCEL messageId (Just res.searchReqId.getId) res.merchant.bapId bapUrl (Just res.providerId) (Just res.providerUrl) res.merchant.city res.merchant.country False
