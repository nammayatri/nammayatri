...
buildRentalSearchReq DRentalSearch.RentalSearchRes {..} =
    buildSearchReq origin Nothing searchId (OnlyTime startTime) Nothing city Nothing
...
buildRecurringSearchReq DRecurringSearch.RecurringSearchRes {..} =
  buildSearchReq origin (Just destination) searchId (TimeAndDays initialRideTime scheduleDays) Nothing "" Nothing
...
buildSearchReq ::
  ...
  Maybe Text ->
  m (BecknReq Search.SearchMessage)
buildSearchReq origin mbDestination searchId searchTime mbRouteInfo city device = do
