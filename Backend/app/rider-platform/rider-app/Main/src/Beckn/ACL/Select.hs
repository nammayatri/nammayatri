...
buildSelectReqV2 dSelectRes = do
  ...
  bapConfig <- (listToMaybe <$> getConfig (BecknConfigDimensions {merchantOperatingCityId = moc.id.getId, merchantId = dSelectRes.merchant.id.getId, domain = Just "MOBILITY", vehicleCategory = Nothing, becknProtocol = Nothing}) (Just (SQBC.findByMerchantIdDomainandMerchantOperatingCityId (Just dSelectRes.merchant.id) "MOBILITY" (Just moc.id)))) >>= fromMaybeM (InvalidRequest $ "BecknConfig not found for merchantId " <> show dSelectRes.merchant.id.getId <> " merchantOperatingCityId " <> show moc.id.getId)
  ...
  let baseTags =
        Tags.buildTagGroups $
          [ Tags.IS_AUTO_ASSIGN_ENABLED Tags.~= show res.autoAssignEnabled,
            ...
            Tags.CUSTOMER_RATING Tags.~=? ((T.pack . show) <$> res.customerRating),
            Tags.CUSTOMER_TOTAL_RATINGS Tags.~= show res.customerTotalRatings,
            Tags.CUSTOMER_GENDER Tags.~= show res.customerGender,
...
tfPayments res bapConfig riderConfig = do
  ...
  Just $ L.singleton $ mkPayment (show res.city) (show bapConfig.collectedBy) Enums.NOT_PAID mPrice Nothing mkParams bapConfig.settlementType bapConfig.settlementWindow bapConfig.staticTermsUrl bapConfig.buyerFinderFee False res.paymentMode (show <$> res.paymentInstrument)
