...
buildRatingReq DFeedback.FeedbackRes {..} = do
  ...
  context <- buildTaxiContext Context.RATING msgId (Just transactionId) merchant.bapId bapUrl (Just providerId) (Just providerUrl) merchantConfig.city merchantConfig.country False
  let message =
