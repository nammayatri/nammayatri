...
buildConfirmReq res = do
  ...
  context <- buildTaxiContext Context.CONFIRM messageId (Just res.transactionId) res.merchant.bapId bapUrl (Just res.bppId) (Just res.bppUrl) res.merchantConfig.city res.merchantConfig.country False
