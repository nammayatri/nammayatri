...
tfOrderTags res =
  Tags.buildTagGroups
    [ Tags.DISCOUNT_AMOUNT Tags.~=? (encodeToText <$> mbDiscountAmount),
      Tags.BOOKING_DEPOSIT_HELD Tags.~=? (show <$> res.bookingDepositAmount)
    ]
  where
  ...
  Just
    [ emptyFulfillment
        { Spec.fulfillmentCustomer = tfCustomer res,
          ...
          Spec.fulfillmentTags = Utils.mkLocationAddressTags res.fromLocation.address (res.mbToLocation <&> (.address)),
