...
instance FromJSON SellerEntityInfo where
  parseJSON = A.withObject "SellerEntityInfo" $ \o ->
    SellerEntityInfo
      <$> o A..: "gst"
      <*> o A..: "pan"
      ...
      <*> o A..: "email_id"
      <*> (o A..: "mobile_no" >>= parseFlexibleText)
      <*> o A..: "country"

instance ToJSON SellerEntityInfo where
  toJSON = genericToJSON constructorsWithSnakeCase

-- mobile_no is authored by operators in config JSON and may arrive as a string ("9876543210")
-- or a bare number (9876543210). Accept either so a string never silently fails the parse and
-- suppresses on_info.
parseFlexibleText :: A.Value -> Parser Text
parseFlexibleText (A.String t) = pure t
parseFlexibleText (A.Number n) = pure $ case Sci.floatingOrInteger n :: Either Double Integer of
  Left d -> show d
  Right i -> show i
parseFlexibleText v = typeMismatch "String or Number" v
