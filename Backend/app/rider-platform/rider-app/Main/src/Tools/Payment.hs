...
refundPaymentByServiceType :: ServiceFlow m r => Id DM.Merchant -> Id DMOC.MerchantOperatingCity -> Maybe (Id TicketPlace) -> PaymentServiceType -> Maybe Text -> Maybe Version -> Payment.RefundPaymentReq -> m Payment.RefundPaymentResp
refundPaymentByServiceType merchantId merchantOperatingCityId mbPlaceId paymentServiceType mRoutingId clientSdkVersion =
  runWithServiceConfigAndServiceName Payment.refundPayment merchantId merchantOperatingCityId mbPlaceId paymentServiceType mRoutingId clientSdkVersion Nothing
...
runWithServiceConfig1 func getCfg merchantId merchantOperatingCityId paymentMode req = do
  ...
  merchantPaymentServiceConfig <-
    getOneConfig (MerchantServiceConfigDimensions {merchantOperatingCityId = merchantOperatingCityId.getId, merchantId = merchantId.getId, serviceName = Just (DMSC.PaymentService paymentService)}) Nothing
...
getIsSplitEnabled merchantId merchantOperatingCityId mbPlaceId paymentServiceType = do
  placeBasedConfig <- case mbPlaceId of
    ...
    Nothing -> return Nothing
  merchantServiceConfig <-
    getOneConfig (MerchantServiceConfigDimensions {merchantOperatingCityId = merchantOperatingCityId.getId, merchantId = merchantId.getId, serviceName = Just (getPaymentServiceByType paymentServiceType)}) Nothing
