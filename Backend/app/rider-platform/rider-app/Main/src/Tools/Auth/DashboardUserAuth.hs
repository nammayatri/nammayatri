...
--
-- > DashboardUserAuth 'APP_BACKEND_MANAGEMENT "RIDER_MANAGEMENT/RIDE/GET_RIDE_LIST"
--
...
import Servant
import Tools.Auth.DashboardUser (DashboardAuthFlow, DashboardUser, assignDashboardRoleByAccessType, auditDashboardAction, dashboardRequestorAccessType, dashboardRequestorId, dashboardRequestorName, requestorCityAccess, resolveRequestorTopic)
...
verifyDashboardUserAction =
  VerificationActionWithPayload $ \(serverName, endpointId) pathSegments token ->
    DashboardUser.verifyDashboardUser serverName endpointId pathSegments token
