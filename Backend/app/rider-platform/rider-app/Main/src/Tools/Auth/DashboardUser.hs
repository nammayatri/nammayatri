...
verifyDashboardUser serverName endpointId pathSegments token = do
  ...
  -- database scope, and before the audit write so a rejected call leaves no row.
  ...
  -- The proxy wrote one audit row per mutating action; keep that record when the
  -- route is served here instead. Reads were never audited.
  --
  ...
  unless (QAudit.isReadOnlyEndpoint endpointId) $
    runInDashboardDb $
      QAudit.writeAuditTransaction
        QAudit.AuditTransaction
          { requestorId = Just verified.personId.getId,
            merchantId = Just verified.merchant.id.getId,
