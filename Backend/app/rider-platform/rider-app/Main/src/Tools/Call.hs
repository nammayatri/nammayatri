...
runWithServiceConfig func getCfg merchantId merchantOperatingCityId req = do
  ...
  merchantCallServiceConfig <-
    getOneConfig (MerchantServiceConfigDimensions {merchantOperatingCityId = merchantOperatingCityId.getId, serviceName = Just (DMSC.CallService $ getCfg merchantConfig)})
