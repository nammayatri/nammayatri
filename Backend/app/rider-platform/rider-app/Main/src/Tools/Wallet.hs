...
runWithServiceConfig ::
  ...
  (Wallet.WalletServiceConfig -> req -> m resp) ->
  Id DM.Merchant ->
  Id DMOC.MerchantOperatingCity ->
  req ->
  m resp
runWithServiceConfig func merchantId merchantOperatingCityId req = do
  let serviceName = DMSC.JuspayWalletService WalletTypes.Juspay
  merchantServiceConfig <-
    CQMSC.findByMerchantOpCityIdAndService merchantId merchantOperatingCityId serviceName
      >>= fromMaybeM (MerchantServiceConfigNotFound merchantId.getId "JuspayWallet" (show WalletTypes.Juspay))
  case merchantServiceConfig.serviceConfig of
    DMSC.JuspayWalletServiceConfig walletCfg -> func walletCfg req
