...
returnConfigs cfgType merchantOpCityId merchantId opCity = do
  case cfgType of
    ...
    (LYTU.UiConfig dt pt) -> do
      ...
      (mbUiConfig, _) <- SCU.findUiConfig uiConfigReq (cast merchantOpCityId)
...
handleConfigDBUpdate merchantOpCityId concludeReq baseLogics mbMerchantId opCity = do
  case concludeReq.domain of
    ...
    LYTU.RIDER_CONFIG (LYTU.UiConfig dt pt) -> do
      let uiConfigReq = LYTU.UiConfigRequest {os = dt, platform = pt, merchantId = if isNothing mbMerchantId then "" else getId (fromJust mbMerchantId), city = opCity, language = Nothing, bundle = Nothing, toss = Nothing}
      handleConfigUpdateWithExtraDimensionsUi SCU.findUiConfig (SCU.clearCache (cast merchantOpCityId) dt pt) SCU.updateByPrimaryKey (cast merchantOpCityId) uiConfigReq
  ...
  where
    ...
    handleConfigUpdateWithExtraDimensionsUi ::
      (MonadFlow m, FromJSON a, ToJSON a, Eq a, Show a) =>
      (LYTU.UiConfigRequest -> Id MerchantOperatingCity -> m (Maybe a, Maybe Int)) -> -- Fetch function
      m () -> -- Cache clearing function
      (a -> m ()) -> -- Update function
      Id MerchantOperatingCity ->
    ...
    handleConfigUpdateWithExtraDimensionsUi fetchFunc clearCacheFunc updateFunc merchantOpCityId' uiConfigReq' = do
      (mbUiConfig, _) <- fetchFunc uiConfigReq' merchantOpCityId'
