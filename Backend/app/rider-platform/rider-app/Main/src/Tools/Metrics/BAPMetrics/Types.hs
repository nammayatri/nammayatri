...
-- Labels: (result ∈ hit|miss|error, merchant_short_id, merchantOperatingCityId)
type OtpPlanCacheCounterMetric = P.Vector P.Label3 P.Counter
...
registerOtpPlanCacheCounterMetric = P.register $ P.vector ("result", "merchant_short_id", "merchantOperatingCityId") $ P.counter $ P.Info "otp_plan_cache_events_total" "OTP plan-response Redis cache hits/misses/errors"
