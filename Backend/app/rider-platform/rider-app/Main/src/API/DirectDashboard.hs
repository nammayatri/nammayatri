...
-- reached through the proxy and is the type provider-dashboard derives its
-- client from, so it keeps its shape until provider-dashboard is retired. Both
-- trees call the same domain handlers in the meantime.
--
...
type API =
  "direct-dashboard"
    ...
    :> ( AppManagementDSL.API
           :<|> ManagementDSL.API
           -- provider-dashboard mounts this folder under an extra "rideBooking"
           -- segment on top of the one each module already carries. Mirroring that
           -- keeps the public path identical, so ingress is a prefix swap rather
