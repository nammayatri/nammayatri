...
callGetPersonDetails merchantId personId = do
  ...
  AP.getPersonDetails (personId, m.id) Nothing Nothing Nothing --CACTODO: this is a temporary implementation need to discuss and implement toss part if required or change response type all together for this api to stop sending cfg version forever.
