  ...
  (singleModeWarningType, otpResponse) <- do
    if not (null directSingleModeRoutes)
      then do
        ...
        otpResponse' <- JMU.measureLatency (PlanCache.getTransitRoutesCached merchantOperatingCityId searchRequest.merchantId (Just searchRequest.id.getId) transitServiceReq transitRoutesReq >>= fromMaybeM (InternalError "routes dont exist")) "getTransitRoutes"
