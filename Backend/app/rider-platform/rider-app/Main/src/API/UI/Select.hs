...
select (personId, _) estimateId req = withFlowHandlerAPI . withPersonIdLogTag personId $ do
  Redis.whenWithLockRedis (selectEstimateLockKey personId) 60 $ do
    ...
    void $ withShortRetry $ CallBPP.selectV2 dSelectReq.providerUrl becknReq dSelectReq.searchRequest.merchantOperatingCityId
      ...
      result <-
        try @_ @SomeException $
          when dCancelSearch.sendToBpp . void . withShortRetry $ do
            CallBPP.cancelV2 dCancelSearch.merchantOperatingCityId dCancelSearch.providerUrl =<< CACL.buildCancelSearchReqV2 dCancelSearch
      case result of
        Left err -> do
