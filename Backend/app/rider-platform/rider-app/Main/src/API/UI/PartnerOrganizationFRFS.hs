...
getFareV2 partnerOrg req = withFlowHandlerAPI . withLogTag $ do
  ...
  where
    withLogTag = Log.withLogTag ("FRFS:GetFareV2:PartnerOrgId:" <> getId partnerOrg.orgId)

    getFareV2HitsCountKey :: Text
    getFareV2HitsCountKey = "BAP:FRFS:PartnerOrgId:" <> getId partnerOrg.orgId <> ":GetFareV2:hitsCount"

partnerQuoteConfirm :: PartnerOrganization -> DPOFRFS.PartnerQuoteConfirmReq -> FlowHandler DPOFRFS.PartnerQuoteConfirmRes
partnerQuoteConfirm partnerOrg req = withFlowHandlerAPI . withLogTag $ do
  ...
  DPOFRFS.partnerQuoteConfirm partnerOrg req
  where
    withLogTag = Log.withLogTag ("FRFS:PartnerQuoteConfirm:PartnerOrgId:" <> getId partnerOrg.orgId <> " FRFS:PartnerQuoteConfirm:SearchId:" <> req.searchId.getId)
    partnerQuoteConfirmHitsCountKey :: Text
    partnerQuoteConfirmHitsCountKey = "BAP:FRFS:PartnerOrgId:" <> getId partnerOrg.orgId <> ":PartnerQuoteConfirm:hitsCount"
