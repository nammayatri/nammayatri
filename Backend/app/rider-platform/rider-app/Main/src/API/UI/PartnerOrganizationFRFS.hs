...
upsertPersonAndGetFare partnerOrg mbIntegratedBPPConfigId req = withFlowHandlerAPI . withLogTag $ do
  ...
  Log.withLogTag ("FRFS:GetFare:PersonId:" <> personId.getId) $ do
    ...
    res <- DFRFSTicketService.postFrfsSearchHandler (personId, merchantId) merchantOperatingCity integratedBPPConfig frfsVehicleType frfsSearchReq frfsRouteDetails req.partnerOrgTransactionId (Just partnerOrg.orgId) Nothing Nothing (\_ -> pure ()) [] [] True Nothing Nothing -- the journey leg upsert function is not required here
  ...
  where
    ...
    buildFRFSSearchReq fromStationCode toStationCode routeCode quantity = DFRFSTypes.FRFSSearchAPIReq {recentLocationId = Nothing, searchAsParentStops = Nothing, serviceTier = Nothing, busLocationData = Nothing, vehicleNumber = Nothing, platformType = Nothing, tripTime = Nothing, journeyDate = Nothing, tripCategory = Nothing, ..}
