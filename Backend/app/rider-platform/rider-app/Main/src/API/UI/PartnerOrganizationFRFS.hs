...
upsertPersonAndGetFare partnerOrg mbIntegratedBPPConfigId req = withFlowHandlerAPI . withLogTag $ do
  ...
  where
    ...
    buildFRFSSearchReq fromStationCode toStationCode routeCode quantity = DFRFSTypes.FRFSSearchAPIReq {recentLocationId = Nothing, searchAsParentStops = Nothing, serviceTier = Nothing, busLocationData = Nothing, vehicleNumber = Nothing, platformType = Nothing, journeyDate = Nothing, ..}
