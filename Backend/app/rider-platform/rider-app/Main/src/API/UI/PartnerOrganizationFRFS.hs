...
upsertPersonAndGetFare partnerOrg req = withFlowHandlerAPI . withLogTag $ do
  ...
  Log.withLogTag ("FRFS:GetFare:PersonId:" <> personId.getId) $ do
    ...
    res <- DFRFSTicketService.postFrfsSearchHandler (Just personId, merchantId) (Just merchantOperatingCity.city) frfsVehicleType frfsSearchReq req.partnerOrgTransactionId (Just partnerOrg.orgId) Nothing Nothing
