...
onCancel _ req = withFlowHandlerBecknAPI . withTransactionIdLogTag req $ do
  case req.contents of
    Left _ -> pure ()
    Right message -> do
      tagsGroup <- fromMaybeM (InvalidRequest "cancellation tag is not present in Oncancel Request.") message.order.fulfillment.tags
      let mbBookingCancelInfoTag = getTag "booking_cancelled_info" "booking_cancelled" tagsGroup
          isBookingCancelled = readMaybe . T.unpack =<< mbBookingCancelInfoTag :: Maybe Bool
      when (isBookingCancelled == Just True) $ do
