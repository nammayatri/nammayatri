...
-- import qualified Domain.Action.Beckn.FRFS.OnInit as DOnSelect
import Environment
...
onSelect _ req = withFlowHandlerAPI $ do
  ...
  _ <- req.onSelectReqContext.contextTransactionId & fromMaybeM (InvalidRequest "TransactionId not found")
  ...
  --       (merchant, booking) <- DOnSelect.validateRequest onSelectReq
  --       Metrics.finishMetrics Metrics.SELECT_FRFS merchant.name transaction_id booking.merchantOperatingCityId.getId
  --       fork "FRFS on_select processing" $ do
  --         Redis.whenWithLockRedis (onSelectProcessingLockKey onSelectReq.messageId) 60 $
  --           DOnSelect.onSelect onSelectReq merchant booking
