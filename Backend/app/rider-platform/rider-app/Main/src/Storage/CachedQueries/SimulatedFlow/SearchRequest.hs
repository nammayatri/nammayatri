...
data KafkaDumpObject = KafkaDumpObject
  { personId :: Id Person,
    searchReqId :: Id SearchRequest,
    fromLocation :: Maybe Maps.LatLong,
    toLocation :: Maybe Maps.LatLong,
...
data CancelledBy = US | THEM deriving (Generic, ToJSON, FromJSON)

convertToKafkaData :: (SimluatedCacheFlow m r, MonadFlow m) => Id Person -> Id SearchRequest -> CancelledBy -> m KafkaDumpObject
convertToKafkaData personId searchReqId cancelledBy = do
  mbSearchReq <- getSearchRequestById searchReqId
  now <- getCurrentTime
  pure $
    KafkaDumpObject
      { personId = personId,
        searchReqId = searchReqId,
        fromLocation = (\fromLoc -> Maps.LatLong fromLoc.lat fromLoc.lon) . (.fromLocation) <$> mbSearchReq,
        toLocation = (\toLoc -> Maps.LatLong toLoc.lat toLoc.lon) <$> ((.toLocation) =<< mbSearchReq),
        createdAt = (.createdAt) <$> mbSearchReq,
        cancelledAt = now,
        cancelledBy = cancelledBy
      }
