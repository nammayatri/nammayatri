...
findMatchingMerchantPN id messageKey tripCategory subCategory personLanguage = do
  merchantPNs <-
    Hedis.safeGet (makeMerchantOpCityIdAndMessageKeyAndTripCategory id messageKey tripCategory) >>= \case
      Just a -> return a
      Nothing -> do
        pns <- Queries.findAllByMerchantOpCityAndMessageKeyAndTripCategory id messageKey tripCategory
        if null pns
          then do
            pnsWithOutTripCategory <- Queries.findAllByMerchantOpCityAndMessageKeyAndTripCategory id messageKey Nothing
