...
deletePartnerOrgStation :: (CacheFlow m r, EsqDBFlow m r, FromJSON PartnerOrgStation) => Id PartnerOrganization -> Id PartnerOrgStation -> m ()
deletePartnerOrgStation partnerOrgId partnerOrgStationId = do
  let key = makePOrgIdAndPOrgStationIdKey partnerOrgId partnerOrgStationId
  Hedis.safeGet key >>= \case
