...
findAllFeedback :: (KvDbFlow m r, HasCacheFeedbackFormConfig r) => m [FeedbackFormRes]
findAllFeedback =
  Hedis.safeGet "CachedQueries:FeedbackForm" >>= \case
    Just a -> return a
    Nothing -> cacheFeedback "CachedQueries:FeedbackForm" /=<< Queries.findAllFeedback

findAllFeedbackByRating :: (KvDbFlow m r, HasCacheFeedbackFormConfig r) => Int -> m [FeedbackFormRes]
