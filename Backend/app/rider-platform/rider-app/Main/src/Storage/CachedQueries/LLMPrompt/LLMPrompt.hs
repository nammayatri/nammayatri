...
makeMerchantOpCityIdKey id = "rider:CachedQueries:LlmPrompt:MerchantOperatingCityId-" <> id.getId
...
makeMerchantOpCityIdAndServiceNameAndUseCaseAndPromptKeyKey opCityId serviceName useCase promptKey = "rider:CachedQueries:LlmPrompt:MerchantOperatingCityId-" <> opCityId.getId <> ":ServiceName-" <> show serviceName <> ":UseCase-" <> show useCase <> ":PromptKey-" <> show promptKey
...
clearCache :: Hedis.HedisFlow m r => Id MerchantOperatingCity -> ServiceName -> UseCase -> PromptKey -> m ()
clearCache opCityId serviceName useCase promptKey = do
  Hedis.del (makeMerchantOpCityIdAndServiceNameAndUseCaseAndPromptKeyKey opCityId serviceName useCase promptKey)
