...
incrementOrSetPersonStats :: DPS.PersonStats -> SqlDB ()
  ...
  Esq.update $ \tbl -> do
    set
      ...
      [ PersonStatsUpdatedAt =. val now,
        PersonStatsUserCancelledRides =. (tbl ^. PersonStatsUserCancelledRides +. val personStats.userCancelledRides),
        PersonStatsDriverCancelledRides =. (tbl ^. PersonStatsDriverCancelledRides +. val personStats.driverCancelledRides),
        PersonStatsCompletedRides =. (tbl ^. PersonStatsCompletedRides +. val personStats.completedRides),
        PersonStatsWeekdayRides =. (tbl ^. PersonStatsWeekdayRides +. val personStats.weekdayRides),
        PersonStatsWeekendRides =. (tbl ^. PersonStatsWeekendRides +. val personStats.weekendRides),
        PersonStatsOffPeakRides =. (tbl ^. PersonStatsOffPeakRides +. val personStats.offPeakRides),
        PersonStatsMorningPeakRides =. (tbl ^. PersonStatsMorningPeakRides +. val personStats.morningPeakRides),
        PersonStatsEveningPeakRides =. (tbl ^. PersonStatsEveningPeakRides +. val personStats.eveningPeakRides),
        PersonStatsWeekendPeakRides =. (tbl ^. PersonStatsWeekendPeakRides +. val personStats.weekendPeakRides)
      ]
    where_ $ tbl ^. PersonStatsTId ==. val (toKey personStats.personId)
...
incrementWeekendPeakRidesCount :: Id Person -> SqlDB ()
incrementWeekendPeakRidesCount personId = do
  now <- getCurrentTime
  Esq.update $ \tbl -> do
    set
      tbl
      [ PersonStatsUpdatedAt =. val now,
        PersonStatsWeekendPeakRides =. (tbl ^. PersonStatsWeekendPeakRides +. val 1)
      ]
    where_ $ tbl ^. PersonStatsTId ==. val (toKey personId)
