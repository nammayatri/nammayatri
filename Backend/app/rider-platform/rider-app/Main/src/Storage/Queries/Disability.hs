...
findAll :: KvDbFlow m r => m [DisabilityItem]
findAll = findAllWithKV [Se.Is BeamDT.disabilityId $ Se.Not $ Se.Eq ""]

findByDisabilityId :: KvDbFlow m r => Text -> m (Maybe DisabilityItem)
findByDisabilityId disabilityId = findOneWithKV [Se.Is BeamD.id $ Se.Eq disabilityId]

findAllByLanguage :: KvDbFlow m r => Language -> m [DisabilityItem]
