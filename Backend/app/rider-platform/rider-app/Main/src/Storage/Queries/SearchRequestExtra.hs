...
createDSReq searchRequest = do
  -- searchRequest.id is freshly generated by the caller for this brand-new search -- it is
  -- structurally guaranteed to have no prior location mappings, so the *NewEntity variants
  -- (safe to serve from Redis alone / tolerate a Postgres outage) are used here. Do not reuse
  -- this pattern for edit flows (see EditLocation.hs) where entityId is pre-existing.
