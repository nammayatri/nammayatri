...
create :: MonadFlow m => SearchRequestMapping -> m ()
create = createWithKV
...
findByEntityId :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Text -> m [SearchRequestMapping]
findByEntityId entityId = findAllWithKVAndConditionalDB [Se.Is BeamLM.entityId $ Se.Eq entityId]

findAllByEntityIdAndOrder :: (MonadFlow m, CacheFlow m r, EsqDBFlow m r) => Text -> Int -> m [SearchRequestMapping]
...
incrementVersion :: MonadFlow m => SearchRequestMapping -> m ()
