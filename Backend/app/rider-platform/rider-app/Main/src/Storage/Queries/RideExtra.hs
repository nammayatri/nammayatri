...
-- Returns (Ride, Booking) to avoid an extra query to the booking table at call sites.
findMostRecentRideForRider :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => Id Person -> m (Maybe (Ride, Booking))
    ...
    Right rows -> do
      case listToMaybe rows of
        ...
        Just (booking', ride') -> do
          mbBooking <- fromTType' booking'
          mbRide <- fromTType' ride'
          pure $ (,) <$> mbRide <*> mbBooking
    Left _ -> pure Nothing
