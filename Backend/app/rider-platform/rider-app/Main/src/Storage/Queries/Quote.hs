...
findAllBySRId :: KvDbFlow m r => Id SearchRequest -> m [Quote]
    ...
    let (fareProductType, distanceToNearestDriver, rentalDetailsId, driverOfferId, specialZoneQuoteId) = case quoteDetails of
     ...
     in BeamQ.QuoteT
          { BeamQ.id = getId id,
            ...
            BeamQ.estimatedFare = estimatedFare.amount,
            BeamQ.discount = discount <&> (.amount),
            BeamQ.estimatedTotalFare = estimatedTotalFare.amount,
            BeamQ.currency = Just estimatedFare.currency,
