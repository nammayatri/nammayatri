...
module Storage.Queries.RiderDashboardTransaction where
...
import qualified "this" Storage.Beam.RiderDashboardTransaction as BeamT
import qualified "lib-dashboard" Storage.Queries.Transaction as QT

instance ToTType' BeamT.RiderDashboardTransaction (DT.Transaction AccessMatrix.UserActionType) where
  toTType' txn = coerceRow (QT.toBeamTransaction txn)
    where
      -- structurally identical rows; only the Beam type tag differs
      coerceRow r =
        BeamT.RiderDashboardTransactionT
          { id = r.id,
