...
upsert journeyLeg = do
  (findOneWithKV [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId journeyLeg.id)]]) >>= \case
    Just _ -> updateLeg journeyLeg
    Nothing -> create journeyLeg
  where
    updateLeg (Domain.Types.JourneyLeg.JourneyLeg {..}) = do
      _now <- getCurrentTime
      updateWithKV
