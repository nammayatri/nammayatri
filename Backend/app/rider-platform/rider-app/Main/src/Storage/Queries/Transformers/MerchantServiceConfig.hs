...
getServiceConfigFromDomain serviceName configJSON = do
  maybe (throwError $ InternalError "Unable to decode MerchantServiceConfigT.configJSON") return $ case serviceName of
    ...
    Domain.PaymentService Payment.JuspayPayout -> Domain.PaymentServiceConfig . Payment.JuspayPayoutConfig <$> valueToMaybe configJSON
    ...
    Domain.MetroPaymentService Payment.JuspayPayout -> Domain.MetroPaymentServiceConfig . Payment.JuspayPayoutConfig <$> valueToMaybe configJSON
...
getServiceNameConfigJson = \case
  ...
  Domain.PaymentServiceConfig paymentCfg -> case paymentCfg of
    ...
    Payment.JuspayPayoutConfig cfg -> (Domain.PaymentService Payment.Juspay, toJSON cfg)
