...
updateOtpByIdForPartnerOrgId :: (MonadFlow m, EsqDBFlow m r) => Id RegistrationToken -> Id DPOrg.PartnerOrganization -> Text -> m ()
updateOtpByIdForPartnerOrgId (Id rtId) (Id orgId) authValueHash = do
  now <- getCurrentTime
  updateOneWithKV
