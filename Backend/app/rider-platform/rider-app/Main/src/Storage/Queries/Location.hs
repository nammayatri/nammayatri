...
create :: (MonadFlow m, EsqDBFlow m r) => Location -> m ()
create = createWithKV
...
updateAddress :: (MonadFlow m, EsqDBFlow m r) => Id Location -> LocationAddress -> m ()
updateAddress (Id blId) LocationAddress {..} = do
  ...
  updateOneWithKV
    [ Se.Set BeamL.street street,
      ...
      Se.Set BeamL.ward ward,
      Se.Set BeamL.placeId placeId,
  ...
  toTType' Location {..} = do
    BeamL.LocationT
      { BeamL.id = getId id,
        ...
        BeamL.ward = (ward :: LocationAddress -> Maybe Text) address,
        BeamL.placeId = (placeId :: LocationAddress -> Maybe Text) address,
