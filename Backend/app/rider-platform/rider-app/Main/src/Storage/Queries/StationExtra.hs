...
findAllMatchingStations :: (MonadFlow m, EsqDBFlow m r, CacheFlow m r) => Maybe Text -> Maybe Integer -> Maybe Integer -> Id MerchantOperatingCity -> VehicleCategory -> Id IntegratedBPPConfig -> m [Station]
findAllMatchingStations mbSearchStr (Just limitVal) (Just offsetVal) (Id merchantOperatingCityId') vehicle (Id integratedBPPConfigId') = do
...
findAllMatchingStations mbSearchStr _ _ (Id merchantOperatingCityId') vehicle (Id integratedBPPConfigId') = do
  dbConf <- getReplicaBeamConfig
  stations <-
    L.runDB dbConf $
      L.findRows $
