...
findByLatLon :: Transactionable m => LatLong -> m (Maybe SavedReqLocation)
findByLatLon LatLong {..} = Esq.findOne $ do
  ...
  where_ $
    saveReqLocation ^. SavedReqLocationLat ==. val lat
      ...
      &&. saveReqLocation ^. SavedReqLocationIsMoved ==. val (Just True)
