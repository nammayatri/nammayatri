...
module Storage.Beam.QuoteRevised where
...
data QuoteRevisedT f = QuoteRevisedT
  { id :: B.C f Text,
...
instance B.Table QuoteRevisedT where
  data PrimaryKey QuoteRevisedT f
    = Id (B.C f Text)
    deriving (Generic, B.Beamable)
  primaryKey = Id . id

type QuoteRevised = QuoteRevisedT Identity

$(enableKVPG ''QuoteRevisedT ['id] [['requestId], ['driverOfferId]])

$(mkTableInstances ''QuoteRevisedT "quote_revised")
