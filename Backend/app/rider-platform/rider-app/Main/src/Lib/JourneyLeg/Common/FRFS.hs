    ...
    filterAvailableBuses arrivalTime fareRouteDetails integratedBPPConfig fares = do
      case vehicleCategory of
        Spec.BUS -> do
          ...
          (_, possibleRoutes, _) <- JMU.findPossibleRoutes Nothing blacklistedServiceTiers startStationCode endStationCode arrivalTime integratedBPPConfig merchant.id merchantOperatingCity.id Enums.BUS True False False False
