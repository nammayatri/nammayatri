...
extendLegEstimatedFare journeyId startPoint mbEndLocation _ = do
  ...
  where
    ...
    getUpdatedFare journey currentLeg startLocation endLocation = do
      ...
      (isFareMandatory, estimatedFare) <- JLI.getFare (Just now) journey.riderId currentLeg.merchantId currentLeg.merchantOperatingCityId Nothing multiModalLeg DTrip.Taxi Nothing [] [] journey.isSingleMode
