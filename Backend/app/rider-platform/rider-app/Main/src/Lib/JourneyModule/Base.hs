...
cancelLeg ::
  ...
  m ()
cancelLeg leg = do
  isCancellable <- checkIfCancellable leg -- Unwrap the monadic boolean
  if isCancellable
    then do
      ...
      case leg.travelMode of
        ...
        DTrip.Bus ->
          JL.cancel $ BusLegRequestCancel $ BusLegRequestCancelData {searchId = cast legSearchId}
    else throwError $ InvalidRequest "Leg cannot be cancelled"
...
cancelAllLegs journeyId = do
  allLegs <- getAllLegsInfo journeyId
  results <- forM allLegs $ \leg -> try @_ @SomeException (cancelLeg leg)
  let failures = [e | Left e <- results]
  unless (null failures) $
    throwError $ InternalError $ "Failed to cancel some legs: " <> show failures
