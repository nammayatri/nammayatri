...
init journeyReq userPreferences blacklistedServiceTiers blacklistedFareQuoteTypes = do
  ...
  riderConfig <- getConfig (RiderConfigDimensions {merchantOperatingCityId = journeyReq.merchantOperatingCityId.getId}) Nothing >>= fromMaybeM (RiderConfigDoesNotExist journeyReq.merchantOperatingCityId.getId)
...
getAllLegsInfo' personId journeyId checkSearch = do
  whenJourneyUpdateInProgress journeyId $ do
    ...
    mbPassCandidates <- loadJourneyPassCandidates personId journeyId allLegs
  ...
  | otherwise = do
    ...
    if journey.status >= DJourney.CONFIRMED
      then pure (Just [])
        ...
        if enabled then loadFor else pure (Just [])
  where
    anyPassOverrideEnabled journey = do
      let categories =
            foldr
              (\vc acc -> if vc `elem` acc then acc else vc : acc)
              []
              (mapMaybe (multiModalTravelModeToBecknVehicleCategory . (.mode)) legs)
      configs <- concat <$> mapM (\vc -> SIBC.findAllIntegratedBPPConfig journey.merchantOperatingCityId vc DIBC.MULTIMODAL) categories
      pure $ any (\config -> config.passOverrideApplicable == Just True) configs

    loadFor = do
      -- personId comes from the authenticated request, so a missing person is a broken invariant,
      -- not a case to absorb. Returning Nothing here would also be read downstream as "not
      -- precomputed, look it up per leg" rather than "no passes" (see mkLegInfoFromFrfsSearchRequest).
      person <- QPerson.findById personId >>= fromMaybeM (PersonNotFound personId.getId)
      now <- getCurrentTime
      -- Clamped to now, like the per-leg derivation in Lib/JourneyModule/Types.hs: fromDepartureTime
      -- is a timetable value and is the epoch where a stop has no timing, and sort puts that first.
      -- The window filtering happens per leg later, so an epoch here does not hide passes -- it does
      -- the opposite, satisfying checkHasPass's `till >= today` for a rider whose pass expired long
      -- ago and loading candidates for nothing.
      let journeyStart = maybe now (\departure -> if departure > now then departure else now) (KP.listToMaybe (sort (mapMaybe (.fromDepartureTime) legs)))
...
getRiderConfig journey = getConfig (RiderConfigDimensions {merchantOperatingCityId = journey.merchantOperatingCityId.getId}) Nothing >>= fromMaybeM (RiderConfigDoesNotExist journey.merchantOperatingCityId.getId)
...
getMultiModalTransitOptions userPreferences merchantId merchantOperatingCityId req = do
  riderConfig <- getConfig (RiderConfigDimensions {merchantOperatingCityId = merchantOperatingCityId.getId}) Nothing >>= fromMaybeM (RiderConfigNotFound merchantOperatingCityId.getId)
...
startJourney riderId confirmElements forcedBookedLegOrder journey mbEnableOffer mbIsMockPayment = do
  ...
  void $
    withTryCatch "startJourney:confirmPassCoveredLegs" $ do
      ...
      --
      -- A deferred leg counts as payable even though it has no booking to inspect yet:
      -- validatePassSelections rejects a pass on a shouldBookLater leg, so a deferred leg is
      -- always one the rider still has to pay for. Without this the pass legs would be ticketed --
      -- and trips spent -- before the rider has committed to the rest of the journey. They are not
      -- stranded by waiting: FRFSStatus calls confirmPassCoveredLegsOfJourney once that payment
      -- succeeds.
      --
      -- bookingAllowed is part of the test, not decoration: skipBooking is what the client sends
      -- for a leg it is not booking in this call, and that includes every Walk leg. Keying on
      -- shouldBookLater alone therefore treats a plain walk-bus-walk journey as having a payable
      -- leg and never confirms it at all.
      --
      -- A non-FRFS payable leg (Taxi) is deliberately NOT counted here, even though its fare is
      -- invisible to legBookings. Blocking on it would strand the pass leg instead of deferring it:
      -- confirmPassCoveredLegs has only two triggers, this one and FRFSStatus on FRFS payment
      -- success, and a taxi leg produces no FRFS payment order -- OnInit skips createPayments when
      -- payableBookings is empty -- so nothing would ever come back for it. The leg would sit in NEW
  ...
  where
    ...
    validatePassSelections plans = do
      ...
      -- Every non-deferred leg, not just the bookingAllowed ones: confirmLeg forwards
      -- purchasedPassPaymentId for all plans, so a pass on a non-bookable leg would otherwise skip
      -- both this check and the trip-count check below and surface later as a vaguer error out of
      -- FRFSConfirm.
      --
      -- Legs that already have a booking are excluded, and that exclusion is load-bearing: their
      -- LegInfo comes from mkLegInfoFromFrfsBooking, which hardcodes applicablePasses = [], so
      -- validateOneLeg would reject a pass that was already accepted and stamped. That turns a
      -- client retrying confirm after a timeout into a hard 400 that aborts before the remaining
      -- legs are booked. They must also stay out of tripsRequired, which would otherwise
      -- double-count trips the pass has already been debited for.
      -- Only legs that actually carry a selection, so a journey with no pass anywhere -- the common
      -- case -- costs no lookups at all rather than one per leg.
      let candidates = filter (\plan -> not plan.shouldBookLater && isJust (plan.mbElement >>= (.purchasedPassPaymentId))) plans
      unbookedCandidates <-
        mapMaybeM
          (\plan -> QTBooking.findBySearchId (Id plan.legInfo.searchId) <&> maybe (Just plan) (const Nothing))
          candidates
      let selections = [(paymentId, plan) | plan <- unbookedCandidates, Just paymentId <- [plan.mbElement >>= (.purchasedPassPaymentId)]]
...
addAllLegs ::
  ...
  Maybe Bool ->
  m ()
addAllLegs journey mbOldJourneyLegs newJourneyLegs blacklistedServiceTiers blacklistedFareQuoteTypes mbHasPasses = do
  ...
  forM_ (traverseWithTriplets allLegs) $ \(mbPrevJourneyLeg, journeyLeg, mbNextJourneyLeg) -> do
    when (isNothing journeyLeg.legSearchId) $ do
      ...
      case journeyLeg.mode of
        ...
        DTrip.Metro -> do
          void $ addMetroLeg journey journeyLeg (upsertJourneyLegAction journeyLeg) blacklistedServiceTiers blacklistedFareQuoteTypes mbHasPasses
        DTrip.Subway -> do
...
addMetroLeg ::
  ...
  Maybe Bool ->
  m JL.SearchResponse
addMetroLeg journey journeyLeg upsertJourneyLegAction blacklistedServiceTiers blacklistedFareQuoteTypes mbHasPasses = do
...
addSubwayLeg ::
  ...
  Maybe Bool ->
  m JL.SearchResponse
addSubwayLeg journey journeyLeg upsertJourneyLegAction blacklistedServiceTiers blacklistedFareQuoteTypes mbHasPasses = do
  merchantOperatingCity <- QMerchOpCity.findById journey.merchantOperatingCityId >>= fromMaybeM (MerchantOperatingCityNotFound journey.merchantOperatingCityId.getId)
...
addBusLeg ::
  ...
  Maybe Bool ->
  m JL.SearchResponse
addBusLeg journey journeyLeg mbServiceTier upsertJourneyLegAction blacklistedServiceTiers blacklistedFareQuoteTypes mbHasPasses = do
  merchantOperatingCity <- QMerchOpCity.findById journey.merchantOperatingCityId >>= fromMaybeM (MerchantOperatingCityNotFound journey.merchantOperatingCityId.getId)
...
extendLeg journeyId startPoint mbEndLocation mbEndLegOrder fare newDistance newDuration bookingUpdateReqId blacklistedServiceTiers blacklistedFareQuoteTypes = do
  ...
  case startPoint of
    JL.StartLegOrder startLegOrder -> do
      ...
      riderConfig <- getConfig (RiderConfigDimensions {merchantOperatingCityId = currentLeg.merchantOperatingCityId.getId}) Nothing >>= fromMaybeM (RiderConfigDoesNotExist currentLeg.merchantOperatingCityId.getId)
