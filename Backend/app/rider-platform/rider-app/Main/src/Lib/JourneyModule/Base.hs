    ...
    mapMaybeM
      ( \leg -> do
          case leg.legSearchId of
            Just legSearchIdText -> getLegInfo leg legSearchIdText skipAddLegFallback
            Nothing -> do
              ...
              if skipAddLegFallback
                then return Nothing
                  ...
                  getLegInfo updatedLeg legSearchIdText' skipAddLegFallback
      )
      allLegsRawData
...
getLegInfo ::
  ...
  Bool ->
  m (Maybe JL.LegInfo)
getLegInfo leg legSearchIdText skipAddLegFallback = do
...
addSkippedLeg journeyId legOrder = do
  ...
  skippedLegInfo <- getLegInfo skippedLeg legSearchIdText False >>= fromMaybeM (InvalidRequest "Failed to get leg info")
