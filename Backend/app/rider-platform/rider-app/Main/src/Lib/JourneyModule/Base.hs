...
addMetroLeg parentSearchReq journeyLeg = do
  ...
  shouldSkip <- case (riderConfig.qrTicketRestrictionStartTime, riderConfig.qrTicketRestrictionEndTime) of
    (Just startTime, Just endTime) -> do
      isOutsideRestrictedHours <- isOutsideRestrictionTime startTime endTime now riderConfig.timeDiffFromUtc
...
isOutsideRestrictionTime :: MonadFlow m => Time.TimeOfDay -> Time.TimeOfDay -> UTCTime -> Seconds -> m Bool
isOutsideRestrictionTime startTime endTime now timeDiffFromUtc = do
