...
updateJourneyLegStatus travelMode recentLocations endLatLong currentStatus isLastCompleted maybeToStation = do
  ...
  where
    ...
    checkProximityViaLatest newStatus recentLocations' threshold endLatLong' = do
      latestPosition <- listToMaybe recentLocations'
      checkDistance latestPosition.latLong threshold newStatus endLatLong'
    ...
    checkProximityViaAverage newStatus recentLocations' threshold toStationGates' endLatLong' = do
      avgPosition <- averagePosition recentLocations'
      checkDistance avgPosition threshold newStatus endLatLong' <|> checkGates newStatus threshold avgPosition toStationGates'
