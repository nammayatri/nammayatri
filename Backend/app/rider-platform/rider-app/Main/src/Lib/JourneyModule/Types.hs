  ...
  overridePasses <-
    if integratedBPPConfig.passOverrideApplicable /= Just True
      then pure []
        ...
        -- timetable lookup and is the epoch where the stop has no timing, which is most subway
        ...
        mbLegStations <- Just <$> FRFSPassOverride.resolveLegStations integratedBPPConfig frfsSearch.fromStationCode frfsSearch.toStationCode
        case mbPassCandidates of
          ...
          Nothing -> FRFSPassOverride.getFRFSOverrideApplicablePassesByPersonId integratedBPPConfig person vehicleType tripTime hasApplicablePass mbLegStations
