...
updateGatesGeneric leg mbPrev mbNext spec transformGate merchantId merchantOpCityId entrances exits = do
  ...
  entrance <- case oldEntrance of
    ...
    Nothing -> do
      ...
      logDebug $ "Got the new entrance\n:" <> show e <> "for leg: " <> show leg.fromStopDetails
  ...
  exit <- case oldExit of
    ...
    Nothing -> do
      ...
      logDebug $ "Got the new exit\n:" <> show e <> "for leg: " <> show leg.toStopDetails
...
fetchStationGatesFromLeg leg merchantOpCityId isEntrance = runMaybeT $ do
  lift $
    logDebug $
  ...
  lift $ logDebug $ "Station found with gates? " <> show (isJust station.gates) <> show stopCode
  gates <- hoistMaybe station.gates
  lift $ logDebug $ "Fetched gates count=" <> show (length gates) <> show stopCode
  pure gates
