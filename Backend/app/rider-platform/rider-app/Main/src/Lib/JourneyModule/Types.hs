...
mkLegInfoFromFrfsSearchRequest FRFSSR.FRFSSearch {..} fallbackFare distance duration = do
  journeyLegInfo' <- journeyLegInfo & fromMaybeM (InvalidRequest "Not a valid mulimodal search as no journeyLegInfo found")
  multiModalConfigs <- CQMMB.findByMerchantOperatingCityId merchantOperatingCityId Nothing
  let bookingAllowed =
        case vehicleType of
          Spec.METRO -> fromMaybe False (multiModalConfigs >>= (.metroBookingAllowed))
          Spec.SUBWAY -> fromMaybe False (multiModalConfigs >>= (.suburbanBookingAllowed))
          _ -> True
  now <- getCurrentTime
...
mkJourney :: MonadFlow m => Id DP.Person -> Maybe UTCTime -> Maybe UTCTime -> Distance -> Seconds -> Id DJ.Journey -> Id DSR.SearchRequest -> Id DM.Merchant -> Id DMOC.MerchantOperatingCity -> [EMInterface.MultiModalLeg] -> Meters -> Meters -> Maybe (Id DRL.RecentLocation) -> Maybe Double -> m DJ.Journey
mkJourney riderId startTime endTime estimatedDistance estiamtedDuration journeyId parentSearchId merchantId merchantOperatingCityId legs maximumWalkDistance straightLineThreshold mbRecentLocationId relevanceScore = do
