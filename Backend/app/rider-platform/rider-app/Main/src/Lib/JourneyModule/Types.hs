    ...
    mkLegExtraInfo qrDataList qrValidity ticketsCreatedAt journeyRouteDetails journeyLegInfo' ticketNo categories categoryBookingDetails commencingHours fareParameters totalBookingAmount integratedBPPConfig = do
      ...
      case booking.vehicleType of
        ...
        Spec.BUS -> do
          ...
          -- Driver name/mobile number only revealed within frfsDriverDetailsLeadTime of departure (PII, gated server-side). Fleet/bus number is not gated.
          mRiderConfig <- getConfig (RiderConfigDimensions {merchantOperatingCityId = booking.merchantOperatingCityId.getId}) Nothing
          let leadWindow = maybe 1800 (\s -> fromIntegral s.getSeconds) (mRiderConfig >>= (.frfsDriverDetailsLeadTime))
          now <- getCurrentTime
