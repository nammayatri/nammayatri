...
getUpdatedLeg journeyLeg' mbNext mbPrev merchantId merchantOpCityId = do
  osmLeg <- updateGatesGeneric journeyLeg' mbNext mbPrev taxiSpec updateGateFromDomain merchantId merchantOpCityId
  updateGatesGeneric osmLeg mbNext mbPrev walkSpec updateGateFromDomain merchantId merchantOpCityId
...
updateGatesGeneric ::
  ...
  Id DMOC.MerchantOperatingCity ->
  m DJourneyLeg.JourneyLeg
updateGatesGeneric leg mbNext mbPrev spec transformGate merchantId merchantOpCityId = do
  let oldEntrance = spec.getEntrance leg
  let oldExit = spec.getExit leg
  entrance <- case oldEntrance of
    Just _ -> pure oldEntrance
    Nothing -> do
      e <- getGate leg.mode (LatLong leg.startLocation.latitude leg.startLocation.longitude) mbNext spec.otherMode merchantId merchantOpCityId
      pure $ transformGate leg.entrance e
  exit <- case oldExit of
    Just _ -> pure oldExit
    Nothing -> do
      e <- getGate leg.mode (LatLong leg.endLocation.latitude leg.endLocation.longitude) mbPrev spec.otherMode merchantId merchantOpCityId
...
getGate :: (CacheFlow m r, EncFlow m r, EsqDBFlow m r, MonadFlow m, HasFlowEnv m r '["kafkaProducerTools" ::: KafkaProducerTools], HasShortDurationRetryCfg r c) => DTrip.MultimodalTravelMode -> LatLong -> Maybe KEMIT.MultiModalLeg -> DTrip.MultimodalTravelMode -> Id DM.Merchant -> Id DMOC.MerchantOperatingCity -> m (Maybe DStation.Gate)
getGate mode point mbOther otherMode merchantId merchantOpCityId = case mbOther of
  Just other | not (generalVehicleTypeToMultiModalTravelMode other.mode `elem` [mode, otherMode]) -> getNearestGateFromLeg mode other point merchantId merchantOpCityId
  _ -> pure Nothing

getNearestGateFromLeg :: (CacheFlow m r, EncFlow m r, EsqDBFlow m r, MonadFlow m, HasFlowEnv m r '["kafkaProducerTools" ::: KafkaProducerTools], HasShortDurationRetryCfg r c) => DTrip.MultimodalTravelMode -> KEMIT.MultiModalLeg -> LatLong -> Id DM.Merchant -> Id DMOC.MerchantOperatingCity -> m (Maybe DStation.Gate)
getNearestGateFromLeg mode leg point merchantId merchantOpCityId = do
  mbGates <- fetchStationGatesFromLeg leg merchantOpCityId
  case mbGates of
...
fetchStationGatesFromLeg :: (CacheFlow m r, EncFlow m r, EsqDBFlow m r, MonadFlow m, HasFlowEnv m r '["kafkaProducerTools" ::: KafkaProducerTools], HasShortDurationRetryCfg r c) => KEMIT.MultiModalLeg -> Id DMOC.MerchantOperatingCity -> m (Maybe [DStation.Gate])
fetchStationGatesFromLeg leg merchantOpCityId = runMaybeT $ do
  lift $
    logDebug $
      "fetchStationGatesFromLeg: legId=" <> show leg
        <> " mode="
        <> show leg.mode
        <> " merchantOperatingCityId="
        <> show merchantOpCityId
        <> " fromStopDetails="
        <> show leg.fromStopDetails
  ...
  lift $ logDebug $ "IntegratedBPPConfigs count=" <> show (length integratedBPPConfigs)
  stopCode <- hoistMaybe (leg.fromStopDetails >>= (.stopCode))
  lift $ logDebug $ "StopCode=" <> show stopCode
  mbStation <-
    MaybeT $
  ...
  lift $ logDebug $ "Station found with gates? " <> show (isJust station.gates)
  gates <- hoistMaybe station.gates
  lift $ logDebug $ "Fetched gates count=" <> show (length gates)
  pure gates
