    ...
    getAllSubwayRoutes = do
      case integratedBppConfig.providerConfig of
        DIntegratedBPPConfig.CRIS crisConfig -> do
          ...
          (_, fares) <- Flow.getFares personId mid mocid integratedBppConfig fareRoute (Utils.becknVehicleCategoryToFrfsVehicleCategory vc) Nothing (Just searchReqId) blacklistedServiceTiers blacklistedFareQuoteTypes True True Nothing
          let filteredFares =
                filter
                  ( \fare ->
...
applyWaybillMetadataToTicket booking mbJourneyLeg meta = do
  ...
  when driverChanged $
    QFRFSTicketBooking.updateFRFSTicketBookingVehicleDataById
      ...
      booking.id
  -- Applied regardless of boarding status: the waybill is the operational source of truth for which
  -- physical vehicle is running this service (e.g. a genuine mid-route bus swap), so a rider's earlier
  -- boarded-on record shouldn't pin it to a bus that's no longer actually there.
  (newBusNumber, newBusTagNumber, busChanged) <- case mbJourneyLeg of
    ...
    Just journeyLeg -> do
      let canRefreshBus = not (T.null meta.vehicle_no)
      ...
      when (busChanged && alreadyBoarded) $
        logError $
          "Waybill reassignment applied after boarding: journeyLeg=" <> journeyLeg.id.getId
            <> " previouslyBoardedOn="
