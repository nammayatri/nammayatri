...
fetchStaticTimings routeCodes stopCode integratedBppConfig mid mocid =
  measureLatency (GRSM.findByRouteCodeAndStopCode integratedBppConfig mid mocid routeCodes stopCode) "fetch route stop timing through graphql"
...
findPossibleRoutes mbAvailableServiceTiers fromStopCode toStopCode currentTime integratedBppConfig mid mocid vc sendWithoutFare = do
  ...
  routeStopTimingsStatic <- case fetchLiveTimingsSource of
    ...
    _ -> case vc of
      Enums.BUS -> measureLatency (fetchStaticTimings validRoutes fromStopCode integratedBppConfig mid mocid) ("fetchStaticTimings" <> show validRoutes <> " fromStopCode: " <> show fromStopCode <> " toStopCode: " <> show toStopCode)
      _ -> pure []
