    ...
    if info.source == LIVE
      then do
        ...
        mbServiceSubTypes <- case mbVehicleId of
          Just vehicleId -> getVehicleServiceSubTypesFromInMem mbRiderConfig integratedBPPConfigs vehicleId
...
getVehicleServiceTypeFromInMem mbRiderConfig integratedBPPConfigs vehicleNumber =
  case (useRedisForVehicleMetadata mbRiderConfig, merchantOperatingCityIdForVehicleMetadata mbRiderConfig integratedBPPConfigs) of
    (True, Just mocId) -> do
      ...
      case mbCached of
        Just (CachedVehicleServiceTier v) -> pure v
        Nothing -> do
          ...
          let v = case res of
                ...
                Nothing -> Nothing
          Hedis.setExp key (CachedVehicleServiceTier v) 43200
          pure v
    _ ->
      IM.withInMemCache ["CACHED_VEHICLE_TYPE", vehicleNumber] 43200 $ do
        res <- getVehicleLiveRouteInfo integratedBPPConfigs vehicleNumber Nothing
        case res of
...
-- | Get tag number for a vehicle, cached in-memory for 12 hours (43200 seconds), or Redis when rider_config.cache_vehicle_metadata is true
...
getVehicleTagNumberFromInMem mbRiderConfig integratedBPPConfigs vehicleNumber =
  case (useRedisForVehicleMetadata mbRiderConfig, merchantOperatingCityIdForVehicleMetadata mbRiderConfig integratedBPPConfigs) of
    (True, Just mocId) -> do
      ...
      mbCached <- Hedis.safeGet key
      case mbCached of
        Just (CachedVehicleTagNumber v) ->
          case sanitizeVehicleTagNumber v of
            Just validTag -> pure $ Just validTag
            Nothing -> do
              mbVehicleLiveRouteInfo <- getVehicleLiveRouteInfo integratedBPPConfigs vehicleNumber Nothing
              let freshTag = sanitizeVehicleTagNumber $ mbVehicleLiveRouteInfo >>= (\(_, info) -> info.busTagNumber)
              Hedis.setExp key (CachedVehicleTagNumber freshTag) 43200
              pure freshTag
        Nothing -> do
          mbVehicleLiveRouteInfo <- getVehicleLiveRouteInfo integratedBPPConfigs vehicleNumber Nothing
          let freshTag = sanitizeVehicleTagNumber $ mbVehicleLiveRouteInfo >>= (\(_, info) -> info.busTagNumber)
          ...
          pure freshTag
    _ ->
      IM.withInMemCache ["CACHED_VEHICLE_TAG_NUMBER", vehicleNumber] 43200 $ do
        mbVehicleLiveRouteInfo <- getVehicleLiveRouteInfo integratedBPPConfigs vehicleNumber Nothing
