module ExternalBPP.Bus.ExternalAPI.CUMTA.Verification where
...
getVerificationDetails :: (MonadTime m, MonadFlow m, CacheFlow m r, EsqDBFlow m r) => CUMTAConfig -> Text -> Integer -> Seconds -> FRFSTicketBooking -> m (Text, UTCTime)
getVerificationDetails _config txnUUID ticketNum qrTtl booking = do
  routeId <-
  ...
  let adultQuantity = booking.quantity
      ...
      ticket = "{tt: [{t: \"" <> fromRoute.providerCode <> "," <> toRoute.providerCode <> "," <> show adultQuantity <> "," <> show childQuantity <> "," <> busTypeId <> "," <> formatUtcTime qrValidityIST <> "," <> txnUUID <> "," <> show amount <> ",,,,," <> show ticketNum <> ",,,\"}]}"
...
generateQR :: (MonadTime m, MonadFlow m, CacheFlow m r, EsqDBFlow m r) => CUMTAConfig -> Text -> Integer -> Seconds -> FRFSTicketBooking -> m (Text, Text, UTCTime, Text)
