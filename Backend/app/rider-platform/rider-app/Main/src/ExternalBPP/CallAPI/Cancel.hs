...
cancel merchant merchantOperatingCity bapConfig cancellationType booking = do
  ...
  let mbServiceTierType = do
        ...
        (routeStations :: [FRFSTicketService.FRFSRouteStationsAPI]) <- decodeFromText rsJson
  ...
  whenJust mbServiceTierType $ \serviceTierType -> do
    ...
    unless (fromMaybe False (mbVst >>= (.isCancellable))) $ throwError CancellationNotSupported
