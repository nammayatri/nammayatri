...
discoverySearch :: FRFSSearchFlow m r => Merchant -> BecknConfig -> IntegratedBPPConfig -> Bool -> Bool -> API.FRFSDiscoverySearchAPIReq -> m ()
discoverySearch merchant bapConfig integratedBPPConfig storeGtfs storeRaw req = do
  ...
  let redisCounter = DOnSearch.DiscoveryCounter {merchantId = merchant.id.getId, pageNo = 0, maxPageNo = 9999, integratedBppConfigId = Just integratedBPPConfig.id.getId, storeGtfs = Just storeGtfs, storeRaw = Just storeRaw}
