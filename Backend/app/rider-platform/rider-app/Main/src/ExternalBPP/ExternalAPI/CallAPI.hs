...
getFares riderId merchant merchanOperatingCity integrationBPPConfig fareRouteDetails vehicleCategory serviceTier mbSearchReqId = do
  ...
  where
    callCRISAPI config' changeOver viaPoints = do
      ...
      resp <- try @_ @SomeException $ if config'.useRouteFareV4 == Just True then CRISRouteFare.getRouteFare config' merchanOperatingCity.id routeFareReq False else CRISRouteFareV3.getRouteFare config' merchanOperatingCity.id routeFareReq True False
      case resp of
        Left err -> do
