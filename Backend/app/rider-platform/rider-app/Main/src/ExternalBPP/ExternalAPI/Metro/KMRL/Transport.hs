...
-- | Managers are cached for the life of the process, keyed by a digest of the material they
-- were built from. Building one is expensive -- a secret decrypt, a PEM parse and a read of
-- the system trust store -- and every 'HTTP.Manager' owns a connection pool and a reaper
-- thread. Rebuilding per call (six call sites) meant no KMRL connection was ever reused: a
-- full mTLS handshake every time, with FD and thread growth under load. Keying on the
-- material rather than the operator means a rotated certificate still yields a new manager.
kmrlManagerCache :: IORef (Map.Map Text HTTP.Manager)
kmrlManagerCache = unsafePerformIO (newIORef Map.empty)
{-# NOINLINE kmrlManagerCache #-}
...
kmrlManager config = do
  ...
  case cached of
    ...
    Nothing -> do
      ...
      -- A racing caller may have inserted first; keep theirs so one manager wins per key.
      liftIO $
        atomicModifyIORef' kmrlManagerCache $ \cache ->
          case Map.lookup cacheKey cache of
            Just existing -> (cache, existing)
            Nothing -> (Map.insert cacheKey manager cache, manager)

digestOf :: Text -> Text
digestOf = show @Text . (CH.hash :: BS.ByteString -> CH.Digest CH.SHA256) . TE.encodeUtf8
