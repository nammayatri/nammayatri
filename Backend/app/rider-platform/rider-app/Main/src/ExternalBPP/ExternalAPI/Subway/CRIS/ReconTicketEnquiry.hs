...
getReconTicketEnquiry config frfsTicketBooking = do
  ...
  fareAmount <- highPrecMoneyToText $ fromMaybeM frfsTicketBooking.price.amount frfsTicketBooking.finalPrice.amount
  person <- QPerson.findById frfsTicketBooking.riderId & fromMaybeM (PersonNotFound frfsTicketBooking.riderId.getId)
  enMobileNumber <- person.mobileNumber & fromMaybeM (PersonFieldNotPresent "mobileNumber")
  mobileNumber <- decrypt enMobileNumber
  ...
  decryptedResponse :: ReconTicketEnquiryDecryptedResponse <- case eitherDecode (encode encryptedResponse) of
    Left err -> throwError (InternalError $ "Failed to parse encrypted getReconTicketEnquiry Resp: " <> T.pack (show err))
    Right encResp -> do
      logInfo $ "getReconTicketEnquiry Resp Code: " <> responseCode encResp
