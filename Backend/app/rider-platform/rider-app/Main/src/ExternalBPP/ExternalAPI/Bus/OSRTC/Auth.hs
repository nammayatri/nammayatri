...
getAuthToken config = do
  ...
  where
    authTokenKey cfg = "OSRTCAuth:Token:" <> cfg.userName <> ":" <> show cfg.platformId <> ":" <> showBaseUrl cfg.baseUrl
    ...
    fetchAndCacheToken cfg = do
      ...
      case resp._data of
        ...
        _ -> throwError $ InternalError $ "OSRTC GenerateAccess failed: returnCode=" <> show resp.returnCode <> " returnMessage=" <> resp.returnMessage
