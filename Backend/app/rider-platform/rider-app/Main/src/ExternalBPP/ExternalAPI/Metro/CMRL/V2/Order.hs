...
createOrder config integratedBPPConfig booking quoteCategories mRiderNumber mbFareQuoteId mbGrpSize mbNoOfTickets = do
  ...
  ticketTypeId <-
    if grpSize > 1 && FRFSSellerCommon.metroJourneyCode booking._type == FRFSSellerCommon.SJT
      then
        (HM.lookup groupTicketTypeKey =<< config.ticketTypeIds)
          & fromMaybeM (InternalError $ "CMRLV2 needs a " <> groupTicketTypeKey <> " entry in ticketTypeIds to put " <> show grpSize <> " passengers on one QR")
      else pure (ticketTypeIdFor config booking._type)
