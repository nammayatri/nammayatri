...
-- | The zone CDAC's timestamps are already in. It is attached to a parsed LocalTime, never
-- used to shift one: the operator sends bare wall-clock strings (see 'operatorTimeFormat'),
-- so labelling 05:30 as +05:30 is correct and adding 5h30m would publish opening hours five
-- and a half hours late.
operatorTimeZone :: TimeZone
operatorTimeZone = TimeZone (5 * 60 + 30) False "IST"
...
renderTagValue tagCode istTime
  ...
  | otherwise = T.pack $ formatTime defaultTimeLocale publishedTimeFormat (ZonedTime istTime operatorTimeZone)
