...
confirmAndUpsertBooking personId quote selectedQuoteCategories crisSdkResponse isSingleMode mbIsMockPayment integratedBppConfig mbTripId isSpotBooking mbVehicleNumber mbPurchasedPassPaymentId = do
  Hedis.withWaitAndLockMasterCloudCrossAppRedis (mkConfirmLockKey quote.searchId.getId) confirmLockTtlSec confirmLockRetryDelayMicros $ do
    ...
    -- The hold is taken above but only tracked against a booking below, so anything that throws in
    -- between -- an inapplicable pass, a BPP error, a bad quote -- would leave those seats held
    -- until their TTL. Release before rethrowing, or repeated failed attempts block the trip.
    ...
    (rider, dConfirmRes) <- case confirmResult of
      ...
      Left err -> do
        whenJust ((,) <$> mbTripId <*> mbHoldId) $ \(tripId, holdId) -> do
          logWarning $ "FRFSConfirm:confirmAndUpsertBooking releasing hold after failure holdId=" <> holdId <> " tripId=" <> tripId <> " err=" <> show err
  ...
  where
    ...
    withPaymentStatusResponseHandler booking quoteCategories fareParameters routeStations stations merchantOperatingCity action = do
      ...
      case (mbPaymentBooking, mbPaymentOrder) of
        ...
        _ -> do
          -- Master, not replica: the pass-covered branch above wrote CONFIRMING, validTill and
          -- possibly FAILED in this same request, and a replica read can still return the row as it
          -- was before them, reporting a status this request has already moved past.
          latestBooking <- QFRFSTicketBooking.findById booking.id >>= fromMaybeM (InvalidRequest "Invalid booking id")
