...
confirmAndUpsertBooking personId quote selectedQuoteCategories crisSdkResponse isSingleMode mbIsMockPayment integratedBppConfig mbTripId = do
  ...
  (selectedQuoteCategoriesFinal, mbHoldCtxForAll) <-
    if shouldAutoAssignBusSeats
      then do
        let tripId = fromMaybe "" mbTripId -- safe: shouldAutoAssignBusSeats guarantees isJust
            requiredSeatCount = Kernel.Prelude.sum ((.quantity) <$> selectedQuoteCategories)
        if requiredSeatCount <= 0
          then pure (clearSeatIds selectedQuoteCategories, Nothing)
            ...
            case mbSeatHoldParams of
              ...
              Just params -> do
                let maxAttempts = 3
                orderedSeatIds <- getOrderedSeatIds tripId params.shpFromIdx params.shpToIdx quote integratedBppConfig
  ...
  where
    ...
    selectAndHoldWithRetries tripId orderedSeatIds fromIdx toIdx defaultTtl seatBitMapTtl requiredSeatCount maxAttempts =
      ...
      where
        go attempt = do
          ...
          holdId <- generateGUID
          logInfo $ "FRFSConfirm:confirmAndUpsertBooking Odisha auto-seat attempt=" <> show (attempt + 1) <> "/" <> show maxAttempts <> " tripId=" <> tripId <> " seatCount=" <> show requiredSeatCount
          success <- SeatBooking.holdSeats tripId chosen fromIdx toIdx holdId defaultTtl seatBitMapTtl
          if success
            then do
              logInfo $ "FRFSConfirm:confirmAndUpsertBooking Odisha auto-seat SUCCESS tripId=" <> tripId <> " holdId=" <> holdId <> " seatIds=" <> show (map (.getId) chosen)
              pure (chosen, holdId)
            else
              if attempt + 1 >= maxAttempts
                then do
                  logWarning $ "FRFSConfirm:confirmAndUpsertBooking Odisha auto-seat FAILED tripId=" <> tripId <> " attempts=" <> show maxAttempts
    ...
    getOrderedSeatIds ::
      ...
      m [Id Seat.Seat]
    getOrderedSeatIds tripId fromIdx toIdx quote' integratedBppConfig' = do
      vehicleNo <- quote'.vehicleNumber & fromMaybeM (InvalidRequest "Vehicle number not found")
      seatLayoutMapping <- CQVehicleSeatLayoutMapping.findByVehicleNoAndGtfsIdCached vehicleNo integratedBppConfig'.feedKey >>= fromMaybeM (InvalidRequest "Seat layout mapping not found for vehicle")
