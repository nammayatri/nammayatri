...
-- Booking creation is serialised per leg search by the lock below (from fix_booking_creation_race).
-- Pass overrides depend on it: a fully covered booking skips payment entirely, so a duplicate would
-- become a second real operator ticket. See .cursor/docs/20-frfs-pass-fare-override.md
confirmAndUpsertBooking :: (CallExternalBPP.FRFSConfirmFlow m r c, HasField "cloudType" r (Maybe CloudType)) => Id Domain.Types.Person.Person -> DFRFSQuote.FRFSQuote -> [API.Types.UI.FRFSTicketService.FRFSCategorySelectionReq] -> Maybe CrisSdkResponse -> Maybe Bool -> Maybe Bool -> DIBC.IntegratedBPPConfig -> Maybe Text -> Maybe Bool -> Maybe Text -> Maybe (Id DPPP.PurchasedPassPayment) -> m (Domain.Types.Person.Person, DFRFSTicketBooking.FRFSTicketBooking, FRFSUtils.FRFSFareParameters, [FRFSQuoteCategory.FRFSQuoteCategory], Bool)
confirmAndUpsertBooking personId quote selectedQuoteCategories crisSdkResponse isSingleMode mbIsMockPayment integratedBppConfig mbTripId isSpotBooking mbVehicleNumber mbPurchasedPassPaymentId = do
  ...
  where
    ...
    buildAndCreateBooking rider quote'@DFRFSQuote.FRFSQuote {..} fareParameters mbMockPayment mbHoldCtxForAll firstTripId mbSeatSelectionType isSpotBooking' mbPurchasedPassPaymentId' = do
      ...
      -- Bus resolves a real waybill departure; metro/subway have no schedule and leave
      -- firstTripId Nothing, so they always fall through. The journey leg's departure comes next
      -- because that is the instant search offered passes against -- a leg departing tomorrow
      -- would otherwise be re-validated against today and its pass rejected -- and only then now.
      ...
      let adultUnitPriceForOverride =
            fromMaybe fareParameters.totalPrice $
          ...
          Just paymentId -> do
            resolved <-
              FRFSPassOverride.resolvePassOverride
                ...
                paymentId
            -- The rider explicitly chose to pay with this pass. Silently falling back to the full
            -- fare and creating a payment order is the wrong answer to every rejection reason.
      ...
      -- The row is created without the override fields and stamped only after the trip is
      -- actually spent, so the fields exist iff a trip was taken. Stamping first would let a
      -- failed consume leave a booking that a release path would later refund a trip it never
      -- spent; consuming first would burn a trip with no row naming it.
      bookingWithOverride <- case mbResolved of
        Nothing -> pure booking
        Just (applicablePass, passOption) -> do
          consumed <- FRFSPassOverride.consumeTripOnce booking.searchId applicablePass.purchasedPassPayment
          case consumed of
            ...
            -- Unreachable once the search lock holds; this is the backstop for when it does not
            -- (Redis down, lock expired under a slow holder). Fails rather than falling back to
            -- full fare: inside a journey an un-overridden booking is not isFullyPassCovered, so
            -- payableBookings picks it up and folds it into the journey's single payment order --
            -- charging the rider full fare for a leg where they explicitly chose a pass. Same
            -- reasoning as resolvePassOverride: a silent full-fare fallback is the wrong answer to
      ...
      return (rider, bookingWithOverride)
...
postFrfsQuoteV2ConfirmUtil :: (CallExternalBPP.FRFSConfirmFlow m r c, HasField "blackListedJobs" r [Text], HasField "cloudType" r (Maybe CloudType), HasMasterCloudForwarder r) => (Kernel.Prelude.Maybe (Kernel.Types.Id.Id Domain.Types.Person.Person), Kernel.Types.Id.Id Domain.Types.Merchant.Merchant) -> DFRFSQuote.FRFSQuote -> [API.Types.UI.FRFSTicketService.FRFSCategorySelectionReq] -> Maybe CrisSdkResponse -> Maybe Bool -> Maybe Bool -> Maybe Bool -> DIBC.IntegratedBPPConfig -> Maybe Text -> Maybe Bool -> Maybe Text -> Maybe (Id DPPP.PurchasedPassPayment) -> m API.Types.UI.FRFSTicketService.FRFSTicketBookingStatusAPIRes
postFrfsQuoteV2ConfirmUtil (mbPersonId, merchantId_) quote selectedQuoteCategories crisSdkResponse isSingleMode mbEnableOffer mbIsMockPayment integratedBppConfig mbTripId isSpotBooking mbVehicleNumber mbPurchasedPassPaymentId = do
  ...
  (rider, dConfirmRes, fareParameters, updatedQuoteCategories, isMultiInitAllowed) <- confirmAndUpsertBooking personId quote selectedQuoteCategories crisSdkResponse isSingleMode mbIsMockPayment integratedBppConfig mbTripId isSpotBooking mbVehicleNumber mbPurchasedPassPaymentId
  ...
  -- The NEW check is what the isMultiInitAllowed gate does for the branch below, and this path
  -- needs it for the same reason. A covered booking has no init step, so NEW is the only state it
  -- can legitimately be confirmed from. Without this, a second confirm that reuses an existing
  -- booking -- the search lock makes the loser do exactly that -- would call the operator again for
  -- a booking already CONFIRMING, and whether that yields a duplicate ticket depends on how
  -- idempotent the operator is on the order id, which is not ours to rely on.
  if isFullyPassCovered && dConfirmRes.status == DFRFSTicketBooking.NEW
    then do
      bapConfig <-
        getOneConfig
          (BecknConfigDimensions {merchantOperatingCityId = merchantOperatingCity.id.getId, merchantId = merchant.id.getId, domain = Just (show Spec.FRFS), vehicleCategory = Just (frfsVehicleCategoryToBecknVehicleCategory dConfirmRes.vehicleType)})
  ...
  where
    ...
    withPaymentStatusResponseHandler booking quoteCategories fareParameters routeStations stations merchantOperatingCity action = do
      ...
      case (mbPaymentBooking, mbPaymentOrder) of
        ...
        -- Re-fetched: `booking` here is the snapshot taken before the fully-covered branch above
        -- moved it to CONFIRMING, so returning it verbatim reports status = NEW on a successful
        -- pass-covered confirm.
        _ -> do
