...
specialLocationListClient :: Text -> Context.City -> Maybe Int -> Maybe Int -> [SL.SpecialLocationType] -> EulerClient [SpecialLocationFull]
...
getSpecialLocationList merchant city mbLimit mbOffset mbSpecialLocationTypes = do
  ...
  let merchantId = merchant.driverOfferMerchantId
  internalEndPointHashMap <- asks (.internalEndPointHashMap)
  result <- try @_ @SomeException $ EC.callApiUnwrappingApiError (identity @Error) Nothing (Just "BPP_INTERNAL_API_ERROR") (Just internalEndPointHashMap) internalUrl (specialLocationListClient merchantId city mbLimit mbOffset (fromMaybe [] mbSpecialLocationTypes)) "SpecialLocationList" specialLocationListApi
  case result of
    Left _ -> pure Nothing
