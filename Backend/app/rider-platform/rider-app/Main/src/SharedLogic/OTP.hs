...
sendOTP ::
  ...
  Maybe Text -> -- Mobile country code (required for SMS/WHATSAPP)
  Maybe Text -> -- Mobile number (required for SMS/WHATSAPP)
  Maybe Text -> -- Email (required for EMAIL)
  Maybe EmailTypes.EmailOTPConfig -> -- Email config (required for EMAIL)
  m ()
sendOTP otpChannel otpCode personId merchantId merchantOperatingCityId mbCountryCode mbMobileNumber mbEmail mbEmailOTPConfig = do
  smsCfg <- asks (.smsCfg)
