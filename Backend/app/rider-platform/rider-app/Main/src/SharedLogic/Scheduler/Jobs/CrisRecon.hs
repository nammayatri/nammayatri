...
crisReconJob ::
  ...
  Job 'CrisRecon ->
  m ExecutionResult
crisReconJob Job {id, jobInfo, merchantId, merchantOperatingCityId} = withLogTag ("JobId-" <> id.getId) do
  ...
  recentFailedBookings <- QFRFSTicketBooking.findAllByProviderNameAndCreatedAtAfterAndStatus "CRIS" oneHourAgo FRFSTicketBookingStatus.FAILED
  ...
  crisConfig <-
    case recentFailedBookings of
      [] -> do
        ...
        return Complete
      (booking : _) -> do
        integratedBppConfig <- QIntegratedBPPConfig.findById booking.integratedBppConfigId >>= fromMaybeM (throwError $ InternalError "IntegratedBPPConfig not found")
        case integratedBppConfig.providerConfig of
          CRISConfig config -> do
            return $ config
          _ -> do
  ...
  (failedOnBothSidesList, failedButSuccessForCrisList) <-
    foldM
      ( \(failedOnBothSides, failedButSuccessForCris) booking -> do
          result <- try (getReconTicketEnquiry crisConfig booking)
          case result of
            Left err -> do
  ...
  -- Reschedule this job to run again in 1 hour
