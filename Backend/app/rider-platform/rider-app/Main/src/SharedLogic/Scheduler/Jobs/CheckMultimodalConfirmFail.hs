...
import Lib.JourneyModule.Base as JM
...
checkMultimodalConfirmFailJob Job {id, jobInfo} = withLogTag ("JobId-" <> id.getId) do
  ...
  if (booking.status == DFRFSTicketBooking.FAILED || null frfsTickets)
    then do
      ...
      let allFailed = all ((== DFRFSTicketBooking.FAILED) . (.status)) allJourneyFrfsBookings
      unless allFailed $ JM.markAllBookingsFailed allJourneyFrfsBookings booking.riderId journeyId
      return Complete
