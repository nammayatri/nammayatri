...
paymentOrderStatusCheckJob Job {id, jobInfo} = withLogTag ("JobId-" <> id.getId) do
  ...
  allRecentOrders <- QPaymentOrder.findAllByCreatedAtAfter oneDayAgo
  let failedPaymentOrderStatuses = [Payment.AUTHENTICATION_FAILED, Payment.AUTHORIZATION_FAILED, Payment.JUSPAY_DECLINED, Payment.CLIENT_AUTH_TOKEN_EXPIRED]
  recentNotFailedOrNotChargedOrders <-
    filterM
      ( \order ->
          if order.status `elem` failedPaymentOrderStatuses
            then return False
            else case order.status of
              Payment.CHARGED -> do
  ...
  mapM_
    ( \paymentOrder -> do
        result <- try @_ @SomeException $ processPaymentOrder merchantId' merchantOperatingCityId' paymentOrder
        case result of
          Left err -> do
