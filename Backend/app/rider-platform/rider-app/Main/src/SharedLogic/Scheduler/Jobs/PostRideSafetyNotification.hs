...
postRideSafetyNotification Job {id, jobInfo} = withLogTag ("JobId-" <> id.getId) do
  ...
  when (isNothing ride.wasRideSafe && isNothing ride.rideRating) $ do
    ...
    let scheduleAfter = riderConfig.ivrTriggerDelay
        ...
        expireAt = addUTCTime (scheduleAfter + riderConfig.ivrTriggerExpireTime) curentTime
    createJobIn @_ @'SafetyIVR ride.merchantId ride.merchantOperatingCityId scheduleAfter (Just expireAt) (safetyIvrJobData :: SafetyIVRJobData)
