...
sendCustomerRefund Job {id, jobInfo} = withLogTag ("JobId-" <> id.getId) do
  ...
  payoutConfig <- getOneConfig (PayoutDimensions {merchantOperatingCityId = merchantOpCityId.getId, vehicleCategory = Nothing, isPayoutEnabled = Just True, payoutEntity = Just METRO_TICKET_CASHBACK})
        ...
        Just payoutVpa -> do
          Redis.withWaitOnLockRedisWithExpiry (DAP.payoutProcessingLockKey booking.id.getId) 3 3 $ do
            ...
            let amount = fromMaybe 0 booking.eventDiscountAmount
                ...
                createPayoutOrderReq = Payout.mkCreatePayoutServiceReq uid amount currency phoneNo emailId person.id.getId config.remark person.firstName payoutVpa config.orderType True
            ...
            let createPayoutOrderCall = TP.createPayoutOrder person.clientSdkVersion person.merchantId person.merchantOperatingCityId (Just person.id.getId) paymentMode
