      ...
      case booking.payerVpa of
        Just payoutVpa -> do
          Redis.withWaitOnLockRedisWithExpiry (DAP.payoutProcessingLockKey booking.id.getId) 3 3 $ do
            ...
            mbPayoutOrderResp <- try @_ @SomeException $ Payout.createPayoutService (cast merchantId) (cast person.id) (Just [booking.id.getId]) (Just entityName) Nothing (show merchantOperatingCity.city) createPayoutOrderReq createPayoutOrderCall
