...
confirm DConfirmReq {..} = do
  ...
  where
    ...
    checkOverlap estimatedDistance estimatedDuration curBookingStartTime booking = do
      ...
      max estRideEndTimeByDuration estRideEndTimeByDist < addUTCTime (intToNominalDiffTime $ fromMaybe 0 ((.getSeconds) <$> booking.estimatedDuration)) curBookingStartTime
