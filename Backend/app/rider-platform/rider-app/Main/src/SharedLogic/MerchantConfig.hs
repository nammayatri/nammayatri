...
updateTotalRidesCounters :: (HedisFlow m r, KvDbFlow m r, EsqDBReplicaFlow m r) => Id Person.Person -> m ()
...
getTotalRidesCount :: (HedisFlow m r, KvDbFlow m r, EsqDBReplicaFlow m r) => Id Person.Person -> m Int
...
getRidesCountInWindow ::
  (HedisFlow m r, KvDbFlow m r, EsqDBReplicaFlow m r) =>
  Id Person.Person ->
  Int ->
  Int ->
  UTCTime ->
...
anyFraudDetected :: (KvDbFlow m r, EsqDBReplicaFlow m r) => Id Person.Person -> Id DMOC.MerchantOperatingCity -> [DMC.MerchantConfig] -> m (Maybe DMC.MerchantConfig)
...
checkFraudDetected :: (KvDbFlow m r, EsqDBReplicaFlow m r) => Id Person.Person -> Id DMOC.MerchantOperatingCity -> [Factors] -> [DMC.MerchantConfig] -> m (Maybe DMC.MerchantConfig)
