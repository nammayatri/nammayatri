...
createPaymentOrder bookings merchantOperatingCityId merchantId amount person paymentType vendorSplitArr basket isMockPayment
  -- Callers must read Nothing as "nothing to pay", not as a failure: a fully covered fare
  -- leaves no order to create and no split to make.
  | amount <= 0 = do
    ...
    then do
      ...
      case allJourneyBookings of
        [booking] -> do
          ...
          -- Priced at what the rider is actually charged, not the quoted fare. The SKU decides
          -- which offer matches; this decides what it computes against. See .cursor/docs/20-frfs-pass-fare-override.md
          ...
          let fareParameters = mkFareParameters (mkCategoryPriceItemFromQuoteCategories quoteCategories)
              ...
              -- One line per category present, not just ADULT/CHILD -- overriddenAmount sums every
              -- priceItem. no_basket_<category> ids are UNVERIFIED against Juspay; see .cursor/docs/20-frfs-pass-fare-override.md
              ...
              -- Pass bookings only get the per-category treatment. A non-pass booking keeps the
              -- ADULT/CHILD pair it has always had -- that basket is short of the order for any
              -- booking carrying SENIOR_CITIZEN/STUDENT/FEMALE/MALE, but that is pre-existing and
              -- not this feature's to change. Widening it would also start sending
              -- no_basket_<category> ids, which are untested against the gateway, on traffic that
              -- has nothing to do with passes.
              baskets
          ...
          -- Must sum to what the order charges. A mismatch means the stored overriddenAmount no
          -- longer matches the config it came from; fall back so the amount stays right. See .cursor/docs/20-frfs-pass-fare-override.md
          if null baskets
