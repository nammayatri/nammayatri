...
mkPOrgStationAPI mbPOrgId merchantOperatingCityId vehicleType stationAPI = do
  integratedBPPConfig <- QIBC.findByDomainAndCityAndVehicleCategory (show Spec.FRFS) merchantOperatingCityId (frfsVehicleCategoryToBecknVehicleCategory vehicleType) (Just DIBC.APPLICATION) >>= fromMaybeM (InternalError "integratedBPPConfig is missing")
  station <- B.runInReplica $ CQS.findByStationCodeAndMerchantOperatingCityId stationAPI.code merchantOperatingCityId integratedBPPConfig.id >>= fromMaybeM (StationNotFound $ "station code:" +|| stationAPI.code ||+ "and integratedBPPConfigId: " +|| integratedBPPConfig.id.getId ||+ "")
...
trackVehicles personId merchantId merchantOpCityId vehicleType routeCode = do
  integratedBPPConfig <- QIBC.findByDomainAndCityAndVehicleCategory (show Spec.FRFS) merchantOpCityId (frfsVehicleCategoryToBecknVehicleCategory vehicleType) (Just DIBC.APPLICATION) >>= fromMaybeM (InternalError "integratedBPPConfig is missing")
