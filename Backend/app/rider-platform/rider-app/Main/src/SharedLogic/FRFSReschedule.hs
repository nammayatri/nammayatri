...
completeReschedule oldBookingId stagingBookingId = do
  ...
  unless (oldBooking.status == DFRFSTicketBookingStatus.RESCHEDULED) $ do
    ...
    payment <-
      QFRFSTicketBookingPayment.findTicketBookingPayment stagingBooking
        >>= fromMaybeM (InvalidRequest "Reused payment not found while completing reschedule")
    ...
    newTickets <- QTicket.findAllByTicketBookingId stagingBookingId
