...
module SharedLogic.FraudConfig
...
updateSearchFraudCounters :: (CacheFlow m r, MonadFlow m) => Id Person.Person -> [DFC.FraudConfig] -> m ()
...
updateCancelledByDriverFraudCounters :: (CacheFlow m r, MonadFlow m) => Id Person.Person -> [DFC.FraudConfig] -> m ()
...
updateCustomerFraudCounters :: (CacheFlow m r, MonadFlow m) => Id Person.Person -> [DFC.FraudConfig] -> m ()
...
updateTotalRidesInWindowCounters :: (CacheFlow m r, MonadFlow m) => Id Person.Person -> [DFC.FraudConfig] -> m ()
...
anyFraudDetected :: (CacheFlow m r, MonadFlow m, EsqDBFlow m r, EsqDBReplicaFlow m r) => Id Person.Person -> Id DM.Merchant -> [DFC.FraudConfig] -> m (Maybe DFC.FraudConfig)
...
checkFraudDetected :: (CacheFlow m r, EsqDBFlow m r, EsqDBReplicaFlow m r) => Id Person.Person -> Id DM.Merchant -> [Factors] -> [DFC.FraudConfig] -> m (Maybe DFC.FraudConfig)
...
blockCustomer :: (CacheFlow m r, MonadFlow m, EsqDBFlow m r) => Id Person.Person -> Maybe (Id DFC.FraudConfig) -> m ()
