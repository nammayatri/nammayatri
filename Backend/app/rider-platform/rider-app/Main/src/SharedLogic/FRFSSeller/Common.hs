...
-- | Sentinel rider for seller bookings. A seller booking has no LOCAL rider -- the buyer is
-- an external BAP with no Person row here -- so riderId carries this well-known synthetic id
-- instead. It is not a UUID and matches no real person, which is deliberate: buyer "my
-- bookings" queries filter by a real riderId and so naturally exclude seller rows. Do NOT
-- "fix" this into a nullable column; that ripples across every buyer path for no gain.
-- 'isSellerRider' is the only reader.
...
-- | The seller merchant's short_id is the operator slug, upper-cased. Kept as a helper so
-- the one place that builds it stays findable; the buyer-vs-seller distinction is carried by
-- the @is_frfs_seller@ flag (see 'findSellerMerchant'), not by a name prefix.
operatorMerchantShortId :: Text -> ShortId a
operatorMerchantShortId slug = ShortId $ T.toUpper slug

-- | Resolve the seller-side merchant for an operator slug. A merchant row only counts as the
-- seller identity when @is_frfs_seller@ is set: this fails closed, so a slug that happens to
-- match some other (buyer/app) merchant of the same name is not mistaken for a seller. A NULL
-- flag on legacy rows reads as not-a-seller.
findSellerMerchant :: (CacheFlow m r, EsqDBFlow m r, MonadFlow m) => Text -> m (Maybe DM.Merchant)
findSellerMerchant slug =
  mfilter (fromMaybe False . (.isFrfsSeller)) <$> CQM.findByShortId (operatorMerchantShortId slug)
...
-- | The ttl we stamp on a callback. beckn_config stores these per action in SECONDS; the
-- wire wants an ISO-8601 duration. An unset column keeps the 5 minutes the seller used to
-- hardcode for every action.
-- | TRV11 core version we stamp on callbacks. Configurable per operator like the NTS10
-- one; unset keeps the network's current 2.0.0.
contextVersionOf :: Maybe OperatorConfig -> Text
contextVersionOf = maybe "2.0.0" (.contextVersion)

callbackTtl :: Maybe Int -> Text
callbackTtl = maybe "PT300S" BUtils.computeTtlISO8601
...
-- | Deterministic per (search, item) so a buyer re-searching the same transaction updates
-- its quote rows instead of laying down duplicates that findQuote would pick between.
sellerQuoteId :: Text -> Text -> Text -> Text
sellerQuoteId operator transactionId itemId =
  UU.toText . UUV5.generateNamed UU.nil . BS.unpack . TE.encodeUtf8 $
    "frfsSeller:quote:" <> operator <> ":" <> transactionId <> ":" <> itemId
