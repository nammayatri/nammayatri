...
import Tools.Error (BookingError (BookingDoesNotExist), GenericError (InvalidRequest), MerchantError (MerchantOperatingCityNotFound))
    ...
    { authenticate = \phone ->
      ...
      searchPlaces = \_auth query mBias ->
        runBot "searchPlaces" $ do
          (centerLat, centerLon) <- case mBias of
            Just ll -> pure (ll.lat, ll.lon)
            Nothing -> do
              moc <- CQMOC.findById mocId >>= fromMaybeM (MerchantOperatingCityNotFound mocId.getId)
              pure (moc.lat, moc.long)
          let locationText = show centerLat <> "," <> show centerLon
          resp <-
      ...
      checkServiceable = \auth ll ->
        runBot "checkServiceable" $ do
          -- NOTE: checks the merchant's whole geofencingConfig (regular
          -- ride-hailing zone), not a flexi-specific narrower area — same
          -- boundary the main app's own search-serviceability check uses.
          res <- DServiceability.checkServiceability (.origin) (Id auth.personId, merchantId) (Maps.LatLong ll.lat ll.lon) False True
          pure res.serviceable,
