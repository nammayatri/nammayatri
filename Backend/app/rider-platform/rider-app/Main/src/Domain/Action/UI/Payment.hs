...
--   is actually reused. The shortfall is recomputed here
...
createBookingDepositPaymentOrder booking = do
  ...
  where
            ...
            Nothing -> do
              ...
              if orderIsDead
                then do
                  ...
                  -- Still live: leave the row PENDING so the same order is reused next call.
    ...
    callCreateOrder orderIdText amount = do
      ...
      isMetroTestTransaction <- asks (.isMetroTestTransaction)
      let createWalletCall = TWallet.createWallet booking.merchantId merchantOperatingCityId
      DPayment.createOrderService commonMerchantId (Just $ cast merchantOperatingCityId) commonPersonId mbPaymentOrderValidity Nothing DOrder.BookingDeposit isMetroTestTransaction createOrderReq createOrderCall (Just createWalletCall) False Nothing False
