...
mkJourneyForSearch searchRequest estimate personId = do
  now <- getCurrentTime
  alreadyCreatedJourneys <- QJourney.findBySearchId searchRequest.id
  case alreadyCreatedJourneys of
    (journey : _) -> pure journey.id
    [] -> do
      ...
      case journeyId of
        ...
        Nothing -> do
          -- create a entry into journeyLegInfo and journey table
          ...
          -- Get fare information from estimate
          ...
          -- Create journey leg entry with fare information
          let journeyLeg =
                DJL.JourneyLeg
                  { id = journeyLegGuid,
          ...
          -- Create JourneySearchData for the search request
