...
getPickupRoutes (personId, merchantId) entityId GetPickupRoutesReq {..} = do
  ...
  merchantConfig <- getConfig (MerchantServiceUsageConfigDimensions {merchantOperatingCityId = mocId.getId}) (Just (CQMSUC.findByMerchantOperatingCityId mocId)) >>= fromMaybeM (MerchantServiceUsageConfigNotFound mocId.getId)
