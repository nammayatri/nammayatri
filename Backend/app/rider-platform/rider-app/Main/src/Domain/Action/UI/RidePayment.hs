...
getPaymentGetDueAmount (mbPersonId, _) = do
  ...
  case mbLatestRide of
    ...
    Just ride -> do
      -- 2. Get all FAILED PAYMENT invoices for this ride
      allInvoices <- runInReplica $ QPaymentInvoice.findAllByRideId ride.id
      let failedPaymentInvoices = filter isFailedPaymentInvoice allInvoices

      -- 3. Apply filters: exclude settled, DEBT_SETTLEMENT, and config-based excluded purposes
      let settledExcluded = filter (isNothing . (.settledByInvoiceId)) failedPaymentInvoices
      let debtSettlementExcluded = filter ((/= DPI.DEBT_SETTLEMENT) . (.paymentPurpose)) settledExcluded
      let excludedPurposeFilter = filter (not . isExcludedPurpose excludedPurposes) debtSettlementExcluded
      let filteredInvoices = excludedPurposeFilter

      -- Audit logging for debugging
      let numSettled = length $ filter (isJust . (.settledByInvoiceId)) failedPaymentInvoices
      let numDebtSettlement = length settledExcluded - length debtSettlementExcluded
      let numExcludedByConfig = length debtSettlementExcluded - length excludedPurposeFilter
      when (numSettled > 0 || numDebtSettlement > 0 || numExcludedByConfig > 0) $
        logInfo $
          "Get Dues filtering for ride " <> ride.id.getId <> ": "
            ...
            <> show numSettled
            <> ", debt_settlement="
            <> show numDebtSettlement
            <> ", excluded_by_config="
            <> show numExcludedByConfig
            <> ", final_due="
  ...
  where
    isFailedPaymentInvoice inv =
      inv.invoiceType == DPI.PAYMENT && inv.paymentStatus == DPI.FAILED

    isExcludedPurpose excludedPurposes inv = inv.paymentPurpose `elem` excludedPurposes

    returnNoDues riderId = do
      -- Derive currency from person's most recent booking, fallback to INR
      -- findLatestByRiderId takes (limit, offset, riderId); use limit 1 for latest
      bookings <- runInReplica $ QBooking.findLatestByRiderId (Just 1) Nothing riderId
      let mbBooking = listToMaybe bookings
      let defaultCurrency = maybe INR (\b -> b.estimatedFare.currency) mbBooking
      pure $
        API.Types.UI.RidePayment.GetDueAmountResp
          { rides = [],
            totalDueAmount = 0,
...
postPaymentClearDues (mbPersonId, merchantId) req = do
  ...
  debtSettlementInvoice <-
    buildDebtSettlementInvoice
      ...
      duesResp.totalDueAmount duesResp.currency
      paymentMethod
  ...
  logInfo $
    "Creating DEBT_SETTLEMENT invoice " <> debtSettlementInvoice.id.getId <> " for ride " <> rideId.getId
      ...
      <> ", parent_invoices: "
      <> show (map (.getId) $ map (.id) parentInvoices)
