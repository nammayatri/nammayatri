...
module Domain.Action.UI.CleverTap
  ( GetCleverTapDataRes (..),
    AppUseCase (..),
    getCleverTapData,
  )
...
data GetCleverTapDataRes = GetCleverTapDataRes
  { riderId :: Id DP.Person,
    ...
    -- latestH3SourceCluster :: Text,
    -- latestH3DestCluster :: Text,
    emergencyContactsNum :: Int,
    overalCancellationRate :: Float,
    favoriteLocationsNum :: Int,
    morningPeakRidesRate :: Float,
    eveningPeakRidesRate :: Float,
...
getCleverTapData :: (EsqDBReplicaFlow m r, EncFlow m r) => (Id Person.Person, Id Merchant.Merchant) -> m GetCleverTapDataRes
getCleverTapData (personId, _) = do
  ...
  return $
    GetCleverTapDataRes
      { riderId = person.id,
        ...
        -- latestH3SourceCluster  = undefined,
        -- latestH3DestCluster  = undefined,
  ...
  where
    days30 :: NominalDiffTime = 30 * 24 * 60 * 60 -- 30 days
