...
deletePickupinstructions (mbPersonId, _) mbLat mbLon mbTarget = do
  ...
  case mbPickupInstruction of
    Just pickupInstruction -> do
      ...
      let closestInstruction = pickupInstruction
          distanceToClosest = 0.0 -- Exact geohash match
      case target of
        API.Instruction -> do
    ...
    Nothing -> do
      -- No exact geohash match found, fall back to proximity search
      logDebug $ "PickupInstructions: No exact geohash match found, searching nearby instructions"
      ...
      if null pickupInstructions
        then do
          ...
          -- Find instructions within proximity by decoding geohashes and calculating distance
          let instructionsWithDistance =
                mapMaybe
                  ( \instruction -> do
              ...
              -- Filter to instructions within 10m proximity (reasonable for pickup spots)
              proximityInstructions = filter (\(_, distance) -> distance <= 10.0) instructionsWithDistance
              -- Sort by distance and get the closest one
              sortedInstructions = List.sortBy (comparing snd) proximityInstructions

          case sortedInstructions of
            [] -> do
              logDebug "PickupInstructions: No instructions found within 10m proximity"
              throwError $ InvalidRequest "No pickup instructions found near the specified location"
            ((closestInstruction, distanceToClosest) : _) -> do
              logDebug $ "PickupInstructions: Found closest instruction at distance " <> show distanceToClosest <> "m, id: " <> show closestInstruction.id.getId
