...
updateDisability hasDisability mbDisability personId = do
  case (hasDisability, mbDisability) of
    ...
    (Just True, Just selectedDisability) -> do
      ...
      disability <- runInReplica $ QD.findByDisabilityId disabilityId >>= fromMaybeM (PersonNotFound disabilityId)
      let mbDescription = fromMaybe (Just disability.description) (Just selectedDisability.description)
