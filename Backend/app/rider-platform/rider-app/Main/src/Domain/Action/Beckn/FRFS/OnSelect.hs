...
onSelect onSelectReq merchant quote isSingleMode = do
  ...
  let categorySelectionReq =
        mapMaybe
          ...
          quoteCategories
  void $ FRFSTicketService.postFrfsQuoteV2ConfirmUtil (Just quote.riderId, merchant.id) quote.id (FRFSQuoteConfirmReq {offered = categorySelectionReq, ticketQuantity = Nothing, childTicketQuantity = Nothing}) Nothing isSingleMode
