...
onInit onInitReq merchant oldBooking quoteCategories mbEnableOffer = do
  ...
  allSlotsCreated <- case bookingPaymentGroup of
    ...
    _ -> pure True
  when (allLegsOnInitDone && allSlotsCreated) $ do
    Redis.withLockRedis (key (bookingPaymentGroupKey booking.id.getId bookingPaymentGroup)) 60 $ do
      ...
      createPayments allJourneyBookings oldBooking.merchantOperatingCityId oldBooking.merchantId amount person paymentType vendorSplitDetails baskets mbEnableOffer bookingPaymentGroup
  where
