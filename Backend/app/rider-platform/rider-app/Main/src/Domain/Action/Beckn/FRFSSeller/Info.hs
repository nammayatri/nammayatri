...
handleInfo operator req = do
  ...
  case ACL.parseSellerEntityInfo becknConfig.sellerEntityInfoJson of
    Nothing ->
      logWarning $
        "FRFS seller info for " <> operator
          <> ": no usable seller_entity_info_json on beckn_config, so no on_info was sent."
          <> " Seed that column rather than letting an entity disclosure go out incomplete."
    Just info -> do
      now <- getCurrentTime
      let self =
