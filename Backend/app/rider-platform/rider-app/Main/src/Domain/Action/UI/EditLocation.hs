...
postEditResultConfirm (mbPersonId, merchantId) bookingUpdateReqId = do
  ...
  bookingUpdateReqL <- QBUR.findById bookingUpdateReqId >>= fromMaybeM (InvalidRequest "Invalid booking update request id")
  when (bookingUpdateReqL.status /= DBUR.SOFT) $
    throwError $ InvalidRequest "BookingUpdateRequest is not in SOFT state"
  dUpdateReq <- case bookingUpdateReqL.updateType of
    Just DBUR.STOPS -> do
      n <- bookingUpdateReqL.preservedPrefixStops & fromMaybeM (InvalidRequest "preservedPrefixStops missing on EDIT_STOPS booking update request")
      ...
      pure $
        ACL.UpdateBuildReq
          { bppId = booking.providerId,
            ...
            messageId = bookingUpdateReqL.id.getId,
