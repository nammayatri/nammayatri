...
handleInit operator req = do
  ...
  mbSelfOperatorConfig <- (.operatorConfig) <$> sellerIntegratedBPPConfig merchantId ctx
  let self =
        OnSearchACL.SellerIdentity
          { subscriberId = becknConfig.subscriberId,
            subscriberUrl = showBaseUrl becknConfig.subscriberUrl,
            callbackTtl = Common.callbackTtl becknConfig.initTTLSec,
...
initialiseOrder operator transactionId becknConfig mbOperatorConfig paymentId operatingHours operatingWindow stations req = runExceptT $ do
  ...
  when (quantity < 1 || quantity > maxTickets) $
    throwE (QuantityExceeded $ "Quantity " <> show quantity <> " outside 1.." <> show maxTickets)
  unitPrice <-
...
sellerEnrichment merchantId ctx = do
  -- The config lookup stays OUTSIDE the degradable block. Inside it, a transient operator
  -- timeout collapsed to `operatorConfig = Nothing`, which initialiseOrder then reads as
  -- "operator_config is not set" and hard-rejects -- so the graceful degradation this
  -- function exists for could never happen, and the buyer got a NACK blaming unseeded config.
  ...
  case result of
    Right (hoursTags, window, stations) -> pure (integratedBPPConfig.operatorConfig, hoursTags, window, stations)
    Left err -> do
      logWarning $ "FRFS seller init: operator enrichment unavailable, publishing without it: " <> show err
      pure (integratedBPPConfig.operatorConfig, [], Nothing, [])
