...
onInit req = do
  ...
  riderConfig <- getConfig (RiderDimensions {merchantOperatingCityId = decRider.merchantOperatingCityId.getId, txnId = Nothing}) >>= fromMaybeM (RiderConfigDoesNotExist decRider.merchantOperatingCityId.getId)
