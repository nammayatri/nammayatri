...
postMultimodalPassSelect (mbPersonId, merchantId) passId = do
  ...
  -- Check if pass is already purchased and active - read from master to avoid race condition
  existingActivePasses <- QPurchasedPass.findActivePassByPersonIdAndPassId personId passId
    ...
    if pass.amount > 0
      then do
        ...
        let commonMerchantId = Id.cast @DM.Merchant @DPayment.Merchant merchantId
            ...
            createOrderCall = TPayment.createOrder merchantId person.merchantOperatingCityId Nothing TPayment.PassBooking (Just personId.getId) person.clientSdkVersion
...
getMultimodalPassStatus (mbPersonId, _merchantId) purchasedPassId = do
  ...
  -- Get pass details
  pass <- B.runInReplica $ QPass.findById purchasedPass.passId >>= fromMaybeM (PassNotFound purchasedPass.passId.getId)
  ...
  -- Calculate trips left
  let tripsLeft = case pass.maxValidTrips of
        Just maxTrips -> Just $ max 0 (maxTrips - purchasedPass.usedCount)
        Nothing -> Nothing

  -- Build pass details entity
  passAPIEntity <- buildPassAPIEntity personId pass
        ...
        Just transaction -> do
          ...
          case transaction.status of
            Payment.CHARGED -> do
              ...
              -- Get pass details to determine validity
              pass <- B.runInReplica $ QPass.findById purchasedPass.passId >>= fromMaybeM (PassNotFound purchasedPass.passId.getId)

              let validTill = case pass.maxValidDays of
                    Just days -> Just $ addUTCTime (fromIntegral $ days * 86400) now
                    Nothing -> Nothing
