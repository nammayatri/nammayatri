  ...
  return $
    PassAPI.PassAPIEntity
      { id = Id.cast purchasedPass.id,
        amount = if purchasedPass.passAmount == (HighPrecMoney 975.0) then (HighPrecMoney 1000.0) else if purchasedPass.passAmount == (HighPrecMoney 1975.0) then (HighPrecMoney 2000.0) else purchasedPass.passAmount,
        savings = Nothing,
        benefit = benefit,
        benefitDescription = benefitDescription,
