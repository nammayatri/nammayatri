...
postTicketPlacesBook (mbPersonId, merchantId) placeId req = do
  ...
  mCreateOrderRes <- DPayment.createOrderService commonMerchantId (Just $ Kernel.Types.Id.cast merchantOpCity.id) commonPersonId Nothing (show Payment.Normal) createOrderReq createOrderCall
