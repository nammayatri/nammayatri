...
-- | How long a booking may sit in CONFIRMING before a retry is allowed to take it over. A
-- delivery that dies mid-issuance (pod restart, killed request) leaves the row CONFIRMING
-- forever: every retry is answered Silent and every status request refuses, so the buyer has
-- paid, holds no ticket, and nothing -- automatic or manual -- can reach a terminal state.
-- Past this window the operator call cannot still be running, so a retry takes it over.
-- Longer than the operator timeout, short enough that a buyer's own retry recovers it.
confirmingTakeoverSeconds :: NominalDiffTime
confirmingTakeoverSeconds = 180
...
handleConfirm operator req = do
  ...
  mbSelfOperatorConfig <- (.operatorConfig) <$> Init.sellerIntegratedBPPConfig merchant.id ctx
  let self =
        OnSearchACL.SellerIdentity
          { subscriberId = becknConfig.subscriberId,
            subscriberUrl = showBaseUrl becknConfig.subscriberUrl,
            callbackTtl = Common.callbackTtl becknConfig.confirmTTLSec,
...
confirmOrder operator transactionId merchant becknConfig req = do
  ...
  -- Serialize find-then-create per order. acceptOnce dedupes only on message_id, so a BAP retry
  -- with a fresh message_id (or two concurrent deliveries on one transaction) would otherwise both
  -- see no existing booking, both QBooking.create, and both buy real tickets against one payment --
  -- and leave findBySearchId (findOneWithKV) permanently ambiguous. Same lock the buyer-side
  -- SharedLogic.FRFSConfirm takes on searchId.
  Hedis.withWaitAndLockMasterCloudCrossAppRedis (mkConfirmLockKey bookingSearchId.getId) 60 100 $ do
    existing <- QBooking.findBySearchId bookingSearchId
    now <- getCurrentTime
    let staleConfirming booking =
          booking.status == DBookingStatus.CONFIRMING
            && diffUTCTime now booking.updatedAt > confirmingTakeoverSeconds
        disposition = case existing of
          Just booking | staleConfirming booking -> IssueFresh
          _ -> dispositionFor ((.status) <$> existing)
    case (disposition, existing) of
      ...
      _ -> do
        whenJust existing $ \booking ->
          when (staleConfirming booking) $
            logWarning $ "FRFS seller confirm: booking " <> booking.id.getId <> " has been CONFIRMING since " <> show booking.updatedAt <> "; a delivery died mid-issuance, so this retry takes it over"
        Answered <$> runExceptT (issue operator transactionId bookingSearchId merchant becknConfig req order existing)
  where
    mkConfirmLockKey searchId = "frfsSeller:confirm:searchId-" <> searchId

issue :: Text -> Text -> Id DSearch.FRFSSearch -> DM.Merchant -> DBC.BecknConfig -> Spec.ConfirmReq -> Spec.Order -> Maybe DBooking.FRFSTicketBooking -> ExceptT ConfirmFailure Flow ACL.ConfirmedOrder
issue operator transactionId bookingSearchId merchant becknConfig req order mbExisting = do
  item <-
  ...
  let maxTickets = Common.maxTicketsPerOrder cfg journeyType.code
  when (requestedQuantity < 1 || requestedQuantity > maxTickets) $
    throwE (QuantityExceeded $ "Quantity " <> show requestedQuantity <> " outside 1.." <> show maxTickets)
  -- Reuse the FAILED row's id. QBooking.findBySearchId is findOneWithKV, so a second row on
  -- the same searchId makes which booking a later confirm sees undefined -- and picking the
  -- stale FAILED one would issue a second set of real tickets against one payment.
  bookingId <- maybe (lift generateGUID) (pure . (.id)) mbExisting
  orderUuid <- lift generateGUID
  now <- lift getCurrentTime
  -- On a retry/takeover of an existing booking, reuse the exact bppOrderId the operator already
  -- saw. If a prior attempt died after the operator issued a ticket, minting a fresh reference
  -- makes the operator's duplicate-detection miss it and issue a SECOND real ticket (the first
  -- then stranded, unrecorded). Reusing the reference lets that guard catch the residual. Only a
  -- brand-new booking (mbExisting = Nothing) gets a fresh id.
  let freshOndcOrderId = Common.sellerOrderId (Common.orderIdPrefixesFor integratedBPPConfig.providerConfig) cfg.defaultOrderIdPrefix bapId orderUuid
      ondcOrderId = fromMaybe freshOndcOrderId (mbExisting >>= (.bppOrderId))
  ...
  lift (maybe (QBooking.create booking) (const (QBooking.updateByPrimaryKey booking)) mbExisting)
  existingCategories <- lift (QQuoteCategory.findAllByQuoteIds [Id bookingId.getId])
  lift $
    forM_ quoteCategories $ \category ->
      if null existingCategories
        then QQuoteCategory.create category
        else QQuoteCategory.updateByPrimaryKey category

  providerOrder <-
    lift (withTryCatch "frfsSeller:createOrder" (ExternalCallAPI.createOrderForJourneyType integratedBPPConfig (Seconds (round (ticketValiditySeconds cfg))) (order.orderBilling >>= (.billingName), order.orderBilling >>= (.billingPhone)) booking (Just bapId) quoteCategories quote.fareQuoteId Nothing (Just requestedQuantity))) >>= \case
  ...
  -- A takeover of a booking whose prior attempt already persisted tickets must not create a
  -- second set. The reused ondcOrderId makes the operator hand back the same tickets, and
  -- QTicket has no upsert, so calling createMany again would duplicate the rows --
  -- findAllByTicketBookingId would then report 2N on every later status/confirm and inflate
  -- ticketCount. Persist only when none exist yet; otherwise reuse what the earlier attempt wrote.
  existingTickets <- lift (QTicket.findAllByTicketBookingId bookingId)
  tickets <-
    if null existingTickets
      then do
        fresh <- lift $ forM providerOrder.tickets (mkTicket booking now)
        lift (QTicket.createMany fresh)
        pure fresh
      else pure existingTickets
  let issuedTotal = unitPrice * fromIntegral (length tickets) :: Double
      -- bppOrderId deliberately keeps ondcOrderId. Every adapter echoes back the id we
      -- filed the order under, so writing providerOrder.orderId here is a no-op today --
      -- but it would silently replace the per-BAP-prefixed id (whose shape is an ONDC
      -- contract) if an adapter ever minted its own.
      confirmedBooking =
        booking
  ...
  lift (QRecon.create (mkRecon confirmedBooking merchant merchantOperatingCity becknConfig account bapId ondcOrderId paymentTxnId (length tickets) now))
...
republish _operator becknConfig mbOperatorConfig mbBilling booking tickets = do
  ...
  -- No tickets means we could not read them, not that the order is empty: pricing would
  -- publish a 0.00 total against a full SETTLEMENT_AMOUNT, and Status.deriveOrderStatus would
  -- report a live order as CANCELLED. Refuse rather than emit either.
  when (null tickets) $
    throwE (Unprocessable $ "Booking " <> booking.id.getId <> " has no ticket rows to report on")
