  ...
  if not (null activeJourneys)
    then do
      ...
      if null paymentSuccessJourneys
        then return $ GetPersonFlowStatusRes Nothing (DPFS.ACTIVE_JOURNEYS {journeys = activeJourneys, journeyInfoResponse = Nothing}) Nothing
        else do
          ...
          return $ GetPersonFlowStatusRes Nothing (DPFS.ACTIVE_JOURNEYS {journeys = activeJourneys, journeyInfoResponse = Just journeyInfoResp}) Nothing
    else do
