  ...
  let updated =
        milestone
          { DIJM.name = maybe milestone.name Just req.name,
            ...
            DIJM.rewardMetadata = milestone.rewardMetadata,
...
getIncentiveJourneyPersonAssignments merchantShortId opCity personId = do
  merchant <- findMerchantByShortId merchantShortId
  merchantOpCityId <- CQMOC.getMerchantOpCityId merchant (Just opCity)
  let riderPersonId = ID.cast @Dashboard.Common.Person @DP.Person personId
  assignments <- SLJourney.findAssignmentsByUserId riderPersonId
  items <-
    mapMaybeM
      (toPersonAssignmentItem (ID.cast merchant.id) (ID.cast merchantOpCityId))
      assignments
  let ordered = sortOn (Down . (.assignedAt)) items
  pure Common.IncentiveJourneyPersonAssignmentListRes {assignments = ordered}
...
putIncentiveJourneyCohortJourneyUpdate merchantShortId opCity req = do
  ...
  CQAssignment.clearCacheByCohortMappingId cjmId
  pure Success
...
getIncentiveJourneyCohortJourneyList merchantShortId _opCity mbLimit mbOffset mbCohortName = do
  merchant <- findMerchantByShortId merchantShortId
  let limitVal = fromMaybe 20 mbLimit
      offsetVal = fromMaybe 0 mbOffset
      mbSearch =
        case T.strip <$> mbCohortName of
          Just name | not (T.null name) -> Just (T.toLower name)
          _ -> Nothing
  mappings <-
    QCJMExtra.findByMerchantIdWithOptionalCohortName
      (Just limitVal)
      (Just offsetVal)
      (ID.cast merchant.id)
      mbCohortName
  cohorts <- QCDExtra.findByIds (map (.cohortId) mappings)
  let cohortNameById = HM.fromList $ map (\c -> (c.id, c.name)) cohorts
      items =
        mapMaybe
          ( \cjm -> do
              cohortName <- HM.lookup cjm.cohortId cohortNameById
              pure $ toCohortJourneyMappingListItem cjm cohortName
          )
          mappings
      filteredItems =
        case mbSearch of
          ...
          Just needle ->
            take limitVal
              ...
              $ filter (\item -> needle `T.isInfixOf` T.toLower item.cohortName) items
  pure Common.CohortJourneyMappingListRes {mappings = filteredItems}

toCohortJourneyMappingListItem :: DCJM.CohortJourneyMapping -> Text -> Common.CohortJourneyMappingListItem
toCohortJourneyMappingListItem cjm cohortName =
  Common.CohortJourneyMappingListItem
...
postIncentiveJourneyAssign merchantShortId opCity req = do
  ...
  let cjmId = ID.cast @Dashboard.Common.CohortJourneyMapping @DCJM.CohortJourneyMapping req.cohortJourneyMappingId
      ...
      riderPersonId = ID.cast @DIJC.Person @DP.Person personId
  person <- QPerson.findById riderPersonId >>= fromMaybeM (InvalidRequest "Person not found")
  unless (person.merchantId == merchant.id && person.merchantOperatingCityId == merchantOpCityId) $
    throwError (InvalidRequest "Person does not belong to this merchant/city")
  cjm <- QCJMExtra.findCohortJourneyMappingById cjmId >>= fromMaybeM (InvalidRequest "Cohort journey mapping not found")
  unless (cjm.merchantId == ID.cast merchant.id) $
    throwError (InvalidRequest "Cohort journey mapping does not belong to this merchant")
  journey <- QJourney.findById cjm.journeyId >>= fromMaybeM (InvalidRequest "Incentive journey not found")
  unless (journey.merchantOperatingCityId == ID.cast merchantOpCityId && journey.merchantId == ID.cast merchant.id) $
    throwError (InvalidRequest "Incentive journey does not belong to this merchant/city")
  void $ QUCMExtra.upsertUserCohortMapping personId cjmId req.isTestGroup req.validTill
  CQAssignment.clearCacheByPersonId riderPersonId
  pure Success
...
toPersonAssignmentItem ::
  ID.Id DIJC.Merchant ->
  ID.Id DIJC.MerchantOperatingCity ->
  IJ.JourneyAssignment ->
  Environment.Flow (Maybe Common.IncentiveJourneyPersonAssignmentItem)
toPersonAssignmentItem merchantId merchantOpCityId assignment = do
  ...
  case mbJourney of
    ...
    Just journey
      | journey.merchantId == merchantId
        ...
        let journeyType = SLJourney.journeyTypeOrDefault journey.journeyType
        pure $
          Just
