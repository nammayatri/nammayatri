                ...
                QRSF.findAllByStartStopAndIntegratedBPPConfigId dStartStation.stationCode dEndStation.stationCode integratedBPPConfig.id >>= \case
                  fareProducts@(_ : _) -> do
                    ...
                    forM_ quote.categories $ \quoteCategory -> do
                      traverse_ (\fp -> QRSF.updateFareByStopCodes quoteCategory.price.amount fp dStartStation.stationCode dEndStation.stationCode quoteCategory.category) farePolicyIds
                  [] -> do
                ...
                QFRFP.findByRouteCode quote.routeCode integratedBPPConfig.id >>= \case
                  fareProducts@(_ : _) -> do
                    ...
                    forM_ quote.categories $ \quoteCategory -> do
                      traverse_
                        ( \fp -> do
                            QRSF.updateFareByStopCodes quoteCategory.price.amount fp.id dStartStation.stationCode dEndStation.stationCode quoteCategory.category
                        )
              ...
              case routeStation.routeFarePolicyId of
                Just farePolicyId -> do
                  ...
                  whenJust ((,) <$> mbStartStopCode <*> mbEndStopCode) $ \(startStopCode, endStopCode) -> do
                    forM_ quote.categories $ \quoteCategory -> do
                      QRSF.updateFareByStopCodes quoteCategory.price.amount farePolicyId startStopCode endStopCode quoteCategory.category
                Nothing -> do
              ...
              then do
                ...
                QFRFP.findByRouteCode quote.routeCode integratedBPPConfig.id >>= \case
                  fareProducts@(_ : _) -> do
                    ...
                    let filteredFarePolicies = filter (\fp -> fp._type == FRFSFarePolicy.MatrixBased) farePolicies
                    forM_ quote.categories $ \quoteCategory ->
                      traverse_ (\fp -> QRSF.updateFareByStopCodes quoteCategory.price.amount fp.id dStartStation.stationCode dEndStation.stationCode quoteCategory.category) filteredFarePolicies
                  [] -> do
                    QFRFP.findAllByIntegratedBPPConfigId integratedBPPConfig.id >>= \case
                      fareProducts@(_ : _) -> do
                        let farePolicyIds = map (.farePolicyId) fareProducts
