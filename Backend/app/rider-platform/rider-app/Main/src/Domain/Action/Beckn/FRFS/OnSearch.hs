...
mkQuotes dOnSearch ValidatedDOnSearch {..} DQuote {..} = do
  ...
  integratedBPPConfig <- QIBC.findByDomainAndCityAndVehicleCategory (show Spec.FRFS) search.merchantOperatingCityId (frfsVehicleCategoryToBecknVehicleCategory vehicleType) (Just DIBC.APPLICATION) >>= fromMaybeM (InternalError "integratedBPPConfig is missing")
