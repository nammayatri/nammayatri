...
mkQuotes dOnSearch ValidatedDOnSearch {..} DQuote {..} = do
  ...
  let cachedQuoteDataKey = FRFSCachedQuoteKey {CachedQuote.fromStationId = startStation.id, CachedQuote.toStationId = endStation.id, CachedQuote.providerId = dOnSearch.providerId, CachedQuote.quoteType = _type}
  CachedQuote.cacheByFromToProviderIdAndQuoteType cachedQuoteDataKey cachedQuoteData
  return
    Quote.FRFSQuote
      { Quote._type = _type,
        Quote.bppItemId,
...
updateQuotes (quotesFromCache, quotesFromOnSearch) = do
  let oldCacheDump =
        if isQuoteChanged (quotesFromCache, quotesFromOnSearch)
          then Just (show FRFSCachedQuote {price = quotesFromCache.price, stationsJson = quotesFromCache.stationsJson})
          else Nothing
  ...
  Quote.FRFSQuote
    { Quote._type = quotesFromCache._type,
      ...
      Quote.riderId = quotesFromOnSearch.riderId,
      ...
      Quote.partnerOrgId = quotesFromOnSearch.partnerOrgId,
      Quote.partnerOrgTransactionId = quotesFromOnSearch.partnerOrgTransactionId,
