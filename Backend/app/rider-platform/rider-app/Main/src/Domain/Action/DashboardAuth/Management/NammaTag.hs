...
--
-- rider-dashboard checked the caller's merchant_access for every merchant and
...
-- reports the rest as failures -- the shape rider-dashboard returned.
...
postNammaTagAppDynamicLogicBulkUpsertLogicRollout a4 a3 a2 a1 = do
  ...
  if null allowedEntries
    then Kernel.Prelude.pure (Lib.Yudhishthira.Types.BulkLogicRolloutResult [] deniedFailures)
      ...
      result <-
        Domain.Action.Dashboard.NammaTag.postNammaTagAppDynamicLogicBulkUpsertLogicRollout a4 a3 $
