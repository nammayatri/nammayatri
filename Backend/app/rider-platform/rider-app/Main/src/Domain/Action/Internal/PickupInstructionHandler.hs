...
handlePickupInstruction ride booking driverIdValue = do
  logDebug $ "PickupInstructionHandler: Processing pickup instruction for ride: " <> ride.id.getId

  -- Extract driverId from Value type
  driverId <- extractDriverId driverIdValue
  logDebug $ "PickupInstructionHandler: Driver ID extracted: " <> driverId

  -- Get pickup location coordinates (fromLocation is already a Location object)
  let fromLocation = ride.fromLocation
  logDebug $ "PickupInstructionHandler: Pickup location found - lat: " <> show fromLocation.lat <> ", lon: " <> show fromLocation.lon
  ...
  logDebug $ "PickupInstructionHandler: Generated geohash: " <> T.pack geohash
  ...
  case mbPickupInstruction of
    Nothing -> do
      logDebug "PickupInstructionHandler: No pickup instruction found for this location, skipping"
      pure () -- No instruction found, skip silently
    Just pickupInstruction -> do
      logDebug $ "PickupInstructionHandler: Pickup instruction found - id: " <> pickupInstruction.id.getId

      -- Generate signed URL for audio if mediaFileId exists
      mbAudioUrl <- case pickupInstruction.mediaFileId of
        Just mediaFileId -> do
          logDebug $ "PickupInstructionHandler: Processing audio file - mediaFileId: " <> mediaFileId.getId
          mbMediaFile <- MFQuery.findById mediaFileId
          case mbMediaFile of
            Just mediaFile -> case mediaFile.s3FilePath of
              Just s3Path -> do
                logDebug $ "PickupInstructionHandler: Generating signed URL for audio in S3 (key from DB): " <> s3Path
                ...
                case s3Cfg of
                  S3.S3AwsConf a -> do
                    ...
                    case result of
                      Right signedUrl -> do
                        logDebug "PickupInstructionHandler: Successfully generated signed URL for audio"
                  ...
                  _ -> do
                    ...
                    return Nothing
              Nothing -> do
                logDebug "PickupInstructionHandler: No S3 path found for media file"
                return Nothing
            Nothing -> do
              logDebug $ "PickupInstructionHandler: Media file not found for ID: " <> mediaFileId.getId
              return Nothing
        Nothing -> do
          logDebug "PickupInstructionHandler: No media file associated with this instruction"
          return Nothing
      ...
      case result of
        Right _ -> logDebug "PickupInstructionHandler: Successfully sent pickup instruction to driver app"
