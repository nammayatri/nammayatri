...
getPickupInstructions bppRideId apiKey = do
  logDebug $ "GetPickupInstructions: Processing request for bppRideId: " <> bppRideId.getId

  -- Find the ride by BPP ride ID
  ride <- QRide.findByBPPRideId bppRideId >>= fromMaybeM (RideNotFound bppRideId.getId)
  logDebug $ "GetPickupInstructions: Found ride with bookingId: " <> ride.bookingId.getId

  -- Find the booking to get personId
  booking <- QBooking.findById ride.bookingId >>= fromMaybeM (BookingNotFound ride.bookingId.getId)
  logDebug $ "GetPickupInstructions: Found booking for personId: " <> booking.riderId.getId
  ...
  logDebug $ "GetPickupInstructions: API key validated"

  -- Get pickup location from ride
  let fromLocation = ride.fromLocation
  logDebug $ "GetPickupInstructions: Processing pickup location: " <> show fromLocation.lat <> "," <> show fromLocation.lon
  ...
  logDebug $ "GetPickupInstructions: Generated geohash: " <> T.pack geohash
  ...
  case mbPickupInstruction of
    Nothing -> do
      logDebug $ "GetPickupInstructions: No pickup instruction found for personId: " <> booking.riderId.getId <> " and geohash: " <> T.pack geohash
      pure $ PickupInstructionResp Nothing Nothing
    Just pickupInstruction -> do
      logDebug $ "GetPickupInstructions: Found pickup instruction: " <> pickupInstruction.instruction

      -- Get audio file if mediaFileId exists
      mbAudioBase64 <- case pickupInstruction.mediaFileId of
        Nothing -> do
          logDebug "GetPickupInstructions: No media file associated with pickup instruction"
          pure Nothing
        Just mediaFileId -> do
          logDebug $ "GetPickupInstructions: Fetching audio file for mediaFileId: " <> mediaFileId.getId
          ...
          case mbMediaFile of
            ...
            Just mediaFile -> case mediaFile.s3FilePath of
              Just s3Path -> do
                logDebug $ "GetPickupInstructions: Found media file, fetching from S3: " <> s3Path
                ...
                case result of
                  ...
                  Right audioContent -> do
                    logDebug "GetPickupInstructions: Successfully fetched audio from S3"
                    pure $ Just audioContent
              Nothing -> do
                logDebug "GetPickupInstructions: No S3 path found for media file"
                pure Nothing

      logDebug $ "GetPickupInstructions: Returning response with instruction and audio"
