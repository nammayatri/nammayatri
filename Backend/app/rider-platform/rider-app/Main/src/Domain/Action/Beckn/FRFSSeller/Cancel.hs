...
failureCode :: CancelFailure -> Text
failureCode = \case
  CancellationNotPossible _ -> "50001"
...
handleCancel operator req = do
  ...
  now <- getCurrentTime
  onCancelReq <-
    cancelOrder operator becknConfig req >>= \case
      ...
      Left failure -> do
        ...
        pure $ ACL.buildOnCancelErrorReq self now ctx (failureCode failure) (failureMessage failure)
  CallBAP.sendOnCancel merchant.id becknConfig.subscriberId bapUri onCancelReq

cancelOrder :: Text -> DBC.BecknConfig -> Spec.CancelReq -> Flow (Either CancelFailure (ACL.CancelOutcome, OnConfirmACL.ConfirmedOrder))
cancelOrder operator becknConfig req = runExceptT $ do
  ...
  order <- withExceptT (Unprocessable . Confirm.failureMessage) (Confirm.republish operator becknConfig Nothing booking tickets)
