...
cancelOrder operator becknConfig mbOperatorConfig integratedBPPConfig req =
  -- Serialize the cancel read-modify-write per order. Without it, two CONFIRM_CANCELs with
  -- distinct message_ids both read a not-yet-CANCELLED booking, both call the operator's hard
  -- cancel, and both publish a refund -- the same race Confirm takes its per-order lock to avoid.
  Hedis.withWaitAndLockMasterCloudCrossAppRedis (mkCancelLockKey orderId) 60 100 $
    runExceptT $ do
  ...
  where
    orderId = req.cancelReqMessage.cancelReqMessageOrderId
    mkCancelLockKey oid = "frfsSeller:cancel:orderId-" <> oid
