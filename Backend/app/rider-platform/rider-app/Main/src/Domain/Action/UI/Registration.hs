...
createPerson req identifierType notificationToken mbBundleVersion mbClientVersion mbClientConfigVersion mbRnVersion mbDevice mbCloudType merchant mbPartnerOrgId = do
  ...
  where
    ...
    addNammaTags person tagData = do
      newPersonTags <- withTryCatch "computeNammaTagsWithExpiry:Login" (LYDL.computeNammaTagsWithExpiryAndDebugLog LYDL.Rider (cast person.merchantOperatingCityId) (cast person.merchantOperatingCityId) Yudhishthira.Login tagData)
