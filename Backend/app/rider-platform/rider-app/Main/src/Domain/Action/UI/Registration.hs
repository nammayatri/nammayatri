...
data AuthReq = AuthReq
  { mobileNumber :: Maybe Text,
    ...
    -- | The city short-id (e.g. "Chennai") used to resolve the IBC config
    cityShortId :: Maybe Text,
...
auth req' mbBundleVersion mbClientVersion mbClientConfigVersion mbRnVersion mbDevice mbXForwardedFor mbSenderHash = do
  ...
  (person, otpChannel) <-
    case identifierType of
      ...
      _ -> throwError $ InvalidRequest "Aadhaar auth is not supported"
...
conductorTokenAuth req mbBundleVersion mbClientVersion mbClientConfigVersion mbRnVersion mbDevice = do
  badgeNumber <- req.operatorBadgeToken & fromMaybeM (InvalidRequest "operatorBadgeNumber is required for ConductorToken auth")
  serialNo <- req.deviceSerialNumber & fromMaybeM (InvalidRequest "deviceSerialNumber is required for ConductorToken auth")
  cityText <- req.cityShortId & fromMaybeM (InvalidRequest "cityShortId is required for ConductorToken auth")
  city <- case readMaybe (T.unpack cityText) of
    Just c -> pure c
    Nothing -> throwError $ InvalidRequest ("Invalid cityShortId: " <> cityText)
  let vehicleCat = case req.vehicleType of
        Just "BUS" -> BecknSpec.BUS
        Just "METRO" -> BecknSpec.METRO
        _ -> BecknSpec.BUS
  ...
  merchant <-
    QMerchant.findByShortId req.merchantId
      >>= fromMaybeM (MerchantNotFound $ getShortId req.merchantId)
  -- Resolve OTPRest base URL and gtfsId from IBC (MULTIMODAL platform)
  merchantOpCity <-
  ...
  bppConfig <- SIBC.findIntegratedBPPConfig Nothing merchantOpCity.id vehicleCat DIBC.MULTIMODAL
  ...
  _verifyResp <-
    NandiFlow.gimsVerifyConductor
      ...
      gtfsId
      GimsVerifyReq
        { operator_badge_token = badgeNumber,
          device_serial_number = serialNo
        }
  -- GIMS returned 200 OK — find or create the Person
  person <-
    Person.findByOperatorBadgeTokenAndMerchantId (Just badgeNumber) merchant.id
      >>= maybe
        ( do
            newPerson <- createPerson req SP.CONDUCTORTOKEN Nothing mbBundleVersion mbClientVersion mbClientConfigVersion mbRnVersion (getDeviceFromText mbDevice) mbCloudType merchant Nothing Nothing
            Person.updateOperatorBadgeTokenAndDeviceSerialNumber (Just badgeNumber) newPerson.id
            pure newPerson
        )
        pure
  -- Generate a direct (no-OTP) session token
  let entityId = getId $ person.id
      useFakeOtpM = (show <$> useFakeSms smsCfg) <|> person.useFakeOtp
      scfg = sessionConfig smsCfg
