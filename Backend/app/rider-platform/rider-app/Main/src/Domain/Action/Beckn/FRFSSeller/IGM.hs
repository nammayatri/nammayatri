...
sellerContext operator = do
  ...
  mbSelfOperatorConfig <- (.operatorConfig) <$> SIBC.findIntegratedBPPConfig Nothing moCity.id BecknSpec.METRO DIBC.MULTIMODAL
  pure
    SellerContext
      { merchant,
        ...
        self =
          OnSearchACL.SellerIdentity
            { subscriberId = becknConfig.subscriberId,
              ...
              contextVersion = Common.contextVersionOf mbSelfOperatorConfig
            }
      }
