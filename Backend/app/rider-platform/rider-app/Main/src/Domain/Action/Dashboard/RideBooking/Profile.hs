...
getProfileDetail merchantShortId _opCity personId = do
  ...
  API.UI.Profile.getPersonDetails' (personId, m.id) Nothing Nothing Nothing --CACTODO: this is a temporary implementation need to discuss and implement toss part if required or change response type all together for this api to stop sending cfg version forever.
...
postProfileUpdate merchantShortId _opCity personId req = do
  m <- findMerchantByShortId merchantShortId
  API.UI.Profile.updatePerson' (personId, m.id) req Nothing Nothing Nothing Nothing
