...
rentalSearch personId bundleVersion clientVersion device req = do
  ...
  now <- getCurrentTime
  searchRequest <- DSearch.buildSearchRequest person fromLocation Nothing Nothing Nothing now bundleVersion clientVersion device Nothing Nothing DSearchReq.RENTAL -- Handle disabilityTag if need for rentals
