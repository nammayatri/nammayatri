...
onUpdate = \case
  ...
  OUValidatedSafetyAlertReq ValidatedSafetyAlertReq {..} -> do
    ...
    (informPoliceSos, notifySafetyTeamForSafetyCheckFailure) <- case mbSafetySettings of
      Nothing -> do
        person <- QPerson.findById booking.riderId >>= fromMaybeM (PersonNotFound booking.riderId.getId)
        return (person.informPoliceSos, False)
      Just safetySettings -> return (safetySettings.informPoliceSos, safetySettings.notifySafetyTeamForSafetyCheckFailure)
    when (informPoliceSos || notifySafetyTeamForSafetyCheckFailure) $ do
