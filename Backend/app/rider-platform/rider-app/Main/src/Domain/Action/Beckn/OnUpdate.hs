...
onUpdate RideAssignedReq {..} = do
  ...
  fork "on update RideAssignedReq processing" $ do
...
onUpdate RideStartedReq {..} = do
  ...
  fork "on update RideStartedReq processing" $ do
...
onUpdate RideCompletedReq {..} = do
  ...
  fork "on update RideCompletedReq processing" $ do
...
onUpdate BookingCancelledReq {..} = do
  ...
  fork "on update BookingCancelledReq processing" $ do
...
onUpdate BookingReallocationReq {..} = do
  booking <- runInReplica $ QRB.findByBPPBookingId bppBookingId >>= fromMaybeM (BookingDoesNotExist $ "BppBookingId: " <> bppBookingId.getId)
  ride <- QRide.findByBPPRideId bppRideId >>= fromMaybeM (RideDoesNotExist $ "BppRideId" <> bppRideId.getId)
  fork "on update BookingReallocationReq processing" $ do
    bookingCancellationReason <- buildBookingCancellationReason booking.id (Just ride.id) reallocationSource
    DB.runTransaction $ do
...
onUpdate DriverArrivedReq {..} = do
  ...
  fork "on update DriverArrivedReq processing" $ do
...
onUpdate NewMessageReq {..} = do
  ...
  ride <- QRide.findByBPPRideId bppRideId >>= fromMaybeM (RideDoesNotExist $ "BppRideId" <> bppRideId.getId)
  unless (isValidRideStatus ride.status) $ throwError $ RideInvalidStatus "The ride has already started."
  fork "on update NewMessageReq processing" $ do
...
onUpdate EstimateRepetitionReq {..} = do
  ...
  fork "on update EstimateRepetitionReq processing" $ do
