...
reconcileOrder operator oid order =
  ...
  where
    withLock amount = do
      ...
      case mbRow of
        -- No ledger row means the order is not ours (or its recon row was never written).
        -- Answering `negate amount` here would render as "02"/higher-amount and machine-assert
        -- that we were overpaid by the whole order value -- final, so never re-sent. Stay silent.
        Nothing -> unanswered oid ("no ledger row for this order; not ours (operator " <> operator <> ")")
        Just row -> Just <$> applyToRow row order oid (settlementReferenceOf order) amount
