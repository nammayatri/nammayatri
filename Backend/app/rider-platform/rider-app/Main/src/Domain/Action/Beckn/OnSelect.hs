    ...
    then do
      ...
      case lowestFareQuote of
        Just autoAssignQuote -> do
          let dConfirmReq = SConfirm.DConfirmReq {personId = person.id, quoteId = autoAssignQuote.id, paymentMethodId = searchRequest.selectedPaymentMethodId, startTime = Just now}
