      ...
      case lowestFareQuote of
        Just (autoAssignQuote, False) -> do
          ...
          when isLockAcquired $ do
            ...
            let dConfirmReq = SConfirm.DConfirmReq {personId = person.id, dashboardAgentId = Nothing, quote = autoAssignQuote, paymentMethodId = searchRequest.selectedPaymentMethodId, paymentInstrument = searchRequest.selectedPaymentInstrument, merchant, requiresPaymentBeforeConfirm = False, mbOneShotDetails = Nothing, supportsBookingDeposit = Nothing}
