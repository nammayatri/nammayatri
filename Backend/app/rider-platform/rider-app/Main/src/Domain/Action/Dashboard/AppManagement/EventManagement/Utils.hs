...
uploadAssetHelper _merchantShortId _opCity ticketPlaceId file reqContentType = do
  ...
  riderConfig <- getConfig (RiderConfigDimensions {merchantOperatingCityId = moCity.id.getId}) >>= fromMaybeM (RiderConfigNotFound $ "merchantShortId: " <> _merchantShortId.getShortId <> " ,city: " <> show m.defaultCity)
...
deleteAsset moCityId req = do
  riderConfig <- getConfig (RiderConfigDimensions {merchantOperatingCityId = moCityId.getId}) >>= fromMaybeM (RiderConfigNotFound $ "merchantOperatingCityId: " <> show moCityId)
