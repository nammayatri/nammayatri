...
deleteAsset moCityId req = do
  riderConfig <- getConfig (RiderConfigDimensions {merchantOperatingCityId = moCityId.getId}) Nothing >>= fromMaybeM (RiderConfigNotFound $ "merchantOperatingCityId: " <> show moCityId)
