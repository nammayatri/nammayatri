...
deleteAsset moCityId req = do
  riderConfig <- getConfig (RiderConfigDimensions {merchantOperatingCityId = moCityId.getId}) (Just (CQRC.findByMerchantOperatingCityId moCityId)) >>= fromMaybeM (RiderConfigNotFound $ "merchantOperatingCityId: " <> show moCityId)
