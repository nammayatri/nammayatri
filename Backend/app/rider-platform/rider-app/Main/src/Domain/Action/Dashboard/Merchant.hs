...
getMerchantConfigSpecialLocationList merchantShortId opCity mbLimit mbOffset mbSpecialLocationTypes = do
  merchant <- findMerchantByShortId merchantShortId
  merchantOpCity <- CQMOC.findByMerchantIdAndCity merchant.id opCity >>= fromMaybeM (MerchantOperatingCityNotFound $ "merchantShortId: " <> merchantShortId.getShortId <> " ,city: " <> show opCity)

  -- Fetch rider special locations
  riderLocations <- QSL.findAllSpecialLocationsWithGeoJSON merchantOpCity.id.getId mbLimit mbOffset mbSpecialLocationTypes
  ...
  mbDriverLocations <- CallBPPInternal.getSpecialLocationList merchant opCity mbLimit mbOffset mbSpecialLocationTypes
