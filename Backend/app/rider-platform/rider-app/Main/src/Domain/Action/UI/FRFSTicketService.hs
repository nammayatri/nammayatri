...
getFrfsRoutes (_personId, _mId) mbEndStationCode mbStartStationCode _city _vehicleType = do
  case (mbStartStationCode, mbEndStationCode) of
    ...
    _ -> do
      ...
      return $
        map
          ( \Route.Route {..} -> FRFSTicketService.FRFSRouteAPI {totalStops = Nothing, waypoints = Nothing, ..}
          )
          routes
...
getFrfsStations (_personId, mId) mbCity mbEndStationCode mbOrigin mbRouteCode mbStartStationCode vehicleType_ = do
  ...
  case (mbRouteCode, mbStartStationCode, mbEndStationCode) of
          ...
          possibleEndStops =
            nubBy (\a b -> a.stopCode == b.stopCode) $
              concatMap
                ...
                groupedStopsByRouteCode
      routes <- QRoute.findByRouteCodes (map (.routeCode) possibleEndStops)
      let stopsWithRoutes = map (\stop -> (stop, find (\route -> route.code == stop.routeCode) routes)) possibleEndStops
      let endStops =
            map
              ( \(routeStop, mbRoute) ->
                  FRFSStationAPI
    ...
    _ -> do
      mappings <- B.runInReplica $ CQRouteStopMapping.findAllByMerchantOperatingCityAndVehicleType merchantOpCity.id vehicleType_
      routes <- B.runInReplica $ CQRoute.findAllByMerchantOperatingCityAndVehicleType merchantOpCity.id vehicleType_
      let stationsWithRoutes = map (\stop -> (stop, find (\route -> route.code == stop.routeCode) routes)) mappings
      let stops =
            map
              ( \(routeStop, mbRoute) ->
                  FRFSStationAPI
...
getFRFSRouteAPIRespById =
  maybe
    ...
    ( \routeId -> do
        ...
        return $ Just FRFSRouteAPI {totalStops = Nothing, waypoints = Nothing, ..}
    )
    ...
    then return []
      ...
      batchedResults <- fmap concat $
        forM stopBatches $ \batch -> do
          ...
          case res of
            Left _ -> return $ map (\StationResult {..} -> FRFSStationAPI {lat = Just lat, lon = Just lon, address = Nothing, color = Nothing, distance = Nothing, towards = Nothing, routeName = Nothing, ..}) batch
