...
postFrfsQuoteV2ConfirmWithActor (mbPersonId, merchantId) quoteId mbIsMockPayment req = do
  ...
  case integratedBppConfig.providerConfig of
    DIBC.ONDC _ | quote.vehicleType == Spec.BUS -> do
      ...
      (_, booking, _, _, _) <- confirmAndUpsertBooking personId quote selectedQuoteCategories req.crisSdkResponse (Just True) mbIsMockPayment integratedBppConfig Nothing req.isSpotBooking Nothing req.purchasedPassPaymentId Nothing
    ...
    _ -> do
      postFrfsQuoteV2ConfirmUtil (Just personId, merchantId) quote selectedQuoteCategories req.crisSdkResponse (Just True) req.enableOffer mbIsMockPayment integratedBppConfig req.tripId req.isSpotBooking Nothing req.purchasedPassPaymentId Nothing
