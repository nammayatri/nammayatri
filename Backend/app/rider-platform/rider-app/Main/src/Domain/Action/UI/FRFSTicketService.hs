...
enrichQuotesWithPassOverride integratedBppConfig personId search mbClientHasPasses mbTripTime
  ...
  | otherwise = do
    ...
    QP.findById personId >>= \case
      Nothing -> pure []
      Just person ->
        FRFSPassOverride.getFRFSOverrideApplicablePassesByPersonId integratedBppConfig person search.vehicleType tripTime mbKnownHasPass $
          Just <$> FRFSPassOverride.resolveLegStations integratedBppConfig search.fromStationCode search.toStationCode
