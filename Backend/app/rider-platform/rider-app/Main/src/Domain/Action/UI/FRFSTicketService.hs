...
postFrfsQuoteV2Confirm (mbPersonId, merchantId) quoteId mbIsMockPayment req = do
  ...
  -- Apply rate limit if seatIds are present in the request
  let seatIds' = map (.seatIds) selectedQuoteCategories
  let hasSeatIds = any (isJust) seatIds'
  when hasSeatIds $ do
    let tripIdStr = fromMaybe "default" req.tripId
    checkRateLimitSeatBooking (rateLimitKey personId.getId tripIdStr)
