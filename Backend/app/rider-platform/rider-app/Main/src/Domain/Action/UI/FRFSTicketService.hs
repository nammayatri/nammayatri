...
forwardShuttleRating merchant booking ratingReq =
  when (isJust ratingReq.driverRating || isJust ratingReq.fleetRating) $ do
    mbGtfsId <- fmap (.feedKey) <$> CQIBC.findById booking.integratedBppConfigId
    let mbFleetNumber = booking.finalBoardedVehicleNumber <|> booking.vehicleNumber
        mbDriverRating = if isJust booking.driverId then ratingReq.driverRating else Nothing
        mbFleetRating = if isJust mbFleetNumber && isJust mbGtfsId then ratingReq.fleetRating else Nothing
    when (isJust ratingReq.driverRating && isNothing mbDriverRating) $
      logWarning $ "FRFS rating: booking " <> booking.id.getId <> " has no driverId; dropping the driver rating."
    when (isJust ratingReq.fleetRating && isNothing mbFleetRating) $
      logWarning $ "FRFS rating: booking " <> booking.id.getId <> " has no boarded vehicle or gtfsId; dropping the fleet rating."
    when (isJust mbDriverRating || isJust mbFleetRating) $ do
      let ratingBppReq =
