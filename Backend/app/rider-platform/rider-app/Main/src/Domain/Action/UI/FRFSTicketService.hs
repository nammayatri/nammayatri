  ...
  case integratedBppConfig.providerConfig of
    ...
    _ -> do
      ...
      FRFSReschedule.withRescheduleLock oldBooking.id $ do
        ...
        eStatusRes <- withTryCatch "FRFSReschedule:confirm" (postFrfsQuoteV2ConfirmUtil (Just personId, merchantId) stagingQuote selectedQuoteCategories Nothing (Just True) (Just False) Nothing integratedBppConfig (Just req.tripId) Nothing Nothing (Just (RescheduleCtx oldBooking.id oldPayment.id)))
