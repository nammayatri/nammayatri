...
getDriverLocWrapper ::
  ( HasCacheConfig r,
    ...
    HasFlowEnv m r '["dumpSimulatedDataTopicName" ::: Text],
            ...
            then do
              ...
              if doneCount > thresholdCount
                then do
                  ...
                  searchReqId <- CSSearchRequest.getSearchRequestIdByEstimateId estimateId >>= fromMaybeM (InvalidRequest "SimulatedFlow:Booking: search reqeust Id not found")
                  kafkaData <- CSSearchRequest.convertToKafkaData personId searchReqId CSSearchRequest.US
...
sendToKafka ::
  ...
  CSSearchRequest.KafkaDumpObject ->
  m ()
sendToKafka dataToDump = do
  topicName <- asks (.dumpSimulatedDataTopicName)
  produceMessage
    (topicName, Just (encodeUtf8 dataToDump.personId.getId))
    dataToDump
