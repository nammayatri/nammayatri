...
getFavorites (mbPersonId, merchantId) = do
  ...
  case (person.mobileNumber, person.mobileCountryCode) of
    (Just mobileNumber, Just countryCode) ->
      do
        CallBPPInternal.getFavouriteDriverList merchant.driverOfferApiKey merchant.driverOfferBaseUrl merchant.driverOfferMerchantId mobileNumber countryCode
        >>= pure . map (\r -> API.Types.UI.FavouriteDriver.FavouriteDriverResp {driverName = r.driverName, id = r.id, favCount = r.favCount, driverRating = r.driverRating, driverPhone = r.driverPhone})
    _ -> pure []
