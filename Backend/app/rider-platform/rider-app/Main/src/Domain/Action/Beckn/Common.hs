...
addOffersNammaTags ride person = do
  ...
  tags <- LYDL.computeNammaTagsWithExpiryAndDebugLog LYDL.Rider (cast person.merchantOperatingCityId) Yudhishthira.RideEndOffers (Nothing :: Maybe Text) (Y.EndRideOffersTagData customerData rideData)
    ...
    when (cancellationSource == DBCR.ByUser) $ do
      case mbRide of
        Just ride -> do
          ...
          nammaTags <- withTryCatch "computeNammaTags:RideCancel" (LYDL.computeNammaTagsWithDebugLog LYDL.Rider (cast booking.merchantOperatingCityId) Yudhishthira.RideCancel (Nothing :: Maybe Text) tagData)
