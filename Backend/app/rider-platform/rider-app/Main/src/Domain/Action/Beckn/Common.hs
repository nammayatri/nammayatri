    ...
    assignRideUpdate req'@ValidatedRideAssignedReq {..} mbMerchant rideStatus now = do
      ...
      whenJust req'.onlinePaymentParameters $ \OnlinePaymentParameters {..} -> do
        ...
        handle (SPayment.paymentErrorHandler booking) $
          withShortRetry $ do
            void $ SPayment.makePaymentIntent booking.merchantId merchantOperatingCityId booking.paymentMode booking.riderId ride createPaymentIntentReq
            ...
            whenJust mbPaymentOrder $ \paymentOrder -> do
              ...
              paymentInvoice <- SPInvoice.createPaymentInvoiceForOrder ride.id (Just paymentOrder.id) booking mbTipAmount paymentOrder.amount paymentOrder.currency paymentMethod now
      ...
      when (isNothing req'.onlinePaymentParameters) $ do
        let paymentMethod = fromMaybe DMPM.Cash booking.paymentInstrument
            ...
            -- Convert Maybe Price to Maybe HighPrecMoney by extracting the amount field
            mbTipAmount = ride.tipAmount <&> (.amount)
        paymentInvoice <- SPInvoice.createPaymentInvoiceForOrder ride.id Nothing booking mbTipAmount booking.estimatedFare.amount booking.estimatedFare.currency paymentMethod now
        QPaymentInvoice.create paymentInvoice
