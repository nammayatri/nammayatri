  ...
  depositApplied <-
    if isJust booking.bookingDepositAmount
      then case cancellationFee of
        ...
        Nothing -> do
          void $ withTryCatch "cancellationTransaction:refundBookingDeposit" $ BookingDeposit.refundBookingDeposit booking
      ...
      case person.payoutVpa of
        Just vpa -> do
          Redis.withWaitOnLockRedisWithExpiry (payoutProcessingLockKey person.id.getId) 5 5 $ do
            ...
            mbPayoutOrderResp <- withTryCatch "createPayoutService:handlePayout" $ Payout.createPayoutService (cast merchantId) (Just $ cast merchantOperatingCityId) (cast person.id) (Just [ride.id.getId]) (Just entityName) (show merchantOperatingCity.city) createPayoutOrderReq createPayoutOrderCall Nothing afterPayoutOrderCreated
...
sendBookingCancelledMessageViaWhatsapp personId riderConfig = do
  ...
  result <- Whatsapp.whatsAppSendMessageWithTemplateIdAPI person.merchantId person.merchantOperatingCityId (Whatsapp.SendWhatsAppMessageWithTemplateIdApIReq phoneNumber merchantMessage.templateId [Just riderConfig.appUrl] Nothing Nothing merchantMessage.mediaUrl) -- Accepts at most 7 variables using GupShup
