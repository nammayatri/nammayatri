...
rideStartedReqHandler ValidatedRideStartedReq {..} = do
  ...
  where
    ...
    convertNonRideSosToRide person riderConfig = do
      mbNonRideSos <- SafetyQSos.findLatestPendingNonRideSosByPersonId (cast booking.riderId) SafetyDSos.Pending (Just SafetyDSos.NonRide)
      whenJust mbNonRideSos $ \sos -> do
