...
getMultimodalBookingInfo (mbPersonId, _merchantId) journeyId = do
  ...
  let failedBookings = filter ((== DFRFSB.FAILED) . (.status)) allJourneyFrfsBookings
      refundInitiatedBookings = filter (\booking -> fromMaybe False booking.isRefundInitiated) failedBookings
      nonRefundInitiatedBookings = filter (\booking -> not (fromMaybe False booking.isRefundInitiated)) failedBookings
  updatedLegs <-
