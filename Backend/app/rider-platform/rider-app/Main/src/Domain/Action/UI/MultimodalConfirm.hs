...
postMultimodalOrderSwitchTaxi (_, _) journeyId legOrder req = do
  ...
  unless (journeyLegInfo.travelMode == DTrip.Taxi) $
    throwError $ InvalidRequest $ "Cannot cancel leg for mode: " <> show journeyLegInfo.travelMode
  when (journeyLegInfo.status `elem` JMTypes.cannotSwitchStatus) $
    throwError $ InvalidRequest $ "Cannot cancel leg for status: " <> show journeyLegInfo.status
