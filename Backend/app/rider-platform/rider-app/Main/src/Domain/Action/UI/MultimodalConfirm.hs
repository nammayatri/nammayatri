...
postMultimodalRouteServiceability (mbPersonId, _merchantId) req =
  ...
  where
    ...
    getValidSingleModeRoute routeServiceabilityContext srcCode' destCode' = do
      ...
      otpResponse <- JMU.measureLatency (PlanCache.getTransitRoutesCached routeServiceabilityContext.merchantOperatingCityId routeServiceabilityContext.merchantId Nothing transitServiceReq transitRoutesReq >>= fromMaybeM (OTPServiceUnavailable "No routes found from OTP")) ("MultiModal.getTransitRoutes req=" <> show transitRoutesReq)
