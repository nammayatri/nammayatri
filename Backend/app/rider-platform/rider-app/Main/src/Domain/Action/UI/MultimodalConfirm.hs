      ...
      Just vehicleNumber -> do
        ...
        case mbVehicleOverrideInfo of
          Just (updatedVehicleNumber, newDeviceWaybillNo) -> do
            updatedVehicleRouteInfo <- JLU.getVehicleLiveRouteInfo integratedBPPConfigs updatedVehicleNumber Nothing >>= fromMaybeM (InvalidVehicleNumber $ "Vehicle " <> vehicleNumber <> ", not found on any route")
...
postMultimodalRouteServiceability (mbPersonId, _merchantId) req = do
  ...
  mbRouteBusData <-
    case req.routeCode of
      ...
      _ -> do
        pure Nothing
  (srcCode, destCode) <- fromMaybeM (InvalidRequest $ "Source or destination stop code not found for req: " <> show req) $
    case (req.sourceStopCode, req.destinationStopCode) of
      ...
      (Nothing, Just destinationStopCode) -> do
        let sourceStopCode = fmap (.stopCode) $ listToMaybe routeStopMapping
