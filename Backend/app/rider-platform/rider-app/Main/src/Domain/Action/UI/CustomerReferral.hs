...
processBacklogReferralPayout personId vpa merchantOpCityId = do
  person <- QPerson.findById personId >>= fromMaybeM (PersonNotFound personId.getId)
  mbPayoutConfig <- CPC.findByCityIdAndVehicleCategoryAndIsPayoutEnabled person.merchantOperatingCityId True VehicleCategory.AUTO_CATEGORY
    ...
    handlePayout person amount mbPayoutConfig entityName = do
      case mbPayoutConfig of
        Just payoutConfig -> do
          ...
          void $ try @_ @SomeException $ Payout.createPayoutService (cast person.merchantId) (cast person.id) (Just []) (Just entityName) (show merchantOperatingCity.city) createPayoutOrderReq createPayoutOrderCall
