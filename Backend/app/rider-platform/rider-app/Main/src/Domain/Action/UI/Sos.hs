...
postSosCreate (mbPersonId, _merchantId) req = do
  ...
  riderConfig <- getConfig (RiderConfigDimensions person.merchantOperatingCityId.getId) Nothing >>= fromMaybeM (RiderConfigDoesNotExist person.merchantOperatingCityId.getId)
        ...
        when sosConfig.mediaRequired $ do
          ...
          whenJust sosDetails.externalReferenceId $ \_ -> do
            ...
            whenJust phoneNumber $ \phoneNo -> do
              ...
              merchantSvcCfg <-
                getOneConfig (MerchantServiceConfigDimensions {merchantOperatingCityId = person.merchantOperatingCityId.getId, merchantId = person.merchantId.getId, serviceName = Just (DMSC.SOSService sosServiceType)}) Nothing
  ...
  case sosDetails.ticketId of
    Just ticketId -> do
      fork "update ticket request" $
        void $ Ticket.updateSosTicket (person.merchantId) person.merchantOperatingCityId Ticket.UpdateTicketReq {comment = fromMaybe "" mbComment, ticketId = ticketId, status = Ticket.Pending, rideDescription = Nothing, issueDetails = Nothing, requesterId = sosDetails.requesterId, ticketContext = Just Ticket.SOSAlert, name = Nothing, phoneNo = Nothing, xyneChannelId = Nothing}
      pure APISuccess.Success
    ...
    (Just _, Just merchantId, Just merchantOpCityId) -> do
      ...
      case mbRiderConfig >>= (.externalSOSConfig) of
        Just sosConfig | sosConfig.latLonRequired -> do
          let sosServiceType = flowToSOSService sosConfig.flow
          mbMerchantSvcCfg <- getOneConfig (MerchantServiceConfigDimensions {merchantOperatingCityId = merchantOpCityId'.getId, merchantId = merchantId.getId, serviceName = Just (DMSC.SOSService sosServiceType)}) Nothing
  ...
  isEmergencyContact <-
    if isSosCreator
      then return True
        ...
        return $ Foldable.any (\ec -> ec.contactPersonId == Just (cast personId)) emergencyContacts
        ...
        void $
          withTryCatch "updateSosTicket:sosUpdateToRide" $
            Ticket.updateSosTicket person.merchantId person.merchantOperatingCityId
              Ticket.UpdateTicketReq {comment = "SOS converted from non-ride to ride", ticketId = existingTicketId, status = Ticket.Pending, rideDescription = Just rideInfo, issueDetails = Nothing, requesterId = sosDetails.requesterId, ticketContext = Just Ticket.SOSAlert, name = Nothing, phoneNo = Nothing, xyneChannelId = Nothing}
      Nothing -> do
...
handleExternalSOS person sosConfig req personId riderConfig = do
  let sosServiceType = flowToSOSService sosConfig.flow
  mbMerchantSvcCfg <- getOneConfig (MerchantServiceConfigDimensions {merchantOperatingCityId = person.merchantOperatingCityId.getId, merchantId = person.merchantId.getId, serviceName = Just (DMSC.SOSService sosServiceType)}) Nothing
