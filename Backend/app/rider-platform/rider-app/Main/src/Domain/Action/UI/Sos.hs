...
createSosDetails personId merchantId req = do
  ...
  ticketId <- do
    if riderConfig.enableSupportForSafety
      then do
        ticketResponse <- try @_ @SomeException (createTicket person.merchantId person.merchantOperatingCityId (mkTicket person phoneNumber trackLink rideInfo))
...
addSosVideo sosId personId SOSVideoUploadReq {..} = do
  ...
  case result of
    ...
    Right _ -> do
      void $ callUpdateTicket person sosDetails fileUrl
