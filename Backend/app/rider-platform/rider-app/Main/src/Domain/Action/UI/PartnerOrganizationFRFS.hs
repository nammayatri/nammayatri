...
createNewBookingAndTriggerInit partnerOrg req regPOCfg = do
  ...
  if not redisLockSearchId
    then throwError $ RedisLockStillProcessing lockKey
      ...
      bookingRes <- DFRFSTicketService.postFrfsQuoteV2ConfirmUtil (Just personId, fromStation.merchantId) quote.id (FRFSTypes.FRFSQuoteConfirmReq {offered = Nothing, ticketQuantity = Nothing, childTicketQuantity = Nothing}) Nothing Nothing Nothing
