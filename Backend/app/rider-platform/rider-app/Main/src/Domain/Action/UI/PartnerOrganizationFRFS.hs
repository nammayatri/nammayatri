...
getFareV2 merchantOperatingCity partnerOrg fromStation toStation partnerOrgTransactionId routeCode integratedBPPConfig = do
  ...
  fork ("FRFS Search: " <> searchReq.id.getId) $ do
    ...
    CallExternalBPP.search merchant merchantOperatingCity bapConfig searchReq Nothing frfsRouteDetails integratedBPPConfig Nothing
...
createNewBookingAndTriggerInit partnerOrg req regPOCfg = do
  ...
  if not redisLockSearchId
    then throwError $ RedisLockStillProcessing lockKey
      ...
      bookingRes <- DFRFSTicketService.postFrfsQuoteV2ConfirmUtil (Just personId, fromStation.merchantId) quote.id (FRFSTypes.FRFSQuoteConfirmReq {offered = [], ticketQuantity = Nothing, childTicketQuantity = Nothing}) Nothing
