...
getRegToken personId pOrgId regPOCfg mId isPartnerOrgAuth = do
  ...
  RegistrationToken.findAllByPersonId entityId
    ...
    >>= maybe (makeSessionViaPartner regPOCfg.sessionConfig entityId.getId mId.getId regPOCfg.fakeOtp pOrgId isPartnerOrgAuth) return
...
makeSessionViaPartner sessionConfig entityId mId fakeOtp partnerOrgId isPartnerOrgAuth = do
  ...
  when isPartnerOrgAuth $ void $ RegistrationToken.setDirectAuth regToken.id authMedium
...
partnerOrgAuth ticketBookingId = do
  ...
  case ticketBooking.partnerOrgId of
    ...
    Just partnerOrgId -> do
      ...
      (regToken, isTokenNew) <- getRegToken entityId partnerOrgId regPOCfg{fakeOtp = newOtp, sessionConfig = updatedSessionConfig} ticketBooking.merchantId isPartnerOrgAuth
