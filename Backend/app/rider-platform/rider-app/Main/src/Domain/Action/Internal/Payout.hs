...
juspayPayoutWebhookHandler merchantShortId mbOpCity authData value = do
  ...
  case osr of
    IPayout.OrderStatusPayoutResp {..} -> do
      ...
      payoutConfig <- CPC.findByCityIdAndVehicleCategoryAndIsPayoutEnabled merchanOperatingCityId True DV.AUTO_CATEGORY >>= fromMaybeM (PayoutConfigNotFound "AUTO_CATEGORY" merchanOperatingCityId.getId)
