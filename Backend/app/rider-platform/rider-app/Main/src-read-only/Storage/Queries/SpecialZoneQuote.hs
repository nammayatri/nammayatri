...
updateByPrimaryKey (Domain.Types.SpecialZoneQuote.SpecialZoneQuote {..}) = do
  _now <- getCurrentTime
  updateWithKV [Se.Set Beam.createdAt createdAt, Se.Set Beam.quoteId quoteId, Se.Set Beam.updatedAt (Just _now)] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]
...
instance FromTType' Beam.SpecialZoneQuote Domain.Types.SpecialZoneQuote.SpecialZoneQuote where
  fromTType' (Beam.SpecialZoneQuoteT {..}) = do
    pure $
      Just
        Domain.Types.SpecialZoneQuote.SpecialZoneQuote
          { createdAt = createdAt,
