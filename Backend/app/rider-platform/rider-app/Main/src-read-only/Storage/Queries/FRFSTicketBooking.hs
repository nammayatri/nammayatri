...
updateBPPOrderIdAndStatusById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateBPPOrderIdAndStatusById bppOrderId status id = do {_now <- getCurrentTime;
...
updateBppBankDetailsById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateBppBankDetailsById bppBankAccountNumber bppBankCode id = do {_now <- getCurrentTime;
...
updateCustomerCancelledByBookingId customerCancelled id = do {_now <- getCurrentTime;
...
updateFailureReasonById failureReason id = do {_now <- getCurrentTime;
...
updateGoogleWalletLinkById googleWalletJWTUrl id = do {_now <- getCurrentTime;
...
updateInterestDelayedByQuoteId bppDelayedInterest quoteId = do {_now <- getCurrentTime;
...
updateIsBookingCancellableByBookingId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                         (Kernel.Prelude.Maybe Kernel.Prelude.Bool -> Kernel.Types.Id.Id Domain.Types.FRFSTicketBooking.FRFSTicketBooking -> m ())
updateIsBookingCancellableByBookingId isBookingCancellable id = do {_now <- getCurrentTime;
...
updateIsFareChangedById isFareChanged id = do {_now <- getCurrentTime;
...
updateOnInitDone journeyOnInitDone id = do {_now <- getCurrentTime;
...
updateOnInitDoneBySearchId journeyOnInitDone searchId = do {_now <- getCurrentTime;
...
updateOndcOnInitReceivedById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateOndcOnInitReceivedById ondcOnInitReceived ondcOnInitReceivedAt id = do {_now <- getCurrentTime;
                                                                              updateOneWithKV [Se.Set Beam.ondcOnInitReceived ondcOnInitReceived,
...
updatePayoutOrderId cashbackPayoutOrderId id = do {_now <- getCurrentTime;
...
updatePayoutStatusById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updatePayoutStatusById cashbackStatus id = do {_now <- getCurrentTime;
...
updateQuoteAndBppItemIdAndRouteStationsJson :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateQuoteAndBppItemIdAndRouteStationsJson quoteId bppItemId routeStationsJson id = do {_now <- getCurrentTime;
                                                                                         updateOneWithKV [Se.Set Beam.quoteId (Kernel.Types.Id.getId quoteId),
                                                                                                          Se.Set Beam.bppItemId bppItemId,
                                                                                                          Se.Set Beam.routeStationsJson routeStationsJson,
                                                                                                          Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]}
updateRefundCancellationChargesAndIsCancellableByBookingId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateRefundCancellationChargesAndIsCancellableByBookingId refundAmount cancellationCharges isBookingCancellable id = do {_now <- getCurrentTime;
                                                                                                                          updateOneWithKV [Se.Set Beam.refundAmount refundAmount,
...
updateStatusValidTillAndPaymentTxnByIdAndTicketBookingPaymentId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateStatusValidTillAndPaymentTxnByIdAndTicketBookingPaymentId status validTill paymentTxnId frfsTicketBookingPaymentIdForTicketGeneration id = do {_now <- getCurrentTime;
...
updateTotalPriceById totalPrice id = do {_now <- getCurrentTime;
                                         updateOneWithKV [Se.Set Beam.currency (((Kernel.Prelude.Just . (.currency))) totalPrice),
...
updateByPrimaryKey (Domain.Types.FRFSTicketBooking.FRFSTicketBooking {..}) = do {_now <- getCurrentTime;
                                                                                 updateWithKV [Se.Set Beam._type _type,
                                                                                               ...
                                                                                               Se.Set Beam.fromStopIdx fromStopIdx,
