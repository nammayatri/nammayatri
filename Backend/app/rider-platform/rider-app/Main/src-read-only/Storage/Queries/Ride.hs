...
markPaymentStatus paymentStatus id = do {_now <- getCurrentTime;
...
updateCancellationChargesOnCancel cancellationChargesOnCancel id = do {_now <- getCurrentTime;
...
updateCancellationFeeIfCancelledField cancellationFeeIfCancelled id = do {_now <- getCurrentTime;
...
updateCancellationFeeStatus cancellationFeeStatus id = do {_now <- getCurrentTime;
...
updateDestinationReachedAt destinationReachedAt id = do {_now <- getCurrentTime;
...
updateEstimatedEndTimeRange estimatedEndTimeRange id = do {_now <- getCurrentTime;
                                                           updateOneWithKV [Se.Set Beam.estimatedEndTimeRangeEnd (Kernel.Prelude.fmap (.end) estimatedEndTimeRange),
...
updateFeedbackSkipped feedbackSkipped id = do {_now <- getCurrentTime;
...
updatePickupEtaLogicVersion pickupEtaLogicVersion id = do {_now <- getCurrentTime;
...
updatePickupRouteCallCount pickupRouteCallCount id = do {_now <- getCurrentTime;
...
updatePickupSpeedInMPS pickupSpeedInMPS id = do {_now <- getCurrentTime;
...
updateRefundRequestStatus refundRequestStatus id = do {_now <- getCurrentTime;
...
updateTalkedWithDriver talkedWithDriver id = do {_now <- getCurrentTime;
...
updateTipByRideId tipAmount id = do {_now <- getCurrentTime;
