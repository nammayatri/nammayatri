...
findFiringIncident :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findFiringIncident limit offset description status = do findAllWithOptionsKV [Se.And [Se.Is Beam.description $ Se.Eq description,
                                                                                      Se.Is Beam.status $ Se.Eq status]] (Se.Desc Beam.firingTime) limit offset
findFiringIncidentByAlertName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findFiringIncidentByAlertName limit offset alertName status = do findAllWithOptionsKV [Se.And [Se.Is Beam.alertName $ Se.Eq alertName,
                                                                                               Se.Is Beam.status $ Se.Eq status]] (Se.Desc Beam.firingTime) limit offset
findIncidentToResolve :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findIncidentToResolve limit offset description resolvedTime = do findAllWithOptionsKV [Se.And [Se.Is Beam.description $ Se.Eq description,
                                                                                               Se.Is Beam.resolvedTime $ Se.Eq resolvedTime]] (Se.Desc Beam.firingTime) limit offset
findIncidentToResolveByAlertName :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findIncidentToResolveByAlertName limit offset alertName resolvedTime = do findAllWithOptionsKV [Se.And [Se.Is Beam.alertName $ Se.Eq alertName,
                                                                                                        Se.Is Beam.resolvedTime $ Se.Eq resolvedTime]] (Se.Desc Beam.firingTime) limit offset
updateToResolved :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateToResolved status resolvedTime downtimeSeconds rca id = do {_now <- getCurrentTime;
...
instance FromTType' Beam.AlertIncident Domain.Types.AlertIncident.AlertIncident
    where fromTType' (Beam.AlertIncidentT {..}) = do pure $ Just Domain.Types.AlertIncident.AlertIncident{alertGroup = alertGroup,
                                                                                                          alertName = alertName,
                                                                                                          createdAt = createdAt,
                                                                                                          ...
                                                                                                          status = status,
                                                                                                          updatedAt = updatedAt}
instance ToTType' Beam.AlertIncident Domain.Types.AlertIncident.AlertIncident
    where toTType' (Domain.Types.AlertIncident.AlertIncident {..}) = do Beam.AlertIncidentT{Beam.alertGroup = alertGroup,
