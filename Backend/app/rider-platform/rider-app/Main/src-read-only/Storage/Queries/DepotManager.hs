...
updateByPrimaryKey (Domain.Types.DepotManager.DepotManager {..}) = do {_now <- getCurrentTime;
                                                                       updateWithKV [Se.Set Beam.enabled enabled,
                                                                                     Se.Set Beam.isAdmin isAdmin,
                                                                                     Se.Set Beam.merchantId (Kernel.Types.Id.getId merchantId),
                                                                                     Se.Set Beam.merchantOperatingCityId (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                     Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.depotCode $ Se.Eq (Kernel.Types.Id.getId depotCode), Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId)]]}



instance FromTType' Beam.DepotManager Domain.Types.DepotManager.DepotManager
    where fromTType' (Beam.DepotManagerT {..}) = do pure $ Just Domain.Types.DepotManager.DepotManager{createdAt = createdAt,
                                                                                                       ...
                                                                                                       updatedAt = updatedAt}
instance ToTType' Beam.DepotManager Domain.Types.DepotManager.DepotManager
    where toTType' (Domain.Types.DepotManager.DepotManager {..}) = do Beam.DepotManagerT{Beam.createdAt = createdAt,
