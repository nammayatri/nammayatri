...
updateByPrimaryKey (Domain.Types.BBPSConfig.BBPSConfig {..}) = do {_now <- getCurrentTime;
                                                                   updateWithKV [Se.Set Beam.bbpsAgentId bbpsAgentId,
                                                                                 Se.Set Beam.bbpsServerUrl bbpsServerUrl,
                                                                                 Se.Set Beam.bbpsSignatureKey (bbpsSignatureKey & unEncrypted),
                                                                                 Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId merchantId)]]}



instance FromTType' Beam.BBPSConfig Domain.Types.BBPSConfig.BBPSConfig
    where fromTType' (Beam.BBPSConfigT {..}) = do pure $ Just Domain.Types.BBPSConfig.BBPSConfig{bbpsAgentId = bbpsAgentId,
                                                                                                 bbpsServerUrl = bbpsServerUrl,
                                                                                                 bbpsSignatureKey = Encrypted bbpsSignatureKey,
                                                                                                 merchantId = Kernel.Types.Id.Id merchantId,
                                                                                                 createdAt = createdAt,
                                                                                                 updatedAt = updatedAt}
instance ToTType' Beam.BBPSConfig Domain.Types.BBPSConfig.BBPSConfig
    where toTType' (Domain.Types.BBPSConfig.BBPSConfig {..}) = do Beam.BBPSConfigT{Beam.bbpsAgentId = bbpsAgentId,
