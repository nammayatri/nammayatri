...
findByTripIds :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByTripIds tripId integratedBppConfigId = do findAllWithKV [Se.And [Se.Is Beam.tripId $ Se.In (Kernel.Types.Id.getId <$> tripId),
...
findByPrimaryKey integratedBppConfigId tripId = do findOneWithKV [Se.And [Se.Is Beam.integratedBppConfigId $ Se.Eq (Kernel.Types.Id.getId integratedBppConfigId),
...
updateByPrimaryKey (Domain.Types.RouteStopCalender.RouteStopCalender {..}) = do {_now <- getCurrentTime;
                                                                                 updateWithKV [Se.Set Beam.serviceability serviceability,
                                                                                               Se.Set Beam.merchantId (Kernel.Types.Id.getId <$> merchantId),
                                                                                               Se.Set Beam.merchantOperatingCityId (Kernel.Types.Id.getId <$> merchantOperatingCityId),
                                                                                               Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.integratedBppConfigId $ Se.Eq (Kernel.Types.Id.getId integratedBppConfigId), Se.Is Beam.tripId $ Se.Eq (Kernel.Types.Id.getId tripId)]]}



instance FromTType' Beam.RouteStopCalender Domain.Types.RouteStopCalender.RouteStopCalender
    where fromTType' (Beam.RouteStopCalenderT {..}) = do pure $ Just Domain.Types.RouteStopCalender.RouteStopCalender{integratedBppConfigId = Kernel.Types.Id.Id integratedBppConfigId,
                                                                                                                      ...
                                                                                                                      createdAt = createdAt,
                                                                                                                      updatedAt = updatedAt}
instance ToTType' Beam.RouteStopCalender Domain.Types.RouteStopCalender.RouteStopCalender
    where toTType' (Domain.Types.RouteStopCalender.RouteStopCalender {..}) = do Beam.RouteStopCalenderT{Beam.integratedBppConfigId = Kernel.Types.Id.getId integratedBppConfigId,
