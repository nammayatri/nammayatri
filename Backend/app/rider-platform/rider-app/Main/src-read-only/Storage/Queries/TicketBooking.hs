...
getAllBookingsByPersonId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                            (Maybe Int -> Maybe Int -> Kernel.Types.Id.Id Domain.Types.Person.Person -> Kernel.Types.Id.Id Domain.Types.MerchantOperatingCity.MerchantOperatingCity -> m ([Domain.Types.TicketBooking.TicketBooking]))
getAllBookingsByPersonId limit offset personId merchantOperatingCityId = do findAllWithOptionsKV [Se.And [Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId),
...
getAllBookingsByPersonIdAndStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
getAllBookingsByPersonIdAndStatus limit offset personId merchantOperatingCityId status = do findAllWithOptionsKV [Se.And [Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId),
                                                                                                                          Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                          Se.Is Beam.status $ Se.Eq status]] (Se.Desc Beam.createdAt) limit offset
getAllBookingsByPlaceIdAndVisitDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
getAllBookingsByPlaceIdAndVisitDate ticketPlaceId visitDate status = do findAllWithKV [Se.And [Se.Is Beam.ticketPlaceId $ Se.Eq (Kernel.Types.Id.getId ticketPlaceId),
                                                                                               Se.Is Beam.visitDate $ Se.Eq visitDate,
                                                                                               Se.Is Beam.status $ Se.Eq status]]
updatePaymentMethodByShortId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updatePaymentMethodByShortId paymentMethod shortId = do {_now <- getCurrentTime;
...
updateStatusAndCancelledSeatsById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateStatusAndCancelledSeatsById status cancelledSeats id = do {_now <- getCurrentTime;
...
updateStatusByShortId status shortId = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.TicketBooking.TicketBooking {..}) = do {_now <- getCurrentTime;
