...
updateAddress address id = do {_now <- getCurrentTime;
                               updateOneWithKV [Se.Set Beam.area (Domain.Types.LocationAddress.area address),
...
instance FromTType' Beam.BookingLocation Domain.Types.BookingLocation.BookingLocation
    where fromTType' (Beam.BookingLocationT {..}) = do pure $ Just Domain.Types.BookingLocation.BookingLocation{address = Domain.Types.LocationAddress.LocationAddress {street,door,city,state,country,building,areaCode,area,ward,placeId,instructions,title,extras},
                                                                                                                id = Kernel.Types.Id.Id id,
                                                                                                                lat = lat,
                                                                                                                lon = lon,
                                                                                                                createdAt = createdAt,
                                                                                                                updatedAt = updatedAt}
instance ToTType' Beam.BookingLocation Domain.Types.BookingLocation.BookingLocation
    where toTType' (Domain.Types.BookingLocation.BookingLocation {..}) = do Beam.BookingLocationT{Beam.area = Domain.Types.LocationAddress.area address,
