...
updateAfterEditLocation :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateAfterEditLocation duration distance endLocation id = do {_now <- getCurrentTime;
                                                               updateWithKV [Se.Set Beam.duration duration,
...
updateDistanceAndDuration :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateDistanceAndDuration distance duration id = do {_now <- getCurrentTime;
                                                     updateWithKV [Se.Set Beam.distance ((.value) <$>  distance),
                                                                   Se.Set Beam.distanceUnit ((.unit) <$>  distance),
...
updateLegPricingIdByLegSearchId legPricingId legSearchId = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.JourneyLeg.JourneyLeg {..}) = do {_now <- getCurrentTime;
                                                                   updateWithKV [Se.Set Beam.agencyGtfsId ((agency >>= (.gtfsId))),
                                                                                 ...
                                                                                 Se.Set Beam.userPreferredServiceTier userPreferredServiceTier,
