...
findAllById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllServicePeopleCategory :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                (Kernel.Types.Id.Id Domain.Types.ServicePeopleCategory.ServicePeopleCategory -> m ([Domain.Types.ServicePeopleCategory.ServicePeopleCategory]))
findAllServicePeopleCategory id = do findAllWithKV [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]
findByIdAndTimebounds :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateByIdAndTimebounds (Domain.Types.ServicePeopleCategory.ServicePeopleCategory {..}) = do {_now <- getCurrentTime;
                                                                                              updateOneWithKV [Se.Set Beam.name name,
                                                                                                               ...
                                                                                                               Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id), Se.Is Beam.timeBounds $ Se.Eq (Kernel.Prelude.Just timeBounds)]]}
...
updateByPrimaryKey (Domain.Types.ServicePeopleCategory.ServicePeopleCategory {..}) = do {_now <- getCurrentTime;
                                                                                         updateWithKV [Se.Set Beam.cancellationCharges (convertCancellationChargesToTable cancellationCharges),
                                                                                                       Se.Set Beam.description description,
                                                                                                       Se.Set Beam.iconUrl iconUrl,
