...
updateDeviceSwitchCount deviceSwitchCount id = do {_now <- getCurrentTime;
...
updatePrefSrcAndDestById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updatePrefSrcAndDestById preferredSource preferredDestination id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.PurchasedPass.PurchasedPass {..}) = do {_now <- getCurrentTime;
                                                                         updateWithKV [Se.Set Beam.applicableVehicleServiceTiers applicableVehicleServiceTiers,
