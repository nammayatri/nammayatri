...
findAllByTicketMerchantIdAndStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByNameAndCity :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateRecommendById recommend id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.TicketPlace.TicketPlace {..}) = do {_now <- getCurrentTime;
                                                                     updateWithKV [Se.Set Beam.allowSameDayBooking allowSameDayBooking,
...
instance FromTType' Beam.TicketPlace Domain.Types.TicketPlace.TicketPlace
    where fromTType' (Beam.TicketPlaceT {..}) = do pure $ Just Domain.Types.TicketPlace.TicketPlace{allowSameDayBooking = allowSameDayBooking,
                                                                                                    ...
                                                                                                    createdAt = createdAt,
                                                                                                    updatedAt = updatedAt}
instance ToTType' Beam.TicketPlace Domain.Types.TicketPlace.TicketPlace
    where toTType' (Domain.Types.TicketPlace.TicketPlace {..}) = do Beam.TicketPlaceT{Beam.allowSameDayBooking = allowSameDayBooking,
