...
findByTicketServiceCategoryIdAndDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByTicketServiceCategoryIdAndDate ticketServiceCategoryId date = do findOneWithKV [Se.And [Se.Is Beam.ticketServiceCategoryId $ Se.Eq (Kernel.Types.Id.getId ticketServiceCategoryId),
...
safeUpdateBookedSeats booked ticketServiceCategoryId date = do {_now <- getCurrentTime;
                                                                updateWithKV [Se.Set Beam.booked booked, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.ticketServiceCategoryId $ Se.Eq (Kernel.Types.Id.getId ticketServiceCategoryId),
                                                                                                                                              Se.Is Beam.date $ Se.Eq date,
...
updateBlockedSeats blocked ticketServiceCategoryId date = do {_now <- getCurrentTime;
                                                              updateWithKV [Se.Set Beam.blocked blocked, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.ticketServiceCategoryId $ Se.Eq (Kernel.Types.Id.getId ticketServiceCategoryId),
                                                                                                                                              Se.Is Beam.date $ Se.Eq date]]}
updateBookedAndBlockedSeats :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                               (Kernel.Prelude.Int -> Kernel.Prelude.Int -> Kernel.Types.Id.Id Domain.Types.ServiceCategory.ServiceCategory -> Data.Time.Day -> m ())
updateBookedAndBlockedSeats booked blocked ticketServiceCategoryId date = do {_now <- getCurrentTime;
                                                                              updateOneWithKV [Se.Set Beam.booked booked,
                                                                                               Se.Set Beam.blocked blocked,
                                                                                               Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.ticketServiceCategoryId $ Se.Eq (Kernel.Types.Id.getId ticketServiceCategoryId), Se.Is Beam.date $ Se.Eq date]]}
...
updateBookedSeats booked ticketServiceCategoryId date = do {_now <- getCurrentTime;
                                                            updateWithKV [Se.Set Beam.booked booked, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.ticketServiceCategoryId $ Se.Eq (Kernel.Types.Id.getId ticketServiceCategoryId),
                                                                                                                                          Se.Is Beam.date $ Se.Eq date]]}
updateMaxCapacity :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                     (Kernel.Prelude.Maybe Kernel.Prelude.Int -> Kernel.Types.Id.Id Domain.Types.ServiceCategory.ServiceCategory -> Data.Time.Day -> m ())
updateMaxCapacity maxCapacity ticketServiceCategoryId date = do {_now <- getCurrentTime;
                                                                 updateOneWithKV [Se.Set Beam.maxCapacity maxCapacity, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.ticketServiceCategoryId $ Se.Eq (Kernel.Types.Id.getId ticketServiceCategoryId),
                                                                                                                                                            Se.Is Beam.date $ Se.Eq date]]}
...
updateByPrimaryKey (Domain.Types.SeatManagement.SeatManagement {..}) = do {_now <- getCurrentTime;
...
instance FromTType' Beam.SeatManagement Domain.Types.SeatManagement.SeatManagement
    where fromTType' (Beam.SeatManagementT {..}) = do pure $ Just Domain.Types.SeatManagement.SeatManagement{blocked = blocked,
                                                                                                             booked = booked,
                                                                                                             date = date,
                                                                                                             ...
                                                                                                             createdAt = createdAt,
                                                                                                             updatedAt = updatedAt}
instance ToTType' Beam.SeatManagement Domain.Types.SeatManagement.SeatManagement
    where toTType' (Domain.Types.SeatManagement.SeatManagement {..}) = do Beam.SeatManagementT{Beam.blocked = blocked,
                                                                                               Beam.booked = booked,
                                                                                               Beam.date = date,
