...
updatePassOrganization name address id = do
  _now <- getCurrentTime
  updateWithKV [Se.Set Beam.name name, Se.Set Beam.address address, Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]
