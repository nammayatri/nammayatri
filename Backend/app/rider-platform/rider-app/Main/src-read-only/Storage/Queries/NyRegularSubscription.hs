...
confirmSubscriptionDetailsById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
confirmSubscriptionDetailsById vehicleServiceTier fixedPrice fixedPriceBreakupDetails fixedPriceExpiryDate initialBppQuoteId status id = do {_now <- getCurrentTime;
                                                                                                                                             updateWithKV [Se.Set Beam.vehicleServiceTier vehicleServiceTier,
...
findByUserId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateLastProcessedAtById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                             (Kernel.Prelude.Maybe Data.Time.UTCTime -> Kernel.Types.Id.Id Domain.Types.NyRegularSubscription.NyRegularSubscription -> m ())
updateLastProcessedAtById lastProcessedAt id = do {_now <- getCurrentTime;
...
updatePauseDetailsById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updatePauseDetailsById pauseStartDate pauseEndDate status id = do {_now <- getCurrentTime;
                                                                   updateWithKV [Se.Set Beam.pauseStartDate pauseStartDate,
                                                                                 Se.Set Beam.pauseEndDate pauseEndDate,
                                                                                 Se.Set Beam.status status,
                                                                                 Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]}
updateRecurrenceById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateRecurrenceById recurrenceRuleDays scheduledTimeOfDay recurrenceEndDate id = do {_now <- getCurrentTime;
                                                                                      updateWithKV [Se.Set Beam.recurrenceRuleDays (map show recurrenceRuleDays),
                                                                                                    Se.Set Beam.scheduledTimeOfDay scheduledTimeOfDay,
                                                                                                    Se.Set Beam.recurrenceEndDate recurrenceEndDate,
                                                                                                    Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]}
updateSchedulingHashById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                            (Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Types.Id.Id Domain.Types.NyRegularSubscription.NyRegularSubscription -> m ())
updateSchedulingHashById schedulingHash id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.NyRegularSubscription.NyRegularSubscription {..}) = do {_now <- getCurrentTime;
