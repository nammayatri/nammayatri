...
updatePaymentInformationAndStatusByRefId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updatePaymentInformationAndStatusByRefId paymentTxnId paymentMode paymentInformation status refId = do {_now <- getCurrentTime;
                                                                                                        updateOneWithKV [Se.Set Beam.paymentTxnId paymentTxnId,
                                                                                                                         Se.Set Beam.paymentMode paymentMode,
                                                                                                                         Se.Set Beam.paymentInformation (Kernel.Prelude.toJSON <$> paymentInformation),
                                                                                                                         Se.Set Beam.status status,
                                                                                                                         Se.Set Beam.updatedAt _now] [Se.Is Beam.refId $ Se.Eq (Kernel.Types.Id.getId refId)]}
updateStatusAndErrorMessageByRefId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                      (Domain.Types.BBPS.BBPSPaymentStatus -> Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Types.Id.Id Domain.Types.BBPS.BBPS -> m ())
updateStatusAndErrorMessageByRefId status errorMessage refId = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.BBPS.BBPS {..}) = do {_now <- getCurrentTime;
                                                       updateWithKV [Se.Set Beam.amount amount,
                                                                     ...
                                                                     Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.refId $ Se.Eq (Kernel.Types.Id.getId refId)]]}
