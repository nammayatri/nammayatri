...
updateBacklogAndReferredByPayoutStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateBacklogAndReferredByPayoutStatus backlogPayoutStatus referredByEarningsPayoutStatus personId = do {_now <- getCurrentTime;
                                                                                                         updateOneWithKV [Se.Set Beam.backlogPayoutStatus backlogPayoutStatus,
...
updateBacklogAndReferredByPayoutStatusAndAmountPaid :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateBacklogAndReferredByPayoutStatusAndAmountPaid backlogPayoutStatus referredByEarningsPayoutStatus referralAmountPaid personId = do {_now <- getCurrentTime;
                                                                                                                                         updateOneWithKV [Se.Set Beam.backlogPayoutStatus backlogPayoutStatus,
                                                                                                                                                          Se.Set Beam.referredByEarningsPayoutStatus referredByEarningsPayoutStatus,
...
updateBacklogPayoutStatus backlogPayoutStatus personId = do {_now <- getCurrentTime;
...
updateBacklogStatusAndAmountPaid :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateBacklogStatusAndAmountPaid backlogPayoutStatus referralAmountPaid personId = do {_now <- getCurrentTime;
                                                                                       updateOneWithKV [Se.Set Beam.backlogPayoutStatus backlogPayoutStatus,
                                                                                                        Se.Set Beam.referralAmountPaid (Kernel.Prelude.Just referralAmountPaid),
                                                                                                        Se.Set Beam.updatedAt _now] [Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId)]}
updateEarningsAndActivations :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateEarningsAndActivations referralEarnings backlogPayoutAmount validActivations personId = do {_now <- getCurrentTime;
                                                                                                  updateOneWithKV [Se.Set Beam.referralEarnings (Kernel.Prelude.Just referralEarnings),
...
updateReferralAmountPaid referralAmountPaid personId = do {_now <- getCurrentTime;
...
updateReferralCount referralCount personId = do {_now <- getCurrentTime;
...
updateReferralEarningsAndValidActivations :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                             (Kernel.Types.Common.HighPrecMoney -> Kernel.Prelude.Int -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateReferralEarningsAndValidActivations referralEarnings validActivations personId = do {_now <- getCurrentTime;
                                                                                           updateOneWithKV [Se.Set Beam.referralEarnings (Kernel.Prelude.Just referralEarnings),
                                                                                                            Se.Set Beam.validActivations (Kernel.Prelude.Just validActivations),
                                                                                                            Se.Set Beam.updatedAt _now] [Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId)]}
...
updateReferredByEarning referredByEarnings personId = do {_now <- getCurrentTime;
...
updateReferredByEarningsPayoutStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                        (Kernel.Prelude.Maybe Domain.Types.PersonStats.PayoutStatus -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateReferredByEarningsPayoutStatus referredByEarningsPayoutStatus personId = do {_now <- getCurrentTime;
...
updateReferredByEarningsPayoutStatusAndAmountPaid :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                                     (Kernel.Prelude.Maybe Domain.Types.PersonStats.PayoutStatus -> Kernel.Types.Common.HighPrecMoney -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateReferredByEarningsPayoutStatusAndAmountPaid referredByEarningsPayoutStatus referralAmountPaid personId = do {_now <- getCurrentTime;
                                                                                                                   updateOneWithKV [Se.Set Beam.referredByEarningsPayoutStatus referredByEarningsPayoutStatus,
                                                                                                                                    Se.Set Beam.referralAmountPaid (Kernel.Prelude.Just referralAmountPaid),
                                                                                                                                    Se.Set Beam.updatedAt _now] [Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId)]}
...
updateByPrimaryKey (Domain.Types.PersonStats.PersonStats {..}) = do {_now <- getCurrentTime;
                                                                     updateWithKV [Se.Set Beam.backlogPayoutAmount (Kernel.Prelude.Just backlogPayoutAmount),
                                                                                   ...
                                                                                   Se.Set Beam.weekendRides weekendRides] [Se.And [Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId)]]}
