...
findAllByOrderId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateStatusByTicketBookingId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateStatusByTicketBookingId status frfsTicketBookingId = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.FRFSTicketBookingPayment.FRFSTicketBookingPayment {..}) = do {_now <- getCurrentTime;
                                                                                               updateWithKV [Se.Set Beam.frfsQuoteId (Kernel.Types.Id.getId <$> frfsQuoteId),
