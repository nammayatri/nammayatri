...
findAllLegsMappingByJourneyId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllLegsMappingByJourneyId limit offset journeyId isDeleted = do findAllWithOptionsKV [Se.And [Se.Is Beam.journeyId $ Se.Eq (Kernel.Types.Id.getId journeyId),
                                                                                                  Se.Is Beam.isDeleted $ Se.Eq isDeleted]] (Se.Asc Beam.sequenceNumber) limit offset
findByJourneyIdAndSequenceNumber :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByJourneyIdAndSequenceNumber journeyId sequenceNumber isDeleted = do findOneWithKV [Se.And [Se.Is Beam.journeyId $ Se.Eq (Kernel.Types.Id.getId journeyId),
...
updateIsDeleted isDeleted journeyLegId = do {_now <- getCurrentTime;
...
updateJourneyLegId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateJourneyLegId journeyLegId id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.JourneyLegMapping.JourneyLegMapping {..}) = do {_now <- getCurrentTime;
                                                                                 updateWithKV [Se.Set Beam.isDeleted isDeleted,
...
instance FromTType' Beam.JourneyLegMapping Domain.Types.JourneyLegMapping.JourneyLegMapping
    where fromTType' (Beam.JourneyLegMappingT {..}) = do pure $ Just Domain.Types.JourneyLegMapping.JourneyLegMapping{id = Kernel.Types.Id.Id id,
                                                                                                                      isDeleted = isDeleted,
                                                                                                                      ...
                                                                                                                      createdAt = createdAt,
                                                                                                                      updatedAt = updatedAt}
instance ToTType' Beam.JourneyLegMapping Domain.Types.JourneyLegMapping.JourneyLegMapping
    where toTType' (Domain.Types.JourneyLegMapping.JourneyLegMapping {..}) = do Beam.JourneyLegMappingT{Beam.id = Kernel.Types.Id.getId id,
