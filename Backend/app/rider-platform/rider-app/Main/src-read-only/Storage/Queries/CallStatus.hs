...
updateCallError callError callService merchantId id = do {_now <- getCurrentTime;
...
updateCallStatus conversationDuration recordingUrl status callAttempt id = do {_now <- getCurrentTime;
                                                                               updateWithKV [Se.Set Beam.conversationDuration conversationDuration,
...
updateCallStatusInformation :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                               (Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Prelude.Maybe Kernel.External.Call.Types.CallService -> Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Types.Id.Id Domain.Types.CallStatus.CallStatus -> m ())
updateCallStatusInformation merchantId callService dtmfNumberUsed id = do {_now <- getCurrentTime;
                                                                           updateWithKV [Se.Set Beam.merchantId merchantId,
...
updateCustomerIvrResponse callId customerIvrResponse status = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.CallStatus.CallStatus {..}) = do {_now <- getCurrentTime;
                                                                   updateWithKV [Se.Set Beam.callAttempt callAttempt,
