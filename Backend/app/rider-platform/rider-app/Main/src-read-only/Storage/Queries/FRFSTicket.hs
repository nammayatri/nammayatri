...
findByTicketBookingIdTicketNumber :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByTicketBookingIdTicketNumber frfsTicketBookingId ticketNumber = do findOneWithKV [Se.And [Se.Is Beam.frfsTicketBookingId $ Se.Eq (Kernel.Types.Id.getId frfsTicketBookingId),
...
udpateQrDataAndValidTill :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
udpateQrDataAndValidTill qrData validTill frfsTicketBookingId ticketNumber = do {_now <- getCurrentTime;
                                                                                 updateWithKV [Se.Set Beam.qrData qrData, Se.Set Beam.validTill validTill, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.frfsTicketBookingId $ Se.Eq (Kernel.Types.Id.getId frfsTicketBookingId),
                                                                                                                                                                                                Se.Is Beam.ticketNumber $ Se.Eq ticketNumber]]}
updateAllStatusByBookingId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                              (Domain.Types.FRFSTicketStatus.FRFSTicketStatus -> Kernel.Types.Id.Id Domain.Types.FRFSTicketBooking.FRFSTicketBooking -> m ())
updateAllStatusByBookingId status frfsTicketBookingId = do {_now <- getCurrentTime;
...
updateRefreshTicketQRByTBookingIdAndTicketNumber :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateRefreshTicketQRByTBookingIdAndTicketNumber qrData qrRefreshAt frfsTicketBookingId ticketNumber = do {_now <- getCurrentTime;
                                                                                                           updateWithKV [Se.Set Beam.qrData qrData,
                                                                                                                         Se.Set Beam.qrRefreshAt qrRefreshAt,
                                                                                                                         Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.frfsTicketBookingId $ Se.Eq (Kernel.Types.Id.getId frfsTicketBookingId), Se.Is Beam.ticketNumber $ Se.Eq ticketNumber]]}
updateStatusByTBookingIdAndTicketNumber :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateStatusByTBookingIdAndTicketNumber status scannedByVehicleNumber frfsTicketBookingId ticketNumber = do {_now <- getCurrentTime;
                                                                                                             updateWithKV [Se.Set Beam.status status,
...
updateByPrimaryKey (Domain.Types.FRFSTicket.FRFSTicket {..}) = do {_now <- getCurrentTime;
                                                                   updateWithKV [Se.Set Beam.cloudType cloudType,
...
instance FromTType' Beam.FRFSTicket Domain.Types.FRFSTicket.FRFSTicket
    where fromTType' (Beam.FRFSTicketT {..}) = do pure $ Just Domain.Types.FRFSTicket.FRFSTicket{cloudType = cloudType,
                                                                                                 ...
                                                                                                 createdAt = createdAt,
                                                                                                 updatedAt = updatedAt}
instance ToTType' Beam.FRFSTicket Domain.Types.FRFSTicket.FRFSTicket
    where toTType' (Domain.Types.FRFSTicket.FRFSTicket {..}) = do Beam.FRFSTicketT{Beam.cloudType = cloudType,
                                                                                   Beam.commencingHours = commencingHours,
                                                                                   Beam.description = description,
                                                                                   Beam.frfsTicketBookingId = Kernel.Types.Id.getId frfsTicketBookingId,
                                                                                   Beam.id = Kernel.Types.Id.getId id,
                                                                                   Beam.isReturnTicket = isReturnTicket,
