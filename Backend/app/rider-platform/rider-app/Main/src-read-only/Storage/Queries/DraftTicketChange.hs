...
findByTicketPlaceIdAndStatus ::
  ...
  (Kernel.Types.Id.Id Domain.Types.TicketPlace.TicketPlace -> [Domain.Types.DraftTicketChange.DraftStatus] -> m (Maybe Domain.Types.DraftTicketChange.DraftTicketChange))
findByTicketPlaceIdAndStatus id status = do findOneWithKV [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id), Se.Is Beam.status $ Se.In status]]
