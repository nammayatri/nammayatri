...
updateDraftById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateDraftById draftPayload isApprovalRequired id = do {_now <- getCurrentTime;
                                                         updateOneWithKV [Se.Set Beam.draftPayload (Data.Aeson.toJSON <$> draftPayload),
...
updateByPrimaryKey (Domain.Types.DraftTicketChange.DraftTicketChange {..}) = do {_now <- getCurrentTime;
                                                                                 updateWithKV [Se.Set Beam.draftPayload (Data.Aeson.toJSON <$> draftPayload),
                                                                                               Se.Set Beam.isApprovalRequired isApprovalRequired,
...
instance FromTType' Beam.DraftTicketChange Domain.Types.DraftTicketChange.DraftTicketChange
    where fromTType' (Beam.DraftTicketChangeT {..}) = do pure $ Just Domain.Types.DraftTicketChange.DraftTicketChange{createdAt = createdAt,
...
instance ToTType' Beam.DraftTicketChange Domain.Types.DraftTicketChange.DraftTicketChange
    where toTType' (Domain.Types.DraftTicketChange.DraftTicketChange {..}) = do Beam.DraftTicketChangeT{Beam.createdAt = createdAt,
