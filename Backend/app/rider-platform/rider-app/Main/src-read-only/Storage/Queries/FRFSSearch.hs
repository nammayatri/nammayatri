...
updateIsOnSearchReceivedById isOnSearchReceived id = do {_now <- getCurrentTime;
...
updateRiderIdById riderId id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.FRFSSearch.FRFSSearch {..}) = do {_now <- getCurrentTime;
                                                                   updateWithKV [Se.Set Beam.busLocationData (Just $ toJSON busLocationData),
