...
updateSelectedOfferId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => (Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Types.Id.Id Domain.Types.Estimate.Estimate -> m ())
updateSelectedOfferId selectedOfferId id = do {_now <- getCurrentTime;
                                               updateOneWithKV [Se.Set Beam.selectedOfferId selectedOfferId, Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]}
...
updateStatusAndProviderUrl status providerUrl id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.Estimate.Estimate {..}) = do {_now <- getCurrentTime;
                                                               updateWithKV [Se.Set Beam.backendAppVersion backendAppVersion,
                                                                             ...
                                                                             Se.Set Beam.waitingChargePerMinAmount (((.waitingChargePerMin) waitingCharges <&> (.amount)))] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]]}
