...
findByPersonIdAndVehicleCategory :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateHasTakenValidRideCount :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateHasTakenValidRideCount rideCount personId vehicleCategory = do {_now <- getCurrentTime;
                                                                      updateOneWithKV [Se.Set Beam.rideCount rideCount, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId),
...
updateByPrimaryKey (Domain.Types.ClientPersonInfo.ClientPersonInfo {..}) = do {_now <- getCurrentTime;
                                                                               updateWithKV [Se.Set Beam.clientId (Kernel.Types.Id.getId <$> clientId),
...
instance FromTType' Beam.ClientPersonInfo Domain.Types.ClientPersonInfo.ClientPersonInfo
    where fromTType' (Beam.ClientPersonInfoT {..}) = do pure $ Just Domain.Types.ClientPersonInfo.ClientPersonInfo{clientId = Kernel.Types.Id.Id <$> clientId,
                                                                                                                   createdAt = createdAt,
...
instance ToTType' Beam.ClientPersonInfo Domain.Types.ClientPersonInfo.ClientPersonInfo
    where toTType' (Domain.Types.ClientPersonInfo.ClientPersonInfo {..}) = do Beam.ClientPersonInfoT{Beam.clientId = Kernel.Types.Id.getId <$> clientId,
