...
findAllByServiceCategoryIdDateAndBtype :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByServiceCategoryIdDateAndBtype serviceCategoryId visitDate btype = do findAllWithKV [Se.And [Se.Is Beam.serviceCategoryId $ Se.Eq serviceCategoryId,
                                                                                                     Se.Is Beam.visitDate $ Se.Eq visitDate,
                                                                                                     Se.Is Beam.btype $ Se.Eq btype]]
findAllByTicketBookingServiceId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateCancellationDetailsById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateCancellationDetailsById cancelledSeats amountToRefund eventCancelledBy id = do {_now <- getCurrentTime;
                                                                                      updateWithKV [Se.Set Beam.cancelledSeats cancelledSeats,
...
updateByPrimaryKey (Domain.Types.TicketBookingServiceCategory.TicketBookingServiceCategory {..}) = do {_now <- getCurrentTime;
                                                                                                       updateWithKV [Se.Set Beam.amount ((.amount) amount),
