...
findByOperatorBadgeTokenAndMerchantId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByOperatorBadgeTokenAndMerchantId operatorBadgeToken merchantId = do findOneWithKV [Se.And [Se.Is Beam.operatorBadgeToken $ Se.Eq operatorBadgeToken,
...
updateAadhaarVerifiedState aadhaarVerified id = do {_now <- getCurrentTime;
...
updateAverageRating totalRatings totalRatingScore isValidRating id = do {_now <- getCurrentTime;
                                                                         updateOneWithKV [Se.Set Beam.totalRatings totalRatings,
...
updateCustomerPaymentId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                           (Kernel.Prelude.Maybe Kernel.External.Payment.Interface.Types.CustomerId -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateCustomerPaymentId customerPaymentId id = do {_now <- getCurrentTime;
...
updateCustomerTags customerNammaTags id = do {_now <- getCurrentTime;
...
updateDefaultPaymentMethodId defaultPaymentMethodId id = do {_now <- getCurrentTime;
...
updateDefaultTestPaymentMethodId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                    (Kernel.Prelude.Maybe Kernel.External.Payment.Interface.Types.PaymentMethodId -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateDefaultTestPaymentMethodId defaultTestPaymentMethodId id = do {_now <- getCurrentTime;
...
updateFreqLocGeohashes frequentLocGeohashes id = do {_now <- getCurrentTime;
...
updateHasDisability hasDisability id = do {_now <- getCurrentTime;
...
updateIsValidRating isValidRating id = do {_now <- getCurrentTime;
...
updateJuspayCustomerPaymentId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                 (Kernel.Prelude.Maybe Kernel.External.Payment.Interface.Types.CustomerId -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateJuspayCustomerPaymentId juspayCustomerPaymentID id = do {_now <- getCurrentTime;
...
updateLastUsedVehicleCategories lastUsedVehicleCategories id = do {_now <- getCurrentTime;
...
updateLastUsedVehicleServiceTiers lastUsedVehicleServiceTiers id = do {_now <- getCurrentTime;
...
updateLastUsedVehicleServiceTiersAndCategories :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateLastUsedVehicleServiceTiersAndCategories lastUsedVehicleServiceTiers lastUsedVehicleCategories id = do {_now <- getCurrentTime;
                                                                                                              updateWithKV [Se.Set Beam.lastUsedVehicleServiceTiers (Just lastUsedVehicleServiceTiers),
...
updateLiveActivityToken liveActivityToken id = do {_now <- getCurrentTime;
...
updateReferralCodeAndReferredAt :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateReferralCodeAndReferredAt referredAt referralCode id = do {_now <- getCurrentTime;
...
updateSafetyDrillStatus hasCompletedMockSafetyDrill id = do {_now <- getCurrentTime;
...
updateTestCustomerPaymentId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                               (Kernel.Prelude.Maybe Kernel.External.Payment.Interface.Types.CustomerId -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateTestCustomerPaymentId customerTestPaymentId id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Domain.Types.Person.Person {..}) = do {_now <- getCurrentTime;
                                                           updateWithKV [Se.Set Beam.aadhaarVerified aadhaarVerified,
