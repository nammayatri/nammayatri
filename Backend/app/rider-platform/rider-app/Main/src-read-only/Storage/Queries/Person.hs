...
updateSafetyMpin :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => (Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Types.Id.Id Domain.Types.Person.Person -> m ())
updateSafetyMpin safetyMpin id = do _now <- getCurrentTime; updateWithKV [Se.Set Beam.safetyMpin safetyMpin, Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]
...
updateByPrimaryKey (Domain.Types.Person.Person {..}) = do
  ...
  updateWithKV
    [ Se.Set Beam.aadhaarVerified aadhaarVerified,
      ...
      Se.Set Beam.safetyMpin safetyMpin,
