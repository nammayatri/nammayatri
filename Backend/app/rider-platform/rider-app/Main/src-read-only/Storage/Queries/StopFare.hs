...
findAllByStartStopAndIntegratedBPPConfigId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByStartStopAndIntegratedBPPConfigId startStopCode endStopCode integratedBppConfigId = do findAllWithKV [Se.And [Se.Is Beam.startStopCode $ Se.Eq startStopCode,
...
findByRouteStartAndStopCode :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByRouteStartAndStopCode farePolicyId startStopCode endStopCode = do findOneWithKV [Se.And [Se.Is Beam.farePolicyId $ Se.Eq (Kernel.Types.Id.getId farePolicyId),
                                                                                               Se.Is Beam.startStopCode $ Se.Eq startStopCode,
                                                                                               Se.Is Beam.endStopCode $ Se.Eq endStopCode]]
findByStartAndEndStopCodeAndIntegratedBPPConfigId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findByStartAndEndStopCodeAndIntegratedBPPConfigId startStopCode endStopCode integratedBppConfigId = do findOneWithKV [Se.And [Se.Is Beam.startStopCode $ Se.Eq startStopCode,
                                                                                                                              Se.Is Beam.endStopCode $ Se.Eq endStopCode,
                                                                                                                              Se.Is Beam.integratedBppConfigId $ Se.Eq (Kernel.Types.Id.getId integratedBppConfigId)]]
updateFareByStopCodes :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateFareByStopCodes amount farePolicyId startStopCode endStopCode category = do {_now <- getCurrentTime;
                                                                                   updateOneWithKV [Se.Set Beam.amount amount, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.farePolicyId $ Se.Eq (Kernel.Types.Id.getId farePolicyId),
                                                                                                                                                                    Se.Is Beam.startStopCode $ Se.Eq startStopCode,
                                                                                                                                                                    Se.Is Beam.endStopCode $ Se.Eq endStopCode,
...
findByPrimaryKey category endStopCode farePolicyId startStopCode = do findOneWithKV [Se.And [Se.Is Beam.category $ Se.Eq (Kernel.Prelude.Just category),
                                                                                             Se.Is Beam.endStopCode $ Se.Eq endStopCode,
...
updateByPrimaryKey (Domain.Types.StopFare.StopFare {..}) = do {_now <- getCurrentTime;
                                                               updateWithKV [Se.Set Beam.amount amount,
                                                                             ...
                                                                             Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.category $ Se.Eq (Kernel.Prelude.Just category),
                                                                                                                  Se.Is Beam.endStopCode $ Se.Eq endStopCode,
                                                                                                                  Se.Is Beam.farePolicyId $ Se.Eq (Kernel.Types.Id.getId farePolicyId),
                                                                                                                  Se.Is Beam.startStopCode $ Se.Eq startStopCode]]}
...
instance FromTType' Beam.StopFare Domain.Types.StopFare.StopFare
    where fromTType' (Beam.StopFareT {..}) = do pure $ Just Domain.Types.StopFare.StopFare{amount = amount,
                                                                                           bppItemId = bppItemId,
                                                                                           ...
                                                                                           createdAt = createdAt,
                                                                                           updatedAt = updatedAt}
instance ToTType' Beam.StopFare Domain.Types.StopFare.StopFare
    where toTType' (Domain.Types.StopFare.StopFare {..}) = do Beam.StopFareT{Beam.amount = amount,
