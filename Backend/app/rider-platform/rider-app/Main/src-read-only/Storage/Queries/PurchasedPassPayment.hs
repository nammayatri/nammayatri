...
findAllByPurchasedPassIdAndStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findAllByPurchasedPassIdAndStatus limit offset purchasedPassId status endDate = do findAllWithOptionsKV [Se.And [Se.Is Beam.purchasedPassId $ Se.Eq (Kernel.Types.Id.getId purchasedPassId),
                                                                                                                 Se.Is Beam.status $ Se.In status,
                                                                                                                 Se.Is Beam.endDate $ Se.GreaterThanOrEq endDate]] (Se.Asc Beam.startDate) limit offset
findAllByPurchasedPassIdAndStatusAndStartDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                                 (Maybe Int -> Maybe Int -> Kernel.Types.Id.Id Domain.Types.PurchasedPass.PurchasedPass -> [Domain.Types.PurchasedPass.StatusType] -> Data.Time.Calendar.Day -> m ([Domain.Types.PurchasedPassPayment.PurchasedPassPayment]))
findAllByPurchasedPassIdAndStatusAndStartDate limit offset purchasedPassId status startDate = do findAllWithOptionsKV [Se.And [Se.Is Beam.purchasedPassId $ Se.Eq (Kernel.Types.Id.getId purchasedPassId),
                                                                                                                               Se.Is Beam.status $ Se.In status,
                                                                                                                               Se.Is Beam.startDate $ Se.Eq startDate]] (Se.Desc Beam.createdAt) limit offset
findAllByPurchasedPassIdAndStatusStartDateGreaterThan :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
                                                         (Maybe Int -> Maybe Int -> Kernel.Types.Id.Id Domain.Types.PurchasedPass.PurchasedPass -> Domain.Types.PurchasedPass.StatusType -> Data.Time.Calendar.Day -> m ([Domain.Types.PurchasedPassPayment.PurchasedPassPayment]))
findAllByPurchasedPassIdAndStatusStartDateGreaterThan limit offset purchasedPassId status startDate = do findAllWithOptionsKV [Se.And [Se.Is Beam.purchasedPassId $ Se.Eq (Kernel.Types.Id.getId purchasedPassId),
                                                                                                                                       Se.Is Beam.status $ Se.Eq status,
                                                                                                                                       Se.Is Beam.startDate $ Se.GreaterThanOrEq startDate]] (Se.Asc Beam.startDate) limit offset
findAllWithPersonId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
findOneByPaymentOrderId :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateProfilePictureByPurchasedPassIdAndStatus :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateProfilePictureByPurchasedPassIdAndStatus profilePicture purchasedPassId status = do {_now <- getCurrentTime;
                                                                                           updateWithKV [Se.Set Beam.profilePicture profilePicture, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.purchasedPassId $ Se.Eq (Kernel.Types.Id.getId purchasedPassId),
                                                                                                                                                                                         Se.Is Beam.status $ Se.In status]]}
updateStatusAndDatesById :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateStatusAndDatesById startDate endDate status id = do {_now <- getCurrentTime;
                                                           updateWithKV [Se.Set Beam.startDate startDate, Se.Set Beam.endDate endDate, Se.Set Beam.status status, Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]}
...
updateStatusByPurchasedPassIdAndStartEndDate :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateStatusByPurchasedPassIdAndStartEndDate status purchasedPassId startDate endDate = do {_now <- getCurrentTime;
                                                                                            updateWithKV [Se.Set Beam.status status, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.purchasedPassId $ Se.Eq (Kernel.Types.Id.getId purchasedPassId),
...
updateByPrimaryKey (Domain.Types.PurchasedPassPayment.PurchasedPassPayment {..}) = do {_now <- getCurrentTime;
