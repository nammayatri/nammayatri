...
findByMerchantIdAndState :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) =>
...
updateByPrimaryKey (Domain.Types.MerchantState.MerchantState {..}) = do {_now <- getCurrentTime;
                                                                         updateWithKV [Se.Set Beam.allowedDestinationStates allowedDestinationStates, Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.merchantId $ Se.Eq (Kernel.Types.Id.getId merchantId),
                                                                                                                                                                                           Se.Is Beam.state $ Se.Eq state]]}



instance FromTType' Beam.MerchantState Domain.Types.MerchantState.MerchantState
    where fromTType' (Beam.MerchantStateT {..}) = do pure $ Just Domain.Types.MerchantState.MerchantState{allowedDestinationStates = allowedDestinationStates,
                                                                                                          merchantId = Kernel.Types.Id.Id merchantId,
                                                                                                          state = state,
                                                                                                          createdAt = createdAt,
                                                                                                          updatedAt = updatedAt}
instance ToTType' Beam.MerchantState Domain.Types.MerchantState.MerchantState
    where toTType' (Domain.Types.MerchantState.MerchantState {..}) = do Beam.MerchantStateT{Beam.allowedDestinationStates = allowedDestinationStates,
