...
data PersonStatsT f
    = PersonStatsT {backlogPayoutAmount :: (B.C f (Kernel.Prelude.Maybe Kernel.Types.Common.HighPrecMoney)),
...
instance B.Table PersonStatsT
    where data PrimaryKey PersonStatsT f = PersonStatsId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
