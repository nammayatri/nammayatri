...
data BBPST f
    = BBPST {amount :: (B.C f Kernel.Types.Common.HighPrecMoney),
...
instance B.Table BBPST
    where data PrimaryKey BBPST f = BBPSId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
