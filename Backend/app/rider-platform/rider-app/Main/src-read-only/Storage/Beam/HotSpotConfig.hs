...
data HotSpotConfigT f
    = HotSpotConfigT {blockRadius :: (B.C f Kernel.Prelude.Int),
...
instance B.Table HotSpotConfigT
    where data PrimaryKey HotSpotConfigT f = HotSpotConfigId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
