...
data MerchantConfigT f
    = MerchantConfigT {createdAt :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.UTCTime)),
...
instance B.Table MerchantConfigT
    where data PrimaryKey MerchantConfigT f = MerchantConfigId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
