...
data DriverOfferT f
    = DriverOfferT {bppQuoteId :: (B.C f Kernel.Prelude.Text),
...
instance B.Table DriverOfferT
    where data PrimaryKey DriverOfferT f = DriverOfferId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
