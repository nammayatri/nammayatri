...
data PurchasedPassPaymentT f
    = PurchasedPassPaymentT {amount :: (B.C f Kernel.Types.Common.HighPrecMoney),
...
instance B.Table PurchasedPassPaymentT
    where data PrimaryKey PurchasedPassPaymentT f = PurchasedPassPaymentId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
