...
data JourneyLegT f
    = JourneyLegT {agencyGtfsId :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.Text)),
...
instance B.Table JourneyLegT
    where data PrimaryKey JourneyLegT f = JourneyLegId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
