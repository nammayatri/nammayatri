...
data FRFSTicketT f
    = FRFSTicketT {cloudType :: (B.C f (Kernel.Prelude.Maybe Kernel.Types.Version.CloudType)),
...
instance B.Table FRFSTicketT
    where data PrimaryKey FRFSTicketT f = FRFSTicketId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
          primaryKey = FRFSTicketId . id
type FRFSTicket = FRFSTicketT Identity

$(enableKVPG (''FRFSTicketT) [('id)] [[('frfsTicketBookingId)], [('ticketNumber)]])
