...
data RouteDetailsT f
    = RouteDetailsT {agencyGtfsId :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.Text)),
...
instance B.Table RouteDetailsT
    where data PrimaryKey RouteDetailsT f = RouteDetailsId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
