...
data FRFSReconT f
    = FRFSReconT {beneficiaryBankAccount :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.Text)),
...
instance B.Table FRFSReconT
    where data PrimaryKey FRFSReconT f = FRFSReconId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
