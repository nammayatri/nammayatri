...
data QuoteT f
    = QuoteT {backendAppVersion :: (B.C f (Kernel.Prelude.Maybe Kernel.Prelude.Text)),
...
instance B.Table QuoteT
    where data PrimaryKey QuoteT f = QuoteId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
