...
instance B.Table MerchantOnboardingT where
  data PrimaryKey MerchantOnboardingT f = MerchantOnboardingId (B.C f Kernel.Prelude.Text) (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
  primaryKey = MerchantOnboardingId <$> id <*> requestorId

type MerchantOnboarding = MerchantOnboardingT Identity

$(enableKVPG (''MerchantOnboardingT) [('id), ('requestorId)] [])
