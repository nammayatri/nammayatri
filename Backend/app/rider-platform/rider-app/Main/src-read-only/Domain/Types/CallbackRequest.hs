...
data CallbackRequestE e
    = CallbackRequest {createdAt :: Kernel.Prelude.UTCTime,
...
instance EncryptedItem CallbackRequest
    where type Unencrypted CallbackRequest = (DecryptedCallbackRequest, HashSalt)
          encryptItem (entity, salt) = do {customerPhone_ <- encryptItem (customerPhone entity, salt);
                                           pure CallbackRequest{createdAt = createdAt entity,
          ...
          decryptItem entity = do {customerPhone_ <- fst <$> decryptItem (customerPhone entity);
                                   pure (CallbackRequest{createdAt = createdAt entity,
                                                         customerMobileCountryCode = customerMobileCountryCode entity,
                                                         customerName = customerName entity,
                                                         customerPhone = customerPhone_,
                                         ...
                                         "")}
instance EncryptedItem' CallbackRequest
    where type UnencryptedItem CallbackRequest = DecryptedCallbackRequest
