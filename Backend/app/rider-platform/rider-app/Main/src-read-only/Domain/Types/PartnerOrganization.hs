...
data PartnerOrganizationE e
    = PartnerOrganization {apiKey :: Kernel.External.Encryption.EncryptedHashedField e Kernel.Prelude.Text,
...
instance EncryptedItem PartnerOrganization
    where type Unencrypted PartnerOrganization = (DecryptedPartnerOrganization, HashSalt)
          encryptItem (entity, salt) = do {apiKey_ <- encryptItem (apiKey entity, salt);
          ...
          decryptItem entity = do {apiKey_ <- fst <$> decryptItem (apiKey entity);
                                   pure (PartnerOrganization{apiKey = apiKey_, createdAt = createdAt entity, merchantId = merchantId entity, name = name entity, orgId = orgId entity, updatedAt = updatedAt entity}, "")}
instance EncryptedItem' PartnerOrganization
    where type UnencryptedItem PartnerOrganization = DecryptedPartnerOrganization
