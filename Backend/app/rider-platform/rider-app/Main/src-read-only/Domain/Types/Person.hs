...
data PersonE e
    = Person {aadhaarVerified :: Kernel.Prelude.Bool,
...
instance EncryptedItem Person
    where type Unencrypted Person = (DecryptedPerson, HashSalt)
          encryptItem (entity, salt) = do {businessEmail_ <- encryptItem $ (, salt) <$> businessEmail entity;
                                           ...
                                           pure Person{aadhaarVerified = aadhaarVerified entity,
          ...
          decryptItem entity = do {businessEmail_ <- fmap fst <$> decryptItem (businessEmail entity);
