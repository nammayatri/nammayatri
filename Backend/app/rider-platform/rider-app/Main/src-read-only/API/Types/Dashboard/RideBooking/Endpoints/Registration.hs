...
data CustomerAuthReq
    = CustomerAuthReq {mobileNumber :: Kernel.Prelude.Text, mobileCountryCode :: Kernel.Prelude.Text, otpChannel :: Kernel.Prelude.Maybe Domain.Action.UI.Registration.OTPChannel}
...
type PostRegistrationVerify = (Capture "authId" (Kernel.Types.Id.Id Domain.Types.RegistrationToken.RegistrationToken) :> "verify" :> ReqBody ('[JSON])
                                                                                                                                             Domain.Action.UI.Registration.AuthVerifyReq :> Post ('[JSON]) Domain.Action.UI.Registration.AuthVerifyRes)
type PostRegistrationOtpResend = ("otp" :> Capture "authId" (Kernel.Types.Id.Id Domain.Types.RegistrationToken.RegistrationToken) :> "resend" :> Post ('[JSON])
...
data RegistrationAPIs
    = RegistrationAPIs {postRegistrationAuth :: (CustomerAuthReq -> EulerHS.Types.EulerClient Domain.Action.UI.Registration.AuthRes),
...
mkRegistrationAPIs registrationClient = (RegistrationAPIs {..})
                       where postRegistrationAuth :<|> postRegistrationVerify :<|> postRegistrationOtpResend :<|> postRegistrationLogout = registrationClient
data RegistrationUserActionType
    = POST_REGISTRATION_AUTH | POST_REGISTRATION_VERIFY | POST_REGISTRATION_OTP_RESEND | POST_REGISTRATION_LOGOUT
