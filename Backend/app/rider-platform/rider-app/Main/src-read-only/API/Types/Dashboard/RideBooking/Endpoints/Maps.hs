...
type PostMapsAutoComplete = ("autoComplete" :> Capture "customerId" (Kernel.Types.Id.Id Domain.Types.Person.Person) :> ReqBody ('[JSON]) Domain.Action.UI.Maps.AutoCompleteReq :> Post ('[JSON])
                                                                                                                                                                                       Domain.Action.UI.Maps.AutoCompleteResp)
type PostMapsGetPlaceDetails = ("getPlaceDetails" :> Capture "customerId" (Kernel.Types.Id.Id Domain.Types.Person.Person) :> ReqBody ('[JSON])
                                                                                                                                     Domain.Action.UI.Maps.GetPlaceDetailsReq :> Post ('[JSON]) Domain.Action.UI.Maps.GetPlaceDetailsResp)
type PostMapsGetPlaceName = ("getPlaceName" :> Capture "customerId" (Kernel.Types.Id.Id Domain.Types.Person.Person) :> ReqBody ('[JSON]) Domain.Action.UI.Maps.GetPlaceNameReq :> Post ('[JSON])
...
mkMapsAPIs mapsClient = (MapsAPIs {..})
               where postMapsAutoComplete :<|> postMapsGetPlaceDetails :<|> postMapsGetPlaceName = mapsClient
data MapsUserActionType
    = POST_MAPS_AUTO_COMPLETE | POST_MAPS_GET_PLACE_DETAILS | POST_MAPS_GET_PLACE_NAME
