...
data SavedReqLocationsListRes = SavedReqLocationsListRes {list :: [Domain.Types.SavedReqLocation.SavedReqLocationAPIEntity]}
  deriving stock (Generic)
  deriving anyclass (ToJSON, FromJSON, ToSchema)
...
mkCustomerAPIs customerClient = (CustomerAPIs {..})
  where
    postCustomerSosCreate :<|> postCustomerDeletedPerson :<|> getCustomerSavedLocations :<|> postCustomerSavedLocations :<|> deleteCustomerSavedLocations = customerClient

data CustomerUserActionType
  = POST_CUSTOMER_SOS_CREATE
  | POST_CUSTOMER_DELETED_PERSON
  | GET_CUSTOMER_SAVED_LOCATIONS
  | POST_CUSTOMER_SAVED_LOCATIONS
  | DELETE_CUSTOMER_SAVED_LOCATIONS
