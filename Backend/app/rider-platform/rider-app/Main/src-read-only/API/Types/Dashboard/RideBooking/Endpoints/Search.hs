...
mkSearchAPIs searchClient = (SearchAPIs {..})
                 where postSearchRide = searchClient
data SearchUserActionType
    = POST_SEARCH_RIDE
    deriving stock (Show, Read, Generic, Eq, Ord)
    deriving anyclass ToSchema
instance ToJSON SearchUserActionType
    where toJSON (POST_SEARCH_RIDE) = Data.Aeson.String "POST_SEARCH_RIDE"
instance FromJSON SearchUserActionType
    where parseJSON (Data.Aeson.String "POST_SEARCH_RIDE") = pure POST_SEARCH_RIDE
