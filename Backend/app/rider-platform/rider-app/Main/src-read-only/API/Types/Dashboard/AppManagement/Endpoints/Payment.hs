...
data RefundRequestInitiateReq = RefundRequestInitiateReq
  { refundComponents :: [RefundComponentItem],
...
mkPaymentAPIs paymentClient = (PaymentAPIs {..})
  where
    getPaymentRefundRequestList :<|> getPaymentRefundRequestInfo :<|> postPaymentRefundRequestRespond :<|> postPaymentRefundRequestInitiate :<|> postPaymentRefundRequestBookingInitiate :<|> getPaymentFareBreakup = paymentClient
...
data PaymentUserActionType
  ...
  | POST_PAYMENT_REFUND_REQUEST_BOOKING_INITIATE
