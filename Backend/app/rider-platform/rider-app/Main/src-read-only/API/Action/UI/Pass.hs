...
type API =
  ( TokenAuth :> "multimodal" :> "pass" :> "availablePasses" :> QueryParam "language" Kernel.External.Types.Language
      ...
      :> ReqBody
           '[JSON]
           API.Types.UI.Pass.PassRechargeReq
      :> Post
           '[JSON]
           Kernel.Types.APISuccess.APISuccess
  )
...
handler = getMultimodalPassAvailablePasses :<|> postMultimodalPassSelect :<|> postMultimodalPassV2Select :<|> getMultimodalPassList :<|> postMultimodalPassVerify :<|> postMultimodalPassSwitchDeviceId :<|> getMultimodalPassTransactions :<|> postMultimodalPassRecharge
...
postMultimodalPassRecharge a2 a1 = withFlowHandlerAPI $ Domain.Action.UI.Pass.postMultimodalPassRecharge (Control.Lens.over Control.Lens._1 Kernel.Prelude.Just a2) a1
