...
type API =
  ( TokenAuth :> "multimodal" :> "pass" :> "availablePasses" :> QueryParam "language" Kernel.External.Types.Language
      ...
      :> ReqBody
           ('[JSON])
           API.Types.UI.Pass.PassCalculatePriceReq
      :> Post
           ('[JSON])
           API.Types.UI.Pass.PassCalculatePriceResp
      :<|> TokenAuth
      :> "multimodal"
      :> "pass"
      :> "list"
...
handler = getMultimodalPassAvailablePasses :<|> postMultimodalPassSelect :<|> postMultimodalPassV2Select :<|> postMultimodalPassCalculatePrice :<|> getMultimodalPassList :<|> postMultimodalPassVerify :<|> postMultimodalPassSwitchDeviceId :<|> getMultimodalPassTransactions :<|> postMultimodalPassActivateToday :<|> postMultimodalPassUploadProfilePicture :<|> postMultimodalPassUploadProfilePictureV1 :<|> getMultimodalPassPhoto :<|> postMultimodalPassSetPrefSrcAndDest
...
postMultimodalPassCalculatePrice ::
  ( ( Kernel.Types.Id.Id Domain.Types.Person.Person,
    ...
    Kernel.Types.Id.Id Domain.Types.Pass.Pass ->
    API.Types.UI.Pass.PassCalculatePriceReq ->
    Environment.FlowHandler API.Types.UI.Pass.PassCalculatePriceResp
  )
postMultimodalPassCalculatePrice a3 a2 a1 = withFlowHandlerAPI $ Domain.Action.UI.Pass.postMultimodalPassCalculatePrice (Control.Lens.over Control.Lens._1 Kernel.Prelude.Just a3) a2 a1
