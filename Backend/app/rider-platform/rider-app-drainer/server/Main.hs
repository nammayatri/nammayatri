  ...
  bracket (async NW.runMetricServer) cancel $ \_ -> do
    R.withFlowRuntime
      ...
      ( \flowRt -> do
          ...
          let environment = Env (T.pack C.kvRedis) dbSyncMetric kafkaProducerTools.producer pgConn appCfg.dontEnableForDb appCfg.dontEnableForKafka appCfg.dontEnableForKafka
