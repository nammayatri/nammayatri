...
runDeleteCommands (cmd, val) dbStreamKey = do
  ...
  withDBObjectContent @DBDeleteObjectContent dbDeleteObject $ \dbModel (DBDeleteObjectContent whereClause) -> do
    runDeleteInKafkaAndDb id val dbStreamKey whereClause (show @Text dbModel) =<< dbConf
...
runDeleteInKafka ::
  ...
  Text ->
  Where Postgres table ->
  ModelName -> -- FIXME we can use IsDBTable instance
  DBConfig Pg ->
  ReaderT Env EL.Flow (Either (MeshError, EL.KVDBStreamEntryID) EL.KVDBStreamEntryID)
-- If KAFKA_PUSH is false then entry will be there in DB Else Delete entry in Kafka only.
runDeleteInKafka id value dbStreamKey whereClause model dbConf = do
  isPushToKafka' <- EL.runIO isPushToKafka
  if not isPushToKafka'
    then runDelete id value dbStreamKey whereClause model dbConf
    else do
      Env {..} <- ask
      res <- EL.runIO $ streamRiderDrainerDeletes _kafkaConnection (getDbDeleteDataJson model whereClause) dbStreamKey
      either
        ( \_ -> do
