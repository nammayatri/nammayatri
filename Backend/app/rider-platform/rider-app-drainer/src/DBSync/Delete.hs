...
runDeleteCommands (cmd, val) dbStreamKey = do
  ...
  case cmd of
    ...
    DeleteDBCommand id _ _ _ _ (AutoCompleteDataDeleteOptions _ whereClause) -> runDeleteInKafkaAndDb id val dbStreamKey whereClause ("AutoCompleteData" :: Text) =<< dbConf
  where
