...
runUpdateCommands (cmd, val) streamKey = do
  ...
  case cmd of
    ...
    UpdateDBCommand id _ tag _ _ (SearchRequestMappingOptions _ setClauses whereClause) -> runUpdateInKafkaAndDb id val streamKey setClauses tag whereClause ("SearchRequestMapping" :: Text) =<< dbConf
  where
