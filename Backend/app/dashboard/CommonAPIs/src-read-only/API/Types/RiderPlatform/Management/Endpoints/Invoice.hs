...
data FareBreakup
    = FareBreakup {price :: Data.Text.Text, title :: Data.Text.Text}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
data InvoiceRes
    = InvoiceRes {chargeableDistance :: Kernel.Prelude.Maybe Kernel.Types.Common.HighPrecMeters,
...
type GetInvoiceInvoice = ("invoice" :> MandatoryQueryParam "from" Kernel.Prelude.UTCTime :> MandatoryQueryParam "phoneNumber" Data.Text.Text :> MandatoryQueryParam "to"
                                                                                                                                                                    Kernel.Prelude.UTCTime :> Get ('[JSON]) [InvoiceRes])
...
mkInvoiceAPIs invoiceClient = (InvoiceAPIs {..})
                  where getInvoiceInvoice = invoiceClient
data InvoiceUserActionType
    = GET_INVOICE_INVOICE
    deriving stock (Show, Read, Generic, Eq, Ord)
    deriving anyclass ToSchema
instance ToJSON InvoiceUserActionType
    where toJSON (GET_INVOICE_INVOICE) = Data.Aeson.String "GET_INVOICE_INVOICE"
instance FromJSON InvoiceUserActionType
    where parseJSON (Data.Aeson.String "GET_INVOICE_INVOICE") = pure GET_INVOICE_INVOICE
