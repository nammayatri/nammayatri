...
module API.Types.RiderPlatform.Management.Endpoints.SosMedia where
...
type GetSosMediaSosMedia = ("sos" :> "media" :> Capture "personId" (Kernel.Types.Id.Id Dashboard.Common.Customer) :> Get ('[JSON]) [GetSosMediaResponse])

newtype SosMediaAPIs = SosMediaAPIs {getSosMediaSosMedia :: (Kernel.Types.Id.Id Dashboard.Common.Customer -> EulerHS.Types.EulerClient [GetSosMediaResponse])}
