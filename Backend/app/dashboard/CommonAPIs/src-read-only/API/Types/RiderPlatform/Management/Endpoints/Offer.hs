...
data CreateOfferReq
    = CreateOfferReq {offerCode :: Kernel.Prelude.Text,
...
instance Kernel.Types.HideSecrets.HideSecrets CreateOfferReq
    where hideSecrets = Kernel.Prelude.identity
data OfferEligibilitySchemaResp
    = OfferEligibilitySchemaResp {defaultValue :: Data.Aeson.Value}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
data OfferResp
    = OfferResp {id :: Kernel.Types.Id.Id Lib.Payment.Domain.Types.Offer.Offer,
    ...
    deriving anyclass (ToJSON, FromJSON, ToSchema)
data UpdateOfferReq
    = UpdateOfferReq {discountValue :: Kernel.Prelude.Maybe Kernel.Types.Common.HighPrecMoney,
                      maxDiscount :: Kernel.Prelude.Maybe Kernel.Types.Common.HighPrecMoney,
                      title :: Kernel.Prelude.Maybe Kernel.Prelude.Text,
                      description :: Kernel.Prelude.Maybe Kernel.Prelude.Text,
                      sponsoredBy :: Kernel.Prelude.Maybe Kernel.Prelude.Text,
    ...
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
instance Kernel.Types.HideSecrets.HideSecrets UpdateOfferReq
    where hideSecrets = Kernel.Prelude.identity
data ValidateOfferEligibilityReq
    = ValidateOfferEligibilityReq {offerId :: Kernel.Types.Id.Id Lib.Payment.Domain.Types.Offer.Offer, inputData :: Data.Aeson.Value}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
instance Kernel.Types.HideSecrets.HideSecrets ValidateOfferEligibilityReq
    where hideSecrets = Kernel.Prelude.identity
data ValidateOfferEligibilityResp
    = ValidateOfferEligibilityResp {eligible :: Kernel.Prelude.Bool, offerCode :: Kernel.Prelude.Text, errors :: [Kernel.Prelude.Text]}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
...
type PostOfferUpdate = (Capture "offerId" (Kernel.Types.Id.Id Lib.Payment.Domain.Types.Offer.Offer) :> "update" :> ReqBody ('[JSON]) UpdateOfferReq :> Post ('[JSON])
...
data OfferAPIs
    = OfferAPIs {postOfferCreate :: (CreateOfferReq -> EulerHS.Types.EulerClient Kernel.Types.APISuccess.APISuccess),
...
mkOfferAPIs offerClient = (OfferAPIs {..})
                where postOfferCreate :<|> postOfferUpdate :<|> getOfferList :<|> postOfferToggle :<|> postOfferValidateEligibility :<|> getOfferEligibilitySchema = offerClient
data OfferUserActionType
    = POST_OFFER_CREATE | POST_OFFER_UPDATE | GET_OFFER_LIST | POST_OFFER_TOGGLE | POST_OFFER_VALIDATE_ELIGIBILITY | GET_OFFER_ELIGIBILITY_SCHEMA
