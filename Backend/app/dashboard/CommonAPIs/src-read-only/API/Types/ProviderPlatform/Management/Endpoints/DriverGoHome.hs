...
data CachedGoHomeRequestInfoRes
    = CachedGoHomeRequestInfoRes {status :: Kernel.Prelude.Maybe Kernel.Prelude.String,
...
instance Kernel.Types.HideSecrets.HideSecrets CachedGoHomeRequestInfoRes
    where hideSecrets = Kernel.Prelude.identity
data DriverHomeLocationAPIEntity
    = DriverHomeLocationAPIEntity {id :: Kernel.Types.Id.Id Dashboard.Common.DriverHomeLocation,
    ...
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
instance Kernel.Types.HideSecrets.HideSecrets DriverHomeLocationAPIEntity
    where hideSecrets = Kernel.Prelude.identity
...
type PostDriverGoHomeUpdateHomeLocation = (Capture "driverId" (Kernel.Types.Id.Id Dashboard.Common.Driver) :> "updateHomeLocation" :> ReqBody ('[JSON]) UpdateDriverHomeLocationReq :> Post ('[JSON])
...
data DriverGoHomeAPIs
    = DriverGoHomeAPIs {getDriverGoHomeGetHomeLocation :: (Kernel.Types.Id.Id Dashboard.Common.Driver -> EulerHS.Types.EulerClient GetHomeLocationsRes),
...
mkDriverGoHomeAPIs driverGoHomeClient = (DriverGoHomeAPIs {..})
                       where getDriverGoHomeGetHomeLocation :<|> postDriverGoHomeUpdateHomeLocation :<|> postDriverGoHomeIncrementGoToCount :<|> getDriverGoHomeGetGoHomeInfo = driverGoHomeClient
data DriverGoHomeUserActionType
    = GET_DRIVER_GO_HOME_GET_HOME_LOCATION | POST_DRIVER_GO_HOME_UPDATE_HOME_LOCATION | POST_DRIVER_GO_HOME_INCREMENT_GO_TO_COUNT | GET_DRIVER_GO_HOME_GET_GO_HOME_INFO
