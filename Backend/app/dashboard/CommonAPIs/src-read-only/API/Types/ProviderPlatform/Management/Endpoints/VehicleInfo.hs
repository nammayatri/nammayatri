...
data UpdateVehicleInfoReq
    = UpdateVehicleInfoReq {rcNo :: Kernel.Prelude.Text, newInfo :: [VehicleInfoPostData]}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
instance Kernel.Types.HideSecrets.HideSecrets UpdateVehicleInfoReq
...
data VehicleInfoAPIEntity
    ...
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
data VehicleInfoPostData
    = VehicleInfoPostData {questionId :: Kernel.Prelude.Text, question :: Kernel.Prelude.Text, answer :: Kernel.Prelude.Text}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
...
data VehicleInfoAPIs
    = VehicleInfoAPIs {getVehicleInfoList :: (Kernel.Prelude.Text -> EulerHS.Types.EulerClient VehicleExtraInformation),
...
mkVehicleInfoAPIs vehicleInfoClient = (VehicleInfoAPIs {..})
                      where getVehicleInfoList :<|> postVehicleInfoUpdate = vehicleInfoClient
data VehicleInfoUserActionType
    = GET_VEHICLE_INFO_LIST | POST_VEHICLE_INFO_UPDATE
