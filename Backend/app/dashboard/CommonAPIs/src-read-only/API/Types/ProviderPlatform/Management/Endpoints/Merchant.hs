...
data UpdateVehicleServiceTierReq = UpdateVehicleServiceTierReq {file :: Kernel.Prelude.FilePath}
  deriving stock (Generic)
  deriving anyclass (ToJSON, FromJSON, ToSchema)

instance Kernel.Types.HideSecrets.HideSecrets UpdateVehicleServiceTierReq where
  hideSecrets = Kernel.Prelude.identity
...
type PostMerchantInsertOrUpdateVehicleServiceTier =
  ( "insertOrUpdateVehicleServiceTier"
      :> Kernel.ServantMultipart.MultipartForm
           Kernel.ServantMultipart.Tmp
...
data MerchantAPIs = MerchantAPIs
  { postMerchantUpdate :: MerchantUpdateReq -> EulerHS.Types.EulerClient MerchantUpdateRes,
    ...
    postMerchantInsertOrUpdateVehicleServiceTier :: (Data.ByteString.Lazy.ByteString, UpdateVehicleServiceTierReq) -> EulerHS.Types.EulerClient Kernel.Types.APISuccess.APISuccess,
...
data MerchantUserActionType
  ...
  | POST_MERCHANT_INSERT_OR_UPDATE_VEHICLE_SERVICE_TIER
