...
instance Text.Show.Show OperatorUserActionType
    where show = \case
                     DRIVER e -> "DRIVER/" <> show e
                     FLEET_MANAGEMENT e -> "FLEET_MANAGEMENT/" <> show e
                     REGISTRATION e -> "REGISTRATION/" <> show e
instance Text.Read.Read OperatorUserActionType
    where readsPrec d' = Text.Read.readParen (d' > app_prec) (\r -> [(DRIVER v1, r2) | r1 <- stripPrefix "DRIVER/" r, (v1, r2) <- Text.Read.readsPrec (app_prec + 1) r1] ++ [(FLEET_MANAGEMENT v1,
                                                                                                                                                                              r2) | r1 <- stripPrefix "FLEET_MANAGEMENT/" r, (v1, r2) <- Text.Read.readsPrec (app_prec + 1) r1] ++ [(REGISTRATION v1,
                                                                                                                                                                                                                                                                                     r2) | r1 <- stripPrefix "REGISTRATION/" r,
                                                                                                                                                                                                                                                                                           (v1,
