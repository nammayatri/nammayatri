...
data CreatePlanReq
    = CreatePlanReq {name :: Kernel.Prelude.Text,
...
instance Kernel.Types.HideSecrets.HideSecrets CreatePlanReq
    where hideSecrets = Kernel.Prelude.identity
newtype CreatePlanResp
  = CreatePlanResp {planId :: Kernel.Prelude.Text}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
data ListPlansResp
    = ListPlansResp {plans :: [PlanAPIEntity]}
    deriving stock Generic
    deriving anyclass (ToJSON, FromJSON, ToSchema)
data PlanAPIEntity
    = PlanAPIEntity {id :: Kernel.Prelude.Text,
...
data PlanBasedOnEntity
    = RIDE | NONE | VEHICLE | VEHICLE_AND_RIDE
...
data PlanManagementAPIs
    = PlanManagementAPIs {postPlanManagementCreate :: (CreatePlanReq -> EulerHS.Types.EulerClient CreatePlanResp),
...
mkPlanManagementAPIs planManagementClient = (PlanManagementAPIs {..})
                         where postPlanManagementCreate :<|> postPlanManagementDeletePlan :<|> postPlanManagementActivatePlan :<|> getPlanManagementListPlans = planManagementClient
data PlanManagementUserActionType
    = POST_PLAN_MANAGEMENT_CREATE | POST_PLAN_MANAGEMENT_DELETE_PLAN | POST_PLAN_MANAGEMENT_ACTIVATE_PLAN | GET_PLAN_MANAGEMENT_LIST_PLANS
