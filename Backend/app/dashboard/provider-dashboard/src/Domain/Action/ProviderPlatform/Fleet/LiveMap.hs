...
getLiveMapDrivers ::
  ...
  API.Types.ProviderPlatform.Fleet.LiveMap.NearbyDriverReq ->
  Environment.Flow [API.Types.ProviderPlatform.Fleet.LiveMap.MapDriverInfoRes]
getLiveMapDrivers merchantShortId opCity apiTokenInfo mbFleetOwnerId mbDriverIdForRadius req = do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  (mbFleetOwnerId', requestorId) <- getMbFleetOwnerAndRequestorIdMerchantBased apiTokenInfo mbFleetOwnerId
  API.Client.ProviderPlatform.Fleet.callFleetAPI checkedMerchantId opCity (.liveMapDSL.getLiveMapDrivers) requestorId mbFleetOwnerId' mbDriverIdForRadius req
