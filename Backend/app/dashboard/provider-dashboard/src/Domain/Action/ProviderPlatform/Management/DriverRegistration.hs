...
postDriverRegistrationUnlinkDocument merchantShortId opCity apiTokenInfo personId documentType = do
  ...
  T.withTransactionStoring transaction $ do
    isFleetOwnerMandatoryDocUnlinked <- Client.callManagementAPI checkedMerchantId opCity (.driverRegistrationDSL.postDriverRegistrationUnlinkDocument) personId documentType mbRequestorId
    when isFleetOwnerMandatoryDocUnlinked $ do
