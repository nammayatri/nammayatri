...
postMerchantUpdateVehicleServiceTier :: ShortId DM.Merchant -> City.City -> ApiTokenInfo -> Common.UpdateVehicleServiceTierReq -> Flow APISuccess
postMerchantUpdateVehicleServiceTier merchantShortId opCity apiTokenInfo req = do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  transaction <- buildTransaction apiTokenInfo T.emptyRequest
  T.withTransactionStoring transaction $
    Client.callManagementAPI checkedMerchantId opCity (addMultipartBoundary . (.merchantDSL.postMerchantUpdateVehicleServiceTier)) req
  where
    addMultipartBoundary clientFn reqBody = clientFn ("XXX00XXX", reqBody)
