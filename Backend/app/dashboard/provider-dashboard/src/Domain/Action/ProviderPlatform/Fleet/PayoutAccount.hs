...
postPayoutAccount :: (Kernel.Types.Id.ShortId Domain.Types.Merchant.Merchant -> Kernel.Types.Beckn.Context.City -> ApiTokenInfo UserActionType -> API.Types.ProviderPlatform.Fleet.PayoutAccount.PayoutAccountReq -> Environment.Flow API.Types.ProviderPlatform.Fleet.PayoutAccount.PayoutAccountResp)
...
postPayoutAccountStatus merchantShortId opCity apiTokenInfo req = do
  ...
  transaction <- SharedLogic.Transaction.buildTransaction (Domain.Types.Transaction.ActionAPI apiTokenInfo.userActionType) (Kernel.Prelude.Just DRIVER_OFFER_BPP_MANAGEMENT) (Kernel.Prelude.Just apiTokenInfo) Kernel.Prelude.Nothing Kernel.Prelude.Nothing (Kernel.Prelude.Just req)
  SharedLogic.Transaction.withTransactionStoring transaction $
    Client.callFleetAPI checkedMerchantId opCity (.payoutAccountDSL.postPayoutAccountStatus) apiTokenInfo.personId.getId req
