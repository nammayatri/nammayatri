...
exotelHeartbeat incomingExotelToken req = withFlowHandlerAPI' $ do
  ...
  needToCallApps <- forM serverNames $ \serverName -> do
    mbLastTransaction <- B.runInReplica $ QT.fetchLastTransaction (show (DT.ExotelAPI Common.ExotelHeartbeatEndpoint :: DT.Endpoint DashAuth.DashboardActionType)) serverName
    let mbLastReq =
          mbLastTransaction
            >>= (.request)
