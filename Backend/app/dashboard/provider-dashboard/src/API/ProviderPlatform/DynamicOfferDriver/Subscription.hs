...
collectManualPayments :: ShortId DMerchant.Merchant -> City.City -> ApiTokenInfo -> Id Common.Driver -> SD.CollectManualPaymentsReq -> FlowHandler APISuccess
collectManualPayments merchantShortId opCity apiTokenInfo driverId req = withFlowHandlerAPI' $ do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  transaction <- buildTransaction SD.CollectPaymentsEndPoint apiTokenInfo (Just driverId)
  T.withTransactionStoring transaction $
    Client.callDriverOfferBPPOperations checkedMerchantId opCity (.subscription.collectManualPayments) driverId req
