...
addLinkAsMedia merchantShortId opCity apiTokenInfo req = withFlowHandlerAPI $ do
  ...
  T.withTransactionStoring transaction $
    Client.callDriverOfferBPP checkedMerchantId opCity (.message.addLinkAsMedia) req
  where
...
uploadFile merchantShortId opCity apiTokenInfo req = withFlowHandlerAPI $ do
  ...
  T.withTransactionStoring transaction $
    Client.callDriverOfferBPP checkedMerchantId opCity (addMultipartBoundary . (.message.uploadFile)) req
  where
...
addMessage merchantShortId opCity apiTokenInfo req = withFlowHandlerAPI $ do
  ...
  T.withTransactionStoring transaction $
    Client.callDriverOfferBPP checkedMerchantId opCity (.message.addMessage) req
...
sendMessage merchantShortId opCity apiTokenInfo req = withFlowHandlerAPI $ do
  ...
  T.withTransactionStoring transaction $
    Client.callDriverOfferBPP checkedMerchantId opCity (addMultipartBoundary . (.message.sendMessage)) req
  where
    addMultipartBoundary clientFn reqBody = clientFn ("XXX00XXX", reqBody)
...
messageList merchantShortId opCity apiTokenInfo mbLimit mbOffset = withFlowHandlerAPI $ do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  Client.callDriverOfferBPP checkedMerchantId opCity (.message.messageList) mbLimit mbOffset
...
messageInfo merchantShortId opCity apiTokenInfo messageId = withFlowHandlerAPI $ do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  Client.callDriverOfferBPP checkedMerchantId opCity (.message.messageInfo) messageId
...
messageDeliveryInfo merchantShortId opCity apiTokenInfo messageId = withFlowHandlerAPI $ do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  Client.callDriverOfferBPP checkedMerchantId opCity (.message.messageDeliveryInfo) messageId
...
messageReceiverList merchantShortId opCity apiTokenInfo messageId number status limit offset = withFlowHandlerAPI $ do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  Client.callDriverOfferBPP checkedMerchantId opCity (.message.messageReceiverList) messageId number status limit offset
