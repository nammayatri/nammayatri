...
type PostCommunicationMarkRead =
  ( ApiAuth
      ('DRIVER_OFFER_BPP_MANAGEMENT)
      ('DSL)
      (('PROVIDER_MANAGEMENT) / ('API.Types.ProviderPlatform.Management.COMMUNICATION) / ('API.Types.ProviderPlatform.Management.Communication.POST_COMMUNICATION_MARK_READ))
      :> API.Types.ProviderPlatform.Management.Communication.PostCommunicationMarkRead
  )
...
postCommunicationMarkRead :: (Kernel.Types.Id.ShortId Domain.Types.Merchant.Merchant -> Kernel.Types.Beckn.Context.City -> ApiTokenInfo -> Kernel.Types.Id.Id Dashboard.Common.Communication -> Kernel.Types.Id.Id Dashboard.Common.Person -> Environment.FlowHandler Kernel.Types.APISuccess.APISuccess)
postCommunicationMarkRead merchantShortId opCity apiTokenInfo communicationId personId = withFlowHandlerAPI' $ Domain.Action.ProviderPlatform.Management.Communication.postCommunicationMarkRead merchantShortId opCity apiTokenInfo communicationId personId
