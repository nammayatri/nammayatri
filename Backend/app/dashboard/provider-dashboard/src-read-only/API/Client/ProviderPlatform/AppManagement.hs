...
mkAppManagementAPIs merchantId city token = do
  ...
  (AppManagementAPIs {..})
  where
    driverClientDSL :<|> driverSubscriptionClientDSL :<|> overlayClientDSL :<|> subscriptionClientDSL :<|> subscriptionTransactionClientDSL = Tools.Client.clientWithMerchantAndCity (Proxy :: Proxy API.Dashboard.AppManagementDSLAPI) merchantId city token
