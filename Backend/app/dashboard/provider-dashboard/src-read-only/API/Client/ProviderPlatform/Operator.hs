...
mkOperatorAPIs merchantId city token = do
  ...
  (OperatorAPIs {..})
  where
    driverClientDSL :<|> fleetClientDSL :<|> registrationClientDSL = Tools.Client.clientWithMerchantAndCity (Proxy :: Proxy API.Dashboard.OperatorDSLAPI) merchantId city token
