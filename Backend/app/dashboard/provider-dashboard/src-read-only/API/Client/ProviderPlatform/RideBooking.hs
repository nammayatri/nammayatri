...
data RideBookingAPIs
    = RideBookingAPIs {driverDSL :: API.Types.Dashboard.RideBooking.Driver.DriverAPIs,
...
mkRideBookingAPIs merchantId city token = do {let {driverDSL = API.Types.Dashboard.RideBooking.Driver.mkDriverAPIs driverClientDSL};
                                              ...
                                              (RideBookingAPIs {..})}
                      where driverClientDSL :<|> driverRegistrationClientDSL :<|> mapsClientDSL :<|> meterRideClientDSL :<|> rideClientDSL :<|> searchRequestClientDSL :<|> volunteerClientDSL = Tools.Client.clientWithMerchantAndCity (Proxy :: Proxy API.Dashboard.RideBookingDSLAPI) merchantId city token
