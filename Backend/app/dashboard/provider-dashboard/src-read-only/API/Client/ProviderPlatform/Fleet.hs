...
data FleetAPIs
    = FleetAPIs {driverDSL :: API.Types.ProviderPlatform.Fleet.Driver.DriverAPIs,
...
mkFleetAPIs merchantId city token = do {let {driverDSL = API.Types.ProviderPlatform.Fleet.Driver.mkDriverAPIs driverClientDSL};
                                        ...
                                        (FleetAPIs {..})}
                where driverClientDSL :<|> liveMapClientDSL :<|> onboardingClientDSL :<|> payoutAccountClientDSL :<|> registrationV2ClientDSL = Tools.Client.clientWithMerchantAndCity (Proxy :: Proxy API.Dashboard.FleetDSLAPI) merchantId city token
callFleetAPI :: forall m r b c . Tools.Client.DashboardClient FleetAPIs m r b c =>
