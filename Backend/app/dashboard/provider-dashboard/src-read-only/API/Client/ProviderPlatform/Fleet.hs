...
mkFleetAPIs merchantId city token = do
  ...
  (FleetAPIs {..})
  where
    driverClientDSL :<|> liveMapClientDSL :<|> onboardingClientDSL = Tools.Client.clientWithMerchantAndCity (Proxy :: Proxy API.Dashboard.FleetDSLAPI) merchantId city token
