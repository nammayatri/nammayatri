...
type GetFrontendFlowStatus = (ApiAuth ('APP_BACKEND)
                                      ('DSL)
                                      (('RIDER_RIDE_BOOKING) / ('API.Types.Dashboard.RideBooking.FRONTEND) / ('API.Types.Dashboard.RideBooking.Frontend.GET_FRONTEND_FLOW_STATUS)) :> API.Types.Dashboard.RideBooking.Frontend.GetFrontendFlowStatus)
type PostFrontendNotifyEvent = (ApiAuth ('APP_BACKEND)
                                        ('DSL)
                                        (('RIDER_RIDE_BOOKING) / ('API.Types.Dashboard.RideBooking.FRONTEND) / ('API.Types.Dashboard.RideBooking.Frontend.POST_FRONTEND_NOTIFY_EVENT)) :> API.Types.Dashboard.RideBooking.Frontend.PostFrontendNotifyEvent)
