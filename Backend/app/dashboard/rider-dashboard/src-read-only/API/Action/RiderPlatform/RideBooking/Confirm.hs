...
type PostConfirmRideSearchQuotes = (ApiAuth ('APP_BACKEND)
                                            ('DSL)
                                            (('RIDER_RIDE_BOOKING) / ('API.Types.Dashboard.RideBooking.CONFIRM) / ('API.Types.Dashboard.RideBooking.Confirm.POST_CONFIRM_RIDE_SEARCH_QUOTES)) :> API.Types.Dashboard.RideBooking.Confirm.PostConfirmRideSearchQuotes)
