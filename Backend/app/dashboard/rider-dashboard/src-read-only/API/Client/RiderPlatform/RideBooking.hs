...
data RideBookingAPIs = RideBookingAPIs
  { bookingDSL :: API.Types.Dashboard.RideBooking.Booking.BookingAPIs,
    ...
    statusDSL :: API.Types.Dashboard.RideBooking.Status.StatusAPIs
  }
...
mkRideBookingAPIs merchantId city token = do
  ...
  let statusDSL = API.Types.Dashboard.RideBooking.Status.mkStatusAPIs statusClientDSL
  (RideBookingAPIs {..})
  where
    bookingClientDSL :<|> cancelClientDSL :<|> confirmClientDSL :<|> frontendClientDSL :<|> mapsClientDSL :<|> profileClientDSL :<|> quoteClientDSL :<|> registrationClientDSL :<|> searchClientDSL :<|> selectClientDSL :<|> statusClientDSL = Tools.Client.clientWithMerchantAndCity (Proxy :: Proxy API.Dashboard.RideBookingDSLAPI) merchantId city token
