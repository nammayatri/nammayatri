...
mkManagementAPIs merchantId city token = do
  ...
  (ManagementAPIs {..})
  where
    bookingClientDSL :<|> customerClientDSL :<|> fRFSTicketClientDSL :<|> insertPartnerOrgStationClientDSL :<|> invoiceClientDSL :<|> merchantClientDSL :<|> nammaTagClientDSL :<|> rideClientDSL :<|> systemClientDSL = Tools.Client.clientWithMerchantAndCity (Proxy :: Proxy API.Dashboard.ManagementDSLAPI) merchantId city token
