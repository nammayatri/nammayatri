...
data ManagementAPIs = ManagementAPIs
  { accountDSL :: API.Types.RiderPlatform.Management.Account.AccountAPIs,
...
mkManagementAPIs merchantId city token = do
  let accountDSL = API.Types.RiderPlatform.Management.Account.mkAccountAPIs accountClientDSL
  ...
  (ManagementAPIs {..})
  where
    accountClientDSL :<|> alertIncidentClientDSL :<|> bookingClientDSL :<|> customerClientDSL :<|> fRFSAlertsClientDSL :<|> fRFSTicketClientDSL :<|> invoiceClientDSL :<|> mediaClientDSL :<|> merchantClientDSL :<|> nammaTagClientDSL :<|> offerClientDSL :<|> rideClientDSL :<|> sosClientDSL :<|> sosMediaClientDSL :<|> systemClientDSL = Tools.Client.clientWithMerchantAndCity (Proxy :: Proxy API.Dashboard.ManagementDSLAPI) merchantId city token
