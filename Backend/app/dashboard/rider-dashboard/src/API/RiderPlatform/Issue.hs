...
type CreateIssueCategoryAPI =
  ApiAuth 'APP_BACKEND_MANAGEMENT 'ISSUE 'CREATE_ISSUE_CATEGORY
    :> Common.CreateIssueCategoryAPI

type UpdateIssueCategoryAPI =
  ApiAuth 'APP_BACKEND_MANAGEMENT 'ISSUE 'UPDATE_ISSUE_CATEGORY
    :> Common.UpdateIssueCategoryAPI

type CreateIssueOptionAPI =
  ApiAuth 'APP_BACKEND_MANAGEMENT 'ISSUE 'CREATE_ISSUE_OPTION
    :> Common.CreateIssueOptionAPI

type UpdateIssueOptionAPI =
  ApiAuth 'APP_BACKEND_MANAGEMENT 'ISSUE 'UPDATE_ISSUE_OPTION
...
ticketStatusCallBack merchantShortId opCity apiTokenInfo req = withFlowHandlerAPI' $ do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  Client.callRiderAppOperations checkedMerchantId opCity (.issuesV2.ticketStatusCallBack_) req
...
createIssueCategory merchantShortId opCity apiTokenInfo req = withFlowHandlerAPI' $ do
  ...
  T.withTransactionStoring transaction $ Client.callRiderAppOperations checkedMerchantId opCity (.issuesV2.createIssueCategory) req
...
updateIssueCategory merchantShortId opCity apiTokenInfo issueCategoryId req = withFlowHandlerAPI' $ do
  ...
  T.withTransactionStoring transaction $ Client.callRiderAppOperations checkedMerchantId opCity (.issuesV2.updateIssueCategory) issueCategoryId req
...
createIssueOption merchantShortId opCity apiTokenInfo issueCategoryId issueMessageId req = withFlowHandlerAPI' $ do
  ...
  T.withTransactionStoring transaction $ Client.callRiderAppOperations checkedMerchantId opCity (.issuesV2.createIssueOption) issueCategoryId issueMessageId req
