...
issueCategoryList merchantShortId apiTokenInfo = withFlowHandlerAPI $ do
  ...
  Client.callRiderApp checkedMerchantId (.issuesV2.issueCategoryList)
...
issueList merchantShortId apiTokenInfo mbLimit mbOffset mbStatus mbCategoryId mbAssignee = withFlowHandlerAPI $ do
  checkedMerchantId <- merchantAccessCheck merchantShortId apiTokenInfo.merchant.shortId
  Client.callRiderApp checkedMerchantId (.issuesV2.issueList) mbLimit mbOffset mbStatus mbCategoryId mbAssignee
...
issueInfo merchantShortId apiTokenInfo issueReportId_ = withFlowHandlerAPI $ do
  checkedMerchantId <- merchantAccessCheck merchantShortId apiTokenInfo.merchant.shortId
  addAuthorDetails =<< Client.callRiderApp checkedMerchantId (.issuesV2.issueInfo) issueReportId_
  where
...
issueUpdate merchantShortId apiTokenInfo issueReportId req = withFlowHandlerAPI $ do
  ...
  T.withTransactionStoring transaction $
    Client.callRiderApp checkedMerchantId (.issuesV2.issueUpdate) issueReportId (mkRequest req)
  where
...
issueAddComment merchantShortId apiTokenInfo issueReportId req = withFlowHandlerAPI $ do
  ...
  T.withTransactionStoring transaction $
    Client.callRiderApp checkedMerchantId (.issuesV2.issueAddComment) issueReportId (mkRequest req)
  where
...
issueFetchMedia merchantShortId apiTokenInfo filePath = withFlowHandlerAPI $ do
  checkedMerchantId <- merchantAccessCheck merchantShortId apiTokenInfo.merchant.shortId
  Client.callRiderApp checkedMerchantId (.issuesV2.issueFetchMedia) filePath
...
ticketStatusCallBack merchantShortId apiTokenInfo req = withFlowHandlerAPI $ do
  ...
  T.withTransactionStoring transaction $ Client.callRiderApp checkedMerchantId (.issuesV2.ticketStatusCallBack_) req
