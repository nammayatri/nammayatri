...
callConfirm :: ShortId DM.Merchant -> City.City -> ApiTokenInfo -> Id DP.Person -> Id Quote.Quote -> Maybe (Id DMPM.MerchantPaymentMethod) -> Maybe UTCTime -> Maybe Int -> FlowHandler UC.ConfirmRes
callConfirm merchantShortId opCity apiTokenInfo personId quoteId mbMerchantPaymentId startTime rentalDurationInHr = withFlowHandlerAPI $ do
  checkedMerchantId <- merchantAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  transaction <- buildTransaction BAP.ConfirmEndPoint apiTokenInfo T.emptyRequest
  T.withTransactionStoring transaction $
    Client.callRiderApp checkedMerchantId opCity (.rideBooking.confirm.rconfirm) personId quoteId mbMerchantPaymentId startTime rentalDurationInHr
