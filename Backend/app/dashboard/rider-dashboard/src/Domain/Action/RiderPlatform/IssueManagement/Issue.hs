...
postIssueCategoryDefaultCopy :: (Kernel.Types.Id.ShortId Domain.Types.Merchant.Merchant -> Kernel.Types.Beckn.Context.City -> ApiTokenInfo -> Environment.Flow IssueManagement.Common.Dashboard.Issue.CopyAllDefaultIssueCategoryRes)
...
postIssueChatMessage merchantShortId opCity apiTokenInfo issueId req = do
  ...
  SharedLogic.Transaction.withTransactionStoring transaction $
    API.Client.RiderPlatform.IssueManagement.callIssueManagementAPI
      checkedMerchantId
      opCity
      (.issueDSL.postIssueChatMessage)
      issueId
      (mkRequest req)
  where
    mkRequest IssueManagement.Common.Dashboard.Issue.SendChatMessageReq {..} =
