...
postCustomerPersonNumbers merchantShortId opCity apiTokenInfo req = do
  ...
  SharedLogic.Transaction.withTransactionStoring transaction $ (do RiderPlatformClient.RiderApp.Operations.callRiderAppOperations checkedMerchantId opCity (Dashboard.Common.addMultipartBoundary "XXX00XXX" . (.customerDSL.postCustomerPersonNumbers)) req)
