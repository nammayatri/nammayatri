...
postCustomerUnblock merchantShortId opCity apiTokenInfo customerId = do
  ...
  SharedLogic.Transaction.withTransactionStoring transaction $ do
    RiderPlatformClient.RiderApp.Operations.callRiderAppOperations checkedMerchantId opCity (.customerDSL.postCustomerUnblock) customerId
...
postCustomerCancellationDuesSync merchantShortId opCity apiTokenInfo customerId req = do
  ...
  SharedLogic.Transaction.withTransactionStoring transaction $
    RiderPlatformClient.RiderApp.Operations.callRiderAppOperations checkedMerchantId opCity (.customerDSL.postCustomerCancellationDuesSync) customerId req
...
postCustomerUpdateSafetyCenterBlocking merchantShortId opCity apiTokenInfo customerId req = do
  ...
  SharedLogic.Transaction.withTransactionStoring transaction $
    RiderPlatformClient.RiderApp.Operations.callRiderAppOperations checkedMerchantId opCity (.customerDSL.postCustomerUpdateSafetyCenterBlocking) customerId req
...
postCustomerPersonId merchantShortId opCity apiTokenInfo req = do
  ...
  SharedLogic.Transaction.withTransactionStoring transaction $
    RiderPlatformClient.RiderApp.Operations.callRiderAppOperations checkedMerchantId opCity (Dashboard.Common.addMultipartBoundary "XXX00XXX" . (.customerDSL.postCustomerPersonId)) req
