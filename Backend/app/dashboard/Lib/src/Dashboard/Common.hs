...
-- | Stored as-is when a JSON request body is audited, as provider-dashboard and
-- rider-dashboard did from their own Storage.Beam.CommonInstances. Lives here so
-- the application servers, which audit direct-dashboard calls, have it too.
instance Kernel.Types.HideSecrets.HideSecrets Value where
  hideSecrets v = v
