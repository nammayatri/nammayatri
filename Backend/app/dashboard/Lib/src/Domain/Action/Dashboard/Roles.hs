...
assignAccessLevel _ roleId req = do
  ...
  mbAccessMatrixItem <- QMatrix.findByRoleIdAndEntityAndActionType roleId req.apiEntity req.userActionType req.otherUserActionType
  case mbAccessMatrixItem of
    Just accessMatrixItem -> QMatrix.updateUserAccessType accessMatrixItem.id req.userActionType req.userAccessType req.otherUserActionType
    Nothing -> do
  ...
  pure
    DMatrix.AccessMatrixItem
      { id = uid,
        ...
        otherUserActionType = req.otherUserActionType,
        createdAt = now,
        updatedAt = now
      }
