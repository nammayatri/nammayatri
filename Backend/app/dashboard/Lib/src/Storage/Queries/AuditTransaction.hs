...
-- | Writing the dashboard audit trail.
--
-- The endpoint is the plain MODULE\/RESOURCE\/ACTION text the column stores, so
-- an application server can write the trail without naming the dashboard API
-- action types (which live in lib-dashboard-api, and which it cannot depend
-- on).
--
-- Every write to the table goes through here, including the dashboards' own:
-- 'Storage.Queries.Transaction' renders its typed @Endpoint@ to that same text
-- and hands the row over. One Beam type, one KV path, one row shape.
module Storage.Queries.AuditTransaction
  ( AuditTransaction (..),
    writeAuditTransaction,
    writeAuditTransactionWithId,
...
writeAuditTransaction entry = do
  ...
  writeAuditTransactionWithId txnId txnCreatedAt entry

-- | Insert one audit row whose identity and timestamp the caller already holds.
--
-- For 'Storage.Queries.Transaction', whose typed transactions are built with
-- both before the call they record. Going through here keeps every write to the
-- table on this one Beam type and its KV path.
writeAuditTransactionWithId :: BeamFlow m r => Text -> UTCTime -> AuditTransaction -> m ()
writeAuditTransactionWithId txnId txnCreatedAt entry =
