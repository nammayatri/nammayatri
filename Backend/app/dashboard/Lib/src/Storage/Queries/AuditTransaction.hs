...
-- | Whether an endpoint id names a read. Generated action names start with
-- their HTTP method (MODULE\/RESOURCE\/GET_...), so the last segment says so.
--
-- Reads are not audited: provider-dashboard recorded mutations only, and a row
-- per read would put a database write on every page load.
isReadOnlyEndpoint :: Text -> Bool
isReadOnlyEndpoint endpointId = "GET_" `T.isPrefixOf` T.takeWhileEnd (/= '/') endpointId
