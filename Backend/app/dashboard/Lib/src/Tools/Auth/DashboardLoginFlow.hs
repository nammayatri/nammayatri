...
-- | 'DashboardLoginFlow'' pinned to the one monad every mount uses it at.
-- Every call site instantiates @m@ as @FlowR r@, so the flow-monad argument is
-- pure repetition; this is the constraint every handler signature should use.
type DashboardLoginFlow r = DashboardLoginFlow' (FlowR r) r
