...
getRemainingLegs :: JourneyLeg leg m => Journey -> m [leg]
  -- get the remaining legs
--   getAllLegs 
--   filter -> based on status

-- createLeg :: RequiedData (OTP response for each leg)
deleteLeg :: JourneyLeg leg m => Journey -> leg -> m ()
  -- cancel the leg if possible
  -- remove it from the journey @@do we actually need to remove?
  
updateLeg :: JourneyLeg leg m => Journey -> leg -> m ()
  -- update the leg
...
-- getAllLegs (findAll), 
-- when to call getFare 
-- every time we call otp do we need to use new journey?

updateCurrentJourneyLeg :: JourneyLegStatus -> m ()
-- let leg = getCurrentLeg 
-- leg' = leg {status = 2} @@ modify obj
-- updateLeg leg'
-- 
...
startJourney :: Journey -> m ()
-- getFirst leg
-- call start for first leg @@update
