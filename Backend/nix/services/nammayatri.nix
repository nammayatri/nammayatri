...
let
  inherit (lib) types;
  # Read resolved ports from data/ports-resolved.nix (generated by resolve-ports.sh).
  ...
  # sets PORTS_RESOLVED_FILE to the absolute path and passes --impure so that
  # builtins.getEnv works. Without --impure, getEnv returns "" → safe fallback.
  resolvedPortsPath = builtins.getEnv "PORTS_RESOLVED_FILE";
  ports = if resolvedPortsPath != "" && builtins.pathExists (/. + resolvedPortsPath)
    ...
    let
      ...
      common = { name, ... }: {
        ...
        environment = {
          ...
          KAFKA_BROKER = "localhost:${toString ports.kafka}";
        ...
        processes = lib.mkMerge [
          ...
          notification-service = {
            ...
            environment = {
              ...
              HTTP_SERVER_PORT = toString ports.notification-service-http;
              DRIVER_APP_PORT = toString ports.dynamic-offer-driver-app;
            };
            availability = {
              restart = "on_failure";
