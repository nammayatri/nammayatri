...
let
  inherit (lib) types;
  # Resolved ports from the devbox registry slice (.users.<DEVBOX_KEY>.ports),
  # written by the run-mobility-stack-dev preflight. Env vars come via getEnv
  # (needs --impure); fall back to base ports.nix if unset/missing.
  ...
  registryPorts =
    ...
    then (builtins.fromJSON (builtins.readFile (/. + registryPath))).users.${devKey}.ports or null
    else null;
  ports = if registryPorts != null then registryPorts else import ./ports.nix;
    ...
    let
      ...
      common = { name, ... }: {
        ...
        environment = {
          ...
          # their dhall configs with the resolved ports (from the devbox
          # registry slice above, which may differ on a shared devbox).
