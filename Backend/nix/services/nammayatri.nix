...
let
  ...
  # pkgs.glibcLocales is null off Linux, and interpolating null anywhere in a
  # process command fails the WHOLE process-compose derivation with a bare
  # "cannot coerce null to a string" naming whichever process was evaluated
  # first — the error has nothing to do with the process it points at.
  #
  # Total by construction: empty off Linux. The only consumer is the
  # CI-namespace branch, which is Linux-only shell that never executes on
  # darwin, so an empty LOCALE_ARCHIVE there is inert. Evaluation stays total on
  # every platform regardless of which caller forces it.
  glibcLocaleArchive =
...
in
{
      ...
      type = types.submodule {
        options = {
          ...
          useCaddy = lib.mkOption {
            ...
            default = true;
            description = ''
              Front the stack with caddy on a single origin. Needed when several
              developers' stacks share a dev-box (each on resolved ports); the
              Caddyfile is generated by the
              run-mobility-stack-dev-on-available-ports preflight.

              With this off, caddy is not started AND the per-process
              "wait for caddy to be healthy" gate is dropped — otherwise every
              process would wait forever on a process that never starts.
            '';
          };
  ...
  config =
    let
      ...
      # Linux-only body (unshare / mount --bind / /proc). macOS gets the bare
      # command: there is no CI cache to overlay there (cache-restore is
      # disabled off Linux for the same reason).
      wrapInCiNamespace = cmd:
        if !pkgs.stdenv.isLinux then cmd else ''
          if [ -f .ci-project-root ]; then
            ...
            unshare --user --mount --map-root-user bash -c "
              set -e
              export LOCALE_ARCHIVE=${glibcLocaleArchive}
      ...
      settings = {
        ...
        processes = lib.mkMerge [
          ...
          })
          {
            ...
            cache-restore = {
              imports = [ common ];
              # Non-Linux hosts are out: the cached dist-newstyle is built by CI
              # on Linux with a Linux GHC derivation, so restoring it on macOS
              # cannot produce a usable incremental build — it would only wipe
              # the local dist-newstyle and force a full rebuild. (The Linux-only
              # CI-namespace overlay the cache relies on is a no-op there too.)
              disabled = !pkgs.stdenv.isLinux || !cfg.useCabal || cfg.profile != "backend";
              command = pkgs.writeShellApplication {
                name = "cache-restore";
            ...
            cabal-build = {
              ...
              # Same platform guard as cache-restore itself: waiting on a
              # process that is never started would stall the build forever.
              depends_on = lib.optionalAttrs (cfg.profile == "backend" && pkgs.stdenv.isLinux) {
