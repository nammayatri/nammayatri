...
testOrder = do
  ...
  res <-
    runARDUFlow "Test ordering" $
      (S.getNearestDrivers (createNearestDriverReq 5000 now) <&> getIds . fst)
...
testInRadius = do
  ...
  res <-
    runARDUFlow "Test radius filtration" $
      (S.getNearestDrivers (createNearestDriverReq 800 now) <&> getIds . fst)
...
testNotInRadius = do
  ...
  res <-
    runARDUFlow "Test outside radius filtration" $
      (S.getNearestDrivers (createNearestDriverReq 10 now) <&> getIds . fst)
