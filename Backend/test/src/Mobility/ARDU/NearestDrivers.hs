...
testOrder = do
  res <-
    runARDUFlow "Test ordering" $
      S.getNearestDrivers Nothing pickupPoint 5000 org1 False (Just hour) False False <&> getIds
...
testInRadius = do
  res <-
    runARDUFlow "Test radius filtration" $
      S.getNearestDrivers Nothing pickupPoint 800 org1 False (Just hour) False False <&> getIds
...
testNotInRadius = do
  res <-
    runARDUFlow "Test outside radius filtration" $
      S.getNearestDrivers Nothing pickupPoint 10 org1 False (Just hour) False False <&> getIds
