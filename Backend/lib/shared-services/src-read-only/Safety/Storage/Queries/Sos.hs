...
findByRideId :: (Safety.Storage.BeamFlow.BeamFlow m r) => (Kernel.Types.Id.Id Safety.Domain.Types.Common.Ride -> m (Maybe Safety.Domain.Types.Sos.Sos))
...
updateMediaFiles :: (Safety.Storage.BeamFlow.BeamFlow m r) => (Kernel.Prelude.Maybe [Kernel.Prelude.Text] -> Kernel.Types.Id.Id Safety.Domain.Types.Sos.Sos -> m ())
updateMediaFiles mediaFiles id = do _now <- getCurrentTime; updateOneWithKV [Se.Set Beam.mediaFiles mediaFiles, Se.Set Beam.updatedAt _now] [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id)]
...
updateByPrimaryKey (Safety.Domain.Types.Sos.Sos {..}) = do
  ...
  updateWithKV
    [ Se.Set Beam.entityType entityType,
      ...
      Se.Set Beam.mediaFiles mediaFiles,
