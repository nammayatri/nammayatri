...
findActiveByPersonId limit offset personId status = do
...
findByPersonIdAndState :: (Safety.Storage.BeamFlow.BeamFlow m r) => (Kernel.Types.Id.Id Safety.Domain.Types.Common.Person -> Safety.Domain.Types.Sos.SosStatus -> m (Maybe Safety.Domain.Types.Sos.Sos))
findByPersonIdAndState personId status = do findOneWithKV [Se.And [Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId), Se.Is Beam.status $ Se.Eq status]]
...
findLatestPendingNonRideSosByPersonId ::
  (Safety.Storage.BeamFlow.BeamFlow m r) =>
...
findLatestPendingNonRideSosByPersonId personId status entityType = do
  findOneWithKV
    [ Se.And
        [ Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId),
