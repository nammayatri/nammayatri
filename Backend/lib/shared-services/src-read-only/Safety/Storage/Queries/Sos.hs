...
findActiveByPersonId :: (Safety.Storage.BeamFlow.BeamFlow m r) =>
                        (Maybe Int -> Maybe Int -> Kernel.Types.Id.Id Safety.Domain.Types.Common.Person -> [Safety.Domain.Types.Sos.SosStatus] -> m ([Safety.Domain.Types.Sos.Sos]))
findActiveByPersonId limit offset personId status = do findAllWithOptionsKV [Se.And [Se.Is Beam.personId $ Se.Eq (Kernel.Types.Id.getId personId),
                                                                                     Se.Is Beam.status $ Se.In status]] (Se.Desc Beam.createdAt) limit offset
...
updateExternalReferenceId externalReferenceId id = do {_now <- getCurrentTime;
...
updateExternalReferenceStatus :: (Safety.Storage.BeamFlow.BeamFlow m r) =>
...
updateExternalReferenceStatus externalReferenceStatus externalStatusHistory id = do {_now <- getCurrentTime;
                                                                                     updateOneWithKV [Se.Set Beam.externalReferenceStatus externalReferenceStatus,
...
updateMediaFiles mediaFiles id = do {_now <- getCurrentTime;
...
updateTrackingExpiresAt trackingExpiresAt id = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Safety.Domain.Types.Sos.Sos {..}) = do {_now <- getCurrentTime;
                                                            updateWithKV [Se.Set Beam.entityType entityType,
...
instance FromTType' Beam.Sos Safety.Domain.Types.Sos.Sos
    where fromTType' (Beam.SosT {..}) = do pure $ Just Safety.Domain.Types.Sos.Sos{createdAt = createdAt,
                                                                                   ...
                                                                                   updatedAt = updatedAt}
instance ToTType' Beam.Sos Safety.Domain.Types.Sos.Sos
    where toTType' (Safety.Domain.Types.Sos.Sos {..}) = do Beam.SosT{Beam.createdAt = createdAt,
