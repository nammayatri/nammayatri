...
getLanguage :: Monad m => Id Person -> Maybe Language -> ServiceHandle m -> m Language
...
issueReportList (personId, _) mbLanguage issueHandle identifier = do
  ...
  where
    ...
    processIssueReport iConfig currTime identifier_ language iReport = do
      ...
      if iReport.status == AWAIT && timeDiff > iConfig.autoMarkIssueResolveDuration
        then do
          QIR.updateStatusAssignee iReport.id (Just RESOLVED) iReport.assignee
          ...
          mkIssueReport iReport (Just RESOLVED) language
...
fetchMedia :: (HasField "s3Env" r (S3Env m), MonadReader r m) => (Id Person, Id Merchant) -> Text -> m Text
...
createIssueReport (personId, merchantId) mbLanguage Common.IssueReportReq {..} issueHandle identifier = do
  ...
  ticketResponse <- issueHandle.createTicket merchantId ticket
  ...
  where
    ...
    buildTicket :: (CacheFlow m r, BeamFlow m, EncFlow m r) => D.IssueReport -> D.IssueCategory -> Maybe D.IssueOption -> Maybe Ride -> ShortId Merchant -> [Text] -> ServiceHandle m -> Identifier -> m TIT.CreateTicketReq
    ...
    buildRideInfo :: (CacheFlow m r, BeamFlow m) => ShortId Merchant -> ServiceHandle m -> Ride -> m TIT.RideInfo
