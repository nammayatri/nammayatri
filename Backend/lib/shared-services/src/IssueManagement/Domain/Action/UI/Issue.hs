...
createIssueReportWithContext creationContext args@(personId, _merchantId) mbLanguage req@Common.IssueReportReq {..} issueHandle identifier becknIssueId = do
  ...
  case mbCached of
    ...
    Nothing ->
      Redis.withWaitAndLockRedis lockKey 5 1000000 $ do
  ...
  where
    doForward merchantId mocId = do
      ...
      let call = case issueHandle.mbUpdateTicketOnService of
            ...
            Nothing -> issueHandle.updateTicket merchantId mocId (issueReport.additionalTicketIds <|> issueReport.ticketId) ticketReq
...
resolveApiCallSteps personId categoryId mbRideId mbMerchantOpCity language identifier issueHandle initialList maxHops = do
  ...
  if not (any (\(msg, _, _) -> msg.messageType == D.ApiCall) initialList)
    then pure (initialList, builtins)
      ...
      -- Left-biased union: builtins (personId, categoryId, …) must win over
      -- anything a previously stored flow context happens to contain.
      go initialList maxHops (Map.union builtins storedCtx)
  where
