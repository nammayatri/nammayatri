    ...
    then pure $ failed Nothing 0 ("Unresolved placeholder in URL: " <> url)
      ...
      result <- liftIO $
        try @_ @SomeException $ do
          manager <-
            Http.newManager
              -- Floor of 100ms guards against 0/negative values from old DB rows
              -- written before the upsert-side clamp existed.
              (HttpTLS.tlsManagerSettings {Http.managerResponseTimeout = Http.responseTimeoutMicro (max 100 timeoutMs * 1000)})
                ...
                (failed (Just status) latency ("Non-success status: " <> show status))
                  { rawResponse =
                      mbJson
                        <|> Just
                          ( A.String
                              (either (const "<non-utf8 response body>") id (TE.decodeUtf8' (BSL.toStrict (BSL.take 2000 bodyBytes))))
                          )
                  }
  where
