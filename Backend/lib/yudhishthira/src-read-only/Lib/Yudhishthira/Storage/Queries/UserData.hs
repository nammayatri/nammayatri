...
findAllByUserIdAndEventId :: (Lib.Yudhishthira.Storage.Beam.BeamFlow.BeamFlow m r) =>
...
updateByPrimaryKey (Lib.Yudhishthira.Types.UserData.UserData {..}) = do {_now <- getCurrentTime;
                                                                         updateWithKV [Se.Set Beam.batchNumber batchNumber,
