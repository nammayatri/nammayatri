...
findAllByPrimaryKeys :: (Lib.Yudhishthira.Storage.Beam.BeamFlow.BeamFlow m r) =>
...
updateByPrimaryKey (Lib.Yudhishthira.Types.NammaTagV2.NammaTagV2 {..}) = do {_now <- getCurrentTime;
                                                                             updateWithKV [Se.Set Beam.actionEngine actionEngine,
                                                                                           ...
                                                                                           Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId), Se.Is Beam.name $ Se.Eq name]]}
