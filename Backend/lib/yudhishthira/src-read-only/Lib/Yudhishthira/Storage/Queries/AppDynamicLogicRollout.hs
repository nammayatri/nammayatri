...
findAllByMerchantOpCityId :: (Lib.Yudhishthira.Storage.Beam.BeamFlow.BeamFlow m r) =>
...
findByMerchantOpCityAndDomain :: (Lib.Yudhishthira.Storage.Beam.BeamFlow.BeamFlow m r) =>
...
findByMerchantOpCityAndDomain merchantOperatingCityId domain = do findAllWithKV [Se.And [Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
...
findByPrimaryKey domain merchantOperatingCityId timeBounds version = do findOneWithKV [Se.And [Se.Is Beam.domain $ Se.Eq domain,
...
updateByPrimaryKey (Lib.Yudhishthira.Types.AppDynamicLogicRollout.AppDynamicLogicRollout {..}) = do {_now <- getCurrentTime;
                                                                                                     updateWithKV [Se.Set Beam.experimentStatus experimentStatus,
                                                                                                                   ...
                                                                                                                   Se.Set Beam.updatedAt _now] [Se.And [Se.Is Beam.domain $ Se.Eq domain,
                                                                                                                                                        Se.Is Beam.merchantOperatingCityId $ Se.Eq (Kernel.Types.Id.getId merchantOperatingCityId),
                                                                                                                                                        Se.Is Beam.timeBounds $ Se.Eq timeBounds,
                                                                                                                                                        Se.Is Beam.version $ Se.Eq version]]}
