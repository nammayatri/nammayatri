...
data UserDataT f
    = UserDataT {batchNumber :: (B.C f Kernel.Prelude.Int),
...
instance B.Table UserDataT
    where data PrimaryKey UserDataT f = UserDataId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
