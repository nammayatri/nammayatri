...
data NammaTagTriggerT f
    = NammaTagTriggerT {createdAt :: (B.C f Kernel.Prelude.UTCTime),
                        event :: (B.C f Lib.Yudhishthira.Types.ApplicationEvent),
                        tagName :: (B.C f Kernel.Prelude.Text),
                        updatedAt :: (B.C f Kernel.Prelude.UTCTime)}
    deriving (Generic, B.Beamable)
instance B.Table NammaTagTriggerT
    where data PrimaryKey NammaTagTriggerT f = NammaTagTriggerId (B.C f Lib.Yudhishthira.Types.ApplicationEvent) (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
