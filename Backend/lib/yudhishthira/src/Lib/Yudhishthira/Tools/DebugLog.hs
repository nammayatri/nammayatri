...
checkDebugLogFlags mocId identifier' = do
  ...
  case mbEnabled of
    Just "1" -> do
      mbStartStr :: Maybe Text <- Hedis.runInMultiCloudRedisMaybeResult $ Hedis.get (mkDebugStartTimeKey mocId identifier')
      mbEndStr :: Maybe Text <- Hedis.runInMultiCloudRedisMaybeResult $ Hedis.get (mkDebugEndTimeKey mocId identifier')
      now <- getCurrentTime
      let mbInWindow = do
            startStr <- mbStartStr
            endStr <- mbEndStr
...
insertJsonLogicTransaction callerApp domain mbEntityTransactionId inputData logic outputData = do
  ...
  let entityTxnIdVal = maybe "NULL" (\t -> "'" <> T.unpack (escapeStr t) <> "'") mbEntityTransactionId
  let query =
