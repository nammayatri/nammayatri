  ...
  where
    ...
    buildNammaTagTriggers now = case tagRequest of
      Lib.Yudhishthira.Types.ApplicationTag Lib.Yudhishthira.Types.NammaTagApplication {..} -> do
        ...
        pure . Just $ events <&> \event -> DNTT.NammaTagTrigger {event, tagName, ..}
      _ -> pure Nothing
...
postTagUpdate tagRequest = do
  ...
  where
    ...
    buildNammaTagTriggers tag now = do
      let mbEvents = tagRequest.tagStages <|> (NE.singleton <$> tagRequest.tagStage)
      case tag.info of
        DNT.Application -> do
          let createdAt = now
              updatedAt = now
          whenJust mbEvents $ \events -> do
            unless (length events == length (NE.nub events)) $
              throwError (InvalidRequest "Tag stages should be unique")
          pure $ mbEvents <&> fmap \event -> DNTT.NammaTagTrigger {event, tagName = tagRequest.tagName, ..}
        _ -> do
          whenJust mbEvents $ \_ -> throwError (InvalidRequest "tagStage relevant only for application tags")
          pure Nothing
