...
findActiveByMerchantOpCityAndDomain merchantOperatingCityId domain =
  do
    Hedis.safeGet $ activeDomainCacheKey merchantOperatingCityId domain
    >>= ( \case
...
clearCityConfigsCache cityId = Hedis.runInMultiCloudRedisWrite $ Hedis.del $ cityConfigsCacheKey cityId
...
clearDomainCache cityId domain = Hedis.runInMultiCloudRedisWrite $ Hedis.del $ domainCacheKey cityId domain
...
clearActiveDomainCache cityId domain = do
  logDebug $ "[ACTIVE_DELETE] domain=" <> show domain <> " city=" <> Kernel.Types.Id.getId cityId
...
clearBaseRolloutCacheKey cityId domain = do
  logDebug $ "[BASEROLLOUT_DELETE] domain=" <> show domain <> " city=" <> Kernel.Types.Id.getId cityId
