...
data LogicDomain
  ...
  | USER_CANCELLATION_DUES_NO_SHOW
  | ESTIMATE_TAGS
...
generateLogicDomainShowInstances =
  [show POOLING]
    ...
    ++ [show USER_CANCELLATION_DUES_NO_SHOW]
...
instance Show LogicDomain where
  ...
  show USER_CANCELLATION_DUES_NO_SHOW = "USER-CANCELLATION-DUES-NO-SHOW"
  ...
  readsPrec _ s =
    let (prefx, rest) = break (== '_') s
     in case prefx of
          ...
          "USER-CANCELLATION-DUES-NO-SHOW" ->
            [(USER_CANCELLATION_DUES_NO_SHOW, drop 1 rest)]
