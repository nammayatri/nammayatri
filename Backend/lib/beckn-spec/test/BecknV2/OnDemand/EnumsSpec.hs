...
spec = describe "BecknV2.OnDemand.Enums" $ do
  ...
  -- ================================================================
  -- BecknErrorCode (new v2.1.0: error code 90203)
  -- ================================================================

  describe "BecknErrorCode (v2.1.0)" $ do
    it "DRIVER_NOT_ASSIGNED shows as \"90203\"" $ do
      DRIVER_NOT_ASSIGNED `showsAs` "90203"

    it "DRIVER_NOT_ASSIGNED serializes to \"90203\"" $ do
      DRIVER_NOT_ASSIGNED `serializesTo` "90203"

    it "DRIVER_NOT_ASSIGNED round-trips" $ do
      roundTrip DRIVER_NOT_ASSIGNED

    it "parses from JSON string \"90203\"" $ do
      let parsed = decode "\"90203\"" :: Maybe BecknErrorCode
      parsed `shouldBe` Just DRIVER_NOT_ASSIGNED

  -- ================================================================
  -- CancellationSource (existing, unchanged)
  -- ================================================================

  describe "CancellationSource (existing, unchanged)" $ do
    it "CONSUMER round-trips" $ do
      roundTrip CONSUMER

    it "PROVIDER round-trips" $ do
      roundTrip PROVIDER

    it "CONSUMER serializes to \"CONSUMER\"" $ do
      CONSUMER `serializesTo` "CONSUMER"
