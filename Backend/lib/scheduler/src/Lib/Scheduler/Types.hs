...
instance (JobProcessor t) => FromJSON (AnyJob t) where
  parseJSON (Object obj) = do
    jobData <- obj .: "jobData"
    jobType <- obj .: "jobType"
    (AnyJobInfo jobInfo) :: AnyJobInfo t <- maybe (fail "Failed to restore AnyJobInfo") pure (restoreAnyJobInfoMain @t (StoredJobInfo jobType jobData))
    id <- obj .: "id"
    shardId <- obj .: "shardId"
...
instance ToJSON (AnyJob t) where
  toJSON (AnyJob Job {..}) = do
    ...
    object
      [ "id" .= getId id,
        "jobType" .= storedJobType storedJobInfo,
        "jobData" .= storedJobContent storedJobInfo,
        "shardId" .= shardId,
