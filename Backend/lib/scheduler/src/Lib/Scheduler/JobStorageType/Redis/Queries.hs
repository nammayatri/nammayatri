...
createJobInRedis :: forall m r t (e :: t). (MonadTime m, MonadGuid m, MonadThrow m, Log m, HedisFlow m r) => AnyJob t -> m ()
createJobInRedis value = do
  hedisConnection <- asks (.hedisClusterEnv.hedisConnection)
  stream <- asks (.jobStreamName)
  entryId <- (\time -> EntryId (show $ nanosSinceEpoch time) "*") <$> getCurrentTime
  void . liftIO $ xAdd hedisConnection stream entryId [(show entryId, value)]

createJob :: forall m r t (e :: t). (SingI e, JobInfoProcessor e, JobProcessor t, MonadTime m, MonadGuid m, MonadThrow m, Log m, HedisFlow m r) => Int -> JobContent e -> m ()
createJob maxShards jobData = do
  void $
    ScheduleJob.createJob createJobInRedis maxShards $
      JobEntry
        { jobData = jobData,
          maxErrors = 5
        }
...
--   void $
--     ScheduleJob.createJobIn (scheduleJobInRedis inTime) inTime maxShards $
--       JobEntry
--         { jobData = jobData,
--           maxErrors = 5
--         }
