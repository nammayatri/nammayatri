...
testInterpolationHandler batchSize =
  RideInterpolationHandler
    { batchSize,
      ...
      interpolatePointsAndCalculateDistance = \pts -> pure (getRouteLinearLength pts, pts, Google),
...
processPointsGroup ih driverId pointsGroup = do
  ...
  run $ processWaypoints ih driverId False 0 Nothing True True False $ NE.fromList pointsGroup
