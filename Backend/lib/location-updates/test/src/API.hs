...
successFlow appEnv config eps expectedDistance rideId route = runFlow "" appEnv $ do
  ...
  forM_ (NE.toList route) $ \updatesBatch ->
    addIntermediateRoutePoints ih Nothing rideId testDriverId updatesBatch False False (21600 :: Int)
  finalDistanceCalculation ih Nothing False rideId testDriverId destination 0 Nothing Nothing True False (21600 :: Int)
...
failFlow appEnv config rideId route = runFlow "" appEnv $ do
  ...
  finalDistanceCalculation ih Nothing rideId testDriverId destination 0 Nothing True False (21600 :: Int)
