...
successFlow appEnv config eps expectedDistance rideId route = runFlow "" appEnv $ do
  ...
  forM_ (NE.toList route) $ \updatesBatch ->
    addIntermediateRoutePoints ih Nothing rideId testDriverId updatesBatch False Nothing
  finalDistanceCalculation ih Nothing False rideId testDriverId destination 0 Nothing Nothing True Nothing Nothing
...
failFlow appEnv config rideId route = runFlow "" appEnv $ do
  ...
  finalDistanceCalculation ih Nothing rideId testDriverId destination 0 Nothing True Nothing Nothing
