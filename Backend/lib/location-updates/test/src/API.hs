...
successFlow appEnv config eps expectedDistance rideId route = runFlow "" appEnv $ do
  ...
  forM_ (NE.toList route) $ \updatesBatch ->
    addIntermediateRoutePoints ih Nothing rideId testDriverId updatesBatch False
  finalDistanceCalculation ih Nothing False rideId testDriverId destination 0 Nothing Nothing True
