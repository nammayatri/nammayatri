...
finalDistanceCalculation :: (CacheFlow m r, Log m, MonadThrow m, MonadFlow m) => I.RideInterpolationHandler person m -> Maybe MapsServiceConfig -> Bool -> Bool -> Bool -> Bool -> Id ride -> Id person -> NonEmpty LatLong -> Meters -> Maybe HighPrecMoney -> Maybe [Text] -> Maybe [Text] -> Maybe HighPrecMoney -> Maybe [Text] -> Maybe [Text] -> Bool -> Bool -> Bool -> m ()
finalDistanceCalculation ih rectifyDistantPointsFailureUsing isTollApplicable isStateEntryPermitApplicable sendTollCrossedNotification sendSepcCrossedNotification rideId driverId pts estDist estTollCharges estTollNames estTollIds estSepcCharges estSepcNames estSepcIds pickupDropOutsideThreshold passedThroughDrop isMeterRide =
  withRideIdLogTag rideId $
    I.processWaypoints ih driverId True estDist estTollCharges estTollNames estTollIds estSepcCharges estSepcNames estSepcIds pickupDropOutsideThreshold rectifyDistantPointsFailureUsing isTollApplicable isStateEntryPermitApplicable sendTollCrossedNotification sendSepcCrossedNotification passedThroughDrop isMeterRide pts
...
addIntermediateRoutePoints :: (CacheFlow m r, Log m, MonadThrow m, MonadFlow m) => I.RideInterpolationHandler person m -> Maybe MapsServiceConfig -> Bool -> Bool -> Bool -> Bool -> Id ride -> Id person -> Bool -> Bool -> NonEmpty LatLong -> m ()
addIntermediateRoutePoints ih rectifyDistantPointsFailureUsing isTollApplicable isStateEntryPermitApplicable sendTollCrossedNotification sendSepcCrossedNotification rideId driverId passedThroughDrop isMeterRide =
  withRideIdLogTag rideId . I.processWaypoints ih driverId False 0 Nothing Nothing Nothing Nothing Nothing Nothing False rectifyDistantPointsFailureUsing isTollApplicable isStateEntryPermitApplicable sendTollCrossedNotification sendSepcCrossedNotification passedThroughDrop isMeterRide -- estimateDistace, estimatedTollCharges/Ids and SEPC estimates not required in case of add intermediatory points
