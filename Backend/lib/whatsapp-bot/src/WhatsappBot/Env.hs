...
-- | Static per-conversation config: the resolved merchant and the poll-loop
-- constants. Held separate from the handles so tests can vary it without
-- touching the effect layer. The poll constants are DB-driven per merchant
-- now (@meta_config.bot_config@, Domain.Types.Extra.MetaWebhookConfig), seeded
-- to match the original TS defaults noted below — not fixed constants anymore.
data BotConfig = BotConfig
  { -- | The merchant resolved for this session (by phone_number_id upstream).
    merchant :: MerchantCtx,
    -- | Flexi quote poll: attempts × interval (seed default from @engine.ts:1074-1078@; 10 × 2000ms).
    flexiQuotePollAttempts :: Int,
    flexiQuotePollIntervalMs :: Int,
    -- | Regular estimate poll (seed default from @engine.ts:825-829@; 6 × 2000ms).
    regularEstimatePollAttempts :: Int,
    regularEstimatePollIntervalMs :: Int,
    -- | Driver-assignment poll (seed default from @engine.ts:890-906,1123-1151@;
    -- 90 × 2000ms, progress notify every 15).
...
-- | One-way hash of the phone number for Redis key material — never embed
-- the raw phone digits in a key (repo convention: phone numbers are
-- DbHash/EncryptedField-only, never plaintext, even in a key string).
hashPhone :: Text -> Text
hashPhone = TE.decodeUtf8 . Base16.encode . SHA256.hash . TE.encodeUtf8
...
scopedSessionId m ev = "whatsapp:" <> m.merchantLabel <> ":" <> hashPhone ev.fromPhone
...
mkUserKey m ev = "whatsapp:" <> m.merchantLabel <> ":" <> hashPhone ev.fromPhone
