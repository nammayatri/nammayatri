...
data BackendHandle m = BackendHandle
  { -- | 10-digit phone -> resolved rider (prod: find-or-create, no OTP; D2/D3).
    ...
    -- | Real, DB-backed geofence check (replaces the old hardcoded-city-list
    -- approximation). Needs BotAuth — the underlying check requires an
    -- already-authenticated person, which is why the caller (Flow.Booking)
    -- authenticates BEFORE calling this, not after.
    checkServiceable :: BotAuth -> LatLon -> m (Either BotError Bool),
