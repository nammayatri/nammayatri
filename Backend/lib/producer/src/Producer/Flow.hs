...
runProducer = do
  ...
  currentJobs <- Hedis.zRangeByScore setName startTime endTime
  _ <- Hedis.zRemRangeByScore setName startTime endTime
  ...
  logError $ "Job chunks producer" <> show jobChunks
