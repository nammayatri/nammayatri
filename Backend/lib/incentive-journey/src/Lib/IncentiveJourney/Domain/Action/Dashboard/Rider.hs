...
getIncentiveJourneyPersonAssignments handle merchantShortId opCity personId = do
  (merchant, merchantOpCityId) <- resolveMerchant handle merchantShortId opCity
  let riderPersonId = ID.cast @Dashboard.Common.Person @DIJC.Person personId
  assignments <- handle.findAssignmentsByUserId riderPersonId
  items <-
    mapMaybeM
...
getIncentiveJourneyCohortJourneyList handle merchantShortId opCity mbLimit mbOffset mbCohortName mbCohortCategory mbIsActive mbJourneyType = do
  ...
  let limitVal = fromMaybe 20 mbLimit
      offsetVal = fromMaybe 0 mbOffset
      merchantId = ID.cast merchant.id
  mbCohortIds <- resolveCohortIdFilter mbCohortName mbCohortCategory
...
resolveCohortIdFilter mbCohortName mbCohortCategory =
...
postIncentiveJourneyAssign handle merchantShortId opCity req = do
  ...
  let cjmId = ID.cast @Dashboard.Common.CohortJourneyMapping @DCJM.CohortJourneyMapping req.cohortJourneyMappingId
      personId = ID.cast @Dashboard.Common.Person @DIJC.Person req.personId
  person <- handle.findPersonById personId >>= fromMaybeM (InvalidRequest "Person not found")
...
validateMilestoneReward :: BeamFlow m r => DIJ.IncentiveJourney -> DIJM.IncentiveJourneyMilestone -> m ()
...
toPersonAssignmentItem ::
  ...
  m (Maybe Common.IncentiveJourneyPersonAssignmentItem)
