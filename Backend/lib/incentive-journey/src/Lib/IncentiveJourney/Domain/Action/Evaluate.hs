...
evaluateIncentiveJourneys ijHandle input = do
  assignments <- CQAssignment.findAssignmentsByUserId ijHandle.actor input.personId
    ...
    _ -> do
      ...
      logInfo $
        "WaivedOff milestone "
          ...
          <> " person="
          <> personId.getId
      assignments <- CQAssignment.findAssignmentsByUserId ijHandle.actor personId
      let mbCjm =
