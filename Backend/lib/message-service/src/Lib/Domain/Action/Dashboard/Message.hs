...
import qualified Lib.Storage.Queries.Message.MediaFile as MFQuery
...
newtype ServiceHandle = ServiceHandle
  { findAllDriverIdExceptProvided :: forall m. Id Domain.Merchant -> [Id Domain.Driver] -> m [Id Domain.Driver]
  }
...
addLinkAsMedia :: ShortId Domain.Merchant -> Common.AddLinkAsMedia -> m Domain.Merchant -> m Common.UploadFileResponse
addLinkAsMedia merchantShortId req merchantByShortId = do
  _ <- merchantByShortId
  createMediaEntry req

createMediaEntry :: Common.AddLinkAsMedia -> m Common.UploadFileResponse
...
uploadFile :: ShortId Domain.Merchant -> Common.UploadFileRequest -> m Domain.Merchant -> (Maybe TransporterConfig) -> m Common.UploadFileResponse
uploadFile merchantShortId Common.UploadFileRequest {..} merchantByShortId merchantId = do
  -- _ <- validateContentType
  merchant <- merchantByShortId
  ...
  transporterConfig <- fromMaybeM (TransporterConfigNotFound merchant.id.getId) merchantId
  let fileUrl =
...
addMessage :: ShortId Domain.Merchant -> Common.AddMessageRequest -> m Domain.Merchant -> m Common.AddMessageResponse
addMessage merchantShortId Common.AddMessageRequest {..} merchantByShortId = do
  merchant <- merchantByShortId
...
sendMessage :: ShortId Domain.Merchant -> Common.SendMessageRequest -> m Domain.Merchant -> ServiceHandle -> m APISuccess
sendMessage merchantShortId Common.SendMessageRequest {..} merchantByShortId sendMessageHandle = do
  merchant <- merchantByShortId
  ...
  allDriverIds <- case _type of
    AllEnabled -> Esq.runInReplica $ sendMessageHandle.findAllDriverIdExceptProvided (merchant.id) []
    Include -> readCsv
    Exclude -> do
      driverIds <- readCsv
      Esq.runInReplica $ sendMessageHandle.findAllDriverIdExceptProvided (merchant.id) driverIds
  ...
  where
    ...
    createMessageLanguageDict :: Domain.RawMessage -> m Domain.MessageDict
...
messageList :: ShortId Domain.Merchant -> Maybe Int -> Maybe Int -> m Domain.Merchant -> m Common.MessageListResponse
messageList merchantShortId mbLimit mbOffset merchantByShortId = do
  merchant <- merchantByShortId
...
messageInfo :: ShortId Domain.Merchant -> Id Domain.Message -> m Domain.Merchant -> m Common.MessageInfoResponse
messageInfo merchantShortId messageId merchantByShortId = do
  _ <- merchantByShortId
...
messageDeliveryInfo :: ShortId Domain.Merchant -> Id Domain.Message -> m Domain.Merchant -> m Common.MessageDeliveryInfoResponse
messageDeliveryInfo merchantShortId messageId merchantByShortId = do
  _ <- merchantByShortId
...
messageReceiverList :: ShortId Domain.Merchant -> Id Domain.Message -> Maybe Text -> Maybe Common.MessageDeliveryStatus -> Maybe Int -> Maybe Int -> m Domain.Merchant -> m Common.MessageReceiverListResponse
messageReceiverList merchantShortId msgId _ mbStatus mbLimit mbOffset merchantByShortId = do
  _ <- merchantByShortId
