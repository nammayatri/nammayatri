...
class (Show a, ConfigTypeInfo (ConfigTypeOf a)) => ConfigDimensions a where
  ...
  -- | Fallback query derived from dimensions, invoked when ConfigPilot fails.
  -- The dimensions record is passed so the fallback can be parameterised by city, service, etc.
  configFallback ::
  ...
  -- The effective fallback is @mbFallback '<|>' configFallback dims@: an explicit caller
  -- fallback takes precedence, then the Config-module-defined fallback, then re-throw.
  getConfig ::
    (MonadFlow m, CacheFlow m r, EsqDBFlow m r) =>
    a ->
    Maybe (m (ConfigValueTypeOf a)) ->
