...
data IndirectTaxTransactionT f
    = IndirectTaxTransactionT {cgstAmount :: (B.C f Kernel.Types.Common.HighPrecMoney),
...
instance B.Table IndirectTaxTransactionT
    where data PrimaryKey IndirectTaxTransactionT f = IndirectTaxTransactionId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
