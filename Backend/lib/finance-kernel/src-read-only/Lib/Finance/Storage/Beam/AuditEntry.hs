...
data AuditEntryT f
    = AuditEntryT {action :: (B.C f Lib.Finance.Domain.Types.AuditEntry.AuditAction),
...
instance B.Table AuditEntryT
    where data PrimaryKey AuditEntryT f = AuditEntryId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
