...
data ReconciliationSummaryT f
    = ReconciliationSummaryT {createdAt :: (B.C f Kernel.Prelude.UTCTime),
...
instance B.Table ReconciliationSummaryT
    where data PrimaryKey ReconciliationSummaryT f = ReconciliationSummaryId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
