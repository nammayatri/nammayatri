...
data LedgerEntryT f
    = LedgerEntryT {amount :: (B.C f Kernel.Types.Common.HighPrecMoney),
...
instance B.Table LedgerEntryT
    where data PrimaryKey LedgerEntryT f = LedgerEntryId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
