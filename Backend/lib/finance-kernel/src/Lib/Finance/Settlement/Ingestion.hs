...
import Kernel.External.Settlement.Interface.Types (ParsePaymentSettlementResult, ParseResult (..), PaymentSettlementReport)
...
storeParseResult merchantId merchantOperatingCityId parseResult = do
  ...
  results <- forM (reports parseResult) $ \report -> do
    let txnType = Transformer.mapTxnType report.txnType
    existing <- QPgReport.findByOrderIdAndTxnType report.orderId txnType
    logDebug $
      "storeParseResult: dedup orderId=" <> show report.orderId
        <> " txnType=" <> show txnType
        <> " existingCount=" <> show (length existing)
