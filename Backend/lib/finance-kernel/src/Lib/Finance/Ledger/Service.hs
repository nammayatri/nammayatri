  ...
  let entry =
        LedgerEntry
          { id = Id entryId,
            ...
            appliedTaxRate = input.appliedTreatment >>= (.taxRate),
            appliedTaxExclusive = (.isTaxExclusive) <$> input.appliedTreatment,
            appliedIndirectTaxDirection = input.appliedTreatment >>= (.indirectTaxDirection),
            appliedCommissionValue = input.appliedTreatment >>= (.commissionValue),
            appliedDirectTaxRates = input.appliedTreatment >>= (.directTaxRates),
      ...
      let reversal =
            LedgerEntry
              { id = Id reversalId,
                ...
                appliedTaxExclusive = original.appliedTaxExclusive,
                appliedIndirectTaxDirection = original.appliedIndirectTaxDirection,
                appliedCommissionValue = original.appliedCommissionValue,
                appliedDirectTaxRates = original.appliedDirectTaxRates,
                metadataV2 =
                  Just $
                    LedgerEntryMetadata
