...
import Lib.Scheduler.JobStorageType.Redis.Types
import Prelude

decodeEither :: (Either Hedis.Reply a) -> (Either String b)
decodeEither eitherVal =
  case eitherVal of
    Left err -> Left $ show err
    Right byteValue -> A.eitherDecode byteValue
...
decodeStreamRecords :: [Hedis.StreamsRecord] -> [[(Text, a)]]
decodeStreamRecords = map (\(Hedis.StreamsRecord _ arrVal) -> mapMaybe (\(field, value) -> (cs field,) <$> A.decode value) $ cs arrVal) -- ignoring recordId here
...
xRange :: Hedis.Connection -> Text -> EntryId -> EntryId -> Count -> IO ([[(Text, a)]])
