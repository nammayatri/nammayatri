...
createTransferService merchantId merchantOperatingCityId entityName entityId createTransferReq createTransferCall = do
  ...
  where
    ...
    getErrMessage exc
      | Just (BaseException baseExc) <- fromException @BaseException exc = "Failed to initiate transfer" <> maybe "" (": " <>) (toMessage baseExc)
    ...
    processTransfer = do
      ...
      createTransferResp <- withTryCatch "createTransferCall:transferService" $ withShortRetry $ createTransferCall createTransferReq
      case createTransferResp of
        Right response -> do
          ...
          pure response
        Left err -> do
          logError $ "Create Transfer API Call Failure with Error: " <> show err
          ...
          throwError $ InvalidRequest "Create Transfer API call error"
