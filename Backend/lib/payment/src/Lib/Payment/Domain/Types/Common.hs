...
data EntityName = MANUAL | DRIVER_DAILY_STATS | BACKLOG | DAILY_STATS_VIA_DASHBOARD | RETRY_VIA_DASHBOARD | DRIVER_FEE | METRO_BOOKING_CASHBACK | REFERRAL_AWARD_RIDE | REFERRED_BY_AWARD | REFERRED_BY_AND_BACKLOG_AWARD | DRIVER_WALLET_TRANSACTION | DRIVER_WALLET_TOPUP | SPECIAL_ZONE_PAYOUT | DRIVER_STCL | COINS_REDEMPTION deriving (Eq, Ord, Show, Read, Generic, ToJSON, FromJSON, ToSchema)
