    ...
    Just transaction -> do
      let updTransaction =
            transaction{statusId = resp.transactionStatusId,
                        ...
                        cardExpMonth = resp.cardExpMonth,
                        cardExpYear = resp.cardExpYear,
                        cardCountry = resp.cardCountry,
                        cardFingerprint = resp.cardFingerprint,
  ...
  let transactionStatus = PInterface.castChargeToTransactionStatus status
      ...
      orderTxn =
        defaultOrderTxn{txnId = paymentIntentId,
                        ...
                        cardBrand = (.brand) <$> cardDetails,
                        cardLastFourDigits = (.last4) <$> cardDetails,
                        cardType = (.funding) =<< cardDetails,
                        cardCountry = (.country) =<< cardDetails,
                        cardExpMonth = (.expMonth) <$> cardDetails,
                        cardExpYear = (.expYear) <$> cardDetails,
                        cardFingerprint = (.fingerprint) =<< cardDetails
                       }
  (orderShortId, orderTxn)
