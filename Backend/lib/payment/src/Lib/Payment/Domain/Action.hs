...
refundService (request, refundId) merchantId refundsCall = do
  ...
  mapM_ (\Payment.RefundsData {..} -> QRefunds.updateRefundsEntryByResponse initiatedBy (Just idAssignedByServiceProvider) errorMessage errorCode status refundId) response.refunds
  return response
  where
    mkRefundsEntry now =
      Refunds
