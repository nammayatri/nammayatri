...
import Kernel.Tools.Metrics.CoreMetrics (incrementGenericMetrics)
...
isTerminalPayoutOrderStatus status = isPayoutOrderSuccess status || isPayoutStatusFailed status || status `elem` [Payout.ERROR, Payout.CANCELLED, Payout.DISCARDED, Payout.INVALID]

isPayoutOrderFailed :: IPayout.PayoutOrderStatus -> Bool
isPayoutOrderFailed status = isTerminalPayoutOrderStatus status && not (isPayoutOrderSuccess status)
          ...
          then do
            ...
            if nextAttempt >= config.maxAttempts
              then do
                ...
                incrementGenericMetrics "payout_status_check_max_attempts_reached"
