...
updatePostCreateFieldsSafely ::
  ...
  Maybe HighPrecMoney ->
  Maybe Text ->
  Maybe Payout.ExternalPayoutStatus ->
  Maybe Payout.TransferId ->
  m ()
updatePostCreateFieldsSafely orderId status mbResponseCode mbResponseMessage mbAccountDetailsType mbPgBaseFee mbPgGst idAssignedByServiceProvider externalPayoutStatus transferId = do
  now <- getCurrentTime
