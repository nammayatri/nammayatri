...
import Kernel.Beam.Functions (FromTType' (fromTType'), ToTType' (toTType'), createWithKV, findAllWithOptionsKV, findOneWithKV, updateWithKV)
...
findByShortId (ShortId shortId) = findAllWithOptionsKV [Se.Is BeamPO.shortId $ Se.Eq shortId] (Se.Desc $ BeamPO.createdAt) (Just 1) Nothing <&> listToMaybe
...
updateStatus order = do
  now <- getCurrentTime
  updateWithKV [Se.Set BeamPO.status order.status, Se.Set BeamPO.updatedAt now] [Se.Is BeamPO.id $ Se.Eq $ getId order.id]

updateStatusToExpired :: (MonadFlow m) => Id DOrder.PaymentOrder -> m ()
updateStatusToExpired (Id orderId) = do
  now <- getCurrentTime
  updateWithKV [Se.Set BeamPO.updatedAt now] [Se.Is BeamPO.id $ Se.Eq orderId]
