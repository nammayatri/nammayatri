...
instance FromField Payment.MandateType where
  fromField = fromFieldEnum

instance HasSqlValueSyntax be String => HasSqlValueSyntax be Payment.MandateType where
  sqlValueSyntax = autoSqlValueSyntax

instance BeamSqlBackend be => B.HasSqlEqualityCheck be Payment.MandateType

instance FromBackendRow Postgres Payment.MandateType

instance IsString Payment.MandateType where
  fromString = show

deriving stock instance Ord Payment.MandateType
