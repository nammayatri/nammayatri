...
data PayoutOrderE e
    = PayoutOrder {accountDetailsType :: Kernel.Prelude.Maybe Kernel.External.Payout.Juspay.Types.Payout.AccountDetailsType,
...
instance EncryptedItem PayoutOrder
    where type Unencrypted PayoutOrder = (DecryptedPayoutOrder, HashSalt)
          encryptItem (entity, salt) = do {customerEmail_ <- encryptItem (customerEmail entity, salt);
                                           mobileNo_ <- encryptItem (mobileNo entity, salt);
                                           pure PayoutOrder{accountDetailsType = accountDetailsType entity,
                                                            ...
                                                            vpa = vpa entity}}
          decryptItem entity = do {customerEmail_ <- fst <$> decryptItem (customerEmail entity);
                                   ...
                                   pure (PayoutOrder{accountDetailsType = accountDetailsType entity,
                                         ...
                                         "")}
instance EncryptedItem' PayoutOrder
    where type UnencryptedItem PayoutOrder = DecryptedPayoutOrder
