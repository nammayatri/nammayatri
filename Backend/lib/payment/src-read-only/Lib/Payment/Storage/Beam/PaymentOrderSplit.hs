...
data PaymentOrderSplitT f
    = PaymentOrderSplitT {currency :: (B.C f (Kernel.Prelude.Maybe Kernel.Types.Common.Currency)),
...
instance B.Table PaymentOrderSplitT
    where data PrimaryKey PaymentOrderSplitT f = PaymentOrderSplitId (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
