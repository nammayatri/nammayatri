...
data PayoutOrderT f
    = PayoutOrderT {accountDetailsType :: (B.C f (Kernel.Prelude.Maybe Kernel.External.Payout.Juspay.Types.Payout.AccountDetailsType)),
...
instance B.Table PayoutOrderT
    where data PrimaryKey PayoutOrderT f = PayoutOrderId (B.C f Kernel.Prelude.Text) (B.C f Kernel.Prelude.Text) deriving (Generic, B.Beamable)
