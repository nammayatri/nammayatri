...
updatePayoutTransactionStatus :: (Lib.Payment.Storage.Beam.BeamFlow.BeamFlow m r) => (Kernel.Prelude.Text -> m ())
updatePayoutTransactionStatus transactionRef = do _now <- getCurrentTime; updateWithKV [Se.Set Beam.updatedAt _now] [Se.Is Beam.transactionRef $ Se.Eq transactionRef]
...
findByPrimaryKey (Kernel.Types.Id.Id id) transactionRef = do findOneWithKV [Se.And [Se.Is Beam.id $ Se.Eq id, Se.Is Beam.transactionRef $ Se.Eq transactionRef]]
