...
updatePayoutOrderTxnRespInfo :: (Lib.Payment.Storage.Beam.BeamFlow.BeamFlow m r) =>
                                (Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Prelude.Maybe Kernel.Prelude.Text -> Kernel.Prelude.Text -> m ())
updatePayoutOrderTxnRespInfo responseCode responseMessage orderId = do {_now <- getCurrentTime;
...
updateByPrimaryKey (Lib.Payment.Domain.Types.PayoutOrder.PayoutOrder {..}) = do {_now <- getCurrentTime;
                                                                                 updateWithKV [Se.Set Beam.accountDetailsType accountDetailsType,
                                                                                               ...
                                                                                               Se.Set Beam.vpa vpa] [Se.And [Se.Is Beam.id $ Se.Eq (Kernel.Types.Id.getId id), Se.Is Beam.orderId $ Se.Eq orderId]]}
