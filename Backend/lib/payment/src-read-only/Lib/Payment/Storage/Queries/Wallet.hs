...
findAllByPerson :: (Lib.Payment.Storage.Beam.BeamFlow.BeamFlow m r) => (Kernel.Prelude.Text -> m ([Lib.Payment.Domain.Types.Wallet.Wallet]))
findAllByPerson personId = do findAllWithKV [Se.Is Beam.personId $ Se.Eq personId]

findByAccountId :: (Lib.Payment.Storage.Beam.BeamFlow.BeamFlow m r) => (Kernel.Types.Id.Id Lib.Finance.Domain.Types.Account.Account -> m (Maybe Lib.Payment.Domain.Types.Wallet.Wallet))
findByAccountId accountId = do findOneWithKV [Se.Is Beam.accountId $ Se.Eq (Kernel.Types.Id.getId accountId)]

findById :: (Lib.Payment.Storage.Beam.BeamFlow.BeamFlow m r) => (Kernel.Types.Id.Id Lib.Payment.Domain.Types.Wallet.Wallet -> m (Maybe Lib.Payment.Domain.Types.Wallet.Wallet))
