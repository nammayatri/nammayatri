...
findByOfferIdAndEntityIdAndEntityType offerId entityId entityType = do
  findOneWithKV
    [ Se.And
...
instance FromTType' Beam.OfferStats Lib.Payment.Domain.Types.OfferStats.OfferStats where
  fromTType' (Beam.OfferStatsT {..}) = do
    pure $
      Just
...
instance ToTType' Beam.OfferStats Lib.Payment.Domain.Types.OfferStats.OfferStats where
