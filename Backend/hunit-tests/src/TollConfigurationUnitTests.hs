...
-- These functions verify that the expected fields exist on the Toll type.
-- They will fail to compile if a field is removed or renamed.
-- The leading underscore suppresses GHC unused-binding warnings (-Wunused-binds).
-- =============================================================================
...
-- Tests for the filtering logic that excludes disabled tolls.
-- The 'enabled' field will be added to Domain.Types.Toll as part of the
-- Toll Configuration API feature. These tests validate the filtering pattern
-- used in SharedLogic.TollsDetector.getTollInfoOnRoute.
--
-- Once the 'enabled' field is added to the Toll type, the filter in
-- TollsDetector should be: let activeTolls = filter (.enabled) tolls
-- =============================================================================

-- | Helper: simulates the enabled-filter predicate on a list of (enabled, name) pairs.
-- This mirrors `filter (.enabled) tolls` that will be applied in TollsDetector.
...
testEnabledFilterDefaultsTrue =
  testGroup
    ...
    [ testCase "Toll with enabled=True is included in active list" $ do
        -- Simulates: toll created with DEFAULT true => appears in active toll list
...
testMixedEnabledTolls =
  testGroup
    ...
    [ testCase "Route with two tolls one disabled - only enabled appears" $ do
        -- Simulates a route passing through two tolls where toll-2 is disabled
  ...
  testGroup
    ...
    [ testCase "Cache read key and invalidation key must be identical" $ do
        ...
        let cityId = Id.Id "city-for-cache-test" :: Id.Id DMOC.MerchantOperatingCity
            ...
            -- clearTollCache (to be added) will also call makeTollsKeyByMerchantOperatingCityId
            invalidationKey = CQToll.makeTollsKeyByMerchantOperatingCityId cityId
        readKey @?= invalidationKey,
      testCase "Cache key format is stable across calls" $ do
...
-- Validates the logic rules for the UpdateTollReq (to be generated from YAML).
-- These test the business rules: toll must exist, must belong to same city.
-- =============================================================================

testUpdateTollReqValidation :: TestTree
testUpdateTollReqValidation =
  testGroup
    "UpdateTollReq Validation Rules"
    [ testCase "Toggling enabled to False produces False" $ do
        let newEnabled = False
        newEnabled @?= False,
      testCase "Toggling enabled to True produces True" $ do
        let newEnabled = True
        newEnabled @?= True,
      ...
      testCase "Enabled toggle from True to False changes value" $ do
        let before = True
            after = False
        (before /= after) @? "Toggling enabled must change the value"
    ]
