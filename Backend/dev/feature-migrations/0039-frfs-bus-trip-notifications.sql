...
WHERE merchant_operating_city_id = 'da7e19c8-2d86-66ea-039c-edfd8e34ebdc';
...
WHERE moc.id = 'da7e19c8-2d86-66ea-039c-edfd8e34ebdc'
  AND NOT EXISTS (
    ...
    WHERE mpn.key = 'BUS_TRIP_STARTED'
...
WHERE moc.id = 'da7e19c8-2d86-66ea-039c-edfd8e34ebdc'
  AND NOT EXISTS (
    SELECT 1
    FROM atlas_app.merchant_push_notification mpn
    WHERE mpn.key = 'SHUTTLE_TRACKING_ON_START'
      AND mpn.merchant_operating_city_id = moc.id
);

------------------------------------------------------------------------------------------------------
-- WhatsApp: trip started
------------------------------------------------------------------------------------------------------
