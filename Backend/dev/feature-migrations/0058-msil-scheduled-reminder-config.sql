...
-- Companion to 0057 (which genericised the existing go-online copy). Two distinct mechanisms:
--
...
-- GO_ONLINE_REMINDER rows. PRECONDITION: a GO_ONLINE_REMINDER config row (notification_type PN,
-- time_diff=2700 i.e. -45min) must already exist for MSIL_PARTNER — the milestone INSERTs clone from it,
-- so if it's absent they no-op silently. Verify that seed exists before running.
