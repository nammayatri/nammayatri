...
--   * Fleet OperatorPartnerCode (fleet enable)  -> lives in fleet_owner_document_verification_config (no applicable_to)
...
------------------------------------------------------------------------------------------------------
-- E) Driver OperatorPartnerCode (DCO-only enable gate, applicable_to='INDIVIDUAL'). Insert if absent.
...
WHERE dvc.document_type IN ('MedicalCertificate', 'OperatorPartnerCode')
  AND dvc.vehicle_category = 'CAR'
  AND dvc.merchant_id = (SELECT id FROM atlas_driver_offer_bpp.merchant WHERE short_id = 'MSIL_PARTNER');

UPDATE atlas_driver_offer_bpp.document_verification_config dvc
SET applicable_to = 'INDIVIDUAL', updated_at = CURRENT_TIMESTAMP
WHERE dvc.document_type = 'OperatorPartnerCode'
  AND dvc.vehicle_category = 'CAR'
  AND dvc.merchant_id = (SELECT id FROM atlas_driver_offer_bpp.merchant WHERE short_id = 'MSIL_PARTNER');
...
------------------------------------------------------------------------------------------------------
-- F) Fleet OperatorPartnerCode — the fleet `enabled` gate. Lives in fleet_owner_document_verification_config
...
-- Normalise fleet OperatorPartnerCode flags if rows already existed (force is_disabled=false so a
...
SET is_mandatory = false, is_mandatory_for_enabling = true, is_disabled = false, updated_at = CURRENT_TIMESTAMP
WHERE fodvc.document_type = 'OperatorPartnerCode'
