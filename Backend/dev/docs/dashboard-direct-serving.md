...
**Every field below is required in the dhall record.** Config is decoded
strictly at startup (`readDhallConfigDefault`), so an environment whose config
lacks any of them fails to start — update each environment's config before, or
together with, this change. The dashboard database fields may be `None`: the
server then skips registering the dashboard database and behaves exactly as
before, with only the direct-dashboard routes unavailable.

```dhall
, esqDashboardDBCfg = None EsqDBConfig
, esqDashboardDBReplicaCfg = None EsqDBConfig
```

New required config fields per service:

| Service | New fields |
|---|---|
| dynamic-offer-driver-app (22) | `esqDashboardDBCfg`, `esqDashboardDBReplicaCfg`, `dashboardApiRateLimitOptions`, `dataServers`, `authTokenCacheKeyPrefix`, `registrationTokenExpiry`, `registrationTokenInactivityTimeout`, `passwordExpiryDays`, `enforceStrongPasswordPolicy`, `merchantUserAccountNumber`, `updateRestrictedBppRoles`, `loginRateLimitOptions`, `is2faMandatory`, `totpClockSkew`, `totpStepSize`, `twoFaEnforcementDeadlineText`, `twoFaExemptRoles`, `twoFaIssuerName`, `twoFaMaxOtpVerifyAttempts`, `twoFaOtpTTLInSecs`, `exotelToken`, `internalAuthAPIKey` |
| rider-app (20) | the same set without `exotelToken` and `internalAuthAPIKey` |
| provider-dashboard, rider-dashboard (1) | `dashboardApiRateLimitOptions` |

Apart from the dashboard database, these mirror the dashboards' own settings
(login, 2FA, rate limits, data servers), so an environment's values can be
copied from its dashboard config. `dashboardApiRateLimitOptions` is the
per-operator limit on dashboard API calls; dev uses
`{ limit = +300, limitResetTimeInSec = +60 }`.
...
`dev/ddl-migrations/provider-dashboard/0109-grant-app-servers-dashboard-access.sql`
...
The two servers get different grants:

- **rider-app** — read, plus the writes session verification makes: audit rows
  in `transaction`, deleting an expired or revoked `registration_token`, and
  back-filling `person.password_updated_at`.
- **driver-app** — the above, plus full write access, because it also serves
  the login tree, which mutates `registration_token`, `person`, `role`,
  `merchant_access`, `person_capability`, `person_resource_access` and
  `deleted_user`.
