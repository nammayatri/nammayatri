...
-- Backfill for pre-existing rows lives in seed-migrations/safety-dashboard/0002-backfill-person-merchant-id.sql.
ALTER TABLE atlas_safety_dashboard.person
  ADD COLUMN IF NOT EXISTS merchant_id character(36) REFERENCES atlas_safety_dashboard.merchant (id);
