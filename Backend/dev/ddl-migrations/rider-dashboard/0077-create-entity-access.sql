...
-- this has soaked.
--
-- Deploy order: APPLY THIS MIGRATION BEFORE the new rider-dashboard binary ships. The binary
-- SELECTs entity_access; every profile and login read 500s without the table.


CREATE TABLE IF NOT EXISTS atlas_bap_dashboard.entity_access (
...
ALTER TABLE atlas_bap_dashboard.entity_access OWNER TO atlas_bap_dashboard_user;

-- Backs findAllByPersonId / findAllByPersonIds, the read path on every profile and login.
CREATE INDEX IF NOT EXISTS entity_access_person_id_idx ON atlas_bap_dashboard.entity_access (person_id);

-- Backs "who manages this depot"; also the lookup a depot deletion has to check.
CREATE INDEX IF NOT EXISTS entity_access_entity_id_idx ON atlas_bap_dashboard.entity_access (entity_id);
