...
-- The unified provider-dashboard now owns the PT-employee flow (bulkCreate
-- writes token_no_*), so the BPP-side CHECK from 0088 that forced these NULL
-- must go. 0088 added them alongside the constraint, which this drops.
ALTER TABLE atlas_dashboard.person
  DROP CONSTRAINT IF EXISTS person_pt_fields_null_on_bpp;

-- Index (not unique) to back the per-merchant tokenNo conflict lookup, which
-- filters on token_no_hash.
CREATE INDEX IF NOT EXISTS person_token_no_hash_idx ON atlas_dashboard.person (token_no_hash);
