...
Write-Host ""
Write-Host "=== 5. Authenticated business call: serviceability ===" -ForegroundColor Cyan
$svc = Invoke-RestMethod -Uri "$b/v2/serviceability/origin" -Method Post `
    -Headers @{ "content-type" = "application/json"; "token" = $verify.token } `
    -Body '{"location":{"lat":12.9715987,"lon":77.5945627}}'
Write-Host "   serviceable: $($svc.serviceable)" -ForegroundColor Green

Write-Host ""
Write-Host "=== DONE - backend fully operational ===" -ForegroundColor Yellow
Write-Host ""
