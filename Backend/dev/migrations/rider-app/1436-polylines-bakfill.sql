...
(select gen_random_uuid() as id, id as route_id, polyline, now() as created_at, now() as updated_at, merchant_id, merchant_operating_city_id, vehicle_type from atlas_app.route
...
(select gen_random_uuid() as id, id as station_id, address, now() as created_at, now() as updated_at, merchant_id, merchant_operating_city_id from atlas_app.station
    where integrated_bpp_config_id in
    (select id from atlas_app.integrated_bpp_config));
