...
Alter table atlas_driver_offer_bpp.driver_plan add column commodity_data json;
...
Alter table atlas_driver_offer_bpp.plan add column subscribed_flag_toggle_allowed boolean DEFAULT true;
Alter table atlas_driver_offer_bpp.plan add column is_deprecated boolean DEFAULT false;
Alter table atlas_driver_offer_bpp.plan add column eligible_for_coin_discount boolean DEFAULT true;
Alter table atlas_driver_offer_bpp.plan add column merchant_op_city_id Text;
Alter table atlas_driver_offer_bpp.plan add column service_name Text DEFAULT 'YATRI_SUBSCRIPTION';
...
---------- create in subscription config -----

CREATE TABLE IF NOT EXISTS atlas_driver_offer_bpp.subscription_config (
    allow_manual_payment_links boolean DEFAULT false NOT NULL,
    payment_link_channel Text NOT NULL,
    payment_link_job_time bigint NOT NULL,
    generic_job_reschedule_time bigint NOT NULL,
    use_overlay_service boolean DEFAULT true NOT NULL,
    send_deep_link boolean DEFAULT false NOT NULL,
    generic_batch_size_for_jobs int NOT NULL,
    deep_link_expiry_time_in_minutes int,
    max_retry_count int NOT NULL,
    id VARCHAR(36) NOT NULL,
    service_name Text NOT NULL,
    merchant_id Text,
    merchant_operating_city_id Text,
    send_in_app_fcm_notifications boolean DEFAULT false NOT NULL,
    allow_due_addition boolean DEFAULT false NOT NULL,
    payment_service_name Text NOT NULL DEFAULT 'Payment_Juspay',
    created_at timestamp with time zone,
    updated_at timestamp with time zone,
    PRIMARY KEY (merchant_operating_city_id, service_name, id)
);

insert into atlas_driver_offer_bpp.subscription_config (allow_manual_payment_links, payment_link_channel, payment_link_job_time, generic_job_reschedule_time, use_overlay_service, generic_batch_size_for_jobs, deep_link_expiry_time_in_minutes, max_retry_count, id, service_name,send_in_app_fcm_notifications , allow_due_addition, payment_service_name,send_deep_link, merchant_id, merchant_operating_city_id, created_at, updated_at)
select false, 'WHATSAPP', 21600, 60, true, 30, 15, 4,  md5(random()::text || clock_timestamp()::text)::uuid, 'YATRI_SUBSCRIPTION', true, true, 'Payment_Juspay' , false, m.merchant_id, m.id, now(), now()
from atlas_driver_offer_bpp.merchant_operating_city as m;

insert into atlas_driver_offer_bpp.subscription_config (allow_manual_payment_links, payment_link_channel, payment_link_job_time, generic_job_reschedule_time, use_overlay_service, generic_batch_size_for_jobs, deep_link_expiry_time_in_minutes, max_retry_count, id, service_name,send_in_app_fcm_notifications , allow_due_addition, payment_service_name,send_deep_link, merchant_id, merchant_operating_city_id, created_at, updated_at)
select true, 'WHATSAPP', 21600, 60, false, 30, 15, 4,  md5(random()::text || clock_timestamp()::text)::uuid, 'YATRI_RENTAL', false, true, 'RentalPayment_Juspay', false,m.merchant_id, m.id, now(), now()
from atlas_driver_offer_bpp.merchant_operating_city as m;
...
Pay Now - {#var1#}', false, m.id
FROM atlas_driver_offer_bpp.merchant_operating_city as m;

---- Post all changes in the plan table, need to insert the translations for the plans in the plan_translation table. -----


INSERT INTO atlas_driver_offer_bpp.plan (id, merchant_id, payment_mode, frequency, plan_base_amount, name, description, max_amount, registration_amount, is_offer_applicable, max_credit_limit, free_ride_count, plan_type, subscribed_flag_toggle_allowed, is_deprecated, eligible_for_coin_discount, service_name, merchant_op_city_id)
  select  md5(random()::text || clock_timestamp()::text)::uuid, m.merchant_id, 'MANUAL', 'DAILY', 'DAILY_257.0', 'DAILY UNLIMITED' , 'Enjoy UNLIMITED rides, every day!', 257, 1, false, 1285, 0, 'SUBSCRIPTION',false, false, false, 'YATRI_RENTAL', m.id
  from atlas_driver_offer_bpp.merchant_operating_city as m;

INSERT INTO atlas_driver_offer_bpp.plan (id, merchant_id, payment_mode, frequency, plan_base_amount, name, description, max_amount, registration_amount, is_offer_applicable, max_credit_limit, free_ride_count, plan_type, subscribed_flag_toggle_allowed, is_deprecated, eligible_for_coin_discount, service_name, merchant_op_city_id)
  select md5(random()::text || clock_timestamp()::text)::uuid, m.merchant_id,  'AUTOPAY', 'DAILY', 'DAILY_257.0', 'DAILY UNLIMITED' , 'Enjoy UNLIMITED rides, every day!', 257, 1, false, 1285, 0, 'SUBSCRIPTION',false, false, false, 'YATRI_RENTAL', m.id
  from atlas_driver_offer_bpp.merchant_operating_city as m;
