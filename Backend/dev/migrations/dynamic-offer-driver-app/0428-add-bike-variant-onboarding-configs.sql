...
INSERT INTO atlas_driver_offer_bpp.plan
	...
	select md5('6d49ec05-74f8-40a5-b038-503af38763fc'|| m.id) :: uuid, m.merchant_id, 'AUTOPAY', 'SUBSCRIPTION', 'DAILY PER RIDE', 'Unlimited number of rides per day', 5000, 1, 'PERRIDE_2', true, 350, 10, 'DAILY', 0.09, 0.09, 150.0, true, false, true, m.id, 'YATRI_SUBSCRIPTION', 'RIDE', 'BIKE'
	from atlas_driver_offer_bpp.merchant_operating_city as m where city = 'Kolkata';

INSERT INTO atlas_driver_offer_bpp.plan
	(id, merchant_id, payment_mode, plan_type, name, description, max_amount, registration_amount, plan_base_amount, is_offer_applicable, max_credit_limit, free_ride_count, frequency, cgst_percentage, sgst_percentage, max_mandate_amount, subscribed_flag_toggle_allowed, is_deprecated, eligible_for_coin_discount, merchant_op_city_id, service_name, based_on_entity, vehicle_variant)
	select md5('6d49ec05-74f8-40a5-b038-503af38763fc'|| m.id) :: uuid, m.merchant_id, 'MANUAL', 'SUBSCRIPTION', 'DAILY PER RIDE', 'Unlimited number of rides per day', 5000, 1, 'PERRIDE_2', true, 350, 10, 'DAILY', 0.09, 0.09, 150.0, true, false, true, m.id, 'YATRI_SUBSCRIPTION', 'RIDE', 'BIKE'
	from atlas_driver_offer_bpp.merchant_operating_city as m where city = 'Kolkata';
...
(select
 ...
 -- DONT RUN IN MASTER
update atlas_driver_offer_bpp.document_verification_config set is_default_enabled_on_manual_verification = 'true' where document_type = 'AadhaarCard' and merchant_operating_city_id = (select id atlas_driver_offer_bpp.merchant_operating_city where city = 'Kolkata') and vehicle_category = 'MOTORCYCLE'