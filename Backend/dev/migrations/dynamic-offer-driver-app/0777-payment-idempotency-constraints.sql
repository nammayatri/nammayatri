...
BEGIN
  IF EXISTS (
    ...
    CREATE TABLE IF NOT EXISTS atlas_driver_offer_bpp.payment_transaction_duplicate_backup AS
      SELECT * FROM atlas_driver_offer_bpp.payment_transaction WHERE false;

    -- Insert duplicate records (all except the most recent per txn_uuid) into backup
    INSERT INTO atlas_driver_offer_bpp.payment_transaction_duplicate_backup
    SELECT pt.* FROM atlas_driver_offer_bpp.payment_transaction pt
    WHERE pt.id NOT IN (
    ...
    RAISE NOTICE 'Backed up % duplicate rows to payment_transaction_duplicate_backup',
      (SELECT COUNT(*) FROM atlas_driver_offer_bpp.payment_transaction_duplicate_backup);
    ...
    WHERE id IN (SELECT id FROM atlas_driver_offer_bpp.payment_transaction_duplicate_backup);
  END IF;
END $$;
