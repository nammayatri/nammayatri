...
def _seed_kind_for_merchant(short_id, country):
    """Classify a BPP merchant into normal | international | fleet for the
    SeedData panel. International = any city outside India. Fleet = the
    well-known fleet operators (MSIL_PARTNER for now). Everything else is
    Normal (NAMMA_YATRI_PARTNER, BHARAT_TAXI_PARTNER, JATRI_SAATHI_PARTNER, …).
    """
    FLEET_SHORT_IDS = {"MSIL_PARTNER", "LYNX_PARTNER"}
    if short_id in FLEET_SHORT_IDS:
        return "fleet"
...
def get_seed_merchants(kind=None):
    """List BPP merchants grouped/filtered for the SeedData UI.

    Returns rows with `kind` (normal/international/fleet) computed per merchant
    based on its primary country (= country of any of its operating cities;
    we pick MIN(country) as a stable representative).
    """
    rows = query("""
        SELECT m.id            AS merchant_id,
               m.short_id      AS merchant_short_id,
               m.name          AS merchant_name,
               COALESCE(MIN(moc.country), '') AS country
        FROM atlas_driver_offer_bpp.merchant m
        LEFT JOIN atlas_driver_offer_bpp.merchant_operating_city moc
               ON moc.merchant_id = m.id
        GROUP BY m.id, m.short_id, m.name
        ORDER BY m.short_id
    """)
    out = []
    for r in rows:
        r["kind"] = _seed_kind_for_merchant(r.get("merchant_short_id"), r.get("country"))
...
def get_seed_cities(merchant_id):
    """List operating cities for a single BPP merchant."""
    return query("""
        SELECT moc.id      AS city_id,
               moc.city    AS city,
               moc.state   AS state,
               moc.country AS country
        FROM atlas_driver_offer_bpp.merchant_operating_city moc
        WHERE moc.merchant_id = %s
        ORDER BY moc.city
    """, (merchant_id,))


def get_seed_service_tiers(city_id):
    """Vehicle service tiers (with allowed variants) for a (merchant, city).
    Returns rows directly usable as the Vehicle Variant dropdown.
    """
    rows = query("""
        SELECT vst.id,
               vst.service_tier_type,
               vst.name,
               vst.seating_capacity,
               vst.is_air_conditioned,
               vst.priority,
               vst.allowed_vehicle_variant,
               vst.default_for_vehicle_variant,
               vst.vehicle_category
        FROM atlas_driver_offer_bpp.vehicle_service_tier vst
        WHERE vst.merchant_operating_city_id = %s
          AND COALESCE(vst.is_enabled, true) = true
        ORDER BY vst.priority
    """, (city_id,))
    # Flatten tiers into a per-variant list so the UI can offer one option
    # per concrete VehicleVariant. Each variant carries its tier metadata.
    variants = []
    seen = set()
    for r in rows:
        # `allowed_vehicle_variant` is a Postgres text[] → already a Python list.
        for v in (r.get("allowed_vehicle_variant") or []):
            key = v
            if key in seen:
                continue
            seen.add(key)
            variants.append({
                ...
                "service_tier_type": r.get("service_tier_type"),
                "service_tier_name": r.get("name"),
                "seating_capacity": r.get("seating_capacity"),
                "is_air_conditioned": r.get("is_air_conditioned"),
            })
    return {"tiers": rows, "variants": variants}


# Mapping from VehicleVariant → dashboard /addVehicle params. The dashboard
# expects vehicleClass + vehicleCategory + a sane model/make. Mirrors the
# VehicleVariant.castVehicleVariantToVehicleCategory function in
# Backend/lib/beckn-spec/src/Domain/Types/VehicleVariant.hs.
def _vehicle_meta(variant):
    v = (variant or "").upper()
    auto_set = {"AUTO_RICKSHAW", "EV_AUTO_RICKSHAW", "AUTO_PLUS", "AUTO_LITE", "PINK_AUTO"}
    bike_set = {"BIKE", "DELIVERY_BIKE", "BIKE_PLUS"}
    if v in auto_set:
        return {"vehicleClass": "3w",   "vehicleCategory": "AUTO_CATEGORY", "make": "Bajaj",  "model": "AUTO RICKSHAW"}
    if v in bike_set:
        return {"vehicleClass": "2w",   "vehicleCategory": "MOTORCYCLE",    "make": "Honda",  "model": "ACTIVA"}
    # Default: 4-wheeler (CAR variants — SEDAN, SUV, HATCHBACK, TAXI, …)
    return {"vehicleClass": "lmv",  "vehicleCategory": "CAR",           "make": "Maruti", "model": "MARUTI DZIRE"}


def _seed_run_flow(kind, merchant_id, city_id, vehicle_variant):
    """Drive the same auth + addVehicle + enable sequence the AddSeedData
    Postman collection used to perform, but server-side. Returns a dict with
    every step's status + a flat log array suitable for the UI.

    Talks to localhost rider/driver/dashboard directly — assumes the stack
    is up (this endpoint is invoked from the dashboard UI which only renders
    ...
    def step(name, ok, detail=""):
        log.append(f"{'✓' if ok else '✗'} {name}{(' — ' + detail) if detail else ''}")

    # Resolve merchant + city short ids/names from the IDs we got
    merch = query("""
        SELECT short_id, name FROM atlas_driver_offer_bpp.merchant WHERE id = %s
    """, (merchant_id,))
    city_rows = query("""
        SELECT city, country FROM atlas_driver_offer_bpp.merchant_operating_city WHERE id = %s
    """, (city_id,))
    if not merch or not city_rows:
        return {"ok": False, "error": "merchant_id or city_id not found", "log": log}
    merchant_short = merch[0]["short_id"]
    city = city_rows[0]["city"]
    country = (city_rows[0].get("country") or "")
    country_code = "+358" if country.lower() == "finland" else "+91"

    # BAP (rider-app) merchant short id — same name without _PARTNER suffix in dev.
    # Look it up explicitly so we don't have to guess.
    bap = query("""
        SELECT id, short_id FROM atlas_app.merchant
        WHERE driver_offer_merchant_id = %s LIMIT 1
    """, (merchant_id,))
    if not bap:
        # Fallback: strip _PARTNER suffix
        bap_short = merchant_short.replace("_PARTNER", "")
    else:
        bap_short = bap[0]["short_id"]

    # Random per-run identifiers
    driver_phone = "9" + str(random.randint(100000000, 999999999))
    rider_phone  = "8" + str(random.randint(100000000, 999999999))
    state_prefix = "FI" if country.lower() == "finland" else "KA"
    reg_no = f"{state_prefix}{random.randint(10,99)}AB{random.randint(1000, 9999)}"
    meta = _vehicle_meta(vehicle_variant)

    DRIVER = "http://localhost:8016/ui"
    RIDER  = "http://localhost:8013/v2"
    DASH   = "http://localhost:8018"
    DASH_INTERNAL = "http://localhost:8018/bpp/driver-offer"
    DASHBOARD_TOKEN = "local-admin-token-bangalore-namma-yatri"

    def post(url, payload, headers=None, timeout=15):
        h = {"Content-Type": "application/json"}
        if headers: h.update(headers)
        req = urllib.request.Request(url,
            data=json.dumps(payload).encode("utf-8"), headers=h, method="POST")
        try:
    ...
    # --- 1. Customer auth + OTP ---
    s, r = post(f"{RIDER}/auth", {"mobileNumber": rider_phone, "mobileCountryCode": country_code, "merchantId": bap_short})
    if s != 200:
        step("Customer Auth", False, f"HTTP {s}: {r}"); return {"ok": False, "log": log}
    rider_auth_id = r.get("authId")
    step("Customer Auth", True, f"authId={rider_auth_id}")
    s, r = post(f"{RIDER}/auth/{rider_auth_id}/verify", {"otp": "7891", "deviceToken": "seed-rider"})
    if s != 200:
        step("Customer OTP", False, f"HTTP {s}"); return {"ok": False, "log": log}
    step("Customer OTP", True, "verified")

    # --- 2. Driver auth + OTP ---
    s, r = post(f"{DRIVER}/auth", {"mobileNumber": driver_phone, "mobileCountryCode": country_code,
                                   "merchantId": merchant_id, "merchantOperatingCity": city})
    if s != 200:
        step("Driver Auth", False, f"HTTP {s}: {r}"); return {"ok": False, "log": log}
    driver_auth_id = r.get("authId")
    step("Driver Auth", True, f"authId={driver_auth_id}")
    s, r = post(f"{DRIVER}/auth/{driver_auth_id}/verify", {"otp": "7891", "deviceToken": "seed-driver"})
    if s != 200:
        step("Driver OTP", False, f"HTTP {s}"); return {"ok": False, "log": log}
    driver_id = (r.get("person") or {}).get("id")
    step("Driver OTP", True, f"driver_id={driver_id}")

    # --- 3. Switch dashboard merchant + city (gives us a fresh dashboard token) ---
    s, r = post(f"{DASH}/user/switchMerchantAndCity",
                {"merchantId": merchant_short, "city": city},
                headers={"token": DASHBOARD_TOKEN})
    if s != 200:
        step("Switch City (Dashboard)", False, f"HTTP {s}: {r}"); return {"ok": False, "log": log}
    new_token = r.get("authToken") or DASHBOARD_TOKEN
    step("Switch City (Dashboard)", True, f"merchant={merchant_short} city={city}")

    # --- 4. Add vehicle ---
    add_v_url = f"{DASH_INTERNAL}/{merchant_short}/{city}/driver/{driver_id}/addVehicle"
    s, r = post(add_v_url, {
    ...
    if s != 200:
        step("Add Vehicle (Dashboard)", False, f"HTTP {s}: {r}"); return {"ok": False, "log": log}
    step("Add Vehicle (Dashboard)", True, f"reg={reg_no} variant={vehicle_variant}")

    # --- 5. Enable driver ---
    enable_url = f"{DASH_INTERNAL}/{merchant_short}/{city}/driver/{driver_id}/enable"
    s, r = post(enable_url, {}, headers={"token": new_token})
    if s != 200:
        step("Enable Driver (Dashboard)", False, f"HTTP {s}: {r}"); return {"ok": False, "log": log}
    step("Enable Driver (Dashboard)", True)

    return {
        "ok": True,
        "kind": kind,
        "merchant": {"id": merchant_id, "short_id": merchant_short, "bap_short_id": bap_short},
        "city": {"id": city_id, "name": city, "country": country},
        "driver": {"id": driver_id, "phone": driver_phone, "registrationNo": reg_no, "variant": vehicle_variant},
...
class ContextHandler(BaseHTTPRequestHandler):
    ...
    def _handle(self, method):
        ...
        if method == "POST" and path == "/api/seed/run":
            body = self._read_json_body()
            kind = body.get("kind") or "normal"
            merchant_id = body.get("merchantId")
            city_id = body.get("cityId")
            vehicle_variant = body.get("vehicleVariant")
            if not (merchant_id and city_id and vehicle_variant):
                self._send_json({"error": "merchantId, cityId, vehicleVariant are required"}, 400)
                return True
            try:
                result = _seed_run_flow(kind, merchant_id, city_id, vehicle_variant)
        ...
        elif path == "/api/seed/merchants":
            from urllib.parse import parse_qs
            qs = parse_qs(parsed.query)
            kind = qs.get("kind", [None])[0]
            self._send_json(get_seed_merchants(kind))
        elif path == "/api/seed/cities":
            from urllib.parse import parse_qs
            qs = parse_qs(parsed.query)
            mid = qs.get("merchant_id", [None])[0]
            if not mid:
                self._send_json({"error": "merchant_id query param required"}, 400)
            else:
                self._send_json(get_seed_cities(mid))
        elif path == "/api/seed/service-tiers":
            from urllib.parse import parse_qs
            qs = parse_qs(parsed.query)
            cid = qs.get("city_id", [None])[0]
            if not cid:
                self._send_json({"error": "city_id query param required"}, 400)
            else:
                self._send_json(get_seed_service_tiers(cid))
