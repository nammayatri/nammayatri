-- unencrypted: email: juspay_admin@dashboard.com, password: juspay_admin, mobile 7777777780
...
-- unencrypted: email: juspay_admin2@dashboard.com, password: juspay_admin2, mobile 7777777781
INSERT INTO atlas_dashboard.person (id, first_name, last_name, role_id, email_encrypted, email_hash, mobile_number_encrypted, mobile_number_hash, mobile_country_code, password_hash, created_at, updated_at) VALUES
	('juspay-admin-2-000000000000000000000', 'juspay_admin2', 'juspay_admin2', '37947162-3b5d-4ed6-bcac-08841be1534d', '0.1.0|1|deo+W2XBGgqaxBjG5dLpnDdafOVlB0e2YIsiHf8NDXYb939XCjo7y/L+Ve0pqHhx8yZZ/6EKcaDezGYLL9pBam0KnNI/fC6y8XW/A+xh', '\xc4cee68ee6ec7896ad8fb4ff1aab47f85e4831a5728f9dde1e263bf8a4523c43', '0.1.0|2|/1W1OzdDn8ly4q6Pb/IDAUOP0btDruGpKN6DiIXYSWB/mrZ8DB9IxGno/OBk2lfMvMbotmf3d3l2JBMjog==', '\xbb63874220d75c23f6b49a63e2c0232408d0dc49c0e1c1bd3c004400488f0db5', '+91', '\x64b674638686daefeadf13d7560b83177625446900786d1f46e43770998a27fd', '2022-09-06 11:25:42.609155+00', '2022-09-06 11:25:42.609155+00')
ON CONFLICT DO NOTHING;

INSERT INTO atlas_dashboard.registration_token (id, token, person_id, merchant_id, operating_city, enabled, created_at)
SELECT
...
INSERT INTO atlas_dashboard.registration_token (id, token, person_id, merchant_id, operating_city, enabled, created_at)
SELECT
    'local-admin2-token-blr-id-0000000000',
    'local-admin2-token-bangalore-namma-yatri',
    'juspay-admin-2-000000000000000000000',
    m.id,
    'Bangalore',
    true,
    now()
...
-- Grant the JUSPAY_ADMIN2 dev token cross-merchant + cross-city dashboard access.
DO $$
DECLARE
    admin2_person_id TEXT := 'juspay-admin-2-000000000000000000000';
BEGIN
    INSERT INTO atlas_dashboard.merchant_access
        (id, person_id, merchant_id, merchant_short_id, operating_city, created_at)
    SELECT gen_random_uuid()::text,
    ...
    WHERE NOT EXISTS (
        SELECT 1 FROM atlas_dashboard.merchant_access ma
        WHERE ma.person_id   = admin2_person_id
          AND ma.merchant_id = m.id
          AND ma.operating_city::text = c.city
    );
END $$;