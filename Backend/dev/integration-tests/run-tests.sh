...
INCENTIVE_DIR="$SCRIPT_DIR/collections/IncentiveJourneyFlow"
...
# Generated by NammaDSL — capability_endpoint + local role_capability for IncentiveJourney peer APIs.
INCENTIVE_CAPABILITY_SQL="$SCRIPT_DIR/../migrations-read-only/provider-dashboard/API_IncentiveJourney_IncentiveJourney.sql"
INCENTIVE_LOCAL_ACCESS_SQL="$SCRIPT_DIR/../migrations-read-only/provider-dashboard/Local_API_IncentiveJourney_IncentiveJourney.sql"
...
# Incentive journey (provider): token + merchant_access, then IJ capability_endpoint + local role grants.
# Coins capabilities themselves come from seed-migrations; these two SQL files map IJ endpoints and
# grant system-config.coins.read/write to the local admin role (ON CONFLICT DO NOTHING — safe to re-run).
seed_incentive_dashboard_access() {
    if [ "${NY_TEST_SKIP_INCENTIVE_SEED:-}" = "1" ]; then
        echo "Skipping incentive-journey dashboard seed (NY_TEST_SKIP_INCENTIVE_SEED=1)"
        return 0
    fi
    if [ ! -f "$PROVIDER_DASHBOARD_SEED_SQL" ]; then
        echo "WARNING: provider-dashboard.sql not found: $PROVIDER_DASHBOARD_SEED_SQL"
        return 0
    fi
    echo "Seeding provider-dashboard local-testing-data (token, merchant_access) for incentive-journey..."
    if ! psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER_SUPER" -d "$DB_NAME" \
        -f "$PROVIDER_DASHBOARD_SEED_SQL" > /dev/null 2>&1; then
        echo "WARNING: provider-dashboard.sql seed failed (postgres on $DB_HOST:$DB_PORT?)"
        echo "  Manual: psql -h $DB_HOST -p $DB_PORT -U $DB_USER_SUPER -d $DB_NAME -f $PROVIDER_DASHBOARD_SEED_SQL"
        return 0
    fi
    for sql in "$INCENTIVE_CAPABILITY_SQL" "$INCENTIVE_LOCAL_ACCESS_SQL"; do
        if [ ! -f "$sql" ]; then
            echo "WARNING: IncentiveJourney capability SQL not found: $sql"
            echo "  Run , run-generator then re-seed, or apply manually once DB is up."
            continue
        fi
        echo "Applying $(basename "$sql")..."
        if ! psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER_SUPER" -d "$DB_NAME" \
            -f "$sql" > /dev/null 2>&1; then
            echo "WARNING: failed to apply $sql (postgres on $DB_HOST:$DB_PORT?)"
            echo "  Manual: psql -h $DB_HOST -p $DB_PORT -U $DB_USER_SUPER -d $DB_NAME -f $sql"
        fi
    done
}
...
setup() {
    ...
    echo "       or: ./run-tests.sh incentive-journey NY_Bangalore"
}
...
run_incentive() {
    seed_incentive_dashboard_access
    run_frfs "$INCENTIVE_DIR" "INCENTIVE JOURNEY" "${1:-}" "${2:-}"
}
...
show_help() {
    ...
    echo "  incentive|incentive-journey  Run provider incentive-journey E2E (needs kafka-consumers / DriverCoinsAndJourney)"
    ...
    echo "  ./run-tests.sh incentive-journey NY_Bangalore # Incentive journey + AUTO ride (Bangalore)"
