...
2. **Test dashboard** — invokes newman directly and never runs the seed. Apply the SQL by
   hand first, then flush Redis and restart `rider-app-exe` yourself (or add
   `dimension_overrides` entries to `dev/config-sync/assets/patches.json` if the suite should
   survive config-sync imports).
3. **Raw newman** — apply the SQL, flush Redis, and restart `rider-app-exe` manually first.

Only path 1 restarts rider-app for you. On paths 2 and 3 a rider-app that is already running
keeps serving the pre-seed provider list, and the Firebase branch never runs — see below.
