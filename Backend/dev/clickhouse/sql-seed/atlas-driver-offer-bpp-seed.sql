...
-- ###########################################################################
-- Association change history (append-only)
-- ---------------------------------------------------------------------------
-- These tables retain the FULL timeline of every association create/update.
-- Unlike the *_association current-state tables (ReplacingMergeTree, which keep
-- only the latest version per key), these use plain MergeTree ordered by
-- (id, version) so every state transition is kept. They are fed by a SECOND
-- materialized view off the SAME *_queue Kafka table -- ClickHouse delivers
-- every Kafka block to all MVs attached to the Kafka-engine table.
--
-- IMPORTANT: the drainer pushes CREATE and UPDATE to Kafka but NOT hard DELETE.
-- So an association "end" must be emitted as an UPDATE (is_active=false /
-- associated_till=now) before the Postgres row is deleted; that UPDATE is what
-- lands here as the terminal event in the timeline.
-- ###########################################################################

-- driver_operator_association history ---------------------------------------
create table atlas_driver_offer_bpp.driver_operator_association_history (
    `id` String,
    `driver_id` String,
    `operator_id` String,
    `is_active` Boolean,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS atlas_driver_offer_bpp.driver_operator_association_history_mv
TO atlas_driver_offer_bpp.driver_operator_association_history AS
SELECT id, driver_id, operator_id, is_active, associated_on, coalesce(updated_at, now()) AS version
FROM atlas_driver_offer_bpp.driver_operator_association_queue;

-- fleet_driver_association history -------------------------------------------
create table atlas_driver_offer_bpp.fleet_driver_association_history (
    `id` String,
    `driver_id` String,
    `fleet_owner_id` String,
    `is_active` Boolean,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS atlas_driver_offer_bpp.fleet_driver_association_history_mv
TO atlas_driver_offer_bpp.fleet_driver_association_history AS
SELECT id, driver_id, fleet_owner_id, is_active, associated_on, coalesce(updated_at, now()) AS version
FROM atlas_driver_offer_bpp.fleet_driver_association_queue;

-- fleet_operator_association history -----------------------------------------
create table atlas_driver_offer_bpp.fleet_operator_association_history (
    `id` String,
    `fleet_owner_id` String,
    `operator_id` String,
    `is_active` Boolean,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS atlas_driver_offer_bpp.fleet_operator_association_history_mv
TO atlas_driver_offer_bpp.fleet_operator_association_history AS
SELECT id, fleet_owner_id, operator_id, is_active, updated_at AS version
FROM atlas_driver_offer_bpp.fleet_operator_association_queue;

-- ###########################################################################
-- fleet_rc_association: full pipeline (was absent from ClickHouse)
-- current-state (ReplacingMergeTree) + append-only history (MergeTree),
-- both fed from the adob-sessionizer-fleet_rc_association Kafka topic.
-- NOTE: this table has no is_rc_active column (dropped); the "ended" signal is
-- associated_till being set to the change time (an active link has
-- associated_till in the future, e.g. 2099).
-- ###########################################################################
create table atlas_driver_offer_bpp.fleet_rc_association (
    ...
    `rc_id` String,
    `associated_on` DateTime,
    `associated_till` Nullable(DateTime),
    `merchant_operating_city_id` Nullable(String),
    `version` DateTime DEFAULT now()
) ENGINE = ReplacingMergeTree(version)
ORDER BY (fleet_owner_id, rc_id);

CREATE TABLE IF NOT EXISTS atlas_driver_offer_bpp.fleet_rc_association_queue (
    `id` String,
    `fleet_owner_id` String,
    `rc_id` String,
    `associated_on` DateTime,
...
SETTINGS kafka_broker_list='localhost:29092', kafka_topic_list='adob-sessionizer-fleet_rc_association', kafka_group_name='ch-atlas_driver_offer_bpp-fleet_rc_association', kafka_format='JSONEachRow', kafka_num_consumers=1, kafka_skip_broken_messages=100;

CREATE MATERIALIZED VIEW IF NOT EXISTS atlas_driver_offer_bpp.fleet_rc_association_mv
TO atlas_driver_offer_bpp.fleet_rc_association AS
SELECT id, fleet_owner_id, rc_id, associated_on, associated_till, merchant_operating_city_id, coalesce(updated_at, now()) AS version
FROM atlas_driver_offer_bpp.fleet_rc_association_queue;

create table atlas_driver_offer_bpp.fleet_rc_association_history (
    `id` String,
    `fleet_owner_id` String,
    `rc_id` String,
    `associated_on` DateTime,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS atlas_driver_offer_bpp.fleet_rc_association_history_mv
TO atlas_driver_offer_bpp.fleet_rc_association_history AS
SELECT id, fleet_owner_id, rc_id, associated_on, associated_till, merchant_operating_city_id, coalesce(updated_at, now()) AS version
FROM atlas_driver_offer_bpp.fleet_rc_association_queue;
