...
-- atlas_dashboard is substituted per dashboard side pre-merge; tables converge into
...
CREATE TABLE atlas_dashboard.capability (
...
ALTER TABLE atlas_dashboard.capability OWNER TO atlas_dashboard_user;

CREATE TABLE atlas_dashboard.capability_endpoint (
capability_id character varying(255) NOT NULL REFERENCES atlas_dashboard.capability (id),
...
CREATE TABLE atlas_dashboard.role_capability (
role_id character(36) NOT NULL REFERENCES atlas_dashboard.role (id),
capability_id character varying(255) NOT NULL REFERENCES atlas_dashboard.capability (id),
created_at timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL
,CONSTRAINT role_capability_pkey PRIMARY KEY (role_id, capability_id)
);
ALTER TABLE atlas_dashboard.role_capability OWNER TO atlas_dashboard_user;

CREATE TABLE atlas_dashboard.person_capability (
...
granted_by character(36) REFERENCES atlas_dashboard.person (id),
...
ALTER TABLE atlas_dashboard.person_capability OWNER TO atlas_dashboard_user;

CREATE TABLE atlas_dashboard.access_audit (
...
ALTER TABLE atlas_dashboard.access_audit OWNER TO atlas_dashboard_user;
CREATE INDEX idx_access_audit_target ON atlas_dashboard.access_audit (target_type, target_id);

ALTER TABLE atlas_dashboard.person ADD COLUMN admin_tier character varying(32) NOT NULL DEFAULT 'USER';
ALTER TABLE atlas_dashboard.person ADD CONSTRAINT person_admin_tier_check
