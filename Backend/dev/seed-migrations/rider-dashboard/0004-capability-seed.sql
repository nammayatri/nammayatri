...
DELETE FROM atlas_bap_dashboard.role_capability;
DELETE FROM atlas_bap_dashboard.capability_endpoint;
DELETE FROM atlas_bap_dashboard.capability
 WHERE id NOT IN (SELECT capability_id FROM atlas_bap_dashboard.person_capability);
...
-- FROM atlas_bap_dashboard.person_capability pc
-- LEFT JOIN atlas_bap_dashboard.capability c ON c.id = pc.capability_id
-- WHERE c.id IS NULL;
...
DELETE FROM atlas_bap_dashboard.capability_endpoint WHERE capability_id IN
    (SELECT id FROM atlas_bap_dashboard.capability WHERE domain IN ('ops', 'agent', 'transit', 'config', 'city', 'tickets', 'system', 'utility', 'access'))
    OR capability_id IN ('analytics.incentive.read');
DELETE FROM atlas_bap_dashboard.role_capability WHERE capability_id IN
    (SELECT id FROM atlas_bap_dashboard.capability WHERE domain IN ('ops', 'agent', 'transit', 'config', 'city', 'tickets', 'system', 'utility', 'access'))
    OR capability_id IN ('analytics.incentive.read');
DELETE FROM atlas_bap_dashboard.person_capability WHERE capability_id IN
    (SELECT id FROM atlas_bap_dashboard.capability WHERE domain IN ('ops', 'agent', 'transit', 'config', 'city', 'tickets', 'system', 'utility', 'access'))
    OR capability_id IN ('analytics.incentive.read');
DELETE FROM atlas_bap_dashboard.capability WHERE domain IN ('ops', 'agent', 'transit', 'config', 'city', 'tickets', 'system', 'utility', 'access') OR id IN ('analytics.incentive.read');
...
INSERT INTO atlas_bap_dashboard.capability_endpoint (capability_id, server_name, endpoint_id) VALUES
    ...
    ('admin.user.write', 'DASHBOARD', 'DASHBOARD_USER_BULK_CREATE'),
...
WHERE NOT EXISTS (SELECT 1 FROM atlas_bap_dashboard.role WHERE name = 'STUDENT_PASS_DEPOT');
INSERT INTO atlas_bap_dashboard.role (id, name, dashboard_access_type, description, created_at, updated_at)
SELECT '96f0598c-c596-536a-8842-31a964ac4e01', 'PT_CONDUCTOR', 'DASHBOARD_USER', 'PT_CONDUCTOR (created by capability seed)', now(), now()
WHERE NOT EXISTS (SELECT 1 FROM atlas_bap_dashboard.role WHERE name = 'PT_CONDUCTOR');
INSERT INTO atlas_bap_dashboard.role (id, name, dashboard_access_type, description, created_at, updated_at)
...
WHERE NOT EXISTS (SELECT 1 FROM atlas_bap_dashboard.role WHERE name = 'PT_DEPOT_MANAGER');
INSERT INTO atlas_bap_dashboard.role (id, name, dashboard_access_type, description, created_at, updated_at)
...
WHERE NOT EXISTS (SELECT 1 FROM atlas_bap_dashboard.role WHERE name = 'TICKET_VALIDATOR');
...
FROM (
  ...
  JOIN atlas_bap_dashboard.capability_endpoint ce
...
JOIN (
  ...
  FROM atlas_bap_dashboard.capability_endpoint GROUP BY capability_id
...
-- JOIN atlas_bap_dashboard.role r ON r.id = held.role_id
-- WHERE held.held_endpoints::numeric / sized.total_endpoints < 0.5
-- ORDER BY pct DESC;
...
DELETE FROM atlas_bap_dashboard.role_capability rc
USING atlas_bap_dashboard.role r
