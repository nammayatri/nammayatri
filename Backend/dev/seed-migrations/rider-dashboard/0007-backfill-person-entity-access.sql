...
-- longer exists.
--
-- Run AFTER ddl-migrations/rider-dashboard/0077-create-entity-access.sql creates the table, and BEFORE the new
-- binary serves traffic -- until this runs, every existing depot manager reads as having none.

INSERT INTO atlas_bap_dashboard.entity_access (id, person_id, entity_id, merchant_id, created_at)
SELECT md5(random()::text || p.id || e.id)::uuid::text, p.id, e.id, e.merchant_id, CURRENT_TIMESTAMP
FROM atlas_bap_dashboard.person p
JOIN atlas_bap_dashboard.entity e ON e.id = p.entity_id
