-- Backfill person.merchant_id (added in ddl-migrations/safety-dashboard/0012) from existing
...
UPDATE atlas_safety_dashboard.person p
SET merchant_id = sub.merchant_id
FROM (
  SELECT person_id, MIN(merchant_id) AS merchant_id
  FROM atlas_safety_dashboard.merchant_access
  GROUP BY person_id
) sub
