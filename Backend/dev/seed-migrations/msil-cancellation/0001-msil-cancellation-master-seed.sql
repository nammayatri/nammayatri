...
BEGIN
  ...
  -- Carry-forward is derived from the matrix row's collectionMode (ImmediateCapture
  -- => no accrual), prepaid-balance debits are a driverDeduction variant, and the three
  -- thresholds are literals inside the fault rule — all seeded by 0003.
