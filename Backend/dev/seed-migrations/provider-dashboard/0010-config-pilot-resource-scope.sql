-- Layer C for ConfigPilot: scope an operator to the individual configs they may
-- read or change, on top of the existing system-config.config_pilot.*
-- capabilities.

BEGIN;

-- 1. The ConfigPilot capabilities gain the CONFIG resource dimension.
UPDATE atlas_dashboard.capability
   SET resource_type = 'CONFIG'
 WHERE id IN ('system-config.config_pilot.read', 'system-config.config_pilot.write');

-- 2. The two ConfigPilot-specific rollout endpoints. They are the config-scoped
--    twins of /appDynamicLogic/verify and /appDynamicLogic/upsertLogicRollout,
--    and they sit under config_pilot.write rather than dynamic_logic.write.
...
-- 3. Where each endpoint carries the config.

UPDATE atlas_dashboard.capability_endpoint ce
...
--     FROM atlas_dashboard.capability_endpoint ce
