...
#### 2.8a Excessive Debug Logging in Hot Path [Reviewer-Added]
- **File**: `app/provider-platform/dynamic-offer-driver-app/Main/src/SharedLogic/DriverPool.hs` (17 locations: lines 583,611,622,648,810,1092,1110-1113,1196,1247)
- **Issue**: `logDebug $ ... <> show driverPool` evaluates `show` on large pool data structures regardless of log level
- **Impact**: Unnecessary string allocation on every search request
- **Fix**: Gate behind log-level check or use lazy logging

#### 2.8b DSL-Generated Unbounded Queries are Structural [Reviewer-Added]
- 137 unbounded `findAllWithKV` queries in driver-app are generated by NammaDSL (`src-read-only/`)
- Fixing requires changes to the DSL generator itself or manual overrides in `*Extra.hs` files
- **Recommendation**: Modify NammaDSL to require explicit limits or add configurable defaults
